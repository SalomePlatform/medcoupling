...
def findNodesToDuplicate(this, otherDimM1OnSameCoords):
    ...
    meshM2, _, _, _, rdit0 = (
        otherDimM1OnSameCoords.buildDescendingConnectivity()
    )  # 2D: a mesh of points, 3D: a mesh of segs
    dsi = rdit0.deltaShiftIndex()
    idsTmp0 = dsi.findIdsNotInRange(
        -1, 3
    ...
    if this.getMeshDimension() == 3:
        m0descSkinDesc, _, _, _, _ = (
            m0descSkin.buildDescendingConnectivity()
        ...
        if validIds.getNumberOfTuples():
            ...
            meshM2Desc, _, _, _, rdit0 = (
                meshM2.buildDescendingConnectivity()
            )  # a mesh made of node cells
            dsi = rdit0.deltaShiftIndex()
            singPoints = dsi.findIdsNotInRange(
                -1, 4
            )  # points connected to (strictly) more than 3 segments
            if singPoints.getNumberOfTuples():
                ...
                cellsAroundBN = this.getCellIdsLyingOnNodes(
                    boundNodes, False
                )  # false= take cell in, even if not all nodes are in dupl
                mAroundBN = this[cellsAroundBN]
                mAroundBNDesc, descBN, descIBN, revDescBN, revDescIBN = (
                    mAroundBN.buildDescendingConnectivity()
                )
                #    2. Identify cells in sub-mesh mAroundBN which have a face in common with M1
                _, idsOfM1BN = mAroundBNDesc.areCellsIncludedIn(
                    otherDimM1OnSameCoords, 2
                )
                nCells, nCellsDesc = (
                    mAroundBN.getNumberOfCells(),
                    mAroundBNDesc.getNumberOfCells(),
                )
                idsTouch = DataArrayInt.New()
                idsTouch.alloc(0, 1)
                for v in idsOfM1BN:
                ...
                fNodes1 = fNodes1.buildSubstraction(
                    inters
                )  # reminder: fNodes1 represent nodes that need dupl.
...
def findCellsToRenumber(this, otherDimM1OnSameCoords, dupl):
    ...
    cellsAroundGroupLarge = this.getCellIdsLyingOnNodes(
        dupl, False
    )  # false= take cell in, even if not all nodes are in dupl
    #
    mAroundGrpLarge = this[cellsAroundGroupLarge]
    mArGrpLargeDesc, descL, descIL, revDescL, revDescIL = (
        mAroundGrpLarge.buildDescendingConnectivity()
    )
    ...
    nCells, nCellsLarge = (
        cellsAroundGroup.getNumberOfTuples(),
        cellsAroundGroupLarge.getNumberOfTuples(),
    )
    mArGrpDesc, desc, descI, revDesc, revDescI = (
        mAroundGrp.buildDescendingConnectivity()
    )
    _, idsOfM1 = mArGrpDesc.areCellsIncludedIn(
        otherDimM1OnSameCoords, 2
    )  # TODO : could we avoid recomputing this??
    ...
    hitCells = DataArrayInt.New()
    hitCells.alloc(nCells)
    hitCells.fillWithValue(
        0
    )  # 0 : not hit, -1: one side of the crack, +1: other side of the crack
    ...
    while nIter < nIterMax:
        ...
        while not done and nIter < nIterMax:  # Start of the ping-pong
            ...
            spreadZone, _ = MEDCouplingUMesh.ComputeSpreadZoneGraduallyFromSeed(
                [seed], neigh, neighI, -1
            )
            done = True
    ...
    hitCellsLarge = DataArrayInt.New()
    hitCellsLarge.alloc(nCellsLarge)
    ...
    neighL, neighIL = MEDCouplingUMesh.ComputeNeighborsOfCellsAdv(
        descL, descIL, revDescL, revDescIL
    )
    for c in nonHitCells:
        ...
        for n in neighs:
            ...
            if (
                neighVal != 0 and abs(neighVal) < MAX_CP
            ):  # (@test_T0) second part of the test to skip cells being assigned and target only cells assigned in the first part of the algo above
                ...
                if currVal != 0:  # Several neighbors have a candidate number
                    ...
                    hitCellsLarge[c[0], 0] = (
                        MAX_CP * neighVal
                    )  # Same sign, but different value to preserve PING_FULL and PONG_FULL
    ...
    cellsToModifyConn0_torenum = hitCellsLarge.findIdsInRange(
        1, MAX_CP * MAX_CP
    )  # Positive spread zone number
    cellsToModifyConn1_torenum = hitCellsLarge.findIdsInRange(
        -MAX_CP * MAX_CP, 0
    )  # Negative spread zone number
    if (
        cellsToModifyConn0_torenum.getNumberOfTuples()
        + cellsToModifyConn1_torenum.getNumberOfTuples()
        != cellsAroundGroupLarge.getNumberOfTuples()
    ):
