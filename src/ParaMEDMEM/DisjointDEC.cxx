...
void
DisjointDEC::copyInstance(const DisjointDEC &other)
{
    DEC::copyFrom(other);
...
DisjointDEC::DisjointDEC(const std::set<int> &source_ids, const std::set<int> &target_ids, const MPI_Comm &world_comm)
    : _local_field(0), _comm_interface(0), _owns_field(false), _owns_groups(true), _union_comm(MPI_COMM_NULL)
{
    ...
    if (union_ids.size() != (source_ids.size() + target_ids.size()))
        throw INTERP_KERNEL::Exception(
            "DisjointDEC constructor : source_ids and target_ids overlap partially or fully. This type of DEC does not "
...
DisjointDEC::~DisjointDEC() { cleanInstance(); }
 ...
 */
void
DisjointDEC::checkPartitionGroup() const
{
    int size = -1;
    ...
    if (comm_t != comm_s)
        throw INTERP_KERNEL::Exception(
            "DisjointDEC constructor: Inconsistent world communicator when building DisjointDEC"
        );
    ...
    if ((int)union_ids.size() != size)
        throw INTERP_KERNEL::Exception(
            "DisjointDEC constructor: source_ids and target_ids do not form a partition of the communicator! Restrain "
...
bool
DisjointDEC::isInTargetSide() const
{
    if (!_target_group)
        return false;
