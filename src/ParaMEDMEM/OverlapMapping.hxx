...
class OverlapMapping
{
   public:
    ...
    void addContributionST(
        const vector<SparseDoubleVec> &matrixST,
   ...
   private:
    ...
    mcIdType serializeMatrixStep1ST(
        const mcIdType *nbOfElemsSrc,
    ...
    void unserializationST(
        mcIdType nbOfTrgElems,
        const mcIdType *nbOfElemsSrcPerProc,
   ...
   private:
    ...
    /**! Number of received source mesh IDs at mesh data exchange.
     Counting the number of IDs suffices, as we just need this to prepare the receive side, when doing the final vector
     matrix multiplication. First dimension is the remote proc ID from which we received. */
     ...
     * This information is stored from the *locally* COMPuted matrices, and corresponds hence to field value that will
     * need to sent later on, if this matrix bit itself is sent aways.  */
     ...
     * This matrix is filled after receival from other procs, contrary to _matrixes_st which contains local
     * computations.*/
    ...
    // Denominators (computed from the numerator matrix). As for _the_matrix_st it is paired with
    // _the_matrix_st_source_proc_id
