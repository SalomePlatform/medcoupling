...
class OverlapElementLocator : public INTERP_KERNEL::InterpolationOptions
{
   public:
    ...
    std::vector<int> getProcsToSendFieldData() const
    {
        return _procs_to_send_field;
    }  // same set as the set of procs we sent mesh data to
   ...
   private:
    ...
    typedef std::pair<int, bool> Proc_SrcOrTgt;  ///< a key indicating a proc ID and whether the data is for source
                                                 ///< mesh/field or target mesh/field
    ...
    //! list of interpolation couples to be done by this proc only. This is a simple extraction of the member above
    //! _all_todo_lists
     ...
     * This can be different from _procs_to_send_mesh (restricted to Source) because interpolation matrix bits are
     * computed on a potentially different proc than the target one.   */
