    ...
    : ProcessorGroup(interface), _world_comm(MPI_COMM_WORLD)
{
    ...
    for (int i = 0; i < size; i++) _proc_ids.insert(i);
}
...
/*! Creates a processor group that is based on the processors included in \a proc_ids_by_name[name].
  ...
  \param interface CommInterface object giving access to the MPI
  communication layer
  \param proc_ids_by_name a map defining a relation between a name and a set of ids that are to be integrated in the
  group. The ids number are to be understood in terms of MPI_COMM_WORLD ranks.
...
*/
MPIProcessorGroup::MPIProcessorGroup(
    const CommInterface &comm_interface, int pstart, int pend, const MPI_Comm &world_comm
)
...
*/
int
MPIProcessorGroup::translateRank(const ProcessorGroup *group, int rank) const
{
...
*/
ProcessorGroup *
MPIProcessorGroup::createComplementProcGroup() const
{
    set<int> procs;
    int world_size = _comm_interface.worldSize();
    for (int i = 0; i < world_size; i++) procs.insert(i);
    for (set<int>::const_iterator iter = _proc_ids.begin(); iter != _proc_ids.end(); iter++) procs.erase(*iter);

    return new MPIProcessorGroup(_comm_interface, procs, _world_comm);
}

MPIProcessorGroup *
MPIProcessorGroup::deepCopy() const
{
...
int
MPIProcessorGroup::myRank() const
{
    int rank;
...
ProcessorGroup *
MPIProcessorGroup::createProcGroup() const
{
    set<int> procs;
    for (set<int>::const_iterator iter = _proc_ids.begin(); iter != _proc_ids.end(); iter++) procs.insert(*iter);
