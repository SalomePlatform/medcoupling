 ...
 */
void
ExplicitCoincidentDEC::synchronize()
{
 ...
 *  */
void
ExplicitCoincidentDEC::prepareSourceDE()
{
    ////////////////////////////////////
    ...
    for (int i = 0; i < group->size(); i++) _sendcounts[i] = counts[i];
    ...
    counter[0] = 0;
    for (int i = 1; i < target_size; i++) counter[i] = counter[i - 1] + (int)target_arrays[i - 1].size();
...
ExplicitCoincidentDEC::prepareTargetDE()
{
    ...
    for (std::size_t i = 1; i < union_size; i++) _recvdispls[i] = _recvdispls[i - 1] + _recvcounts[i - 1];
 ...
 *
 * \param toposend Topology that is transmitted. It is read on processes where it already exists, and it is created and
 * filled on others.
 * \param toporecv Topology which is received.
 * \param tag Communication tag associated with this operation.
 */
void
ExplicitCoincidentDEC::broadcastTopology(const ExplicitTopology *toposend, ExplicitTopology *toporecv, int tag)
{
    MPI_Status status;
    ...
    else
    {
        ...
        for (int iproc = 0; iproc < group->size(); iproc++)
        {
            ...
            {
                mcIdType nbelem;
                _comm_interface->recv(
                    &nbelem, 1, MPI_ID_TYPE, isource, tag + myworldrank, *(group->getComm()), &status
                );
                mcIdType *buffer = new mcIdType[nbelem];
                _comm_interface->recv(
                    buffer, (int)nbelem, MPI_ID_TYPE, isource, tag + myworldrank, *(group->getComm()), &status
                );
...
void
ExplicitCoincidentDEC::transferMappingToSource()
{
    MPIProcessorGroup *group = new MPIProcessorGroup(*_comm_interface);
    ...
    if (_topotarget != 0 && _topotarget->getProcGroup()->containsMyRank())
    {
        ...
        for (int i = 0; i < world_size; i++) nb_transfer_union[i] = 0;
        // converts the rank in target to the rank in union communicator
    ...
    else
    {
        ...
        for (int i = 0; i < world_size; i++) dummy_send[i] = 0;
        _comm_interface->allToAll(dummy_send, 1, MPI_INT, nb_transfer_union, 1, MPI_INT, MPI_COMM_WORLD);

        int total_size = 0;
        for (int i = 0; i < world_size; i++) total_size += nb_transfer_union[i];
        int nbtarget = _targetgroup->size();
        int *targetranks = new int[nbtarget];
        for (int i = 0; i < nbtarget; i++) targetranks[i] = group->translateRank(_targetgroup, i);
...
void
ExplicitCoincidentDEC::recvData()
{
    ...
    _local_field->getField()->getArray()->useArray(
        value, true, DeallocType::CPP_DEALLOC, nb_local, _topotarget->getNbComponents()
    );
}
