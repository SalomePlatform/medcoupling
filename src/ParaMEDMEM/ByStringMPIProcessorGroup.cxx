...
/*! Build and return a map between different simCodeTag and the set of MPI ranks ids (based on the passed communicator)
  grouped by the same identifier.
...
DefineSetIdByStringName(const CommInterface &interface, const std::string &simCodeTag, const MPI_Comm &world_comm)
{
    int size_world;
    int rank_world;
 ...
 * Creates a processor group that is based on all the
 ...
 communication layer
*/
ByStringMPIProcessorGroup::ByStringMPIProcessorGroup(const CommInterface &interface) : MPIProcessorGroup(interface) {}
...
ByStringMPIProcessorGroup::ByStringMPIProcessorGroup(
    const CommInterface &interface, const std::string &simCodeTag, const MPI_Comm &world_comm
)
    : MPIProcessorGroup(interface, DefineSetIdByStringName(interface, simCodeTag, world_comm), simCodeTag, world_comm)
{
}

ByStringMPIProcessorGroup::ByStringMPIProcessorGroup(const ByStringMPIProcessorGroup &other) : MPIProcessorGroup(other)
{
}
...
ByStringMPIProcessorGroup::deepCopy() const
{
