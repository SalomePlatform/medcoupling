...
ExplicitMapping::ExplicitMapping()
    : _mapping(), _distant_domains(), _numbers(0), _domains(0), _comm_buffer(0), _buffer_index(0), _send_counts(0)
{
}
...
void
ExplicitMapping::pushBackElem(std::pair<int, mcIdType> idistant)
{
    _mapping.push_back(idistant);
}

void
ExplicitMapping::setDistantElem(mcIdType ilocal, std::pair<int, mcIdType> idistant)
{
    _mapping[ilocal] = idistant;
}

int
ExplicitMapping::nbDistantDomains()
{
    if (_distant_domains.empty())
    {
        for (std::vector<std::pair<int, mcIdType> >::const_iterator iter = _mapping.begin(); iter != _mapping.end();
             iter++)
...
ExplicitMapping::getDistantNumbering(mcIdType ielem) const
{
    return _mapping[ielem];
}

int
ExplicitMapping::getDistantDomain(int i)
{
    if (_domains == 0)
        computeNumbers();
...
int
ExplicitMapping::getNbDistantElems(int i)
{
    if (_numbers == 0)
        computeNumbers();
    return _numbers[i];
...
mcIdType *
ExplicitMapping::serialize(int idproc)
{
    _comm_buffer = new mcIdType[_mapping.size() * 2];
    std::vector<mcIdType> offsets(_distant_domains.size());
    offsets[0] = 0;
    for (int i = 1; i < (int)_distant_domains.size(); i++) offsets[i] = offsets[i - 1] + _numbers[i - 1];
...
void
ExplicitMapping::unserialize(int nbprocs, int *sizes, int nbtarget, int *targetrank, mcIdType *commbuffer)
{
    int total_size = 0;
    for (int i = 0; i < nbprocs; i++) total_size += sizes[i];
    ...
    for (int i = 0; i < nbprocs; i++) _send_counts[i] = sizes[i];
}

void
ExplicitMapping::computeNumbers()
{
