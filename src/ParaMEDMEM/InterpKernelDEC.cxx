...
{
InterpKernelDEC::InterpKernelDEC() : DisjointDEC(), _interpolation_matrix(0) {}

/*!
  This constructor creates an InterpKernelDEC which has \a source_group as a working side
  and  \a target_group as an idle side. All the processors will actually participate, but intersection computations will
  be performed on the working side during the \a synchronize() phase. The constructor must be called synchronously on
  all processors of both processor groups. The source group and target group MUST form a partition of all the procs
  within the communicator passed as 'world_comm' when building the group.
 ...
 */
InterpKernelDEC::InterpKernelDEC(const std::set<int> &src_ids, const std::set<int> &trg_ids, const MPI_Comm &world_comm)
    : DisjointDEC(src_ids, trg_ids, world_comm), _interpolation_matrix(0)
{
}
...
InterpKernelDEC::InterpKernelDEC(
    ProcessorGroup &generic_group, const std::string &source_group, const std::string &target_group
)
...
GetGroupsName(const std::string &interaction_group)
{
...
InterpKernelDEC::~InterpKernelDEC() { release(); }
...
/*!
  ...
  This method prepares all the structures necessary for sending data from a processor group to the other. It uses the
  mesh underlying the fields that have been set with attachLocalField method. It works in four steps :
  -# Bounding boxes are computed for each sub-domain,
  -# The lazy side mesh parts that are likely to intersect the working side local processor are sent to the working
  side,
    ...
    if (_source_group->containsMyRank())
    {
        ...
        for (int i = 0; i < _target_group->size(); i++)
        {
            ...
            if (distant_mesh != 0)
            {
                ...
                // std::cout <<"add contribution from proc "<<idistant_proc_in_union<<" to proc
                // "<<_union_group->myRank()<<std::endl;
                _interpolation_matrix->addContribution(
                    *distant_mesh, idistant_proc_in_union, distant_ids, _method, distantMeth
                );
 ...
 */
void
InterpKernelDEC::synchronizeWithDefaultValue(double val)
{
    this->synchronize();
...
/*!
  Receives the data whether the processor is on the working side or on the lazy side. It must match a \a sendData() call
  on the other side.
*/
void
...
InterpKernelDEC::retrieveNonFetchedIds() const
{
    if (_source_group->containsMyRank())
    {
...
MCAuto<DataArrayIdType>
InterpKernelDEC::retrieveNonFetchedIdsSource() const
{
    return _interpolation_matrix->retrieveNonFetchedIdsSource();
}

MCAuto<DataArrayIdType>
InterpKernelDEC::retrieveNonFetchedIdsTarget() const
{
...
*/
void
InterpKernelDEC::recvData(double time)
{
...
*/
void
InterpKernelDEC::sendData(double time, double deltatime)
{
