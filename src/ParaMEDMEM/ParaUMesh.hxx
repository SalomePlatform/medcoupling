...
class ParaUMesh : public RefCountObject
{
   public:
    ...
    DataArrayDouble *redistributeCellField(
        const DataArrayIdType *globalCellIds, const DataArrayDouble *fieldValueToRed
    ) const;
    DataArrayIdType *redistributeCellField(
        const DataArrayIdType *globalCellIds, const DataArrayIdType *fieldValueToRed
    ) const;
    DataArrayDouble *redistributeNodeField(
        const DataArrayIdType *globalCellIds, const DataArrayDouble *fieldValueToRed
    ) const;
    DataArrayIdType *redistributeNodeField(
        const DataArrayIdType *globalCellIds, const DataArrayIdType *fieldValueToRed
    ) const;
   ...
   private:
    ...
    template <class T>
    typename Traits<T>::ArrayType *redistributeCellFieldT(
        const DataArrayIdType *globalCellIds, const typename Traits<T>::ArrayType *fieldValueToRed
    ) const
    {
        ...
        for (int curRk = 0; curRk < size; ++curRk)
        {
            ...
            fieldToBeSent[curRk] =
                fieldValueToRed->selectByTupleIdSafe(localCellIdsCaptured->begin(), localCellIdsCaptured->end());
        }
        // Receive
        ...
        );  // new == new ordering so that global cell ids are sorted . old == coarse ordering implied by the
            // aggregation
        //
        MCAuto<DataArrayT> fieldAggregated(
            DataArrayT::Aggregate(FromVecAutoToVecOfConst<DataArrayT>(fieldValueReceived))
        );
    ...
    template <class T>
    typename Traits<T>::ArrayType *redistributeNodeFieldT(
        const DataArrayIdType *globalCellIds, const typename Traits<T>::ArrayType *fieldValueToRed
    ) const
    {
        using DataArrayT = typename Traits<T>::ArrayType;
        ...
        );  // new == new ordering so that global node ids are sorted . old == coarse ordering implied by the
            // aggregation
        //
        MCAuto<DataArrayT> fieldAggregated(
            DataArrayT::Aggregate(FromVecAutoToVecOfConst<DataArrayT>(fieldValueReceived))
