...
const MPI_Comm *
ElementLocator::getCommunicator() const
{
...
NatureOfField
ElementLocator::getLocalNature() const
{
...
*/
void
ElementLocator::exchangeMesh(int idistantrank, MEDCouplingPointSet *&distant_mesh, mcIdType *&distant_ids)
{
    ...
    MEDCouplingPointSet *send_mesh = (MEDCouplingPointSet *)_local_para_field.getField()->buildSubMeshData(
        elems->begin(), elems->end(), distant_ids_send
    );
...
void
ElementLocator::exchangeMethod(const std::string &sourceMeth, int idistantrank, std::string &targetMeth)
{
...
*/
void
ElementLocator::_computeBoundingBoxes()
{
    CommInterface comm_interface = _union_group->getCommInterface();
    MPIProcessorGroup *group = static_cast<MPIProcessorGroup *>(_union_group);
    const MPI_Comm *comm = group->getComm();
    ...
    comm_interface.allGather(&_local_cell_mesh_space_dim, 1, MPI_INT, spaceDimForAll, 1, MPI_INT, *comm);
    ...
    if (_local_cell_mesh->getMeshDimension() != -1)
        dbb = INTERP_KERNEL::DirectedBoundingBox(
            _local_cell_mesh->getCoords()->getPointer(),
            _local_cell_mesh->getNumberOfNodes(),
            _local_cell_mesh_space_dim
        );
    std::vector<double> dbbData = dbb.getData();
    if (dbbData.size() < bbSize)
        dbbData.resize(bbSize, 0);
 ...
 */
bool
ElementLocator::_intersectsBoundingBox(int irank)
{
#ifdef USE_DIRECTED_BB
...
)
{
    ...
    comm_interface.sendRecv(
        &tinyInfoLocal[0],
        (int)tinyInfoLocal.size(),
        MPI_ID_TYPE,
        iprocdistant_in_union,
        1112,
        &tinyInfoDistant[0],
        (int)tinyInfoDistant.size(),
        MPI_ID_TYPE,
        iprocdistant_in_union,
        1112,
        *comm,
    ...
    MEDCouplingPointSet *distant_mesh_tmp =
        MEDCouplingPointSet::BuildInstanceFromMeshType((MEDCouplingMeshType)tinyInfoDistant[0]);
    ...
    comm_interface.sendRecv(
        const_cast<void *>(reinterpret_cast<const void *>(distant_ids_send->getConstPointer())),
        (int)tinyInfoLocal.back(),
        MPI_ID_TYPE,
        iprocdistant_in_union,
        1113,
        distant_ids_recv,
        (int)tinyInfoDistant.back(),
        MPI_ID_TYPE,
        iprocdistant_in_union,
        1113,
        *comm,
 ...
 */
void
ElementLocator::recvPolicyFromLazySideW(std::vector<int> &policy)
{
    policy.resize(_distant_proc_ids.size());
    int procId = 0;
    CommInterface comm;
    MPI_Status status;
    for (vector<int>::const_iterator iter = _distant_proc_ids.begin(); iter != _distant_proc_ids.end();
         iter++, procId++)
    {
        int toRecv;
 ...
 */
void
ElementLocator::sendSumToLazySideW(
...
)
{
         ...
         iter++, procId++)
    {
        ...
        comm.send(
            const_cast<void *>(reinterpret_cast<const void *>(&eltIds[0])), lgth, MPI_ID_TYPE, *iter, 1115, *_comm
        );
        comm.send(
            const_cast<void *>(reinterpret_cast<const void *>(&valued[0])), lgth, MPI_DOUBLE, *iter, 1116, *_comm
        );
    }
}
 ...
 */
void
ElementLocator::recvSumFromLazySideW(std::vector<std::vector<double> > &globalSumRelToDistantIds)
{
    int procId = 0;
    CommInterface comm;
    MPI_Status status;
 ...
 */
void
ElementLocator::sendLocalIdsToLazyProcsW(const std::vector<std::vector<mcIdType> > &distantLocEltIds)
{
    int procId = 0;
    CommInterface comm;
    for (vector<int>::const_iterator iter = _distant_proc_ids.begin(); iter != _distant_proc_ids.end();
         iter++, procId++)
    {
        ...
        comm.send(
            const_cast<void *>(reinterpret_cast<const void *>(&eltIds[0])), lgth, MPI_ID_TYPE, *iter, 1122, *_comm
        );
    }
}
 ...
 */
void
ElementLocator::recvGlobalIdsFromLazyProcsW(
 ...
 */
void
ElementLocator::recvCandidatesGlobalIdsFromLazyProcsW(std::vector<std::vector<mcIdType> > &globalIds)
{
    int procId = 0;
    CommInterface comm;
    MPI_Status status;
 ...
 */
void
ElementLocator::sendPartialSumToLazyProcsW(const std::vector<mcIdType> &distantGlobIds, const std::vector<double> &sum)
{
    int procId = 0;
    CommInterface comm;
         ...
         iter++, procId++)
    {
        ...
        comm.send(
            const_cast<void *>(reinterpret_cast<const void *>(&distantGlobIds[0])),
            lgth,
            MPI_ID_TYPE,
            *iter,
            1125,
 ...
 */
void
ElementLocator::sendCandidatesForAddElementsW(const std::vector<mcIdType> &distantGlobIds)
{
    int procId = 0;
    CommInterface comm;
    int lgth = (int)distantGlobIds.size();
 ...
 */
void
ElementLocator::recvAddElementsFromLazyProcsW(std::vector<std::vector<mcIdType> > &elementsToAdd)
{
    int procId = 0;
    CommInterface comm;
    MPI_Status status;
 ...
 */
int
ElementLocator::sendPolicyToWorkingSideL()
{
    CommInterface comm;
    int toSend;
...
ElementLocator::recvFromWorkingSideL()
{
         ...
         iter++, procId++)
    {
        ...
        for (mcIdType i = 0; i < lgth; i++) _values_added[ids[i]] += values[i];
    }
}
 ...
 */
void
ElementLocator::recvSumFromWorkingSideL()
{
    std::size_t procId = 0;
         ...
         iter++, procId++)
    {
        ...
        for (; iter1 != ids.end(); iter1++, iter2++) sums[*iter1] += *iter2;
    }
    ...
    for (procId = 0; procId < wProcSize; procId++)
    {
        ...
        for (; iter1 != ids.end(); iter1++, iter2++) *iter2 = sums[*iter1];
        ids.clear();
    }
}
...
ElementLocator::recvCandidatesForAddElementsL()
{
         ...
         iter++, procId++)
    {
        ...
        for (std::size_t i = 0; i < lgth0; i++) elts0.insert(globalIdsC[ids0[i]]);
        int lgth;
...
ElementLocator::sendCandidatesGlobalIdsToWorkingSideL()
{
    ...
    for (mcIdType *iter1 = candidates->getPointer();
         iter1 != candidates->getPointer() + candidates->getNumberOfTuples();
         iter1++)
         ...
         iter++, procId++)
    {
        ...
        set_intersection(
            ...
            tmp2.begin(),
            tmp2.end(),
            std::back_insert_iterator<std::vector<mcIdType> >(tmp3)
        );
