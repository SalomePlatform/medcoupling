...
*/
ParaFIELD::ParaFIELD(
    TypeOfField type, TypeOfTimeDiscretization td, ParaMESH *para_support, const ComponentTopology &component_topology
)
    : _field(0), _component_topology(component_topology), _topology(0), _own_support(false), _support(para_support)
{
    if (para_support->isStructured() ||
    ...
    else
    {
        if (component_topology.nbBlocks() != 1 && para_support->getTopology()->getProcGroup()->size() != 1)
            throw INTERP_KERNEL::Exception(LOCALIZED(
...
/*! \brief Constructor creating the ParaFIELD
  ...
  attached and it therefore recreates one. It therefore takes ownership over _support. The component topology associated
  with the field is a basic one (all components on the same processor).
*/
ParaFIELD::ParaFIELD(MEDCouplingFieldDouble *subdomain_field, ParaMESH *sup, const ProcessorGroup &proc_group)
    : _field(subdomain_field),
      _component_topology(ComponentTopology((int)_field->getNumberOfComponents())),
      _topology(0),
      _own_support(false),
      _support(sup)
{
...
void
ParaFIELD::synchronizeTarget(ParaFIELD *source_field)
{
...
void
ParaFIELD::synchronizeSource(ParaFIELD *target_field)
{
    DisjointDEC *data_channel;
    if (dynamic_cast<BlockTopology *>(_topology) != 0)
    {
 ...
 */
DataArrayIdType *
ParaFIELD::returnCumulativeGlobalNumbering() const
{
    if (!_field)
        return 0;
...
DataArrayIdType *
ParaFIELD::returnGlobalNumbering() const
{
    if (!_field)
        return 0;
    TypeOfField type = _field->getTypeOfField();
...
/*! This method retrieves the integral of component \a icomp
  over the all domain. */
double
ParaFIELD::getVolumeIntegral(int icomp, bool isWAbs) const
{
