...
void
OverlapInterpolationMatrix::keepTracksOfSourceIds(int procId, DataArrayIdType *ids)
{
    _mapping.keepTracksOfSourceIds(procId, ids);
}

void
OverlapInterpolationMatrix::keepTracksOfTargetIds(int procId, DataArrayIdType *ids)
{
 ...
 */
void
OverlapInterpolationMatrix::computeLocalIntersection(
...
)
{
    ...
    else if (src->getMeshDimension() == 2 && trg->getMeshDimension() == 3 && trg->getSpaceDimension() == 3 &&
             src->getSpaceDimension() == 3)
    {
    ...
    else if (src->getMeshDimension() == 3 && trg->getMeshDimension() == 2 && trg->getSpaceDimension() == 3 &&
             src->getSpaceDimension() == 3)
    {
        MEDCouplingNormalizedUnstructuredMesh<3, 3> target_wrapper(trgC);
        MEDCouplingNormalizedUnstructuredMesh<3, 3> source_wrapper(srcC);
        ...
        colSize = interpolator.interpolateMeshes(
            target_wrapper, source_wrapper, sparse_matrix_part, interpMethod
        );  // not a bug target in source.
        TransposeMatrix(matrixTranspose, colSize, sparse_matrix_part);
        colSize = ToIdType(matrixTranspose.size());
    }
    else if (src->getMeshDimension() == 1 && trg->getMeshDimension() == 2 && trg->getSpaceDimension() == 2 &&
             src->getSpaceDimension() == 2)
    {
    ...
    else if (src->getMeshDimension() == 2 && trg->getMeshDimension() == 1 && trg->getSpaceDimension() == 2 &&
             src->getSpaceDimension() == 2)
    {
        MEDCouplingNormalizedUnstructuredMesh<2, 2> target_wrapper(trgC);
        MEDCouplingNormalizedUnstructuredMesh<2, 2> source_wrapper(srcC);
        ...
        vector<SparseDoubleVec> matrixTranspose;
        colSize = interpolator.interpolateMeshes(
            target_wrapper, source_wrapper, matrixTranspose, interpMethod
        );  // not a bug target in source.
        TransposeMatrix(matrixTranspose, colSize, sparse_matrix_part);
        colSize = ToIdType(matrixTranspose.size());
    }
    ...
    else if (src->getMeshDimension() == 1 && src->getSpaceDimension() == 1)
    {
    ...
    else if (trg->getMeshDimension() == 1 && trg->getSpaceDimension() == 2)
    {
    ...
    else if (trg->getMeshDimension() == 2 && trg->getSpaceDimension() == 3)
    {
    ...
    else if (trg->getMeshDimension() == 2 && trg->getSpaceDimension() == 2)
    {
        MEDCouplingNormalizedUnstructuredMesh<2, 2> target_wrapper(trgC);
        MEDCouplingNormalizedUnstructuredMesh<2, 2> source_wrapper(srcC);
    ...
    else if (trg->getMeshDimension() == 3 && trg->getSpaceDimension() == 3)
    {
        MEDCouplingNormalizedUnstructuredMesh<3, 3> target_wrapper(trgC);
        MEDCouplingNormalizedUnstructuredMesh<3, 3> source_wrapper(srcC);
 ...
 */
void
OverlapInterpolationMatrix::prepare(const std::vector<int> &procsToSendField)
{
    if (_source_support)
...
void
OverlapInterpolationMatrix::computeSurfacesAndDeno()
{
    if (_target_field->getField()->getNature() == IntensiveMaximum)
        ...
        throw INTERP_KERNEL::Exception(
            "OverlapDEC: Policy not set (did you call setNature()?) or not implemented yet: only IntensiveMaximum!"
        );
    //      {
    ...
    //        }
    //      else
    //        throw INTERP_KERNEL::Exception("OverlapDEC: Policy not implemented yet: only IntensiveMaximum and
    //        IntensiveConservation defined!");
    //      }
}

void
OverlapInterpolationMatrix::multiply(double default_val)
{
    _mapping.multiply(_source_field->getField(), _target_field->getField(), default_val);
}

void
OverlapInterpolationMatrix::transposeMultiply()
{
    _mapping.transposeMultiply(_target_field->getField(), _source_field->getField());
}

void
OverlapInterpolationMatrix::TransposeMatrix(
