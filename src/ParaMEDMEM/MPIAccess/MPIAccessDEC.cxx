...
*/
MPIAccessDEC::MPIAccessDEC(const ProcessorGroup &source_group, const ProcessorGroup &target_group, bool Asynchronous)
{
    ...
    _MPI_union_group =
        new MEDCoupling::MPIProcessorGroup(union_group->getCommInterface(), procs, mpilg->getWorldComm());
    delete union_group;
...
void
MPIAccessDEC::setTimeInterpolator(
    TimeInterpolationMethod aTimeInterp, double InterpPrecision, int nStepBefore, int nStepAfter
)
{
    if (_time_interpolator)
        delete _time_interpolator;
    switch (aTimeInterp)
    {
        ...
        case LinearTimeInterp:
            _time_interpolator = new LinearTimeInterpolator(InterpPrecision, nStepBefore, nStepAfter);
...
*/
int
MPIAccessDEC::send(void *sendbuf, int sendcount, int offset, MPI_Datatype sendtype, int target, int &SendRequestId)
{
    int sts;
    if (_asynchronous)
    {
        if (sendtype == MPI_INT)
        {
            sts = _MPI_access->ISend(&((int *)sendbuf)[offset], sendcount, sendtype, target, SendRequestId);
        }
        else if (sendtype == MPI_LONG)
        {
            sts = _MPI_access->ISend(&((long *)sendbuf)[offset], sendcount, sendtype, target, SendRequestId);
        }
        else
        {
            sts = _MPI_access->ISend(&((double *)sendbuf)[offset], sendcount, sendtype, target, SendRequestId);
        }
    }
    else
    {
        if (sendtype == MPI_INT)
        {
            sts = _MPI_access->send(&((int *)sendbuf)[offset], sendcount, sendtype, target, SendRequestId);
        }
        else if (sendtype == MPI_LONG)
        {
            sts = _MPI_access->send(&((long *)sendbuf)[offset], sendcount, sendtype, target, SendRequestId);
        }
        else
        {
            sts = _MPI_access->send(&((double *)sendbuf)[offset], sendcount, sendtype, target, SendRequestId);
        }
    }
    return sts;
}
...
*/
int
MPIAccessDEC::recv(void *recvbuf, int recvcount, int offset, MPI_Datatype recvtype, int target, int &RecvRequestId)
{
    int sts;
    if (_asynchronous)
    {
        if (recvtype == MPI_INT)
        {
            sts = _MPI_access->IRecv(&((int *)recvbuf)[offset], recvcount, recvtype, target, RecvRequestId);
        }
        else if (recvtype == MPI_LONG)
        {
            sts = _MPI_access->IRecv(&((long *)recvbuf)[offset], recvcount, recvtype, target, RecvRequestId);
        }
        else
        {
            sts = _MPI_access->IRecv(&((double *)recvbuf)[offset], recvcount, recvtype, target, RecvRequestId);
        }
    }
    else
    {
        if (recvtype == MPI_INT)
        {
            sts = _MPI_access->recv(&((int *)recvbuf)[offset], recvcount, recvtype, target, RecvRequestId);
        }
        else if (recvtype == MPI_LONG)
        {
            sts = _MPI_access->recv(&((long *)recvbuf)[offset], recvcount, recvtype, target, RecvRequestId);
        }
        else
        {
            sts = _MPI_access->recv(&((double *)recvbuf)[offset], recvcount, recvtype, target, RecvRequestId);
        }
    }
    return sts;
}
...
)
{
    ...
    if (_asynchronous)
    {
        if (sendtype == MPI_INT)
        {
            if (recvtype == MPI_INT)
            {
                sts = _MPI_access->ISendRecv(
                    &((int *)sendbuf)[sendoffset],
                    sendcount,
                    sendtype,
                    target,
                    SendRequestId,
...
)
{
    if (_time_interpolator)
    {
        return allToAllvTime(sendbuf, sendcounts, sdispls, sendtype, recvbuf, recvcounts, rdispls, recvtype);
    }
    int sts = 0;
    int target;
    int SendRequestId;
...
)
{
    ...
    if (sendbuf && sendcount)
    {
        ...
        for (target = 0; target < _group_size; target++)
        {
            sts = send(aSendTimeMessage, 1, 0, _MPI_access->timeType(), target, SendTimeRequestId);
            ...
            if (_time_interpolator && (*_time_messages)[target][0].time != -1)
            {
                if ((*_out_of_time)[target])
                {
                    cout << " =====================================================" << endl
                         ...
                         << (*_time_messages)[target][0].time << " < t1 " << (*_time_messages)[target][1].time
                         << " < t* " << _t << endl
                         << " =====================================================" << endl;
                }
                if (recvtype == MPI_INT)
                {
                    _time_interpolator->doInterp(
                        (*_time_messages)[target][0].time,
                        (*_time_messages)[target][1].time,
                        _t,
                        recvcount,
                        _n_step_before,
                        _n_step_after,
    ...
    */
    ...
    if (sendbuf)
    {
        ...
        for (target = 0; target < _group_size; target++)
        {
            ...
            {
                sts = send(aSendTimeMessage, 1, 0, _MPI_access->timeType(), target, SendTimeRequestId);
                sts = send(sendbuf, sendcounts[target], sdispls[target], sendtype, target, SendDataRequestId);
                if (_asynchronous)
                {
                    aSendTimeStruct->Counter += 1;
                    (*_map_of_send_buffers)[SendTimeRequestId] = aSendTimeStruct;
        ...
        for (target = 0; target < _group_size; target++)
        {
            if (recvcounts[target])
            {
                ...
                else
                {
                    char *buffdest = (char *)recvbuf;
                    char *buffsrc = (char *)(*_data_messages)[target][1];
                    memcpy(&buffdest[rdispls[target] * _MPI_access->extent(recvtype)], buffsrc, recvsize);
                }
            }
        }
    }
...
*/
int
MPIAccessDEC::checkTime(int recvcount, MPI_Datatype recvtype, int target, bool UntilEnd)
{
    int sts = MPI_SUCCESS;
    ...
    if ((*_time_messages)[target][1].time == -1)
    {
        ...
        sts = recv(&(*_time_messages)[target][1], 1, _MPI_access->timeType(), target, RecvTimeRequestId);
        ...
        sts = recv((*_data_messages)[target][1], recvcount, recvtype, target, RecvDataRequestId);
    }
    else
    {
        while ((_t > (*_time_messages)[target][1].time || UntilEnd) && (*_time_messages)[target][1].deltatime != 0)
        {
            (*_time_messages)[target][0] = (*_time_messages)[target][1];
            sts = recv(&(*_time_messages)[target][1], 1, _MPI_access->timeType(), target, RecvTimeRequestId);
            if (UntilEnd)
            {
                cout << "CheckTime" << _my_rank << " TimeMessage target " << target << " RecvTimeRequestId "
                     << RecvTimeRequestId << " MPITag " << _MPI_access->recvMPITag(target) << endl;
            }
            if (recvtype == MPI_INT)
            {
            ...
            if (UntilEnd)
            {
                cout << "CheckTime" << _my_rank << " DataMessage target " << target << " RecvDataRequestId "
                     << RecvDataRequestId << " MPITag " << _MPI_access->recvMPITag(target) << endl;
            }
        }

        if (_t > (*_time_messages)[target][0].time && _t <= (*_time_messages)[target][1].time)
        {
        }
        else
        {
...
*/
int
MPIAccessDEC::checkSent(bool WithWait)
{
    int sts = MPI_SUCCESS;
    int flag = WithWait;
    ...
    for (i = 0; i < nSendRequest; i++)
    {
        if (WithWait)
        {
            if (SendTrace)
            {
                cout << "CheckSent" << _my_rank << " " << i << "./" << nSendRequest << " SendRequestId "
                     << ArrayOfSendRequests[i] << " MPITarget " << _MPI_access->MPITarget(ArrayOfSendRequests[i])
                     << " MPITag " << _MPI_access->MPITag(ArrayOfSendRequests[i]) << " Wait :" << endl;
            }
        ...
        if (flag)
        {
            ...
            {
                cout << "CheckSent" << _my_rank << " " << i << "./" << nSendRequest << " SendRequestId "
                     << ArrayOfSendRequests[i] << " flag " << flag << " Counter "
                     << (*_map_of_send_buffers)[ArrayOfSendRequests[i]]->Counter << " DataType "
                     << (*_map_of_send_buffers)[ArrayOfSendRequests[i]]->DataType << endl;
            }
            (*_map_of_send_buffers)[ArrayOfSendRequests[i]]->Counter -= 1;
            if (SendTrace)
            {
                if ((*_map_of_send_buffers)[ArrayOfSendRequests[i]]->DataType == _MPI_access->timeType())
                {
                ...
                cout << ArrayOfSendRequests[i] << " _map_of_send_buffers->SendBuffer "
                     << (*_map_of_send_buffers)[ArrayOfSendRequests[i]]->SendBuffer << " Counter "
                     << (*_map_of_send_buffers)[ArrayOfSendRequests[i]]->Counter << endl;
            }
            if ((*_map_of_send_buffers)[ArrayOfSendRequests[i]]->Counter == 0)
            {
                if (SendTrace)
                {
                    cout << "CheckSent" << _my_rank << " SendRequestId " << ArrayOfSendRequests[i] << " Counter "
                         << (*_map_of_send_buffers)[ArrayOfSendRequests[i]]->Counter << " flag " << flag
                         << " SendBuffer " << (*_map_of_send_buffers)[ArrayOfSendRequests[i]]->SendBuffer
...
MPIAccessDEC::checkFinalRecv()
{
    ...
    if (_time_interpolator)
    {
        ...
        for (target = 0; target < _group_size; target++)
        {
            ...
            {
                sts = checkTime((*_data_messages_recv_count)[target], (*_data_messages_type)[target], target, true);
...
ostream &
operator<<(ostream &f, const TimeInterpolationMethod &interpolationmethod)
{
