...
class MPIAccessDEC
{
   public:
    ...
    void setTimeInterpolator(
        TimeInterpolationMethod anInterp, double InterpPrecision = 0, int n_step_before = 1, int nStepAfter = 1
    );
    ...
    int recv(
        void *recvbuf, int recvcount, MPI_Datatype recvtype, int target, int &RecvRequestId, bool Asynchronous = false
    );
    int sendRecv(
        void *sendbuf,
        int sendcount,
...
inline int
MPIAccessDEC::send(void *sendbuf, int sendcount, MPI_Datatype sendtype, int target)
{
    int SendRequestId;
    int sts;
    if (_asynchronous)
    {
        sts = _MPI_access->ISend(sendbuf, sendcount, sendtype, target, SendRequestId);
    }
    else
    {
        sts = _MPI_access->send(sendbuf, sendcount, sendtype, target, SendRequestId);
        if (sts == MPI_SUCCESS)
            free(sendbuf);
    }
    return sts;
...
inline int
MPIAccessDEC::recv(void *recvbuf, int recvcount, MPI_Datatype recvtype, int target)
{
    int RecvRequestId;
    int sts;
    if (_asynchronous)
...
inline int
MPIAccessDEC::recv(
    void *recvbuf, int recvcount, MPI_Datatype recvtype, int target, int &RecvRequestId, bool Asynchronous
)
{
    int sts;
    if (Asynchronous)
...
MPIAccessDEC::sendRecv(
    void *sendbuf, int sendcount, MPI_Datatype sendtype, void *recvbuf, int recvcount, MPI_Datatype recvtype, int target
)
{
    int SendRequestId;
    int RecvRequestId;
    ...
    if (_asynchronous)
        sts = _MPI_access->ISendRecv(
            sendbuf, sendcount, sendtype, target, SendRequestId, recvbuf, recvcount, recvtype, target, RecvRequestId
        );
    else
...
std::ostream &
operator<<(std::ostream &, const TimeInterpolationMethod &);
}  // namespace MEDCoupling
