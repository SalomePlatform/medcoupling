...
/**!
  ...
  It is typically called after the MPI_Init() call in a program. It is afterwards passed as a parameter to the
  constructors of ParaMEDMEM objects so that they access the MPI library via the MPIAccess.
...
MPIAccess::MPIAccess(MPIProcessorGroup *ProcessorGroup, int BaseTag, int MaxTag)
    : _comm_interface(ProcessorGroup->getCommInterface()), _intra_communicator(ProcessorGroup->getComm())
{
    void *v;
    int mpitagub;
    int flag;
    ...
    MPI_Type_create_struct(3, array_of_blocklengths, array_of_displacements, array_of_types, &_MPI_TIME);
...
int
MPIAccess::newRequest(MPI_Datatype datatype, int tag, int destsourcerank, bool fromsourcerank, bool asynchronous)
{
    ...
    if (_trace)
        cout << "NewRequest" << _my_rank << "( " << _request << " ) " << mpiaccessstruct << endl;
    return _request;
}
...
*/
int
MPIAccess::newSendTag(MPI_Datatype datatype, int destrank, int method, bool asynchronous, int &RequestId)
{
    int tag;
...
int
MPIAccess::newRecvTag(MPI_Datatype datatype, int sourcerank, int method, bool asynchronous, int &RequestId)
{
    int tag;
...
int
MPIAccess::sendRequestIdsSize()
{
    int size = 0;
    for (int i = 0; i < _processor_group_size; i++) size += (int)_send_requests[i].size();
    return size;
}
...
// SendRequestIds
int
MPIAccess::sendRequestIds(int size, int *ArrayOfSendRequests)
{
    int destrank;
    int i = 0;
...
int
MPIAccess::recvRequestIdsSize()
{
    int size = 0;
    for (int i = 0; i < _processor_group_size; i++) size += (int)_recv_requests[i].size();
    return size;
}
...
// RecvRequestIds
int
MPIAccess::recvRequestIds(int size, int *ArrayOfRecvRequests)
{
    int sourcerank;
    int i = 0;
...
int
MPIAccess::sendRequestIds(int destrank, int size, int *ArrayOfSendRequests)
{
...
int
MPIAccess::recvRequestIds(int sourcerank, int size, int *ArrayOfRecvRequests)
{
...
int
MPIAccess::send(void *buffer, int count, MPI_Datatype datatype, int target, int &RequestId)
{
    int sts = MPI_SUCCESS;
    RequestId = -1;
    if (count)
    {
        ...
        sts = _comm_interface.send(buffer, count, datatype, target, MPItag, *_intra_communicator);
        if (_trace)
            cout << "MPIAccess::Send" << _my_rank << " SendRequestId " << RequestId << " count " << count << " target "
                 << target << " MPItag " << MPItag << endl;
    }
    return sts;
}
...
int
MPIAccess::recv(void *buffer, int count, MPI_Datatype datatype, int source, int &RequestId, int *OutCount)
{
    int sts = MPI_SUCCESS;
    RequestId = -1;
    if (OutCount != NULL)
    ...
    if (count)
    {
        ...
        sts = _comm_interface.recv(buffer, count, datatype, source, MPItag, *_intra_communicator, MPIStatus(RequestId));
        int outcount = 0;
        if (sts == MPI_SUCCESS)
        {
        ...
        if (OutCount != NULL)
            *OutCount = outcount;
        if (_trace)
            cout << "MPIAccess::Recv" << _my_rank << " RecvRequestId " << RequestId << " count " << count << " source "
                 << source << " MPItag " << MPItag << endl;
        deleteRequest(RequestId);
    }
    return sts;
}
...
int
MPIAccess::ISend(void *buffer, int count, MPI_Datatype datatype, int target, int &RequestId)
{
    int sts = MPI_SUCCESS;
    RequestId = -1;
    if (count)
    {
        ...
        if (_trace)
        {
            cout << "MPIAccess::ISend" << _my_rank << " ISendRequestId " << RequestId << " count " << count
                 << " target " << target << " MPItag " << MPItag << endl;
            if (MPItag == 1)
                cout << "MPIAccess::ISend" << _my_rank << " time " << ((TimeMessage *)buffer)->time << " "
                     << ((TimeMessage *)buffer)->deltatime << endl;
        }
        sts = _comm_interface.Isend(buffer, count, datatype, target, MPItag, *_intra_communicator, aSendRequest);
    }
    return sts;
}
...
// returns RequestId identifier.
int
MPIAccess::IRecv(void *buffer, int count, MPI_Datatype datatype, int source, int &RequestId)
{
    int sts = MPI_SUCCESS;
    RequestId = -1;
    if (count)
    {
        ...
        if (_trace)
        {
            cout << "MPIAccess::IRecv" << _my_rank << " IRecvRequestId " << RequestId << " count " << count
                 << " source " << source << " MPItag " << MPItag << endl;
            if (MPItag == 1)
                cout << "MPIAccess::ISend" << _my_rank << " time " << ((TimeMessage *)buffer)->time << " "
                     << ((TimeMessage *)buffer)->deltatime << endl;
        }
        sts = _comm_interface.Irecv(buffer, count, datatype, source, MPItag, *_intra_communicator, aRecvRequest);
    }
    return sts;
}
...
)
{
    ...
    int outcount = -1;
    if (_trace)
        cout << "MPIAccess::SendRecv" << _my_rank << " IRecv RecvRequestId " << RecvRequestId << endl;
    if (sts == MPI_SUCCESS)
    {
        if (sendcount)
            sts = send(sendbuf, sendcount, sendtype, dest, SendRequestId);
        if (_trace)
            cout << "MPIAccess::SendRecv" << _my_rank << " Send SendRequestId " << SendRequestId << endl;
        if (sts == MPI_SUCCESS && recvcount)
        {
            sts = wait(RecvRequestId);
            outcount = MPIOutCount(RecvRequestId);
            if (_trace)
                cout << "MPIAccess::SendRecv" << _my_rank << " IRecv RecvRequestId " << RecvRequestId << " outcount "
                     << outcount << endl;
        }
    }
    if (OutCount != NULL)
    {
        *OutCount = outcount;
        if (_trace)
            cout << "MPIAccess::SendRecv" << _my_rank << " *OutCount = " << *OutCount << endl;
    }
...
MPIAccess::wait(int RequestId)
{
    ...
    if (!MPICompleted(RequestId))
    {
        ...
        {
            if (_trace)
                cout << "MPIAccess::Wait" << _my_rank << " -> wait( " << RequestId << " ) MPIRequest "
                     << MPIRequest(RequestId) << " MPIStatus " << MPIStatus(RequestId) << " MPITag "
                     << MPITag(RequestId) << " MPIIsRecv " << MPIIsRecv(RequestId) << endl;
            status = _comm_interface.wait(MPIRequest(RequestId), MPIStatus(RequestId));
        }
        else
        {
            if (_trace)
                cout << "MPIAccess::Wait" << _my_rank << " MPIRequest == MPI_REQUEST_NULL" << endl;
        }
        setMPICompleted(RequestId, true);
        if (MPIIsRecv(RequestId) && MPIStatus(RequestId))
        {
            ...
            int outcount;
            status = _comm_interface.getCount(MPIStatus(RequestId), datatype, &outcount);
            if (status == MPI_SUCCESS)
            {
                setMPIOutCount(RequestId, outcount);
                deleteStatus(RequestId);
                if (_trace)
                    cout << "MPIAccess::Wait" << _my_rank << " RequestId " << RequestId << "MPIIsRecv "
                         << MPIIsRecv(RequestId) << " outcount " << outcount << endl;
            }
            else
            {
                if (_trace)
                    cout << "MPIAccess::Wait" << _my_rank << " MPIIsRecv " << MPIIsRecv(RequestId) << " outcount "
                         << outcount << endl;
            }
        }
        else
        {
            if (_trace)
                cout << "MPIAccess::Wait" << _my_rank << " MPIIsRecv " << MPIIsRecv(RequestId) << " MPIOutCount "
                     << MPIOutCount(RequestId) << endl;
        }
    }
    if (_trace)
        cout << "MPIAccess::Wait" << _my_rank << " RequestId " << RequestId << " Request " << MPIRequest(RequestId)
             << " Status " << MPIStatus(RequestId) << " MPICompleted " << MPICompleted(RequestId) << " MPIOutCount "
             << MPIOutCount(RequestId) << endl;
    return status;
}
...
// Manage MPI_request * and MPI_status * structure
int
MPIAccess::test(int RequestId, int &flag)
{
    int status = MPI_SUCCESS;
    ...
    if (_trace)
        cout << "Request" << RequestId << " " << MPIRequest(RequestId) << " Status " << MPIStatus(RequestId) << endl;
    if (!flag)
    {
        if (*MPIRequest(RequestId) != MPI_REQUEST_NULL)
        {
            if (_trace)
                cout << "MPIAccess::Test" << _my_rank << " -> test( " << RequestId << " ) MPIRequest "
                     << MPIRequest(RequestId) << " MPIStatus " << MPIStatus(RequestId) << " MPITag "
                     << MPITag(RequestId) << " MPIIsRecv " << MPIIsRecv(RequestId) << endl;
            status = _comm_interface.test(MPIRequest(RequestId), &flag, MPIStatus(RequestId));
        }
        else
        {
            if (_trace)
                cout << "MPIAccess::Test" << _my_rank << " MPIRequest == MPI_REQUEST_NULL" << endl;
        }
        if (flag)
        {
            setMPICompleted(RequestId, true);
            if (MPIIsRecv(RequestId) && MPIStatus(RequestId))
            {
                ...
                if (status == MPI_SUCCESS)
                {
                    ...
                    deleteStatus(RequestId);
                    if (_trace)
                        cout << "MPIAccess::Test" << _my_rank << " MPIIsRecv " << MPIIsRecv(RequestId) << " outcount "
                             << outcount << endl;
                }
                else
                {
            ...
            {
                if (_trace)
                    cout << "MPIAccess::Test" << _my_rank << " MPIIsRecv " << MPIIsRecv(RequestId) << " MPIOutCount "
                         << MPIOutCount(RequestId) << endl;
            }
        }
    }
    if (_trace)
        cout << "MPIAccess::Test" << _my_rank << " RequestId " << RequestId << " flag " << flag << " MPICompleted "
             << MPICompleted(RequestId) << " MPIOutCount " << MPIOutCount(RequestId) << endl;
    return status;
}

int
MPIAccess::waitAny(int count, int *array_of_RequestIds, int &RequestId)
{
...
int
MPIAccess::testAny(int count, int *array_of_RequestIds, int &RequestId, int &flag)
{
    int status = MPI_ERR_OTHER;
    RequestId = -1;
    flag = 0;
...
// Manage MPI_Request * and MPI_Status * structure
int
MPIAccess::waitAll(int count, int *array_of_RequestIds)
{
    if (_trace)
    ...
    {
        if (_trace)
            cout << "WaitAll" << _my_rank << " " << i << " -> Wait( " << array_of_RequestIds[i] << " )" << endl;
...
// Manage MPI_Request * and MPI_Status * structure
int
MPIAccess::testAll(int count, int *array_of_RequestIds, int &flag)
{
    if (_trace)
...
int
MPIAccess::waitSome(int count, int *array_of_RequestIds, int outcount, int *outarray_of_RequestIds)
{
    int status = MPI_ERR_OTHER;
...
int
MPIAccess::testSome(int count, int *array_of_RequestIds, int outcounts, int *outarray_of_RequestIds)
{
    int status = MPI_ERR_OTHER;
...
int
MPIAccess::probe(int FromSource, int &source, int &MPITag, MPI_Datatype &myDatatype, int &outcount)
{
    MPI_Status aMPIStatus;
    int sts = _comm_interface.probe(FromSource, MPI_ANY_TAG, *_intra_communicator, &aMPIStatus);
    if (sts == MPI_SUCCESS)
    {
        ...
        if (_trace)
            cout << "MPIAccess::Probe" << _my_rank << " FromSource " << FromSource << " source " << source << " MPITag "
                 << MPITag << " MethodId " << MethodId << " datatype " << myDatatype << " outcount " << outcount
                 << endl;
    }
    else
    {
...
int
MPIAccess::IProbe(int FromSource, int &source, int &MPITag, MPI_Datatype &myDataType, int &outcount, int &flag)
{
    MPI_Status aMPIStatus;
    int sts = _comm_interface.Iprobe(FromSource, MPI_ANY_TAG, *_intra_communicator, &flag, &aMPIStatus);
    if (sts == MPI_SUCCESS && flag)
    {
        ...
        _comm_interface.getCount(&aMPIStatus, myDataType, &outcount);
        if (_trace)
            cout << "MPIAccess::IProbe" << _my_rank << " FromSource " << FromSource << " source " << source
                 << " MPITag " << MPITag << " MethodId " << MethodId << " datatype " << myDataType << " outcount "
...
int
MPIAccess::cancel(int RecvRequestId, int &flag)
{
    flag = 0;
    int sts = _comm_interface.cancel(MPIRequest(RecvRequestId));
    if (sts == MPI_SUCCESS)
    {
        sts = _comm_interface.wait(MPIRequest(RecvRequestId), MPIStatus(RecvRequestId));
        if (sts == MPI_SUCCESS)
...
int
MPIAccess::cancel(int source, int theMPITag, MPI_Datatype datatype, int outcount, int &flag)
{
    int sts;
    ...
    if (sts == MPI_SUCCESS)
    {
        ...
        if (_trace)
            cout << "MPIAccess::Cancel" << _my_rank << " Irecv extent " << extent << " datatype " << datatype
                 << " source " << source << " theMPITag " << theMPITag << endl;
        sts =
            _comm_interface.Irecv(recvbuf, outcount, datatype, source, theMPITag, *_intra_communicator, &aRecvRequest);
        if (sts == MPI_SUCCESS)
        {
            sts = _comm_interface.cancel(&aRecvRequest);
            if (_trace)
                cout << "MPIAccess::Cancel" << _my_rank << " theMPITag " << theMPITag << " cancel done" << endl;
            if (sts == MPI_SUCCESS)
            {
                MPI_Status aStatus;
                if (_trace)
        ...
        if (_trace && datatype == timeType())
            cout << "MPIAccess::Cancel" << _my_rank << " time " << ((TimeMessage *)recvbuf)->time << " "
...
MPIAccess::cancelAll()
{
    ...
    for (target = 0; target < _processor_group_size; target++)
    {
        ...
        if (sts == MPI_SUCCESS && flag)
        {
            ...
            if (_trace)
                cout << "MPIAccess::CancelAll" << _my_rank << " source " << source << " MPITag " << MPITag
                     << " datatype " << datatype << " outcount " << outcount << " Cancel flag " << flag << endl;
            if (sts != MPI_SUCCESS)
                break;
        }
...
int
MPIAccess::errorString(int errorcode, char *string, int *resultlen) const
{
...
int
MPIAccess::status(int RequestId, int &source, int &tag, int &error, int &outcount, bool keepRequestStruct)
{
    MPI_Status *myStatus = MPIStatus(RequestId);
    if (_trace)
        cout << "MPIAccess::status" << _my_rank << " RequestId " << RequestId << " status " << myStatus << endl;
    if (myStatus != NULL && MPIAsynchronous(RequestId) && MPICompleted(RequestId))
    {
        if (MPIIsRecv(RequestId))
        {
            ...
            if (_trace)
                cout << "MPIAccess::status" << _my_rank << " RequestId " << RequestId << " status " << myStatus
...
int
MPIAccess::requestFree(MPI_Request *request)
{
...
void
MPIAccess::check() const
{
    int i = 0;
    map<int, RequestStruct *>::const_iterator MapOfRequestStructiterator;
    cout << "MPIAccess::Check" << _my_rank << "_map_of_request_struct_size " << _map_of_request_struct.size() << endl;
         ...
         MapOfRequestStructiterator++)
    {
        ...
        {
            cout << "    Check" << _my_rank << " " << i << ". Request" << MapOfRequestStructiterator->first << "-->";
            ...
            if ((MapOfRequestStructiterator->second)->MPIIsRecv)
                ...
                cout << "Send to ";
            cout << (MapOfRequestStructiterator->second)->MPITarget << " MPITag "
                 << (MapOfRequestStructiterator->second)->MPITag << " DataType "
                 << (MapOfRequestStructiterator->second)->MPIDatatype << " Request "
                 << (MapOfRequestStructiterator->second)->MPIRequest << " Status "
                 << (MapOfRequestStructiterator->second)->MPIStatus << " Completed "
                 << (MapOfRequestStructiterator->second)->MPICompleted << endl;
        }
        i++;
    }
}
...
ostream &
operator<<(ostream &f, const TimeMessage &aTimeMsg)
{
    f << " time " << aTimeMsg.time << " deltatime " << aTimeMsg.deltatime << " tag " << aTimeMsg.tag;
    return f;
}

// Outputs the DataType coded in a Tag
ostream &
operator<<(ostream &f, const _MessageIdent &methodtype)
{
    switch (methodtype)
    {
        case _message_time:
