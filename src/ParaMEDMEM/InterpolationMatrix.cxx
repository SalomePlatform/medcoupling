...
InterpolationMatrix::InterpolationMatrix(
    const MEDCoupling::ParaFIELD *source_field,
 ...
 */
void
InterpolationMatrix::addContribution(
    MEDCouplingPointSet &distant_support,
    int iproc_distant,
...
)
{
    ...
    if (distant_support.getMeshDimension() == -1)
    {
        ...
        else if (source_supportC->getMeshDimension() == 2 && source_supportC->getSpaceDimension() == 3)
        {
            ...
            throw INTERP_KERNEL::Exception(
                "No para interpolation available for the given mesh and space dimension of source mesh to -1D "
                "targetMesh"
            );
    }
    else if (source_supportC->getMeshDimension() == -1)
    {
        ...
        else if (distant_supportC->getMeshDimension() == 2 && distant_supportC->getSpaceDimension() == 3)
        {
            ...
            throw INTERP_KERNEL::Exception(
                "No para interpolation available for the given mesh and space dimension of distant mesh to -1D "
                "sourceMesh"
            );
    }
    else if (distant_support.getMeshDimension() == 2 && _source_support->getMeshDimension() == 3 &&
    ...
    else if (distant_support.getMeshDimension() == 1 && _source_support->getMeshDimension() == 2 &&
    ...
    else if (distant_support.getMeshDimension() == 3 && _source_support->getMeshDimension() == 1 &&
             distant_support.getSpaceDimension() == 3 && _source_support->getSpaceDimension() == 3)
    {
        MEDCouplingNormalizedUnstructuredMesh<3, 3> target_wrapper(distant_supportC);
        MEDCouplingNormalizedUnstructuredMesh<3, 3> source_wrapper(source_supportC);
    ...
    else if (distant_support.getMeshDimension() == 1 && distant_support.getSpaceDimension() == 1)
    {
    ...
    else if (distant_support.getMeshDimension() == 1 && distant_support.getSpaceDimension() == 2)
    {
    ...
    else if (distant_support.getMeshDimension() == 2 && distant_support.getSpaceDimension() == 3)
    {
...
void
InterpolationMatrix::fillDSFromVM(
    int iproc_distant,
    const mcIdType *distant_elems,
    const std::vector<std::map<mcIdType, double> > &values,
...
void
InterpolationMatrix::serializeMe(
    std::vector<std::vector<std::map<mcIdType, double> > > &data1, std::vector<int> &data2
) const
{
    data1.clear();
    data2.clear();
    ...
    std::set<int> procsS;
    for (std::vector<std::pair<int, mcIdType> >::const_iterator iter1 = sendingIds.begin(); iter1 != sendingIds.end();
         iter1++)
    ...
    for (std::vector<std::vector<std::map<mcIdType, double> > >::iterator iter3 = data1.begin(); iter3 != data1.end();
         iter3++)
        (*iter3).resize(nbOfSrcElt);
    id = 0;
    for (std::vector<std::vector<std::pair<int, double> > >::const_iterator iter4 = _coeffs.begin();
         iter4 != _coeffs.end();
         iter4++, id++)
    {
        for (std::vector<std::pair<int, double> >::const_iterator iter5 = (*iter4).begin(); iter5 != (*iter4).end();
             iter5++)
        {
...
void
InterpolationMatrix::initialize()
{
    mcIdType lgth = ToIdType(_coeffs.size());
    _row_offsets.clear();
    _row_offsets.resize(lgth + 1);
    _coeffs.clear();
    _coeffs.resize(lgth);
    _target_volume.clear();
    _target_volume.resize(lgth);
    _col_offsets.clear();
    _mapping.initialize();
}

void
InterpolationMatrix::finishContributionW(ElementLocator &elementLocator)
{
...
void
InterpolationMatrix::finishContributionL(ElementLocator &elementLocator)
{
    NatureOfField nature = elementLocator.getLocalNature();
    switch (nature)
    {
...
void
InterpolationMatrix::computeConservVolDenoW(ElementLocator &elementLocator)
{
...
void
InterpolationMatrix::computeConservVolDenoL(ElementLocator &elementLocator)
{
...
void
InterpolationMatrix::computeIntegralDenoW(ElementLocator &elementLocator)
{
    ...
    for (vector<vector<pair<int, double> > >::const_iterator iter4 = _coeffs.begin(); iter4 != _coeffs.end();
         iter4++, iter6++, values++)
    {
...
void
InterpolationMatrix::computeRevIntegralDenoW(ElementLocator &elementLocator)
{
 ...
 */
void
InterpolationMatrix::computeIntegralDenoL(ElementLocator &elementLocator)
{
}
 ...
 */
void
InterpolationMatrix::computeRevIntegralDenoL(ElementLocator &elementLocator)
{
}

void
InterpolationMatrix::computeGlobConstraintDenoW(ElementLocator &elementLocator)
{
...
void
InterpolationMatrix::computeGlobalRowSum(
...
)
{
    ...
    else if (pol1 == ElementLocator::CUMULATIVE_POLICY)
    {
        ...
        // stores for each lazy procs connected with, the ids in global mode if it exists (regarding policyPartial).
        // This array has exactly the size of  rowsPartialSumI, if policyPartial has CUMALATIVE_POLICY in each.
 ...
 * @param resPerProcI out parameter that must be cleared before calling this method. The size of 1st dimension is equal
 * to the size of 'distantProcs'. It contains the element ids (2nd dimension) of the corresponding lazy proc.
 * @param  resPerProcD out parameter with the same format than 'resPerProcI'. It contains corresponding sum values.
 */
void
InterpolationMatrix::computeLocalRowSum(
    const std::vector<int> &distantProcs,
    std::vector<std::vector<mcIdType> > &resPerProcI,
...
/*!
 * This method is only usable when CUMULATIVE_POLICY detected. This method finds elements ids (typically nodes) lazy
 * side that are not present in columns of 'this' and that should regarding cumulative merge of elements regarding their
 * global ids.
 */
void
InterpolationMatrix::findAdditionnalElements(
    ElementLocator &elementLocator,
    std::vector<std::vector<mcIdType> > &elementsToAdd,
    const std::vector<std::vector<mcIdType> > &resPerProcI,
    const std::vector<std::vector<mcIdType> > &globalIdsPartial
)
{
...
void
InterpolationMatrix::addGhostElements(
    const std::vector<int> &distantProcs, const std::vector<std::vector<mcIdType> > &elementsToAdd
)
{
    ...
    for (std::size_t i = 0; i < nbOfDistProcs; i++)
    {
        ...
        if (!eltsForThisProc.empty())
        {
            ...
            for (std::vector<mcIdType>::const_iterator iter2 = eltsForThisProc.begin(); iter2 != eltsForThisProc.end();
                 iter2++)
                (*toFeed)[*iter2] = 0.;
        }
    }
    ...
    for (std::size_t j = 0; j < nbOfDistProcs; j++) fillDSFromVM(data2[j], 0, data1[j], 0);
}

int
InterpolationMatrix::mergePolicies(const std::vector<int> &policyPartial)
{
    ...
    if (iter1 != policyPartial.end())
    {
        ...
        msg << "Incompatible policies between lazy procs each other : proc # "
            << std::distance(policyPartial.begin(), iter1);
 ...
 * As precondition rowsPartialSumD.size()==policyPartial.size()==globalIdsPartial.size(). Foreach i in
 * [0;rowsPartialSumD.size() ) rowsPartialSumD[i].size()==globalIdsPartial[i].size()
 ...
 */
void
InterpolationMatrix::mergeRowSum(
    const std::vector<std::vector<double> > &rowsPartialSumD,
    const std::vector<std::vector<mcIdType> > &globalIdsPartial,
    std::vector<mcIdType> &globalIdsLazySideInteraction,
...
)
{
    ...
    for (std::size_t i = 0; i < nbLazyProcs; i++)
    {
        ...
        for (; iter1 != rowSumOfP.end(); iter1++, iter2++) sumToReturn[*iter2] += *iter1;
    }
    //
    ...
    for (std::map<mcIdType, double>::const_iterator iter5 = sumToReturn.begin(); iter5 != sumToReturn.end();
         iter5++, iter3++, iter4++)
    {
        *iter3 = (*iter5).first;
        *iter4 = (*iter5).second;
...
/*!
 * This method simply reorganize the result contained in 'sumCorresponding' computed by lazy side into 'rowsPartialSumD'
 * with help of 'globalIdsPartial' and 'globalIdsLazySideInteraction'
 *
 ...
 * @param globalIdsLazySideInteraction : in parameter that represents ALL the global ids of every lazy procs in
 * interaction
 * @param sumCorresponding : in parameter with same size as 'globalIdsLazySideInteraction' that stores the corresponding
 * sum of 'globalIdsLazySideInteraction'
 */
void
InterpolationMatrix::mergeRowSum2(
    const std::vector<std::vector<mcIdType> > &globalIdsPartial,
    std::vector<std::vector<double> > &rowsPartialSumD,
    const std::vector<mcIdType> &globalIdsLazySideInteraction,
...
)
{
    ...
    for (; iter1 != globalIdsLazySideInteraction.end(); iter1++, iter2++) acc[*iter1] = *iter2;
    //
    std::size_t nbLazyProcs = globalIdsPartial.size();
    for (std::size_t i = 0; i < nbLazyProcs; i++)
    {
        ...
        for (; iter3 != tmp1.end(); iter3++, iter4++) *iter4 = acc[*iter3];
    }
}

void
InterpolationMatrix::mergeRowSum3(
    const std::vector<std::vector<mcIdType> > &globalIdsPartial, std::vector<std::vector<double> > &rowsPartialSumD
)
{
    ...
    for (; iter1 != globalIdsPartial.end(); iter1++, iter2++)
    {
        ...
        for (; iter3 != (*iter1).end(); iter3++, iter4++) sum[*iter3] += *iter4;
    }
    ...
    for (iter1 = globalIdsPartial.begin(); iter1 != globalIdsPartial.end(); iter1++, iter2++)
    {
        ...
        for (; iter3 != (*iter1).end(); iter3++, iter4++) *iter4 = sum[*iter3];
    }
}

/*!
 * This method updates this->_coeffs attribute in order to take into account hidden (because having the same global
 * number) similar nodes in _coeffs array. If in this->_coeffs two distant element id have the same global id their
 * values will be replaced for each by the sum of the two.
 * @param procsInInteraction input parameter : specifies the procId in absolute of distant lazy procs in interaction
 * with
 ...
 */
void
InterpolationMatrix::mergeCoeffs(
    const std::vector<int> &procsInInteraction,
    const std::vector<std::vector<mcIdType> > &rowsPartialSumI,
    const std::vector<std::vector<mcIdType> > &globalIdsPartial,
    std::vector<std::vector<double> > &denoStrorageInv
)
{
    ...
    int localProcId = 0;
    for (std::vector<int>::const_iterator iter1 = procsInInteraction.begin(); iter1 != procsInInteraction.end();
         iter1++, localProcId++)
    ...
    for (std::size_t i = 0; i < size; i++)
    {
        ...
        for (; iter3 != locals.end(); iter3++, iter4++) myLocalToGlobal[*iter3] = *iter4;
    }
    //
    const vector<std::pair<int, mcIdType> > &mapping = _mapping.getSendingIds();
    ...
    for (vector<vector<pair<int, double> > >::iterator iter1 = _coeffs.begin(); iter1 != _coeffs.end();
         iter1++, iter3++)
    {
...
void
InterpolationMatrix::divideByGlobalRowSum(
    const std::vector<int> &distantProcs,
    const std::vector<std::vector<mcIdType> > &resPerProcI,
...
)
{
    ...
    for (vector<int>::const_iterator iter1 = distantProcs.begin(); iter1 != distantProcs.end(); iter1++, procId++)
    {
        ...
        for (vector<mcIdType>::const_iterator iter2 = currentProcI.begin(); iter2 != currentProcI.end();
             iter2++, iter3++)
         ...
         iter4++, iter6++)
    {
        ...
        for (vector<pair<int, double> >::const_iterator iter5 = (*iter4).begin(); iter5 != (*iter4).end();
             iter5++, iter7++)
...
void
InterpolationMatrix::computeGlobalColSum(std::vector<std::vector<double> > &denoStrorage)
{
    denoStrorage.resize(_coeffs.size());
    vector<vector<double> >::iterator iter2 = denoStrorage.begin();
    for (vector<vector<pair<int, double> > >::const_iterator iter1 = _coeffs.begin(); iter1 != _coeffs.end();
         iter1++, iter2++)
    {
...
void
InterpolationMatrix::resizeGlobalColSum(std::vector<std::vector<double> > &denoStrorage)
{
    vector<vector<double> >::iterator iter2 = denoStrorage.begin();
    for (vector<vector<pair<int, double> > >::const_iterator iter1 = _coeffs.begin(); iter1 != _coeffs.end();
         iter1++, iter2++)
...
InterpolationMatrix::retrieveNonFetchedIdsTarget(mcIdType nbTuples) const
{
 ...
 */
void
InterpolationMatrix::multiply(MEDCouplingFieldDouble &field) const
{
    ...
    if (_target_group.containsMyRank())
    {
        ...
        {
            const MCAuto<DataArrayIdType> nonFetchedEntities =
                _mapping.retrieveNonFetchedIdsTarget(field.getArray()->getNumberOfTuples());
...
MCAuto<DataArrayIdType>
InterpolationMatrix::retrieveNonFetchedIdsSource() const
{
    ...
    if (_source_group.containsMyRank())
    {
        ...
        THROW_IK_EXCEPTION(
            "InterpolationMatrix::retrieveNonFetchedIdsSource : not supposed to be called target side !"
        );
    return ret;
}
...
/**!
 ...
 WT is the transpose matrix from W
   ...
   */
void
InterpolationMatrix::transposeMultiply(MEDCouplingFieldDouble &field) const
{
...
bool
InterpolationMatrix::isSurfaceComputationNeeded(const std::string &method) const
{
    return method == "P0";
}
}  // namespace MEDCoupling
