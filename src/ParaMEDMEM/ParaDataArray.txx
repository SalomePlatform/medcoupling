...
template <class T>
std::size_t
ParaDataArrayTemplate<T>::getHeapMemorySizeWithoutChildren() const
{
    return 0;
}
...
ParaDataArrayTemplate<T>::getDirectChildrenWithNull() const
{
    return {this->_seq_da};
}
...
template <class T>
void
ParaDataArrayTemplate<T>::checkOKOneComponent(const std::string &msg) const
{
    if (this->_seq_da.isNull())
    {
        std::ostringstream oss;
        oss << msg << " : nullptr internal pointer !";
    ...
    {
        std::ostringstream oss;
        oss << msg << " : internal seq dataarray does not contain one component as expected !";
        throw INTERP_KERNEL::Exception(oss.str());
    }
}

/*!
  Parallel version of DataArrayInt::buildComplement. Returns result on proc 0. Not allocated DataArrayT is returned for
  all procs.
 */
template <class T>
DataArrayIdType *
ParaDataArrayDiscrete<T>::buildComplement(T nbOfElems) const
{
