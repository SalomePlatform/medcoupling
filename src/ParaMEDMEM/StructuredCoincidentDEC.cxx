 ...
 */
void
StructuredCoincidentDEC::synchronizeTopology()
{
 ...
 *  */
void
StructuredCoincidentDEC::prepareSourceDE()
{
    ////////////////////////////////////
 ...
 */
void
StructuredCoincidentDEC::prepareTargetDE()
{
    ...
    for (std::size_t i = 1; i < union_size; i++) _recv_displs[i] = _recv_displs[i - 1] + _recv_counts[i - 1];
 ...
 * group possesses it.
 *
 * \param topo Topology that is transmitted. It is read on processes where it already exists, and it is created and
 * filled on others.
 * \param tag Communication tag associated with this operation.
 */
void
StructuredCoincidentDEC::broadcastTopology(BlockTopology *&topo, int tag)
{
    MPI_Status status;
    ...
    else
    {
        ...
        _comm_interface->recv(
            &rank_master, 1, MPI_INT, MPI_ANY_SOURCE, tag + group->myRank(), *(group->getComm()), &status
        );
...
void
StructuredCoincidentDEC::sendData()
{
    MESSAGE("sendData");
    for (int i = 0; i < 4; i++) cout << _send_counts[i] << " ";
    cout << endl;
    for (int i = 0; i < 4; i++) cout << _send_displs[i] << " ";
    cout << endl;
    cout << "start AllToAll" << endl;
    MPI_Comm comm = *(dynamic_cast<MPIProcessorGroup *>(_union_group)->getComm());
    _comm_interface->allToAllV(
...
void
StructuredCoincidentDEC::synchronize()
{
    if (_source_group->containsMyRank())
    {
        synchronizeTopology();
