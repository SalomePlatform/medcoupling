...
class CommInterface
{
   public:
    ...
    int Isend(
        void *buffer, int count, MPI_Datatype datatype, int target, int tag, MPI_Comm comm, MPI_Request *request
    ) const
    {
        return MPI_Isend(buffer, count, datatype, target, tag, comm, request);
    }
    int Irecv(
        void *buffer, int count, MPI_Datatype datatype, int source, int tag, MPI_Comm comm, MPI_Request *request
    ) const
    {
    ...
    int waitsome(
        int incount, MPI_Request *array_of_requests, int *outcount, int *array_of_indices, MPI_Status *array_of_status
    ) const
    {
    ...
    {
        return MPI_Gather(const_cast<void *>(sendbuf), sendcount, sendtype, recvbuf, recvcount, recvtype, root, comm);
    }
    int gatherV(
        const void *sendbuf,
        int sendcount,
   ...
   public:
    void gatherArrays(
        MPI_Comm comm, int root, const DataArrayIdType *array, std::vector<MCAuto<DataArrayIdType> > &arraysOut
    ) const;
    void allGatherArrays(
        MPI_Comm comm, const DataArrayIdType *array, std::vector<MCAuto<DataArrayIdType> > &arraysOut
    ) const;
    int allGatherArrays(
        MPI_Comm comm,
    ...
    void allToAllArrays(
        MPI_Comm comm, const std::vector<MCAuto<DataArrayDouble> > &arrays, MCAuto<DataArrayDouble> &arraysOut
    ) const;
            ...
            if (nbOfComponents != std::numeric_limits<mcIdType>::max())
            {
                if (nbOfComponents != curNbOfCompo)
                    throw INTERP_KERNEL::Exception(
                        "AllToAllArrays : internal error ! Nb of components is not homogeneous !"
                    );
            }
            else
            {
    ...
    ) const
    {
        ...
        for (int curRk = 0; curRk < size; ++curRk)
        {
            arraysOut[curRk] = DataArrayT::NewFromArray(
                cellIdsFromProcs->begin() + offsetsOutIdType[curRk],
                cellIdsFromProcs->begin() + offsetsOutIdType[curRk] + nbOfElems2[curRk]
            );
   ...
   public:
     ...
     */
    static std::unique_ptr<mcIdType[]> SplitArrayOfLength(
        const std::unique_ptr<mcIdType[]> &counts, std::size_t countsSz, int rk, int size
    )
    {
