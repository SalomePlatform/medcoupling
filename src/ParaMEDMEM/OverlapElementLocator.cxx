      ...
      _group(group)
{
    ...
    switch (workSharingAlgo)
    {
        case 0:
            ...
            break;
        case 1:
            computeTodoList_new(false);
            break;
        case 2:
            computeTodoList_new(true);
            break;
        default:
            throw INTERP_KERNEL::Exception(
                "OverlapElementLocator::OverlapElementLocator(): invalid algorithm selected!"
            );
    }
...
OverlapElementLocator::~OverlapElementLocator() { delete[] _domain_bounding_boxes; }

const MPI_Comm *
OverlapElementLocator::getCommunicator() const
{
...
void
OverlapElementLocator::computeBoundingBoxesAndInteractionList()
{
    ...
    // Format minmax :
    // Xmin_src,Xmax_src,Ymin_src,Ymax_src,Zmin_src,Zmax_src,Xmin_trg,Xmax_trg,Ymin_trg,Ymax_trg,Zmin_trg,Zmax_trg
    if (_local_source_mesh)
    ...
    comm_interface.allGather(minmax, bbSize, MPI_DOUBLE, _domain_bounding_boxes, bbSize, MPI_DOUBLE, *comm);
 ...
 */
void
OverlapElementLocator::computeTodoList_original()
{
    ...
    // Keeping todo list of current proc. _to_do_list contains a set of pair where at least _group.myRank() appears
    // once. This proc will be in charge to perform interpolation of any of element of '_to_do_list' If
    // _group.myRank()==myPair.first, current proc should fetch target mesh of myPair.second (if different from
    // _group.myRank()). If _group.myRank()==myPair.second, current proc should fetch source mesh of myPair.second.
    ...
    for (const ProcCouple &pc : _to_do_list) scout << "(" << pc.first << "," << pc.second << ")";
    std::cout << scout.str() << "\n";
#endif
}
 ...
 */
void
OverlapElementLocator::computeTodoList_new(bool revertIter)
{
    using namespace std;
    ...
    //    For each proc, a map giving for a job (=an interaction (i,j)) its 'load', i.e. the amount of work found on
    //    proc #j
    ...
    // Now the algo:
    //
    while (find((bool *)proc_valid, proc_valid + grp_size, true) !=
           proc_valid + grp_size)  // as long as proc_valid is not full of 'false'
    {
        // Most loaded proc:
    ...
    {
        for (const auto &mapIt : itVector) _all_todo_lists[procID].push_back(mapIt.first);
        procID++;
    }
    _to_do_list = _all_todo_lists[myProcId];
...
void
OverlapElementLocator::debugPrintWorkSharing(std::ostream &ostr) const
{
    std::vector<std::vector<ProcCouple> >::const_iterator it = _all_todo_lists.begin();
    ostr << "TODO list lengths: ";
    for (; it != _all_todo_lists.end(); ++it) ostr << (*it).size() << " ";
    ostr << "\n";
}

void
OverlapElementLocator::fillProcToSend()
{
    ...
    for (const auto &pc : _procs_to_send_mesh) scout << "(" << pc.first << "," << (pc.second ? "src" : "tgt") << ") ";
    std::cout << scout.str() << "\n";
#endif
}
 ...
 * The principle is the following : if proc n1 and n2 need to perform a cross sending with n1<n2, then n1 will send
 * first and receive then.
 */
void
OverlapElementLocator::exchangeMeshes(OverlapInterpolationMatrix &matrix)
{
    int myProcId = _group.myRank();
    ...
    for (const ProcCouple &pc : _to_do_list)
    {
        if (pc.first == pc.second)
            continue;  // no xchg needed

        if (pc.first == myProcId)
        {
            if (pc.second < myProcId)
                firstRcv.push_back(Proc_SrcOrTgt(pc.second, false));
            else
                secondRcv.push_back(Proc_SrcOrTgt(pc.second, false));
        }
        else
        {  // pc.second==myProcId
            if (pc.first < myProcId)
                firstRcv.push_back(Proc_SrcOrTgt(pc.first, true));
            else
                secondRcv.push_back(Proc_SrcOrTgt(pc.first, true));
        }
    }
    // Actual receiving, in order please to avoid deadlocks!
    std::sort(firstRcv.begin(), firstRcv.end());
    for (const Proc_SrcOrTgt &pst : firstRcv) receiveRemoteMeshFrom(pst.first, pst.second);

    // Sending source or target mesh to remote procs (_procs_to_send_mesh is sorted too!)
    for (const Proc_SrcOrTgt &pst : _procs_to_send_mesh) sendLocalMeshTo(pst.first, pst.second, matrix);

    // Actual 2nd receiving, in order again please to avoid deadlocks!
    std::sort(secondRcv.begin(), secondRcv.end());
    for (const Proc_SrcOrTgt &pst : secondRcv) receiveRemoteMeshFrom(pst.first, pst.second);
}

std::string
OverlapElementLocator::getSourceMethod() const
{
    return _local_source_field->getField()->getDiscretization()->getStringRepr();
}

std::string
OverlapElementLocator::getTargetMethod() const
{
    return _local_target_field->getField()->getDiscretization()->getStringRepr();
}

const MEDCouplingPointSet *
OverlapElementLocator::getSourceMesh(int procId) const
{
    int myProcId = _group.myRank();
    if (myProcId == procId)
...
OverlapElementLocator::getSourceIds(int procId) const
{
    int myProcId = _group.myRank();
    if (myProcId == procId)
        return 0;
...
const MEDCouplingPointSet *
OverlapElementLocator::getTargetMesh(int procId) const
{
    int myProcId = _group.myRank();
    if (myProcId == procId)
...
const DataArrayIdType *
OverlapElementLocator::getTargetIds(int procId) const
{
    int myProcId = _group.myRank();
    if (myProcId == procId)
        return 0;
...
bool
OverlapElementLocator::isInMyTodoList(int i, int j) const
{
...
bool
OverlapElementLocator::intersectsBoundingBox(int isource, int itarget) const
{
 ...
 */
void
OverlapElementLocator::sendLocalMeshTo(int procId, bool sourceOrTarget, OverlapInterpolationMatrix &matrix) const
{
#ifdef DEC_DEBUG
    ...
    MEDCouplingPointSet *send_mesh = static_cast<MEDCouplingPointSet *>(
        field->getField()->buildSubMeshData(elems->begin(), elems->end(), old2new_map)
    );
    if (sourceOrTarget)
 ...
 */
void
OverlapElementLocator::receiveRemoteMeshFrom(int procId, bool sourceOrTarget)
{
#ifdef DEC_DEBUG
    int rank = _group.myRank();
    std::string st = sourceOrTarget ? "src" : "tgt";
...
void
OverlapElementLocator::sendMesh(int procId, const MEDCouplingPointSet *mesh, const DataArrayIdType *idsToSend) const
{
    ...
    comInterface.send(
        v1Local->getPointer(), (int)v1Local->getNbOfElems(), MPI_ID_TYPE, procId, START_TAG_MESH_XCH + 2, *comm
    );
    comInterface.send(
        v2Local->getPointer(), (int)v2Local->getNbOfElems(), MPI_DOUBLE, procId, START_TAG_MESH_XCH + 3, *comm
    );
    // finished for mesh, ids now
    comInterface.send(
        const_cast<mcIdType *>(idsToSend->getConstPointer()),
        (int)lgth[1],
        MPI_ID_TYPE,
        procId,
        START_TAG_MESH_XCH + 4,
        *comm
    );
    //
    v1Local->decrRef();
...
void
OverlapElementLocator::receiveMesh(int procId, MEDCouplingPointSet *&mesh, DataArrayIdType *&ids) const
{
    mcIdType lgth[2];
    MPI_Status status;
    const MPI_Comm *comm = getCommunicator();
    ...
    comInterface.recv(
        v1Distant->getPointer(),
        (int)v1Distant->getNbOfElems(),
        MPI_ID_TYPE,
        procId,
        START_TAG_MESH_XCH + 2,
        *comm,
        &status
    );
    comInterface.recv(
        v2Distant->getPointer(),
        (int)v2Distant->getNbOfElems(),
        MPI_DOUBLE,
        procId,
        START_TAG_MESH_XCH + 3,
        *comm,
        &status
    );
