    ...
    : _dimension(grid->getSpaceDimension()), _proc_group(&group), _owns_processor_group(false)
{
    ...
    {
        _local_array_indices[0][i] = _local_array_indices[0][i - 1] + axis_length[0] / _proc_group->size();
    ...
    for (int i = 0; i < _dimension; i++) _cycle_type[i] = MEDCoupling::Block;
}
    ...
    : _owns_processor_group(false)
{
    ...
    if (geom_topo.getProcGroup()->size() > 1 && comp_topo.nbBlocks() > 1)
        throw INTERP_KERNEL::Exception(
            LOCALIZED("BlockTopology cannot yet be constructed with both complex geo and components topology")
        );
 ...
 */
BlockTopology::BlockTopology(const ProcessorGroup &group, mcIdType nb_elem)
    : _dimension(1), _proc_group(&group), _owns_processor_group(false)
{
    ...
    mpi_group->getCommInterface().allGather(&nbtemp, 1, MPI_ID_TYPE, nbelems_per_proc, 1, MPI_ID_TYPE, *comm);
    _nb_elems = 0;
    ...
    for (int i = 1; i <= _proc_group->size(); i++)
    {
        _local_array_indices[0][i] = _local_array_indices[0][i - 1] + nbelems_per_proc[i - 1];
...
std::pair<int, mcIdType>
BlockTopology::globalToLocal(const mcIdType global) const
{
    int subdomain_id = 0;
    ...
    for (int idim = _dimension - 1; idim >= 0; idim--)
    {
        ...
        local_increment *=
            _local_array_indices[idim][axis_offset[idim]] - _local_array_indices[idim][axis_offset[idim] - 1];
    }
...
mcIdType
BlockTopology::localToGlobal(const pair<int, mcIdType> local) const
{
...
mcIdType
BlockTopology::getNbLocalElements() const
{
...
BlockTopology::getLocalArrayMinMax() const
{
...
void
BlockTopology::serialize(mcIdType *&serializer, mcIdType &size) const
{
    ...
    for (int i = 0; i < _dimension; i++)
    {
        ...
        for (std::size_t j = 0; j < _local_array_indices[i].size(); j++) buffer.push_back(_local_array_indices[i][j]);
    }
 ...
 */
void
BlockTopology::unserialize(const mcIdType *serializer, const CommInterface &comm_interface)
{
    ...
    for (int i = 0; i < size_comm; i++) procs.insert((int)*(ptr_serializer++));
