...
{
ParaMESH::ParaMESH(
    MEDCouplingPointSet *subdomain_mesh,
...
ParaMESH::ParaMESH(MEDCouplingPointSet *mesh, const ProcessorGroup &proc_group, const std::string &name)
    : _my_domain_id(proc_group.myRank()), _block_topology(new BlockTopology(proc_group, mesh->getNumberOfCells()))
{
...
void
ParaMESH::setNodeGlobal(DataArrayIdType *nodeGlobal)
{
    _node_global.takeRef(nodeGlobal);
}

void
ParaMESH::setCellGlobal(DataArrayIdType *cellGlobal)
{
    _cell_global.takeRef(cellGlobal);
}

ParaMESH::~ParaMESH() { release(); }
