...
ParaUMesh *
ParaUMesh::New(MEDCouplingUMesh *mesh, DataArrayIdType *globalCellIds, DataArrayIdType *globalNodeIds)
{
...
std::size_t
ParaUMesh::getHeapMemorySizeWithoutChildren() const
{
    return 0;
}

std::vector<const BigMemoryObject *>
ParaUMesh::getDirectChildrenWithNull() const
{
...
ParaUMesh::getCellIdsLyingOnNodes(const DataArrayIdType *globalNodeIds, bool fullyIn) const
{
    if (fullyIn)
...
MCAuto<DataArrayIdType>
ParaUMesh::getCellIdsLyingOnNodesTrue(const DataArrayIdType *globalNodeIds) const
{
    ...
    {
        std::unique_ptr<mcIdType[]> nbOfElemsSp(
            CommInterface::SplitArrayOfLength(nbOfElems, size, subDiv, nbOfCollectiveCalls)
        );
        ...
        DataArray::GetSlice(
            0, globalNodeIds->getNumberOfTuples(), 1, subDiv, nbOfCollectiveCalls, startGlobalNodeIds, endGlobalNodeIds
        );
        ci.allGatherV(
            globalNodeIds->begin() + startGlobalNodeIds,
            FromIdType<int>(endGlobalNodeIds - startGlobalNodeIds),
            MPI_ID_TYPE,
            allGlobalNodeIds.get(),
            nbOfElemsInt.get(),
            offsetsIn.get(),
            MPI_ID_TYPE,
            comm
        ...
        for (int curRk = 0; curRk < size; ++curRk)
        {
            ...
            globalNodeIdsOfCurProc->useArray(
                allGlobalNodeIds.get() + offset, false, DeallocType::CPP_DEALLOC, nbOfElemsSp[curRk], 1
            );
            ...
            MCAuto<DataArrayIdType> localNodeIdsToLocate(
                _node_global->findIdForEach(globalNodeIdsCaptured->begin(), globalNodeIdsCaptured->end())
            );
    ...
    for (int curRk = 0; curRk < size; ++curRk)
    {
        ...
        MCAuto<DataArrayIdType> localCellCaptured(
            _mesh->getCellIdsLyingOnNodes(localNodeIdsUnique->begin(), localNodeIdsUnique->end(), true)
        );
        MCAuto<DataArrayIdType> localCellCapturedGlob(
            _cell_global->selectByTupleIdSafe(localCellCaptured->begin(), localCellCaptured->end())
        );
    ...
    {
        std::unique_ptr<int[]> nbOfElemsInt(CommInterface::ToIntArray<mcIdType>(nbOfElems3, size)),
            nbOfElemsOutInt(CommInterface::ToIntArray<mcIdType>(nbOfElems2, size));
        std::unique_ptr<int[]> offsetsIn(CommInterface::ComputeOffset(nbOfElemsInt, size)),
            offsetsOut(CommInterface::ComputeOffset(nbOfElemsOutInt, size));
        ci.allToAllV(
            cells->begin(),
            nbOfElemsInt.get(),
            offsetsIn.get(),
            MPI_ID_TYPE,
            cellIdsFromProcs->getPointer(),
            nbOfElemsOutInt.get(),
            offsetsOut.get(),
            MPI_ID_TYPE,
            comm
...
MCAuto<DataArrayIdType>
ParaUMesh::getCellIdsLyingOnNodesFalse(const DataArrayIdType *globalNodeIds) const
{
    MPI_Comm comm(MPI_COMM_WORLD);
    CommInterface ci;
    int size;
    ...
    for (int subDiv = 0; subDiv < nbOfCollectiveCalls; ++subDiv)
    {
        ...
        for (int curRk = 0; curRk < size; ++curRk)
        {
            ...
            MCAuto<DataArrayIdType> localCellCaptured(
                _mesh->getCellIdsLyingOnNodes(localNodeIdsToLocate->begin(), localNodeIdsToLocate->end(), false)
            );
            MCAuto<DataArrayIdType> localCellCapturedGlob(
                _cell_global->selectByTupleIdSafe(localCellCaptured->begin(), localCellCaptured->end())
            );
...
DataArrayIdType *
ParaUMesh::redistributeCellField(const DataArrayIdType *globalCellIds, const DataArrayIdType *fieldValueToRed) const
{
    return this->redistributeCellFieldT<mcIdType>(globalCellIds, fieldValueToRed);
}

DataArrayDouble *
ParaUMesh::redistributeCellField(const DataArrayIdType *globalCellIds, const DataArrayDouble *fieldValueToRed) const
{
    return this->redistributeCellFieldT<double>(globalCellIds, fieldValueToRed);
}

DataArrayIdType *
ParaUMesh::redistributeNodeField(const DataArrayIdType *globalCellIds, const DataArrayIdType *fieldValueToRed) const
{
    return this->redistributeNodeFieldT<mcIdType>(globalCellIds, fieldValueToRed);
}

DataArrayDouble *
ParaUMesh::redistributeNodeField(const DataArrayIdType *globalCellIds, const DataArrayDouble *fieldValueToRed) const
{
...
ParaUMesh::redistributeCells(const DataArrayIdType *globalCellIds) const
{
    ...
    for (int curRk = 0; curRk < size; ++curRk)
    {
        ...
        globalCellIdsOfCurProc->useArray(
            allGlobalCellIds.get() + offset,
            false,
            DeallocType::CPP_DEALLOC,
            allGlobalCellIdsIndex[curRk + 1] - offset,
            1
        );
        // the key call is here : compute for rank curRk the cells to be sent
        MCAuto<DataArrayIdType> globalCellIdsCaptured(
            _cell_global->buildIntersection(globalCellIdsOfCurProc)
        );  // OK for the global cellIds
        MCAuto<DataArrayIdType> localCellIdsCaptured(
            _cell_global->findIdForEach(globalCellIdsCaptured->begin(), globalCellIdsCaptured->end())
        );
        MCAuto<MEDCouplingUMesh> meshPart(
            _mesh->buildPartOfMySelf(localCellIdsCaptured->begin(), localCellIdsCaptured->end(), true)
        );
        ...
        MCAuto<DataArrayIdType> globalNodeIdsPart(
            _node_global->selectByTupleIdSafe(n2o->begin(), n2o->end())
        );  // OK for the global nodeIds
    ...
    std::vector<const MEDCouplingUMesh *> meshPartsToBeSent2(
        FromVecAutoToVecOfConst<MEDCouplingUMesh>(meshPartsToBeSent)
    );
    // connectivityindex
    std::vector<MCAuto<DataArrayIdType> > connectivityIndexReceived, connectivityReceived;
    {
        std::vector<const DataArrayIdType *> connectivityIndexToBeSent(
            UMeshConnectivityIndexIterator(0, &meshPartsToBeSent2),
            UMeshConnectivityIndexIterator(meshPartsToBeSent2.size(), &meshPartsToBeSent2)
        );
        ci.allToAllArrays(
            comm, FromVecConstToVecAuto<DataArrayIdType>(connectivityIndexToBeSent), connectivityIndexReceived
        );
    }
    // connectivity
    {
        std::vector<const DataArrayIdType *> connectivityToBeSent(
            UMeshConnectivityIterator(0, &meshPartsToBeSent2),
            UMeshConnectivityIterator(meshPartsToBeSent2.size(), &meshPartsToBeSent2)
        );
    ...
    {
        std::vector<const DataArrayDouble *> coordsToBeSent(
            UMeshCoordsIterator(0, &meshPartsToBeSent2),
            UMeshCoordsIterator(meshPartsToBeSent2.size(), &meshPartsToBeSent2)
        );
        ci.allToAllArrays(comm, FromVecConstToVecAuto<DataArrayDouble>(coordsToBeSent), coords);
    }
    /////// Sort it all !
    // firstly deal with nodes.
    MCAuto<DataArrayIdType> aggregatedNodeIds(
        DataArrayIdType::Aggregate(FromVecAutoToVecOfConst<DataArrayIdType>(globalNodeIdsReceived))
    );
    MCAuto<DataArrayIdType> aggregatedNodeIdsSorted(aggregatedNodeIds->copySorted());
    MCAuto<DataArrayIdType> nodeIdsIntoAggregatedIds(
        DataArrayIdType::FindPermutationFromFirstToSecondDuplicate(aggregatedNodeIdsSorted, aggregatedNodeIds)
    );
    MCAuto<DataArrayIdType> idxOfSameNodeIds(aggregatedNodeIdsSorted->indexOfSameConsecutiveValueGroups());
    MCAuto<DataArrayIdType> n2o_nodes(
        nodeIdsIntoAggregatedIds->selectByTupleIdSafe(idxOfSameNodeIds->begin(), idxOfSameNodeIds->end() - 1)
    );  // new == new ordering so that global node ids are sorted . old == coarse ordering implied by the aggregation
    MCAuto<DataArrayIdType> finalGlobalNodeIds(
        aggregatedNodeIdsSorted->selectByTupleIdSafe(idxOfSameNodeIds->begin(), idxOfSameNodeIds->end() - 1)
    );
    ...
    for (int curRk = 0; curRk < size; ++curRk)
    {
        ...
        // work on connectivityReceived[curRk] with transformWithIndArr but do not forget type of cells that should be
        // excluded !
        auto connectivityToModify(connectivityReceived[curRk]);
        auto connectivityIndex(connectivityIndexReceived[curRk]);
        MCAuto<DataArrayIdType> types(
            connectivityToModify->selectByTupleIdSafe(connectivityIndex->begin(), connectivityIndex->end() - 1)
        );
        connectivityToModify->setPartOfValuesSimple3(
            0, connectivityIndex->begin(), connectivityIndex->end() - 1, 0, 1, 1
        );
        connectivityToModify->transformWithIndArr(aa->begin(), aa->end());
        connectivityToModify->setPartOfValues3(
            types, connectivityIndex->begin(), connectivityIndex->end() - 1, 0, 1, 1, true
        );
    }
    // thirdly renumber cells
    MCAuto<DataArrayIdType> aggregatedCellIds(
        DataArrayIdType::Aggregate(FromVecAutoToVecOfConst<DataArrayIdType>(globalCellIdsReceived))
    );
    MCAuto<DataArrayIdType> aggregatedCellIdsSorted(aggregatedCellIds->copySorted());
    MCAuto<DataArrayIdType> idsIntoAggregatedIds(
        DataArrayIdType::FindPermutationFromFirstToSecondDuplicate(aggregatedCellIdsSorted, aggregatedCellIds)
    );
    MCAuto<DataArrayIdType> cellIdsOfSameNodeIds(aggregatedCellIdsSorted->indexOfSameConsecutiveValueGroups());
    MCAuto<DataArrayIdType> n2o_cells(
        idsIntoAggregatedIds->selectByTupleIdSafe(cellIdsOfSameNodeIds->begin(), cellIdsOfSameNodeIds->end() - 1)
    );  // new == new ordering so that global cell ids are sorted . old == coarse ordering implied by the aggregation
    // TODO : check coordsReceived==globalCellIds
    MCAuto<DataArrayIdType> connSorted, indicesSorted;
    {
        MCAuto<DataArrayIdType> conn(
            DataArrayIdType::Aggregate(FromVecAutoToVecOfConst<DataArrayIdType>(connectivityReceived))
        );
        MCAuto<DataArrayIdType> connIndex(
            DataArrayIdType::AggregateIndexes(FromVecAutoToVecOfConst<DataArrayIdType>(connectivityIndexReceived))
        );
        {
            DataArrayIdType *indicesSortedTmp(nullptr), *valuesSortedTmp(nullptr);
            DataArrayIdType::ExtractFromIndexedArrays(
                n2o_cells->begin(), n2o_cells->end(), conn, connIndex, valuesSortedTmp, indicesSortedTmp
            );
            indicesSorted = indicesSortedTmp;
            connSorted = valuesSortedTmp;
        }
    }
    // finalize all
