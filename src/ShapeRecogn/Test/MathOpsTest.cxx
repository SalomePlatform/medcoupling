...
MathOpsTest::testLstsq()
{
    ...
    std::vector<double> b = {0.91167508, 0.95878824, 0.7234827, 0.51753917, 0.18603306};
    std::vector<double> x = MathOps::lstsq(a, b);
    std::array<double, 3> xRef = {0.35719095, 0.5134345, 0.26039343};
    for (size_t i = 0; i < 3; ++i) CPPUNIT_ASSERT_DOUBLES_EQUAL(xRef[i], x[i], 1E-6);
}

void
MathOpsTest::testLstsq2()
{
    std::vector<double> a = {0.4564562, 0.3517006, 0.28928215, 0.72309086, 0.05944836, 0.56024464};
...
void
MathOpsTest::testLstsqBig()
{
    std::vector<double> a(5000000 * 3, 1.0);
    std::vector<double> b(5000000, 1.0);
    std::vector<double> x = MathOps::lstsq(a, b);
    std::array<double, 3> xRef = {1.0 / 3.0, 1.0 / 3.0, 1.0 / 3.0};
    for (size_t i = 2; i < 3; ++i) CPPUNIT_ASSERT_DOUBLES_EQUAL(xRef[i], x[i], 1E-6);
}

void
MathOpsTest::testComputeCov()
{
    std::vector<double> coordinates{1, 2, -3, -8, 6, -3, 9, 2, -1, -10, -11, -120};
    std::array<double, 9> covMatrix = MathOps::computeCov(coordinates);
...
void
MathOpsTest::testComputePCAFirstAxis()
{
    std::vector<double> coordinates{1, 2, -3, -8, 6, -3, 9, 2, -1, -10, -11, -120};
    std::array<double, 3> axis = MathOps::computePCAFirstAxis(coordinates);
...
void
MathOpsTest::testComputeAngles()
{
    std::vector<double> directions{1, 2, -3, -8, 6, -3, 9, 2, -1, -10, -11, -120};
    std::array<double, 3> axis{1, 2, 3};
    std::vector<double> angles = MathOps::computeAngles(directions, axis);
...
void
MathOpsTest::testComputeBaseFromNormal()
{
    std::array<double, 3> normal = {1.0, 2.0, 3.0};
    std::array<double, 6> base = MathOps::computeBaseFromNormal(normal);
    std::array<double, 6> baseRef = {-0.53452248, 0.77454192, -0.33818712, -0.80178373, -0.33818712, 0.49271932};
