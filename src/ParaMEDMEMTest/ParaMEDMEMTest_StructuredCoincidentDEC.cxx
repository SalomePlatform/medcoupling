...
void
ParaMEDMEMTest::testStructuredCoincidentDEC()
{
    ...
    if (source_group.containsMyRank())
    {
        ...
        for (int ielem = 0; ielem < nb_local; ielem++)
            for (int icomp = 0; icomp < 6; icomp++) value[ielem * 6 + icomp] = global_numbering[ielem] * 6 + icomp;

        // ICoCo::Field* icocofield=new ICoCo::MEDField((MEDCouplingUMesh
        // *)paramesh->getCellMesh(),parafield->getField());
        ...
        for (int i = 0; i < nb_local; i++)
        {
            ...
            for (int icomp = 0; icomp < comptopo.nbLocalComponents(); icomp++)
                CPPUNIT_ASSERT_DOUBLES_EQUAL(
                    recv_value[i * comptopo.nbLocalComponents() + icomp], (double)(i * 6 + icomp + first), 1e-12
                );
        }
        // delete icocofield;
    }
