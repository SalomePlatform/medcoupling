...
void
MPIAccessDECTest::test_AllToAllvDECSynchronousPointToPoint()
{
    test_AllToAllvDEC(false);
}
void
MPIAccessDECTest::test_AllToAllvDECAsynchronousPointToPoint()
{
...
chksts(int sts, int myrank, MEDCoupling::MPIAccess mpi_access)
{
    ...
    if (sts != MPI_SUCCESS)
    {
        ...
        debugStream << "test_AllToAllvDEC" << myrank << " lenerr " << lenerr << " " << msgerr << endl;
...
void
MPIAccessDECTest::test_AllToAllvDEC(bool Asynchronous)
{
...
#define datamsglength 10
    ...
    for (ireq = 0; ireq < maxreq; ireq++)
    {
        ...
        MyMPIAccessDEC->allToAllv(sendbuf, sendcounts, sdispls, MPI_INT, recvbuf, recvcounts, rdispls, MPI_INT);
    ...
    debugStream << "test_AllToAllvDEC" << myrank << " final SendRequestIds " << nSendReq << " SendRequests" << endl;
    if (nSendReq)
    {
    ...
    {
        ostringstream strstream;
        strstream << "test_AllToAllvDEC" << myrank << " final RecvRequestIds " << nRecvReq << " RecvRequests # 0 Error"
                  << endl;
        debugStream << strstream.str() << endl;
        CPPUNIT_FAIL(strstream.str());
    }
    ...
    {
        debugStream << "test_AllToAllvDEC" << myrank << " final RecvRequestIds " << nRecvReq << " RecvRequests = 0 OK"
                    << endl;
    }
