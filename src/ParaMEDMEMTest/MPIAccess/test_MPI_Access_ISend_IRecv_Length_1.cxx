...
void
MPIAccessTest::test_MPI_Access_ISend_IRecv_Length_1()
{
    //  MPI_Init(&argc, &argv) ;
...
#define maxreq 10
    ...
    for (i = 0; i < maxreq; i++)
    {
        ...
        for (j = 1; j <= i; j++)
        {
            ...
            else
            {
                ...
                debugStream << "test" << myrank << " IProbe lenerr " << lenerr << " " << msgerr << endl;
                if (sts != MPI_SUCCESS)
                {
                    ostringstream strstream;
                    strstream << "==========================================================="
                ...
                debugStream << "test" << myrank << " IProbe i/j " << i << "/" << j << " MPITag " << MPITag
                            << " datatype " << datatype << " outcount " << outcount << " flag " << flag << endl;
            }
            if (flag)
            {
                if (myrank == 0)
                ...
                {
                    sts = mpi_access.IRecv(recvbuf[maxirecv], outcount, datatype, source, RecvRequestId[maxirecv]);
                    debugStream << "test" << myrank << " maxirecv " << maxirecv << " IRecv RequestId "
                                << RecvRequestId[maxirecv] << " source " << source << " outcount " << outcount
            ...
            {
                sts = mpi_access.IRecv(recvbuf[j], 1000 * j, MPI_INT, target, RecvRequestId[j]);
                debugStream << "test" << myrank << " maxirecv " << maxirecv << " IRecv RequestId " << RecvRequestId[j]
                            << " target " << target << " length " << 1000 * j << " tag "
    ...
    else
    {
        ...
        if (maxirecv != maxreq)
        {
            ostringstream strstream;
            strstream << "===========================================================" << endl
                      << "test" << myrank << " KO" << " maxirecv " << maxirecv << " != maxreq " << maxreq << endl
                      << "===========================================================" << endl;
            debugStream << strstream.str() << endl;
            CPPUNIT_FAIL(strstream.str());
        }
        while (maxirecv > 0)
        {
            for (i = 1; i < maxreq; i++)
            {
                ...
                int lenerr;
                mpi_access.errorString(sts, msgerr, &lenerr);
                debugStream << "test" << myrank << " flag " << flag << " lenerr " << lenerr << " " << msgerr
                            << " maxirecv " << maxirecv << endl;
                if (sts != MPI_SUCCESS)
                {
                    ostringstream strstream;
                    strstream << "==========================================================="
                ...
                {
                    int tag, error;
                    mpi_access.status(RecvRequestId[i], source, tag, error, outcount);
                    if (i != 0)
                    {
                        if ((outcount != 1000 * i) || ((recvbuf[i][outcount - 1] != (outcount - 1))))
                        {
                            ostringstream strstream;
                            strstream << "========================================================" << endl
                                      << "test" << myrank << " outcount " << outcount << " KO" << " i " << i
                    ...
                    else if (outcount != 0)
                    {
                        ostringstream strstream;
                        strstream << "========================================================" << endl
                                  << "test" << myrank << " outcount " << outcount << " KO" << " i " << i << endl
                                  << "========================================================" << endl;
                        debugStream << strstream.str() << endl;
                        CPPUNIT_FAIL(strstream.str());
                    }
