...
void
MPIAccessDECTest::test_AllToAllvTimeDoubleDECSynchronousPointToPoint()
{
    test_AllToAllvTimeDoubleDEC(false);
}
void
MPIAccessDECTest::test_AllToAllvTimeDoubleDECAsynchronousPointToPoint()
{
...
void
MPIAccessDECTest::test_AllToAllvTimeDoubleDEC(bool Asynchronous)
{
...
#define datamsglength 10
         ...
         timeLoc[myrank] += nextdeltatime[myrank])
    {
        ...
        debugStream << "test" << myrank << "=====TIME " << timeLoc[myrank] << "=====DELTATIME " << nextdeltatime[myrank]
                    << "=====MAXTIME " << maxtime[myrank] << " ======" << endl;
        ...
        for (target = 0; target < size; target++)
        {
            for (j = 0; j < datamsglength; j++)
            {
                ...
                sendbuf[target * datamsglength + j] =
                    myrank * 1000000 + target * 10000 + (timeLoc[myrank] / deltatime[myrank]) * 100 + j;
        ...
        sts = MyMPIAccessDEC->allToAllvTime(
            sendbuf, sendcounts, sdispls, MPI_DOUBLE, recvbuf, recvcounts, rdispls, MPI_DOUBLE
        );
        ...
        if (nRecvReq != 0)
        {
            ostringstream strstream;
            strstream << "=============================================================" << endl
                      << "test_AllToAllvTimeDoubleDEC" << myrank << " WaitAllRecv " << nRecvReq << " Requests # 0 ERROR"
                      << endl
                      << "============================================================" << endl;
        ...
        for (target = 0; target < size; target++)
        {
            for (int jj = 0; jj < datamsglength; jj++)
            {
                ...
                if (jj < recvcounts[target])
                {
                    if (fabs(
                            recvbuf[index] -
                            (target * 1000000 + myrank * 10000 + (timeLoc[target] / deltatime[target]) * 100 + jj)
                        ) > 101)
                    {
                        badrecvbuf = true;
                        debugStream << "test_AllToAllvTimeDoubleDEC" << myrank << " target " << target
                                    << " timeLoc[target] " << timeLoc[target] << " recvbuf[" << index << "] "
                                    << (int)recvbuf[index] << " # "
                                    << (int)(target * 1000000 + myrank * 10000 +
                                             (timeLoc[target] / deltatime[target]) * 100 + jj)
                                    << endl;
                    }
                    else if (badrecvbuf)
                    {
        ...
        if (badrecvbuf)
        {
            ostringstream strstream;
            strstream << "==================================================================" << endl
                      << "test_AllToAllvTimeDoubleDEC" << myrank << " badrecvbuf" << endl
                      << "==================================================================" << endl;
            debugStream << strstream.str() << endl;
            CPPUNIT_FAIL(strstream.str());
        }
    ...
    if (sts != MPI_SUCCESS)
    {
        ostringstream strstream;
        strstream << "=================================================================" << endl
                  << "test_AllToAllvTimeDoubleDEC" << myrank << " CheckFinalSent ERROR" << endl
                  << "=================================================================" << endl;
        debugStream << strstream.str() << endl;
        CPPUNIT_FAIL(strstream.str());
    }
    ...
    if (sts != MPI_SUCCESS)
    {
        ostringstream strstream;
        strstream << "=================================================================" << endl
                  << "test_AllToAllvTimeDoubleDEC" << myrank << " CheckFinalRecv ERROR" << endl
                  << "================================================================" << endl;
        debugStream << strstream.str() << endl;
        CPPUNIT_FAIL(strstream.str());
    }
