...
void
MPIAccessTest::test_MPI_Access_IProbe()
{
    ...
    for (i = 0; i < 10; i++)
    {
        if (myrank == 0)
        {
            ...
            debugStream << "test" << myrank << " Send RequestId " << RequestId[i] << endl;
        }
        else
        {
            int flag = false;
            while (!flag)
            {
                ...
                if (flag)
                {
                    debugStream << "test" << myrank << " " << i << " IProbe target " << target << " source " << source
                                << " tag " << tag << " outcount " << outcount << " flag " << flag << endl;
                }
                else
                {
                ...
                {
                    int recvbuf;
                    sts = mpi_access.recv(&recvbuf, outcount, datatype, source, RequestId[i], &outcount);
                    ...
                    debugStream << "===========================================================" << endl
                                << "test" << myrank << " outcount " << outcount << " recvbuf " << recvbuf << " OK"
                                << endl
                                << "===========================================================" << endl;
                }
            }
