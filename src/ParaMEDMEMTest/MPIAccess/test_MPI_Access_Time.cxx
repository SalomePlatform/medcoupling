...
void
MPIAccessTest::test_MPI_Access_Time()
{
...
#define maxreq 10
    ...
    for (t = 0; t < maxt; t = t + dt)
    {
        if (myrank == 0)
        {
            ...
            sts = mpi_access.ISend(&aSendTimeMsg[i], 1, mpi_access.timeType(), target, SendTimeRequestId[i]);
            debugStream << "test" << myrank << " ISend RequestId " << SendTimeRequestId[i] << " tag "
                        << mpi_access.sendMPITag(target) << endl;
            sendbuf[i] = i;
            sts = mpi_access.ISend(&sendbuf[i], 1, MPI_INT, target, SendRequestId[i]);
            debugStream << "test" << myrank << " ISend RequestId " << SendRequestId[i] << " tag "
                        << mpi_access.sendMPITag(target) << endl;
        }
        else
        {
            // sts = mpi_access.IRecv( &aRecvTimeMsg , mpi_access.timeExtent() ,
            sts = mpi_access.IRecv(&aRecvTimeMsg[i], 1, mpi_access.timeType(), target, RecvTimeRequestId[i]);
            debugStream << "test" << myrank << " IRecv RequestId " << RecvTimeRequestId[i] << " tag "
                        << mpi_access.recvMPITag(target) << endl;
            sts = mpi_access.IRecv(&recvbuf[i], 1, MPI_INT, target, RecvRequestId[i]);
            debugStream << "test" << myrank << " IRecv RequestId " << RecvRequestId[i] << " tag "
                        << mpi_access.recvMPITag(target) << endl;
        }
        int j;
        for (j = 0; j <= i; j++)
        {
            ...
            if (flag)
            {
                ...
                if (myrank == 0)
                {
                    mpi_access.status(SendTimeRequestId[j], target, tag, error, outcount, true);
                    debugStream << "test" << myrank << " Test(Send TimeRequestId " << SendTimeRequestId[j]
                                << ") : target " << target << " tag " << tag << " error " << error << " flag " << flag
                                << aSendTimeMsg[j] << endl;
                }
                else
                {
                    mpi_access.status(RecvTimeRequestId[j], source, tag, error, outcount, true);
                    debugStream << "test" << myrank << " Test(Recv TimeRequestId " << RecvTimeRequestId[j]
                                << ") : source " << source << " tag " << tag << " error " << error << " outcount "
                                << outcount << " flag " << flag << aRecvTimeMsg[j] << endl;
                    if ((outcount != 1) | (aRecvTimeMsg[j].time != j))
                    {
                        ostringstream strstream;
                        strstream << "===========================================================" << endl
                                  << "test" << myrank << " outcount " << outcount << " KO"
                    ...
                    {
                        debugStream << "===========================================================" << endl
                                    << "test" << myrank << " outcount " << outcount << " RecvTimeRequestId "
                                    << RecvTimeRequestId[j] << " OK" << endl
                                    << "===========================================================" << endl;
                    }
                }
            }
            ...
            if (flag)
            {
                ...
                if (myrank == 0)
                {
                    mpi_access.status(SendRequestId[j], target, tag, error, outcount, true);
                    debugStream << "test" << myrank << " Test(Send RequestId " << SendRequestId[j] << ") : target "
                                << target << " tag " << tag << " error " << error << " flag " << flag << endl;
                }
                else
                {
                    mpi_access.status(RecvRequestId[j], source, tag, error, outcount, true);
                    debugStream << "test" << myrank << " Test(Recv RequestId " << RecvRequestId[j] << ") : source "
                                << source << " tag " << tag << " error " << error << " outcount " << outcount
                                << " flag " << flag << endl;
                    if ((outcount != 1) | (recvbuf[j] != j))
                    {
                        ostringstream strstream;
                        strstream << "===========================================================" << endl
                                  << "test" << myrank << " outcount " << outcount << " recvbuf " << recvbuf[j] << " KO"
                                  << endl
                                  << "===========================================================" << endl;
                        debugStream << strstream.str() << endl;
                        CPPUNIT_FAIL(strstream.str());
                    ...
                    {
                        debugStream << "===========================================================" << endl
                                    << "test" << myrank << " outcount " << outcount << " RequestId " << RecvRequestId[j]
                                    << " OK" << endl
                                    << "===========================================================" << endl;
                    }
                }
