...
void
MPIAccessDECTest::test_AllToAllDECSynchronousPointToPoint()
{
    test_AllToAllDEC(false);
}
void
MPIAccessDECTest::test_AllToAllDECAsynchronousPointToPoint()
{
...
void
MPIAccessDECTest::test_AllToAllDEC(bool Asynchronous)
{
...
#define datamsglength 10
    ...
    for (ireq = 0; ireq < maxreq; ireq++)
    {
        ...
        MyMPIAccessDEC->allToAll(sendbuf, sendcount, MPI_INT, recvbuf, recvcount, MPI_INT);
    ...
    debugStream << "test_AllToAllDEC" << myrank << " final SendRequestIds " << nSendReq << " SendRequests" << endl;
    if (nSendReq)
    {
    ...
    {
        ostringstream strstream;
        strstream << "test_AllToAllDEC" << myrank << " final RecvRequestIds " << nRecvReq << " RecvRequests # 0 Error"
                  << endl;
        debugStream << strstream.str() << endl;
        CPPUNIT_FAIL(strstream.str());
    }
    ...
    {
        debugStream << "test_AllToAllDEC" << myrank << " final RecvRequestIds " << nRecvReq << " RecvRequests = 0 OK"
                    << endl;
    }
