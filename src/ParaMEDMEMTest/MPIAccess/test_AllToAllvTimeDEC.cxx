...
void
MPIAccessDECTest::test_AllToAllvTimeDECSynchronousNative()
{
    test_AllToAllvTimeDEC(false, true);
}
void
MPIAccessDECTest::test_AllToAllvTimeDECSynchronousPointToPoint()
{
    test_AllToAllvTimeDEC(false, false);
}
void
MPIAccessDECTest::test_AllToAllvTimeDECAsynchronousPointToPoint()
{
...
chksts(int sts, int myrank, MEDCoupling::MPIAccess *mpi_access)
{
    ...
    if (sts != MPI_SUCCESS)
    {
        ...
        debugStream << "test_AllToAllvTimeDEC" << myrank << " lenerr " << lenerr << " " << msgerr << endl;
...
void
MPIAccessDECTest::test_AllToAllvTimeDEC(bool Asynchronous, bool UseMPINative)
{
    ...
    if (size < 2 || size > 11)
    {
        ...
        strstream << "usage :" << endl
                  ...
                  << " (nbprocs >=2)" << endl
                  << "test must be run with more than 1 proc and less than 12 procs" << endl;
    ...
    debugStream << "test_AllToAllvTimeDEC" << myrank << " Asynchronous " << Asynchronous << " UseMPI_Alltoallv "
    ...
    MPIAccessDEC *MyMPIAccessDEC = new MPIAccessDEC(*sourcegroup, *targetgroup, Asynchronous);
...
#define datamsglength 10
    ...
    for (timeLoc = 0; timeLoc <= maxtime && nextdeltatime != 0; timeLoc += nextdeltatime)
    {
        ...
        if (UseMPI_Alltoallv)
        {
            ...
            interface.allToAllV(sendbuf, sendcounts, sdispls, MPI_INT, recvbuf, recvcounts, rdispls, MPI_INT, *comm);
            //       free(sendbuf) ;
            delete[] sendbuf;
        }
        else
        {
            int sts2 = MyMPIAccessDEC->allToAllvTime(
                sendbuf, sendcounts, sdispls, MPI_INT, recvbuf, recvcounts, rdispls, MPI_INT
            );
        ...
        for (i = 0; i < size; i++)
        {
            for (int jj = 0; jj < datamsglength; jj++)
            {
                ...
                if (jj < recvcounts[i])
                {
                    if (recvbuf[index] != (index / datamsglength) * 1000000 + myrank * 1000 + myrank * datamsglength +
                                              (index % datamsglength))
                    {
                        badrecvbuf = true;
                        debugStream << "test_AllToAllvTimeDEC" << myrank << " recvbuf[" << index << "] "
                                    << recvbuf[index] << " # "
                                    << (index / datamsglength) * 1000000 + myrank * 1000 + myrank * datamsglength +
                                           (index % datamsglength)
                                    << endl;
                    }
                    else if (badrecvbuf)
                ...
                {
                    badrecvbuf = true;
                    debugStream << "test_AllToAllvTimeDEC" << myrank << " recvbuf[" << index << "] " << recvbuf[index]
                                << " # -1" << endl;
                }
            }
        }
        if (badrecvbuf)
        {
            ostringstream strstream;
            strstream << "==============================================================" << endl
                      << "test_AllToAllvTimeDEC" << myrank << " badrecvbuf" << endl
                      << "=============================================================" << endl;
            debugStream << strstream.str() << endl;
            CPPUNIT_FAIL(strstream.str());
        }
    ...
    if (sts != MPI_SUCCESS)
    {
        ostringstream strstream;
        strstream << "================================================================" << endl
                  << "test_AllToAllvTimeDEC" << myrank << " final CheckSent ERROR" << endl
                  << "================================================================" << endl;
        debugStream << strstream.str() << endl;
        CPPUNIT_FAIL(strstream.str());
    }
    ...
    if (sts != MPI_SUCCESS)
    {
        ostringstream strstream;
        strstream << "================================================================" << endl
                  << "test_AllToAllvTimeDEC" << myrank << " CheckFinalRecv ERROR" << endl
                  << "================================================================" << endl;
        debugStream << strstream.str() << endl;
        CPPUNIT_FAIL(strstream.str());
    }
    ...
    if (nRecvReq)
    {
        ostringstream strstream;
        strstream << "===============================================================" << endl
                  << "test_AllToAllvTimeDEC" << myrank << " RecvRequestIds " << nRecvReq << " RecvRequests # 0 Error"
                  << endl
                  << "===============================================================" << endl;
        debugStream << strstream.str() << endl;
        CPPUNIT_FAIL(strstream.str());
    ...
    {
        debugStream << "test_AllToAllvTimeDEC" << myrank << " RecvRequestIds " << nRecvReq << " RecvRequests = 0 OK"
                    << endl;
    }

    time_t endtime = time(NULL);
    debugStream << "test_AllToAllvTimeDEC" << myrank << " begintime " << begintime << " endtime " << endtime
                << " elapse " << endtime - begintime << " " << maxtime / deltatime[myrank] << " calls to AllToAll"
                << endl;
