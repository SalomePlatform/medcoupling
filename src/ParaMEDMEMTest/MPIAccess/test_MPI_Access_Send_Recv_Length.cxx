...
void
MPIAccessTest::test_MPI_Access_Send_Recv_Length()
{
    ...
    for (i = 0; i < 10; i++)
    {
        ...
        else
        {
            ...
            if (i != 0)
            {
                sts = mpi_access.recv(recvbuf, 1000 * i + 1, MPI_INT, target, RequestId[i], &outcount);
            }
            ...
            debugStream << "test" << myrank << " Recv RequestId " << RequestId[i] << " tag "
                        << mpi_access.recvMPITag(target) << " outcount " << outcount << endl;
            recvbufok = true;
            for (j = 0; j < outcount; j++)
            {
                if (recvbuf[j] != j)
                {
                    debugStream << "test" << myrank << " recvbuf[ " << j << " ] = " << recvbuf[j] << endl;
                    recvbufok = false;
                    break;
                }
            }
