...
void
MPIAccessDECTest::test_AllToAllTimeDECSynchronousPointToPoint()
{
    test_AllToAllTimeDEC(false);
}
void
MPIAccessDECTest::test_AllToAllTimeDECAsynchronousPointToPoint()
{
...
chksts(int sts, int myrank, MEDCoupling::MPIAccess *mpi_access)
{
    ...
    if (sts != MPI_SUCCESS)
    {
        ...
        debugStream << "test_AllToAllTimeDEC" << myrank << " lenerr " << lenerr << " " << msgerr << endl;
...
void
MPIAccessDECTest::test_AllToAllTimeDEC(bool Asynchronous)
{
...
#define datamsglength 10
    ...
    for (time = 0; time <= maxtime && nextdeltatime != 0; time += nextdeltatime)
    {
        ...
        debugStream << "test_AllToAllTimeDEC" << myrank << "=====TIME " << time << "=====DELTATIME " << nextdeltatime
        ...
        sts = MyMPIAccessDEC->allToAllTime(sendbuf, sendcount, MPI_INT, recvbuf, recvcount, MPI_INT);
        ...
        {
            if (recvbuf[i] !=
                (i / datamsglength) * 1000000 + myrank * 1000 + myrank * datamsglength + (i % datamsglength))
            {
                badrecvbuf = true;
                debugStream << "test_AllToAllTimeDEC" << myrank << " recvbuf[" << i << "] " << recvbuf[i] << " # "
                            << (i / datamsglength) * 1000000 + myrank * 1000 + myrank * datamsglength +
                                   (i % datamsglength)
                            << endl;
            }
            else if (badrecvbuf)
        ...
        if (badrecvbuf)
        {
            ostringstream strstream;
            strstream << "==============================================================" << endl
                      << "test_AllToAllTimeDEC" << myrank << " badrecvbuf" << endl
                      << "=============================================================" << endl;
            debugStream << strstream.str() << endl;
            CPPUNIT_FAIL(strstream.str());
        }
    ...
    if (sts != MPI_SUCCESS)
    {
        ostringstream strstream;
        strstream << "================================================================" << endl
                  << "test_AllToAllTimeDEC" << myrank << " final CheckSent ERROR" << endl
                  << "================================================================" << endl;
        debugStream << strstream.str() << endl;
        CPPUNIT_FAIL(strstream.str());
    }
    ...
    debugStream << "test_AllToAllTimeDEC" << myrank << " final SendRequestIds " << nSendReq << " SendRequests" << endl;
    if (nSendReq)
    {
    ...
    if (nRecvReq)
    {
        ostringstream strstream;
        strstream << "===============================================================" << endl
                  << "test_AllToAllTimeDEC" << myrank << " RecvRequestIds " << nRecvReq << " RecvRequests # 0 Error"
                  << endl
                  << "===============================================================" << endl;
        debugStream << strstream.str() << endl;
        CPPUNIT_FAIL(strstream.str());
    ...
    {
        debugStream << "test_AllToAllTimeDEC" << myrank << " RecvRequestIds " << nRecvReq << " RecvRequests = 0 OK"
                    << endl;
    }
