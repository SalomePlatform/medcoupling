...
void
MPIAccessTest::test_MPI_Access_Cyclic_Send_Recv()
{
    ...
    if (myrank == 0)
    {
        ...
        debugStream << "test" << myrank << " Send RequestId " << RequestId[i] << " tag "
    ...
    for (i = 0; i < 10; i++)
    {
        ...
        {
            outcount = 0;
            sts = mpi_access.recv(&recvbuf, 1, MPI_INT, allsource[myrank], RequestId[i], &outcount);
        }
        else
        {
        ...
        debugStream << "test" << myrank << " Recv RequestId " << RequestId[i] << " tag "
                    << mpi_access.recvMPITag(allsource[myrank]) << " outcount " << outcount << endl;
    ...
    int sendrequests[10];
    int sendreqsize = mpi_access.sendRequestIds(alltarget[myrank], 10, sendrequests);
    if (sendreqsize != 0)
    {
        ostringstream strstream;
        strstream << "=========================================================" << endl
    ...
    int recvrequests[10];
    int recvreqsize = mpi_access.sendRequestIds(allsource[myrank], 10, recvrequests);
    if (recvreqsize != 0)
    {
        ostringstream strstream;
        strstream << "=========================================================" << endl
