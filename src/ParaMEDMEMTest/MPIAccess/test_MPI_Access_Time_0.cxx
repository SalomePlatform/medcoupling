...
void
chksts(int sts, int myrank, MEDCoupling::MPIAccess *mpi_access)
{
...
void
MPIAccessTest::test_MPI_Access_Time_0()
{
...
#define maxreq 100
    ...
    for (t = 0; t < maxt; t = t + dt[myrank])
    {
        ...
        if (myrank == 0)
        {
            ...
            sts = mpi_access->ISend(&aSendTimeMsg[istep], 1, mpi_access->timeType(), target, SendTimeRequestId[istep]);
            debugStream << "test" << myrank << " ISend TimeRequestId " << SendTimeRequestId[istep] << " tag "
                        << mpi_access->MPITag(SendTimeRequestId[istep]) << endl;
            chksts(sts, myrank, mpi_access);
            sendbuf[istep] = istep;
            sts = mpi_access->ISend(&sendbuf[istep], 1, MPI_INT, target, SendRequestId[istep]);
            debugStream << "test" << myrank << " ISend Data RequestId " << SendRequestId[istep] << " tag "
                        << mpi_access->MPITag(SendRequestId[istep]) << endl;
            chksts(sts, myrank, mpi_access);
            // CheckSent
            //=========
            ...
            for (j = 0; j < sendreqsize; j++)
            {
                ...
                if (flag)
                {
                    ...
                    debugStream << "test" << myrank << " " << j << ". " << sendrequests[j] << " sendrequest deleted"
                                << endl;
                }
            }
        }
        else
        {
            ...
            if (t == 0)
            {
                ...
                sts = mpi_access->IRecv(
                    &aRecvTimeMsg[lasttime + 1], 1, mpi_access->timeType(), target, RecvTimeRequestId[lasttime + 1]
                );
                debugStream << "test" << myrank << " t == 0 IRecv TimeRequestId " << RecvTimeRequestId[lasttime + 1]
                            << " MPITag " << mpi_access->MPITag(RecvTimeRequestId[lasttime + 1]) << " MPICompleted "
            ...
            else
            {
                ...
                if (lasttime != -1)
                {
                    if (t <= aRecvTimeMsg[lasttime - 1].time)
                    {
                        ...
                        strstream << "===========================================================" << endl
                                  << "test" << myrank << " t " << t << " <= "
                                  << "aRecvTimeMsg[ " << lasttime << "-1 ].time " << aRecvTimeMsg[lasttime - 1].time
                                  << " KO" << endl
                                  << "===========================================================" << endl;
                        debugStream << strstream.str() << endl;
                        CPPUNIT_FAIL(strstream.str());
                ...
                debugStream << "test" << myrank << " while outtime( " << outtime << " && t " << t << " > aRecvTimeMsg[ "
                            << lasttime << " ] " << aRecvTimeMsg[lasttime].time << " )" << endl;
                while (!outtime && (t > aRecvTimeMsg[lasttime].time))
                {
                    ...
                    chksts(sts, myrank, mpi_access);
                    debugStream << "test" << myrank << " Wait done RecvTimeRequestId " << RecvTimeRequestId[lasttime]
                                << " lasttime " << lasttime << " tag "
                                << mpi_access->MPITag(RecvTimeRequestId[lasttime]) << aRecvTimeMsg[lasttime] << endl;
                    if (lasttime == 0)
                    {
                    ...
                    debugStream << "test" << myrank << " t " << t << " lasttime " << lasttime << " deltatime "
                                << deltatime << " nexttime " << nexttime << endl;
                    ...
                    if (deltatime != 0 && t > nexttime)
                    {
                        ...
                        while (deltatime != 0 && t > nexttime)
                        {
                            ...
                            sts = mpi_access->probe(target, source, MPITag, datatype, outcount);
                            chksts(sts, myrank, mpi_access);
                            // Cancel DataMessages jusqu'a un TimeMessage
                            int cancelflag;
                            while (!mpi_access->isTimeMessage(MPITag))
                            {
                                sts = mpi_access->cancel(
                                    source,
                                    MPITag,
                                    datatype,
                                    outcount,
                                ...
                                debugStream << "test" << myrank << " Recv TO CANCEL RequestId "
                                            << RecvRequestId[lasttime] << " tag " << mpi_access->recvMPITag(target)
                    ...
                    else
                    {
                        // DoRecv
                        //======
                        debugStream << "test" << myrank << " Recv target " << target << " lasttime " << lasttime
                                    << " lasttime-1 " << aRecvTimeMsg[lasttime - 1] << " lasttime "
                                    << aRecvTimeMsg[lasttime] << endl;
                        sts = mpi_access->recv(&recvbuf[lasttime], 1, MPI_INT, target, RecvRequestId[lasttime]);
                        debugStream << "test" << myrank << " Recv RequestId " << RecvRequestId[lasttime] << " tag "
                    ...
                    if (!outtime)
                    {
                        ...
                        sts = mpi_access->IRecv(
                            &aRecvTimeMsg[lasttime + 1],
                            1,
                            mpi_access->timeType(),
                            target,
                            RecvTimeRequestId[lasttime + 1]
                        );
                        debugStream << "test" << myrank << " IRecv TimeRequestId " << RecvTimeRequestId[lasttime + 1]
                                    << " MPITag " << mpi_access->MPITag(RecvTimeRequestId[lasttime + 1])
                                    << " MPICompleted " << mpi_access->MPICompleted(RecvTimeRequestId[lasttime + 1])
                                    << endl;
                        chksts(sts, myrank, mpi_access);
                    }
                ...
                if (((t <= aRecvTimeMsg[lasttime - 1].time) || (t > aRecvTimeMsg[lasttime].time)) && !outtime)
                {
                    ostringstream strstream;
                    strstream << "===========================================================" << endl
                              << "test" << myrank << " t " << t << " <= "
                              << "aRecvTimeMsg[ " << lasttime << "-1 ].time " << aRecvTimeMsg[lasttime - 1].time
                              << " ou t " << t << " > "
                              << "aRecvTimeMsg[ " << lasttime << " ].time " << aRecvTimeMsg[lasttime].time << endl
                              << " ou bien outtime " << outtime << " KO RequestTimeIds "
                              << RecvTimeRequestId[lasttime - 1] << " " << RecvTimeRequestId[lasttime] << " RequestIds "
                ...
                {
                    debugStream << "===========================================================" << endl
                                << "test" << myrank << " aRecvTimeMsg[ " << lasttime << "-1 ].time "
                                ...
                                << " RequestIds " << RecvRequestId[lasttime - 1] << " " << RecvRequestId[lasttime]
                                << endl
                                << "===========================================================" << endl;
                }
            }
    ...
    if (myrank == 0)
    {
        ...
        for (j = 0; j < sendreqsize; j++)
        {
            ...
            chksts(sts, myrank, mpi_access);
            mpi_access->deleteRequest(sendrequests[j]);
            debugStream << "test" << myrank << " " << j << ". " << sendrequests[j] << " deleted" << endl;
        }
    }
    else
    {
        ...
        MPI_Datatype datatype;
        sts = mpi_access->IProbe(target, source, MPITag, datatype, outcount, flag);
        chksts(sts, myrank, mpi_access);
        while (flag)
        {
            sts = mpi_access->cancel(
            ...
            debugStream << "test" << myrank << " TO CANCEL RequestId " << RecvRequestId[lasttime] << " tag "
                        << mpi_access->recvMPITag(target) << " cancelflag " << cancelflag << endl;
            chksts(sts, myrank, mpi_access);
            sts = mpi_access->IProbe(target, source, MPITag, datatype, outcount, flag);
            chksts(sts, myrank, mpi_access);
        }
    ...
    {
        debugStream << "test" << myrank << " " << i << ". RecvTimeMsg " << aRecvTimeMsg[i].time << " recvbuf "
                    << recvbuf[i] << endl;
    }
