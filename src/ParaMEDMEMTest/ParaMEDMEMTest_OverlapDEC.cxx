...
//
void
prepareData1(int rank, NatureOfField nature, MEDCouplingFieldDouble *&fieldS, MEDCouplingFieldDouble *&fieldT)
{
    if (rank == 0)
    {
        ...
        DADouble arr = DataArrayDouble::New();
        arr->alloc(meshS->getNumberOfCells(), 1);
        fieldS->setMesh(meshS);
        fieldS->setArray(arr);
        ...
        DADouble arr2 = DataArrayDouble::New();
        arr2->alloc(meshT->getNumberOfCells(), 1);
        fieldT->setMesh(meshT);
        fieldT->setArray(arr2);
...
void
prepareData2_buildOneSquare(MEDCouplingUMesh *&meshS_0, MEDCouplingUMesh *&meshT_0)
{
...
prepareData2(
    int rank,
    ProcessorGroup *grp,
    NatureOfField nature,
    MEDCouplingUMesh *&meshS,
    MEDCouplingUMesh *&meshT,
    ParaMESH *&parameshS,
    ParaMESH *&parameshT,
    ParaFIELD *&parafieldS,
    ParaFIELD *&parafieldT,
...
)
{
    ...
    if (rank == 0)
    {
        ...
        vec.push_back(meshS_0);
        vec.push_back(meshS_1);
        ...
        vec.clear();
        vec.push_back(meshT_0);
        vec.push_back(meshT_3);
    ...
    if (rank == 1)
    {
        ...
        std::vector<const MEDCouplingUMesh *> vec;
        vec.push_back(meshS_3);
        vec.push_back(meshS_4);
        meshS = MEDCouplingUMesh::MergeUMeshes(vec);
        meshS_3->decrRef();
        meshS_4->decrRef();
        ...
        vec.clear();
        vec.push_back(meshT_1);
        vec.push_back(meshT_2);
        vec.push_back(meshT_4);
        meshT = MEDCouplingUMesh::MergeUMeshes(vec);
        meshT_1->decrRef();
        meshT_2->decrRef();
        meshT_4->decrRef();
 ...
 *  WARNING: bounding boxes might be tweaked here to make the case more interesting (i.e. to avoid an all to all
 * exchange between all procs).
 */
void
testOverlapDEC_generic(int workSharingAlgo, double bbAdj)
{
    int size, rank;
    MPI_Comm_size(MPI_COMM_WORLD, &size);
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    ...
    int nproc = 3;
    std::set<int> procs;
    for (int i = 0; i < nproc; i++) procs.insert(i);
...
void
ParaMEDMEMTest::testOverlapDEC1()
{
    /*!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
void
ParaMEDMEMTest::testOverlapDEC1_bis()
{
...
void
ParaMEDMEMTest::testOverlapDEC1_ter()
{
    // Same BB hack as above
 ...
 */
void
ParaMEDMEMTest::testOverlapDEC2()
{
    testOverlapDEC_generic(0, 1.0e-12);
}

void
ParaMEDMEMTest::testOverlapDEC2_bis()
{
    testOverlapDEC_generic(1, 1.0e-12);
}

void
ParaMEDMEMTest::testOverlapDEC2_ter()
{
    testOverlapDEC_generic(2, 1.0e-12);
}
 ...
 */
void
ParaMEDMEMTest::testOverlapDEC3()
{
    int size, rank;
    MPI_Comm_size(MPI_COMM_WORLD, &size);
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    ...
    if (rank == 0)
    {
        ...
        for (int i = 0; i < 4; i++) CPPUNIT_ASSERT_DOUBLES_EQUAL(1.0, resField->getArray()->getIJ(i, 0), 1e-12);
        for (int i = 4; i < 8; i++) CPPUNIT_ASSERT_DOUBLES_EQUAL(4.0, resField->getArray()->getIJ(i, 0), 1e-12);
    }
    if (rank == 1)
    {
        CPPUNIT_ASSERT_EQUAL(12, (int)resField->getNumberOfTuples());
        for (int i = 0; i < 4; i++) CPPUNIT_ASSERT_DOUBLES_EQUAL(2.0, resField->getArray()->getIJ(i, 0), 1e-12);
        for (int i = 4; i < 8; i++) CPPUNIT_ASSERT_DOUBLES_EQUAL(3.0, resField->getArray()->getIJ(i, 0), 1e-12);
        for (int i = 8; i < 12; i++) CPPUNIT_ASSERT_DOUBLES_EQUAL(5.0, resField->getArray()->getIJ(i, 0), 1e-12);
    }
    delete parafieldS;
    delete parafieldT;
    delete parameshS;
 ...
 */
void
ParaMEDMEMTest::testOverlapDEC4()
{
    int size, rank;
    MPI_Comm_size(MPI_COMM_WORLD, &size);
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    ...
    prepareData2(rank, grp, IntensiveMaximum, meshS, meshT, parameshS, parameshT, parafieldS, parafieldT, true, 2);
    //  if (rank == 1)
    //    {
    //      int i=1, j=0;
    //      while (i!=0)
