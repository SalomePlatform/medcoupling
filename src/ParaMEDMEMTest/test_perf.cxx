...
void
get_time(float *telps, float *tuser, float *tsys, float *tcpu);
...
main(int argc, char *argv[])
{
    ...
    if (count != 4)
    {
        cout << "usage test_perf -f1 filename1 -m1 meshname1 -f2 filename2 -m2 meshname2 (-ns nproc_source -eps "
                "epsilon -tri -all)"
             << endl;
        exit(0);
    }
...
void
testInterpKernelDEC_2D(
    const string &filename_xml1,
    const string &meshname1,
    const string &filename_xml2,
    const string &meshname2,
    int nproc_source,
    double epsilon,
    bool tri,
...
)
{
    ...
    if (source_group->containsMyRank())
    {
        ...
        get_time(&telps, &tcpu_u, &tcpu_s, &tcpu);
        if (rank == 0)
            cout << "IO : Telapse = " << telps << " TuserCPU = " << tcpu_u << " TsysCPU = " << tcpu_s
                 << " TCPU = " << tcpu << endl;
        mesh->incrRef();
    ...
    if (source_group->containsMyRank())
    {
        ...
        get_time(&telps, &tcpu_u, &tcpu_s, &tcpu);
        if (rank == 0)
            cout << "SYNCHRONIZE : Telapse = " << telps << " TuserCPU = " << tcpu_u << " TsysCPU = " << tcpu_s
                 << " TCPU = " << tcpu << endl;
        cout << "DEC usage" << endl;
        ...
        get_time(&telps, &tcpu_u, &tcpu_s, &tcpu);
        if (rank == 0)
            cout << "SEND DATA : Telapse = " << telps << " TuserCPU = " << tcpu_u << " TsysCPU = " << tcpu_s
                 << " TCPU = " << tcpu << endl;
        dec.recvData();
    ...
    get_time(&telps, &tcpu_u, &tcpu_s, &tcpu);
    if (rank == 0)
        cout << "RECV DATA : Telapse = " << telps << " TuserCPU = " << tcpu_u << " TsysCPU = " << tcpu_s
             << " TCPU = " << tcpu << endl;
