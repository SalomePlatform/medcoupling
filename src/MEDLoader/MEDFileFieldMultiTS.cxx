...
extern INTERP_KERNEL::NormalizedCellType
ConvertGeometryType(med_geometry_type geotype);
...
std::size_t
MEDFileAnyTypeFieldMultiTSWithoutSDA::getHeapMemorySizeWithoutChildren() const
{
    std::size_t ret(
        _mesh_name.capacity() + _name.capacity() + _infos.capacity() * sizeof(std::string) +
        _time_steps.capacity() * sizeof(MCAuto<MEDFileField1TSWithoutSDA>)
    );
...
MEDFileAnyTypeFieldMultiTSWithoutSDA::getDirectChildrenWithNull() const
{
    std::vector<const BigMemoryObject *> ret;
    for (std::vector<MCAuto<MEDFileAnyTypeField1TSWithoutSDA> >::const_iterator it = _time_steps.begin();
         it != _time_steps.end();
         it++)
...
/*!
 * If one of the id in [ \a startIds , \a endIds ) points to a null element, there is not throw. Simply, this empty
 * element is added as if it were not NULL.
 */
MEDFileAnyTypeFieldMultiTSWithoutSDA *
MEDFileAnyTypeFieldMultiTSWithoutSDA::buildFromTimeStepIds(const int *startIds, const int *endIds) const
{
    ...
    for (const int *id = startIds; id != endIds; id++)
    {
        ...
        else
        {
            ...
            oss << "MEDFileAnyTypeFieldMultiTSWithoutSDA::buildFromTimeStepIds : At pos #"
                << std::distance(startIds, id) << " value is " << *id;
...
/*!
 * If one of the id in the input range points to a null element, there is not throw. Simply, this empty element is added
 * as if it were not NULL.
 */
MEDFileAnyTypeFieldMultiTSWithoutSDA *
MEDFileAnyTypeFieldMultiTSWithoutSDA::buildFromTimeStepIds2(int bg, int end, int step) const
{
    ...
    for (int i = 0; i < nbOfEntriesToKeep; i++, j += step)
    {
        ...
        {
            std::ostringstream oss;
            oss << "MEDFileAnyTypeFieldMultiTSWithoutSDA::buildFromTimeStepIds : At pos #" << i << " value is " << j;
            oss << " ! Should be in [0," << sz << ") !";
            throw INTERP_KERNEL::Exception(oss.str());
        }
    }
...
MEDFileAnyTypeFieldMultiTSWithoutSDA *
MEDFileAnyTypeFieldMultiTSWithoutSDA::partOfThisLyingOnSpecifiedTimeSteps(
    const std::vector<std::pair<int, int> > &timeSteps
) const
{
    int id = 0;
    MCAuto<DataArrayInt> ids = DataArrayInt::New();
    ids->alloc(0, 1);
    for (std::vector<MCAuto<MEDFileAnyTypeField1TSWithoutSDA> >::const_iterator it = _time_steps.begin();
         it != _time_steps.end();
         it++, id++)
...
bool
MEDFileAnyTypeFieldMultiTSWithoutSDA::presenceOfStructureElements() const
{
    for (std::vector<MCAuto<MEDFileAnyTypeField1TSWithoutSDA> >::const_iterator it = _time_steps.begin();
         it != _time_steps.end();
         it++)
...
bool
MEDFileAnyTypeFieldMultiTSWithoutSDA::onlyStructureElements() const
{
    for (std::vector<MCAuto<MEDFileAnyTypeField1TSWithoutSDA> >::const_iterator it = _time_steps.begin();
         it != _time_steps.end();
         it++)
...
void
MEDFileAnyTypeFieldMultiTSWithoutSDA::killStructureElements()
{
    std::vector<MCAuto<MEDFileAnyTypeField1TSWithoutSDA> > ret;
    for (std::vector<MCAuto<MEDFileAnyTypeField1TSWithoutSDA> >::iterator it = _time_steps.begin();
         it != _time_steps.end();
         it++)
        if ((*it).isNotNull())
        {
...
void
MEDFileAnyTypeFieldMultiTSWithoutSDA::keepOnlyStructureElements()
{
    std::vector<MCAuto<MEDFileAnyTypeField1TSWithoutSDA> > ret;
    for (std::vector<MCAuto<MEDFileAnyTypeField1TSWithoutSDA> >::iterator it = _time_steps.begin();
         it != _time_steps.end();
...
void
MEDFileAnyTypeFieldMultiTSWithoutSDA::keepOnlyOnSE(const std::string &seName)
{
    std::vector<MCAuto<MEDFileAnyTypeField1TSWithoutSDA> > ret;
    for (std::vector<MCAuto<MEDFileAnyTypeField1TSWithoutSDA> >::iterator it = _time_steps.begin();
         it != _time_steps.end();
...
void
MEDFileAnyTypeFieldMultiTSWithoutSDA::getMeshSENames(std::vector<std::pair<std::string, std::string> > &ps) const
{
    ...
    if (ps2.empty())
        throw INTERP_KERNEL::Exception(
            "MEDFileAnyTypeFieldMultiTSWithoutSDA::getMeshSENames : this appears to not contain SE only !"
        );
    for (std::vector<MCAuto<MEDFileAnyTypeField1TSWithoutSDA> >::const_iterator it = _time_steps.begin();
         it != _time_steps.end();
         it++)
        if ((*it).isNotNull())
        {
            ...
            if (ps2 != ps3)
                throw INTERP_KERNEL::Exception(
                    "MEDFileAnyTypeFieldMultiTSWithoutSDA::getMeshSENames : For the moment only homogeneous SE def "
                    "through time managed !"
                );
        }
...
bool
MEDFileAnyTypeFieldMultiTSWithoutSDA::presenceOfMultiDiscPerGeoType() const
{
    for (std::vector<MCAuto<MEDFileAnyTypeField1TSWithoutSDA> >::const_iterator it = _time_steps.begin();
         it != _time_steps.end();
         it++)
...
MEDFileAnyTypeFieldMultiTSWithoutSDA::getInfo() const
{
    return _infos;
}

void
MEDFileAnyTypeFieldMultiTSWithoutSDA::setInfo(const std::vector<std::string> &info)
{
    _infos = info;
}

int
MEDFileAnyTypeFieldMultiTSWithoutSDA::getTimeStepPos(int iteration, int order) const
{
    int ret = 0;
    for (std::vector<MCAuto<MEDFileAnyTypeField1TSWithoutSDA> >::const_iterator it = _time_steps.begin();
         it != _time_steps.end();
    ...
    std::ostringstream oss;
    oss << "MEDFileFieldMultiTS::getTimeStepPos : Muli timestep field on time (" << iteration << "," << order
        << ") does not exist ! Available (iteration,order) are :\n";
...
MEDFileAnyTypeFieldMultiTSWithoutSDA::getTimeStepEntry(int iteration, int order) const
{
    return *_time_steps[getTimeStepPos(iteration, order)];
}

MEDFileAnyTypeField1TSWithoutSDA &
MEDFileAnyTypeFieldMultiTSWithoutSDA::getTimeStepEntry(int iteration, int order)
{
    return *_time_steps[getTimeStepPos(iteration, order)];
}

bool
MEDFileAnyTypeFieldMultiTSWithoutSDA::changeMeshNames(const std::vector<std::pair<std::string, std::string> > &modifTab)
{
    bool ret(false);
 ...
 */
DataArray *
MEDFileAnyTypeFieldMultiTSWithoutSDA::getUndergroundDataArray(int iteration, int order) const
{
 ...
 */
DataArray *
MEDFileAnyTypeFieldMultiTSWithoutSDA::getUndergroundDataArrayExt(
    int iteration,
    int order,
...
bool
MEDFileAnyTypeFieldMultiTSWithoutSDA::renumberEntitiesLyingOnMesh(
    const std::string &meshName,
...
void
MEDFileAnyTypeFieldMultiTSWithoutSDA::accept(MEDFileFieldVisitor &visitor) const
{
    for (std::vector<MCAuto<MEDFileAnyTypeField1TSWithoutSDA> >::const_iterator it = _time_steps.begin();
         it != _time_steps.end();
         it++)
...
void
MEDFileAnyTypeFieldMultiTSWithoutSDA::simpleRepr(int bkOffset, std::ostream &oss, int fmtsId) const
{
    ...
    oss << startLine << "Field multi time steps has " << _infos.size()
        << " components with the following infos :" << std::endl;
    for (std::vector<std::string>::const_iterator it = _infos.begin(); it != _infos.end(); it++)
    {
...
MEDFileAnyTypeFieldMultiTSWithoutSDA::getTimeSteps(std::vector<double> &ret1) const
{
    std::size_t sz = _time_steps.size();
    ...
    for (std::size_t i = 0; i < sz; i++)
    {
        ...
        {
            std::ostringstream oss;
            oss << "MEDFileFieldMultiTSWithoutSDA::getTimeSteps : At rank #" << i
                << " time step is not defined. Invoke eraseEmptyTS method !";
            throw INTERP_KERNEL::Exception(oss.str());
        }
    }
    return ret;
...
void
MEDFileAnyTypeFieldMultiTSWithoutSDA::pushBackTimeStep(MCAuto<MEDFileAnyTypeField1TSWithoutSDA> &tse)
{
    MEDFileAnyTypeField1TSWithoutSDA *tse2(tse);
    if (!tse2)
        throw INTERP_KERNEL::Exception(
            "MEDFileAnyTypeFieldMultiTSWithoutSDA::pushBackTimeStep : input content object is null !"
        );
...
void
MEDFileAnyTypeFieldMultiTSWithoutSDA::synchronizeNameScope()
{
         ...
         it++)
    {
        ...
        if (cur)
        {
            ...
            {
                std::ostringstream oss;
                oss << "MEDFileAnyTypeFieldMultiTSWithoutSDA::synchronizeNameScope : Mismatch in the number of "
                       "components of parts ! Should be "
                    << nbOfCompo;
                oss << " ! but the field at iteration=" << cur->getIteration() << " order=" << cur->getOrder()
                    << " has " << (cur->getInfo()).size() << " components !";
                throw INTERP_KERNEL::Exception(oss.str());
            }
...
void
MEDFileAnyTypeFieldMultiTSWithoutSDA::loadStructureOrStructureAndBigArraysRecursively(
    med_idt fid,
    int nbPdt,
    med_field_type fieldTyp,
    bool loadAll,
...
)
{
    ...
    {  // no entities
        ...
        for (int i = 0; i < nentitype; i++)
        {
            ...
            MEDFILESAFECALLERRD0(
                MEDfieldGeometryType, (fid, _name.c_str(), MED_ALL_DT, MED_ALL_IT, types[i], geotypes, &geousedbyncs)
            );
            for (med_int j = 0; j < ngeotype; j++)
            {
                std::pair<TypeOfField, INTERP_KERNEL::NormalizedCellType> p(
                    MEDFileMesh::ConvertFromMEDFileEntity(types[i]), ConvertGeometryType(geotypes[j])
                );
    ...
    for (int i = 0; i < nbPdt; i++)
    {
        ...
        switch (fieldTyp)
        {
            ...
            default:
                throw INTERP_KERNEL::Exception(
                    "MEDFileAnyTypeFieldMultiTSWithoutSDA::loadStructureOrStructureAndBigArraysRecursively : managed "
                    "field type are : FLOAT64, INT32, FLOAT32, INT64 !"
                );
        }
        if (loadAll)
...
void
MEDFileAnyTypeFieldMultiTSWithoutSDA::writeLL(med_idt fid, const MEDFileWritable &opts) const
{
    if (_time_steps.empty())
    ...
    if (_name.empty())
        throw INTERP_KERNEL::Exception(
            "MEDFileFieldMultiTSWithoutSDA::write : MED file does not accept field with empty name !"
        );
...
void
MEDFileAnyTypeFieldMultiTSWithoutSDA::readDescription(med_idt fid, const MEDFileWritable &opts)
{
    for (std::vector<MCAuto<MEDFileAnyTypeField1TSWithoutSDA> >::iterator it = _time_steps.begin();
         it != _time_steps.end();
         it++)
...
void
MEDFileAnyTypeFieldMultiTSWithoutSDA::writeDescription(med_idt fid, const MEDFileWritable &opts) const
{
    for (std::vector<MCAuto<MEDFileAnyTypeField1TSWithoutSDA> >::const_iterator it = _time_steps.begin();
         it != _time_steps.end();
         it++)
...
void
MEDFileAnyTypeFieldMultiTSWithoutSDA::loadBigArraysRecursively(med_idt fid, const MEDFileFieldNameScope &nasc)
{
    for (std::vector<MCAuto<MEDFileAnyTypeField1TSWithoutSDA> >::iterator it = _time_steps.begin();
         it != _time_steps.end();
         it++)
...
void
MEDFileAnyTypeFieldMultiTSWithoutSDA::loadBigArraysRecursivelyIfNecessary(
    med_idt fid, const MEDFileFieldNameScope &nasc
)
{
    for (std::vector<MCAuto<MEDFileAnyTypeField1TSWithoutSDA> >::iterator it = _time_steps.begin();
         it != _time_steps.end();
...
void
MEDFileAnyTypeFieldMultiTSWithoutSDA::unloadArrays()
{
    for (std::vector<MCAuto<MEDFileAnyTypeField1TSWithoutSDA> >::iterator it = _time_steps.begin();
         it != _time_steps.end();
         it++)
...
int
MEDFileAnyTypeFieldMultiTSWithoutSDA::getNumberOfTS() const
{
    return (int)_time_steps.size();
}

void
MEDFileAnyTypeFieldMultiTSWithoutSDA::eraseEmptyTS()
{
...
void
MEDFileAnyTypeFieldMultiTSWithoutSDA::eraseTimeStepIds(const int *startIds, const int *endIds)
{
    std::vector<MCAuto<MEDFileAnyTypeField1TSWithoutSDA> > newTS;
    ...
    for (const int *id = startIds; id != endIds; id++, ii++)
    {
        ...
        {
            std::ostringstream oss;
            oss << "MEDFileFieldMultiTSWithoutSDA::eraseTimeStepIds : At pos #" << ii << " request for id=" << *id
                << " not in [0," << maxId << ") !";
            throw INTERP_KERNEL::Exception(oss.str());
        }
    }
...
void
MEDFileAnyTypeFieldMultiTSWithoutSDA::eraseTimeStepIds2(int bg, int end, int step)
{
    ...
    int j = bg;
    for (mcIdType i = 0; i < nbOfEntriesToKill; i++, j += step) b[j] = false;
    std::vector<MCAuto<MEDFileAnyTypeField1TSWithoutSDA> > newTS;
    for (std::size_t i = 0; i < sz; i++)
        if (b[i])
...
int
MEDFileAnyTypeFieldMultiTSWithoutSDA::getPosOfTimeStep(int iteration, int order) const
{
    int ret = 0;
    std::ostringstream oss;
    oss << "MEDFileFieldMultiTSWithoutSDA::getPosOfTimeStep : No such time step (" << iteration << "," << order
        << ") !\nPossibilities are : ";
    for (std::vector<MCAuto<MEDFileAnyTypeField1TSWithoutSDA> >::const_iterator it = _time_steps.begin();
         it != _time_steps.end();
         it++, ret++)
...
int
MEDFileAnyTypeFieldMultiTSWithoutSDA::getPosGivenTime(double time, double eps) const
{
    int ret = 0;
    std::ostringstream oss;
    oss << "MEDFileFieldMultiTSWithoutSDA::getPosGivenTime : No such time step " << time << "! \nPossibilities are : ";
    oss.precision(15);
    for (std::vector<MCAuto<MEDFileAnyTypeField1TSWithoutSDA> >::const_iterator it = _time_steps.begin();
         it != _time_steps.end();
         it++, ret++)
...
std::vector<std::pair<int, int> >
MEDFileAnyTypeFieldMultiTSWithoutSDA::getIterations() const
{
    std::size_t lgth = _time_steps.size();
    std::vector<std::pair<int, int> > ret(lgth);
    for (std::size_t i = 0; i < lgth; i++) _time_steps[i]->fillIteration(ret[i]);
    return ret;
}

/*!
 * This method has 3 inputs 'iteration' 'order' 'mname'. 'mname' can be null if the user is the general case where there
 * is only one meshName lying on 'this' This method returns two things.
 * - The absolute dimension of 'this' in first parameter.
 * - The available ext levels relative to the absolute dimension returned in first parameter. These relative levels are
 * relative to the first output parameter. The values in 'levs' will be returned in decreasing order.
 *
 * This method is designed for MEDFileFieldMultiTS instances that have a discritization ON_CELLS, ON_GAUSS_NE and
 * ON_GAUSS. Only these 3 discretizations will be taken into account here.
 *
 ...
 *
 * This method is useful to make the link between meshDimension of the underlying mesh in 'this' and the levels on
 * 'this'. It is possible (even if it is not common) that the highest level in 'this' were not equal to the
 * meshDimension of the underlying mesh in 'this'.
 *
 ...
 *
 * To retrieve the highest level of f1 it should be done,
 * f1->getFieldAtLevel(ON_CELLS,3-3+0);//absDim-meshDim+relativeLev To retrieve the lowest level of f1 it should be
 * done, f1->getFieldAtLevel(ON_CELLS,3-3+(-2));//absDim-meshDim+relativeLev To retrieve the highest level of f2 it
 * should be done, f1->getFieldAtLevel(ON_CELLS,2-3+0);//absDim-meshDim+relativeLev To retrieve the lowest level of f2
 * it should be done, f1->getFieldAtLevel(ON_CELLS,2-3+(-1));//absDim-meshDim+relativeLev
 */
int
MEDFileAnyTypeFieldMultiTSWithoutSDA::getNonEmptyLevels(
    int iteration, int order, const std::string &mname, std::vector<int> &levs
) const
{
...
const MEDFileAnyTypeField1TSWithoutSDA *
MEDFileAnyTypeFieldMultiTSWithoutSDA::getTimeStepAtPos2(int pos) const
{
    if (pos < 0 || pos >= (int)_time_steps.size())
    {
        std::ostringstream oss;
        oss << "MEDFileAnyTypeFieldMultiTSWithoutSDA::getTimeStepAtPos2 : request for pos #" << pos
            << " whereas should be in [0," << _time_steps.size() << ") !";
        throw INTERP_KERNEL::Exception(oss.str());
    }
    ...
    if (item == 0)
    {
        std::ostringstream oss;
        oss << "MEDFileAnyTypeFieldMultiTSWithoutSDA::getTimeStepAtPos2 : request for pos #" << pos
            << ", this pos id exists but the underlying Field1TS is null !";
...
MEDFileAnyTypeField1TSWithoutSDA *
MEDFileAnyTypeFieldMultiTSWithoutSDA::getTimeStepAtPos2(int pos)
{
    if (pos < 0 || pos >= (int)_time_steps.size())
    {
        std::ostringstream oss;
...
MEDFileAnyTypeFieldMultiTSWithoutSDA::getPflsReallyUsed2() const
{
...
std::vector<std::string>
MEDFileAnyTypeFieldMultiTSWithoutSDA::getLocsReallyUsed2() const
{
    std::vector<std::string> ret;
    std::set<std::string> ret2;
    for (std::vector<MCAuto<MEDFileAnyTypeField1TSWithoutSDA> >::const_iterator it = _time_steps.begin();
...
std::vector<std::string>
MEDFileAnyTypeFieldMultiTSWithoutSDA::getPflsReallyUsedMulti2() const
{
    std::vector<std::string> ret;
    for (std::vector<MCAuto<MEDFileAnyTypeField1TSWithoutSDA> >::const_iterator it = _time_steps.begin();
         it != _time_steps.end();
...
std::vector<std::string>
MEDFileAnyTypeFieldMultiTSWithoutSDA::getLocsReallyUsedMulti2() const
{
    std::vector<std::string> ret;
    for (std::vector<MCAuto<MEDFileAnyTypeField1TSWithoutSDA> >::const_iterator it = _time_steps.begin();
         it != _time_steps.end();
...
void
MEDFileAnyTypeFieldMultiTSWithoutSDA::changePflsRefsNamesGen2(
...
void
MEDFileAnyTypeFieldMultiTSWithoutSDA::changeLocsRefsNamesGen2(
    const std::vector<std::pair<std::vector<std::string>, std::string> > &mapOfModif
)
{
    for (std::vector<MCAuto<MEDFileAnyTypeField1TSWithoutSDA> >::iterator it = _time_steps.begin();
...
std::vector<std::vector<TypeOfField> >
MEDFileAnyTypeFieldMultiTSWithoutSDA::getTypesOfFieldAvailable() const
{
    std::size_t lgth = _time_steps.size();
    std::vector<std::vector<TypeOfField> > ret(lgth);
    for (std::size_t i = 0; i < lgth; i++) _time_steps[i]->fillTypesOfFieldAvailable(ret[i]);
    return ret;
}
...
MEDFileAnyTypeFieldMultiTSWithoutSDA::getFieldSplitedByType(
    int iteration,
    int order,
...
MEDFileAnyTypeFieldMultiTSWithoutSDA::splitComponents() const
{
    ...
    {
        ret[i] = shallowCpy();
        ret[i]->_infos.resize(1);
        ret[i]->_infos[0] = _infos[i];
    }
    for (std::size_t i = 0; i < sz2; i++)
    {
        ...
        {
            std::ostringstream oss;
            oss << "MEDFileAnyTypeFieldMultiTSWithoutSDA::splitComponents : At rank #" << i
                << " number of components is " << ret1.size() << " whereas it should be for all time steps " << sz
                << " !";
            throw INTERP_KERNEL::Exception(oss.str());
        }
        ts[i] = ret1;
    ...
    for (std::size_t i = 0; i < sz; i++)
        for (std::size_t j = 0; j < sz2; j++) ret[i]->_time_steps[j] = ts[j][i];
    return ret;
}
...
MEDFileAnyTypeFieldMultiTSWithoutSDA::splitDiscretizations() const
{
    ...
    for (std::size_t i = 0; i < sz; i++)
    {
        ...
        {
            std::ostringstream oss;
            oss << "MEDFileAnyTypeFieldMultiTSWithoutSDA::splitDiscretizations : time step #" << i << " is null !";
            throw INTERP_KERNEL::Exception(oss.str());
        }
    ...
    for (std::vector<std::vector<MCAuto<MEDFileAnyTypeField1TSWithoutSDA> > >::const_iterator it0 = items.begin();
         it0 != items.end();
         it0++)
        for (std::vector<MCAuto<MEDFileAnyTypeField1TSWithoutSDA> >::const_iterator it1 = (*it0).begin();
             it1 != (*it0).end();
             it1++)
        {
            ...
            if (ts.size() != 1)
                throw INTERP_KERNEL::Exception(
                    "MEDFileAnyTypeFieldMultiTSWithoutSDA::splitDiscretizations : it appears that the splitting of "
                    "MEDFileAnyTypeField1TSWithoutSDA::splitDiscretizations has returned invalid result !"
                );
    ...
    for (std::size_t i = 0; i < types.size(); i++)
    {
        ...
        for (std::vector<MCAuto<MEDFileAnyTypeField1TSWithoutSDA> >::iterator it1 = ret2[i].begin();
             it1 != ret2[i].end();
             it1++)
...
/*!
 * Contrary to splitDiscretizations method this method makes the hypothesis that the times series are **NOT** impacted
 * by the splitting of multi discretization.
 */
std::vector<MCAuto<MEDFileAnyTypeFieldMultiTSWithoutSDA> >
MEDFileAnyTypeFieldMultiTSWithoutSDA::splitMultiDiscrPerGeoTypes() const
{
    ...
    for (std::size_t i = 0; i < sz; i++)
    {
        ...
        {
            std::ostringstream oss;
            oss << "MEDFileAnyTypeFieldMultiTSWithoutSDA::splitMultiDiscrPerGeoTypes : time step #" << i
                << " is null !";
            throw INTERP_KERNEL::Exception(oss.str());
        }
        ...
        if (szOut == std::numeric_limits<std::size_t>::max())
            szOut = items[i].size();
        else if (items[i].size() != szOut)
            throw INTERP_KERNEL::Exception(
                "MEDFileAnyTypeFieldMultiTSWithoutSDA::splitMultiDiscrPerGeoTypes : The splitting per discretization "
                "is expected to be same among time steps !"
            );
    }
    if (szOut == std::numeric_limits<std::size_t>::max())
        throw INTERP_KERNEL::Exception(
            "MEDFileAnyTypeFieldMultiTSWithoutSDA::splitMultiDiscrPerGeoTypes : empty field !"
        );
    std::vector<MCAuto<MEDFileAnyTypeFieldMultiTSWithoutSDA> > ret(szOut);
    for (std::size_t i = 0; i < szOut; i++)
    {
        MCAuto<MEDFileAnyTypeFieldMultiTSWithoutSDA> elt(createNew());
        for (std::size_t j = 0; j < sz; j++) elt->pushBackTimeStep(items[j][i]);
        ret[i] = elt;
        elt->MEDFileFieldNameScope::operator=(*this);
    }
    return ret;
...
MEDFileAnyTypeFieldMultiTSWithoutSDA::copyTinyInfoFrom(const MEDCouplingFieldDouble *field, const DataArray *arr)
{
    ...
    if (_name.empty())
        throw INTERP_KERNEL::Exception(
            "MEDFileFieldMultiTSWithoutSDA::copyTinyInfoFrom : unsupported fields with no name in MED file !"
        );
    if (!arr)
...
void
MEDFileAnyTypeFieldMultiTSWithoutSDA::checkCoherencyOfTinyInfo(
    const MEDCouplingFieldDouble *field, const DataArray *arr
) const
{
...
void
MEDFileAnyTypeFieldMultiTSWithoutSDA::checkThatComponentsMatch(const std::vector<std::string> &compos) const
{
    static const char MSG[] = "MEDFileFieldMultiTSWithoutSDA::checkThatComponentsMatch : ";
    if (getInfo().size() != compos.size())
    {
        std::ostringstream oss;
        oss << MSG << "mismatch of number of components between this (" << getInfo().size() << ") and ";
    ...
    {
        std::ostringstream oss;
        oss << MSG << "components have same size but are different ! should be \"";
...
void
MEDFileAnyTypeFieldMultiTSWithoutSDA::checkThatNbOfCompoOfTSMatchThis() const
{
         ...
         it++, j++)
    {
        ...
        if (elt)
            ...
            {
                std::ostringstream oss;
                oss << "MEDFileAnyTypeFieldMultiTSWithoutSDA::checkThatNbOfCompoOfTSMatchThis : At pos #" << j
...
void
MEDFileAnyTypeFieldMultiTSWithoutSDA::appendFieldNoProfileSBT(
    const MEDCouplingFieldDouble *field, const DataArray *arr, MEDFileFieldGlobsReal &glob
)
{
    if (!field)
        throw INTERP_KERNEL::Exception(
            "MEDFileAnyTypeFieldMultiTSWithoutSDA::appendFieldNoProfileSBT : input field is NULL !"
        );
    if (!_time_steps.empty())
...
)
{
    ...
    MCAuto<MEDFileAnyTypeField1TSWithoutSDA> obj(createNew1TSWithoutSDAEmptyInstance());
    {
        ...
        obj->setFieldProfile(
            field->timeDiscrSafe(), ft, arr, mesh, meshDimRelToMax, profile, glob, *this, smartPflKiller
        );
    }
    copyTinyInfoFrom(field, arr);
...
void
MEDFileAnyTypeFieldMultiTSWithoutSDA::setIteration(int i, MCAuto<MEDFileAnyTypeField1TSWithoutSDA> ts)
{
    int sz = (int)_time_steps.size();
    if (i < 0 || i >= sz)
    {
        std::ostringstream oss;
        oss << "MEDFileAnyTypeFieldMultiTSWithoutSDA::setIteration : trying to set element at place #" << i
            << " should be in [0," << sz << ") !";
        throw INTERP_KERNEL::Exception(oss.str());
    }
    ...
    if (tsPtr)
    {
        ...
        {
            std::ostringstream oss;
            oss << "MEDFileAnyTypeFieldMultiTSWithoutSDA::setIteration : trying to set element with "
                << tsPtr->getNumberOfComponents() << " components ! Should be " << _infos.size() << " !";
            throw INTERP_KERNEL::Exception(oss.str());
        }
    }
    _time_steps[i] = ts;
 ...
 * are extracted (created) specially for the call of this method. That's why the DataArrayDouble instance in returned
 * vector of vector should be dealed by the caller.
 */
std::vector<std::vector<DataArrayDouble *> >
MEDFileFieldMultiTSWithoutSDA::getFieldSplitedByType2(
    int iteration,
    int order,
    const std::string &mname,
    std::vector<INTERP_KERNEL::NormalizedCellType> &types,
...
) const
{
    ...
    if (!myF1TSC)
        throw INTERP_KERNEL::Exception(
            "MEDFileFieldMultiTSWithoutSDA::getFieldSplitedByType2 : mismatch of type of field expecting FLOAT64 !"
        );
    return myF1TSC->getFieldSplitedByType2(mname, types, typesF, pfls, locs);
}

MEDFileInt32FieldMultiTSWithoutSDA *
MEDFileFieldMultiTSWithoutSDA::convertToInt() const
{
         ...
         it++, i++)
    {
        ...
        if (eltToConv)
        {
            ...
            if (!eltToConvC)
                throw INTERP_KERNEL::Exception(
                    "MEDFileFieldMultiTSWithoutSDA::convertToInt : presence of an invalid 1TS type ! Should be of type "
                    "FLOAT64 !"
                );
...
MEDFileFieldMultiTSWithoutSDA::createNewWithSDA() const
{
...
MCAuto<MEDFileAnyTypeFieldMultiTS>
MEDFileInt32FieldMultiTSWithoutSDA::createNewWithSDA() const
{
...
MCAuto<MEDFileAnyTypeFieldMultiTS>
MEDFileInt64FieldMultiTSWithoutSDA::createNewWithSDA() const
{
...
MCAuto<MEDFileAnyTypeFieldMultiTS>
MEDFileFloatFieldMultiTSWithoutSDA::createNewWithSDA() const
{
...
MEDFileAnyTypeFieldMultiTSWithoutSDA *
MEDFileAnyTypeFieldMultiTS::BuildContentFrom(
...
)
{
    ...
    switch (typcha)
    {
        ...
        {
            std::ostringstream oss;
            oss << "MEDFileAnyTypeFieldMultiTS::BuildContentFrom(fid,fieldName) : file \'" << FileNameFromFID(fid)
                << "\' contains field with name \'" << fieldName
...
MEDFileAnyTypeFieldMultiTSWithoutSDA *
MEDFileAnyTypeFieldMultiTS::BuildContentFrom(med_idt fid, bool loadAll, const MEDFileMeshes *ms)
{
    med_field_type typcha;
    //
    std::vector<std::string> infos;
    ...
    switch (typcha)
    {
        ...
        {
            std::ostringstream oss;
            oss << "MEDFileAnyTypeFieldMultiTS::BuildContentFrom(fid) : file \'" << FileNameFromFID(fid)
                << "\' contains field with name \'" << fieldName
                << "\' but the type of the first field is not in [MED_FLOAT64, MED_INT32, MED_FLOAT32, MED_INT64] !";
            throw INTERP_KERNEL::Exception(oss.str());
        }
    }
    ret->setMeshName(meshName);
...
MEDFileAnyTypeFieldMultiTS *
MEDFileAnyTypeFieldMultiTS::BuildNewInstanceFromContent(MEDFileAnyTypeFieldMultiTSWithoutSDA *c)
{
    if (!c)
        throw INTERP_KERNEL::Exception(
            "MEDFileAnyTypeFieldMultiTS::BuildNewInstanceFromContent : empty content in input : unable to build a new "
            "instance !"
        );
    ...
    throw INTERP_KERNEL::Exception(
        "MEDFileAnyTypeFieldMultiTS::BuildNewInstanceFromContent : internal error ! a content of type different from "
        "FLOAT64 FLOAT32 INT32 and INT64 has been built but not intercepted !"
    );
}

MEDFileAnyTypeFieldMultiTS *
MEDFileAnyTypeFieldMultiTS::BuildNewInstanceFromContent(MEDFileAnyTypeFieldMultiTSWithoutSDA *c, med_idt fid)
{
 ...
 */
MEDFileAnyTypeFieldMultiTS *
MEDFileAnyTypeFieldMultiTS::New(const std::string &fileName, bool loadAll)
{
...
MEDFileAnyTypeFieldMultiTS *
MEDFileAnyTypeFieldMultiTS::New(med_idt fid, bool loadAll)
{
 ...
 */
MEDFileAnyTypeFieldMultiTS *
MEDFileAnyTypeFieldMultiTS::New(const std::string &fileName, const std::string &fieldName, bool loadAll)
{
    MEDFileUtilities::AutoFid fid(OpenMEDFileForRead(fileName));
...
MEDFileAnyTypeFieldMultiTS *
MEDFileAnyTypeFieldMultiTS::New(med_idt fid, const std::string &fieldName, bool loadAll)
{
 ...
 */
MEDFileAnyTypeFieldMultiTS::MEDFileAnyTypeFieldMultiTS(
    const MEDFileAnyTypeFieldMultiTSWithoutSDA &other, bool shallowCopyOfContent
)
{
...
MEDFileAnyTypeFieldMultiTSWithoutSDA *
MEDFileAnyTypeFieldMultiTS::contentNotNullBase()
{
...
const MEDFileAnyTypeFieldMultiTSWithoutSDA *
MEDFileAnyTypeFieldMultiTS::contentNotNullBase() const
{
...
std::vector<std::string>
MEDFileAnyTypeFieldMultiTS::getPflsReallyUsed() const
{
    return contentNotNullBase()->getPflsReallyUsed2();
}

std::vector<std::string>
MEDFileAnyTypeFieldMultiTS::getLocsReallyUsed() const
{
    return contentNotNullBase()->getLocsReallyUsed2();
}

std::vector<std::string>
MEDFileAnyTypeFieldMultiTS::getPflsReallyUsedMulti() const
{
    return contentNotNullBase()->getPflsReallyUsedMulti2();
}

std::vector<std::string>
MEDFileAnyTypeFieldMultiTS::getLocsReallyUsedMulti() const
{
    return contentNotNullBase()->getLocsReallyUsedMulti2();
}

void
MEDFileAnyTypeFieldMultiTS::changePflsRefsNamesGen(
    const std::vector<std::pair<std::vector<std::string>, std::string> > &mapOfModif
)
{
...
void
MEDFileAnyTypeFieldMultiTS::changeLocsRefsNamesGen(
    const std::vector<std::pair<std::vector<std::string>, std::string> > &mapOfModif
)
{
...
int
MEDFileAnyTypeFieldMultiTS::getNumberOfTS() const
{
    return contentNotNullBase()->getNumberOfTS();
}

void
MEDFileAnyTypeFieldMultiTS::eraseEmptyTS()
{
    contentNotNullBase()->eraseEmptyTS();
}

void
MEDFileAnyTypeFieldMultiTS::eraseTimeStepIds(const int *startIds, const int *endIds)
{
    contentNotNullBase()->eraseTimeStepIds(startIds, endIds);
}

void
MEDFileAnyTypeFieldMultiTS::eraseTimeStepIds2(int bg, int end, int step)
{
...
std::vector<std::pair<int, int> >
MEDFileAnyTypeFieldMultiTS::getIterations() const
{
    return contentNotNullBase()->getIterations();
}

void
MEDFileAnyTypeFieldMultiTS::pushBackTimeSteps(const std::vector<MEDFileAnyTypeField1TS *> &f1ts)
{
...
void
MEDFileAnyTypeFieldMultiTS::pushBackTimeSteps(MEDFileAnyTypeFieldMultiTS *fmts)
{
    if (!fmts)
...
void
MEDFileAnyTypeFieldMultiTS::pushBackTimeStep(MEDFileAnyTypeField1TS *f1ts)
{
    if (!f1ts)
        throw INTERP_KERNEL::Exception(
            "MEDFileAnyTypeFieldMultiTSWithoutSDA::pushBackTimeStep : input pointer is NULL !"
        );
...
void
MEDFileAnyTypeFieldMultiTS::synchronizeNameScope()
{
    contentNotNullBase()->synchronizeNameScope();
}

int
MEDFileAnyTypeFieldMultiTS::getPosOfTimeStep(int iteration, int order) const
{
    return contentNotNullBase()->getPosOfTimeStep(iteration, order);
}

int
MEDFileAnyTypeFieldMultiTS::getPosGivenTime(double time, double eps) const
{
    return contentNotNullBase()->getPosGivenTime(time, eps);
}

int
MEDFileAnyTypeFieldMultiTS::getNonEmptyLevels(
    int iteration, int order, const std::string &mname, std::vector<int> &levs
) const
{
...
std::vector<std::vector<TypeOfField> >
MEDFileAnyTypeFieldMultiTS::getTypesOfFieldAvailable() const
{
    return contentNotNullBase()->getTypesOfFieldAvailable();
}

std::vector<std::vector<std::pair<mcIdType, mcIdType> > >
MEDFileAnyTypeFieldMultiTS::getFieldSplitedByType(
    int iteration,
    int order,
    const std::string &mname,
    std::vector<INTERP_KERNEL::NormalizedCellType> &types,
...
std::string
MEDFileAnyTypeFieldMultiTS::getName() const
{
    return contentNotNullBase()->getName();
}

void
MEDFileAnyTypeFieldMultiTS::setName(const std::string &name)
{
    contentNotNullBase()->setName(name);
}

std::string
MEDFileAnyTypeFieldMultiTS::getDtUnit() const
{
    return contentNotNullBase()->getDtUnit();
}

void
MEDFileAnyTypeFieldMultiTS::setDtUnit(const std::string &dtUnit)
{
    contentNotNullBase()->setDtUnit(dtUnit);
}

void
MEDFileAnyTypeFieldMultiTS::simpleRepr(int bkOffset, std::ostream &oss, int fmtsId) const
{
    contentNotNullBase()->simpleRepr(bkOffset, oss, fmtsId);
}

std::vector<std::pair<int, int> >
MEDFileAnyTypeFieldMultiTS::getTimeSteps(std::vector<double> &ret1) const
{
...
void
MEDFileAnyTypeFieldMultiTS::setMeshName(const std::string &newMeshName)
{
    contentNotNullBase()->setMeshName(newMeshName);
}

bool
MEDFileAnyTypeFieldMultiTS::changeMeshNames(const std::vector<std::pair<std::string, std::string> > &modifTab)
{
    return contentNotNullBase()->changeMeshNames(modifTab);
}

const std::vector<std::string> &
MEDFileAnyTypeFieldMultiTS::getInfo() const
{
    return contentNotNullBase()->getInfo();
}

bool
MEDFileAnyTypeFieldMultiTS::presenceOfMultiDiscPerGeoType() const
{
    return contentNotNullBase()->presenceOfMultiDiscPerGeoType();
}

void
MEDFileAnyTypeFieldMultiTS::setInfo(const std::vector<std::string> &info)
{
    return contentNotNullBase()->setInfo(info);
}

int
MEDFileAnyTypeFieldMultiTS::getNumberOfComponents() const
{
...
void
MEDFileAnyTypeFieldMultiTS::writeLL(med_idt fid) const
{
 ...
 * This method should be called when a MEDFileAnyTypeFieldMultiTS::New constructor has been with false as the last
 ...
 */
void
MEDFileAnyTypeFieldMultiTS::loadArrays()
{
    if (getFileName().empty())
        throw INTERP_KERNEL::Exception(
            "MEDFileAnyTypeFieldMultiTS::loadArrays : the structure does not come from a file !"
        );
...
/*!
 * This method behaves as MEDFileAnyTypeFieldMultiTS::loadArrays does, the first call, if \a this was built using a file
 * without loading big arrays. But once data loaded once, this method does nothing.
 *
 * \throw If the fileName is not set or points to a non readable MED file.
 ...
 * MEDFileAnyTypeFieldMultiTS::unloadArraysWithoutDataLoss instead.
 *
 * \sa MEDFileAnyTypeFieldMultiTS::loadArrays, MEDFileAnyTypeFieldMultiTS::loadArraysIfNecessary,
 * MEDFileAnyTypeFieldMultiTS::unloadArraysWithoutDataLoss
 */
void
MEDFileAnyTypeFieldMultiTS::unloadArrays()
{
 ...
 * MEDFileAnyTypeFieldMultiTS::loadArraysIfNecessary. This method is useful to reduce \b safely amount of heap memory
...
std::string
MEDFileAnyTypeFieldMultiTS::simpleRepr() const
{
    std::ostringstream oss;
...
std::size_t
MEDFileAnyTypeFieldMultiTS::getHeapMemorySizeWithoutChildren() const
{
    return MEDFileFieldGlobsReal::getHeapMemorySizeWithoutChildren();
}

std::vector<const BigMemoryObject *>
MEDFileAnyTypeFieldMultiTS::getDirectChildrenWithNull() const
{
 ...
 */
std::vector<MCAuto<MEDFileAnyTypeFieldMultiTS> >
MEDFileAnyTypeFieldMultiTS::splitComponents() const
{
    const MEDFileAnyTypeFieldMultiTSWithoutSDA *content(_content);
    if (!content)
        throw INTERP_KERNEL::Exception(
            "MEDFileAnyTypeFieldMultiTS::splitComponents : no content in this ! Unable to split components !"
        );
...
/*!
 * This method returns as MEDFileAnyTypeFieldMultiTS new instances as number of discretizations over time steps in \a
 * this. The returned instances are shallow copied of \a this included globals that are shared with those contained in
 * \a this.
 */
std::vector<MCAuto<MEDFileAnyTypeFieldMultiTS> >
MEDFileAnyTypeFieldMultiTS::splitDiscretizations() const
{
    const MEDFileAnyTypeFieldMultiTSWithoutSDA *content(_content);
    if (!content)
        throw INTERP_KERNEL::Exception(
            "MEDFileAnyTypeFieldMultiTS::splitDiscretizations : no content in this ! Unable to split discretizations !"
        );
...
/*!
 * This method returns as MEDFileAnyTypeFieldMultiTS new instances as number of sub-discretizations over time steps in
 * \a this. The returned instances are shallow copied of \a this included globals that are shared with those contained
 * in \a this.
 */
std::vector<MCAuto<MEDFileAnyTypeFieldMultiTS> >
MEDFileAnyTypeFieldMultiTS::splitMultiDiscrPerGeoTypes() const
{
    const MEDFileAnyTypeFieldMultiTSWithoutSDA *content(_content);
    if (!content)
        throw INTERP_KERNEL::Exception(
            "MEDFileAnyTypeFieldMultiTS::splitMultiDiscrPerGeoTypes : no content in this ! Unable to split "
            "discretizations !"
        );
...
MCAuto<MEDFileAnyTypeFieldMultiTSWithoutSDA>
MEDFileAnyTypeFieldMultiTS::getContent()
{
    return _content;
}
 ...
 *  \return MEDFileField1TS * or MEDFileInt32Field1TS *- a new instance of MEDFileField1TS or MEDFileInt32Field1TS. The
 * caller is to delete this field using decrRef() as it is no more needed.
 *  \throw If there is no required time step in \a this field.
 */
MEDFileAnyTypeField1TS *
MEDFileAnyTypeFieldMultiTS::getTimeStep(int iteration, int order) const
{
 ...
 */
MEDFileAnyTypeField1TS *
MEDFileAnyTypeFieldMultiTS::getTimeStepGivenTime(double time, double eps) const
{
...
/*!
 * This method groups not null items in \a vectFMTS per time step series. Two time series are considered equal if the
 * list of their pair of integers iteration,order are equal. The float64 value of time attached to the pair of integers
 * are not considered here. WARNING the returned pointers are not incremented. The caller is \b not responsible to
 * deallocate them ! This method only reorganizes entries in \a vectFMTS.
 *
...
MEDFileAnyTypeFieldMultiTS::SplitIntoCommonTimeSeries(const std::vector<MEDFileAnyTypeFieldMultiTS *> &vectFMTS)
{
    static const char msg[] =
        "MEDFileAnyTypeFieldMultiTS::SplitIntoCommonTimeSeries : presence of null instance in input vector !";
 ...
 * All items must be defined on the same time step ids ( see MEDFileAnyTypeFieldMultiTS::SplitIntoCommonTimeSeries
 * method ). Each item in \a vectFMTS is expected to have one and exactly one spatial discretization along time. All
 * items in \a vectFMTS must lie on the mesh (located by meshname and time step) and compatible with the input mesh \a
 * mesh (having the same name than those in items). All items in \a vectFMTS whose spatial discretization is not
 * ON_NODES will appear once. For items in \a vectFMTS that are ON_NODES it is possible to appear several times (more
 * than once or once) in the returned vector.
 *
 * \param [in] vectFMTS - list of multi times step part all defined each on a same spatial discretization along time and
 * pointing to a mesh whose name is equal to \c mesh->getName().
 * \param [in] mesh - the mesh shared by all items in \a vectFMTS across time.
 * \param [out] fsc - A vector having same size than returned vector. It specifies the support comporator of the
 * corresponding vector of MEDFileAnyTypeFieldMultiTS in returned vector of vector.
 * \return - A vector of vector of objects that contains the same pointers (objects) than thoose in \a vectFMTS except
 * that there are organized differently. So pointers included in returned vector of vector should \b not been dealt by
 * the caller.
 *
...
MEDFileAnyTypeFieldMultiTS::SplitPerCommonSupport(
    const std::vector<MEDFileAnyTypeFieldMultiTS *> &vectFMTS,
    const MEDFileMesh *mesh,
    std::vector<MCAuto<MEDFileFastCellSupportComparator> > &fsc
)
{
    static const char msg[] =
        "MEDFileAnyTypeFieldMultiTS::SplitPerCommonSupport : presence of a null instance in the input vector !";
    if (!mesh)
    ...
    std::vector<std::vector<MEDFileAnyTypeFieldMultiTS *> > retCell =
        SplitPerCommonSupportNotNodesAlg(vectFMTSNotNodes, mesh, cmps);
    ret = retCell;
    for (std::vector<MEDFileAnyTypeFieldMultiTS *>::const_iterator it2 = vectFMTSNodes.begin();
         it2 != vectFMTSNodes.end();
         it2++)
    {
        i = 0;
        bool isFetched(false);
        for (std::vector<std::vector<MEDFileAnyTypeFieldMultiTS *> >::const_iterator it0 = retCell.begin();
             it0 != retCell.end();
             it0++, i++)
        {
            if ((*it0).empty())
        ...
        if (!isFetched)
        {
            ...
            ret.push_back(tmp);
            retCell.push_back(tmp);
            cmps.push_back(MEDFileFastCellSupportComparator::New(tmp2, *it2));
        }
    }
    fsc = cmps;
    return ret;
...
/*!
 * WARNING no check here. The caller must be sure that all items in vectFMTS are coherent each other in time steps, only
 * one same spatial discretization and not ON_NODES.
 * \param [out] cmps - same size than the returned vector.
 */
std::vector<std::vector<MEDFileAnyTypeFieldMultiTS *> >
MEDFileAnyTypeFieldMultiTS::SplitPerCommonSupportNotNodesAlg(
    const std::vector<MEDFileAnyTypeFieldMultiTS *> &vectFMTS,
    const MEDFileMesh *mesh,
    std::vector<MCAuto<MEDFileFastCellSupportComparator> > &cmps
)
...
/*!
 * This method scan the two main structs along time of \a f0 and \a f1 to see if there are all lying on the same mesh
 * along time than those in \a mesh.
 ...
 */
int
MEDFileAnyTypeFieldMultiTS::CheckSupportAcrossTime(
    MEDFileAnyTypeFieldMultiTS *f0,
...
)
{
    ...
    if (!f0 || !f1)
        throw INTERP_KERNEL::Exception(
            "MEDFileAnyTypeFieldMultiTS::CheckSupportAcrossTime : presence of null instance in fields over time !"
        );
    if (f0->getMeshName() != mesh->getName())
    {
        std::ostringstream oss;
        oss << "MEDFileAnyTypeFieldMultiTS::CheckSupportAcrossTime : first field points to mesh \"" << f0->getMeshName()
            << "\" and input mesh to compare has name \"" << mesh->getName() << "\" !";
        throw INTERP_KERNEL::Exception(oss.str());
    }
    if (f1->getMeshName() != mesh->getName())
    {
        std::ostringstream oss;
        oss << "MEDFileAnyTypeFieldMultiTS::CheckSupportAcrossTime : second field points to mesh \""
            << f1->getMeshName() << "\" and input mesh to compare has name \"" << mesh->getName() << "\" !";
        throw INTERP_KERNEL::Exception(oss.str());
    }
    int nts = f0->getNumberOfTS();
    if (nts != f1->getNumberOfTS())
        throw INTERP_KERNEL::Exception(
            "MEDFileAnyTypeFieldMultiTS::CheckSupportAcrossTime : number of time steps are not the same !"
        );
    if (nts == 0)
        return nts;
    for (int i = 0; i < nts; i++)
    {
        ...
        if (tofs0.size() != 1 || tofs1.size() != 1)
            throw INTERP_KERNEL::Exception(
                "MEDFileAnyTypeFieldMultiTS::CheckSupportAcrossTime : All time steps must be defined on only one "
        ...
        if (i != 0)
        {
            if (tof0 != tofs0[0] || tof1 != tofs1[0])
                throw INTERP_KERNEL::Exception(
                    "MEDFileAnyTypeFieldMultiTS::CheckSupportAcrossTime : Across times steps "
                    "MEDFileAnyTypeFieldMultiTS instances have to keep the same unique spatial discretization !"
                );
        }
        else
        {
        ...
        {
            std::ostringstream oss;
            oss << "MEDFileAnyTypeFieldMultiTS::CheckSupportAcrossTime : first field points to mesh time step ("
                << f0cur->getMeshIteration() << "," << f0cur->getMeshOrder()
                << ") whereas input mesh points to time step (" << mesh->getIteration() << "," << mesh->getOrder()
                << ") !";
            throw INTERP_KERNEL::Exception(oss.str());
        }
        ...
        {
            std::ostringstream oss;
            oss << "MEDFileAnyTypeFieldMultiTS::CheckSupportAcrossTime : second field points to mesh time step ("
                << f1cur->getMeshIteration() << "," << f1cur->getMeshOrder()
                << ") whereas input mesh points to time step (" << mesh->getIteration() << "," << mesh->getOrder()
                << ") !";
            throw INTERP_KERNEL::Exception(oss.str());
        }
        ...
        {
            std::ostringstream oss;
            oss << "MEDFileAnyTypeFieldMultiTS::CheckSupportAcrossTime : all the time steps must be the same ! it is "
                   "not the case ("
                << f0cur->getIteration() << "," << f0cur->getOrder() << ")!=(" << f1cur->getIteration() << ","
                << f1cur->getOrder() << ") !";
            throw INTERP_KERNEL::Exception(oss.str());
        }
    }
...
AggregateHelperF1TS(
    const std::vector<typename MLFieldTraits<T>::F1TSType const *> &f1tss,
...
)
{
    ...
    for (typename std::vector<typename MLFieldTraits<T>::F1TSType const *>::const_iterator it = f1tss.begin();
         it != f1tss.end();
         it++, i++)
    {
    ...
    typename MLFieldTraits<T>::F1TSWSDAType *retc(
        dynamic_cast<typename MLFieldTraits<T>::F1TSWSDAType *>(ret->contentNotNullBase())
    );
    if (!retc)
...
MCAuto<MEDFileAnyTypeFieldMultiTS>
AggregateHelperFMTS(
    const std::vector<typename MLFieldTraits<T>::FMTSType const *> &fmtss,
    const std::vector<std::vector<std::pair<int, mcIdType> > > &dts
)
{
    ...
    for (typename std::vector<typename MLFieldTraits<T>::FMTSType const *>::const_iterator it = fmtss.begin();
         it != fmtss.end();
         it++)
    {
    ...
    for (int iterTS = 0; iterTS < nbTS; iterTS++)
    {
        ...
        for (typename std::vector<typename MLFieldTraits<T>::FMTSType const *>::const_iterator it = fmtss.begin();
             ...
             it++, i++)
        {
            f1tss2[i] = (*it)->getTimeStepAtPos(iterTS);
            f1tss[i] = f1tss2[i];
        }
 ...
 */
MCAuto<MEDFileAnyTypeFieldMultiTS>
MEDFileAnyTypeFieldMultiTS::Aggregate(
    const std::vector<const MEDFileAnyTypeFieldMultiTS *> &fmtss,
    const std::vector<std::vector<std::pair<int, mcIdType> > > &dts
)
{
    if (fmtss.empty())
    ...
    for (std::vector<const MEDFileAnyTypeFieldMultiTS *>::const_iterator it = fmtss.begin(); it != fmtss.end(); it++)
    {
        if (!(*it))
            throw INTERP_KERNEL::Exception(
                "MEDFileAnyTypeFieldMultiTS::Aggregate : presence of null instance in input vector !"
            );
...
MEDFileAnyTypeFieldMultiTS::iterator()
{
...
MEDFileAnyTypeFieldMultiTS *
MEDFileFieldMultiTS::shallowCpy() const
{
...
MEDFileFieldMultiTS::convertToInt(bool isDeepCpyGlobs) const
{
    ...
    if (content)
    {
        ...
        if (!contc)
            throw INTERP_KERNEL::Exception(
                "MEDFileFieldMultiTS::convertToInt : the content inside this is not FLOAT64 ! This is incoherent !"
            );
        MCAuto<MEDFileInt32FieldMultiTSWithoutSDA> newc(contc->convertToInt());
        ret = static_cast<MEDFileInt32FieldMultiTS *>(
            MEDFileAnyTypeFieldMultiTS::BuildNewInstanceFromContent((MEDFileInt32FieldMultiTSWithoutSDA *)newc)
        );
    }
    else
...
try : MEDFileTemplateFieldMultiTS
...
try : MEDFileTemplateFieldMultiTS
    <double>(fid, fieldName, loadAll, ms, entities) {}
catch (INTERP_KERNEL::Exception &e)
{
    throw e;
}
...
MEDFileFieldMultiTS::MEDFileFieldMultiTS(const MEDFileFieldMultiTSWithoutSDA &other, bool shallowCopyOfContent)
    : MEDFileTemplateFieldMultiTS<double>(other, shallowCopyOfContent)
{
}

std::vector<std::vector<DataArrayDouble *> >
MEDFileFieldMultiTS::getFieldSplitedByType2(
    int iteration,
    int order,
    const std::string &mname,
    std::vector<INTERP_KERNEL::NormalizedCellType> &types,
...
MEDFileAnyTypeFieldMultiTSIterator::MEDFileAnyTypeFieldMultiTSIterator(MEDFileAnyTypeFieldMultiTS *fmts)
    : _fmts(fmts), _iter_id(0), _nb_iter(0)
{
    if (fmts)
    {
        fmts->incrRef();
...
MEDFileAnyTypeField1TS *
MEDFileAnyTypeFieldMultiTSIterator::nextt()
{
