...
MEDFileFields *
MEDFileFields::New(const std::string &fileName, bool loadAll)
{
...
MEDFileFields *
MEDFileFields::NewAdv(const std::string &fileName, bool loadAll, const MEDFileEntities *entities)
{
    MEDFileUtilities::AutoFid fid(OpenMEDFileForRead(fileName));
...
MEDFileFields *
MEDFileFields::NewAdv(med_idt fid, bool loadAll, const MEDFileEntities *entities)
{
    return new MEDFileFields(fid, loadAll, 0, entities);
}

MEDFileFields *
MEDFileFields::NewWithDynGT(const std::string &fileName, const MEDFileStructureElements *se, bool loadAll)
{
    MEDFileUtilities::AutoFid fid(OpenMEDFileForRead(fileName));
...
MEDFileFields *
MEDFileFields::NewWithDynGT(med_idt fid, const MEDFileStructureElements *se, bool loadAll)
{
    if (!se)
...
MEDFileFields *
MEDFileFields::New(med_idt fid, bool loadAll)
{
    return new MEDFileFields(fid, loadAll, 0, 0);
}

MEDFileFields *
MEDFileFields::LoadPartOf(const std::string &fileName, bool loadAll, const MEDFileMeshes *ms)
{
    MEDFileUtilities::AutoFid fid(OpenMEDFileForRead(fileName));
...
MEDFileFields *
MEDFileFields::LoadSpecificEntities(
...
std::size_t
MEDFileFields::getHeapMemorySizeWithoutChildren() const
{
...
/*!
 * This method scans for all fields in \a this which time steps ids are common. Time step are discriminated by the pair
 * of integer (iteration,order) whatever the double time value. If all returned time steps are \b exactly those for all
 * fields in \a this output parameter \a areThereSomeForgottenTS will be set to false. If \a areThereSomeForgottenTS is
 * set to true, only the sorted intersection of time steps present for all fields in \a this will be returned.
 *
 * \param [out] areThereSomeForgottenTS - indicates to the caller if there is some time steps in \a this that are not
 * present for all fields in \a this.
 * \return the sorted list of time steps (specified with a pair of integer iteration first and order second) present for
 * all fields in \a this.
 *
 * \sa MEDFileFields::partOfThisLyingOnSpecifiedTimeSteps, MEDFileFields::partOfThisNotLyingOnSpecifiedTimeSteps
 */
std::vector<std::pair<int, int>>
MEDFileFields::getCommonIterations(bool &areThereSomeForgottenTS) const
{
         ...
         it++)
    {
        ...
        std::copy(v.begin(), v.end(), std::inserter(s1, s1.end()));
        if (firstShot)
        {
            s = s1;
            firstShot = false;
        ...
        else
        {
            ...
            std::set_intersection(s.begin(), s.end(), s1.begin(), s1.end(), std::inserter(s2, s2.end()));
            if (s != s2)
...
int
MEDFileFields::getNumberOfFields() const
{
    return (int)_fields.size();
}

std::vector<std::string>
MEDFileFields::getFieldsNames() const
{
         ...
         it++, i++)
    {
        ...
        else
        {
            ...
            oss << "MEDFileFields::getFieldsNames : At rank #" << i << " field is not defined !";
...
std::vector<std::string>
MEDFileFields::getMeshesNames() const
{
...
std::string
MEDFileFields::simpleRepr() const
{
    std::ostringstream oss;
    oss << "(*****************)\n(* MEDFileFields *)\n(*****************)\n\n";
    simpleRepr(0, oss);
...
void
MEDFileFields::simpleRepr(int bkOffset, std::ostream &oss) const
{
...
try : MEDFileFieldGlobsReal(fid)
{
    ...
    for (int i = 0; i < nbFields; i++)
    {
        ...
        switch (typcha)
        {
            ...
            {
                std::ostringstream oss;
                oss << "constructor MEDFileFields(fileName) : file \'" << FileNameFromFID(fid) << "\' at pos #" << i
                    << " field has name \'" << fieldName
...
void
MEDFileFields::writeLL(med_idt fid) const
{
    int i = 0;
         ...
         it++, i++)
    {
        ...
        {
            std::ostringstream oss;
            oss << "MEDFileFields::write : at rank #" << i << "/" << _fields.size() << " field is empty !";
            throw INTERP_KERNEL::Exception(oss.str());
        }
...
MEDFileFields::loadArrays()
{
    ...
    for (std::vector<MCAuto<MEDFileAnyTypeFieldMultiTSWithoutSDA>>::iterator it = _fields.begin(); it != _fields.end();
         it++)
    {
...
/*!
 * This method behaves as MEDFileFields::loadArrays does, the first call, if \a this was built using a file without
 * loading big arrays. But once data loaded once, this method does nothing.
 *
 * \throw If the fileName is not set or points to a non readable MED file.
...
MEDFileFields::loadArraysIfNecessary()
{
    ...
    {
        MEDFileUtilities::AutoFid fid(OpenMEDFileForRead(getFileName()));
        for (std::vector<MCAuto<MEDFileAnyTypeFieldMultiTSWithoutSDA>>::iterator it = _fields.begin();
             it != _fields.end();
             it++)
        {
            MEDFileAnyTypeFieldMultiTSWithoutSDA *elt(*it);
 ...
 * MEDFileFields::loadArraysIfNecessary. This method is useful to reduce \b safely amount of heap memory necessary for
 * \a this by using MED file as database.
 *
...
std::vector<std::string>
MEDFileFields::getPflsReallyUsed() const
{
    std::vector<std::string> ret;
...
std::vector<std::string>
MEDFileFields::getLocsReallyUsed() const
{
    std::vector<std::string> ret;
    std::set<std::string> ret2;
    for (std::vector<MCAuto<MEDFileAnyTypeFieldMultiTSWithoutSDA>>::const_iterator it = _fields.begin();
...
std::vector<std::string>
MEDFileFields::getPflsReallyUsedMulti() const
{
    std::vector<std::string> ret;
    for (std::vector<MCAuto<MEDFileAnyTypeFieldMultiTSWithoutSDA>>::const_iterator it = _fields.begin();
         it != _fields.end();
...
std::vector<std::string>
MEDFileFields::getLocsReallyUsedMulti() const
{
    std::vector<std::string> ret;
    for (std::vector<MCAuto<MEDFileAnyTypeFieldMultiTSWithoutSDA>>::const_iterator it = _fields.begin();
         it != _fields.end();
...
void
MEDFileFields::changePflsRefsNamesGen(const std::vector<std::pair<std::vector<std::string>, std::string>> &mapOfModif)
{
    for (std::vector<MCAuto<MEDFileAnyTypeFieldMultiTSWithoutSDA>>::iterator it = _fields.begin(); it != _fields.end();
         it++)
...
void
MEDFileFields::changeLocsRefsNamesGen(const std::vector<std::pair<std::vector<std::string>, std::string>> &mapOfModif)
{
    for (std::vector<MCAuto<MEDFileAnyTypeFieldMultiTSWithoutSDA>>::iterator it = _fields.begin(); it != _fields.end();
         it++)
...
void
MEDFileFields::resize(int newSize)
{
    _fields.resize(newSize);
}

void
MEDFileFields::pushFields(const std::vector<MEDFileAnyTypeFieldMultiTS *> &fields)
{
...
void
MEDFileFields::pushField(MEDFileAnyTypeFieldMultiTS *field)
{
    if (!field)
        throw INTERP_KERNEL::Exception(
            "MEDFileFields::pushMesh : invalid input pointer ! should be different from 0 !"
        );
...
void
MEDFileFields::setFieldAtPos(int i, MEDFileAnyTypeFieldMultiTS *field)
{
    if (!field)
        throw INTERP_KERNEL::Exception(
            "MEDFileFields::setFieldAtPos : invalid input pointer ! should be different from 0 !"
        );
...
void
MEDFileFields::destroyFieldAtPos(int i)
{
    destroyFieldsAtPos(&i, &i + 1);
}

void
MEDFileFields::destroyFieldsAtPos(const int *startIds, const int *endIds)
{
    ...
    for (const int *i = startIds; i != endIds; i++)
    {
        ...
        {
            std::ostringstream oss;
            oss << "MEDFileFields::destroyFieldsAtPos : Invalid given id in input (" << *i << ") should be in [0,"
                << _fields.size() << ") !";
            throw INTERP_KERNEL::Exception(oss.str());
        }
        b[*i] = false;
...
void
MEDFileFields::destroyFieldsAtPos2(int bg, int end, int step)
{
    ...
    for (int i = 0; i < nbOfEntriesToKill; i++, k += step)
    {
        ...
        {
            std::ostringstream oss;
            oss << "MEDFileFields::destroyFieldsAtPos2 : Invalid given id in input (" << k << ") should be in [0,"
                << _fields.size() << ") !";
            throw INTERP_KERNEL::Exception(oss.str());
        }
        b[k] = false;
...
bool
MEDFileFields::changeMeshNames(const std::vector<std::pair<std::string, std::string>> &modifTab)
{
    bool ret(false);
    for (std::vector<MCAuto<MEDFileAnyTypeFieldMultiTSWithoutSDA>>::iterator it = _fields.begin(); it != _fields.end();
         it++)
 ...
 * \param [in] oldCode is of format returned by MEDCouplingUMesh::getDistributionOfTypes. And for each *i*
 ...
 * \return If true a renumbering has been performed. The structure in \a this has been modified. If false, nothing has
 * been done: it is typically the case if \a meshName is not referred by any field in \a this.
 */
bool
MEDFileFields::renumberEntitiesLyingOnMesh(
...
MEDFileFields::extractPart(const std::map<int, MCAuto<DataArrayIdType>> &extractDef, MEDFileMesh *mm) const
{
    ...
    for (int i = 0; i < nbFields; i++)
    {
        ...
        {
            std::ostringstream oss;
            oss << "MEDFileFields::extractPart : at pos #" << i << " field is null !";
            throw INTERP_KERNEL::Exception(oss.str());
        }
...
void
MEDFileFields::accept(MEDFileFieldVisitor &visitor) const
{
    for (std::vector<MCAuto<MEDFileAnyTypeFieldMultiTSWithoutSDA>>::const_iterator it = _fields.begin();
         it != _fields.end();
         it++)
...
class PFLData
{
   public:
    ...
    PFLData(const MCAuto<DataArrayIdType> &mat, const MCAuto<DataArrayIdType> &pfl, mcIdType nbOfNewPts)
        : _matrix(mat), _pfl(pfl), _add_pts_in_pfl(nbOfNewPts)
    {
    }
...
class MEDFileFieldLin2QuadVisitor : public MEDFileFieldVisitor
{
   public:
    MEDFileFieldLin2QuadVisitor(
        const MEDFileUMesh *lin, const MEDFileUMesh *quad, const MEDFileFieldGlobsReal *linGlobs, MEDFileFields *outFs
    )
        : _lin(lin),
          _quad(quad),
          _lin_globs(linGlobs),
          _out_fs(outFs),
          _gt(INTERP_KERNEL::NORM_ERROR),
          _1ts_update_requested(false)
    {
    }
    void newFieldEntry(const MEDFileAnyTypeFieldMultiTSWithoutSDA *field)
    {
        if (field->getMeshName() != _lin->getName())
            return;
        _cur_fmts = MEDFileFieldMultiTS::New();
    }
    void endFieldEntry(const MEDFileAnyTypeFieldMultiTSWithoutSDA *field)
    {
        if (_cur_fmts.isNotNull())
        {
            if (_cur_fmts->getNumberOfTS() > 0)
                _out_fs->pushField(_cur_fmts);
        }
    }
    //
...
void
MEDFileFieldLin2QuadVisitor::newPerMeshPerTypePerDisc(const MEDFileFieldPerMeshPerTypePerDisc *pmptpd)
{
    if (_cur_f1ts.isNull())
        return;
    ...
    MCAuto<MEDCouplingUMesh> mesh3DQuadraticRestricted(
        mesh3DQuadratic->buildPartOfMySelf(cellIds->begin(), cellIds->end(), true)
    );
    ...
    if (allSeg3->getCellModelEnum() != INTERP_KERNEL::NORM_SEG3)
        throw INTERP_KERNEL::Exception(
            "MEDFileFieldLin2QuadVisitor::newPerMeshPerTypePerDisc : invalid situation where SEG3 expected !"
        );
...
void
MEDFileFieldLin2QuadVisitor::updateData(MEDFileFieldPerMeshPerTypePerDisc *pmtd, mcIdType deltaNbNodes)
{
    pmtd->incrementNbOfVals(deltaNbNodes);
}

void
MEDFileFieldLin2QuadVisitor::newPerMeshPerTypeEntry(const MEDFileFieldPerMeshPerTypeCommon *pmpt)
{
    const MEDFileFieldPerMeshPerType *pmpt2(dynamic_cast<const MEDFileFieldPerMeshPerType *>(pmpt));
    if (!pmpt2)
        throw INTERP_KERNEL::Exception(
            "MEDFileFieldLin2QuadVisitor::newPerMeshPerTypeEntry : not managed for structure elements !"
        );
    if (pmpt2->getNumberOfLoc() != 1)
        throw INTERP_KERNEL::Exception(
            "MEDFileFieldLin2QuadVisitor::newPerMeshPerTypeEntry : not managed for multi discr per timestep !"
        );
    _gt = pmpt->getGeoType();
}

void
MEDFileFieldLin2QuadVisitor::newMeshEntry(const MEDFileFieldPerMesh *fpm)
{
...
void
MEDFileFieldLin2QuadVisitor::newTimeStepEntry(const MEDFileAnyTypeField1TSWithoutSDA *ts)
{
    _1ts_update_requested = false;
    _pfls_to_be_updated.clear();
    if (!ts)
        return;
    ...
    MCAuto<MEDFileField1TSWithoutSDA> contentCpy2(
        DynamicCastSafe<MEDFileAnyTypeField1TSWithoutSDA, MEDFileField1TSWithoutSDA>(contentCpy)
    );
...
void
MEDFileFieldLin2QuadVisitor::endTimeStepEntry(const MEDFileAnyTypeField1TSWithoutSDA *ts)
{
    if (_cur_f1ts.isNull())
        return;
    if (_1ts_update_requested)
    {
        ...
        for (std::vector<std::string>::const_iterator it = _pfls_to_be_updated.begin(); it != _pfls_to_be_updated.end();
             it++)
        {
...
MEDFileFields::linearToQuadratic(const MEDFileMeshes *oldLin, const MEDFileMeshes *newQuad) const
{
    ...
    for (int i = 0; i < oldLin->getNumberOfMeshes(); i++)
    {
        ...
        {
            std::ostringstream oss;
            oss << "MEDFileFields::linearToQuadratic : mismatch of name between input meshes for name \""
                << mmu->getName() << "\"";
            throw INTERP_KERNEL::Exception(oss.str());
        }
...
MEDFileFields::getFieldAtPos(int i) const
{
    if (i < 0 || i >= (int)_fields.size())
    {
        std::ostringstream oss;
        oss << "MEDFileFields::getFieldAtPos : Invalid given id in input (" << i << ") should be in [0,"
            << _fields.size() << ") !";
        throw INTERP_KERNEL::Exception(oss.str());
    }
    ...
    {
        std::ostringstream oss;
        oss << "MEDFileFields::getFieldAtPos : At pos #" << i
            << " field is neither double (FLOAT64) nor float (FLOAT32) nor integer (INT32) nor integer (INT64) !";
        throw INTERP_KERNEL::Exception(oss.str());
    }
...
MEDFileFields::buildSubPart(const int *startIds, const int *endIds) const
{
    ...
    for (const int *i = startIds; i != endIds; i++, j++)
    {
        ...
        {
            std::ostringstream oss;
            oss << "MEDFileFields::buildSubPart : Invalid given id in input (" << *i << ") should be in [0,"
                << _fields.size() << ") !";
            throw INTERP_KERNEL::Exception(oss.str());
        }
        fields[j] = _fields[*i];
...
MEDFileAnyTypeFieldMultiTS *
MEDFileFields::getFieldWithName(const std::string &fieldName) const
{
 ...
 * If there is one or more than one such field in \a this true is returned and those fields will not be referenced
 * anymore in \a this.
 *
 * If false is returned \a this does not contain such fields. If false is returned this method can be considered as
 * const.
 */
bool
MEDFileFields::removeFieldsWithoutAnyTimeStep()
{
...
/*!
 * This method returns a new object containing part of \a this fields lying on mesh name specified by the input
 * parameter \a meshName. This method can be seen as a filter applied on \a this, that returns an object containing
 * reduced the list of fields compared to those in \a this. The returned object is a new object but the object on which
 * it lies are only shallow copied from \a this.
 *
...
/*!
 * This method returns a new object containing part of \a this fields lying ** exactly ** on the time steps specified by
 * input parameter \a timeSteps. Input time steps are specified using a pair of integer (iteration, order). This method
 * can be seen as a filter applied on \a this, that returns an object containing the same number of fields than those in
 * \a this, but for each multitimestep only the time steps in \a timeSteps are kept. Typically the input parameter \a
 * timeSteps comes from the call of MEDFileFields::getCommonIterations.
 *
...
bool
MEDFileFields::presenceOfStructureElements() const
{
    for (std::vector<MCAuto<MEDFileAnyTypeFieldMultiTSWithoutSDA>>::const_iterator it = _fields.begin();
         it != _fields.end();
         it++)
...
void
MEDFileFields::killStructureElements()
{
...
void
MEDFileFields::keepOnlyStructureElements()
{
    std::vector<MCAuto<MEDFileAnyTypeFieldMultiTSWithoutSDA>> ret;
    for (std::vector<MCAuto<MEDFileAnyTypeFieldMultiTSWithoutSDA>>::iterator it = _fields.begin(); it != _fields.end();
         it++)
...
void
MEDFileFields::keepOnlyOnMeshSE(const std::string &meshName, const std::string &seName)
{
    std::vector<MCAuto<MEDFileAnyTypeFieldMultiTSWithoutSDA>> ret;
    for (std::vector<MCAuto<MEDFileAnyTypeFieldMultiTSWithoutSDA>>::iterator it = _fields.begin(); it != _fields.end();
         it++)
...
void
MEDFileFields::getMeshSENames(std::vector<std::pair<std::string, std::string>> &ps) const
{
    for (std::vector<MCAuto<MEDFileAnyTypeFieldMultiTSWithoutSDA>>::const_iterator it = _fields.begin();
         it != _fields.end();
         it++)
...
void
MEDFileFields::blowUpSE(MEDFileMeshes *ms, const MEDFileStructureElements *ses)
{
 ...
 */
void
MEDFileFields::aggregateFieldsOnSameMeshes(MEDFileMeshes *ms)
{
    if (!ms)
    ...
    for (auto fieldsWithSame : fsByName)
    {
        if (fieldsWithSame.second.size() > 1)
        {
            ...
            for (auto fmtsWithSameName : fieldsWithSame.second) zeMeshNames.insert(fmtsWithSameName->getMeshName());
            if (zeMeshNames.size() != 1)
                THROW_IK_EXCEPTION(
                    "MEDFileFields::aggregateFieldsOnSameMeshes : Presence of multiple MultiTS instances with same "
                    "name but lying on same meshName. Looks bad !"
                );
            ...
            if (expectedMeshNamesToMerge.find(meshNameToMerge) != expectedMeshNamesToMerge.end())
                THROW_IK_EXCEPTION(
                    "MEDFileFields::aggregateFieldsOnSameMeshes : unexpected situation ! Error in implementation !"
                );
    ...
    for (auto fieldNameToBeAggregated : fieldsNamesToBeAggregated)
    {
        ...
        {
            std::vector<std::pair<std::pair<INTERP_KERNEL::NormalizedCellType, int>, std::pair<mcIdType, mcIdType>>>
                entries;
            int iteration, order;
            {
            ...
            MCAuto<MEDFileAnyTypeFieldMultiTS> eltToAggregate =
                MEDFileAnyTypeFieldMultiTS::BuildNewInstanceFromContent(fieldToBeAggregated);
            eltsToAggregate.push_back(eltToAggregate);
        }
        MCAuto<MEDFileAnyTypeFieldMultiTS> gg =
            MEDFileAnyTypeFieldMultiTS::Aggregate(FromVecAutoToVecOfConst(eltsToAggregate), dtsToAggregate);
    ...
    if (meshesNamesToBeAggregated != expectedMeshNamesToMerge)
        THROW_IK_EXCEPTION(
            "MEDFileFields::aggregateFieldsOnSameMeshes : mismatch between meshes to be aggregated and meshnames into "
    ...
    ms->resize(0);
    for (auto mesh : otherMeshes) ms->pushMesh(mesh);
    _fields = otherFields;
}
...
void
MEDFileFields::aggregate(const MEDFileFields &other)
{
    ...
    for (int i = 0; i < nbFieldsToAdd; i++)
    {
        ...
        {
            std::ostringstream oss;
            oss << "MEDFileFields::aggregate : name \"" << name << "\" already appears !";
            throw INTERP_KERNEL::Exception(oss.str());
        }
        pushField(elt);
    }
...
MEDFileFields::iterator()
{
    return new MEDFileFieldsIterator(this);
}

int
MEDFileFields::getPosFromFieldName(const std::string &fieldName) const
{
    ...
    std::ostringstream oss;
    oss << "MEDFileFields::getPosFromFieldName : impossible to find field '" << tmp
        << "' in this ! Possibilities are : ";
