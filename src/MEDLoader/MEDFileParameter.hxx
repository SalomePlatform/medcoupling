...
class MEDFileParameter1TS : public RefCountObject
{
   public:
    ...
    MEDLOADER_EXPORT virtual void writeAdvanced(
        med_idt fid, const std::string &name, const MEDFileWritable &mw
    ) const = 0;
   ...
   public:
    ...
    MEDLOADER_EXPORT double getTime(int &dt, int &it)
    {
        dt = _iteration;
        it = _order;
        return _time;
...
class MEDFileParameters : public RefCountObject, public MEDFileWritableStandAlone
{
   public:
    ...
    MEDLOADER_EXPORT static MEDFileParameters *New(DataArrayByte *db)
    {
        return BuildFromMemoryChunk<MEDFileParameters>(db);
    }
