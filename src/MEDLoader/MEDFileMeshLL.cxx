...
const char MEDFileMeshL2::ZE_SEP_FOR_FAMILY_KILLERS[] =
    "!/__\\!";  // important start by - because ord('!')==33 the smallest (!=' ') to preserve orders at most.

int MEDFileMeshL2::ZE_SEP2_FOR_FAMILY_KILLERS = 4;

std::vector<std::string>
MeshCls::getAxisInfoOnMesh(
    med_idt fid,
    ...
    int &nstep,
    int &Mdim,
    MEDFileString &description,
    MEDFileString &dtunit,
    MEDFileString &univName
) const
{
    ...
    MEDmeshUniversalNameRd(
        fid, nameTmp, univName.getPointer()
    );  // do not protect  MEDFILESAFECALLERRD0 call : Thanks to fra.med.
            ...
            switch (gt)
            {
                ...
                default:
                    throw INTERP_KERNEL::Exception(
                        "MEDFileMeshL2::getAxisInfoOnMesh : unrecognized structured mesh type ! Supported are :\n - "
                        "cartesian\n - curve linear\n"
                    );
            }
            break;
    ...
    {
        std::string info(
            MEDLoaderBase::buildUnionUnit(
                ((char *)axisname) + i * MED_SNAME_SIZE,
                MED_SNAME_SIZE,
                ((char *)axisunit) + i * MED_SNAME_SIZE,
                MED_SNAME_SIZE
            )
        );
        infosOnComp[i] = info;
...
double
MeshCls::checkMeshTimeStep(med_idt fid, const std::string &mName, int nstep, int dt, int it) const
{
    bool found = false;
    med_int numdt, numit;
    med_float dtt = 0.0;
    ...
    if (!found)
    {
        ...
        oss << "No such iteration=" << dt << ",order=" << it << " numbers found for mesh '" << mName << "' ! ";
        oss << "Possibilities are : ";
        for (int i = 0; i < nstep; i++) oss << "(" << p[i].first << "," << p[i].second << "), ";
...
std::vector<std::string>
StructMeshCls::getAxisInfoOnMesh(
    med_idt fid,
    const std::string &mName,
    MEDCoupling::MEDCouplingMeshType &meshType,
    MEDCoupling::MEDCouplingAxisType &axType,
...
) const
{
    ...
    INTERP_KERNEL::AutoPtr<char> axisName(new char[MED_SNAME_SIZE * nAxis + 1]),
        axisUnit(new char[MED_SNAME_SIZE * nAxis + 1]);
    med_int spaceDim(0), meshDim(0);
    MEDFILESAFECALLERRD0(
        MEDsupportMeshInfo, (fid, getID(), msn, &spaceDim, &meshDim, zeDescription, &medAxType, axisName, axisUnit)
    );
    ...
    {
        std::string info(
            DataArray::BuildInfoFromVarAndUnit(
                MEDLoaderBase::buildStringFromFortran(axisName + i * MED_SNAME_SIZE, MED_SNAME_SIZE),
...
double
StructMeshCls::checkMeshTimeStep(med_idt fid, const std::string &mName, int nstep, int dt, int it) const
{
    return 0.;
}

MEDFileMeshL2::MEDFileMeshL2()
    : _name(MED_NAME_SIZE), _description(MED_COMMENT_SIZE), _univ_name(MED_LNAME_SIZE), _dt_unit(MED_LNAME_SIZE)
{
}

std::size_t
MEDFileMeshL2::getHeapMemorySizeWithoutChildren() const
{
    return 0;
}

std::vector<const BigMemoryObject *>
MEDFileMeshL2::getDirectChildrenWithNull() const
{
    return std::vector<const BigMemoryObject *>();
}

INTERP_KERNEL::AutoCppPtr<MeshOrStructMeshCls>
MEDFileMeshL2::GetMeshIdFromName(
    med_idt fid,
    const std::string &mName,
    MEDCoupling::MEDCouplingMeshType &meshType,
    MEDCoupling::MEDCouplingAxisType &axType,
...
)
{
    ...
    for (int i = 0; i < n && found == 0; i++)
    {
        ...
        INTERP_KERNEL::AutoPtr<char> axisname(MEDLoaderBase::buildEmptyString(naxis * MED_SNAME_SIZE)),
            axisunit(MEDLoaderBase::buildEmptyString(naxis * MED_SNAME_SIZE));
        MEDFILESAFECALLERRD0(
            MEDmeshInfo,
            (fid,
             i + 1,
    ...
    {  // last chance ! Is it a support mesh ?
        ...
        for (int i = 0; i < nbSM && found == 0; i++)
        {
            ...
            MEDFILESAFECALLERRD0(
                MEDsupportMeshInfo,
                (fid, i + 1, nommaa, &spaceDim, &dim, maillage_description, &axistype, axisname, axisunit)
            );
            std::string cur(MEDLoaderBase::buildStringFromFortran(nommaa, sizeof(nommaa)));
            ms.push_back(cur);
            if (cur == mName)
    ...
    switch (found)
    {
        case 1:
        {
            ...
            switch (type_maillage)
            {
                ...
                case MED_STRUCTURED_MESH:
                {
                    ...
                    switch (gt)
                    {
                        ...
                        default:
                            throw INTERP_KERNEL::Exception(
                                "MEDFileMeshL2::getMeshIdFromName : unrecognized structured mesh type ! Supported are "
                                ":\n - cartesian\n - curve linear\n"
                            );
                    }
                    break;
            ...
            dt = FromMedInt<int>(numdt);
            it = FromMedInt<int>(numit);
 ...
 */
std::vector<std::string>
MEDFileMeshL2::getAxisInfoOnMesh(
    med_idt fid,
    const MeshOrStructMeshCls *mId,
    const std::string &mName,
    MEDCoupling::MEDCouplingMeshType &meshType,
    MEDCoupling::MEDCouplingAxisType &axType,
    int &nstep,
...
void
MEDFileMeshL2::ReadFamiliesAndGrps(
    med_idt fid,
...
)
{
    ...
    for (int i = 0; i < nfam; i++)
    {
        ...
        crudeFams[i] = std::pair<std::string, std::pair<mcIdType, std::vector<std::string>>>(
            famName, std::pair<mcIdType, std::vector<std::string>>(numfam, grps2)
        );
    }
    RenameFamiliesFromFileToMemInternal(crudeFams);
    for (std::vector<std::pair<std::string, std::pair<mcIdType, std::vector<std::string>>>>::const_iterator it0 =
             crudeFams.begin();
         it0 != crudeFams.end();
         it0++)
    {
        fams[(*it0).first] = (*it0).second.first;
        for (std::vector<std::string>::const_iterator it1 = (*it0).second.second.begin();
             it1 != (*it0).second.second.end();
             it1++)
...
void
MEDFileMeshL2::WriteFamiliesAndGrps(
    med_idt fid,
    const std::string &mname,
    const std::map<std::string, mcIdType> &fams,
...
)
{
    ...
    for (std::map<std::string, mcIdType>::const_iterator it = fams.begin(); it != fams.end(); it++, ii++)
    {
        ...
        for (std::map<std::string, std::vector<std::string>>::const_iterator it1 = grps.begin(); it1 != grps.end();
             it1++)
        {
            if (std::find((*it1).second.begin(), (*it1).second.end(), (*it).first) != (*it1).second.end())
                grpsOfFam.push_back((*it1).first);
        }
        crudeFams[ii] = std::pair<std::string, std::pair<mcIdType, std::vector<std::string>>>(
            (*it).first, std::pair<mcIdType, std::vector<std::string>>((*it).second, grpsOfFam)
        );
    }
    RenameFamiliesFromMemToFileInternal(crudeFams);
    for (std::vector<std::pair<std::string, std::pair<mcIdType, std::vector<std::string>>>>::const_iterator it =
             crudeFams.begin();
         it != crudeFams.end();
         it++)
    {
        ...
        int i = 0;
        for (std::vector<std::string>::const_iterator it2 = (*it).second.second.begin();
...
void
MEDFileMeshL2::RenameFamiliesPatternInternal(
    std::vector<std::pair<std::string, std::pair<mcIdType, std::vector<std::string>>>> &crudeFams,
    RenameFamiliesPatternFunc func
)
{
    std::size_t ii(0);
    ...
    if (!func(fams))
        return;
    ii = 0;
    for (std::vector<std::pair<std::string, std::pair<mcIdType, std::vector<std::string>>>>::iterator it =
             crudeFams.begin();
         it != crudeFams.end();
         it++, ii++)
        (*it).first = fams[ii];
...
/*!
 * This method is dedicated to the killers that use a same family name to store different family ids. MED file API
 * authorizes it. So this method renames families (if needed generally not !) in order to have a discriminant name for
 * families.
 */
void
MEDFileMeshL2::RenameFamiliesFromFileToMemInternal(
    std::vector<std::pair<std::string, std::pair<mcIdType, std::vector<std::string>>>> &crudeFams
)
{
...
bool
MEDFileMeshL2::RenameFamiliesFromFileToMem(std::vector<std::string> &famNames)
{
    ...
    for (std::vector<std::string>::iterator it = famNames.begin(); it != famNames.end(); it++)
    {
        ...
        {
            std::ostringstream oss;
            oss << *it << ZE_SEP_FOR_FAMILY_KILLERS << std::setfill('0') << std::setw(ZE_SEP2_FOR_FAMILY_KILLERS)
                << (*it2).second++;
            *it = oss.str();
        }
    }
 ...
 */
void
MEDFileMeshL2::RenameFamiliesFromMemToFileInternal(
    std::vector<std::pair<std::string, std::pair<mcIdType, std::vector<std::string>>>> &crudeFams
)
{
...
bool
MEDFileMeshL2::RenameFamiliesFromMemToFile(std::vector<std::string> &famNames)
{
...
MEDCoupling::MEDCouplingAxisType
MEDFileMeshL2::TraduceAxisType(med_axis_type at)
{
    switch (at)
    {
        case MED_CARTESIAN:
...
MEDCoupling::MEDCouplingAxisType
MEDFileMeshL2::TraduceAxisTypeStruct(med_grid_type gt)
{
    switch (gt)
    {
        case MED_CARTESIAN_GRID:
        ...
        default:
            throw INTERP_KERNEL::Exception(
                "MEDFileMeshL2::TraduceAxisTypeStruct : only Cartesian and Cylindrical supported by MED file !"
            );
    }
}

med_axis_type
MEDFileMeshL2::TraduceAxisTypeRev(MEDCoupling::MEDCouplingAxisType at)
{
    switch (at)
    {
        case AX_CART:
...
med_grid_type
MEDFileMeshL2::TraduceAxisTypeRevStruct(MEDCoupling::MEDCouplingAxisType at)
{
    switch (at)
    {
        case AX_CART:
...
std::vector<std::string>
MEDFileUMeshL2::loadCommonPart(
    med_idt fid, const MeshOrStructMeshCls *mId, const std::string &mName, int dt, int it, int &Mdim
)
{
    Mdim = -3;
    ...
    if (meshType != UNSTRUCTURED)
        throw INTERP_KERNEL::Exception(
            "Invalid mesh type ! You are expected an unstructured one whereas in file it is not an unstructured !"
        );
...
void
MEDFileUMeshL2::loadAll(
    med_idt fid, const MeshOrStructMeshCls *mId, const std::string &mName, int dt, int it, MEDFileMeshReadSelector *mrs
)
{
    int Mdim;
 ...
 */
void
MEDFileUMeshL2::dealWithCoordsInLoadPart(
    med_idt fid,
    const MeshOrStructMeshCls *mId,
    const std::string &mName,
...
)
{
    ...
    mcIdType nCoords(MEDmeshnEntity(
        fid, mName.c_str(), dt, it, MED_NODE, MED_NONE, MED_COORDINATE, MED_NO_CMODE, &changement, &transformation
    ));
    std::vector<bool> fetchedNodeIds(nCoords, false);
    for (std::vector<std::vector<MCAuto<MEDFileUMeshPerType>>>::const_iterator it0 = _per_type_mesh.begin();
         it0 != _per_type_mesh.end();
         it0++)
    ...
    {
        mcIdType nMin(ToIdType(
            std::distance(fetchedNodeIds.begin(), std::find(fetchedNodeIds.begin(), fetchedNodeIds.end(), true))
        ));
        mcIdType nMax(ToIdType(
            std::distance(fetchedNodeIds.rbegin(), std::find(fetchedNodeIds.rbegin(), fetchedNodeIds.rend(), true))
        ));
        nMax = nCoords - nMax;
        for (std::vector<std::vector<MCAuto<MEDFileUMeshPerType>>>::const_iterator it0 = _per_type_mesh.begin();
             it0 != _per_type_mesh.end();
             it0++)
            for (std::vector<MCAuto<MEDFileUMeshPerType>>::const_iterator it1 = (*it0).begin(); it1 != (*it0).end();
                 it1++)
...
std::vector<std::string>
MEDFileUMeshL2::loadPartConnectivityOnly(
    med_idt fid,
    const MeshOrStructMeshCls *mId,
    const std::string &mName,
    const std::vector<INTERP_KERNEL::NormalizedCellType> &types,
...
{
    int Mdim;
    std::vector<std::string> infosOnComp(
        loadPartConnectivityOnly(fid, mId, mName, types, slicPerTyp, dt, it, mrs, Mdim)
    );
    if (Mdim == -4)
        return;
    loadPartOfConnectivity(fid, Mdim, mName, types, slicPerTyp, dt, it, mrs);
...
/*!
 * This method loads from file \a fid a part of the mesh (made of same geometrical type cells \a type) called \a mName.
 * The loading is done in 2 steps: First, we load the connectivity of nodes. Second, we load coordinates of nodes lying
 * in the specified cells (same as MEDFileUMeshL2::dealWithCoordsInLoadPart, except in this case, we're not limited to
 * slice of nodes)
 * \throw exception if multiple load sessions are requested
 */
void
MEDFileUMeshL2::loadPartFromUserDistrib(
    med_idt fid,
    const MeshOrStructMeshCls *mId,
    const std::string &mName,
...
)
{
    ...
    /* First step : loading connectivity of nodes, ie building a new mesh of one geometrical type with only the
     * specified cells in distrib */
    ...
    for (std::vector<std::vector<MCAuto<MEDFileUMeshPerType>>>::const_iterator it0 = _per_type_mesh.begin();
         ...
         it0++)
        for (std::vector<MCAuto<MEDFileUMeshPerType>>::const_iterator it1 = (*it0).begin(); it1 != (*it0).end(); it1++)
            (*it1)->getMesh()->computeNodeIdsAlg(
                fetchedNodeIds
            );  // for each node in the original mesh, which ones are laying on the current single geometrical type
                // partial mesh
    ...
    if (!mrs || mrs->getNumberOfCoordsLoadSessions() == 1)
    {
        ...
        for (std::map<mcIdType, mcIdType>::const_iterator mapIter = o2n->data().begin(); mapIter != o2n->data().end();
             ++mapIter)
...
void
MEDFileUMeshL2::loadConnectivity(
    med_idt fid, int mdim, const std::string &mName, int dt, int it, MEDFileMeshReadSelector *mrs
)
{
    ...
    {
        MEDFileUMeshPerType *tmp(
            MEDFileUMeshPerType::New(fid, mName.c_str(), dt, it, mdim, typmai[j], typmai2[j], mrs)
        );
        if (tmp)
...
void
MEDFileUMeshL2::loadPartOfConnectivity(
    med_idt fid,
    int mdim,
    const std::string &mName,
    const std::vector<INTERP_KERNEL::NormalizedCellType> &types,
...
)
{
    ...
    if (slicPerTyp.size() != 3 * nbOfTypes)
        throw INTERP_KERNEL::Exception(
            "MEDFileUMeshL2::loadPartOfConnectivity : The size of slicPerTyp array is expected to be equal to 3 times "
            "size of array types !"
        );
    std::set<INTERP_KERNEL::NormalizedCellType> types2(types.begin(), types.end());
    if (types2.size() != nbOfTypes)
        throw INTERP_KERNEL::Exception(
            "MEDFileUMeshL2::loadPartOfConnectivity : the geometric types in types array must appear once !"
        );
    _per_type_mesh.resize(1);
    _per_type_mesh[0].clear();
    for (std::size_t ii = 0; ii < nbOfTypes; ii++)
    {
        ...
        MCAuto<MEDFileUMeshPerType> tmp(
            MEDFileUMeshPerType::NewPart(fid, mName.c_str(), dt, it, mdim, types[ii], strt, stp, step, mrs)
        );
        _per_type_mesh[0].push_back(tmp);
    }
    sortTypes();
...
/*!
 * This method builds a new mesh of single geometrical type based on the partition of cells \a distrib, from mesh \a
 * mName in file \a fid. This distribution is not necessarily a slice.
 */
void
MEDFileUMeshL2::loadPartOfConnectivityFromUserDistrib(
    med_idt fid,
    int mdim,
    const std::string &mName,
    const std::map<INTERP_KERNEL::NormalizedCellType, std::vector<mcIdType>> &distrib,
        ...
        if (iter->second.size() != 0)
        {
            MCAuto<MEDFileUMeshPerType> tmp(
                MEDFileUMeshPerType::NewPart(
                    ...
                    geoElt,
                    whichEntity,
                    iter->second /*distrib over the current type*/,
                    mrs
                )
            );
            _per_type_mesh[0].push_back(tmp);
...
void
MEDFileUMeshL2::loadCoords(
    med_idt fid, const std::vector<std::string> &infosOnComp, const std::string &mName, int dt, int it
)
{
    int spaceDim((int)infosOnComp.size());
    med_bool changement, transformation;
    med_int nCoords(MEDmeshnEntity(
        fid, mName.c_str(), dt, it, MED_NODE, MED_NONE, MED_COORDINATE, MED_NO_CMODE, &changement, &transformation
    ));
        ...
        ) > 0)
    {
        ...
        MEDFILESAFECALLERRD0(
            ...
            (fid, mName.c_str(), dt, it, MED_NODE, MED_NO_GEOTYPE, miFamCoord->getPointer())
        );
        ...
        _fam_coords = 0;
    if (MEDmeshnEntity(
            fid, mName.c_str(), dt, it, MED_NODE, MED_NO_GEOTYPE, MED_NUMBER, MED_NODAL, &changement, &transformation
        ) > 0)
    {
        MCAuto<DataArrayMedInt> miNumCoord = DataArrayMedInt::New();
        miNumCoord->alloc(nCoords, 1);
        MEDFILESAFECALLERRD0(
            MEDmeshEntityNumberRd, (fid, mName.c_str(), dt, it, MED_NODE, MED_NO_GEOTYPE, miNumCoord->getPointer())
        );
        ...
        _num_coords = 0;
    if (MEDmeshnEntity(
            fid, mName.c_str(), dt, it, MED_NODE, MED_NO_GEOTYPE, MED_NAME, MED_NODAL, &changement, &transformation
        ) > 0)
    {
        _name_coords = DataArrayAsciiChar::New();
        _name_coords->alloc(
            nCoords + 1, MED_SNAME_SIZE
        ...
        ) > 0)
    {
        ...
        miNumCoord->alloc(nCoords, 1);
        MEDFILESAFECALLERRD0(
            MEDmeshGlobalNumberRd, (fid, mName.c_str(), dt, it, MED_NODE, MED_NO_GEOTYPE, miNumCoord->getPointer())
        );
        _global_num_coords = FromMedIntArray<mcIdType>(miNumCoord);
    }
    for (int i = 0; i < spaceDim; i++) _coords->setInfoOnComponent(i, infosOnComp[i]);
}
...
MEDFileUMeshL2::LoadPartCoords(
    ...
    int dt,
    int it,
    const std::vector<mcIdType> &distribNodes,
...
/*!
 * This method allocates the space needed to load coordinates of nodes specified in the vector \a nodeIds and creates a
 * PartDefinition object to store the ids in \a nodeIds
 */
void
MEDFileUMeshL2::allocCoordsPartCoords(
    mcIdType spaceDim,
...
/*!
 * This method allocates the space needed to load coordinates of all nodes between \a nMin and \a nMax and creates a
 * PartDefinition object to store them
 */
void
MEDFileUMeshL2::allocCoordsPartCoords(
    mcIdType spaceDim,
 ...
 */
void
MEDFileUMeshL2::fillPartCoords(
    med_idt fid,
    mcIdType spaceDim,
    const std::string &mName,
    int dt,
    int it,
    const PartDefinition *partCoords,
    MCAuto<DataArrayDouble> &_coords,
    MCAuto<DataArrayIdType> &_fam_coords,
    MCAuto<DataArrayIdType> &_num_coords,
    MCAuto<DataArrayAsciiChar> &_name_coords
)
{
    ...
    // Based on the ids in \a partCoords, defining the appropriate med_filter (filter of block if the ids form a slice,
    // a generic filter otherwise)
    {
        ...
        MEDFILESAFECALLERRD0(
            MEDmeshNodeCoordinateAdvancedRd, (fid, mName.c_str(), dt, it, filter1.getPtr(), _coords->getPointer())
        );
    }

    {
        ...
        filter2.fill(
            fid, nCoords, 1, 1, MED_ALL_CONSTITUENT, MED_FULL_INTERLACE, MED_COMPACT_STMODE, MED_NO_PROFILE, partCoords
        );
            ...
            ) > 0)
        {
            _name_coords = DataArrayAsciiChar::New();
            _name_coords->alloc(
                nbNodesToLoad + 1, MED_SNAME_SIZE
            );  // not a bug to avoid the memory corruption due to last \0 at the end
            MEDFILESAFECALLERRD0(
                MEDmeshEntityAttributeAdvancedRd,
                (fid,
 ...
 */
void
MEDFileUMeshL2::LoadPartCoordsArray(
    med_idt fid,
    const std::vector<std::string> &infosOnComp,
    const std::string &mName,
    int dt,
    int it,
    const DataArrayIdType *nodeIds,
    MCAuto<DataArrayDouble> &_coords,
    MCAuto<DataArrayIdType> &_fam_coords,
    MCAuto<DataArrayIdType> &_num_coords,
    MCAuto<DataArrayAsciiChar> &_name_coords
)
{
    ...
    LoadPartCoords(
        fid, infosOnComp, mName, dt, it, nMin, nMax, _coords, useless, _fam_coords, _num_coords, _name_coords
    );
    if (nodeIds->empty())
        return;
...
{
    LoadPartCoords(
        fid, infosOnComp, mName, dt, it, nMin, nMax, _coords, _part_coords, _fam_coords, _num_coords, _name_coords
    );
}
...
{
    LoadPartCoords(
        fid, infosOnComp, mName, dt, it, distribNodes, _coords, _part_coords, _fam_coords, _num_coords, _name_coords
    );
}

void
MEDFileUMeshL2::loadPartCoordsSlice(
    med_idt fid,
    const std::vector<std::string> &infosOnComp,
    const std::string &mName,
    int dt,
...
)
{
    ...
    for (mcIdType ipart = 0; ipart < nbOfCoordLS; ++ipart)
    {
        ...
        MCAuto<DataArrayIdType> nodeIdsToKeep(
            nodeIds->selectByTupleIdSafe(idsNodeIdsToKeep->begin(), idsNodeIdsToKeep->end())
        );
        LoadPartCoordsArray(
            fid,
            infosOnComp,
...
MEDFileUMeshL2::sortTypes()
{
    ...
    bool isFirst = true;
    for (std::vector<std::vector<MCAuto<MEDFileUMeshPerType>>>::reverse_iterator it2 = _per_type_mesh.rbegin();
         it2 != _per_type_mesh.rend();
         it2++)
    {
...
MEDFileUMeshL2::WriteCoords(
    ...
    const DataArrayIdType *famCoords,
    const DataArrayIdType *numCoords,
    const DataArrayAsciiChar *nameCoords,
    const DataArrayIdType *globalNumCoords
)
{
    if (!coords)
        return;
    MEDFILESAFECALLERWR0(
        ...
        (fid, mname.c_str(), dt, it, time, MED_FULL_INTERLACE, ToMedInt(coords->getNumberOfTuples()), coords->begin())
    );
    if (famCoords)
        MEDFILESAFECALLERWR0(
            ...
            (fid,
             ...
             MED_NODE,
             MED_NO_GEOTYPE,
             ToMedInt(famCoords->getNumberOfTuples()),
             ToMedIntArray<mcIdType>(famCoords)->begin())
        );
    if (numCoords)
        MEDFILESAFECALLERWR0(
            ...
            (fid,
             ...
             MED_NODE,
             MED_NO_GEOTYPE,
             ToMedInt(numCoords->getNumberOfTuples()),
             ToMedIntArray<mcIdType>(numCoords)->begin())
        );
    if (nameCoords)
    {
        ...
        {
            std::ostringstream oss;
            oss << " MEDFileUMeshL2::WriteCoords : expected a name field on nodes with number of components set to "
                << MED_SNAME_SIZE;
        ...
        MEDFILESAFECALLERWR0(
            ...
            (fid,
             ...
             MED_NODE,
             MED_NO_GEOTYPE,
             ToMedInt(nameCoords->getNumberOfTuples()),
             nameCoords->begin())
        );
    }
    if (globalNumCoords)
        MEDFILESAFECALLERWR0(
            ...
            (fid,
             ...
             MED_NODE,
             MED_NONE,
             ToMedInt(globalNumCoords->getNumberOfTuples()),
             ToMedIntArray<mcIdType>(globalNumCoords)->begin())
        );
}

bool
MEDFileUMeshL2::isFamDefinedOnLev(int levId) const
{
    for (std::vector<MCAuto<MEDFileUMeshPerType>>::const_iterator it = _per_type_mesh[levId].begin();
         it != _per_type_mesh[levId].end();
         it++)
        if ((*it)->getFam() == 0)
            return false;
    return true;
...
bool
MEDFileUMeshL2::isNumDefinedOnLev(int levId) const
{
    for (std::vector<MCAuto<MEDFileUMeshPerType>>::const_iterator it = _per_type_mesh[levId].begin();
         it != _per_type_mesh[levId].end();
         it++)
...
bool
MEDFileUMeshL2::isNamesDefinedOnLev(int levId) const
{
    for (std::vector<MCAuto<MEDFileUMeshPerType>>::const_iterator it = _per_type_mesh[levId].begin();
         it != _per_type_mesh[levId].end();
         it++)
...
void
MEDFileCMeshL2::loadAll(med_idt fid, const MeshOrStructMeshCls *mId, const std::string &mName, int dt, int it)
{
    _name.set(mName.c_str());
    int nstep;
    int Mdim;
    ...
    if (meshType != CARTESIAN)
        throw INTERP_KERNEL::Exception(
            "Invalid mesh type ! You are expected a structured one whereas in file it is not a structured !"
        );
    _time = mId->checkMeshTimeStep(fid, mName, nstep, dt, it);
    _iteration = dt;
    _order = it;
    ...
    for (int i = 0; i < Mdim; i++)
    {
        ...
        med_bool chgt = MED_FALSE, trsf = MED_FALSE;
        med_int nbOfElt(
            MEDmeshnEntity(fid, mName.c_str(), dt, it, MED_NODE, MED_NONE, dataTypeReq, MED_NO_CMODE, &chgt, &trsf)
        );
...
med_data_type
MEDFileCMeshL2::GetDataTypeCorrespondingToSpaceId(int id)
{
    switch (id)
    {
        case 0:
...
void
MEDFileCLMeshL2::loadAll(med_idt fid, const MeshOrStructMeshCls *mId, const std::string &mName, int dt, int it)
{
    _name.set(mName.c_str());
    int nstep;
    int Mdim;
    ...
    med_bool chgt = MED_FALSE, trsf = MED_FALSE;
    med_int nbNodes(
        MEDmeshnEntity(fid, mName.c_str(), dt, it, MED_NODE, MED_NONE, MED_COORDINATE, MED_NO_CMODE, &chgt, &trsf)
    );
    MCAuto<DataArrayDouble> da = DataArrayDouble::New();
...
void
MEDFileUMeshPermCompute::operator=(MEDCouplingUMesh *m)
{
    _m = m;
}

void
MEDFileUMeshPermCompute::updateTime() const
{
...
std::vector<const BigMemoryObject *>
MEDFileUMeshPermCompute::getDirectChildrenWithNull() const
{
...
std::size_t
MEDFileUMeshPermCompute::getHeapMemorySizeWithoutChildren() const
{
    return sizeof(MEDFileUMeshPermCompute);
}

MEDFileUMeshSplitL1::MEDFileUMeshSplitL1(const MEDFileUMeshSplitL1 &other)
    : RefCountObject(other),
      _m_by_types(other._m_by_types),
      _fam(other._fam),
      _num(other._num),
      _names(other._names),
      _rev_num(other._rev_num),
      _m(this)
{
}
...
MEDFileUMeshSplitL1::MEDFileUMeshSplitL1(const MEDFileUMeshL2 &l2, const std::string &mName, int id) : _m(this)
{
    ...
    {
        for (std::size_t i = 0; i < sz; i++) fams[i] = v[i]->getFam();
        if (sz != 1)
    ...
    {
        for (std::size_t i = 0; i < sz; i++) nums[i] = v[i]->getNum();
        if (sz != 1)
    ...
    {
        for (std::size_t i = 0; i < sz; i++) names[i] = v[i]->getNames();
...
MEDFileUMeshSplitL1::MEDFileUMeshSplitL1(MEDCouplingUMesh *m) : _m(this) { assignMesh(m, true); }

MEDFileUMeshSplitL1::MEDFileUMeshSplitL1(MEDCouplingUMesh *m, bool newOrOld) : _m(this) { assignMesh(m, newOrOld); }

void
MEDFileUMeshSplitL1::setName(const std::string &name)
{
    _m_by_types.setName(name);
}

std::size_t
MEDFileUMeshSplitL1::getHeapMemorySizeWithoutChildren() const
{
    return 0;
}

std::vector<const BigMemoryObject *>
MEDFileUMeshSplitL1::getDirectChildrenWithNull() const
{
    std::vector<const BigMemoryObject *> ret;
...
void
MEDFileUMeshSplitL1::checkConsistency() const
{
    if (!_fam || _fam->getNumberOfTuples() != getSize())
        throw INTERP_KERNEL::Exception(
            "MEDFileUMeshSplitL1::checkConsistency(): internal family array has an invalid size!"
        );
    mcIdType nbCells = getSize();
    if (_num)
    {
        _num->checkNbOfTuplesAndComp(
            nbCells, 1, "MEDFileUMeshSplitL1::checkConsistency(): inconsistent internal node numbering array!"
        );
        mcIdType pos;
        mcIdType maxValue = _num->getMaxValue(pos);
        if (!_rev_num || _rev_num->getNumberOfTuples() != (maxValue + 1))
            throw INTERP_KERNEL::Exception(
                "MEDFileUMeshSplitL1::checkConsistency(): inconsistent internal revert node numbering array!"
            );
    }
    if ((_num && !_rev_num) || (!_num && _rev_num))
        throw INTERP_KERNEL::Exception(
            "MEDFileUMeshSplitL1::checkConsistency(): inconsistent internal numbering arrays (one is null)!"
        );
    if (_num && !_num->hasUniqueValues())
        throw INTERP_KERNEL::Exception(
            "MEDFileUMeshSplitL1::checkConsistency(): inconsistent internal node numbering array: duplicates found!"
        );
    if (_names)
        _names->checkNbOfTuplesAndComp(
            nbCells, 1, "MEDFileUMeshSplitL1::checkConsistency(): internal cell naming array has an invalid size!"
        );
...
bool
MEDFileUMeshSplitL1::isEqual(const MEDFileUMeshSplitL1 *other, double eps, std::string &what) const
{
...
void
MEDFileUMeshSplitL1::synchronizeTinyInfo(const MEDFileMesh &master) const
{
    _m_by_types.synchronizeTinyInfo(master);
}

void
MEDFileUMeshSplitL1::clearNonDiscrAttributes() const
{
    _m_by_types.clearNonDiscrAttributes();
}

void
MEDFileUMeshSplitL1::ClearNonDiscrAttributes(const MEDCouplingMesh *tmp)
{
    if (!tmp)
        return;
...
void
MEDFileUMeshSplitL1::setCoords(DataArrayDouble *coords)
{
    _m_by_types.setCoords(coords);
}

void
MEDFileUMeshSplitL1::assignMesh(MEDCouplingUMesh *m, bool newOrOld)
{
    if (newOrOld)
    {
        m->incrRef();
        ...
        MCAuto<DataArrayIdType> da =
            _m_by_types.getUmesh()->getRenumArrForConsecutiveCellTypesSpec(typmai2, typmai2 + MED_N_CELL_FIXED_GEO);
    ...
    else
    {
        if (!m->checkConsecutiveCellTypesAndOrder(typmai2, typmai2 + MED_N_CELL_FIXED_GEO))
            throw INTERP_KERNEL::Exception(
                "MEDFileUMeshSplitL1::assignMesh(): the mesh does not follow the MED file numbering convention! Invoke "
                "sortCellsInMEDFileFrmt() first!"
            );
        m->incrRef();
...
void
MEDFileUMeshSplitL1::forceComputationOfParts() const
{
    _m_by_types.forceComputationOfPartsFromUMesh();
}

void
MEDFileUMeshSplitL1::declarePartsUpdated() const
{
    _m_by_types.declarePartsUpdated();
}

void
MEDFileUMeshSplitL1::assignParts(const std::vector<const MEDCoupling1GTUMesh *> &mParts)
{
...
void
MEDFileUMeshSplitL1::assignCommonPart()
{
...
bool
MEDFileUMeshSplitL1::empty() const
{
    return _m_by_types.empty();
}

bool
MEDFileUMeshSplitL1::presenceOfOneFams(const std::vector<mcIdType> &ids) const
{
...
int
MEDFileUMeshSplitL1::getMeshDimension() const
{
    return _m_by_types.getMeshDimension();
}

void
MEDFileUMeshSplitL1::simpleRepr(std::ostream &oss) const
{
    ...
    for (std::size_t i = 0; i < nbOfTypes; i++)
    {
        ...
        oss << "    - Number of cells with type " << INTERP_KERNEL::CellModel::GetCellModel(typ).getRepr() << " : "
            << code[3 * i + 1] << std::endl;
    }
}

mcIdType
MEDFileUMeshSplitL1::getSize() const
{
    return _m_by_types.getSize();
}

MEDCouplingUMesh *
MEDFileUMeshSplitL1::getFamilyPart(const mcIdType *idsBg, const mcIdType *idsEnd, bool renum) const
{
    MCAuto<DataArrayIdType> eltsToKeep = _fam->findIdsEqualList(idsBg, idsEnd);
    MEDCouplingUMesh *m =
        (MEDCouplingUMesh *)_m_by_types.getUmesh()->buildPartOfMySelf(eltsToKeep->begin(), eltsToKeep->end(), true);
    if (renum)
...
DataArrayIdType *
MEDFileUMeshSplitL1::getFamilyPartArr(const mcIdType *idsBg, const mcIdType *idsEnd, bool renum) const
{
...
MEDFileUMeshSplitL1::getGeoTypes() const
{
    return _m_by_types.getGeoTypes();
}

mcIdType
MEDFileUMeshSplitL1::getNumberOfCellsWithType(INTERP_KERNEL::NormalizedCellType ct) const
{
    return _m_by_types.getNumberOfCellsWithType(ct);
}

MEDCouplingUMesh *
MEDFileUMeshSplitL1::getWholeMesh(bool renum) const
{
...
mcIdType
MEDFileUMeshSplitL1::getNumberOfCells() const
{
    return _m_by_types.getNumberOfCells();
}

DataArrayIdType *
MEDFileUMeshSplitL1::extractFamilyFieldOnGeoType(INTERP_KERNEL::NormalizedCellType gt) const
{
    const DataArrayIdType *fam(_fam);
    if (!fam)
        return 0;
...
DataArrayIdType *
MEDFileUMeshSplitL1::extractNumberFieldOnGeoType(INTERP_KERNEL::NormalizedCellType gt) const
{
...
DataArrayIdType *
MEDFileUMeshSplitL1::getOrCreateAndGetFamilyField()
{
    if ((DataArrayIdType *)_fam)
        return _fam;
    mcIdType nbOfTuples = _m_by_types.getSize();
    _fam = DataArrayIdType::New();
    _fam->alloc(nbOfTuples, 1);
    _fam->fillWithZero();
    return _fam;
}

const DataArrayIdType *
MEDFileUMeshSplitL1::getFamilyField() const
{
    return _fam;
}

const DataArrayIdType *
MEDFileUMeshSplitL1::getNumberField() const
{
    return _num;
}

const DataArrayIdType *
MEDFileUMeshSplitL1::getRevNumberField() const
{
    return _rev_num;
}

const DataArrayAsciiChar *
MEDFileUMeshSplitL1::getNameField() const
{
    return _names;
}

const PartDefinition *
MEDFileUMeshSplitL1::getPartDef(INTERP_KERNEL::NormalizedCellType gt) const
{
    return _m_by_types.getPartDefOfWithoutComputation(gt);
}

void
MEDFileUMeshSplitL1::eraseFamilyField()
{
    _fam->fillWithZero();
}
 ...
 */
void
MEDFileUMeshSplitL1::setGroupsFromScratch(
    const std::vector<const MEDCouplingUMesh *> &ms,
    std::map<std::string, mcIdType> &familyIds,
...
)
{
    ...
    for (mcIdType i = 0; i < nbOfCells; i++, w++) *w = famIdTrad[*w];
}

void
MEDFileUMeshSplitL1::checkCoordsConsistency(const DataArrayDouble *coords) const
{
    ...
    for (auto mesh : ms)
    {
        if (mesh)
            if (mesh->getCoords() != coords)
                mesh->getCoords()->checkNbOfTuplesAndComp(
                    *coords,
                    "MEDFileUMeshSplitL1::checkCoordsConsistency : mismatch between coordinates instance in "
                    "MEDFileUMesh and instance in subparts"
                );
    }
}

void
MEDFileUMeshSplitL1::write(med_idt fid, const std::string &mName, int mdim) const
{
    std::vector<MEDCoupling1GTUMesh *> ms(_m_by_types.getParts());
    mcIdType start = 0;
...
void
MEDFileUMeshSplitL1::renumberNodesInConn(const mcIdType *newNodeNumbersO2N)
{
    _m_by_types.renumberNodesInConnWithoutComputation(newNodeNumbersO2N);
}

void
MEDFileUMeshSplitL1::serialize(std::vector<mcIdType> &tinyInt, std::vector<MCAuto<DataArrayIdType>> &bigArraysI) const
{
...
void
MEDFileUMeshSplitL1::unserialize(
...
{
    _fam = bigArraysI.back();
    bigArraysI.pop_back();
    _num = bigArraysI.back();
    bigArraysI.pop_back();
    _m_by_types.unserialize(name, coo, tinyInt, bigArraysI);
}

void
MEDFileUMeshSplitL1::changeFamilyIdArr(mcIdType oldId, mcIdType newId)
{
...
void
MEDFileUMeshSplitL1::setFamilyArr(DataArrayIdType *famArr)
{
    if (!famArr)
    {
        _fam = 0;
...
DataArrayIdType *
MEDFileUMeshSplitL1::getFamilyField()
{
    return _fam;
}

void
MEDFileUMeshSplitL1::setRenumArr(DataArrayIdType *renumArr)
{
    if (!renumArr)
    {
        _num = 0;
...
void
MEDFileUMeshSplitL1::setNameArr(DataArrayAsciiChar *nameArr)
{
    if (!nameArr)
    {
        _names = 0;
    ...
    nameArr->checkNbOfTuplesAndComp(
        sz, MED_SNAME_SIZE, "MEDFileUMeshSplitL1::setNameArr : Problem in size of name arr ! "
    );
    nameArr->incrRef();
    _names = nameArr;
}
...
MEDCouplingUMesh *
MEDFileUMeshSplitL1::Renumber2(const DataArrayIdType *renum, MEDCouplingUMesh *m, const mcIdType *cellIds)
{
    if (renum == 0)
        return m;
    if (cellIds == 0)
...
MEDCouplingUMesh *
MEDFileUMeshSplitL1::renumIfNeeded(MEDCouplingUMesh *m, const mcIdType *cellIds) const
{
    return Renumber2(_num, m, cellIds);
}

DataArrayIdType *
MEDFileUMeshSplitL1::Renumber(const DataArrayIdType *renum, const DataArrayIdType *da)
{
...
DataArrayIdType *
MEDFileUMeshSplitL1::renumIfNeededArr(const DataArrayIdType *da) const
{
    return Renumber(_num, da);
}

std::vector<mcIdType>
MEDFileUMeshSplitL1::GetNewFamiliesNumber(mcIdType nb, const std::map<std::string, mcIdType> &families)
{
    mcIdType id = -1;
    ...
    for (mcIdType i = 1; i <= nb; i++) ret[i] = id + i;
    return ret;
}

void
MEDFileUMeshSplitL1::TraduceFamilyNumber(
    const std::vector<std::vector<mcIdType>> &fidsGrps,
    std::map<std::string, mcIdType> &familyIds,
    std::map<mcIdType, mcIdType> &famIdTrad,
    std::map<mcIdType, std::string> &newfams
)
...
void
MEDFileUMeshSplitL1::computeRevNum() const
{
    mcIdType pos;
    if (!_num->empty())
    {
...
void
MEDFileUMeshAggregateCompute::setName(const std::string &name)
{
    if (_m_time >= _mp_time)
    {
...
void
MEDFileUMeshAggregateCompute::assignParts(const std::vector<const MEDCoupling1GTUMesh *> &mParts)
{
...
void
MEDFileUMeshAggregateCompute::assignDefParts(const std::vector<const PartDefinition *> &partDefs)
{
    if (_mp_time < _m_time)
        throw INTERP_KERNEL::Exception(
            "MEDFileUMeshAggregateCompute::assignDefParts : the parts require a computation !"
        );
    std::size_t sz(partDefs.size());
    if (_part_def.size() != partDefs.size() || _part_def.size() != _m_parts.size())
        throw INTERP_KERNEL::Exception(
            "MEDFileUMeshAggregateCompute::assignDefParts : sizes of vectors of part definition mismatch !"
        );
    for (std::size_t i = 0; i < sz; i++)
    {
...
void
MEDFileUMeshAggregateCompute::assignUMesh(MEDCouplingUMesh *m)
{
    _m = m;
    _m_parts.clear();
...
MEDCouplingUMesh *
MEDFileUMeshAggregateCompute::getUmesh() const
{
    if (_mp_time <= _m_time)
        return _m;
...
mcIdType
MEDFileUMeshAggregateCompute::getNumberOfCells() const
{
    if (_mp_time <= _m_time)
...
std::vector<INTERP_KERNEL::NormalizedCellType>
MEDFileUMeshAggregateCompute::getGeoTypes() const
{
    if (_mp_time >= _m_time)
    {
        std::size_t sz(_m_parts.size());
        std::vector<INTERP_KERNEL::NormalizedCellType> ret(sz);
        for (std::size_t i = 0; i < sz; i++) ret[i] = _m_parts[i]->getCellModelEnum();
        return ret;
    }
    else
...
mcIdType
MEDFileUMeshAggregateCompute::getNumberOfCellsWithType(INTERP_KERNEL::NormalizedCellType ct) const
{
    if (_mp_time >= _m_time)
    {
        for (std::vector<MCAuto<MEDCoupling1GTUMesh>>::const_iterator it = _m_parts.begin(); it != _m_parts.end(); it++)
...
MEDFileUMeshAggregateCompute::retrievePartsWithoutComputation() const
{
    if (_mp_time < _m_time)
        throw INTERP_KERNEL::Exception(
            "MEDFileUMeshAggregateCompute::getPartsWithoutComputation : the parts require a computation !"
        );
    //
    std::vector<MEDCoupling1GTUMesh *> ret(_m_parts.size());
    std::size_t i(0);
    for (std::vector<MCAuto<MEDCoupling1GTUMesh>>::const_iterator it = _m_parts.begin(); it != _m_parts.end();
         it++, i++)
    {
        const MEDCoupling1GTUMesh *elt(*it);
        ret[i] = const_cast<MEDCoupling1GTUMesh *>(elt);
...
std::vector<MEDCoupling1GTUMesh *>
MEDFileUMeshAggregateCompute::getParts() const
{
    if (_mp_time < _m_time)
...
void
MEDFileUMeshAggregateCompute::highlightUsedNodes(std::vector<bool> &nodesToBeHighlighted) const
{
    if (_mp_time < _m_time)
        forceComputationOfPartsFromUMesh();
...
MEDCoupling1GTUMesh *
MEDFileUMeshAggregateCompute::retrievePartWithoutComputation(INTERP_KERNEL::NormalizedCellType gt) const
{
    ...
    throw INTERP_KERNEL::Exception(
        "MEDFileUMeshAggregateCompute::getPartWithoutComputation : the geometric type is not existing !"
    );
}

void
MEDFileUMeshAggregateCompute::getStartStopOfGeoTypeWithoutComputation(
    INTERP_KERNEL::NormalizedCellType gt, mcIdType &start, mcIdType &stop
) const
{
    start = 0;
    stop = 0;
    ...
    throw INTERP_KERNEL::Exception(
        "MEDFileUMeshAggregateCompute::getStartStopOfGeoTypeWithoutComputation : the geometric type is not existing !"
    );
}

void
MEDFileUMeshAggregateCompute::renumberNodesInConnWithoutComputation(const mcIdType *newNodeNumbersO2N)
{
    if (_mp_time > _m_time)
    {
        for (std::vector<MCAuto<MEDCoupling1GTUMesh>>::iterator it = _m_parts.begin(); it != _m_parts.end(); it++)
...
void
MEDFileUMeshAggregateCompute::forceComputationOfPartsFromUMesh() const
{
    ...
    if (!m)
    {
        if (_m_parts.empty())
            throw INTERP_KERNEL::Exception(
                "MEDFileUMeshAggregateCompute::forceComputationOfPartsFromUMesh : null UMesh !"
            );
        else
    ...
    _m_parts.resize(sz);
    for (std::size_t i = 0; i < sz; i++) _m_parts[i] = MEDCoupling1GTUMesh::New(ms[i]);
    _part_def.clear();
...
void
MEDFileUMeshAggregateCompute::declarePartsUpdated() const
{
    _mp_time = std::max(_mp_time, _m_time) + 1;
    _m.nullify();
}
...
const PartDefinition *
MEDFileUMeshAggregateCompute::getPartDefOfWithoutComputation(INTERP_KERNEL::NormalizedCellType gt) const
{
    if (_mp_time < _m_time)
        throw INTERP_KERNEL::Exception(
            "MEDFileUMeshAggregateCompute::getPartDefOfWithoutComputation : the parts require a computation !"
        );
    if (_m_parts.size() != _part_def.size())
        throw INTERP_KERNEL::Exception(
            "MEDFileUMeshAggregateCompute::getPartDefOfWithoutComputation : size of arrays are expected to be the same "
            "!"
        );
    std::size_t sz(_m_parts.size());
    for (std::size_t i = 0; i < sz; i++)
    ...
    throw INTERP_KERNEL::Exception(
        "MEDFileUMeshAggregateCompute::getPartDefOfWithoutComputation : The input geo type is not existing in this !"
    );
}

void
MEDFileUMeshAggregateCompute::serialize(
...
) const
{
    ...
    for (std::size_t i = 0; i < sz; i++)
    {
        ...
        else if (mesh2)
        {
            ...
            bigArraysI.push_back(elt11);
            bigArraysI.push_back(elt22);
        }
        else
            throw INTERP_KERNEL::Exception(
                "MEDFileUMeshAggregateCompute::serialize : unrecognized single geo type mesh !"
            );
...
void
MEDFileUMeshAggregateCompute::unserialize(
    const std::string &name,
    DataArrayDouble *coo,
    std::vector<mcIdType> &tinyInt,
    std::vector<MCAuto<DataArrayIdType>> &bigArraysI
...
{
    mcIdType nbParts(tinyInt.back());
    tinyInt.pop_back();
    _part_def.clear();
    _part_def.resize(nbParts);
    _m_parts.clear();
    _m_parts.resize(nbParts);
    for (mcIdType i = 0; i < nbParts; i++)
    {
        INTERP_KERNEL::NormalizedCellType tp((INTERP_KERNEL::NormalizedCellType)tinyInt.back());
        tinyInt.pop_back();
        ...
        if (mesh1)
        {
            mesh1->setNodalConnectivity(bigArraysI.back());
            bigArraysI.pop_back();
        }
        else if (mesh2)
        {
            MCAuto<DataArrayIdType> elt0, elt1;
            elt0 = bigArraysI.back();
            bigArraysI.pop_back();
            elt1 = bigArraysI.back();
            bigArraysI.pop_back();
            mesh2->setNodalConnectivity(elt0, elt1);
        }
        else
            throw INTERP_KERNEL::Exception(
                "MEDFileUMeshAggregateCompute::unserialize : unrecognized single geo type mesh !"
            );
        _m_parts[i] = mesh;
        mcIdType pdid(tinyInt.back());
        tinyInt.pop_back();
        if (pdid != -1)
 ...
 */
bool
MEDFileUMeshAggregateCompute::isStoredSplitByType() const
{
    return _mp_time >= _m_time;
}

std::size_t
MEDFileUMeshAggregateCompute::getTimeOfThis() const
{
    if (_mp_time > _m_time)
...
std::size_t
MEDFileUMeshAggregateCompute::getTimeOfParts() const
{
    std::size_t ret(0);
    for (std::vector<MCAuto<MEDCoupling1GTUMesh>>::const_iterator it = _m_parts.begin(); it != _m_parts.end(); it++)
    {
...
std::size_t
MEDFileUMeshAggregateCompute::getTimeOfUMesh() const
{
    const MEDCouplingUMesh *m(_m);
    if (!m)
...
std::size_t
MEDFileUMeshAggregateCompute::getHeapMemorySizeWithoutChildren() const
{
...
std::vector<const BigMemoryObject *>
MEDFileUMeshAggregateCompute::getDirectChildrenWithNull() const
{
    std::vector<const BigMemoryObject *> ret;
    for (std::vector<MCAuto<MEDCoupling1GTUMesh>>::const_iterator it = _m_parts.begin(); it != _m_parts.end(); it++)
...
void
MEDFileUMeshAggregateCompute::shallowCpyMeshes()
{
    for (std::vector<MCAuto<MEDCoupling1GTUMesh>>::iterator it = _m_parts.begin(); it != _m_parts.end(); it++)
    {
        const MEDCoupling1GTUMesh *elt(*it);
...
bool
MEDFileUMeshAggregateCompute::isEqual(const MEDFileUMeshAggregateCompute &other, double eps, std::string &what) const
{
...
void
MEDFileUMeshAggregateCompute::checkConsistency() const
{
    if (_mp_time >= _m_time)
        for (std::vector<MCAuto<MEDCoupling1GTUMesh>>::const_iterator it = _m_parts.begin(); it != _m_parts.end(); it++)
            (*it)->checkConsistency();
...
void
MEDFileUMeshAggregateCompute::clearNonDiscrAttributes() const
{
    for (std::vector<MCAuto<MEDCoupling1GTUMesh>>::const_iterator it = _m_parts.begin(); it != _m_parts.end(); it++)
...
void
MEDFileUMeshAggregateCompute::synchronizeTinyInfo(const MEDFileMesh &master) const
{
    for (std::vector<MCAuto<MEDCoupling1GTUMesh>>::const_iterator it = _m_parts.begin(); it != _m_parts.end(); it++)
    {
        ...
        if (tmp)
        {
            ...
            (const_cast<MEDCoupling1GTUMesh *>(tmp))
                ->setTime(master.getTimeValue(), master.getIteration(), master.getOrder());
...
bool
MEDFileUMeshAggregateCompute::empty() const
{
    if (_mp_time < _m_time)
...
int
MEDFileUMeshAggregateCompute::getMeshDimension() const
{
    if (_mp_time < _m_time)
    {
        const MEDCouplingUMesh *m(_m);
    ...
    else
    {
        ...
        if (!m)
            throw INTERP_KERNEL::Exception(
                "MEDFileUMeshAggregateCompute::getMeshDimension : part mesh contains null instance !"
            );
        return m->getMeshDimension();
    }
}
...
std::vector<mcIdType>
MEDFileUMeshAggregateCompute::getDistributionOfTypes() const
{
    if (_mp_time < _m_time)
    {
        const MEDCouplingUMesh *m(_m);
    ...
    else
    {
        ...
        for (std::vector<MCAuto<MEDCoupling1GTUMesh>>::const_iterator it = _m_parts.begin(); it != _m_parts.end(); it++)
        {
            ...
            if (!tmp)
                throw INTERP_KERNEL::Exception(
                    "MEDFileUMeshAggregateCompute::getDistributionOfTypes : part mesh contains null instance !"
                );
...
mcIdType
MEDFileUMeshAggregateCompute::getSize() const
{
    if (_mp_time < _m_time)
    {
        const MEDCouplingUMesh *m(_m);
    ...
    else
    {
        ...
        for (std::vector<MCAuto<MEDCoupling1GTUMesh>>::const_iterator it = _m_parts.begin(); it != _m_parts.end(); it++)
        {
            ...
            if (!m)
                throw INTERP_KERNEL::Exception(
                    "MEDFileUMeshAggregateCompute::getSize : part mesh contains null instance !"
                );
...
void
MEDFileUMeshAggregateCompute::setCoords(DataArrayDouble *coords)
{
    for (std::vector<MCAuto<MEDCoupling1GTUMesh>>::iterator it = _m_parts.begin(); it != _m_parts.end(); it++)
    {
        MEDCoupling1GTUMesh *tmp(*it);
...
MEDFileEltStruct4Mesh::New(
    med_idt fid, const std::string &mName, int dt, int it, int iterOnStEltOfMesh, MEDFileMeshReadSelector *mrs
)
{
...
std::size_t
MEDFileEltStruct4Mesh::getHeapMemorySizeWithoutChildren() const
{
    return _geo_type_name.capacity() + _vars.capacity() * sizeof(MCAuto<DataArray>);
}

std::vector<const MEDCoupling::BigMemoryObject *>
MEDFileEltStruct4Mesh::getDirectChildrenWithNull() const
{
    std::vector<const MEDCoupling::BigMemoryObject *> ret;
    ret.push_back(_conn);
    ret.push_back(_common);
    for (std::vector<MCAuto<DataArray>>::const_iterator it = _vars.begin(); it != _vars.end(); it++) ret.push_back(*it);
    return ret;
}

MEDFileEltStruct4Mesh::MEDFileEltStruct4Mesh(
    med_idt fid, const std::string &mName, int dt, int it, int iterOnStEltOfMesh, MEDFileMeshReadSelector *mrs
)
{
    ...
    MEDFILESAFECALLERRD0(
        MEDmeshEntityInfo,
        (fid, mName.c_str(), dt, it, MED_STRUCT_ELEMENT, iterOnStEltOfMesh + 1, geoTypeName, &geoType)
    );
    _geo_type = geoType;
    ...
    {
        med_bool chgt = MED_FALSE, trsf = MED_FALSE;
        nCells = MEDmeshnEntity(
            fid, mName.c_str(), dt, it, MED_STRUCT_ELEMENT, geoType, MED_CONNECTIVITY, MED_NODAL, &chgt, &trsf
        );
    }
    ...
    MCAuto<DataArrayMedInt> miConn = DataArrayMedInt::New();
    miConn->alloc(nCells * nbEntities);
    MEDFILESAFECALLERRD0(
        MEDmeshElementConnectivityRd,
        (fid, mName.c_str(), dt, it, MED_STRUCT_ELEMENT, _geo_type, MED_NODAL, MED_FULL_INTERLACE, miConn->getPointer())
    );
