...
MEDFileBlowStrEltUp::MEDFileBlowStrEltUp(
    const MEDFileFields *fsOnlyOnSE, const MEDFileMeshes *ms, const MEDFileStructureElements *ses
)
{
    ...
    for (std::size_t i = 0; i < sz; i++)
    {
        ...
        if (!umesh)
            throw INTERP_KERNEL::Exception(
                "MEDFileBlowStrEltUp : Blow up of Stru Elt not managed yet for unstructured meshes !"
            );
    }
}

/*!
 * \param [in] mesh - The mesh containing structure element called \a seName. After the call of this method the
 * Structure elements parts will be removed.
 ...
 */
MCAuto<MEDFileEltStruct4Mesh>
MEDFileBlowStrEltUp::dealWithSEInMesh(
...
) const
{
    ...
    throw INTERP_KERNEL::Exception(
        "MEDFileBlowStrEltUp::dealWithSEInMesh : only MED_BALL is managed for the moment, but if you are interested "
        "please send spec to anthony.geay@edf.fr !"
    );
}

MCAuto<MEDFileEltStruct4Mesh>
MEDFileBlowStrEltUp::dealWithMEDBALLInMesh(
    const MEDFileUMesh *mesh, MCAuto<MEDFileUMesh> &mOut, MCAuto<MEDFileFields> &fsOut
) const
{
    mOut = MEDFileUMesh::New();
    fsOut = MEDFileFields::New();
    ...
    if (zeStr.isNull())
    {
        ...
        oss << "MEDFileBlowStrEltUp::dealWithMEDBALLInMesh : no geo type with name " << MED_BALL_STR << " in "
            << mesh->getName() << " !";
 ...
 * \param [in] varAtt - fields containing var att of current structure element. WARNING at this stage the number of
 * iteration are equal to one for each field in \a varAtt
 * \param [out] zeOutputs - ze fields that are the concatenation of fields in \a fs transformed and those in \a varAtt
 * normalized in time space
 */
void
MEDFileBlowStrEltUp::dealWithSEInFields(
...
) const
{
    ...
    throw INTERP_KERNEL::Exception(
        "MEDFileBlowStrEltUp::dealWithSEInFields : only MED_BALL is managed for the moment, but if you are interested "
        "please send spec to anthony.geay@edf.fr !"
    );
}

void
MEDFileBlowStrEltUp::dealWithMEDBALLSInFields(
    const MEDFileFields *fs, const MEDFileEltStruct4Mesh *zeStr, const MEDFileFields *varAtt, MEDFileFields *zeOutputs
) const
{
    ...
    for (int i = 0; i < nbf; i++)
    {
        ...
        elts0.push_back(eltOut);
        elts1.push_back(eltOut);
    }
    //
    const MEDFileMesh *zeCurrentMesh(_ms->getMeshWithName(zeMeshName));
    //
    std::size_t ii(0);
    std::vector<std::vector<MEDFileAnyTypeFieldMultiTS *> > sp(
        MEDFileAnyTypeFieldMultiTS::SplitIntoCommonTimeSeries(elts1)
    );
    for (std::vector<std::vector<MEDFileAnyTypeFieldMultiTS *> >::const_iterator it0 = sp.begin(); it0 != sp.end();
         it0++, ii++)
    {
        std::vector<MCAuto<MEDFileFastCellSupportComparator> > fsc;
        std::vector<std::vector<MEDFileAnyTypeFieldMultiTS *> > sp2(
            MEDFileAnyTypeFieldMultiTS::SplitPerCommonSupport(*it0, zeCurrentMesh, fsc)
        );
        std::size_t jj(0);
        for (std::vector<std::vector<MEDFileAnyTypeFieldMultiTS *> >::const_iterator it1 = sp2.begin();
             it1 != sp2.end();
             it1++, jj++)
        {
            for (std::vector<MEDFileAnyTypeFieldMultiTS *>::const_iterator it2 = (*it1).begin(); it2 != (*it1).end();
                 it2++)
            ...
            MCAuto<MEDFileAnyTypeField1TS> zeGuideForPfl;  // This var is the reference for pfl management.
            {
                if (!(*it1)[0])
                    throw INTERP_KERNEL::Exception(
                        "MEDFileBlowStrEltUp::dealWithMEDBALLSInFields : take a deep breath 2 !"
                    );
                int pdm((*it1)[0]->getNumberOfTS());
                if (pdm < 1)
                    throw INTERP_KERNEL::Exception(
                        "MEDFileBlowStrEltUp::dealWithMEDBALLSInFields : take a deep breath 3 !"
                    );
                zeGuideForPfl = (*it1)[0]->getTimeStepAtPos(0);
            }
            if (zeGuideForPfl.isNull())
                throw INTERP_KERNEL::Exception(
                    "MEDFileBlowStrEltUp::dealWithMEDBALLSInFields : take a deep breath 4 !"
                );
            ...
            for (int i = 0; i < nbf2; i++)
            {
                ...
                if (nbTS2 != 1)
                    throw INTERP_KERNEL::Exception(
                        "MEDFileBlowStrEltUp::dealWithMEDBALLSInFields : internal error ! The dealWithMEDBALLInMesh is "
                        "expected to return a single TS !"
                    );
...
void
MEDFileBlowStrEltUp::generate(MEDFileMeshes *msOut, MEDFileFields *allZeOutFields)
{
    for (std::vector<MCAuto<MEDFileFields> >::iterator elt = _elts.begin(); elt != _elts.end(); elt++)
    {
        ...
        if (!umesh)
            throw INTERP_KERNEL::Exception(
                "MEDFileBlowStrEltUp::generateMeshes : Blow up of Stru Elt not managed yet for unstructured meshes !"
            );
        //
...
std::string
MEDFileBlowStrEltUp::BuildNewMeshName(const std::string &meshName, const std::string &seName)
{
...
std::string
MEDFileBlowStrEltUp::BuildVarAttName(
...
void
MEDFileBlowStrEltUp::DealWithSE(MEDFileFields *fs, MEDFileMeshes *ms, const MEDFileStructureElements *ses)
{
...
class LocInfo
{
   public:
    ...
    void push(const std::string &loc, const std::string &pfl)
    {
        checkUniqueLoc(loc);
        _locs.push_back(loc);
        _pfl.push_back(pfl);
    }
    MCAuto<MEDFileUMesh> generateNonClassicalData(
        int zePos, const MEDFileUMesh *mesh, const MEDFileFieldGlobsReal *globs
    ) const;
    const PartDefinition *getPartDef() const { return _pd; }

   private:
    void checkUniqueLoc(const std::string &loc) const;
    static MCAuto<DataArrayDouble> BuildMeshFromAngleVrille(
        INTERP_KERNEL::NormalizedCellType gt,
        const DataArrayDouble *angleDeVrille,
        const std::string &pfl,
        const MEDFileFieldLoc &loc,
        const MEDFileEltStruct4Mesh *zeStr,
        const MEDFileUMesh *mesh,
        const MEDFileUMesh *section,
        const MEDFileFieldGlobsReal *globs
    );
    static MCAuto<DataArrayDouble> BuildMeshFromEpaisseur(
        INTERP_KERNEL::NormalizedCellType gt,
        const DataArrayDouble *thickness,
        const std::string &pfl,
        const MEDFileFieldLoc &loc,
        const MEDFileEltStruct4Mesh *zeStr,
        const MEDFileUMesh *mesh,
    ...
    static MCAuto<DataArrayDouble> BuildMeshPipeSEG3(
        const DataArrayDouble *angle,
        const DataArrayDouble *scale,
        const std::string &pfl,
        const MEDFileFieldLoc &loc,
        const MEDFileEltStruct4Mesh *zeStr,
        const MEDFileUMesh *mesh,
    ...
    static MCAuto<DataArrayDouble> BuildMeshPipeSEG4(
        const DataArrayDouble *angle,
        const DataArrayDouble *scale,
        const std::string &pfl,
        const MEDFileFieldLoc &loc,
    ...
    static MCAuto<MEDCouplingUMesh> BuildMeshCommon(
        INTERP_KERNEL::NormalizedCellType gt,
        const std::string &pfl,
        const MEDFileFieldLoc &loc,
        const MEDFileEltStruct4Mesh *zeStr,
    ...
    static MCAuto<DataArrayDouble> BuildMeshFromStructure(
        INTERP_KERNEL::NormalizedCellType gt,
        const std::string &pfl,
        const MEDFileFieldLoc &loc,
        const MEDFileEltStruct4Mesh *zeStr,
...
void
LocInfo::checkUniqueLoc(const std::string &loc) const
{
    if (std::find(_locs.begin(), _locs.end(), loc) != _locs.end())
    {
        std::ostringstream oss;
        oss << "LocInfo::checkUniqueLoc : loc \"" << loc << "\" already exists !";
        throw INTERP_KERNEL::Exception(oss.str());
    }
}

MCAuto<MEDCouplingUMesh>
LocInfo::BuildMeshCommon(
    INTERP_KERNEL::NormalizedCellType gt,
    const std::string &pfl,
    const MEDFileFieldLoc &loc,
    const MEDFileEltStruct4Mesh *zeStr,
...
LocInfo::BuildMeshFromAngleVrille(
    INTERP_KERNEL::NormalizedCellType gt,
    const DataArrayDouble *angleDeVrille,
    const std::string &pfl,
    const MEDFileFieldLoc &loc,
...
)
{
    ...
    mcIdType nbSecPts(secPts->getNumberOfTuples()), nbCells(geoMesh->getNumberOfCells()),
        nbg(ToIdType(loc.getGaussWeights().size()));
    {
    ...
    for (int j = 0; j < nbCells; j++)
    {
        ...
        DataArrayDouble::Rotate3DAlg(
            CENTER, dir->getArray()->begin() + j * 3, angleVrille->getIJ(j, 0), nbSecPts, p->begin(), p->getPointer()
        );
        for (int l = 0; l < nbg; l++)
        {
            MCAuto<DataArrayDouble> p2(p->deepCopy());
            for (std::size_t k = 0; k < nbCompo; k++) p2->applyLin(1., ptsForLoc->getIJ(j * nbg + l, k), k);
            arrs[j * nbg + l] = p2;
        }
    }
...
)
{
    ...
    for (mcIdType j = 0; j < nbCells; j++)
    {
        ...
        MCAuto<DataArrayDouble> fact(DataArrayDouble::New()), fact2(DataArrayDouble::New());
        fact->alloc(1, nbCompo);
        fact2->alloc(1, nbCompo);
        ...
        std::transform(
            fact->begin(), fact->end(), fact->getPointer(), [thck](const double &val) { return val * thck / 2.; }
        );
        ...
        {
            MCAuto<DataArrayDouble> p2(p->deepCopy());
            for (int k = 0; k < nbCompo; k++) p2->applyLin(1., ptsForLoc->getIJ(j * nbg + l, k), k);
            arrs[j * nbg + l] = p2;
        }
    }
    std::vector<const DataArrayDouble *> arrs2(VecAutoToVecOfCstPt(arrs));
...
)
{
    static const char MSG1[] =
        "BuildMeshPipeSEG3 : not recognized pattern ! Send mail to anthony.geay@edf.fr with corresponding MED file !";
    MCAuto<DataArrayDouble> ptsForLoc;
    MCAuto<MEDCouplingUMesh> geoMesh(
        BuildMeshCommon(INTERP_KERNEL::NORM_SEG3, pfl, loc, zeStr, mesh, section, globs, ptsForLoc)
    );
    mcIdType nbSecPts(section->getNumberOfNodes()), nbCells(geoMesh->getNumberOfCells()),
        nbg(ToIdType(loc.getGaussWeights().size()));
    ...
    for (mcIdType j = 0; j < nbCells; j++)
    {
        ...
        for (int l = 0; l < nbg; l++)
        {
            ...
            DataArrayDouble::Rotate3DAlg(
                CENTER, dir->getArray()->begin() + j * 3, zeAngle->getIJ(j, l), nbSecPts, p3->begin(), p3->getPointer()
            );
...
{
    static const char MSG1[] =
        "BuildMeshPipeSEG4 : not recognized pattern ! Send mail to anthony.geay@edf.fr with corresponding MED file !";
    MCAuto<DataArrayDouble> ptsForLoc;
    MCAuto<MEDCouplingUMesh> geoMesh(
        BuildMeshCommon(INTERP_KERNEL::NORM_SEG4, pfl, loc, zeStr, mesh, section, globs, ptsForLoc)
    );
    mcIdType nbSecPts(section->getNumberOfNodes()), nbCells(geoMesh->getNumberOfCells()),
        nbg(ToIdType(loc.getGaussWeights().size()));
    MCConstAuto<DataArrayDouble> zeAngle, zeScale;
...
MCAuto<DataArrayDouble>
LocInfo::BuildMeshFromStructure(
    INTERP_KERNEL::NormalizedCellType gt,
    const std::string &pfl,
    const MEDFileFieldLoc &loc,
    const MEDFileEltStruct4Mesh *zeStr,
...
{
    static const char MSG1[] =
        "BuildMeshFromStructure : not recognized pattern ! Send mail to anthony.geay@edf.fr with corresponding MED "
        "file !";
    const std::vector<MCAuto<DataArray> > &vars(zeStr->getVars());
    if (vars.size() == 1)
    {
    ...
    if (vars.size() == 2)
    {
        ...
        if (zeArr0.isNull() || zeArr1.isNull())
            throw INTERP_KERNEL::Exception(MSG1);
        MCAuto<DataArrayDouble> zeArr00(DynamicCastSafe<DataArray, DataArrayDouble>(zeArr0)),
            zeArr11(DynamicCastSafe<DataArray, DataArrayDouble>(zeArr1));
...
MCAuto<MEDFileUMesh>
LocInfo::generateNonClassicalData(int zePos, const MEDFileUMesh *mesh, const MEDFileFieldGlobsReal *globs) const
{
    static const char MSG1[] =
        "LocInfo::generateNonClassicalData : no spec for GAUSS on StructureElement with more than one cell !";
    ...
    for (std::size_t i = 0; i < sz; i++)
    {
        ...
        {
            std::ostringstream oss;
            oss << "LocInfo::generateNonClassicalData :  : no geo type with name " << se->getName() << " in "
                << mesh->getName() << " !";
            throw INTERP_KERNEL::Exception(oss.str());
        }
    ...
    meshName << mesh->getName() << "_on_" << sz << "_sections" << "_" << zePos;
...
bool
FieldWalker2::operator!=(const FieldWalker2 &other) const
{
    return !((*this) == other);
}
...
void
FieldWalker1::newMeshEntry(const MEDFileFieldPerMesh *fpm)
{
    if (_nb_mesh++ == 1)
...
void
FieldWalker1::newPerMeshPerTypeEntry(const MEDFileFieldPerMeshPerTypeCommon *pmpt)
{
    if (_pm_pt)
...
void
FieldWalker1::endPerMeshPerTypeEntry(const MEDFileFieldPerMeshPerTypeCommon *)
{
    isClassical();
}

void
FieldWalker1::newPerMeshPerTypePerDisc(const MEDFileFieldPerMeshPerTypePerDisc *pmptpd)
{
    _fw.push_back(FieldWalker2(pmptpd));
}

void
FieldWalker1::checkOK(const FieldWalker1 &other) const
{
...
bool
FieldWalker1::isClassical() const
{
    if (_fw.empty())
...
bool
FieldWalker::isEmpty() const
{
    return _fw_prev.isNull();
}

bool
FieldWalker::isClassical() const
{
    return _fw_prev->isClassical();
}

void
FieldWalker::newTimeStepEntry(const MEDFileAnyTypeField1TSWithoutSDA *ts)
{
    _fw = new FieldWalker1(ts);
}

void
FieldWalker::endTimeStepEntry(const MEDFileAnyTypeField1TSWithoutSDA *ts)
{
    if (_fw_prev.isNull())
...
void
FieldWalker::newMeshEntry(const MEDFileFieldPerMesh *fpm)
{
    _fw->newMeshEntry(fpm);
}

void
FieldWalker::endMeshEntry(const MEDFileFieldPerMesh *fpm)
{
    _fw->endMeshEntry(fpm);
}

void
FieldWalker::newPerMeshPerTypeEntry(const MEDFileFieldPerMeshPerTypeCommon *pmpt)
{
    _fw->newPerMeshPerTypeEntry(pmpt);
}

void
FieldWalker::endPerMeshPerTypeEntry(const MEDFileFieldPerMeshPerTypeCommon *pmpt)
{
    _fw->endPerMeshPerTypeEntry(pmpt);
}

void
FieldWalker::newPerMeshPerTypePerDisc(const MEDFileFieldPerMeshPerTypePerDisc *pmptpd)
{
...
void
LocSpliter::newFieldEntry(const MEDFileAnyTypeFieldMultiTSWithoutSDA *field)
{
    _fw = new FieldWalker(field);
}

void
LocSpliter::endFieldEntry(const MEDFileAnyTypeFieldMultiTSWithoutSDA *field)
{
    if (_fw->isEmpty())
        return;
    MCAuto<MEDFileAnyTypeFieldMultiTS> f(
        MEDFileAnyTypeFieldMultiTS::BuildNewInstanceFromContent(
            const_cast<MEDFileAnyTypeFieldMultiTSWithoutSDA *>(field)
        )
    );
    if (_fw->isClassical())
    {
...
void
LocSpliter::newTimeStepEntry(const MEDFileAnyTypeField1TSWithoutSDA *ts)
{
    _fw->newTimeStepEntry(ts);
}

void
LocSpliter::endTimeStepEntry(const MEDFileAnyTypeField1TSWithoutSDA *ts)
{
    _fw->endTimeStepEntry(ts);
}

void
LocSpliter::newMeshEntry(const MEDFileFieldPerMesh *fpm)
{
    _fw->newMeshEntry(fpm);
}

void
LocSpliter::endMeshEntry(const MEDFileFieldPerMesh *fpm)
{
    _fw->endMeshEntry(fpm);
}

void
LocSpliter::newPerMeshPerTypeEntry(const MEDFileFieldPerMeshPerTypeCommon *pmpt)
{
    _fw->newPerMeshPerTypeEntry(pmpt);
}

void
LocSpliter::endPerMeshPerTypeEntry(const MEDFileFieldPerMeshPerTypeCommon *pmpt)
{
    _fw->endPerMeshPerTypeEntry(pmpt);
}

void
LocSpliter::newPerMeshPerTypePerDisc(const MEDFileFieldPerMeshPerTypePerDisc *pmptpd)
{
    _fw->newPerMeshPerTypePerDisc(pmptpd);
}

void
MEDFileBlowStrEltUp::DealWithConflictNames(MEDFileAnyTypeFieldMultiTS *fmtsToAdd, const MEDFileFields *fs)
{
    std::vector<std::string> fnames(fs->getFieldsNames());
    for (int i = 0; i < 1000; i++)
    {
        std::ostringstream oss;
        oss << fmtsToAdd->getName();
        if (i >= 1)
            oss << "_" << i - 1;
...
MEDFileBlowStrEltUp::splitFieldsPerLoc(
