...
class Localizer
{
   ...
   public:
    ...
    ~Localizer() { setlocale(LC_NUMERIC, _locale.c_str()); }
};
}  // namespace
...
SauvReader *
SauvReader::New(const std::string &fileName)
{
    if (fileName.empty())
        THROW_IK_EXCEPTION("Invalid file name");
...
SauvReader::~SauvReader() { _fileReader->decrRef(); }

std::size_t
SauvReader::getHeapMemorySizeWithoutChildren() const
{
    return 0;
}

std::vector<const BigMemoryObject *>
SauvReader::getDirectChildrenWithNull() const
{
...
std::string
SauvReader::lineNb() const
{
    if (isASCII())
        return string(" (line #") +
               SauvUtilities::toString(static_cast<SauvUtilities::ASCIIReader *>(_fileReader)->lineNb()) + ")";
...
int
SauvReader::readPileNumber(int &nbNamedObjects, int &nbObjects)
{
...
SauvReader::readRecord2()
{
    ...
    for (initNameReading(nbNamedObjects); more(); next()) objectNames[index()] = getName();

    vector<int> nameIndices(nbNamedObjects);
    for (initIntReading(nbNamedObjects); more(); next()) nameIndices[index()] = getInt();
...
void
SauvReader::read_PILE_SOUS_MAILLAGE(
...
)
{
    ...
    for (int object = 0; object != nbObjects; ++object)  // loop on sub-groups
    {
        ...
        if (castemCellType == 0 && nbSubGroups > 0)
        {
            ...
            for (initIntReading(nbSubGroups); more(); next()) group._groups[index()] = &_iMed->_groups[getInt() - 1];
        ...
        // skip colors
        if (isASCII())
            for (i = 0; i < nbElements; i += 10) getNextLine(line);
        else
...
void
SauvReader::read_PILE_LREEL(const int nbObjects, std::vector<std::string> &, std::vector<int> &)
{
    if (isXRD())
    {
...
void
SauvReader::read_PILE_LOGIQUES(const int, std::vector<std::string> &, std::vector<int> &)
{
    if (isXRD())
    {
        initIntReading(1);
...
void
SauvReader::read_PILE_FLOATS(const int, std::vector<std::string> &, std::vector<int> &)
{
    if (isXRD())
    {
        initIntReading(1);
...
void
SauvReader::read_PILE_INTEGERS(const int, std::vector<std::string> &, std::vector<int> &)
{
    if (isXRD())
    {
        initIntReading(1);
...
void
SauvReader::read_PILE_LMOTS(const int nbObjects, std::vector<std::string> &, std::vector<int> &)
{
    if (isXRD())
    {
        for (int object = 0; object != nbObjects; ++object)  // pour chaque Group
...
void
SauvReader::read_PILE_MODL(const int nbObjects, std::vector<std::string> &, std::vector<int> &)
{
    if (isXRD())
    {
        for (int object = 0; object != nbObjects; ++object)  // pour chaque Group
...
void
SauvReader::read_PILE_NOEUDS(const int nbObjects, std::vector<std::string> &, std::vector<int> &)
{
    initIntReading(1);
...
void
SauvReader::read_PILE_COORDONNEES(const int nbObjects, std::vector<std::string> &, std::vector<int> &)
{
    initIntReading(1);
    ...
    while (more())
    {
        for (unsigned j = 0; j < _iMed->_spaceDim; ++j, next()) *coordPtr++ = getDouble();
        // skip density
        getDouble();
        next();
    }
...
void
SauvReader::setFieldSupport(const vector<SauvUtilities::Group *> &supports, SauvUtilities::DoubleField *field)
{
    ...
    else
    {
        ...
        {
            vector<SauvUtilities::Group *> newGroups(supports.begin(), supports.begin() + sup_set.size());
                    ...
                    for (size_t iG = 0; iG < groups.size(); ++iG)
                    {
                        ...
                        for (size_t iSub = iFromSub; iSub < field->_sub.size(); ++iSub)
                            if (field->_sub[iSub]._support == groups[iG])
                            {
                                ...
                                for (; iC < nbC; ++iC) newValues[iNewComp++].swap(field->_comp_values[iComp++]);
                                break;
                            }
                            else
                            {
...
void
SauvReader::setFieldNames(
    const vector<SauvUtilities::DoubleField *> &fields,
...
void
SauvReader::read_PILE_NODES_FIELD(
    const int nbObjects, std::vector<std::string> &objectNames, std::vector<int> &nameIndices
)
{
    ...
    for (int object = 0; object != nbObjects; ++object)  // loop on fields
    {
        ...
        for (i_sub = 0; i_sub < nb_sub; ++i_sub)
        {
            ...
            nb_comps[i_sub] = getInt();
            next();  // (c) nb of components in i_sub
        }
        ...
        for (i_sub = 0; i_sub < nb_sub; ++i_sub)
        {
            ...
            for (i_comp = 0; i_comp < nb_comps[i_sub]; ++i_comp)
            {
                if (fdouble)
                {
                    ...
                    for (int i = 0; more() && i < nb_values[i_sub]; next(), ++i) vals[i] = getDouble();
                }
                else
                {
        ...
        if (fdouble && fdouble->hasSameComponentsBySupport())
            ...
            for (i_sub = 0; i_sub < nb_sub; ++i_sub) fdouble->_sub[i_sub]._support->_isProfile = true;
...
void
SauvReader::read_PILE_FIELD(const int nbObjects, std::vector<std::string> &objectNames, std::vector<int> &nameIndices)
{
    // REAL EXAMPLE
    ...
    for (int object = 0; object != nbObjects; ++object)  // pour chaque field
    {
        ...
        for (i_sub = 0; i_sub < nb_sub; ++i_sub)
        {
            ...
            // (7) component names
            for (initNameReading(nb_comp[i_sub]); more(); next()) comp_names[index()] = getName();
            // (8) component type
            for (initNameReading(nb_comp[i_sub], 17); more(); next())  // 17 is name width
            {
                ...
                if (index() > 0 && comp_type[index()] != comp_type[index() - 1])
                    THROW_IK_EXCEPTION(
                        "Error of field reading: diff component types <" << comp_type[index()] << "> != <"
                                                                         << comp_type[index() - 1] << ">" << lineNb()
                    );
            }
...
void
SauvReader::read_PILE_TABLES(const int nbObjects, std::vector<std::string> &objectNames, std::vector<int> &nameIndices)
{
    ...
    for (size_t iname = 0; iname < objectNames.size(); iname++)
        if (objectNames[iname] == table_med_mail)
            table_med_mail_id = nameIndices[iname];
        else if (objectNames[iname] == table_med_cham)
            table_med_cham_id = nameIndices[iname];
        else if (objectNames[iname] == table_med_comp)
            table_med_comp_id = nameIndices[iname];
    ...
    for (int itable = 1; itable <= nbObjects; itable++)
    {
        ...
        {
            if (itable == table_med_mail_id || itable == table_med_cham_id || itable == table_med_comp_id)
            {
                ...
                {
                    if (name_i.gibi_pile != PILE_FIELD && name_i.gibi_pile != PILE_NODES_FIELD)
                    {
...
void
SauvReader::read_PILE_STRINGS(const int nbObjects, std::vector<std::string> &objectNames, std::vector<int> &nameIndices)
{
    // IMP 0020434: mapping GIBI names to MED names
    initIntReading(2);
