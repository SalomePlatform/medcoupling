...
MEDFileEquivalencePair::Load(MEDFileEquivalences *father, med_idt fid, const std::string &name, const std::string &desc)
{
    if (!father)
...
void
MEDFileEquivalencePair::writeLL(med_idt fid) const
{
    ...
    MEDLoaderBase::safeStrCpy(
        meshName.c_str(), MED_NAME_SIZE, meshName2, getFather()->getMesh()->getTooLongStrPolicy()
    );
    MEDLoaderBase::safeStrCpy(_name.c_str(), MED_NAME_SIZE, name, getFather()->getMesh()->getTooLongStrPolicy());
    MEDLoaderBase::safeStrCpy(
        _description.c_str(), MED_COMMENT_SIZE, desc, getFather()->getMesh()->getTooLongStrPolicy()
    );
...
const MEDFileMesh *
MEDFileEquivalencePair::getMesh() const
{
    return getFather()->getMesh();
}

MEDFileMesh *
MEDFileEquivalencePair::getMesh()
{
    return getFather()->getMesh();
}
...
bool
MEDFileEquivalencePair::isEqual(const MEDFileEquivalencePair *other, std::string &what) const
{
    if (_name != other->_name)
    {
        std::ostringstream oss;
        oss << "Names differs : " << _name << " != " << other->_name << " !";
        what = oss.str();
        return false;
    }
    ...
    {
        std::ostringstream oss;
        oss << "Description differs : " << _description << " != " << other->_description << " !";
        what = oss.str();
        return false;
    }
    ...
    {
        std::ostringstream oss;
        oss << "Cell def of Equiv " << _name << " are defined for this and not for other (or reversely) !";
        what = oss.str();
        return false;
    }
    if (c1 && c2)
    ...
    {
        std::ostringstream oss;
        oss << "Node def of Equiv " << _name << " are defined for this and not for other (or reversely) !";
        what = oss.str();
        return false;
    }
    if (n1 && n2)
...
void
MEDFileEquivalencePair::getRepr(std::ostream &oss) const
{
    const MEDFileEquivalenceNode *node(_node);
    const MEDFileEquivalenceCell *cell(_cell);
...
MEDFileEquivalencePair *
MEDFileEquivalencePair::New(MEDFileEquivalences *father, const std::string &name)
{
    return new MEDFileEquivalencePair(father, name, std::string());
}

std::vector<const BigMemoryObject *>
MEDFileEquivalencePair::getDirectChildrenWithNull() const
{
...
void
MEDFileEquivalencePair::setArray(int meshDimRelToMaxExt, DataArrayInt *da)
{
    if (meshDimRelToMaxExt > 1)
        throw INTERP_KERNEL::Exception(
            "MEDFileEquivalencePair::setArray : meshDimRelToMaxExt must be in [1,0,-1,-2,-3] at most !"
        );
    if (meshDimRelToMaxExt == 1)
    {
...
MEDFileEquivalencePair::initCell()
{
    _cell = new MEDFileEquivalenceCell(this);
    return _cell;
}
...
MEDFileEquivalencePair::initNode()
{
    _node = new MEDFileEquivalenceNode(this, 0);
    return _node;
}
...
std::size_t
MEDFileEquivalencePair::getHeapMemorySizeWithoutChildren() const
{
    return 0;
}

void
MEDFileEquivalencePair::load(med_idt fid)
{
    ...
    MEDFILESAFECALLERRD0(
        MEDequivalenceCorrespondenceSize, (fid, meshName.c_str(), _name.c_str(), dt, it, MED_NODE, MED_NONE, &ncor)
    );
    if (ncor > 0)
    {
...
std::vector<const BigMemoryObject *>
MEDFileEquivalences::getDirectChildrenWithNull() const
{
    std::size_t sz(_equ.size());
    std::vector<const BigMemoryObject *> ret(sz);
    for (std::size_t i = 0; i < sz; i++) ret[i] = _equ[i];
    return ret;
}

std::size_t
MEDFileEquivalences::getHeapMemorySizeWithoutChildren() const
{
    return sizeof(MEDFileEquivalences) + _equ.capacity() * sizeof(MEDFileEquivalencePair);
}

void
MEDFileEquivalences::getDtIt(int &dt, int &it) const
{
    dt = _owner->getIteration();
    it = _owner->getOrder();
}

std::string
MEDFileEquivalences::getMeshName() const
{
    return _owner->getName();
}

void
MEDFileEquivalences::pushEquivalence(MEDFileEquivalencePair *elt)
{
...
MEDFileEquivalencePair *
MEDFileEquivalences::getEquivalence(int i)
{
    int sz(size());
    if (i < 0 || i >= sz)
    {
        std::ostringstream oss;
        oss << "MEDFileEquivalences::getEquivalence : invalid id ! Must be in [0," << sz << ") !";
...
MEDFileEquivalencePair *
MEDFileEquivalences::getEquivalenceWithName(const std::string &name)
{
    ...
    std::ostringstream oss;
    oss << "MEDFileEquivalences::getEquivalenceWithName : no equivalence with name \"" << name << "\" ! Must be in [ ";
...
int
MEDFileEquivalences::size() const
{
    return (int)_equ.size();
}

std::vector<std::string>
MEDFileEquivalences::getEquivalenceNames() const
{
...
MEDFileEquivalencePair *
MEDFileEquivalences::appendEmptyEquivalenceWithName(const std::string &name)
{
...
bool
MEDFileEquivalences::isEqual(const MEDFileEquivalences *other, std::string &what) const
{
    std::size_t sz(_equ.size());
    ...
    for (std::size_t i = 0; i < sz; i++)
    {
        ...
        if (thisp && otherp)
        {
            ...
            {
                std::ostringstream oss;
                oss << "At Eq #" << i << " there is a difference !";
                what = oss.str() + what;
                return false;
            }
        }
        ...
        {
            std::ostringstream oss;
            oss << "At Eq #" << i << " defined in this not is other (or reversely) !";
            what = oss.str() + what;
            return false;
        }
    }
...
void
MEDFileEquivalences::getRepr(std::ostream &oss) const
{
    std::size_t ii(0);
...
void
MEDFileEquivalences::killEquivalenceWithName(const std::string &name)
{
    ...
    {
        std::ostringstream oss;
        oss << "MEDFileEquivalences::killEquivalenceWithName : Equivalence with name \"" << name << "\" not found !";
        throw INTERP_KERNEL::Exception(oss.str().c_str());
    }
    _equ.erase(it);
}
...
void
MEDFileEquivalences::killEquivalenceAt(int i)
{
    int sz(size());
    if (i < 0 || i >= sz)
    {
        std::ostringstream oss;
        oss << "MEDFileEquivalences::killEquivalenceAt : Id must be in [0," << sz << ") !";
        throw INTERP_KERNEL::Exception(oss.str().c_str());
    }
    std::vector<MCAuto<MEDFileEquivalencePair> >::iterator it(_equ.begin());
...
void
MEDFileEquivalences::clear()
{
    _equ.clear();
}

void
MEDFileEquivalences::writeLL(med_idt fid) const
{
    for (std::vector<MCAuto<MEDFileEquivalencePair> >::const_iterator it = _equ.begin(); it != _equ.end(); it++)
    {
        const MEDFileEquivalencePair *elt(*it);
...
int
MEDFileEquivalences::PresenceOfEquivalences(med_idt fid, const std::string &meshName)
{
...
MEDFileEquivalences *
MEDFileEquivalences::Load(med_idt fid, int nbOfEq, MEDFileMesh *owner)
{
    MCAuto<MEDFileEquivalences> ret(new MEDFileEquivalences(owner));
    if (!owner)
    ...
    for (int i = 0; i < nbOfEq; i++)
    {
        ...
        std::string eqName(MEDLoaderBase::buildStringFromFortran(equ, MED_NAME_SIZE)),
            eqDescName(MEDLoaderBase::buildStringFromFortran(desc, MED_COMMENT_SIZE));
...
void
MEDFileEquivalences::CheckDataArray(const DataArrayInt *data)
{
    if (!data)
        return;
    data->checkAllocated();
    ...
    {
        std::ostringstream oss;
        oss << "MEDFileEquivalences::CheckDataArray : Input DataArray must have 2 components !";
        throw INTERP_KERNEL::Exception(oss.str().c_str());
    }
}

void
MEDFileEquivalences::deepCpyFrom(const MEDFileEquivalences &other)
{
    for (std::vector<MCAuto<MEDFileEquivalencePair> >::const_iterator it = other._equ.begin(); it != other._equ.end();
         it++)
    {
        const MEDFileEquivalencePair *elt(*it);
...
MEDFileEquivalenceData::MEDFileEquivalenceData(MEDFileEquivalencePair *owner, DataArrayInt *data)
    : MEDFileEquivalenceBase(owner), _data(data)
{
    if (data)
        data->incrRef();
}
...
void
MEDFileEquivalenceData::setArray(DataArrayInt *data)
{
...
std::vector<const BigMemoryObject *>
MEDFileEquivalenceData::getDirectChildrenWithNull() const
{
...
bool
MEDFileEquivalenceData::isEqual(const MEDFileEquivalenceData *other, std::string &what) const
{
...
void
MEDFileEquivalenceData::writeAdvanced(med_idt fid, med_entity_type medtype, med_geometry_type medgt) const
{
    ...
    MCAuto<DataArrayMedInt> da2(DataArrayMedInt_Copy(da));
    da2->rearrange(1);
    da2->applyLin(1, 1);
    da2->rearrange(2);
    MEDFILESAFECALLERWR0(
        MEDequivalenceCorrespondenceWr,
        (fid, meshName2, name, dt, it, medtype, medgt, ToMedInt(da2->getNumberOfTuples()), da2->begin())
    );
}

std::size_t
MEDFileEquivalenceCellType::getHeapMemorySizeWithoutChildren() const
{
...
bool
MEDFileEquivalenceCellType::isEqual(const MEDFileEquivalenceCellType *other, std::string &what) const
{
...
void
MEDFileEquivalenceCellType::getRepr(std::ostream &oss) const
{
...
void
MEDFileEquivalenceCellType::writeLL(med_idt fid) const
{
    writeAdvanced(fid, MED_CELL, typmai3[_type]);
}

std::vector<const BigMemoryObject *>
MEDFileEquivalenceCell::getDirectChildrenWithNull() const
{
    std::size_t sz(_types.size());
    std::vector<const BigMemoryObject *> ret(sz);
    for (std::size_t i = 0; i < sz; i++) ret[i] = _types[i];
    return ret;
}

std::size_t
MEDFileEquivalenceCell::getHeapMemorySizeWithoutChildren() const
{
    return sizeof(MEDFileEquivalenceCell) + _types.capacity() * sizeof(MEDFileEquivalenceCellType);
}

MEDFileEquivalenceCell *
MEDFileEquivalenceCell::Load(med_idt fid, MEDFileEquivalencePair *owner)
{
...
void
MEDFileEquivalenceCell::writeLL(med_idt fid) const
{
...
bool
MEDFileEquivalenceCell::isEqual(const MEDFileEquivalenceCell *other, std::string &what) const
{
    std::size_t sz(_types.size());
    if (sz != other->_types.size())
    {
        std::ostringstream oss;
        oss << "Nb of geo types differs : " << sz << " != " << other->_types.size();
        what = oss.str();
        return false;
    }
    for (std::size_t i = 0; i < sz; i++)
    {
        ...
        {
            std::ostringstream oss;
            oss << "At gt #" << i << " this is defined not other (or reversely !)";
            what = oss.str();
            return false;
        }
        if (ct1 && ct2)
        {
            ...
            {
                std::ostringstream oss;
                oss << "At gt #" << i << " of Eq " << getFather()->getName() << " it differs !";
                what = oss.str() + what;
                return false;
            }
        }
...
void
MEDFileEquivalenceCell::getRepr(std::ostream &oss) const
{
    for (std::vector<MCAuto<MEDFileEquivalenceCellType> >::const_iterator it = _types.begin(); it != _types.end(); it++)
    {
        const MEDFileEquivalenceCellType *elt(*it);
...
DataArrayInt *
MEDFileEquivalenceCell::getArray(INTERP_KERNEL::NormalizedCellType type)
{
    ...
    std::ostringstream oss;
    oss << "MEDFileEquivalenceCell::getArray : In Equivalence \"" << getFather()->getName() << "\" the geotype " << type
        << " is not available !";
    throw INTERP_KERNEL::Exception(oss.str().c_str());
}

void
MEDFileEquivalenceCell::setArray(int meshDimRelToMax, DataArrayInt *da)
{
    if (!da)
        return;
    MEDFileEquivalences::CheckDataArray(da);
    ...
    //
    MCAuto<DataArrayInt> tmp(da->deepCopy());
    tmp->rearrange(1);
    int maxv, minv;
    ...
    {
        std::ostringstream oss;
        oss << "MEDFileEquivalenceCell::setArray : Input 2 component DataArray has incorrect values ! all values must "
               "be in [0,"
            << totalNbOfCells << ") !";
        throw INTERP_KERNEL::Exception(oss.str().c_str());
    }
...
void
MEDFileEquivalenceCell::setArrayForType(INTERP_KERNEL::NormalizedCellType type, DataArrayInt *da)
{
    for (std::vector<MCAuto<MEDFileEquivalenceCellType> >::iterator it = _types.begin(); it != _types.end(); it++)
    {
        MEDFileEquivalenceCellType *elt(*it);
...
MEDFileEquivalenceCell::getTypes() const
{
...
void
MEDFileEquivalenceCell::load(med_idt fid)
{
    ...
    for (int i = 0; i < MED_N_CELL_FIXED_GEO; i++)
    {
        med_int ncor;
        MEDFILESAFECALLERRD0(
            MEDequivalenceCorrespondenceSize, (fid, meshName.c_str(), name.c_str(), dt, it, MED_CELL, typmai[i], &ncor)
        );
        if (ncor > 0)
        {
            MCAuto<DataArrayMedInt> da(DataArrayMedInt::New());
            ...
            da->applyLin(1, -1);
            da->rearrange(2);
            MCAuto<MEDFileEquivalenceCellType> ct(
                new MEDFileEquivalenceCellType(getFather(), typmai2[i], FromMedIntArray<int>(da))
            );
            _types.push_back(ct);
        }
    }
...
std::size_t
MEDFileEquivalenceNode::getHeapMemorySizeWithoutChildren() const
{
    return sizeof(MEDFileEquivalenceNode);
}

void
MEDFileEquivalenceNode::writeLL(med_idt fid) const
{
...
bool
MEDFileEquivalenceNode::isEqual(const MEDFileEquivalenceNode *other, std::string &what) const
{
    return MEDFileEquivalenceData::isEqual(other, what);
}

void
MEDFileEquivalenceNode::getRepr(std::ostream &oss) const
{
    const DataArrayInt *da(getArray());
    if (!da)
