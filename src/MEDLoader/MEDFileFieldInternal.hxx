...
{
   public:
    static MEDFileFieldPerMeshPerTypePerDisc *NewOnRead(
        MEDFileFieldPerMeshPerTypeCommon *fath, TypeOfField type, int profileIt, const PartDefinition *pd
    );
    static MEDFileFieldPerMeshPerTypePerDisc *New(
        MEDFileFieldPerMeshPerTypeCommon *fath, TypeOfField type, mcIdType locId
    );
    ...
    static std::vector<std::vector<const MEDFileFieldPerMeshPerTypePerDisc *> > SplitPerDiscretization(
        const std::vector<const MEDFileFieldPerMeshPerTypePerDisc *> &entries
    );
    static bool RenumberChunks(
        mcIdType offset,
    ...
    static MEDFileFieldPerMeshPerTypePerDisc *NewObjectOnSameDiscThanPool(
        TypeOfField typeF,
        INTERP_KERNEL::NormalizedCellType geoType,
    ...
    static MCAuto<MEDFileFieldPerMeshPerTypePerDisc> Aggregate(
        mcIdType &start,
    ...
    MEDFileFieldPerMeshPerTypePerDisc(MEDFileFieldPerMeshPerTypeCommon *fath, TypeOfField type)
        : _type(type), _father(fath), _start(-1), _end(-1), _nval(-1), _loc_id(-5), _profile_it(-1)
    {
    }

   private:
    MEDFileFieldPerMeshPerTypePerDisc(
        MEDFileFieldPerMeshPerTypeCommon *fath, TypeOfField type, mcIdType profileIt, const PartDefinition *pd
    );
    MEDFileFieldPerMeshPerTypePerDisc(
        MEDFileFieldPerMeshPerTypeCommon *fath, TypeOfField type, mcIdType profileIt, const std::string &dummy
    );
    MEDFileFieldPerMeshPerTypePerDisc(const MEDFileFieldPerMeshPerTypePerDisc &other);
    MEDFileFieldPerMeshPerTypePerDisc();

   private:
    void goReadZeValuesInFile(
        med_idt fid,
...
class MEDFileFieldPerMeshPerTypeCommon : public RefCountObject, public MEDFileWritable
{
   ...
   protected:
    ...
    std::vector<mcIdType> addNewEntryIfNecessary(
        const MEDCouplingFieldTemplate *field, mcIdType offset, mcIdType nbOfCells
    );
    std::vector<mcIdType> addNewEntryIfNecessaryGauss(
        const MEDCouplingFieldTemplate *field, mcIdType offset, mcIdType nbOfCells
    );
    std::vector<mcIdType> addNewEntryIfNecessary(
...
class MEDFileFieldPerMeshPerType : public MEDFileFieldPerMeshPerTypeCommon
{
   public:
    ...
    static MEDFileFieldPerMeshPerType *NewOnRead(
        med_idt fid,
    ...
    static MCAuto<MEDFileFieldPerMeshPerType> Aggregate(
        mcIdType &start,
...
{
   public:
    static MEDFileFieldPerMeshPerTypeDyn *NewOnRead(
        med_idt fid,
        MEDFileFieldPerMesh *fath,
...
class MEDFileFieldPerMesh : public RefCountObject, public MEDFileWritable
{
   public:
    ...
    static MEDFileFieldPerMesh *NewOnRead(
        med_idt fid,
    ...
    void assignFieldNoProfileNoRenum(
        mcIdType &start,
        const std::vector<mcIdType> &code,
        const MEDCouplingFieldTemplate *field,
        const DataArray *arr,
    ...
    void writeLL(med_idt fid, const MEDFileFieldNameScope &nasc) const;
    void fillTypesOfFieldAvailable(std::set<TypeOfField> &types) const;
    std::vector<std::vector<std::pair<mcIdType, mcIdType> > > getFieldSplitedByType(
    ...
    bool renumberEntitiesLyingOnMesh(
    ...
    void keepOnlySpatialDiscretization(
        TypeOfField tof, mcIdType &globalNum, std::vector<std::pair<mcIdType, mcIdType> > &its
    );
    void keepOnlyGaussDiscretization(
        std::size_t idOfDisc, mcIdType &globalNum, std::vector<std::pair<mcIdType, mcIdType> > &its
    );
    void changePflsRefsNamesGen(const std::vector<std::pair<std::vector<std::string>, std::string> > &mapOfModif);
    void changeLocsRefsNamesGen(const std::vector<std::pair<std::vector<std::string>, std::string> > &mapOfModif);
    MEDCouplingFieldDouble *getFieldOnMeshAtLevel(
        TypeOfField type,
        const MEDFileFieldGlobsReal *glob,
        const MEDCouplingMesh *mesh,
        bool &isPfl,
    ...
    DataArray *getFieldOnMeshAtLevelWithPfl(
        TypeOfField type,
        const MEDCouplingMesh *mesh,
        DataArrayIdType *&pfl,
        const MEDFileFieldGlobsReal *glob,
    ...
    void getUndergroundDataArrayExt(
        std::vector<std::pair<std::pair<INTERP_KERNEL::NormalizedCellType, int>, std::pair<mcIdType, mcIdType> > >
            &entries
    ) const;
    MEDFileFieldPerMeshPerTypePerDisc *getLeafGivenTypeAndLocId(INTERP_KERNEL::NormalizedCellType typ, mcIdType locId);
    const MEDFileFieldPerMeshPerTypePerDisc *getLeafGivenTypeAndLocId(
        INTERP_KERNEL::NormalizedCellType typ, mcIdType locId
    ) const;
    static MCAuto<MEDFileFieldPerMesh> Aggregate(
        mcIdType &start,
   ...
   private:
    ...
    MEDCouplingFieldDouble *finishField(
        TypeOfField type,
        const MEDFileFieldGlobsReal *glob,
    ...
    MEDCouplingFieldDouble *finishField2(
        TypeOfField type,
        const MEDFileFieldGlobsReal *glob,
        const std::vector<std::pair<mcIdType, mcIdType> > &dads,
        const std::vector<int> &locs,
    ...
    MEDCouplingFieldDouble *finishFieldNode2(
        const MEDFileFieldGlobsReal *glob,
        const std::vector<std::pair<mcIdType, mcIdType> > &dads,
        const std::vector<int> &locs,
        const MEDCouplingMesh *mesh,
    ...
    MEDFileFieldPerMesh(MEDFileAnyTypeField1TSWithoutSDA *fath, int meshIt, int meshOrd)
        : _mesh_iteration(meshIt), _mesh_order(meshOrd), _father(fath)
    {
    }
