...
class MeshOrStructMeshCls
{
   ...
   public:
    ...
    virtual std::vector<std::string> getAxisInfoOnMesh(
        med_idt fid,
...
class MeshCls : public MeshOrStructMeshCls
{
   public:
    ...
    std::vector<std::string> getAxisInfoOnMesh(
        med_idt fid,
        const std::string &mName,
        MEDCoupling::MEDCouplingMeshType &meshType,
        MEDCoupling::MEDCouplingAxisType &axType,
...
class MEDLOADER_EXPORT MEDFileMeshL2 : public RefCountObject
{
   public:
    ...
    static INTERP_KERNEL::AutoCppPtr<MeshOrStructMeshCls> GetMeshIdFromName(
        med_idt fid,
        const std::string &mName,
        MEDCoupling::MEDCouplingMeshType &meshType,
        MEDCoupling::MEDCouplingAxisType &axType,
    ...
    static void ReadFamiliesAndGrps(
        med_idt fid,
        const std::string &mname,
    ...
    static void WriteFamiliesAndGrps(
        med_idt fid,
        const std::string &mname,
   ...
   private:
    ...
    static void RenameFamiliesPatternInternal(
        std::vector<std::pair<std::string, std::pair<mcIdType, std::vector<std::string>>>> &crudeFams,
        RenameFamiliesPatternFunc func
    );
    static void RenameFamiliesFromFileToMemInternal(
        std::vector<std::pair<std::string, std::pair<mcIdType, std::vector<std::string>>>> &crudeFams
    );
    static void RenameFamiliesFromMemToFileInternal(
        std::vector<std::pair<std::string, std::pair<mcIdType, std::vector<std::string>>>> &crudeFams
    );
...
class MEDFileUMeshL2 : public MEDFileMeshL2
{
   public:
    ...
    std::vector<std::string> loadCommonPart(
    ...
    std::vector<std::string> loadPartConnectivityOnly(
        med_idt fid,
        const MeshOrStructMeshCls *mId,
        const std::string &mName,
        const std::vector<INTERP_KERNEL::NormalizedCellType> &types,
    ...
    void loadPartOfConnectivityFromUserDistrib(
        med_idt fid,
        int mdim,
        const std::string &mName,
        const std::map<INTERP_KERNEL::NormalizedCellType, std::vector<mcIdType>> &distrib,
    ...
    static void LoadPartCoordsArray(
        med_idt fid,
        const std::vector<std::string> &infosOnComp,
        const std::string &mName,
        int dt,
    ...
    static void allocCoordsPartCoords(
        mcIdType spaceDim,
        mcIdType nMin,
        mcIdType nMax,
        MCAuto<DataArrayDouble> &_coords,
...
class MEDFileUMeshAggregateCompute : public BigMemoryObject
{
   public:
    ...
    void getStartStopOfGeoTypeWithoutComputation(
...
class MEDFileUMeshSplitL1 : public RefCountObject
{
   ...
   public:
    ...
    std::vector<INTERP_KERNEL::NormalizedCellType> getGeoTypes() const;
    mcIdType getNumberOfCellsWithType(INTERP_KERNEL::NormalizedCellType ct) const;
    std::vector<MEDCoupling1GTUMesh *> getDirectUndergroundSingleGeoTypeMeshes() const
    {
        return _m_by_types.retrievePartsWithoutComputation();
    }
    MEDCoupling1GTUMesh *getDirectUndergroundSingleGeoTypeMesh(INTERP_KERNEL::NormalizedCellType gt) const
    {
        return _m_by_types.retrievePartWithoutComputation(gt);
    }
    ...
    static void TraduceFamilyNumber(
    ...
    static MEDFileUMeshSplitL1 *Unserialize(
        const std::string &name,
        DataArrayDouble *coo,
        std::vector<mcIdType> &tinyInt,
        std::vector<MCAuto<DataArrayIdType>> &bigArraysI
...
{
   public:
    static MEDFileEltStruct4Mesh *New(
