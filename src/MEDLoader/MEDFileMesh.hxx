...
class MEDFileMesh : public RefCountObject, public MEDFileWritableStandAlone
{
   public:
    ...
    MEDLOADER_EXPORT static MEDFileMesh *New(
    ...
    MEDLOADER_EXPORT void setTime(int dt, int it, double time)
    {
        _time = time;
        _iteration = dt;
        _order = it;
    }
    MEDLOADER_EXPORT double getTime(int &dt, int &it) const
    {
        dt = _iteration;
        it = _order;
        return _time;
    ...
    MEDLOADER_EXPORT virtual std::vector<INTERP_KERNEL::NormalizedCellType> getGeoTypesAtLevel(
        int meshDimRelToMax
    ) const = 0;
    ...
    MEDLOADER_EXPORT virtual void whichAreNodesFetched(
    ...
    MEDLOADER_EXPORT virtual bool keepFamIdsOnlyOnLevs(
        const std::vector<mcIdType> &famIds, const std::vector<int> &levs
    );
    ...
    MEDLOADER_EXPORT virtual std::vector<mcIdType> getFamsNonEmptyLevels(
        const std::vector<std::string> &fams
    ) const = 0;
    MEDLOADER_EXPORT virtual std::vector<mcIdType> getFamsNonEmptyLevelsExt(
        const std::vector<std::string> &fams
    ) const = 0;
    ...
    MEDLOADER_EXPORT void changeAllGroupsContainingFamily(
    ...
    //
    MEDLOADER_EXPORT virtual void setGroupsAtLevel(
        int meshDimRelToMaxExt, const std::vector<const DataArrayIdType *> &grps, bool renum = false
    );
    ...
    MEDLOADER_EXPORT virtual DataArrayIdType *getFamiliesArr(
        int meshDimRelToMaxExt, const std::vector<std::string> &fams, bool renum = false
    ) const = 0;
    MEDLOADER_EXPORT virtual DataArrayIdType *getGroupsArr(
        int meshDimRelToMaxExt, const std::vector<std::string> &grps, bool renum = false
    ) const;
    MEDLOADER_EXPORT virtual DataArrayIdType *getGroupArr(
        int meshDimRelToMaxExt, const std::string &grp, bool renum = false
    ) const;
    MEDLOADER_EXPORT virtual DataArrayIdType *getFamilyArr(
        int meshDimRelToMaxExt, const std::string &fam, bool renum = false
    ) const;
    MEDLOADER_EXPORT virtual DataArrayIdType *getNodeGroupArr(const std::string &grp, bool renum = false) const;
    MEDLOADER_EXPORT virtual DataArrayIdType *getNodeGroupsArr(
        const std::vector<std::string> &grps, bool renum = false
    ) const;
    MEDLOADER_EXPORT virtual DataArrayIdType *getNodeFamilyArr(const std::string &fam, bool renum = false) const;
    MEDLOADER_EXPORT virtual DataArrayIdType *getNodeFamiliesArr(
        const std::vector<std::string> &fams, bool renum = false
    ) const;
    // tools
    MEDLOADER_EXPORT virtual bool unPolyze(
   ...
   protected:
    ...
    virtual void appendFamilyEntries(
    ...
    static void TranslateFamilyIds(
        mcIdType offset, DataArrayIdType *famArr, std::vector<std::vector<mcIdType>> &famIdsPerGrp
    );
    static void ChangeAllGroupsContainingFamily(
        std::map<std::string, std::vector<std::string>> &groups,
        const std::string &familyNameToChange,
        const std::vector<std::string> &newFamiliesNames
    );
    static std::string FindOrCreateAndGiveFamilyWithId(
        std::map<std::string, mcIdType> &families, mcIdType id, bool &created
    );
...
class MEDFileUMesh : public MEDFileMesh
{
   ...
   public:
    ...
    MEDLOADER_EXPORT static MEDFileUMesh *New(
        med_idt fid, const std::string &mName, int dt = -1, int it = -1, MEDFileMeshReadSelector *mrs = 0
    );
    ...
    MEDLOADER_EXPORT static MCAuto<MEDFileUMesh> LoadConnectivityOnlyPartOf(
        const std::string &fileName,
        const std::string &mName,
    ...
    MEDLOADER_EXPORT static MEDFileUMesh *LoadPartOf(
        const std::string &fileName,
        const std::string &mName,
        const std::vector<INTERP_KERNEL::NormalizedCellType> &types,
        const std::vector<mcIdType> &slicPerTyp,
    ...
    MEDLOADER_EXPORT static MEDFileUMesh *LoadPartOfFromUserDistrib(
        med_idt fid,
        const std::string &mName,
    ...
    MEDLOADER_EXPORT static void LoadPartCoords(
        const std::string &fileName,
        const std::string &mName,
        int dt,
        int it,
    ...
    MEDLOADER_EXPORT const std::vector<MCAuto<MEDFileEltStruct4Mesh>> &getAccessOfUndergroundEltStrs() const
    {
        return _elt_str;
    }
    //
    ...
    MEDLOADER_EXPORT const PartDefinition *getPartDefAtLevel(
        int meshDimRelToMaxExt, INTERP_KERNEL::NormalizedCellType gt = INTERP_KERNEL::NORM_ERROR
    ) const;
    ...
    MEDLOADER_EXPORT void whichAreNodesFetched(
        const MEDFileField1TSStructItem &st, const MEDFileFieldGlobsReal *globs, std::vector<bool> &nodesFetched
    ) const;
    ...
    MEDLOADER_EXPORT MEDCouplingUMesh *getGroup(
        int meshDimRelToMaxExt, const std::string &grp, bool renum = false
    ) const;
    MEDLOADER_EXPORT MEDCouplingUMesh *getGroups(
        int meshDimRelToMaxExt, const std::vector<std::string> &grps, bool renum = false
    ) const;
    MEDLOADER_EXPORT MEDCouplingUMesh *getFamily(
        int meshDimRelToMaxExt, const std::string &fam, bool renum = false
    ) const;
    MEDLOADER_EXPORT MEDCouplingUMesh *getFamilies(
        int meshDimRelToMaxExt, const std::vector<std::string> &fams, bool renum = false
    ) const;
    MEDLOADER_EXPORT DataArrayIdType *getFamiliesArr(
        int meshDimRelToMaxExt, const std::vector<std::string> &fams, bool renum = false
    ) const;
    ...
    MEDLOADER_EXPORT std::vector<MEDCoupling1GTUMesh *> getDirectUndergroundSingleGeoTypeMeshes(
        int meshDimRelToMax
    ) const;
    MEDLOADER_EXPORT MEDCoupling1GTUMesh *getDirectUndergroundSingleGeoTypeMesh(
    ...
    MEDLOADER_EXPORT void setGroupsFromScratch(
        int meshDimRelToMax, const std::vector<const MEDCouplingUMesh *> &ms, bool renum = false
    );
    MEDLOADER_EXPORT void setGroupsOnSetMesh(
        int meshDimRelToMax, const std::vector<const MEDCouplingUMesh *> &ms, bool renum = false
    );
    ...
    MEDLOADER_EXPORT void openCrack(
        const std::map<mcIdType, std::map<mcIdType, mcIdType>> &c2o2nN, const double &factor
    );
    MEDLOADER_EXPORT void buildInnerBoundaryAlongM1Group(
        const std::string &grpNameM1,
        DataArrayIdType *&nodesDuplicated,
        DataArrayIdType *&cellsModified,
        DataArrayIdType *&cellsNotModified
    );
    MEDLOADER_EXPORT bool unPolyze(
        std::vector<mcIdType> &oldCode, std::vector<mcIdType> &newCode, DataArrayIdType *&o2nRenumCell
    );
    MEDLOADER_EXPORT DataArrayIdType *zipCoords();
    MEDLOADER_EXPORT DataArrayIdType *computeFetchedNodeIds() const;
    MEDLOADER_EXPORT DataArrayIdType *deduceNodeSubPartFromCellSubPart(
        const std::map<int, MCAuto<DataArrayIdType>> &extractDef
    ) const;
    MEDLOADER_EXPORT MEDFileUMesh *extractPart(const std::map<int, MCAuto<DataArrayIdType>> &extractDef) const;
    // MCAuto<MEDFileUMesh> extractPart(int level, DataArrayIdType *keepCells, bool removeOrphanNodes = true) const; //
    ...
    // serialization
    MEDLOADER_EXPORT void serialize(
    ...
    MEDLOADER_EXPORT void unserialize(
        std::vector<double> &tinyDouble,
        std::vector<mcIdType> &tinyInt,
        std::vector<std::string> &tinyStr,
        std::vector<MCAuto<DataArrayIdType>> &bigArraysI,
   ...
   private:
    ...
    void loadPartUMeshFromFileFromUserDistrib(
        med_idt fid,
        const std::string &mName,
        const std::map<INTERP_KERNEL::NormalizedCellType, std::vector<mcIdType>> &distrib,
        std::function<void(
    ...
    static std::vector<std::shared_ptr<std::vector<mcIdType>>> findConnectedComponents(
   ...
   private:
    std::vector<MCAuto<MEDFileUMeshSplitL1>> _ms;  ///< The array of single-dimension constituting meshes, stored in
                                                   ///< decreasing order (dimRelativeToMax=0,-1,-2, ...)
...
class MEDFileStructuredMesh : public MEDFileMesh
{
   ...
   private:
    static void LoadStrMeshDAFromFile(
        med_idt fid,
        int meshDim,
        int dt,
        int it,
...
class MEDFileCurveLinearMesh : public MEDFileStructuredMesh
{
   ...
   public:
    ...
    MEDLOADER_EXPORT static MEDFileCurveLinearMesh *New(DataArrayByte *db)
    {
        return BuildFromMemoryChunk<MEDFileCurveLinearMesh>(db);
    }
    MEDLOADER_EXPORT static MEDFileCurveLinearMesh *New(
        const std::string &fileName,
        const std::string &mName,
