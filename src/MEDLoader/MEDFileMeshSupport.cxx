...
MEDFileMeshSupports *
MEDFileMeshSupports::New(const std::string &fileName)
{
...
MEDFileMeshSupports *
MEDFileMeshSupports::New(med_idt fid)
{
    return new MEDFileMeshSupports(fid);
}

MEDFileMeshSupports *
MEDFileMeshSupports::New()
{
...
MEDFileMeshSupports::MEDFileMeshSupports(med_idt fid)
{
    ...
    for (int i = 0; i < nbSM; i++)
    {
        ...
        MEDFILESAFECALLERRD0(
            MEDsupportMeshInfo, (fid, i + 1, msn, &spaceDim, &meshDim, description, &axType, axisName, axisUnit)
        );
...
MEDFileMeshSupports::getDirectChildrenWithNull() const
{
    std::size_t sz(_supports.size());
    std::vector<const BigMemoryObject *> ret(sz);
    for (std::size_t i = 0; i < sz; i++) ret[i] = _supports[i];
    return ret;
}

std::size_t
MEDFileMeshSupports::getHeapMemorySizeWithoutChildren() const
{
    return _supports.capacity() * sizeof(MCAuto<MEDFileUMesh>);
}

void
MEDFileMeshSupports::writeLL(med_idt fid) const
{
...
MEDFileMeshSupports::getSupMeshNames() const
{
...
const MEDFileUMesh *
MEDFileMeshSupports::getSupMeshWithName(const std::string &name) const
{
...
mcIdType
MEDFileMeshSupports::getNumberOfNodesInConnOf(
    TypeOfField entity, INTERP_KERNEL::NormalizedCellType gt, const std::string &name
) const
{
    ...
    switch (entity)
    {
        ...
        case ON_CELLS:
        {
            ...
            if (std::find(gts.begin(), gts.end(), gt) == gts.end())
                throw INTERP_KERNEL::Exception(
                    "MEDFileMeshSupports::getNumberOfNodesInConnOf : specified geometric type not found !"
                );
        ...
        default:
            throw INTERP_KERNEL::Exception(
                "MEDFileMeshSupports::getNumberOfNodesInConnOf : not recognized entity type !"
            );
    }
}
