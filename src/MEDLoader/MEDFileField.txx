...
template <class T>
void
MEDFileField1TSTemplateWithoutSDA<T>::setArray(DataArray *arr)
{
    if (!arr)
    {
    ...
    if (!arrC)
        throw INTERP_KERNEL::Exception(
            "MEDFileField1TSTemplateWithoutSDA::setArray : the input not null array is not of type DataArrayDouble !"
        );
    else
...
template <class T>
DataArray *
MEDFileField1TSTemplateWithoutSDA<T>::getOrCreateAndGetArray()
{
...
template <class T>
const DataArray *
MEDFileField1TSTemplateWithoutSDA<T>::getOrCreateAndGetArray() const
{
    return getOrCreateAndGetArrayTemplate();
}
...
template <class T>
DataArray *
MEDFileField1TSTemplateWithoutSDA<T>::createNewEmptyDataArrayInstance() const
{
    return Traits<T>::ArrayType::New();
}

template <class T>
typename Traits<T>::ArrayType const *
MEDFileField1TSTemplateWithoutSDA<T>::getOrCreateAndGetArrayTemplate() const
{
...
MEDFileField1TSTemplateWithoutSDA<T>::getOrCreateAndGetArrayTemplate()
{
...
template <class T>
typename Traits<T>::ArrayType *
MEDFileField1TSTemplateWithoutSDA<T>::getUndergroundDataArrayTemplate() const
{
    typename Traits<T>::ArrayType const *ret(_arr);
    if (ret)
...
template <class T>
typename Traits<T>::ArrayType *
MEDFileField1TSTemplateWithoutSDA<T>::getUndergroundDataArrayTemplateExt(
...
template <class T>
DataArray *
MEDFileField1TSTemplateWithoutSDA<T>::getUndergroundDataArray() const
{
    return getUndergroundDataArrayTemplate();
}
...
template <class T>
void
MEDFileField1TSTemplateWithoutSDA<T>::aggregate(
    const std::vector<typename MLFieldTraits<T>::F1TSWSDAType const *> &f1tss,
...
)
{
    ...
    for (typename std::vector<typename MLFieldTraits<T>::F1TSWSDAType const *>::const_iterator it = f1tss.begin();
         it != f1tss.end();
         it++, ii++)
    {
        if (!*it)
            throw INTERP_KERNEL::Exception(
                "MEDFileField1TSTemplateWithoutSDA::aggregate : presence of null pointer in input vector !"
            );
         ...
         it++)
    {
        ...
        if (myArr->getNumberOfComponents() != nbCompo)
            throw INTERP_KERNEL::Exception(
                "MEDFileField1TSTemplateWithoutSDA::aggregate : arrays must have same number of components !"
            );
    }
    ...
    _dt = tv;
    _arr = Traits<T>::ArrayType::New();
    _arr->alloc(start, nbCompo);
    _arr->copyStringInfoFrom(*arr);
    start = 0;
    for (std::vector<std::pair<int, std::pair<mcIdType, mcIdType> > >::const_iterator it = extractInfo.begin();
         it != extractInfo.end();
         it++)
    {
...
template <class T>
void
MEDFileField1TSTemplateWithoutSDA<T>::copyTimeInfoFrom(const typename Traits<T>::FieldType *mcf)
{
    if (!mcf)
        throw INTERP_KERNEL::Exception(
            "MEDFileField1TSTemplateWithoutSDA<T>::copyTimeInfoFrom : input field is nullptr !"
        );
    int b(0), c(0);
...
MEDFileField1TSNDTemplateWithoutSDA<T>::convertToDouble() const
{
 ...
 */
template <class T>
typename MLFieldTraits<T>::F1TSType *
MEDFileTemplateField1TS<T>::New()
{
...
template <class T>
typename MLFieldTraits<T>::F1TSType *
MEDFileTemplateField1TS<T>::New(const std::string &fileName, bool loadAll)
{
...
template <class T>
typename MLFieldTraits<T>::F1TSType *
MEDFileTemplateField1TS<T>::New(med_idt fid, bool loadAll)
{
...
template <class T>
typename MLFieldTraits<T>::F1TSType *
MEDFileTemplateField1TS<T>::New(const std::string &fileName, const std::string &fieldName, bool loadAll)
{
    MEDFileUtilities::AutoFid fid(OpenMEDFileForRead(fileName));
...
template <class T>
typename MLFieldTraits<T>::F1TSType *
MEDFileTemplateField1TS<T>::New(med_idt fid, const std::string &fieldName, bool loadAll)
{
    MCAuto<typename MLFieldTraits<T>::F1TSType> ret(new
                                                    typename MLFieldTraits<T>::F1TSType(fid, fieldName, loadAll, 0));
    ret->contentNotNull();
    return ret.retn();
}
...
template <class T>
typename MLFieldTraits<T>::F1TSType *
MEDFileTemplateField1TS<T>::New(med_idt fid, const std::string &fieldName, int iteration, int order, bool loadAll)
{
    MCAuto<typename MLFieldTraits<T>::F1TSType> ret(
        new typename MLFieldTraits<T>::F1TSType(fid, fieldName, iteration, order, loadAll, 0)
    );
    ret->contentNotNull();
    return ret.retn();
}
...
/*!
 * Returns a new instance of MEDFileField1TS. If \a shallowCopyOfContent is true the content of \a other is shallow
...
template <class T>
typename MLFieldTraits<T>::F1TSType *
MEDFileTemplateField1TS<T>::New(const typename MLFieldTraits<T>::F1TSWSDAType &other, bool shallowCopyOfContent)
{
    MCAuto<typename MLFieldTraits<T>::F1TSType> ret(new
                                                    typename MLFieldTraits<T>::F1TSType(other, shallowCopyOfContent));
    ret->contentNotNull();
    return ret.retn();
}

template <class T>
const typename MLFieldTraits<T>::F1TSWSDAType *
MEDFileTemplateField1TS<T>::contentNotNull() const
{
    ...
    const typename MLFieldTraits<T>::F1TSWSDAType *ret(
        dynamic_cast<const typename MLFieldTraits<T>::F1TSWSDAType *>(pt)
    );
    if (!ret)
    {
        std::ostringstream oss;
        oss << "MEDFileTemplateField1TS<T>::contentNotNull : the content pointer is not null but it is not of type "
               "double ! Reason is maybe that the read field has not the type "
            << MLFieldTraits<T>::F1TSWSDAType::TYPE_STR;
...
template <class T>
typename MLFieldTraits<T>::F1TSWSDAType *
MEDFileTemplateField1TS<T>::contentNotNull()
{
    MEDFileAnyTypeField1TSWithoutSDA *pt(_content);
    if (!pt)
        throw INTERP_KERNEL::Exception(
            "MEDFileTemplateField1TS<T>::contentNotNull : the non const content pointer is null !"
        );
    typename MLFieldTraits<T>::F1TSWSDAType *ret(dynamic_cast<typename MLFieldTraits<T>::F1TSWSDAType *>(pt));
    if (!ret)
    {
        std::ostringstream oss;
        oss << "MEDFileTemplateField1TS<T>::contentNotNull : the non const content pointer is not null but it is not "
               "of type double ! Reason is maybe that the read field has not the type "
            << MLFieldTraits<T>::F1TSWSDAType::TYPE_STR;
        throw INTERP_KERNEL::Exception(oss.str());
    }
    return ret;
...
template <class T>
typename Traits<T>::ArrayType *
MEDFileTemplateField1TS<T>::ReturnSafelyTypedDataArray(MCAuto<DataArray> &arr)
{
    if (arr.isNull())
    ...
    if (!arrOutC)
        throw INTERP_KERNEL::Exception(
            "MEDFileField1TS::ReturnSafelyTypedDataArray : mismatch between dataArrays type and MEDFileField1TS ! "
            "Expected double !"
        );
    arrOutC->incrRef();
    return arrOutC;
...
template <class T>
typename Traits<T>::ArrayType *
MEDFileTemplateField1TS<T>::getFieldWithProfile(
    TypeOfField type, int meshDimRelToMax, const MEDFileMesh *mesh, DataArrayIdType *&pfl
) const
{
    MCAuto<DataArray> arr(
        contentNotNull()->getFieldWithProfile(type, meshDimRelToMax, mesh, pfl, this, *contentNotNull())
    );
...
template <class T>
void
MEDFileTemplateField1TS<T>::setArray(DataArray *arr)
{
...
template <class T>
typename Traits<T>::ArrayType *
MEDFileTemplateField1TS<T>::getUndergroundDataArray() const
{
...
template <class T>
typename Traits<T>::ArrayType *
MEDFileTemplateField1TS<T>::getUndergroundDataArrayExt(
    std::vector<std::pair<std::pair<INTERP_KERNEL::NormalizedCellType, int>, std::pair<mcIdType, mcIdType> > > &entries
) const
{
...
template <class T>
MCAuto<typename Traits<T>::FieldType>
MEDFileTemplateField1TS<T>::SetDataArrayInField(MEDCouplingFieldDouble *f, MCAuto<DataArray> &arr)
{
    if (!f)
...
MEDFileTemplateField1TS<T>::ToFieldTemplateWithTime(const typename Traits<T>::FieldType *f)
{
    int t1, t2;
    double t0(f->getTime(t1, t2));
    std::string tu(f->getTimeUnit());
...
template <class T>
void
MEDFileTemplateField1TS<T>::copyTimeInfoFrom(const typename Traits<T>::FieldType *mcf)
{
    contentNotNull()->copyTimeInfoFrom(mcf);
}

/*!
 * This is the simplest version to fetch a field for MED structure. One drawback : if \a this is a complex field (multi
 * spatial discretization inside a same field) this method will throw exception and more advance method should be called
 * (getFieldOnMeshAtLevel for example). But for normal usage of field in MED file world this method is the most
 * efficient to fetch data.
 *
...
MEDFileTemplateField1TS<T>::field(const MEDFileMesh *mesh) const
{
...
template <class T>
typename Traits<T>::FieldType *
MEDFileTemplateField1TS<T>::getFieldAtLevel(TypeOfField type, int meshDimRelToMax, int renumPol) const
{
    if (getFileName().empty())
        throw INTERP_KERNEL::Exception(
            "MEDFileTemplateField1TS<T>::getFieldAtLevel : Request for a method that can be used for instances coming "
            "from file loading ! Use getFieldOnMeshAtLevel method instead !"
        );
    MCAuto<DataArray> arrOut;
    MCAuto<MEDCouplingFieldDouble> ret(
        contentNotNull()->getFieldAtLevel(
            type, meshDimRelToMax, std::string(), renumPol, this, arrOut, *contentNotNull()
        )
    );
    MCAuto<typename Traits<T>::FieldType> ret2(MEDFileTemplateField1TS<T>::SetDataArrayInField(ret, arrOut));
    return ret2.retn();
...
template <class T>
typename Traits<T>::FieldType *
MEDFileTemplateField1TS<T>::getFieldAtTopLevel(TypeOfField type, int renumPol) const
{
    if (getFileName().empty())
        throw INTERP_KERNEL::Exception(
            "MEDFileTemplateField1TS<T>::getFieldAtTopLevel : Request for a method that can be used for instances "
            "coming from file loading ! Use getFieldOnMeshAtTopLevel method instead !"
        );
    MCAuto<DataArray> arrOut;
    MCAuto<MEDCouplingFieldDouble> ret(
        contentNotNull()->getFieldAtTopLevel(type, std::string(), renumPol, this, arrOut, *contentNotNull())
    );
    MCAuto<typename Traits<T>::FieldType> ret2(MEDFileTemplateField1TS<T>::SetDataArrayInField(ret, arrOut));
    return ret2.retn();
}
...
template <class T>
typename Traits<T>::FieldType *
MEDFileTemplateField1TS<T>::getFieldOnMeshAtLevel(TypeOfField type, const MEDCouplingMesh *mesh, int renumPol) const
{
    MCAuto<DataArray> arrOut;
    MCAuto<MEDCouplingFieldDouble> ret(
        contentNotNull()->getFieldOnMeshAtLevel(type, renumPol, this, mesh, 0, 0, arrOut, *contentNotNull())
    );
    MCAuto<typename Traits<T>::FieldType> ret2(MEDFileTemplateField1TS<T>::SetDataArrayInField(ret, arrOut));
    return ret2.retn();
}
...
template <class T>
typename Traits<T>::FieldType *
MEDFileTemplateField1TS<T>::getFieldOnMeshAtLevel(
    TypeOfField type, int meshDimRelToMax, const MEDFileMesh *mesh, int renumPol
) const
{
    MCAuto<DataArray> arrOut;
    MCAuto<MEDCouplingFieldDouble> ret(
        contentNotNull()->getFieldOnMeshAtLevel(type, meshDimRelToMax, renumPol, this, mesh, arrOut, *contentNotNull())
    );
    MCAuto<typename Traits<T>::FieldType> ret2(MEDFileTemplateField1TS<T>::SetDataArrayInField(ret, arrOut));
    return ret2.retn();
}
...
template <class T>
typename Traits<T>::FieldType *
MEDFileTemplateField1TS<T>::getFieldAtLevelOld(
    TypeOfField type, const std::string &mname, int meshDimRelToMax, int renumPol
) const
{
    if (getFileName().empty())
        throw INTERP_KERNEL::Exception(
            "MEDFileTemplateField1TS<T>::getFieldAtLevelOld : Request for a method that can be used for instances "
            "coming from file loading ! Use getFieldOnMeshAtLevel method instead !"
        );
    MCAuto<DataArray> arrOut;
    MCAuto<MEDCouplingFieldDouble> ret(
        contentNotNull()->getFieldAtLevel(type, meshDimRelToMax, mname, renumPol, this, arrOut, *contentNotNull())
    );
    MCAuto<typename Traits<T>::FieldType> ret2(MEDFileTemplateField1TS<T>::SetDataArrayInField(ret, arrOut));
    return ret2.retn();
}
...
template <class T>
void
MEDFileTemplateField1TS<T>::setFieldNoProfileSBT(const typename Traits<T>::FieldType *field)
{
    setFileName("");
 ...
 *  \param [in] meshDimRelToMax - a relative dimension of mesh entities \a field lies on (useless if field spatial
 * discretization is ON_NODES).
...
template <class T>
void
MEDFileTemplateField1TS<T>::setFieldProfile(
    const typename Traits<T>::FieldType *field,
...
template <class T>
void
MEDFileTemplateField1TS<T>::setFieldProfileFlatly(
    const typename Traits<T>::FieldType *field,
    const MEDFileMesh *mesh,
    int meshDimRelToMax,
    const DataArrayIdType *profile
...
template <class T>
void
MEDFileTemplateField1TS<T>::setFieldProfileGeneral(
    const typename Traits<T>::FieldType *field,
    const MEDFileMesh *mesh,
    int meshDimRelToMax,
    const DataArrayIdType *profile,
...
template <class T>
typename MLFieldTraits<T>::F1TSType *
MEDFileTemplateField1TS<T>::extractPartImpl(
    const std::map<int, MCAuto<DataArrayIdType> > &extractDef, MEDFileMesh *mm
) const
{
    if (!mm)
    ...
    for (std::vector<TypeOfField>::const_iterator it0 = tof.begin(); it0 != tof.end(); it0++)
    {
        if ((*it0) != ON_NODES)
        {
            ...
            for (std::vector<int>::const_iterator lev = levs.begin(); lev != levs.end(); lev++)
            {
                ...
                if (it2 != extractDef.end())
                {
                    ...
                    if (t.isNull())
                        throw INTERP_KERNEL::Exception(
                            "MEDFileField1TS::extractPart : presence of a value with null pointer 1 !"
                        );
        ...
        else
        {
            ...
            if (it2 == extractDef.end())
                throw INTERP_KERNEL::Exception(
                    "MEDFileField1TS::extractPart : presence of a NODE field and no extract array available for NODE !"
                );
            MCAuto<DataArrayIdType> t((*it2).second);
            if (t.isNull())
                throw INTERP_KERNEL::Exception(
...
/*!
 * This method performs a copy with datatype modification ( int32->float64 ) of \a this. The globals information are
...
template <class T>
MEDFileField1TS *
MEDFileNDTemplateField1TS<T>::convertToDouble(bool isDeepCpyGlobs) const
{
    MCAuto<MEDFileField1TS> ret;
    const MEDFileAnyTypeField1TSWithoutSDA *content(this->_content);
    if (content)
    {
        const typename MLFieldTraits<T>::F1TSWSDAType *contc(
            dynamic_cast<const typename MLFieldTraits<T>::F1TSWSDAType *>(content)
        );
        if (!contc)
        {
            std::ostringstream oss;
            oss << "MEDFileNDTemplateField1TS<T>::convertToDouble : the content inside this is not "
                << MLFieldTraits<T>::F1TSWSDAType::TYPE_STR << " ! This is incoherent !";
            throw INTERP_KERNEL::Exception(oss.str());
        }
        MCAuto<MEDFileField1TSWithoutSDA> newc(contc->convertToDouble());
        ret = static_cast<MEDFileField1TS *>(
            MEDFileAnyTypeField1TS::BuildNewInstanceFromContent((MEDFileField1TSWithoutSDA *)newc)
        );
    }
    else
...
template <class T>
void
MEDFileTemplateFieldMultiTSWithoutSDA<T>::checkCoherencyOfType(const MEDFileAnyTypeField1TSWithoutSDA *f1ts) const
{
    if (!f1ts)
        throw INTERP_KERNEL::Exception(
            "MEDFileFieldMultiTSWithoutSDA::checkCoherencyOfType : input field1TS is NULL ! Impossible to check !"
        );
    const typename MLFieldTraits<T>::F1TSWSDAType *f1tsC(
        dynamic_cast<const typename MLFieldTraits<T>::F1TSWSDAType *>(f1ts)
    );
    if (!f1tsC)
    {
        std::ostringstream oss;
        oss << "MEDFileFieldMultiTSWithoutSDA::checkCoherencyOfType : the input field1TS is not a "
            << MLFieldTraits<T>::F1TSWSDAType::TYPE_STR << " type !";
        throw INTERP_KERNEL::Exception(oss.str());
    }
}
...
template <class T>
const char *
MEDFileTemplateFieldMultiTSWithoutSDA<T>::getTypeStr() const
{
...
MEDFileTemplateFieldMultiTSWithoutSDA<T>::createNew() const
{
    return new typename MLFieldTraits<T>::FMTSWSDAType;
}
...
MEDFileTemplateFieldMultiTSWithoutSDA<T>::createNew1TSWithoutSDAEmptyInstance() const
{
...
MEDFileNDTemplateFieldMultiTSWithoutSDA<T>::convertToDouble() const
{
    ...
    int i = 0;
    for (std::vector<MCAuto<MEDFileAnyTypeField1TSWithoutSDA> >::const_iterator it = this->_time_steps.begin();
         it != this->_time_steps.end();
         it++, i++)
    {
        const MEDFileAnyTypeField1TSWithoutSDA *eltToConv(*it);
        if (eltToConv)
        {
            const typename MLFieldTraits<T>::F1TSWSDAType *eltToConvC(
                dynamic_cast<const typename MLFieldTraits<T>::F1TSWSDAType *>(eltToConv)
            );
            if (!eltToConvC)
                throw INTERP_KERNEL::Exception(
                    "MEDFileIntFieldMultiTSWithoutSDA::convertToInt : presence of an invalid 1TS type ! Should be of "
                    "type INT32 !"
                );
...
template <class T>
MEDFileTemplateFieldMultiTS<T>::MEDFileTemplateFieldMultiTS(med_idt fid, bool loadAll, const MEDFileMeshes *ms)
    : MEDFileAnyTypeFieldMultiTS(fid, loadAll, ms)
{
}
...
MEDFileTemplateFieldMultiTS<T>::MEDFileTemplateFieldMultiTS(
    ...
    : MEDFileAnyTypeFieldMultiTS(fid, fieldName, loadAll, ms, entities)
{
}

template <class T>
MEDFileTemplateFieldMultiTS<T>::MEDFileTemplateFieldMultiTS(
    const typename MLFieldTraits<T>::FMTSWSDAType &other, bool shallowCopyOfContent
)
    : MEDFileAnyTypeFieldMultiTS(other, shallowCopyOfContent)
{
}
 ...
 */
template <class T>
typename MLFieldTraits<T>::FMTSType *
MEDFileTemplateFieldMultiTS<T>::extractPartImpl(
    const std::map<int, MCAuto<DataArrayIdType> > &extractDef, MEDFileMesh *mm
) const
{
    if (!mm)
    ...
    for (int i = 0; i < nbTS; i++)
    {
        ...
        MCAuto<typename MLFieldTraits<T>::F1TSType> f1ts2(
            DynamicCastSafe<MEDFileAnyTypeField1TS, typename MLFieldTraits<T>::F1TSType>(f1ts)
        );
...
template <class T>
typename MLFieldTraits<T>::FMTSType *
MEDFileTemplateFieldMultiTS<T>::New()
{
...
template <class T>
typename MLFieldTraits<T>::FMTSType *
MEDFileTemplateFieldMultiTS<T>::New(const std::string &fileName, bool loadAll)
{
    MEDFileUtilities::AutoFid fid(OpenMEDFileForRead(fileName));
    return New(fid, loadAll);
}
...
template <class T>
typename MLFieldTraits<T>::FMTSType *
MEDFileTemplateFieldMultiTS<T>::New(med_idt fid, bool loadAll)
{
...
template <class T>
typename MLFieldTraits<T>::FMTSType *
MEDFileTemplateFieldMultiTS<T>::New(const std::string &fileName, const std::string &fieldName, bool loadAll)
{
    MEDFileUtilities::AutoFid fid(OpenMEDFileForRead(fileName));
    return New(fid, fieldName, loadAll);
}
...
template <class T>
typename MLFieldTraits<T>::FMTSType *
MEDFileTemplateFieldMultiTS<T>::New(med_idt fid, const std::string &fieldName, bool loadAll)
{
    MCAuto<typename MLFieldTraits<T>::FMTSType> ret(new
                                                    typename MLFieldTraits<T>::FMTSType(fid, fieldName, loadAll, 0));
    ret->contentNotNull();  // to check that content type matches with \a this type.
    return ret.retn();
}

/*!
 * Returns a new instance of MEDFileFieldMultiTS. If \a shallowCopyOfContent is true the content of \a other is shallow
 * copied. If \a shallowCopyOfContent is false, \a other is taken to be the content of \a this.
 *
...
template <class T>
typename MLFieldTraits<T>::FMTSType *
MEDFileTemplateFieldMultiTS<T>::New(const typename MLFieldTraits<T>::FMTSWSDAType &other, bool shallowCopyOfContent)
{
...
template <class T>
typename MLFieldTraits<T>::FMTSType *
MEDFileTemplateFieldMultiTS<T>::LoadSpecificEntities(
...
)
{
    ...
    MCAuto<typename MLFieldTraits<T>::FMTSType> ret(
        new typename MLFieldTraits<T>::FMTSType(fid, fieldName, loadAll, 0, ent)
    );
    ret->contentNotNull();  // to check that content type matches with \a this type.
    return ret.retn();
}
...
template <class T>
typename Traits<T>::FieldType *
MEDFileTemplateFieldMultiTS<T>::field(int iteration, int order, const MEDFileMesh *mesh) const
{
...
template <class T>
typename Traits<T>::FieldType *
MEDFileTemplateFieldMultiTS<T>::getFieldAtLevel(
    TypeOfField type, int iteration, int order, int meshDimRelToMax, int renumPol
) const
{
    const MEDFileAnyTypeField1TSWithoutSDA &myF1TS(contentNotNullBase()->getTimeStepEntry(iteration, order));
    const typename MLFieldTraits<T>::F1TSWSDAType *myF1TSC(
        dynamic_cast<const typename MLFieldTraits<T>::F1TSWSDAType *>(&myF1TS)
    );
    if (!myF1TSC)
        throw INTERP_KERNEL::Exception(
            "MEDFileTemplateFieldMultiTS<T>::getFieldAtLevel : mismatch of type of field expecting FLOAT64 !"
        );
    MCAuto<DataArray> arrOut;
    MCAuto<MEDCouplingFieldDouble> ret(
        myF1TSC->getFieldAtLevel(type, meshDimRelToMax, std::string(), renumPol, this, arrOut, *contentNotNullBase())
    );
    MCAuto<typename Traits<T>::FieldType> ret2(MEDFileTemplateField1TS<T>::SetDataArrayInField(ret, arrOut));
    return ret2.retn();
}
...
template <class T>
typename Traits<T>::FieldType *
MEDFileTemplateFieldMultiTS<T>::getFieldAtTopLevel(TypeOfField type, int iteration, int order, int renumPol) const
{
    const MEDFileAnyTypeField1TSWithoutSDA &myF1TS(contentNotNullBase()->getTimeStepEntry(iteration, order));
    const typename MLFieldTraits<T>::F1TSWSDAType *myF1TSC(
        dynamic_cast<const typename MLFieldTraits<T>::F1TSWSDAType *>(&myF1TS)
    ...
    if (!myF1TSC)
        throw INTERP_KERNEL::Exception(
            "MEDFileTemplateFieldMultiTS<T>::getFieldAtTopLevel : mismatch of type of field !"
        );
    MCAuto<DataArray> arrOut;
    MCAuto<MEDCouplingFieldDouble> ret(
        myF1TSC->getFieldAtTopLevel(type, std::string(), renumPol, this, arrOut, *contentNotNullBase())
    );
    MCAuto<typename Traits<T>::FieldType> ret2(MEDFileTemplateField1TS<T>::SetDataArrayInField(ret, arrOut));
    return ret2.retn();
}
...
template <class T>
typename Traits<T>::FieldType *
MEDFileTemplateFieldMultiTS<T>::getFieldOnMeshAtLevel(
    TypeOfField type, int iteration, int order, int meshDimRelToMax, const MEDFileMesh *mesh, int renumPol
) const
{
    const MEDFileAnyTypeField1TSWithoutSDA &myF1TS(contentNotNullBase()->getTimeStepEntry(iteration, order));
    const typename MLFieldTraits<T>::F1TSWSDAType *myF1TSC(
    ...
    if (!myF1TSC)
        throw INTERP_KERNEL::Exception(
            "MEDFileTemplateFieldMultiTS<T>::getFieldOnMeshAtLevel : mismatch of type of field !"
        );
    MCAuto<DataArray> arrOut;
    MCAuto<MEDCouplingFieldDouble> ret(
        myF1TSC->getFieldOnMeshAtLevel(type, meshDimRelToMax, renumPol, this, mesh, arrOut, *contentNotNullBase())
    );
    MCAuto<typename Traits<T>::FieldType> ret2(MEDFileTemplateField1TS<T>::SetDataArrayInField(ret, arrOut));
    return ret2.retn();
}
...
typename Traits<T>::FieldType *
MEDFileTemplateFieldMultiTS<T>::getFieldOnMeshAtLevel(
    TypeOfField type, int iteration, int order, const MEDCouplingMesh *mesh, int renumPol
) const
{
    const MEDFileAnyTypeField1TSWithoutSDA &myF1TS(contentNotNullBase()->getTimeStepEntry(iteration, order));
    const typename MLFieldTraits<T>::F1TSWSDAType *myF1TSC(
    ...
    MCAuto<DataArray> arrOut;
    MCAuto<MEDCouplingFieldDouble> ret(
        myF1TSC->getFieldOnMeshAtLevel(type, renumPol, this, mesh, 0, 0, arrOut, *contentNotNullBase())
    );
    MCAuto<typename Traits<T>::FieldType> ret2(MEDFileTemplateField1TS<T>::SetDataArrayInField(ret, arrOut));
    return ret2.retn();
}
...
template <class T>
typename Traits<T>::FieldType *
MEDFileTemplateFieldMultiTS<T>::getFieldAtLevelOld(
    TypeOfField type, int iteration, int order, const std::string &mname, int meshDimRelToMax, int renumPol
) const
{
    const MEDFileAnyTypeField1TSWithoutSDA &myF1TS(contentNotNullBase()->getTimeStepEntry(iteration, order));
    const typename MLFieldTraits<T>::F1TSWSDAType *myF1TSC(
    ...
    if (!myF1TSC)
        throw INTERP_KERNEL::Exception(
            "MEDFileTemplateFieldMultiTS<T>::getFieldAtLevelOld : mismatch of type of field !"
        );
    MCAuto<DataArray> arrOut;
    MCAuto<MEDCouplingFieldDouble> ret(
        myF1TSC->getFieldAtLevel(type, meshDimRelToMax, mname, renumPol, this, arrOut, *contentNotNullBase())
    );
    MCAuto<typename Traits<T>::FieldType> ret2(MEDFileTemplateField1TS<T>::SetDataArrayInField(ret, arrOut));
    return ret2.retn();
}
...
template <class T>
typename Traits<T>::ArrayType *
MEDFileTemplateFieldMultiTS<T>::getFieldWithProfile(
    TypeOfField type, int iteration, int order, int meshDimRelToMax, const MEDFileMesh *mesh, DataArrayIdType *&pfl
) const
{
    const MEDFileAnyTypeField1TSWithoutSDA &myF1TS(contentNotNullBase()->getTimeStepEntry(iteration, order));
    const typename MLFieldTraits<T>::F1TSWSDAType *myF1TSC(
    ...
    if (!myF1TSC)
        throw INTERP_KERNEL::Exception(
            "MEDFileTemplateFieldMultiTS<T>::getFieldWithProfile : mismatch of type of field !"
        );
...
template <class T>
void
MEDFileTemplateFieldMultiTS<T>::appendFieldNoProfileSBT(const typename Traits<T>::FieldType *field)
{
...
template <class T>
void
MEDFileTemplateFieldMultiTS<T>::appendFieldProfile(
    const typename Traits<T>::FieldType *field,
    const MEDFileMesh *mesh,
    int meshDimRelToMax,
    const DataArrayIdType *profile
...
template <class T>
void
MEDFileTemplateFieldMultiTS<T>::appendFieldProfileFlatly(
    const typename Traits<T>::FieldType *field,
    const MEDFileMesh *mesh,
    int meshDimRelToMax,
    const DataArrayIdType *profile
...
template <class T>
void
MEDFileTemplateFieldMultiTS<T>::appendFieldProfileGeneral(
    const typename Traits<T>::FieldType *field,
    const MEDFileMesh *mesh,
    int meshDimRelToMax,
    const DataArrayIdType *profile,
...
template <class T>
const typename MLFieldTraits<T>::FMTSWSDAType *
MEDFileTemplateFieldMultiTS<T>::contentNotNull() const
{
    const MEDFileAnyTypeFieldMultiTSWithoutSDA *pt(_content);
    if (!pt)
        throw INTERP_KERNEL::Exception(
            "MEDFileTemplateFieldMultiTS<T>::contentNotNull : the content pointer is null !"
        );
    const typename MLFieldTraits<T>::FMTSWSDAType *ret =
        dynamic_cast<const typename MLFieldTraits<T>::FMTSWSDAType *>(pt);
    if (!ret)
        throw INTERP_KERNEL::Exception(
            "MEDFileTemplateFieldMultiTS<T>::contentNotNull : the content pointer is not null but it is not of type "
            "double ! Reason is maybe that the read field has not the type FLOAT64 !"
        );
    return ret;
}
...
template <class T>
typename MLFieldTraits<T>::FMTSWSDAType *
MEDFileTemplateFieldMultiTS<T>::contentNotNull()
{
    MEDFileAnyTypeFieldMultiTSWithoutSDA *pt(_content);
    if (!pt)
        throw INTERP_KERNEL::Exception(
            "MEDFileTemplateFieldMultiTS<T>::contentNotNull : the non const content pointer is null !"
        );
    typename MLFieldTraits<T>::FMTSWSDAType *ret(dynamic_cast<typename MLFieldTraits<T>::FMTSWSDAType *>(pt));
    if (!ret)
        throw INTERP_KERNEL::Exception(
            "MEDFileTemplateFieldMultiTS<T>::contentNotNull : the non const content pointer is not null but it is not "
            "of type double ! Reason is maybe that the read field has not the type FLOAT64 !"
        );
    return ret;
}
...
template <class T>
typename MLFieldTraits<T>::F1TSType *
MEDFileTemplateFieldMultiTS<T>::getTimeStepAtPos(int pos) const
{
    const MEDFileAnyTypeField1TSWithoutSDA *item(contentNotNullBase()->getTimeStepAtPos2(pos));
    if (!item)
    {
        std::ostringstream oss;
        oss << "MEDFileFieldMultiTS::getTimeStepAtPos : field at pos #" << pos << " is null !";
        throw INTERP_KERNEL::Exception(oss.str());
    }
    const typename MLFieldTraits<T>::F1TSWSDAType *itemC =
        dynamic_cast<const typename MLFieldTraits<T>::F1TSWSDAType *>(item);
    if (itemC)
    {
    ...
    std::ostringstream oss;
    oss << "MEDFileFieldMultiTS::getTimeStepAtPos : type of field at pos #" << pos << " is not "
        << MLFieldTraits<T>::F1TSWSDAType::TYPE_STR << " !";
    throw INTERP_KERNEL::Exception(oss.str());
}
...
template <class T>
typename Traits<T>::ArrayType *
MEDFileTemplateFieldMultiTS<T>::getUndergroundDataArray(int iteration, int order) const
{
    ...
    {
        std::ostringstream oss;
        oss << "MEDFileTemplateFieldMultiTS<T>::getUndergroundDataArray : invalid type of data detected ! Expecting "
            << MLFieldTraits<T>::F1TSWSDAType::TYPE_STR;
        throw INTERP_KERNEL::Exception(oss.str());
    }
    return ret2;
...
template <class T>
typename Traits<T>::ArrayType *
MEDFileTemplateFieldMultiTS<T>::getUndergroundDataArrayExt(
    int iteration,
    int order,
    std::vector<std::pair<std::pair<INTERP_KERNEL::NormalizedCellType, int>, std::pair<mcIdType, mcIdType> > > &entries
) const
{
    ...
    {
        std::ostringstream oss;
        oss << "MEDFileTemplateFieldMultiTS<T>::getUndergroundDataArrayExt : invalid type of data detected ! Expecting "
            << MLFieldTraits<T>::F1TSWSDAType::TYPE_STR;
        throw INTERP_KERNEL::Exception(oss.str());
    }
    return ret2;
...
template <class T>
typename MLFieldTraits<T>::FMTSType *
MEDFileTemplateFieldMultiTS<T>::buildNewEmptyImpl() const
{
...
template <class T>
void
MEDFileTemplateFieldMultiTS<T>::checkCoherencyOfType(const MEDFileAnyTypeField1TS *f1ts) const
{
    if (!f1ts)
        throw INTERP_KERNEL::Exception(
            "MEDFileTemplateFieldMultiTS<T>::checkCoherencyOfType : input field1TS is NULL ! Impossible to check !"
        );
    const typename MLFieldTraits<T>::F1TSType *f1tsC = dynamic_cast<const typename MLFieldTraits<T>::F1TSType *>(f1ts);
    if (!f1tsC)
    {
        std::ostringstream oss;
        oss << "MEDFileTemplateFieldMultiTS<T>::checkCoherencyOfType : the input field1TS is not a "
            << MLFieldTraits<T>::F1TSWSDAType::TYPE_STR << " type !";
        throw INTERP_KERNEL::Exception(oss.str());
    }
}
...
template <class T>
MEDFileFieldMultiTS *
MEDFileNDTemplateFieldMultiTS<T>::convertToDouble(bool isDeepCpyGlobs) const
{
    MCAuto<MEDFileFieldMultiTS> ret;
    const MEDFileAnyTypeFieldMultiTSWithoutSDA *content(this->_content);
    if (content)
    {
        const typename MLFieldTraits<T>::FMTSWSDAType *contc =
            dynamic_cast<const typename MLFieldTraits<T>::FMTSWSDAType *>(content);
        if (!contc)
            throw INTERP_KERNEL::Exception(
                "MEDFileIntFieldMultiTS::convertToInt : the content inside this is not INT32 ! This is incoherent !"
            );
        MCAuto<MEDFileFieldMultiTSWithoutSDA> newc(contc->convertToDouble());
        ret = static_cast<MEDFileFieldMultiTS *>(
            MEDFileAnyTypeFieldMultiTS::BuildNewInstanceFromContent((MEDFileFieldMultiTSWithoutSDA *)newc)
        );
    }
    else
