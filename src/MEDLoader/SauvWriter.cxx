...
string
noProfileName(INTERP_KERNEL::NormalizedCellType type)
{
...
string
cleanName(const string &theName)
{
    string name = theName;
    if (!name.empty())
    {
...
string
addName(map<string, int> &nameMap, map<string, int> &namePrefixesMap, const string &theName, const int index)
{
    // Converts names like:
    // MED:                       GIBI:
    ...
    if (!healedName.empty())
    {
        ...
        for (std::size_t i = 0; i < len; ++i) name[i] = (char)toupper(name[i]);
                ...
                if (len == 8)
                {
                    ...
                    if (new_loc_index > 0)
                    {
                        ...
                        if (namePrefixesMap.find(str) != namePrefixesMap.end())
                        {
                            ...
                            if (new_loc_index < old_loc_index)
                                new_loc_index = old_loc_index;
                        }
...
std::size_t
SauvWriter::getHeapMemorySizeWithoutChildren() const
{
    return 0;
}

std::vector<const BigMemoryObject *>
SauvWriter::getDirectChildrenWithNull() const
{
    return std::vector<const BigMemoryObject *>();
}

void
SauvWriter::setCpyGrpIfOnASingleFamilyStatus(bool status)
{
    _cpy_grp_if_on_single_family = status;
}

bool
SauvWriter::getCpyGrpIfOnASingleFamilyStatus() const
{
...
void
SauvWriter::setMEDFileDS(const MEDFileData *medData, unsigned meshIndex)
{
    if (!medData)
        THROW_IK_EXCEPTION("NULL MEDFileData");

    MEDFileMeshes *meshes = medData->getMeshes();
    MEDFileFields *fields = medData->getFields();
    if (!meshes)
        THROW_IK_EXCEPTION("No meshes in MEDFileData");
...
SauvWriter::SubMesh *
SauvWriter::addSubMesh(const std::string &name, int dimRelExt)
{
...
int
SauvWriter::SubMesh::nbTypes() const
{
    int nb = 0;
    for (int i = 0; i < cellIDsByTypeSize(); ++i) nb += int(!_cellIDsByType[i].empty());
    return nb;
}
...
void
SauvWriter::fillSubMeshes(int &nbSauvObjects, map<string, int> &nameNbMap)
{
...
void
SauvWriter::fillFamilySubMeshes()
{
    ...
    for (size_t iDim = 0; iDim < dims.size(); ++iDim)
    {
        ...
        else if (dimRelExt == 0)
        {
            ...
            else
            {
                ...
                for (std::size_t i = sub0Index; i < _subs.size() - 1; ++i) curSubMesh->_subs.push_back(&_subs[i]);
            }
        }
    }
}
...
void
SauvWriter::fillProfileSubMeshes()
{
    _profile2Sub.clear();
    SubMesh *nilSm = (SubMesh *)0;
    for (int isOnNodes = 0; isOnNodes < 2; ++isOnNodes)
    {
        vector<MCAuto<MEDFileFieldMultiTS> > fields = isOnNodes ? _nodeFields : _cellFields;
        for (size_t i = 0; i < fields.size(); ++i)
        {
            ...
            fields[i]->getFieldSplitedByType(
                iters[0].first, iters[0].second, _fileMesh->getName().c_str(), types, typesF, pfls, locs
            );
            int dimRelExt;
            for (size_t iType = 0; iType < types.size(); ++iType)
            {
                ...
                for (size_t iPfl = 0; iPfl < pfls[iType].size(); ++iPfl)
                {
                    ...
                    if (isOnAll)
                        pfls[iType][iPfl] = noProfileName(types[iType]);
...
int
SauvWriter::evaluateNbProfileSubMeshes() const
{
    std::size_t nb = 0;
    for (std::size_t i = 0; i < _nodeFields.size(); ++i) nb += 1 + _nodeFields[i]->getPflsReallyUsed().size();
    ...
    for (std::size_t i = 0; i < _cellFields.size(); ++i)
    {
        ...
        vector<vector<TypeOfField> > typesF;
        vector<vector<string> > pfls, locs;
        _cellFields[i]->getFieldSplitedByType(
            iters[0].first, iters[0].second, _fileMesh->getName().c_str(), types, typesF, pfls, locs
        );
...
void
SauvWriter::makeProfileIDs(SubMesh *sm, INTERP_KERNEL::NormalizedCellType type, const DataArrayIdType *profile)
{
    MCAuto<MEDCouplingMesh> mesh = _fileMesh->getMeshAtLevel(sm->_dimRelExt);
    const MEDCouplingUMesh *uMesh = dynamic_cast<const MEDCouplingUMesh *>((const MEDCouplingMesh *)mesh);

    if (sm->_dimRelExt == 1)
        type = INTERP_KERNEL::NORM_POINT1;
    ...
    if (sm->_dimRelExt == 1 || !uMesh)
    {
        ...
        else  // on all
        {
            ...
            for (size_t i = 0; i < ids.size(); ++i) ids[i] = ToIdType(i);
        }
    }
    else
    {
        ...
        MCAuto<DataArrayIdType> resIDs = uMesh->checkTypeConsistencyAndContig(code, idsPerType);
        ...
        {
            mcIdType nbE = code[1];
            for (ids.resize(nbE); nbE; --nbE) ids[nbE - 1] = nbE - 1;
        }
    }
}
...
void
SauvWriter::write(const std::string &fileName)
{
...
void
SauvWriter::writeFileHead()
{
    MCAuto<MEDCouplingMesh> mesh = _fileMesh->getMeshAtLevel(0);

    *_sauvFile << " ENREGISTREMENT DE TYPE   4" << endl
...
void
SauvWriter::writeNames(const map<string, int> &nameNbMap)
{
    if (!nameNbMap.empty())
    {
        ...
        *_sauvFile << left;
        ...
        for (; nameNbIt != nameNbMap.end(); nameNbIt++, fcount++) *_sauvFile << " " << setw(8) << nameNbIt->first;
        fcount.stop();
        *_sauvFile << right;
...
SauvWriter::writeSubMeshes()
{
    ...
    *_sauvFile << " PILE NUMERO   1NBRE OBJETS NOMMES" << setw(8) << nameNbMap.size() << "NBRE OBJETS" << setw(8)
               << nbSauvObjects << endl;
    ...
    for (unsigned int iSub = 0; iSub < _subs.size(); ++iSub)
    {
        ...
        else
        {
            ...
            for (int iType = 0; iType < sm.cellIDsByTypeSize(); ++iType)
            {
                ...
                INTERP_KERNEL::NormalizedCellType cellType = INTERP_KERNEL::NormalizedCellType(iType);
                const INTERP_KERNEL::CellModel &cell = INTERP_KERNEL::CellModel::GetCellModel(cellType);
                ...
                *_sauvFile << setw(8) << castemType << zeroI8 << zeroI8 << setw(8) << nbElemNodes << setw(8) << nbElems
                           << endl;
                ...
                {
                    for (std::size_t i = 0; i < nbElems; ++i, fcount++) *_sauvFile << setw(8) << (cellIDs[i] + 1);
                }
                else
                {
                    ...
                    for (std::size_t i = 0; i < nbElems; ++i)
                    {
                        ...
                        if (toMedConn)
                        {
                            for (unsigned j = 0; j < nbElemNodes; ++j) transformedConn[toMedConn[j]] = cellConn[j];
                            cellConn.swap(transformedConn);
                        }
                        for (unsigned j = 0; j < nbElemNodes; ++j, fcount++) *_sauvFile << setw(8) << (cellConn[j] + 1);
                    }
                }
                fcount.stop();
...
void
SauvWriter::writeCompoundSubMesh(int iSub)
{
    SubMesh &sm = _subs[iSub];
    if (sm._nbSauvObjects < 1 || sm._subs.empty())
        return;

    vector<int> subIDs;
    for (size_t i = 0; i < sm._subs.size(); ++i)  // loop on sub-meshes of families
        for (int j = 0; j < sm._subs[i]->_nbSauvObjects; ++j) subIDs.push_back(sm._subs[i]->_id + j);

    *_sauvFile << zeroI8 << setw(8) << subIDs.size() << zeroI8 << zeroI8 << zeroI8 << endl;

    TFieldCounter fcount(*_sauvFile, 10);  // 10 integers per line
    for (size_t i = 0; i < subIDs.size(); ++i, fcount++) *_sauvFile << setw(8) << subIDs[i];
}
...
SauvWriter::writeNodes()
{
    ...
    *_sauvFile << setw(8) << nbNodes << endl;
    ...
    for (mcIdType i = 0; i < nbNodes; ++i, fcount++) *_sauvFile << setw(8) << i + 1;
    fcount.stop();
...
void
SauvWriter::writeLongNames()
{
    int nbTables = 3 - _longNames[LN_MAIL].empty() - _longNames[LN_CHAM].empty() - _longNames[LN_COMP].empty();
    if (nbTables == 0)
        return;
    ...
    *_sauvFile << " ENREGISTREMENT DE TYPE   2" << endl
               << " PILE NUMERO  10NBRE OBJETS NOMMES" << setw(8) << nbTables << "NBRE OBJETS" << setw(8) << nbTables
               << endl;
    // table names
    if (!_longNames[LN_MAIL].empty())
        *_sauvFile << " MED_MAIL";
    if (!_longNames[LN_CHAM].empty())
        *_sauvFile << " MED_CHAM";
    if (!_longNames[LN_COMP].empty())
        *_sauvFile << " MED_COMP";
    *_sauvFile << endl;
    // table indices
    ...
    *_sauvFile << " ENREGISTREMENT DE TYPE   2" << endl
    ...
    // write the offsets
    for (std::size_t i = 0; i < theOffsets.size(); ++i, fcount++) *_sauvFile << setw(8) << theOffsets[i];
}
...
void
SauvWriter::writeFieldNames(const bool isNodal, std::map<std::string, int> &fldNamePrefixMap)
{
    vector<MCAuto<MEDFileFieldMultiTS> > &flds = isNodal ? _nodeFields : _cellFields;
    map<string, int> nameNbMap;
    ...
    *_sauvFile << " ENREGISTREMENT DE TYPE   2" << endl
               << (isNodal ? " PILE NUMERO   2" : " PILE NUMERO  39") << "NBRE OBJETS NOMMES" << setw(8)
               << nameNbMap.size() << "NBRE OBJETS" << setw(8) << flds.size() << endl;
    writeNames(nameNbMap);
}
...
void
SauvWriter::makeCompNames(const string &fieldName, const vector<string> &compInfo, map<string, string> &mapMedToGibi)
{
    for (size_t i = 0; i < compInfo.size(); ++i) mapMedToGibi[compInfo[i]] = cleanName(compInfo[i]);
...
void
SauvWriter::writeNodalFields(map<string, int> &fldNamePrefixMap)
{
    ...
    for (size_t iF = 0; iF < _nodeFields.size(); ++iF)
    {
        ...
        *_sauvFile << setw(8) << nbSub << setw(8) << totalNbComp << setw(8) << -1  // IFOUR
        ...
        for (std::size_t iIt = 0; iIt < iters.size(); ++iIt)
        {
            ...
            valsVec = _nodeFields[iF]->getFieldSplitedByType(
                it.first, it.second, _fileMesh->getName().c_str(), types, typesF, pfls, locs
            );
            // believe that there can be only one type in a nodal field,
            // so do not use a loop on types
            if (pfls[0][0].empty())
                pfls[0][0] = noProfileName(types[0]);
            map<string, SubMesh *>::iterator pfl2Sub = _profile2Sub.find(pfls[0][0]);
            if (pfl2Sub == _profile2Sub.end())
                THROW_IK_EXCEPTION("SauvWriter::writeNodalFields(): no sub-mesh for profile |" << pfls[0][0] << "|");
            vals[0] = -pfl2Sub->second->_id;
            vals[1] = (valsVec[0][0].second - valsVec[0][0].first);
            vals[2] = ToIdType(compInfo.size());
            for (size_t i = 0; i < vals.size(); ++i, fcount++) *_sauvFile << setw(8) << vals[i];
        }
        fcount.stop();
        ...
        *_sauvFile << right;
        ...
        // (4) nb harmonics
        fcount.init(10);
        for (size_t i = 0; i < (std::size_t)totalNbComp; ++i, fcount++) *_sauvFile << " " << setw(8) << 0;
        fcount.stop();
...
void
SauvWriter::writeElemFields(map<string, int> &fldNamePrefixMap)
{
    ...
    for (unsigned int iF = 0; iF < _cellFields.size(); ++iF)
    {
        ...
        for (std::size_t iIt = 0; iIt < iters.size(); ++iIt)
        {
            ...
            vector<vector<string> > pfls, locs;
            vector<vector<std::pair<mcIdType, mcIdType> > > valsVec;
            valsVec = _cellFields[iF]->getFieldSplitedByType(
                it.first, it.second, _fileMesh->getName().c_str(), types, typesF, pfls, locs
            );
            for (size_t i = 0; i < valsVec.size(); ++i) nbSub += valsVec[i].size();
        }
        // (1) write nb sub-components, title length
        *_sauvFile << setw(8) << nbSub << setw(8) << -1  // whatever
                   << setw(8) << 6                       // whatever
        ...
        *_sauvFile << setw(72) << " " << endl;
        ...
        for (std::size_t iIt = 0; iIt < iters.size(); ++iIt)
        {
            ...
            vector<vector<TypeOfField> > typesF;
            vector<vector<string> > pfls, locs;
            _cellFields[iF]->getFieldSplitedByType(
                it.first, it.second, _fileMesh->getName().c_str(), types, typesF, pfls, locs
            );
            for (size_t iType = 0; iType < pfls.size(); ++iType)
                for (size_t iP = 0; iP < pfls[iType].size(); ++iP)
                {
                    if (pfls[iType][iP].empty())
                        pfls[iType][iP] = noProfileName(types[iType]);
                    map<string, SubMesh *>::iterator pfl2Sub = _profile2Sub.find(pfls[iType][iP]);
                    if (pfl2Sub == _profile2Sub.end())
                        THROW_IK_EXCEPTION(
                            "SauvWriter::writeElemFields(): no sub-mesh for profile |" << pfls[iType][iP] << "|"
                        );
        ...
        // (4) dummy strings
        for (fcount.init(4), iSub = 0; iSub < nbSub; ++iSub, fcount++) *_sauvFile << "                  ";
        fcount.stop();

        // (5) dummy strings
        for (fcount.init(8), iSub = 0; iSub < nbSub; ++iSub, fcount++) *_sauvFile << "         ";
        fcount.stop();
...
void
SauvWriter::writeElemTimeStamp(int iF, int iter, int order)
{
    // (6)   317767  317761  317755  317815
    // (7)  YOUN     NU       H        SIGY
    // (8)  REAL*8            REAL*8            REAL*8            REAL*8
    ...
    vector<vector<std::pair<mcIdType, mcIdType> > > valsVec;
    valsVec =
        _cellFields[iF]->getFieldSplitedByType(iter, order, _fileMesh->getName().c_str(), types, typesF, pfls, locs);
    for (size_t iType = 0; iType < pfls.size(); ++iType)
        for (size_t iP = 0; iP < pfls[iType].size(); ++iP)
        {
            const vector<string> &compInfo = _cellFields[iF]->getInfo();
            ...
            for (fcount.init(10); iComp < nbComp; ++iComp, fcount++) *_sauvFile << setw(8) << 777;  // a good number
            fcount.stop();
            ...
            *_sauvFile << right;
            ...
            {
                *_sauvFile << setw(8) << nbPntPerCell << setw(8) << (bgEnd.second - bgEnd.first) / nbPntPerCell
                           << setw(8) << 0 << setw(8) << 0 << endl;
                fcount.init(3);
