...
inline void
ConvertQuadratic(const INTERP_KERNEL::NormalizedCellType type, const Cell &aCell)
{
    if (const int *conn = getGibi2MedQuadraticInterlace(type))
    {
        ...
        for (std::size_t i = 0; i < new_nodes.size(); ++i) new_nodes[i] = ma->_nodes[conn[i]];
...
void
getReverseVector(const INTERP_KERNEL::NormalizedCellType type, std::vector<std::pair<int, int> > &swapVec)
{
    swapVec.clear();
...
inline void
reverse(const Cell &aCell, const std::vector<std::pair<int, int> > &swapVec)
{
    Cell *ma = const_cast<Cell *>(&aCell);
    for (unsigned i = 0; i < swapVec.size(); ++i)
        std::swap(ma->_nodes[swapVec[i].first], ma->_nodes[swapVec[i].second]);
    if (swapVec.empty())
        ma->_reverse = true;
    else
        ma->_reverse = false;
...
{
    bool operator()(const Cell *i1, const Cell *i2) const { return i1->_number < i2->_number; }
};
...
void
setRelocationTable(Group *grp, TCellToOrderMap &cell2order)
{
    if (!grp->_isProfile)
        return;
    ...
    for (; !isRelocated && c2oIt != c2oEnd; ++c2oIt, ++newOrder) isRelocated = (c2oIt->second != newOrder);
...
struct TShapeFun
{
    ...
    TShapeFun(TInt theDim = 0, TInt theNbRef = 0) : myDim(theDim), myNbRef(theNbRef), myRefCoord(theNbRef * theDim) {}
...
void
TGaussDef::add(const double x, const double weight)
{
    if (dim() != 1)
...
void
TGaussDef::add(const double x, const double y, const double weight)
{
    if (dim() != 2)
...
void
TGaussDef::add(const double x, const double y, const double z, const double weight)
{
    if (dim() != 3)
...
//---------------------------------------------------------------
TTetra4a::TTetra4a() : TShapeFun(3, 4)
{
    TInt aNbRef = GetNbRef();
    for (TInt aRefId = 0; aRefId < aNbRef; aRefId++)
    {
...
//---------------------------------------------------------------
TTetra10a::TTetra10a() : TShapeFun(3, 10)
{
    TInt aNbRef = GetNbRef();
    for (TInt aRefId = 0; aRefId < aNbRef; aRefId++)
    {
...
//---------------------------------------------------------------
TTetra4b::TTetra4b() : TShapeFun(3, 4)
{
    TInt aNbRef = GetNbRef();
    for (TInt aRefId = 0; aRefId < aNbRef; aRefId++)
    {
...
//---------------------------------------------------------------
TTetra10b::TTetra10b() : TShapeFun(3, 10)
{
    TInt aNbRef = GetNbRef();
    for (TInt aRefId = 0; aRefId < aNbRef; aRefId++)
    {
...
//---------------------------------------------------------------
THexa20a::THexa20a(TInt theDim, TInt theNbRef) : TShapeFun(theDim, theNbRef)
{
...
//---------------------------------------------------------------
THexa27a::THexa27a() : THexa20a(3, 27)
{
    std::size_t aNbRef = myRefCoord.size();
    for (std::size_t aRefId = 0; aRefId < aNbRef; aRefId++)
    {
...
//---------------------------------------------------------------
THexa20b::THexa20b(TInt theDim, TInt theNbRef) : TShapeFun(theDim, theNbRef)
{
    std::size_t aNbRef = myRefCoord.size();
    for (std::size_t aRefId = 0; aRefId < aNbRef; aRefId++)
    {
...
//---------------------------------------------------------------
TPenta6a::TPenta6a() : TShapeFun(3, 6)
{
    std::size_t aNbRef = myRefCoord.size();
    for (std::size_t aRefId = 0; aRefId < aNbRef; aRefId++)
    {
...
//---------------------------------------------------------------
TPenta6b::TPenta6b() : TShapeFun(3, 6)
{
    std::size_t aNbRef = myRefCoord.size();
    for (std::size_t aRefId = 0; aRefId < aNbRef; aRefId++)
    {
...
//---------------------------------------------------------------
TPenta15a::TPenta15a() : TShapeFun(3, 15)
{
    std::size_t aNbRef = myRefCoord.size();
    for (std::size_t aRefId = 0; aRefId < aNbRef; aRefId++)
    {
...
//---------------------------------------------------------------
TPenta15b::TPenta15b() : TShapeFun(3, 15)
{
    std::size_t aNbRef = myRefCoord.size();
    for (std::size_t aRefId = 0; aRefId < aNbRef; aRefId++)
    {
...
//---------------------------------------------------------------
TPyra13a::TPyra13a() : TShapeFun(3, 13)
{
    std::size_t aNbRef = myRefCoord.size();
    for (std::size_t aRefId = 0; aRefId < aNbRef; aRefId++)
    {
...
//---------------------------------------------------------------
TPyra13b::TPyra13b() : TShapeFun(3, 13)
{
    std::size_t aNbRef = myRefCoord.size();
    for (std::size_t aRefId = 0; aRefId < aNbRef; aRefId++)
    {
...
TGaussDef::TGaussDef(const int geom, const int nbGauss, const int variant)
{
    ...
    switch (geom)
    {
        ...
        case NORM_TRI6:
            ...
            {
                if (geom == NORM_TRI3)
                    setRefCoords(TTria3b());
                else
                    setRefCoords(TTria6b());
                switch (nbGauss)
                {
                    case 1:
                    {  // FPG1
                    ...
                    default:
                        THROW_IK_EXCEPTION("TGaussDef: Invalid nb of gauss points for TRIA, variant 1: " << nbGauss);
                }
            }
            else if (variant == 2)
            {
                if (geom == NORM_TRI3)
                    setRefCoords(TTria3a());
                else
                    setRefCoords(TTria6a());
                switch (nbGauss)
                {
                    case 1:
                    {
                    ...
                    case 6:
                    {
                        ...
                        add(2 * A - 1, 2 * A - 1, 4 * P1);
                        break;
                    }
                    default:
                        THROW_IK_EXCEPTION("TGaussDef: Invalid nb of gauss points for TRIA, variant 2: " << nbGauss);
                }
            }
            else if (variant == 3)
            {
                ...
                switch (nbGauss)
                {
                    ...
                    default:
                        THROW_IK_EXCEPTION("TGaussDef: Invalid nb of gauss points for TRIA, variant 3: " << nbGauss);
                }
            }
            break;
        ...
        case NORM_QUAD8:
            ...
            {
                if (geom == NORM_QUAD4)
                    setRefCoords(TQuad4b());
                else
                    setRefCoords(TQuad8b());
                switch (nbGauss)
                {
                    case 1:
                    {  // FPG1
                    ...
                    default:
                        THROW_IK_EXCEPTION("TGaussDef: Invalid nb of gauss points for QUAD, variant 1: " << nbGauss);
                }
            }
            else if (variant == 2)
            {
                if (geom == NORM_QUAD4)
                    setRefCoords(TQuad4a());
                else
                    setRefCoords(TQuad8a());
                switch (nbGauss)
                {
                    case 4:
                    {
            ...
            else if (variant == 3)
            {
                ...
                switch (nbGauss)
                {
                    ...
                    default:
                        THROW_IK_EXCEPTION("TGaussDef: Invalid nb of gauss points for QUAD, variant 3: " << nbGauss);
                }
            }
            break;
        ...
        case NORM_TETRA10:
            if (geom == NORM_TETRA4)
                setRefCoords(TTetra4a());
            else
                setRefCoords(TTetra10a());
            switch (nbGauss)
            {
                case 4:
                {  // FPG4
        ...
        case NORM_PYRA13:
            if (geom == NORM_PYRA5)
                setRefCoords(TPyra5a());
            else
                setRefCoords(TPyra13a());
            switch (nbGauss)
            {
                case 5:
                {  // FPG5
        ...
        case NORM_PENTA15:
            if (geom == NORM_PENTA6)
                setRefCoords(TPenta6a());
            else
                setRefCoords(TPenta15a());
            switch (nbGauss)
            {
                case 6:
                {  // FPG6
        ...
        case NORM_HEXA20:
            if (geom == NORM_HEXA8)
                setRefCoords(THexa8a());
            else
                setRefCoords(THexa20a());
            switch (nbGauss)
            {
                case 8:
                {  // FPG8
...
unsigned
SauvUtilities::getDimension(INTERP_KERNEL::NormalizedCellType type)
{
...
const int *
SauvUtilities::getGibi2MedQuadraticInterlace(INTERP_KERNEL::NormalizedCellType type)
{
...
Cell::Cell(const Cell &ma) : _nodes(ma._nodes), _reverse(ma._reverse), _sortedNodeIDs(0), _number(ma._number)
{
...
Cell::getSortedNodes() const
{
    if (!_sortedNodeIDs)
    {
        ...
        for (size_t i = 0; i != l; ++i) _sortedNodeIDs[i] = _nodes[i]->_number;
...
bool
Cell::operator<(const Cell &ma) const
{
    if (_nodes.size() == 1)
...
std::ostream &
SauvUtilities::operator<<(std::ostream &os, const SauvUtilities::Cell &ma)
{
    os << "cell " << ma._number << " (" << ma._nodes.size() << " nodes) : < " << ma._nodes[0]->_number;
    for (size_t i = 1; i != ma._nodes.size(); ++i) os << ", " << ma._nodes[i]->_number;
#ifdef _DEBUG_
    os << " > sortedNodes: ";
    if (ma._sortedNodeIDs)
    {
        os << "< ";
        for (size_t i = 0; i != ma._nodes.size(); ++i) os << (i ? ", " : "") << ma._sortedNodeIDs[i];
        os << " >";
    }
    else
    {
...
Group::size() const
{
    ...
    if (!_relocTable.empty())
        ...
        for (size_t i = 0; i < _groups.size(); ++i) sizze += _groups[i]->size();
...
int
SauvUtilities::med2gibiGeom(INTERP_KERNEL::NormalizedCellType medGeomType)
{
...
ASCIIReader::ASCIIReader(const char *fileName) : FileReader(fileName), _file(-1) {}
...
bool
ASCIIReader::getNextLine(char *&line, bool raiseOEF /*= true*/)
{
    if (getLine(line))
        return true;
    if (raiseOEF)
...
ASCIIReader::getLine(char *&line)
{
    ...
    if (nBytesRest < GIBI_MaxOutputLen)
    {
        if (nBytesRest > 0)
        {
            ...
            for (std::size_t i = 0; i < nBytesRest; ++i) _start[i] = _ptr[i];
        ...
        _ptr = _start;
        const std::size_t nBytesRead = ::read(_file, &_start[nBytesRest], GIBI_BufferSize - nBytesRest);
...
void
ASCIIReader::init(int nbToRead, int nbPosInLine, int width, int shift /*= 0*/)
{
    _nbToRead = nbToRead;
...
void
ASCIIReader::initNameReading(int nbValues, int width /*= 8*/)
{
...
void
ASCIIReader::initIntReading(int nbValues)
{
    init(nbValues, 10, 8);
}
...
void
ASCIIReader::initDoubleReading(int nbValues)
{
    init(nbValues, 3, 22);
}
...
std::string
ASCIIReader::getName() const
{
    int len = _width;
    while ((_curPos[len - 1] == ' ' || _curPos[len - 1] == 0) && len > 0) len--;
...
bool
XDRReader::getNextLine(char *&, bool)
{
    return true;
}
...
void
XDRReader::init(int nbToRead, int width /*=0*/)
{
    if (_iRead < _nbToRead)
    {
...
void
XDRReader::initNameReading(int nbValues, int width)
{
    init(nbValues, width);
...
void
XDRReader::initIntReading(int nbValues)
{
    init(nbValues);
...
void
XDRReader::initDoubleReading(int nbValues)
{
    init(nbValues);
...
XDRReader::next()
{
    ...
    {
        if (_xdr_kind == _xdr_kind_char)
            free(_xdr_cvals);
        if (_xdr_kind == _xdr_kind_int)
            free(_xdr_ivals);
        if (_xdr_kind == _xdr_kind_double)
            free(_xdr_dvals);
...
std::string
XDRReader::getName() const
{
    int len = _width;
    char *s = _xdr_cvals + _iRead * _width;
    while ((s[len - 1] == ' ' || s[len - 1] == 0) && len > 0) len--;
...
void
IntermediateMED::checkDataAvailability() const
{
    if (_spaceDim == 0)
...
Group *
IntermediateMED::addNewGroup(std::vector<SauvUtilities::Group *> *groupsToFix)
{
...
MEDCoupling::MEDFileData *
IntermediateMED::convertInMEDFileDS()
{
...
MEDCoupling::MEDFileUMesh *
IntermediateMED::makeMEDFileMesh()
{
...
void
IntermediateMED::setGroupLongNames()
{
    ...
    {
        if ((int)_groups.size() < itGIBItoMED->gibi_id)
            continue;
    ...
    for (size_t i = 0; i < _groups.size(); ++i)
    {
        ...
        {
            for (size_t iC = 0; iC < grp._cells.size(); ++iC) grp._cells[iC]->_reverse = true;
...
void
IntermediateMED::setFieldLongNames(std::set<std::string> &usedNames)
{
    ...
    for (size_t i = 0; i < _nodeFields.size(); i++) usedNames.insert(_nodeFields[i]->_name);
    for (size_t i = 0; i < _cellFields.size(); i++) usedNames.insert(_cellFields[i]->_name);
}
...
void
IntermediateMED::decreaseHierarchicalDepthOfSubgroups()
{
    for (size_t i = 0; i != _groups.size(); ++i)
    {
        Group &grp = _groups[i];
...
void
IntermediateMED::eraseUselessGroups()
{
...
void
IntermediateMED::detectMixDimGroups()
{
    // hasMixedCells = false;
    for (size_t i = 0; i < _groups.size(); ++i)
    {
...
void
IntermediateMED::orientElements2D()
{
    ...
    if (!quadEdges.empty())
    {
        ...
        for (; elemIt != elemEnd; ++elemIt) ConvertQuadratic(INTERP_KERNEL::NORM_SEG3, *elemIt);
    }
...
void
IntermediateMED::orientElements3D()
{
...
void
IntermediateMED::numberElements()
{
    std::set<Cell>::const_iterator elemIt, elemEnd;
    ...
    // numbering _cells of type NORM_POINT1 by node number
    {
        ...
        for (; elemIt != elemEnd; ++elemIt) elemIt->_number = elemIt->_nodes[0]->_number;
    }
    ...
    for (int dim = 1; dim <= 3; ++dim)
    {
        ...
        while (const std::set<Cell> *typeCells = cellsIt.nextType())
        {
            ...
            {
                if (elemIt->_number < minNumber)
                    minNumber = elemIt->_number;
                if (elemIt->_number > maxNumber)
                    maxNumber = elemIt->_number;
            }
...
void
IntermediateMED::setConnectivity(MEDCoupling::MEDFileUMesh *mesh, MEDCoupling::DataArrayDouble *coords)
{
    int meshDim = 0;
    ...
    for (int dim = 3; dim > 0; --dim)
    {
        ...
        while (const std::set<Cell> *cells = dimCells.nextType()) nbOfCells += ToIdType(cells->size());
        if (nbOfCells == 0)
            continue;
        ...
        while (const std::set<Cell> *cells = dimCells.nextType())
        {
            ...
            for (elemIt = cells->begin(), elemEnd = cells->end(); elemIt != elemEnd; ++elemIt)
            {
                ...
                if (cell._reverse)
                    for (mcIdType i = nbCellNodes - 1; i >= 0; --i) *nodalConnOfCell++ = cell._nodes[i]->_number - 1;
                else
                    for (mcIdType i = 0; i < nbCellNodes; ++i) *nodalConnOfCell++ = cell._nodes[i]->_number - 1;
            }
...
void
IntermediateMED::setGroups(MEDCoupling::MEDFileUMesh *mesh)
{
    ...
    for (int dim = 0; dim <= meshDim; ++dim)
    {
        ...
        {
            Group &grp = _groups[i];
            if ((int)getDim(&grp) != dim && grp._groups.empty())  // to allow groups on diff dims
                continue;
            ...
            if (grp._groups.empty())
                groupVec.push_back(&grp);
            else
                groupVec = grp._groups;
            ...
            if (isSelfIntersect)  // self intersecting group
            {
                ...
                msg << "Self intersecting sub-mesh: id = " << i + 1 << ", name = |" << grp._name << "|" << std::endl
            ...
            if (!isMeshNameSet && dim == meshDim && !grp._name.empty() &&
...
bool
IntermediateMED::isOnAll(const Group *grp, int &dimRel) const
{
    int dim = getDim(grp);
    ...
    {
        CellsByDimIterator dimCells(*this, dim);
        while (const std::set<Cell> *cells = dimCells.nextType()) nbElems += ToIdType(cells->size());
...
IntermediateMED::makeMEDFileFields(MEDCoupling::MEDFileUMesh *mesh)
{
    if (_nodeFields.empty() && _cellFields.empty())
        return 0;
...
)
{
    ...
    if (!fld || !fld->isMedCompatible(sameNbGauss))
        return;
...
)
{
    ...
    MEDCouplingFieldDouble *timeStamp = MEDCouplingFieldDouble::New(fld->getMedType(iSub), fld->getMedTimeDisc());
    timeStamp->setName(fld->_name.c_str());
    timeStamp->setDescription(fld->_description.c_str());
    // set the mesh
    if (onAll)
    {
        MCAuto<MEDCouplingUMesh> dimMesh = mesh->getMeshAtLevel(dimRel);
        timeStamp->setMesh(dimMesh);
    }
    else if (timeStamp->getTypeOfField() == MEDCoupling::ON_NODES)
    {
        DataArrayDouble *coo = mesh->getCoords();
        MCAuto<DataArrayDouble> subCoo = coo->selectByTupleId(support->_medGroup->begin(), support->_medGroup->end());
        MCAuto<MEDCouplingUMesh> nodeSubMesh = MEDCouplingUMesh::Build0DMeshFromCoords(subCoo);
        timeStamp->setMesh(nodeSubMesh);
    }
    else
    {
        MCAuto<MEDCouplingUMesh> dimMesh = mesh->getMeshAtLevel(dimRel);
        MCAuto<MEDCouplingMesh> subMesh = dimMesh->buildPart(support->_medGroup->begin(), support->_medGroup->end());
    ...
    {
        TGaussDef gaussDef(fld->_sub[iSub]._support->_cellType, fld->_sub[iSub].nbGauss());
        timeStamp->setGaussLocalizationOnType(
            fld->_sub[iSub]._support->_cellType, gaussDef.myRefCoords, gaussDef.myCoords, gaussDef.myWeights
        );
    }
...
void
IntermediateMED::makeFieldNewName(std::set<std::string> &usedNames, SauvUtilities::DoubleField *fld)
{
...
void
DoubleField::splitSubWithDiffNbGauss()
{
    for (size_t iSub = 0; iSub < _sub.size(); ++iSub)
    {
        ...
        size_t iDiff = 1;
        while (subToSplit._nb_gauss[0] == subToSplit._nb_gauss[iDiff]) ++iDiff;
        subNew._support = subToSplit._support;
        subNew._comp_names.assign(subToSplit._comp_names.begin() + iDiff, subToSplit._comp_names.end());
        subNew._nb_gauss.assign(subToSplit._nb_gauss.begin() + iDiff, subToSplit._nb_gauss.end());
...
std::vector<double> &
DoubleField::addComponent(int nb_values)
{
...
const Group *
DoubleField::getSupport(const int iSub) const
{
...
bool
DoubleField::isMultiTimeStamps() const
{
    if (_sub.size() < 2)
        return false;
    bool sameSupports = true;
    Group *grpp1 = _sub[0]._support;  // grpp NOT grp because XDR under Windows defines grp...
    for (size_t i = 1; i < _sub.size() && sameSupports; ++i) sameSupports = (grpp1 == _sub[i]._support);
...
bool
DoubleField::isMedCompatible(bool &sameNbGauss) const
{
...
bool
DoubleField::hasSameComponentsBySupport() const
{
...
DoubleField::getMedType(const int iSub) const
{
    using namespace INTERP_KERNEL;
...
MEDCoupling::TypeOfTimeDiscretization
DoubleField::getMedTimeDisc() const
{
    return ONE_TIME;
    // NO_TIME = 4,
    // ONE_TIME = 5,
...
mcIdType
DoubleField::getNbTuples(const int iSub) const
{
    mcIdType nb = 0;
    if (hasCommonSupport() && !_group->_groups.empty())
        for (size_t i = 0; i < _group->_groups.size(); ++i) nb += _sub[i].nbGauss() * _sub[i]._support->size();
    else
...
mcIdType
DoubleField::setValues(double *valPtr, const int iSub, const mcIdType elemShift) const
{
    // find values for iSub
    int iComp = 0;
    for (int iS = 0; iS < iSub; ++iS) iComp += _sub[iS].nbComponents();
    ...
    // check nb values
    mcIdType nbVals = 0;
    for (iComp = 0; iComp < nbComponents; ++iComp) nbVals += ToIdType(compValues[iComp].size());
    ...
    if (isConstField)
        ...
        {
            mcIdType iMed =
                valsShift + nbValsByElem * (relocTable.empty() ? iE : relocTable[iE + elemShift] - elemShift);
            for (iComp = 0; iComp < nbComponents; ++iComp) valPtr[iMed + iComp] = compValues[iComp][0];
        }
    else
        for (mcIdType iE = 0; iE < nbElems; ++iE)
        {
 ...
 */
void
CellsByDimIterator::init(const int dimm)
{
    myCurType = -1;
 ...
 */
int
CellsByDimIterator::dim(const bool last) const
{
    int typp = myCurType;
    if (!last)
        while (typp < myTypeEnd && myImed->_cellsByType[typp].empty()) ++typp;
