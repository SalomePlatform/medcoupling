...
med_geometry_type typmai[MED_N_CELL_FIXED_GEO] = {MED_POINT1,  MED_SEG2,    MED_SEG3,     MED_SEG4,      MED_TRIA3,
 ...
 * @param possibilities the relativeToMeshDim authorized to returned maxdim. This vector is systematically cleared at
 * the begin of this method.
 * @return the maximal mesh dimension of specified mesh. If nothing found -1 is returned.
 */
int
MEDLoaderNS::readUMeshDimFromFile(
    const std::string &fileName, const std::string &meshName, std::vector<int> &possibilities
)
{
    possibilities.clear();
    ...
    for (int i = 0; i < MED_N_CELL_GEO_FIXED_CON; i++)
    {
        ...
        med_int curNbOfElemM(MEDmeshnEntity(
            fid, nommaa, numdt, numit, MED_CELL, curMedType, MED_CONNECTIVITY, MED_NODAL, &changement, &transformation
        ));
        med_int curNbOfElemF(MEDmeshnEntity(
            fid, nommaa, numdt, numit, MED_CELL, curMedType, MED_CONNECTIVITY, MED_NODAL, &changement, &transformation
        ));  // limitation
        med_int curNbOfElem;
...
int
MEDLoaderNS::getIdFromMeshName(med_idt fid, const std::string &meshName, std::string &trueMeshName)
{
    if (meshName.empty())
    {
...
MEDLoaderNS::getMeshNamesFid(med_idt fid)
{
    med_mesh_type type_maillage;
    char maillage_description[MED_COMMENT_SIZE + 1];
 ...
 */
void
MEDLoaderNS::dispatchElems(med_int nbOfElemCell, med_int nbOfElemFace, med_int &nbOfElem, med_entity_type &whichEntity)
{
...
void
MEDCoupling::AssignStaticWritePropertiesTo(MEDCoupling::MEDFileWritable &obj)
{
...
MEDCoupling::MEDFileVersionStr()
{
    ...
    if (pos != 0)
    {
        ...
        oss << "MEDFileVersionStr : internal error ! The MEDFile returned version (\"" << zeRet
            << "\") has not the right pattern !";
...
std::string
MEDCoupling::MEDFileVersionOfFileStr(const std::string &fileName)
{
#if MED_NUM_MAJEUR > 3 || \
    (MED_NUM_MAJEUR == 3 && ((MED_NUM_MINEUR == 2 && MED_NUM_RELEASE >= 1) || MED_NUM_MINEUR >= 3))
    MEDFileUtilities::AutoFid fid(MEDCoupling::OpenMEDFileForRead(fileName));
    const int SZ = 20;
    const char START_EXPECTED[] = "MED-";
    char buf[SZ];
    ...
    {
        std::ostringstream oss;
        oss << "MEDFileVersionOfFileStr : internal error ! The MEDFile returned version (\"" << ret
            << "\") has not the right pattern !";
        throw INTERP_KERNEL::Exception(oss.str());
    }
...
#else
    std::ostringstream oss;
    oss << "MEDFileVersionOfFileStr : is implemented with MEDFile " << MEDFileVersionStr()
        << " ! If you need this feature please use version >= 3.2.1.";
    throw INTERP_KERNEL::Exception(oss.str());
#endif
}

void
MEDCoupling::MEDFileVersion(int &major, int &minor, int &release)
{
 ...
 */
void
MEDCoupling::SetEpsilonForNodeComp(double val)
{
    _EPS_FOR_NODE_COMP = val;
}

/*!
 * This method sets the policy comparison when trying to fit the already written mesh on a field. The semantic of the
 * policy is specified in MEDCouplingUMesh::zipConnectivityTraducer.
 */
void
MEDCoupling::SetCompPolicyForCell(int val)
{
    _COMP_FOR_CELL = val;
}

/*!
 * This method set the behaviour of MEDLoader when a too long string is seen in datastructure before copy it in MED
 * file. By default (0) an exception is thrown. If equal to 1 a warning is emitted in std_err but no exception is
 * thrown.
 */
void
MEDCoupling::SetTooLongStrPolicy(int val)
{
    _TOO_LONG_STR = val;
}
 ...
 * - number of cells sorted by dimension and by geometry type. The first entry in the vector is the maximal dimension,
 * the 2nd in the vector is the maximal dimension-1...
 * - the mesh dimension
 * - the space dimension
 * - the number of nodes
 */
std::vector<std::vector<std::pair<INTERP_KERNEL::NormalizedCellType, int>>>
MEDCoupling::GetUMeshGlobalInfo(
    const std::string &fileName, const std::string &meshName, int &meshDim, int &spaceDim, mcIdType &numberOfNodes
)
{
    MEDFileUtilities::AutoFid fid(MEDCoupling::OpenMEDFileForRead(fileName));
    std::set<int> poss;
    ...
    {
        std::ostringstream oss;
        oss << "GetUMeshGlobalInfo : Mesh \"" << meshName << "\" in file \"" << fileName;
        oss << "\" exists but it is not an unstructured mesh ! This method is not relevant for mesh types that are not "
               "unstructured !";
    ...
    numberOfNodes = ToIdType(MEDmeshnEntity(
        fid, nommaa, numdt, numit, MED_NODE, MED_NONE, MED_COORDINATE, MED_NO_CMODE, &changement, &transformation
    ));
    return ret;
}

void
MEDCoupling::CheckFileForRead(const std::string &fileName)
{
    MEDFileUtilities::CheckFileForRead(fileName);
}

std::vector<std::string>
MEDCoupling::GetMeshNames(const std::string &fileName)
{
    MEDFileUtilities::AutoFid fid(MEDCoupling::OpenMEDFileForRead(fileName));
...
MEDCoupling::GetComponentsNamesOfField(const std::string &fileName, const std::string &fieldName)
{
    MEDFileUtilities::AutoFid fid(MEDCoupling::OpenMEDFileForRead(fileName));
    ...
    for (int i = 0; i < nbFields; i++)
    {
        ...
        if (curFieldName == fieldName)
        {
            ...
            for (int j = 0; j < ncomp; j++)
                ret[j] = std::pair<std::string, std::string>(
                    MEDLoaderBase::buildStringFromFortran(((char *)comp) + j * MED_SNAME_SIZE, MED_SNAME_SIZE),
    ...
    std::ostringstream oss;
    oss << "GetComponentsNamesOfField : no such field \"" << fieldName << "\" in file \"" << fileName << "\" !"
        << std::endl;
...
class MEDVectorStringIterator
{
   public:
    ...
    MEDVectorStringIterator &operator++()
    {
        ++_num;
        return *this;
    }
    ...
    {
        return MEDLoaderBase::buildStringFromFortran(_data + _num * MED_LNAME_SIZE, MED_LNAME_SIZE);
    }
...
void
MEDCoupling::GetFamiliesGroupsInfo(
...
std::vector<std::string>
MEDCoupling::GetMeshNamesOnField(const std::string &fileName, const std::string &fieldName)
{
...
std::vector<std::string>
MEDCoupling::GetMeshFamiliesNames(const std::string &fileName, const std::string &meshName)
{
    MEDFileUtilities::AutoFid fid(MEDCoupling::OpenMEDFileForRead(fileName));
...
std::vector<std::string>
MEDCoupling::GetMeshFamiliesNamesOnGroup(
    const std::string &fileName, const std::string &meshName, const std::string &grpName
)
{
    MEDFileUtilities::AutoFid fid(MEDCoupling::OpenMEDFileForRead(fileName));
    med_int nfam = MEDnFamily(fid, meshName.c_str());
...
std::vector<std::string>
MEDCoupling::GetMeshGroupsNamesOnFamily(
    const std::string &fileName, const std::string &meshName, const std::string &famName
)
{
    MEDFileUtilities::AutoFid fid(MEDCoupling::OpenMEDFileForRead(fileName));
    med_int nfam(MEDnFamily(fid, meshName.c_str()));
    ...
    {
        std::ostringstream oss;
        oss << "GetMeshGroupsNamesOnFamily : no such family \"" << famName << "\" in file \"" << fileName
            << "\" in mesh \"" << meshName << "\" !";
        throw INTERP_KERNEL::Exception(oss.str().c_str());
    }
    return ret;
...
std::vector<std::string>
MEDCoupling::GetMeshGroupsNames(const std::string &fileName, const std::string &meshName)
{
    MEDFileUtilities::AutoFid fid(MEDCoupling::OpenMEDFileForRead(fileName));
    med_int nfam = MEDnFamily(fid, meshName.c_str());
    std::vector<std::string> ret;
...
std::vector<MEDCoupling::TypeOfField>
MEDCoupling::GetTypesOfField(const std::string &fileName, const std::string &meshName, const std::string &fieldName)
{
    ...
    {
        std::ostringstream oss;
        oss << "GetTypesOfField : The field \"" << fieldName << "\" in file \"" << fileName
            << "\" is not lying on mesh \"" << meshName << "\"";
...
std::vector<std::string>
MEDCoupling::GetAllFieldNames(const std::string &fileName)
{
    std::vector<std::string> ret;
    MEDFileUtilities::AutoFid fid(MEDCoupling::OpenMEDFileForRead(fileName));
    med_int nbFields = MEDnField(fid);
...
std::vector<std::string>
MEDCoupling::GetAllFieldNamesOnMesh(const std::string &fileName, const std::string &meshName)
{
    std::vector<std::string> ret;
    MEDFileUtilities::AutoFid fid(MEDCoupling::OpenMEDFileForRead(fileName));
    med_int nbFields = MEDnField(fid);
...
std::vector<std::string>
MEDCoupling::GetFieldNamesOnMesh(
    MEDCoupling::TypeOfField type, const std::string &fileName, const std::string &meshName
)
{
...
std::vector<std::string>
MEDCoupling::GetCellFieldNamesOnMesh(const std::string &fileName, const std::string &meshName)
{
    std::vector<std::string> ret;
    MEDFileUtilities::AutoFid fid(MEDCoupling::OpenMEDFileForRead(fileName));
    med_int nbFields = MEDnField(fid);
...
std::vector<std::string>
MEDCoupling::GetNodeFieldNamesOnMesh(const std::string &fileName, const std::string &meshName)
{
    std::vector<std::string> ret;
    MEDFileUtilities::AutoFid fid(MEDCoupling::OpenMEDFileForRead(fileName));
    med_int nbFields = MEDnField(fid);
...
MEDCoupling::GetAllFieldIterations(const std::string &fileName, const std::string &fieldName)
{
    ...
    //
    std::ostringstream oss;
    oss << "GetAllFieldIterations : No field with name \"" << fieldName << "\" in file \"" << fileName
        << "\" ! Possible fields are : ";
    for (int i = 0; i < nbFields; i++)
    {
        med_int ncomp(MEDfieldnComponent(fid, i + 1));
...
double
MEDCoupling::GetTimeAttachedOnFieldIteration(
    const std::string &fileName, const std::string &fieldName, int iteration, int order
)
{
    MEDFileUtilities::AutoFid fid(MEDCoupling::OpenMEDFileForRead(fileName));
    med_int nbFields = MEDnField(fid);
    ...
    {
        std::ostringstream oss;
        oss << "No such field with name \"" << fieldName << "\" and iteration,order=(" << iteration << "," << order
...
MEDCoupling::GetFieldIterations(
...
std::vector<std::pair<int, int>>
MEDCoupling::GetCellFieldIterations(
    const std::string &fileName, const std::string &meshName, const std::string &fieldName
)
{
    ...
    //
    std::ostringstream oss;
    oss << "GetCellFieldIterations : No cell Field field with name \"" << fieldName << "\" in file \"" << fileName
        << "\" ! Possible fields are : ";
...
std::vector<std::pair<int, int>>
MEDCoupling::GetNodeFieldIterations(
    const std::string &fileName, const std::string &meshName, const std::string &fieldName
)
{
    std::string meshNameCpp(meshName);
    ...
    //
    std::ostringstream oss;
    oss << "GetNodeFieldIterations : No node Field field with name \"" << fieldName << "\" in file \"" << fileName
        << "\" ! Possible fields are : ";
    std::set<std::string> s2;
    for (int i = 0; i < nbFields; i++)
    {
...
MEDCoupling::ReadMeshFromFile(const std::string &fileName, const std::string &meshName, int meshDimRelToMax)
{
    MEDCoupling::CheckFileForRead(fileName);
    ...
    if (mmcPtr)
    {
        const MEDCouplingCMesh *ret(mmcPtr->getMesh());
        ret->incrRef();
    ...
    if (mmc2Ptr)
    {
        const MEDCouplingCurveLinearMesh *ret(mmc2Ptr->getMesh());
        ret->incrRef();
        return const_cast<MEDCouplingCurveLinearMesh *>(ret);
    }
    std::ostringstream oss;
    oss << "ReadMeshFromFile : The mesh \"" << meshName << "\" in file \"" << fileName
        << "\" has not a recognized type !";
    throw INTERP_KERNEL::Exception(oss.str().c_str());
}

MEDCoupling::MEDCouplingMesh *
MEDCoupling::ReadMeshFromFile(const std::string &fileName, int meshDimRelToMax)
{
    MEDCoupling::CheckFileForRead(fileName);
    ...
    std::ostringstream oss;
    oss << "ReadMeshFromFile (2) : The first mesh \"" << mm->getName() << "\" in file \"" << fileName
        << "\" has not a recognized type !";
    throw INTERP_KERNEL::Exception(oss.str().c_str());
}

MEDCoupling::MEDCouplingUMesh *
MEDCoupling::ReadUMeshFromFile(const std::string &fileName, const std::string &meshName, int meshDimRelToMax)
{
    MEDCoupling::CheckFileForRead(fileName);
    MCAuto<MEDFileMesh> mm(MEDFileMesh::New(fileName, meshName));
    MEDFileMesh *mmPtr(mm);
    ...
    {
        std::ostringstream oss;
        oss << "ReadUMeshFromFile : With fileName=\"" << fileName << "\", meshName=\"" << meshName
...
MEDCoupling::MEDCouplingUMesh *
MEDCoupling::ReadUMeshFromFile(const std::string &fileName, int meshDimRelToMax)
{
    MEDCoupling::CheckFileForRead(fileName);
    MCAuto<MEDFileMesh> mm(MEDFileMesh::New(fileName));
    MEDFileMesh *mmPtr(mm);
    ...
    {
        std::ostringstream oss;
        oss << "ReadUMeshFromFile : With fileName=\"" << fileName << "\", meshName (the first) =\"" << mm->getName()
            << "\" exists but it is not an unstructured mesh !";
        throw INTERP_KERNEL::Exception(oss.str().c_str());
    }
    return mmuPtr->getMeshAtLevel(meshDimRelToMax, true);
...
int
MEDCoupling::ReadUMeshDimFromFile(const std::string &fileName, const std::string &meshName)
{
    MEDCoupling::CheckFileForRead(fileName);
    std::vector<int> poss;
...
MEDCoupling::MEDCouplingUMesh *
MEDCoupling::ReadUMeshFromFamilies(
    const std::string &fileName, const std::string &meshName, int meshDimRelToMax, const std::vector<std::string> &fams
)
{
    MEDCoupling::CheckFileForRead(fileName);
    MCAuto<MEDFileMesh> mm(MEDFileMesh::New(fileName, meshName));
    ...
    {
        std::ostringstream oss;
        oss << "ReadUMeshFromFamilies : With fileName=\"" << fileName << "\", meshName (the first) =\"" << mm->getName()
            << "\" exists but it is not an unstructured mesh !";
        throw INTERP_KERNEL::Exception(oss.str().c_str());
    }
...
MEDCoupling::ReadUMeshFromGroups(
    const std::string &fileName, const std::string &meshName, int meshDimRelToMax, const std::vector<std::string> &grps
)
{
    MEDCoupling::CheckFileForRead(fileName);
    MCAuto<MEDFileMesh> mm = MEDFileMesh::New(fileName, meshName);
    ...
    {
        std::ostringstream oss;
        oss << "ReadUMeshFromGroups : With fileName=\"" << fileName << "\", meshName (the first) =\"" << mm->getName()
            << "\" exists but it is not an unstructured mesh !";
        throw INTERP_KERNEL::Exception(oss.str().c_str());
    }
...
MCAuto<MEDCoupling::MEDCouplingField>
MEDCoupling::ReadField(const std::string &fileName)
{
    ...
    if (sz > 1)
    {
        ...
        oss << "You are invited to use ReadField(fileName, fieldName) instead to avoid misleading concerning field you "
               "want to read !"
            << std::endl;
...
MCAuto<MEDCoupling::MEDCouplingField>
MEDCoupling::ReadField(const std::string &fileName, const std::string &fieldName)
{
    ...
    {
        std::ostringstream oss;
        oss << "In file \"" << fileName << "\" field \"" << fieldName << "\" exists but with more than one time steps !"
            << std::endl;
        oss << "You are invited to use ReadField(fileName, fieldName, iteration, order) instead to avoid misleading "
               "concerning time steps."
            << std::endl;
        oss << "For information, time steps available for field \"" << fieldName << "\" are :" << std::endl;
        for (std::vector<std::pair<std::pair<int, int>, double>>::const_iterator it = iterations.begin();
             it != iterations.end();
             it++)
...
MCAuto<MEDCoupling::MEDCouplingField>
MEDCoupling::ReadField(const std::string &fileName, const std::string &fieldName, int iteration, int order)
{
    ...
    throw INTERP_KERNEL::Exception(
        "MEDCoupling::ReadField : only FLOAT32, FLOAT64, INT32 and INT64 supported for the moment !"
    );
}

MCAuto<MEDCoupling::MEDCouplingField>
MEDCoupling::ReadField(
    MEDCoupling::TypeOfField type,
    const std::string &fileName,
    const std::string &meshName,
    int meshDimRelToMax,
...
)
{
    ...
    switch (type)
    {
        ...
        default:
            throw INTERP_KERNEL::Exception(
                "Type of field specified not managed ! manages are ON_NODES, ON_CELLS, ON_GAUSS_PT or ON_GAUSS_NE !"
            );
    }
}

std::vector<MEDCoupling::MEDCouplingFieldDouble *>
MEDCoupling::ReadFieldsOnSameMesh(
    MEDCoupling::TypeOfField type,
    const std::string &fileName,
    const std::string &meshName,
    int meshDimRelToMax,
...
std::vector<MEDCoupling::MEDCouplingFieldDouble *>
MEDCoupling::ReadFieldsCellOnSameMesh(
    const std::string &fileName,
    const std::string &meshName,
    int meshDimRelToMax,
    const std::string &fieldName,
...
std::vector<MEDCoupling::MEDCouplingFieldDouble *>
MEDCoupling::ReadFieldsNodeOnSameMesh(
    const std::string &fileName,
    const std::string &meshName,
    int meshDimRelToMax,
    const std::string &fieldName,
...
std::vector<MEDCoupling::MEDCouplingFieldDouble *>
MEDCoupling::ReadFieldsGaussOnSameMesh(
    const std::string &fileName,
    const std::string &meshName,
    int meshDimRelToMax,
    const std::string &fieldName,
...
std::vector<MEDCoupling::MEDCouplingFieldDouble *>
MEDCoupling::ReadFieldsGaussNEOnSameMesh(
    const std::string &fileName,
    const std::string &meshName,
    int meshDimRelToMax,
    const std::string &fieldName,
...
ReadFieldCellLikeT(
    typename MLFieldTraits<T>::F1TSType *ff,
    MEDCoupling::TypeOfField type,
    const std::string &fileName,
    const std::string &meshName,
    int meshDimRelToMax,
...
)
{
    MCAuto<MEDFileAnyTypeField1TS> f(MEDFileAnyTypeField1TS::New(fileName, fieldName, iteration, order));
    {
        ...
        if (f1.isNotNull())
        {
            MCAuto<MEDCoupling::MEDCouplingFieldDouble> ret(
                ReadFieldCellLikeT<double>(f1, type, fileName, meshName, meshDimRelToMax, fieldName, iteration, order)
            );
            return ret.retn();
        }
    }
    {
        ...
        if (f1.isNotNull())
        {
            MCAuto<MEDCoupling::MEDCouplingFieldInt32> ret(
                ReadFieldCellLikeT<int>(f1, type, fileName, meshName, meshDimRelToMax, fieldName, iteration, order)
            );
            return ret.retn();
        }
    }
    {
        ...
        if (f1.isNotNull())
        {
            MCAuto<MEDCoupling::MEDCouplingFieldInt64> ret(
                ReadFieldCellLikeT<Int64>(f1, type, fileName, meshName, meshDimRelToMax, fieldName, iteration, order)
            );
            return ret.retn();
        }
    }
    {
        ...
        if (f1.isNotNull())
        {
            MCAuto<MEDCoupling::MEDCouplingFieldFloat> ret(
                ReadFieldCellLikeT<float>(f1, type, fileName, meshName, meshDimRelToMax, fieldName, iteration, order)
            );
            return ret.retn();
        }
    }
    throw INTERP_KERNEL::Exception(
        "MEDCoupling::ReadFieldCell : only FLOAT32, FLOAT64, INT32 and INT64 supported for the moment !"
    );
}
...
)
{
    ...
    else
    {
        ...
        MCAuto<MEDCouplingUMesh> mzip(
            static_cast<MEDCouplingUMesh *>(m->buildPartAndReduceNodes(mp->begin(), mp->end(), arr2))
        );
        MCAuto<DataArrayIdType> arr2Safe(arr2);
        MCAuto<DataArrayIdType> arr3(arr2->invertArrayO2N2N2O(mzip->getNumberOfNodes()));
        MCAuto<DataArrayIdType> pflSorted(pflSafe->deepCopy());
        pflSorted->sort(true);
...
)
{
    MCAuto<MEDFileAnyTypeField1TS> f(MEDFileAnyTypeField1TS::New(fileName, fieldName, iteration, order));
    {
        ...
        {
            MCAuto<MEDCoupling::MEDCouplingFieldDouble> ret(
                ReadFieldNodeT<double>(f1, fileName, meshName, meshDimRelToMax, fieldName, iteration, order)
            );
            return ret.retn();
        }
    }
    {
        ...
        {
            MCAuto<MEDCoupling::MEDCouplingFieldInt32> ret(
                ReadFieldNodeT<int>(f1, fileName, meshName, meshDimRelToMax, fieldName, iteration, order)
            );
            return ret.retn();
        }
    }
    {
        ...
        {
            MCAuto<MEDCoupling::MEDCouplingFieldInt64> ret(
                ReadFieldNodeT<Int64>(f1, fileName, meshName, meshDimRelToMax, fieldName, iteration, order)
            );
            return ret.retn();
        }
    }
    {
        ...
        {
            MCAuto<MEDCoupling::MEDCouplingFieldFloat> ret(
                ReadFieldNodeT<float>(f1, fileName, meshName, meshDimRelToMax, fieldName, iteration, order)
            );
            return ret.retn();
        }
    }
    throw INTERP_KERNEL::Exception(
        "MEDCoupling::ReadFieldNode : only FLOAT32, FLOAT64, INT32 and INT64 supported for the moment !"
    );
}

MEDCoupling::MEDCouplingField *
MEDCoupling::ReadFieldGauss(
    const std::string &fileName,
    const std::string &meshName,
    int meshDimRelToMax,
    const std::string &fieldName,
...
void
MEDCoupling::WriteMesh(const std::string &fileName, const MEDCoupling::MEDCouplingMesh *mesh, bool writeFromScratch)
{
    if (!mesh)
    ...
    throw INTERP_KERNEL::Exception(
        "WriteMesh : only MEDCouplingUMesh, MEDCoupling1GTUMesh, MEDCouplingCMesh, MEDCouplingCurveLinear are dealed "
        "in this API for the moment !"
    );
}

void
MEDCoupling::WriteUMesh(const std::string &fileName, const MEDCoupling::MEDCouplingUMesh *mesh, bool writeFromScratch)
{
    if (!mesh)
...
void
MEDCoupling::WriteUMeshDep(
    const std::string &fileName, const MEDCoupling::MEDCouplingUMesh *mesh, bool writeFromScratch
)
{
...
)
{
    ...
    if (meshName.empty())
        throw INTERP_KERNEL::Exception(
            "Trying to write a unstructured mesh with no name ! MED file format needs a not empty mesh name : change "
            "2nd parameter !"
        );
    ...
    {
        std::ostringstream oss;
        oss << "File with name \'" << fileName << "\' has not valid permissions !";
        throw INTERP_KERNEL::Exception(oss.str().c_str());
    }
    MCAuto<MEDFileUMesh> m(MEDFileUMesh::New());
    AssignStaticWritePropertiesTo(*m);
...
void
MEDCoupling::WriteUMeshesPartitionDep(
    const std::string &fileName,
    const std::string &meshNameC,
    const std::vector<const MEDCoupling::MEDCouplingUMesh *> &meshes,
    bool writeFromScratch
...
MEDCoupling::WriteUMeshes(
    const std::string &fileName, const std::vector<const MEDCoupling::MEDCouplingUMesh *> &meshes, bool writeFromScratch
)
{
    int mod(writeFromScratch ? 2 : 0);
    MCAuto<MEDFileUMesh> m(MEDFileUMesh::New());
...
template <class T>
void
MEDLoaderNS::writeFieldWithoutReadingAndMappingOfMeshInFile(
...
)
{
    ...
    else if (um4)
    {
        ...
        throw INTERP_KERNEL::Exception(
            "MEDLoaderNS::writeFieldWithoutReadingAndMappingOfMeshInFile : only MEDCouplingUMesh, MEDCoupling1GTUMesh, "
            "MEDCouplingCMesh, MEDCouplingCurveLinear are dealed in this API for the moment !"
        );
    ff->write(fileName, 0);
}
...
template <class T>
void
WriteFieldT(const std::string &fileName, const typename MEDCoupling::Traits<T>::FieldType *f, bool writeFromScratch)
{
    if (!f)
    ...
    else
    {
        ...
        else
        {
            ...
            if (f2->getTypeOfField() != ON_NODES)
            {
                ...
                {
                    std::ostringstream oss;
                    oss << "WriteField : The file \"" << fileName << "\" already contains a mesh named \""
                        << f->getMesh()->getName()
                        << "\" and this mesh in the file is not compatible (a subpart) with the mesh you intend to "
                           "write ! This is maybe due to a too strict policy ! Try with to lease it by calling "
            ...
            else
            {
                ...
                if (!b)
                {
                    ...
                    oss << "WriteField : The file \"" << fileName << "\" already contains a mesh named \""
                        << f->getMesh()->getName()
                        << "\" and this mesh in the file is not compatible (a subpart regarding nodes) with the mesh "
                           "you intend to write ! This is maybe due to a too strict epsilon ! Try with to lease it by "
                           "calling SetEpsilonForNodeComp !";
                    throw INTERP_KERNEL::Exception(oss.str().c_str());
                }
                MCAuto<typename MLFieldTraits<T>::F1TSType> f1ts(MLFieldTraits<T>::F1TSType::New());
                AssignStaticWritePropertiesTo(*f1ts);
...
void
MEDCoupling::WriteField(const std::string &fileName, const MEDCoupling::MEDCouplingField *f, bool writeFromScratch)
{
    if (!f)
        throw INTERP_KERNEL::Exception("WriteField : input field is null !");
    {
...
void
MEDCoupling::WriteFieldDep(const std::string &fileName, const MEDCoupling::MEDCouplingField *f, bool writeFromScratch)
{
...
template <class T>
void
WriteFieldUsingAlreadyWrittenMeshT(const std::string &fileName, const typename MEDCoupling::Traits<T>::FieldType *f)
{
    if (!f)
    ...
    {
        std::ostringstream oss;
        oss << "File with name \'" << fileName << "\' has not valid permissions or not exists !";
        throw INTERP_KERNEL::Exception(oss.str().c_str());
    }
    MCAuto<typename MLFieldTraits<T>::F1TSType> f1ts(MLFieldTraits<T>::F1TSType::New());
    AssignStaticWritePropertiesTo(*f1ts);
...
void
MEDCoupling::WriteFieldUsingAlreadyWrittenMesh(const std::string &fileName, const MEDCoupling::MEDCouplingField *f)
{
    if (!f)
    ...
    throw INTERP_KERNEL::Exception(
        "WriteFieldUsingAlreadyWrittenMesh : input field is not in FLOAT32, FLOAT64, INT32 and INT64 !"
    );
}
