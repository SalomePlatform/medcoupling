...
std::ostream &
operator<<(std::ostream &os, const Cell &ma);
...
struct DoubleField
{
    ...
    DoubleField(int nb_sub, int total_nb_comp) : _sub(nb_sub), _group(NULL), _curMedField(NULL)
    {
        _comp_values.reserve(total_nb_comp);
    }
    ~DoubleField();
   ...
   public:
    Node *getNode(const TID nID)
    {
        ...
        if (_nodes.size() < chunkID + 1)
        {
            ...
            for (size_t i = 0; i < _nodes.size(); ++i) newNodes[i].swap(_nodes[i]);
            for (size_t i = _nodes.size(); i < newNodes.size(); ++i) newNodes[i].resize(chunkSize);
