...
MEDFileParameter1TS::MEDFileParameter1TS(int iteration, int order, double time)
    : _iteration(iteration), _order(order), _time(time)
{
}
...
bool
MEDFileParameter1TS::isEqual(const MEDFileParameter1TS *other, double eps, std::string &what) const
{
    ...
    {
        oss << "iteration number mismatches " << _iteration << " != " << other->_iteration;
        what = oss.str();
        return false;
    }
    if (_order != other->_order)
    {
        oss << "order number mismatches " << _iteration << " != " << other->_iteration;
        what = oss.str();
        return false;
    }
    if (fabs(_time - other->_time) > eps)
    {
        oss << "time mismatches " << _time << " != " << other->_time << " eps=" << eps;
        what = oss.str();
        return false;
    }
    return true;
...
MEDFileParameter1TS *
MEDFileParameterDouble1TSWTI::deepCopy() const
{
    return new MEDFileParameterDouble1TSWTI(*this);
}

bool
MEDFileParameterDouble1TSWTI::isEqual(const MEDFileParameter1TS *other, double eps, std::string &what) const
{
    ...
    if (!otherC)
    {
        what = "IsEqual fails because this is double parameter other no !";
        return false;
    }
    if (fabs(_arr - otherC->_arr) > eps)
    {
        std::ostringstream oss;
        oss << "value differ " << _arr << " != " << otherC->_arr << " (eps=" << eps << ")";
        return false;
    }
    return true;
}
...
std::size_t
MEDFileParameterDouble1TSWTI::getHeapMemorySizeWithoutChildren() const
{
    return sizeof(MEDFileParameterDouble1TSWTI);
}

std::vector<const BigMemoryObject *>
MEDFileParameterDouble1TSWTI::getDirectChildrenWithNull() const
{
    return std::vector<const BigMemoryObject *>();
}

std::string
MEDFileParameterDouble1TSWTI::simpleRepr() const
{
    std::ostringstream oss;
    simpleRepr2(0, oss);
    return oss.str();
...
void
MEDFileParameterDouble1TSWTI::simpleRepr2(int bkOffset, std::ostream &oss) const
{
    std::string startOfLine(bkOffset, ' ');
    oss << startOfLine << "ParameterDoubleItem with (iteration,order) = (" << _iteration << "," << _order << ")"
        << std::endl;
...
MEDFileParameterDouble1TSWTI::New(int iteration, int order, double time)
{
...
MEDFileParameterDouble1TSWTI::MEDFileParameterDouble1TSWTI(int iteration, int order, double time)
    : MEDFileParameter1TS(iteration, order, time)
{
}

void
MEDFileParameterDouble1TSWTI::finishLoading(med_idt fid, const std::string &name, int dt, int it, int nbOfSteps)
{
    std::ostringstream oss;
    oss << "MEDFileParameterDouble1TS::finishLoading : no specified time step (" << dt << "," << it
    ...
    for (int i = 0; i < nbOfSteps; i++)
    {
        ...
        {
            _iteration = FromMedInt<int>(locDt);
            _order = FromMedInt<int>(locIt);
            _time = tim;
            MEDFILESAFECALLERRD0(
                MEDparameterValueRd,
                (fid, name.c_str(), _iteration, _order, reinterpret_cast<unsigned char *const>(&_arr))
            );
            return;
        }
        else
...
void
MEDFileParameterDouble1TSWTI::readValue(med_idt fid, const std::string &name)
{
    MEDFILESAFECALLERRD0(
...
void
MEDFileParameterDouble1TSWTI::finishLoading(med_idt fid, const std::string &name, int timeStepId)
{
    med_int locDt, locIt;
    double dt;
...
void
MEDFileParameterDouble1TSWTI::writeAdvanced(med_idt fid, const std::string &name, const MEDFileWritable &mw) const
{
...
std::size_t
MEDFileParameterTinyInfo::getHeapMemSizeOfStrings() const
{
    return _dt_unit.capacity() + _name.capacity() + _desc_name.capacity();
}

bool
MEDFileParameterTinyInfo::isEqualStrings(const MEDFileParameterTinyInfo &other, std::string &what) const
{
    std::ostringstream oss;
    if (_name != other._name)
    {
        oss << "name differ ! this=" << _name << " and other=" << other._name;
        what = oss.str();
        return false;
    }
    if (_desc_name != other._desc_name)
    {
        oss << "name differ ! this=" << _desc_name << " and other=" << other._desc_name;
        what = oss.str();
        return false;
    }
    if (_dt_unit != other._dt_unit)
    {
        oss << "unit of time differ ! this=" << _dt_unit << " and other=" << other._dt_unit;
        what = oss.str();
        return false;
    }
    return true;
...
void
MEDFileParameterTinyInfo::writeLLHeader(med_idt fid, med_parameter_type typ) const
{
...
void
MEDFileParameterTinyInfo::mainRepr(int bkOffset, std::ostream &oss) const
{
    std::string startOfLine(bkOffset, ' ');
...
MEDFileParameterDouble1TS::New()
{
    return new MEDFileParameterDouble1TS;
}

MEDFileParameterDouble1TS *
MEDFileParameterDouble1TS::New(const std::string &fileName)
{
    return new MEDFileParameterDouble1TS(fileName);
}

MEDFileParameterDouble1TS *
MEDFileParameterDouble1TS::New(const std::string &fileName, const std::string &paramName)
{
    return new MEDFileParameterDouble1TS(fileName, paramName);
}

MEDFileParameterDouble1TS *
MEDFileParameterDouble1TS::New(const std::string &fileName, const std::string &paramName, int dt, int it)
{
...
MEDFileParameterDouble1TS::MEDFileParameterDouble1TS(
    const std::string &fileName, const std::string &paramName, int dt, int it
)
{
    ...
    std::ostringstream oss;
    oss << "MEDFileParameterDouble1TS : no double param name \"" << paramName
        << "\" ! Double Parameters available are : ";
    ...
    for (int i = 0; i < nbPar; i++)
    {
        ...
        if (paramNameCpp == paramName && paramType == MED_FLOAT64)
        {
            ...
            else
            {
                ...
                oss2 << "Param name \"" << paramName << "\" exists but no time steps on it !";
...
MEDFileParameterDouble1TS::MEDFileParameterDouble1TS(const std::string &fileName)
{
    ...
    {
        std::ostringstream oss2;
        oss2 << "No parameter in file \"" << fileName << "\" !";
        throw INTERP_KERNEL::Exception(oss2.str().c_str());
    }
    INTERP_KERNEL::AutoPtr<char> pName = MEDLoaderBase::buildEmptyString(MED_NAME_SIZE);
    INTERP_KERNEL::AutoPtr<char> descName = MEDLoaderBase::buildEmptyString(MED_COMMENT_SIZE);
    ...
    if (paramType == MED_FLOAT64)
    {
        ...
        {
            std::ostringstream oss2;
            oss2 << "Double param name \"" << paramNameCpp << "\" exists in file \"" << fileName
                 << "\"but no time steps on it !";
            throw INTERP_KERNEL::Exception(oss2.str().c_str());
        }
    }
    else
    {
        std::ostringstream oss2;
        oss2 << "First parameter in file \"" << fileName << "\" is not double !";
        throw INTERP_KERNEL::Exception(oss2.str().c_str());
    }
}

bool
MEDFileParameterDouble1TS::isEqual(const MEDFileParameter1TS *other, double eps, std::string &what) const
{
    ...
    if (!otherC)
    {
        what = "Other is not of type MEDFileParameterDouble1TS as this";
        return false;
    }
...
MEDFileParameter1TS *
MEDFileParameterDouble1TS::deepCopy() const
{
    return new MEDFileParameterDouble1TS(*this);
}

std::string
MEDFileParameterDouble1TS::simpleRepr() const
{
    std::ostringstream oss;
...
std::size_t
MEDFileParameterDouble1TS::getHeapMemorySizeWithoutChildren() const
{
    return getHeapMemSizeOfStrings() + sizeof(MEDFileParameterDouble1TS);
}

std::vector<const BigMemoryObject *>
MEDFileParameterDouble1TS::getDirectChildrenWithNull() const
{
    return std::vector<const BigMemoryObject *>();
}

void
MEDFileParameterDouble1TS::write(const std::string &fileName, int mode) const
{
...
MEDFileParameterMultiTS::New()
{
    return new MEDFileParameterMultiTS;
}

MEDFileParameterMultiTS *
MEDFileParameterMultiTS::New(const std::string &fileName)
{
...
MEDFileParameterMultiTS *
MEDFileParameterMultiTS::New(med_idt fid)
{
    return new MEDFileParameterMultiTS(fid);
}

MEDFileParameterMultiTS *
MEDFileParameterMultiTS::New(const std::string &fileName, const std::string &paramName)
{
    MEDFileUtilities::AutoFid fid(OpenMEDFileForRead(fileName));
...
MEDFileParameterMultiTS *
MEDFileParameterMultiTS::New(med_idt fid, const std::string &paramName)
{
...
MEDFileParameterMultiTS::MEDFileParameterMultiTS(const MEDFileParameterMultiTS &other, bool deepCopy)
    : MEDFileParameterTinyInfo(other), _param_per_ts(other._param_per_ts)
{
    if (deepCopy)
...
MEDFileParameterMultiTS::MEDFileParameterMultiTS(med_idt fid)
{
    ...
    {
        std::ostringstream oss;
        oss << "MEDFileParameterMultiTS : no parameter in file \"" << FileNameFromFID(fid) << "\" !";
        throw INTERP_KERNEL::Exception(oss.str().c_str());
    }
    INTERP_KERNEL::AutoPtr<char> pName(MEDLoaderBase::buildEmptyString(MED_NAME_SIZE));
    INTERP_KERNEL::AutoPtr<char> descName(MEDLoaderBase::buildEmptyString(MED_COMMENT_SIZE));
...
void
MEDFileParameterMultiTS::finishLoading(med_idt fid, med_parameter_type typ, int nbOfSteps)
{
...
std::size_t
MEDFileParameterMultiTS::getHeapMemorySizeWithoutChildren() const
{
...
std::vector<const BigMemoryObject *>
MEDFileParameterMultiTS::getDirectChildrenWithNull() const
{
    std::vector<const BigMemoryObject *> ret;
    for (std::vector<MCAuto<MEDFileParameter1TS> >::const_iterator it = _param_per_ts.begin();
         it != _param_per_ts.end();
         it++)
...
bool
MEDFileParameterMultiTS::isEqual(const MEDFileParameterMultiTS *other, double eps, std::string &what) const
{
    if (!other)
    {
        what = "other is null !";
    ...
    {
        what = "number of time steps differs !";
        return false;
    }
    std::ostringstream oss;
    for (std::size_t i = 0; i < _param_per_ts.size(); i++)
    {
        ...
        if ((a && !b) || (!a && b))
        {
            oss << "At time step id #" << i << " pointer is defined on one side not in the other !";
            what = oss.str();
            return false;
        }
        if (a)
            ...
            {
                oss << " At time step id #" << i << " non equality !";
                what += oss.str();
                return false;
            }
    }
...
void
MEDFileParameterMultiTS::write(const std::string &fileName, int mode) const
{
    med_access_mode medmod = MEDFileUtilities::TraduceWriteMode(mode);
    MEDFileUtilities::AutoFid fid = MEDfileOpen(fileName.c_str(), medmod);
...
void
MEDFileParameterMultiTS::writeAdvanced(med_idt fid, const MEDFileWritable &mw) const
{
    ...
    if (diffType.size() > 1)
        throw INTERP_KERNEL::Exception(
            "MEDFileParameterMultiTS::writeAdvanced : impossible to mix type of data in parameters in MED file ! Only "
            "float64 or only int32 ..."
        );
    if (diffType.empty())
        return;
...
std::string
MEDFileParameterMultiTS::simpleRepr() const
{
    std::ostringstream oss;
    simpleRepr2(0, oss);
    return oss.str();
...
void
MEDFileParameterMultiTS::simpleRepr2(int bkOffset, std::ostream &oss) const
{
...
void
MEDFileParameterMultiTS::appendValue(int dt, int it, double time, double val)
{
    MCAuto<MEDFileParameterDouble1TSWTI> elt = MEDFileParameterDouble1TSWTI::New(dt, it, time);
    elt->setValue(val);
    MCAuto<MEDFileParameter1TS> elt2((MEDFileParameterDouble1TSWTI *)elt);
    elt2->incrRef();
    _param_per_ts.push_back(elt2);
}

double
MEDFileParameterMultiTS::getDoubleValue(int iteration, int order) const
{
    ...
    {
        std::ostringstream oss;
        oss << "MEDFileParameterMultiTS::getDoubleValue : time iteration it=" << iteration << " order=" << order;
...
int
MEDFileParameterMultiTS::getPosOfTimeStep(int iteration, int order) const
{
    int ret = 0;
    std::ostringstream oss;
    oss << "MEDFileParameterMultiTS::getPosOfTimeStep : no such iteration=" << iteration << " order=" << order
        << " ! Possibilities are :";
    for (std::vector<MCAuto<MEDFileParameter1TS> >::const_iterator it = _param_per_ts.begin();
         it != _param_per_ts.end();
         it++, ret++)
...
int
MEDFileParameterMultiTS::getPosGivenTime(double time, double eps) const
{
    int ret = 0;
    std::ostringstream oss;
    oss << "MEDFileParameterMultiTS::getPosGivenTime : no such time=" << time << " ! Possibilities are :";
    for (std::vector<MCAuto<MEDFileParameter1TS> >::const_iterator it = _param_per_ts.begin();
         it != _param_per_ts.end();
         it++, ret++)
    {
 ...
 */
MEDFileParameter1TS *
MEDFileParameterMultiTS::getTimeStepAtPos(int posId) const
{
    if (posId < 0 || posId >= (int)_param_per_ts.size())
    {
        std::ostringstream oss;
        oss << "MEDFileParameterMultiTS::getTimeStepAtPos : invalid pos ! Should be in [0," << _param_per_ts.size()
            << ") !";
        throw INTERP_KERNEL::Exception(oss.str().c_str());
    }
...
void
MEDFileParameterMultiTS::eraseTimeStepIds(const int *startIds, const int *endIds)
{
    ...
    for (const int *w = startIds; w != endIds; w++)
        ...
        {
            std::ostringstream oss;
            oss << "MEDFileParameterMultiTS::eraseTimeStepIds : At pos #" << std::distance(startIds, w) << " value is "
                << *w << " should be in [0," << len << ") !";
            throw INTERP_KERNEL::Exception(oss.str().c_str());
        }
...
int
MEDFileParameterMultiTS::getNumberOfTS() const
{
    return (int)getIterations().size();
}

std::vector<std::pair<int, int> >
MEDFileParameterMultiTS::getIterations() const
{
 ...
 */
std::vector<std::pair<int, int> >
MEDFileParameterMultiTS::getTimeSteps(std::vector<double> &ret1) const
{
...
MEDFileParameters *
MEDFileParameters::New()
{
    return new MEDFileParameters;
}

MEDFileParameters *
MEDFileParameters::New(const std::string &fileName)
{
    MEDFileUtilities::AutoFid fid(OpenMEDFileForRead(fileName));
    return New(fid);
}
...
MEDFileParameters *
MEDFileParameters::New(med_idt fid)
{
...
std::size_t
MEDFileParameters::getHeapMemorySizeWithoutChildren() const
{
...
std::vector<const BigMemoryObject *>
MEDFileParameters::getDirectChildrenWithNull() const
{
    std::vector<const BigMemoryObject *> ret;
...
bool
MEDFileParameters::isEqual(const MEDFileParameters *other, double eps, std::string &what) const
{
    if (!other)
    {
        what = "other is null !";
    ...
    {
        what = "number of parameters differs !";
        return false;
    }
    std::ostringstream oss;
    for (std::size_t i = 0; i < _params.size(); i++)
    {
        ...
        if ((a && !b) || (!a && b))
        {
            oss << "At param with id #" << i << " pointer is defined on one side not in the other !";
            what = oss.str();
            return false;
        }
        if (a)
            if (!a->isEqual(b, eps, what))
            {
                oss << " At param with id #" << i << " non equality !";
                what += oss.str();
                return false;
            }
    }
...
MEDFileParameters::MEDFileParameters(const MEDFileParameters &other, bool deepCopy)
    : MEDFileWritableStandAlone(other), _params(other._params)
{
    if (deepCopy)
        for (std::size_t i = 0; i < _params.size(); i++)
        {
...
void
MEDFileParameters::writeLL(med_idt fid) const
{
    for (std::vector<MCAuto<MEDFileParameterMultiTS> >::const_iterator it = _params.begin(); it != _params.end(); it++)
    {
...
MEDFileParameters::getParamsNames() const
{
    std::vector<std::string> ret(_params.size());
    int i = 0;
    for (std::vector<MCAuto<MEDFileParameterMultiTS> >::const_iterator it = _params.begin(); it != _params.end();
         it++, i++)
    {
        ...
        {
            std::ostringstream oss;
            oss << "MEDFileParameters::getParamsNames : At rank #" << i << " param is not defined !";
            throw INTERP_KERNEL::Exception(oss.str().c_str());
        }
    }
    return ret;
...
std::string
MEDFileParameters::simpleRepr() const
{
    std::ostringstream oss;
...
void
MEDFileParameters::simpleReprWithoutHeader(std::ostream &oss) const
{
    for (std::vector<MCAuto<MEDFileParameterMultiTS> >::const_iterator it = _params.begin(); it != _params.end(); it++)
    {
        const MEDFileParameterMultiTS *elt(*it);
...
void
MEDFileParameters::resize(int newSize)
{
    if (newSize < 0)
...
void
MEDFileParameters::pushParam(MEDFileParameterMultiTS *param)
{
    if (param)
        param->incrRef();
...
void
MEDFileParameters::setParamAtPos(int i, MEDFileParameterMultiTS *param)
{
    if (i < 0)
 ...
 */
MEDFileParameterMultiTS *
MEDFileParameters::getParamAtPos(int i) const
{
    if (i < 0 || i >= (int)_params.size())
    {
        std::ostringstream oss;
        oss << "MEDFileParameters::getParamAtPos : should be in [0," << _params.size() << ") !";
        throw INTERP_KERNEL::Exception(oss.str().c_str());
    }
    const MEDFileParameterMultiTS *elt = _params[i];
 ...
 */
MEDFileParameterMultiTS *
MEDFileParameters::getParamWithName(const std::string &paramName) const
{
...
void
MEDFileParameters::destroyParamAtPos(int i)
{
    if (i < 0 || i >= (int)_params.size())
    {
        std::ostringstream oss;
        oss << "MEDFileParameters::destroyParamAtPos : should be in [0," << _params.size() << ") !";
        throw INTERP_KERNEL::Exception(oss.str().c_str());
    }
...
int
MEDFileParameters::getPosFromParamName(const std::string &paramName) const
{
    std::ostringstream oss;
    oss << "MEDFileParameters::getPosFromParamName : no such name=" << paramName << " ! Possibilities are :";
    int ret = 0;
    for (std::vector<MCAuto<MEDFileParameterMultiTS> >::const_iterator it = _params.begin(); it != _params.end();
         it++, ret++)
    {
...
int
MEDFileParameters::getNumberOfParams() const
{
