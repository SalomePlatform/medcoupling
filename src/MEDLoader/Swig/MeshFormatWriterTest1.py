...
class MeshFormatWriterTest2(unittest.TestCase):
    def createMesh(self, nb_seg):
        ...
        box_sizes = [
            size_coords[1] - size_coords[0] for size_coords in self.bounding_box_coords
        ]
    ...
    def createField(self):
        ...
        size_min = 1e-4
        size_max = 1e-3
        coords_z_normed.applyFuncOnThis(
            "(%f-%f)*tanh(x)/tanh(1)+%f" % (size_max, size_min, size_min)
        )
    ...
    def compareFamilies(self, levels):
        ...
        for level in levels:
            ...
            referenceFamilyIds = (
                meshRef.getFamilyFieldAtLevel(level).getDifferentValues().getValues()
            )
            print("referenceFamilyIds; ", referenceFamilyIds)
            readFamilyIds = (
                meshRead.getFamilyFieldAtLevel(level).getDifferentValues().getValues()
            )
