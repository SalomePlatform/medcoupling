...
def _buildUmesh(coords, elements, title, groupesNo, groupesMa, mapGeoCellName ):
    ...
    allDims = set([ ml.MEDCouplingUMesh.GetDimensionOfGeometricType(_getGeoTypeDict()[elt]) for elt in elements ] )

    # Get max dimension
    maxDim = max([ml.MEDCouplingUMesh.GetDimensionOfGeometricType(_getGeoTypeDict()[elt]) for elt in elements])
    ...
    for dim in reversed( list(allDims) ):
        ...
        gts = [(elt,_getGeoTypeDict()[elt]) for elt in elements if ml.MEDCouplingUMesh.GetDimensionOfGeometricType(_getGeoTypeDict()[elt])==dim]
        ms = []
        offset = 0
    ...
    for gtMail in mapIdNameCell:
        for cellName, medId in mapIdNameCell[gtMail]:
            dimPerCell[ cellName ] = ml.MEDCouplingUMesh.GetDimensionOfGeometricType( _getGeoTypeDict()[gtMail] )
    ...
    for cellGrp in groupesMa:
        ...
        for dimOfCellGrp in dimsOfCellGrp:
            idsOfCellsWithDim = ml.DataArrayInt(dimPerCellVect).findIdsEqual( dimOfCellGrp )
...
def _parseMeshFile(fichier):
    ...
    for line in lines:
        ...
        if mode == "title":
            ...
            lineNum = _parseElementsLine(line, lines, currentElemType, elements, currentElemList, lineNum)  # Parse element connectivity
        elif mode == "groupNo" or mode == "groupMa":
            groupName, currentGroup = _parseGroupLine(line, groupName, currentGroup, lineNum)  # Parse group definitions
...
def _is_mode_switch(line):
    ...
    return (line.startswith("TITRE") or line.startswith("COOR_") or
            line in _getGeoTypeDict() or line.startswith("GROUP_NO") or line.startswith("GROUP_MA"))
...
def _parseElementsLine(line, lines, elementType, elements, cell_names,lineNum):
    ...
    name = parts[0]
    conn = parts[1:]
    nodes_needed = ml.MEDCouplingUMesh.GetNumberOfNodesOfGeometricType(_getGeoTypeDict()[elementType])
...
def _parseGroupLine(line, groupName, groupData, lineNum):
    ...
    if groupName == "GROUP_NO" or groupName == "GROUP_MA":
        ...
        if not re.search(pat, groupName):
            raise ValueError(f" Group name is missing or badly formatted on line {lineNum - 1}")
...
def LoadMailFileInMEDFileUMeshInstance( inputMailFilePath : str ):
    ...
    coords, elements, title, groupesNo, groupesMa, mapGeoCellName = _parseMeshFile(inputMailFilePath)
