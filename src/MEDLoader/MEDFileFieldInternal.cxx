...
MEDFileGTKeeper *
MEDFileGTKeeperSta::deepCopy() const
{
    return new MEDFileGTKeeperSta(_geo_type);
}

INTERP_KERNEL::NormalizedCellType
MEDFileGTKeeperSta::getGeoType() const
{
    return _geo_type;
}

std::string
MEDFileGTKeeperSta::getRepr() const
{
    return INTERP_KERNEL::CellModel::GetCellModel(_geo_type).getRepr();
}

bool
MEDFileGTKeeperSta::isEqual(const MEDFileGTKeeper *other) const
{
...
MEDFileGTKeeperDyn::MEDFileGTKeeperDyn(
    const MEDFileUMesh *mesh, const MEDFileUMesh *section, const MEDFileStructureElement *se
)
    : _mesh(mesh), _section(section), _se(se)
{
    if (mesh)
        mesh->incrRef();
    if (section)
...
bool
MEDFileGTKeeperDyn::isEqual(const MEDFileGTKeeper *other) const
{
...
MEDFileFieldLoc *
MEDFileFieldLoc::New(med_idt fid, const std::string &locName)
{
    return new MEDFileFieldLoc(fid, locName);
}

MEDFileFieldLoc *
MEDFileFieldLoc::New(med_idt fid, int id, const MEDFileEntities *entities)
{
...
MEDFileFieldLoc::MEDFileFieldLoc(med_idt fid, const std::string &locName) : _name(locName)
{
    ...
    _gt = new MEDFileGTKeeperSta((INTERP_KERNEL::NormalizedCellType)(std::distance(
        typmai3, std::find(typmai3, typmai3 + INTERP_KERNEL::NORM_MAXTYPE, geotype)
    )));
...
MEDFileFieldLoc::MEDFileFieldLoc(med_idt fid, int id, const MEDFileEntities *entities)
{
    ...
    {
        const MEDFileAllStaticEntitiesPlusDyn *entities2(
            dynamic_cast<const MEDFileAllStaticEntitiesPlusDyn *>(entities)
        );
        if (!entities2)
        {
            std::ostringstream oss;
            oss << "MEDFileFieldLoc cstr : for loc \"" << _name << "\" presence of non static type ! Expect entities !";
...
MEDFileFieldLoc::MEDFileFieldLoc(
    ...
    : _gt(new MEDFileGTKeeperSta(geoType)), _name(locName), _ref_coo(refCoo), _gs_coo(gsCoo), _w(w)
{
    const INTERP_KERNEL::CellModel &cm(INTERP_KERNEL::CellModel::GetCellModel(getGeoType()));
...
MEDFileFieldLoc::MEDFileFieldLoc(const MEDFileFieldLoc &other)
    : _dim(other._dim),
      _nb_gauss_pt(other._nb_gauss_pt),
      _gt(other._gt->deepCopy()),
      _nb_node_per_cell(other._nb_node_per_cell),
      _name(other._name),
      _ref_coo(other._ref_coo),
      _gs_coo(other._gs_coo),
      _w(other._w)
{
}
...
bool
MEDFileFieldLoc::isOnStructureElement() const
{
...
std::size_t
MEDFileFieldLoc::getHeapMemorySizeWithoutChildren() const
{
    return (_ref_coo.capacity() + _gs_coo.capacity() + _w.capacity()) * sizeof(double) + _name.capacity();
}

std::vector<const BigMemoryObject *>
MEDFileFieldLoc::getDirectChildrenWithNull() const
{
    return std::vector<const BigMemoryObject *>();
}

void
MEDFileFieldLoc::simpleRepr(std::ostream &oss) const
{
    ...
    oss << OFF7;
    oss << "Weights=";
    std::copy(_w.begin(), _w.end(), std::ostream_iterator<double>(oss, " "));
    oss << OFF7;
    oss << "GaussPtsCoords=";
...
void
MEDFileFieldLoc::setName(const std::string &name)
{
    _name = name;
}

bool
MEDFileFieldLoc::isEqual(const MEDFileFieldLoc &other, double eps) const
{
...
void
MEDFileFieldLoc::writeLL(med_idt fid) const
{
    MEDFILESAFECALLERWR0(
        MEDlocalizationWr,
        (fid,
         _name.c_str(),
         typmai3[(int)getGeoType()],
         _dim,
         &_ref_coo[0],
         MED_FULL_INTERLACE,
         _nb_gauss_pt,
...
MEDFileFieldLoc::repr() const
{
    ...
    if (sz % _dim == 0)
    {
        ...
        std::copy(_gs_coo.begin(), _gs_coo.end(), std::ostream_iterator<double>(oss, " "));
    oss << "\n  - Weights of Gauss coords are : ";
    std::copy(_w.begin(), _w.end(), std::ostream_iterator<double>(oss, " "));
    return oss.str();
}

void
MEDFileFieldPerMeshPerTypePerDisc::assignFieldNoProfile(
    mcIdType &start,
    mcIdType offset,
    mcIdType nbOfCells,
    const MEDCouplingFieldTemplate *field,
    const DataArray *arrr,
...
)
{
    ...
    switch (_type)
    {
        ...
        case ON_GAUSS_NE:
        {
            ...
            getOrCreateAndGetArray()->setContigPartOfSelectedValuesSlice(
                _start, arrr, arrPtr[offset], arrPtr[offset + nbOfCells], 1
            );
        ...
        case ON_GAUSS_PT:
        {
            ...
            const MEDCouplingFieldDiscretizationGauss *disc2(
                dynamic_cast<const MEDCouplingFieldDiscretizationGauss *>(disc)
            );
            if (!disc2)
            ...
            {  // profile : for gauss even in NoProfile !!!
                std::ostringstream oss;
                oss << "Pfl_" << nasc.getName() << "_" << INTERP_KERNEL::CellModel::GetCellModel(getGeoType()).getRepr()
                    << "_" << _loc_id;
                _profile = oss.str();
            ...
            for (mcIdType i = 0; i < _nval; i++)
            {
                ...
                for (mcIdType j = 0; j < nbi; j++) *da4Ptr++ = ref + j;
            }
            std::ostringstream oss2;
            oss2 << "Loc_" << nasc.getName() << "_" << INTERP_KERNEL::CellModel::GetCellModel(getGeoType()).getRepr()
                 << "_" << _loc_id;
            _localization = oss2.str();
            getOrCreateAndGetArray()->setContigPartOfSelectedValues(_start, arrr, da4);
            _end = _start + _nval * nbi;
            glob.appendLoc(
                _localization.c_str(), getGeoType(), gsLoc.getRefCoords(), gsLoc.getGaussCoords(), gsLoc.getWeights()
            );
            break;
        }
        default:
            throw INTERP_KERNEL::Exception(
                "MEDFileFieldPerMeshPerTypePerDisc::assignFieldNoProfile : not implemented yet for such discretization "
                "type of field !"
            );
    }
    start = _end;
 ...
 * \param [in] idsInPfl is the selection into the \a multiTypePfl whole profile that corresponds to the current
 * geometric type.
 * \param [in] locIds is the profile needed to be created for MED file format. It can be null if all cells of current
 ...
 * \param [in] mesh is the mesh coming from the MEDFileMesh instance in correspondence with the MEDFileField. The mesh
 * inside the \a field is simply ignored.
 */
void
MEDFileFieldPerMeshPerTypePerDisc::assignFieldProfile(
    bool isPflAlone,
    mcIdType &start,
    const DataArrayIdType *multiTypePfl,
    const DataArrayIdType *idsInPfl,
    DataArrayIdType *locIds,
    mcIdType nbOfEltsInWholeMesh,
    const MEDCouplingFieldTemplate *field,
    const DataArray *arrr,
...
)
{
    ...
    if (locIds)
    {
        if (pflName.empty())
            throw INTERP_KERNEL::Exception(
                "MEDFileFieldPerMeshPerTypePerDisc::assignFieldProfile : existing profile with empty name !"
            );
    ...
    switch (_type)
    {
        ...
        {
            const MEDCouplingFieldDiscretizationGauss *disc2 =
                dynamic_cast<const MEDCouplingFieldDiscretizationGauss *>(field->getDiscretization());
            if (!disc2)
                throw INTERP_KERNEL::Exception(
                    "addNewEntryIfNecessaryGauss : invalid call to this method ! Internal Error !"
                );
            ...
            for (const mcIdType *pt = da4->begin(); pt != da4->end(); pt++)
                for (mcIdType j = arr->getIJ(*pt, 0); j < arr->getIJ(*pt + 1, 0); j++) *tmpPtr++ = j;
            //
        ...
        default:
            throw INTERP_KERNEL::Exception(
                "MEDFileFieldPerMeshPerTypePerDisc::assignFieldProfile : not implemented yet for such discretization "
                "type of field !"
            );
    }
    start = _end;
...
void
MEDFileFieldPerMeshPerTypePerDisc::assignNodeFieldNoProfile(
    mcIdType &start, const MEDCouplingFieldTemplate *field, const DataArray *arrr, MEDFileFieldGlobsReal &glob
)
{
    _start = start;
...
MEDFileFieldPerMeshPerTypePerDisc::NewOnRead(
    MEDFileFieldPerMeshPerTypeCommon *fath, TypeOfField type, int profileIt, const PartDefinition *pd
)
{
...
MEDFileFieldPerMeshPerTypePerDisc *
MEDFileFieldPerMeshPerTypePerDisc::New(MEDFileFieldPerMeshPerTypeCommon *fath, TypeOfField type, mcIdType locId)
{
    return new MEDFileFieldPerMeshPerTypePerDisc(fath, type, locId, std::string());
}

MEDFileFieldPerMeshPerTypePerDisc *
MEDFileFieldPerMeshPerTypePerDisc::New(const MEDFileFieldPerMeshPerTypePerDisc &other)
{
    return new MEDFileFieldPerMeshPerTypePerDisc(other);
}

std::size_t
MEDFileFieldPerMeshPerTypePerDisc::getHeapMemorySizeWithoutChildren() const
{
    return _profile.capacity() + _localization.capacity() + sizeof(MEDFileFieldPerMeshPerTypePerDisc);
}

std::vector<const BigMemoryObject *>
MEDFileFieldPerMeshPerTypePerDisc::getDirectChildrenWithNull() const
{
...
MEDFileFieldPerMeshPerTypePerDisc::MEDFileFieldPerMeshPerTypePerDisc(
    MEDFileFieldPerMeshPerTypeCommon *fath, TypeOfField atype, mcIdType profileIt, const PartDefinition *pd
)
...
MEDFileFieldPerMeshPerTypePerDisc::MEDFileFieldPerMeshPerTypePerDisc(
    MEDFileFieldPerMeshPerTypeCommon *fath, TypeOfField type, mcIdType locId, const std::string &dummy
)
    : _type(type), _father(fath), _loc_id(locId)
{
}

MEDFileFieldPerMeshPerTypePerDisc::MEDFileFieldPerMeshPerTypePerDisc(const MEDFileFieldPerMeshPerTypePerDisc &other)
    : RefCountObject(other),
      ...
      _localization(other._localization),
      _loc_id(other._loc_id),
      _profile_it(other._profile_it),
      _pd(other._pd),
...
MEDFileFieldPerMeshPerTypePerDisc::MEDFileFieldPerMeshPerTypePerDisc()
    : _type(ON_CELLS),
      _father(0),
      _start(-std::numeric_limits<mcIdType>::max()),
      _end(-std::numeric_limits<mcIdType>::max()),
      _nval(-std::numeric_limits<mcIdType>::max()),
      _loc_id(-std::numeric_limits<int>::max())
{
}

void
MEDFileFieldPerMeshPerTypePerDisc::goReadZeValuesInFile(
    med_idt fid,
...
)
{
    ...
    if (!pd)
    {
        ...
        INTERP_KERNEL::AutoPtr<char> locname(MEDLoaderBase::buildEmptyString(MED_NAME_SIZE));
        med_int nbi, tmp1;
        med_int nbValsInFile(MEDfieldnValueWithProfileByName(
            fid,
            fieldName.c_str(),
            iteration,
            order,
        ...
        {  //
            nbValsInFile = MEDfieldnValueWithProfileByName(
                fid,
                fieldName.c_str(),
                iteration,
                order,
        ...
        {
            std::ostringstream oss;
            oss << "MEDFileFieldPerMeshPerTypePerDisc::goReadZeValuesInFile : The number of tuples to read is "
                << nbValsInFile << "*" << nbi << " (nb integration points) ! But in data structure it values "
                << _end - _start << " is expected !";
            throw INTERP_KERNEL::Exception(oss.str());
        }
        MEDFILESAFECALLERRD0(
    ...
    else
    {
        if (!_profile.empty())
            throw INTERP_KERNEL::Exception(
                "MEDFileFieldPerMeshPerTypePerDisc::goReadZeValuesInFile : not implemented !"
            );
        INTERP_KERNEL::AutoPtr<char> pflname(MEDLoaderBase::buildEmptyString(MED_NAME_SIZE)),
            locname(MEDLoaderBase::buildEmptyString(MED_NAME_SIZE));
        med_int profilesize, nbi;
        med_int overallNval(MEDfieldnValueWithProfile(
            fid,
            fieldName.c_str(),
            iteration,
            order,
        ...
        {  // TODO : manage int32 !
            ...
            filter.fill(
                fid,
                /*nentity*/ overallNval,
                /*nvaluesperentity*/ nbi,
                /*nconstituentpervalue*/ nbOfCompo,
                MED_ALL_CONSTITUENT,
                MED_FULL_INTERLACE,
                MED_COMPACT_STMODE,
                MED_NO_PROFILE,
            ...
            MEDFILESAFECALLERRD0(
                ...
                (fid, fieldName.c_str(), iteration, order, menti, mgeoti, filter.getPtr(), startFeedingPtr)
            );
        }
    }
}
...
MEDFileFieldPerMeshPerTypePerDisc::getFather() const
{
    return _father;
}

void
MEDFileFieldPerMeshPerTypePerDisc::loadOnlyStructureOfDataRecursively(
    med_idt fid, mcIdType &start, const MEDFileFieldNameScope &nasc
)
{
    INTERP_KERNEL::AutoPtr<char> locname(MEDLoaderBase::buildEmptyString(MED_NAME_SIZE));
    INTERP_KERNEL::AutoPtr<char> pflname(MEDLoaderBase::buildEmptyString(MED_NAME_SIZE));
    ...
    med_int zeNVal(MEDfieldnValueWithProfile(
        fid,
        fieldName.c_str(),
        iteration,
        order,
    ...
    {  // eheh maybe there's a surprise :)
        med_int zeNVal1(MEDfieldnValueWithProfile(
            fid,
            fieldName.c_str(),
            iteration,
            order,
        ...
        if (zeNVal1 == 0)
        {
            med_int zeNVal2(MEDfieldnValueWithProfile(
                fid,
                fieldName.c_str(),
                iteration,
                order,
    ...
    else
    {
        if (!_profile.empty())
            throw INTERP_KERNEL::Exception(
                "MEDFileFieldPerMeshPerTypePerDisc::loadOnlyStructureOfDataRecursively : profiles are not managed yet "
                "with part of def !"
            );
...
void
MEDFileFieldPerMeshPerTypePerDisc::loadBigArray(med_idt fid, const MEDFileFieldNameScope &nasc)
{
    std::string fieldName(nasc.getName()), meshName(getMeshName());
    ...
    {
        std::ostringstream oss;
        oss << "MEDFileFieldPerMeshPerTypePerDisc::loadBigArray : Invalid start (" << _start
            << ") regarding admissible range of allocated array [0," << arr->getNumberOfTuples() << ") !";
        throw INTERP_KERNEL::Exception(oss.str());
    }
    ...
    if (arrD)
    {
        ...
        goReadZeValuesInFile(
            fid, fieldName, nbOfCompo, iteration, order, menti, mgeoti, reinterpret_cast<unsigned char *>(startFeeding)
        );
        return;
    }
    ...
    throw INTERP_KERNEL::Exception(
        "Error on array reading ! Unrecognized type of field ! Should be in FLOAT64 FLOAT32 INT32 or INT64 !"
    );
}
 ...
 */
void
MEDFileFieldPerMeshPerTypePerDisc::setNewStart(mcIdType newValueOfStart)
{
...
int
MEDFileFieldPerMeshPerTypePerDisc::getIteration() const
{
    return _father->getIteration();
}

int
MEDFileFieldPerMeshPerTypePerDisc::getOrder() const
{
    return _father->getOrder();
}

double
MEDFileFieldPerMeshPerTypePerDisc::getTime() const
{
    return _father->getTime();
}

std::string
MEDFileFieldPerMeshPerTypePerDisc::getMeshName() const
{
    return _father->getMeshName();
}

void
MEDFileFieldPerMeshPerTypePerDisc::simpleRepr(int bkOffset, std::ostream &oss, int id) const
{
    ...
    oss << startLine2 << "  This type discretization lies on profile : \"" << _profile
        << "\" and on the following localization : \"" << _localization << "\"." << std::endl;
    oss << startLine2 << "  This type discretization has " << _end - _start << " tuples (start=" << _start
        << ", end=" << _end << ")." << std::endl;
    oss << startLine2 << "  This type discretization has " << (_end - _start) / _nval << " integration points."
        << std::endl;
}

TypeOfField
MEDFileFieldPerMeshPerTypePerDisc::getType() const
{
    return _type;
}

INTERP_KERNEL::NormalizedCellType
MEDFileFieldPerMeshPerTypePerDisc::getGeoType() const
{
    return _father->getGeoType();
}

INTERP_KERNEL::NormalizedCellType
MEDFileFieldPerMeshPerTypePerDisc::getGeoTypeStatic() const
{
    return _father->getGeoTypeStatic();
}

void
MEDFileFieldPerMeshPerTypePerDisc::fillTypesOfFieldAvailable(std::set<TypeOfField> &types) const
{
    types.insert(_type);
}

void
MEDFileFieldPerMeshPerTypePerDisc::setType(TypeOfField newType)
{
    _type = newType;
}

std::size_t
MEDFileFieldPerMeshPerTypePerDisc::getNumberOfComponents() const
{
    return _father->getNumberOfComponents();
}

mcIdType
MEDFileFieldPerMeshPerTypePerDisc::getNumberOfTuples() const
{
    return _end - _start;
}

void
MEDFileFieldPerMeshPerTypePerDisc::incrementNbOfVals(mcIdType deltaNbVal)
{
...
DataArray *
MEDFileFieldPerMeshPerTypePerDisc::getOrCreateAndGetArray()
{
    return _father->getOrCreateAndGetArray();
}

const DataArray *
MEDFileFieldPerMeshPerTypePerDisc::getOrCreateAndGetArray() const
{
...
MEDFileFieldPerMeshPerTypePerDisc::getInfo() const
{
    return _father->getInfo();
}

std::string
MEDFileFieldPerMeshPerTypePerDisc::getProfile() const
{
    return _profile;
}

void
MEDFileFieldPerMeshPerTypePerDisc::setProfile(const std::string &newPflName)
{
    _profile = newPflName;
}

std::string
MEDFileFieldPerMeshPerTypePerDisc::getLocalization() const
{
    return _localization;
}

void
MEDFileFieldPerMeshPerTypePerDisc::setLocalization(const std::string &newLocName)
{
    _localization = newLocName;
}

void
MEDFileFieldPerMeshPerTypePerDisc::changePflsRefsNamesGen(
    const std::vector<std::pair<std::vector<std::string>, std::string> > &mapOfModif
)
{
    for (std::vector<std::pair<std::vector<std::string>, std::string> >::const_iterator it2 = mapOfModif.begin();
         it2 != mapOfModif.end();
         it2++)
    {
...
void
MEDFileFieldPerMeshPerTypePerDisc::changeLocsRefsNamesGen(
    const std::vector<std::pair<std::vector<std::string>, std::string> > &mapOfModif
)
{
    for (std::vector<std::pair<std::vector<std::string>, std::string> >::const_iterator it2 = mapOfModif.begin();
...
void
MEDFileFieldPerMeshPerTypePerDisc::getFieldAtLevel(
    TypeOfField type,
...
void
MEDFileFieldPerMeshPerTypePerDisc::fillValues(
    int discId,
    mcIdType &startEntryId,
    std::vector<std::pair<std::pair<INTERP_KERNEL::NormalizedCellType, int>, std::pair<mcIdType, mcIdType> > > &entries
) const
{
    entries[startEntryId] =
        std::pair<std::pair<INTERP_KERNEL::NormalizedCellType, mcIdType>, std::pair<mcIdType, mcIdType> >(
            std::pair<INTERP_KERNEL::NormalizedCellType, mcIdType>(getGeoType(), discId),
            std::pair<mcIdType, mcIdType>(_start, _end)
        );
    startEntryId++;
}

void
MEDFileFieldPerMeshPerTypePerDisc::writeLL(med_idt fid, const MEDFileFieldNameScope &nasc) const
{
    med_geometry_type mgeoti;
    med_entity_type menti;
    ...
    if (!arr->isAllocated())
        throw INTERP_KERNEL::Exception(
            "MEDFileFieldPerMeshPerTypePerDisc::writeLL : the array to be written is not allocated !"
        );
    ...
    if (arrD)
        locToWrite =
            reinterpret_cast<const unsigned char *>(arrD->getConstPointer() + _start * arr->getNumberOfComponents());
    else if (arrI)
        locToWrite =
            reinterpret_cast<const unsigned char *>(arrI->getConstPointer() + _start * arr->getNumberOfComponents());
    else if (arrI64)
        locToWrite =
            reinterpret_cast<const unsigned char *>(arrI64->getConstPointer() + _start * arr->getNumberOfComponents());
    else if (arrF)
        locToWrite =
            reinterpret_cast<const unsigned char *>(arrF->getConstPointer() + _start * arr->getNumberOfComponents());
    else
        throw INTERP_KERNEL::Exception(
            "MEDFileFieldPerMeshPerTypePerDisc::writeLL : not recognized type of values ! Supported are FLOAT64 "
            "FLOAT32 INT32 and INT64 !"
        );
    MEDFILESAFECALLERWR0(
...
void
MEDFileFieldPerMeshPerTypePerDisc::getCoarseData(
    TypeOfField &type, std::pair<mcIdType, mcIdType> &dad, std::string &pfl, std::string &loc
) const
{
    type = _type;
    pfl = _profile;
...
/*!
 * \param [in] codeOfMesh is of format returned by MEDCouplingUMesh::getDistributionOfTypes. And for each *i*
 * oldCode[3*i+2] gives the position (MEDFileUMesh::PutInThirdComponentOfCodeOffset). This code corresponds to the
 * distribution of types in the corresponding mesh.
 ...
 */
mcIdType
MEDFileFieldPerMeshPerTypePerDisc::fillEltIdsFromCode(
    mcIdType offset, const std::vector<mcIdType> &codeOfMesh, const MEDFileFieldGlobsReal &glob, mcIdType *ptToFill
) const
{
    ...
    {
        const INTERP_KERNEL::CellModel &cm = INTERP_KERNEL::CellModel::GetCellModel(getGeoType());
        oss << "MEDFileFieldPerMeshPerTypePerDisc::fillEltIdsFromCode : not found geometric type " << cm.getRepr()
            << " in the referenced mesh of field !";
        throw INTERP_KERNEL::Exception(oss.str());
    }
    ...
    if (_profile.empty())
    {
        ...
        {
            const INTERP_KERNEL::CellModel &cm = INTERP_KERNEL::CellModel::GetCellModel(getGeoType());
            oss << "MEDFileFieldPerMeshPerTypePerDisc::fillEltIdsFromCode : for geometric type " << cm.getRepr()
                << " number of elt ids in mesh is equal to " << _nval;
            oss << " whereas mesh has " << codeOfMesh[3 * found + 1] << " for this geometric type !";
            throw INTERP_KERNEL::Exception(oss.str());
        }
        for (mcIdType ii = codeOfMesh[3 * found + 2]; ii < codeOfMesh[3 * found + 2] + _nval; ii++) *work++ = ii;
    }
    else
    {
        ...
        if (pfl->getNumberOfTuples() != _nval)
        {
            const INTERP_KERNEL::CellModel &cm = INTERP_KERNEL::CellModel::GetCellModel(getGeoType());
            oss << "MEDFileFieldPerMeshPerTypePerDisc::fillEltIdsFromCode : for geometric type " << cm.getRepr()
                << ", field is defined on profile \"" << _profile << "\" and size of profile is ";
            oss << _nval;
            oss << pfl->getNumberOfTuples() << " whereas the number of ids is set to " << _nval
                << " for this geometric type !";
            throw INTERP_KERNEL::Exception(oss.str());
        }
...
mcIdType
MEDFileFieldPerMeshPerTypePerDisc::fillTupleIds(mcIdType *ptToFill) const
{
    for (mcIdType i = _start; i < _end; i++) *ptToFill++ = i;
    return _end - _start;
}

int
MEDFileFieldPerMeshPerTypePerDisc::ConvertType(TypeOfField type, mcIdType locId)
{
    switch (type)
    {
        case ON_CELLS:
        ...
        default:
            throw INTERP_KERNEL::Exception(
                "MEDFileFieldPerMeshPerTypePerDisc::ConvertType : not managed type of field !"
            );
    }
}
...
MEDFileFieldPerMeshPerTypePerDisc::SplitPerDiscretization(
    const std::vector<const MEDFileFieldPerMeshPerTypePerDisc *> &entries
)
{
    int id = 0;
    std::map<std::pair<std::string, TypeOfField>, int> m;
    std::vector<std::vector<const MEDFileFieldPerMeshPerTypePerDisc *> > ret;
    for (std::vector<const MEDFileFieldPerMeshPerTypePerDisc *>::const_iterator it = entries.begin();
         it != entries.end();
         it++)
 ...
 * \param [in] entriesOnSameDisc some entries **on same localization** if not the result can be invalid. The _start and
 * _end on them are relative to \a arr parameter.
 * \param [in] explicitIdsInMesh ids in mesh of the considered chunk.
 * \param [in] newCode one of the input parameter to explicit the new geo type dispatch (in classical format same than
 * those asked by MEDFileFields::renumberEntitiesLyingOnMesh)
 * \param [in,out] glob if necessary by the method, new profiles can be added to it
 * \param [in,out] arr after the call of this method \a arr is renumbered to be compliant with added entries to \a
 * result.
 * \param [out] result All new entries will be appended on it.
 * \return false if the configuration of renumbering leads to an unnecessary resplit of input \a entriesOnSameDisc. If
 * not true is returned (the most general case !)
 */
bool
MEDFileFieldPerMeshPerTypePerDisc::RenumberChunks(
    mcIdType offset,
    const std::vector<const MEDFileFieldPerMeshPerTypePerDisc *> &entriesOnSameDisc,
...
)
{
    ...
    for (std::vector<const MEDFileFieldPerMeshPerTypePerDisc *>::const_iterator it = entriesOnSameDisc.begin();
         it != entriesOnSameDisc.end();
         it++)
    {
        szEntities += (*it)->_nval;
        szTuples += (*it)->_end - (*it)->_start;
    }
    mcIdType nbi = szTuples / szEntities;
    if (szTuples % szEntities != 0)
        throw INTERP_KERNEL::Exception(
            "MEDFileFieldPerMeshPerTypePerDisc::RenumberChunks : internal error the splitting into same dicretization "
            "failed !"
        );
    MCAuto<DataArrayIdType> renumTuples = DataArrayIdType::New();
    renumTuples->alloc(szTuples, 1);
    ...
    MCAuto<DataArrayIdType> newGeoTypesPerChunk4 = DataArrayIdType::New();
    newGeoTypesPerChunk4->alloc(szEntities, nbi);
    int id = 0;
    for (std::vector<const MEDFileFieldPerMeshPerTypePerDisc *>::const_iterator it = entriesOnSameDisc.begin();
         it != entriesOnSameDisc.end();
         it++, id++)
    {
        ...
        MCAuto<DataArrayIdType> newEltIds =
            explicitIdsInMesh->subArray(startOfEltIdOfChunk, startOfEltIdOfChunk + (*it)->_nval);
        ...
        //
        MCAuto<DataArrayIdType> tmp = rangeIdsForChunk->duplicateEachTupleNTimes(nbi);
        rangeIdsForChunk->rearrange(nbi);
        newGeoTypesPerChunk4->setPartOfValues1(
            tmp, (*it)->_tmp_work1 - offset, (*it)->_tmp_work1 + (*it)->_nval * nbi - offset, 1, 0, nbi, 1
        );
        //
        newGeoTypesPerChunk[id] = rangeIdsForChunk;
        newGeoTypesPerChunk2[id] = rangeIdsForChunk;
        newGeoTypesPerChunk_bis[id] = idsInRrangeForChunk;
        newGeoTypesPerChunk3[id] = idsInRrangeForChunk;
    }
    MCAuto<DataArrayIdType> newGeoTypesEltIdsAllGather = DataArrayIdType::Aggregate(newGeoTypesPerChunk2);
    newGeoTypesPerChunk.clear();
    newGeoTypesPerChunk2.clear();
    MCAuto<DataArrayIdType> newGeoTypesEltIdsAllGather2 = DataArrayIdType::Aggregate(newGeoTypesPerChunk3);
    newGeoTypesPerChunk_bis.clear();
    newGeoTypesPerChunk3.clear();
    ...
    for (mcIdType i = 0; i < diffVals->getNumberOfTuples(); i++, idIt++)
    {
        ...
        bool ret2;
        MCAuto<MEDFileFieldPerMeshPerTypePerDisc> eltToAdd =
            MEDFileFieldPerMeshPerTypePerDisc::NewObjectOnSameDiscThanPool(
                type,
                (INTERP_KERNEL::NormalizedCellType)newCode[3 * (*idIt)],
                subIds,
                !subIds->isIota(newCode[3 * (*idIt) + 1]),
                nbi,
                offset + offset2,
                li,
                glob,
 ...
 * \param [in] isPfl specifies if a profile is requested regarding size of \a idsOfMeshElt and the number of such
 * entities regarding underlying mesh.
 * \param [in] nbi number of integration points
 * \param [in] offset The offset in the **global array of data**.
 * \param [in,out] entriesOnSameDisc the pool **on the same discretization** inside which it will be attempted to find
 * an existing entry corresponding exactly to the new chunk to create.
 * \param [in,out] glob the global shared info that will be requested for existing profiles or to append a new profile
 * if needed.
 * \param [out] notInExisting If false the return newly allocated entry is not coming from \a entriesOnSameDisc. If true
 * the output comes from copy of \a entriesOnSameDisc and corresponding entry erased from \a entriesOnSameDisc.
...
void
MEDFileFieldPerMeshPerTypeCommon::setFather(MEDFileFieldPerMesh *father)
{
    _father = father;
}

void
MEDFileFieldPerMeshPerTypeCommon::accept(MEDFileFieldVisitor &visitor) const
{
    for (std::vector<MCAuto<MEDFileFieldPerMeshPerTypePerDisc> >::const_iterator it = _field_pm_pt_pd.begin();
         it != _field_pm_pt_pd.end();
         it++)
        if ((*it).isNotNull())
        {
...
void
MEDFileFieldPerMeshPerTypeCommon::deepCopyElements()
{
    std::size_t i = 0;
    for (std::vector<MCAuto<MEDFileFieldPerMeshPerTypePerDisc> >::const_iterator it = _field_pm_pt_pd.begin();
         it != _field_pm_pt_pd.end();
...
std::size_t
MEDFileFieldPerMeshPerTypeCommon::getHeapMemorySizeWithoutChildren() const
{
    return _field_pm_pt_pd.capacity() * sizeof(MCAuto<MEDFileFieldPerMeshPerTypePerDisc>);
}

std::vector<const BigMemoryObject *>
MEDFileFieldPerMeshPerTypeCommon::getDirectChildrenWithNull() const
{
...
void
MEDFileFieldPerMeshPerTypeCommon::assignFieldNoProfile(
    mcIdType &start,
    mcIdType offset,
    mcIdType nbOfCells,
    const MEDCouplingFieldTemplate *field,
 ...
 */
void
MEDFileFieldPerMeshPerTypeCommon::assignFieldProfile(
    bool isPflAlone,
    mcIdType &start,
    const DataArrayIdType *multiTypePfl,
    const DataArrayIdType *idsInPfl,
...
)
{
    ...
    for (std::vector<mcIdType>::const_iterator it = pos.begin(); it != pos.end(); it++)
        _field_pm_pt_pd[*it]->assignFieldProfile(
            isPflAlone, start, multiTypePfl, idsInPfl, locIds, nbOfEltsInWholeMesh, field, arr, mesh, glob, nasc
        );
}

void
MEDFileFieldPerMeshPerTypeCommon::assignNodeFieldNoProfile(
    mcIdType &start, const MEDCouplingFieldTemplate *field, const DataArray *arr, MEDFileFieldGlobsReal &glob
)
{
...
)
{
    ...
    if (!arr || !arr->isAllocated())
        throw INTERP_KERNEL::Exception(
            "MEDFileFieldPerMeshPerTypeCommon::assignNodeFieldProfile : input array is null, or not allocated !"
        );
    _field_pm_pt_pd.resize(1);
    _field_pm_pt_pd[0] = MEDFileFieldPerMeshPerTypePerDisc::New(this, ON_NODES, -3);
...
std::vector<mcIdType>
MEDFileFieldPerMeshPerTypeCommon::addNewEntryIfNecessary(
    const MEDCouplingFieldTemplate *field, mcIdType offset, mcIdType nbOfCells
)
{
    ...
    else
    {
        ...
        for (std::size_t i = 0; i < sz2; i++)
        {
            ...
            for (mcIdType j = 0; j < sz && !found; j++)
            {
                ...
                {
                    _field_pm_pt_pd[j] =
                        MEDFileFieldPerMeshPerTypePerDisc::New(this, type, FromIdType<int>(locIdToFind));
                    ret3[k++] = j;
                    found = true;
                }
            }
...
std::vector<mcIdType>
MEDFileFieldPerMeshPerTypeCommon::addNewEntryIfNecessaryGauss(
    const MEDCouplingFieldTemplate *field, mcIdType offset, mcIdType nbOfCells
)
{
    const MEDCouplingFieldDiscretization *disc = field->getDiscretization();
    ...
    if (!da)
        throw INTERP_KERNEL::Exception(
            "addNewEntryIfNecessaryGauss (no profile) : no localization ids per cell array available ! The input Gauss "
            "node field is maybe invalid !"
        );
...
std::vector<mcIdType>
MEDFileFieldPerMeshPerTypeCommon::addNewEntryIfNecessary(
    const MEDCouplingFieldTemplate *field, const DataArrayIdType *subCells
)
{
    TypeOfField type = field->getTypeOfField();
    if (type != ON_GAUSS_PT)
...
)
{
    ...
    if (!da)
        throw INTERP_KERNEL::Exception(
            "addNewEntryIfNecessaryGauss : no localization ids per cell array available ! The input Gauss node field "
            "is maybe invalid !"
        );
    MCAuto<DataArrayIdType> da2 = da->selectByTupleIdSafe(
        subCells->getConstPointer(), subCells->getConstPointer() + subCells->getNumberOfTuples()
    );
    MCAuto<DataArrayIdType> retTmp = da2->getDifferentValues();
    if (retTmp->presenceOfValue(-1))
        throw INTERP_KERNEL::Exception("addNewEntryIfNecessaryGauss : some cells have no dicretization description !");
...
MEDFileFieldPerMeshPerTypeCommon::getFather() const
{
    return _father;
}

bool
MEDFileFieldPerMeshPerTypeCommon::isUniqueLevel(int &dim) const
{
    const INTERP_KERNEL::CellModel &cm(INTERP_KERNEL::CellModel::GetCellModel(getGeoType()));
...
void
MEDFileFieldPerMeshPerTypeCommon::fillTypesOfFieldAvailable(std::set<TypeOfField> &types) const
{
    for (std::vector<MCAuto<MEDFileFieldPerMeshPerTypePerDisc> >::const_iterator it = _field_pm_pt_pd.begin();
         it != _field_pm_pt_pd.end();
         it++)
...
void
MEDFileFieldPerMeshPerTypeCommon::fillFieldSplitedByType(
    std::vector<std::pair<mcIdType, mcIdType> > &dads,
    std::vector<TypeOfField> &types,
    std::vector<std::string> &pfls,
...
int
MEDFileFieldPerMeshPerTypeCommon::getIteration() const
{
    return _father->getIteration();
}

int
MEDFileFieldPerMeshPerTypeCommon::getOrder() const
{
    return _father->getOrder();
}

double
MEDFileFieldPerMeshPerTypeCommon::getTime() const
{
    return _father->getTime();
}

std::string
MEDFileFieldPerMeshPerTypeCommon::getMeshName() const
{
    return _father->getMeshName();
}

void
MEDFileFieldPerMeshPerTypeCommon::getSizes(mcIdType &globalSz, mcIdType &nbOfEntries) const
{
    for (std::vector<MCAuto<MEDFileFieldPerMeshPerTypePerDisc> >::const_iterator it = _field_pm_pt_pd.begin();
         it != _field_pm_pt_pd.end();
         it++)
...
std::size_t
MEDFileFieldPerMeshPerTypeCommon::getNumberOfComponents() const
{
    return _father->getNumberOfComponents();
}

bool
MEDFileFieldPerMeshPerTypeCommon::presenceOfMultiDiscPerGeoType() const
{
    std::size_t nb(0);
    for (std::vector<MCAuto<MEDFileFieldPerMeshPerTypePerDisc> >::const_iterator it = _field_pm_pt_pd.begin();
         it != _field_pm_pt_pd.end();
...
void
MEDFileFieldPerMeshPerTypeCommon::pushDiscretization(MEDFileFieldPerMeshPerTypePerDisc *disc)
{
...
DataArray *
MEDFileFieldPerMeshPerTypeCommon::getOrCreateAndGetArray()
{
    return _father->getOrCreateAndGetArray();
}

const DataArray *
MEDFileFieldPerMeshPerTypeCommon::getOrCreateAndGetArray() const
{
...
const std::vector<std::string> &
MEDFileFieldPerMeshPerTypeCommon::getInfo() const
{
    return _father->getInfo();
}

std::vector<std::string>
MEDFileFieldPerMeshPerTypeCommon::getPflsReallyUsed() const
{
    std::vector<std::string> ret;
    std::set<std::string> ret2;
    for (std::vector<MCAuto<MEDFileFieldPerMeshPerTypePerDisc> >::const_iterator it1 = _field_pm_pt_pd.begin();
         it1 != _field_pm_pt_pd.end();
         it1++)
    {
...
std::vector<std::string>
MEDFileFieldPerMeshPerTypeCommon::getLocsReallyUsed() const
{
    std::vector<std::string> ret;
    std::set<std::string> ret2;
    for (std::vector<MCAuto<MEDFileFieldPerMeshPerTypePerDisc> >::const_iterator it1 = _field_pm_pt_pd.begin();
...
std::vector<std::string>
MEDFileFieldPerMeshPerTypeCommon::getPflsReallyUsedMulti() const
{
    std::vector<std::string> ret;
    std::set<std::string> ret2;
    for (std::vector<MCAuto<MEDFileFieldPerMeshPerTypePerDisc> >::const_iterator it1 = _field_pm_pt_pd.begin();
...
std::vector<std::string>
MEDFileFieldPerMeshPerTypeCommon::getLocsReallyUsedMulti() const
{
    std::vector<std::string> ret;
    for (std::vector<MCAuto<MEDFileFieldPerMeshPerTypePerDisc> >::const_iterator it1 = _field_pm_pt_pd.begin();
         it1 != _field_pm_pt_pd.end();
...
void
MEDFileFieldPerMeshPerTypeCommon::changePflsRefsNamesGen(
    const std::vector<std::pair<std::vector<std::string>, std::string> > &mapOfModif
)
{
    for (std::vector<MCAuto<MEDFileFieldPerMeshPerTypePerDisc> >::iterator it1 = _field_pm_pt_pd.begin();
         it1 != _field_pm_pt_pd.end();
         it1++)
...
void
MEDFileFieldPerMeshPerTypeCommon::changeLocsRefsNamesGen(
    const std::vector<std::pair<std::vector<std::string>, std::string> > &mapOfModif
)
{
    for (std::vector<MCAuto<MEDFileFieldPerMeshPerTypePerDisc> >::iterator it1 = _field_pm_pt_pd.begin();
...
MEDFileFieldPerMeshPerTypePerDisc *
MEDFileFieldPerMeshPerTypeCommon::getLeafGivenLocId(mcIdType locId)
{
    if (_field_pm_pt_pd.empty())
    {
        std::ostringstream oss;
        oss << "MEDFileFieldPerMeshPerTypeCommon::getLeafGivenLocId : no localizations for geotype \""
            << getGeoTypeRepr() << "\" !";
        throw INTERP_KERNEL::Exception(oss.str());
    }
    ...
    std::ostringstream oss2;
    oss2 << "MEDFileFieldPerMeshPerTypeCommon::getLeafGivenLocId : no such locId available (" << locId;
...
MEDFileFieldPerMeshPerTypeCommon::getLeafGivenLocId(mcIdType locId) const
{
    if (_field_pm_pt_pd.empty())
    {
        std::ostringstream oss;
...
int
MEDFileFieldPerMeshPerTypeCommon::locIdOfLeaf(const MEDFileFieldPerMeshPerTypePerDisc *leaf) const
{
    int ret(0);
    for (std::vector<MCAuto<MEDFileFieldPerMeshPerTypePerDisc> >::const_iterator it = _field_pm_pt_pd.begin();
         it != _field_pm_pt_pd.end();
...
void
MEDFileFieldPerMeshPerTypeCommon::fillValues(
    mcIdType &startEntryId,
    std::vector<std::pair<std::pair<INTERP_KERNEL::NormalizedCellType, int>, std::pair<mcIdType, mcIdType> > > &entries
) const
{
...
void
MEDFileFieldPerMeshPerTypeCommon::setLeaves(const std::vector<MCAuto<MEDFileFieldPerMeshPerTypePerDisc> > &leaves)
{
    _field_pm_pt_pd = leaves;
    for (std::vector<MCAuto<MEDFileFieldPerMeshPerTypePerDisc> >::iterator it = _field_pm_pt_pd.begin();
         it != _field_pm_pt_pd.end();
         it++)
        (*it)->setFather(this);
}
 ...
 */
bool
MEDFileFieldPerMeshPerTypeCommon::keepOnlySpatialDiscretization(
    TypeOfField tof, mcIdType &globalNum, std::vector<std::pair<mcIdType, mcIdType> > &its
)
{
    bool ret(false);
    ...
    for (std::vector<MCAuto<MEDFileFieldPerMeshPerTypePerDisc> >::iterator it = _field_pm_pt_pd.begin();
        ...
        if ((*it)->getType() == tof)
        {
            ...
            std::pair<mcIdType, mcIdType> bgEnd;
            bgEnd.first = (*it)->getStart();
            bgEnd.second = (*it)->getEnd();
 ...
 */
bool
MEDFileFieldPerMeshPerTypeCommon::keepOnlyGaussDiscretization(
    std::size_t idOfDisc, mcIdType &globalNum, std::vector<std::pair<mcIdType, mcIdType> > &its
)
{
    ...
    std::pair<mcIdType, mcIdType> bgEnd;
    bgEnd.first = _field_pm_pt_pd[idOfDisc]->getStart();
    bgEnd.second = _field_pm_pt_pd[idOfDisc]->getEnd();
...
void
MEDFileFieldPerMeshPerTypeCommon::loadOnlyStructureOfDataRecursively(
    med_idt fid, mcIdType &start, const MEDFileFieldNameScope &nasc
)
{
    for (std::vector<MCAuto<MEDFileFieldPerMeshPerTypePerDisc> >::iterator it = _field_pm_pt_pd.begin();
...
void
MEDFileFieldPerMeshPerTypeCommon::loadBigArraysRecursively(med_idt fid, const MEDFileFieldNameScope &nasc)
{
    for (std::vector<MCAuto<MEDFileFieldPerMeshPerTypePerDisc> >::iterator it = _field_pm_pt_pd.begin();
         it != _field_pm_pt_pd.end();
         it++)
...
void
MEDFileFieldPerMeshPerTypeCommon::writeLL(med_idt fid, const MEDFileFieldNameScope &nasc) const
{
    for (std::vector<MCAuto<MEDFileFieldPerMeshPerTypePerDisc> >::const_iterator it = _field_pm_pt_pd.begin();
         it != _field_pm_pt_pd.end();
         it++)
...
med_entity_type
MEDFileFieldPerMeshPerTypeCommon::ConvertIntoMEDFileType(
    TypeOfField ikType, INTERP_KERNEL::NormalizedCellType ikGeoType, med_geometry_type &medfGeoType
)
{
    switch (ikType)
    {
        ...
        default:
            throw INTERP_KERNEL::Exception(
                "MEDFileFieldPerMeshPerTypeCommon::ConvertIntoMEDFileType : unexpected entity type ! internal error"
            );
    }
...
MEDFileFieldPerMeshPerType::NewOnRead(
    med_idt fid,
    MEDFileFieldPerMesh *fath,
    TypeOfField type,
    INTERP_KERNEL::NormalizedCellType geoType,
    const MEDFileFieldNameScope &nasc,
...
MEDFileFieldPerMeshPerType *
MEDFileFieldPerMeshPerType::New(MEDFileFieldPerMesh *fath, INTERP_KERNEL::NormalizedCellType geoType)
{
...
void
MEDFileFieldPerMeshPerType::getFieldAtLevel(
    int meshDim,
    TypeOfField type,
    const MEDFileFieldGlobsReal *glob,
    std::vector<std::pair<mcIdType, mcIdType> > &dads,
...
INTERP_KERNEL::NormalizedCellType
MEDFileFieldPerMeshPerType::getGeoType() const
{
    return _geo_type;
}

INTERP_KERNEL::NormalizedCellType
MEDFileFieldPerMeshPerType::getGeoTypeStatic() const
{
    return _geo_type;
}

void
MEDFileFieldPerMeshPerType::entriesForMEDfile(TypeOfField mct, med_geometry_type &gt, med_entity_type &ent) const
{
    ent = MEDFileFieldPerMeshPerTypeCommon::ConvertIntoMEDFileType(mct, _geo_type, gt);
}

void
MEDFileFieldPerMeshPerType::getDimension(int &dim) const
{
    const INTERP_KERNEL::CellModel &cm(INTERP_KERNEL::CellModel::GetCellModel(_geo_type));
    int curDim((int)cm.getDimension());
...
void
MEDFileFieldPerMeshPerType::simpleRepr(int bkOffset, std::ostream &oss, int id) const
{
    const char startLine[] = "  ## ";
    std::string startLine2(bkOffset, ' ');
    ...
    {
        const INTERP_KERNEL::CellModel &cm = INTERP_KERNEL::CellModel::GetCellModel(_geo_type);
        oss << startLine3 << "Entry geometry type #" << id << " is lying on geometry types " << cm.getRepr() << "."
            << std::endl;
    }
    else
...
std::string
MEDFileFieldPerMeshPerType::getGeoTypeRepr() const
{
    const INTERP_KERNEL::CellModel &cm(INTERP_KERNEL::CellModel::GetCellModel(_geo_type));
    return std::string(cm.getRepr());
}

MEDFileFieldPerMeshPerType::MEDFileFieldPerMeshPerType(
    MEDFileFieldPerMesh *father, INTERP_KERNEL::NormalizedCellType gt
)
    : MEDFileFieldPerMeshPerTypeCommon(father), _geo_type(gt)
{
}
...
MEDFileFieldPerMeshPerType::MEDFileFieldPerMeshPerType(
    ...
    : MEDFileFieldPerMeshPerTypeCommon(fath), _geo_type(geoType)
{
    INTERP_KERNEL::AutoPtr<char> pflName = MEDLoaderBase::buildEmptyString(MED_NAME_SIZE);
    INTERP_KERNEL::AutoPtr<char> locName = MEDLoaderBase::buildEmptyString(MED_NAME_SIZE);
    med_geometry_type mgeoti;
    ...
    med_int nbProfiles(
        MEDfieldnProfile(fid, nasc.getName().c_str(), getIteration(), getOrder(), menti, mgeoti, pflName, locName)
    );
    ...
    if (type == ON_CELLS)
    {
        med_int nbProfiles2(MEDfieldnProfile(
            fid, nasc.getName().c_str(), getIteration(), getOrder(), MED_NODE_ELEMENT, mgeoti, pflName, locName
        ));
    ...
    {
        med_int nbProfiles1(MEDfieldnProfile(
            fid, nasc.getName().c_str(), getIteration(), getOrder(), MED_DESCENDING_FACE, mgeoti, pflName, locName
        ));
        med_int nbProfiles2(MEDfieldnProfile(
            fid, nasc.getName().c_str(), getIteration(), getOrder(), MED_DESCENDING_EDGE, mgeoti, pflName, locName
        ));
        if (nbProfiles1 == 0 && nbProfiles2 == 0)
            return;  // OK definitely nothing in field
        menti = nbProfiles1 >= nbProfiles2 ? MED_DESCENDING_FACE
                                           : MED_DESCENDING_EDGE;  // not enough words to describe the beauty
...
MCAuto<MEDFileFieldPerMeshPerType>
MEDFileFieldPerMeshPerType::Aggregate(
    mcIdType &start,
    const std::vector<std::pair<int, const MEDFileFieldPerMeshPerType *> > &pms,
    const std::vector<std::vector<std::pair<int, mcIdType> > > &dts,
    INTERP_KERNEL::NormalizedCellType gt,
    MEDFileFieldPerMesh *father,
...
)
{
    ...
    for (std::vector<std::pair<int, const MEDFileFieldPerMeshPerType *> >::const_iterator it = pms.begin();
         it != pms.end();
         it++)
    {
        for (std::vector<MCAuto<MEDFileFieldPerMeshPerTypePerDisc> >::const_iterator it2 =
                 (*it).second->_field_pm_pt_pd.begin();
             it2 != (*it).second->_field_pm_pt_pd.end();
             it2++)
            m[(*it2)->getType()].push_back(
                std::pair<int, const MEDFileFieldPerMeshPerTypePerDisc *>((*it).first, *it2)
            );
    }
    for (std::map<TypeOfField, std::vector<std::pair<int, const MEDFileFieldPerMeshPerTypePerDisc *> > >::const_iterator
             it = m.begin();
         it != m.end();
         it++)
    {
        MCAuto<MEDFileFieldPerMeshPerTypePerDisc> agg(
            MEDFileFieldPerMeshPerTypePerDisc::Aggregate(start, (*it).second, dts, (*it).first, ret, extractInfo)
        );
...
MEDFileFieldPerMeshPerTypeDyn *
MEDFileFieldPerMeshPerTypeDyn::NewOnRead(
    med_idt fid, MEDFileFieldPerMesh *fath, const MEDFileEntities *entities, int idGT, const MEDFileFieldNameScope &nasc
)
{
    if (!entities)
...
MEDFileFieldPerMeshPerTypeDyn::MEDFileFieldPerMeshPerTypeDyn(
    med_idt fid, MEDFileFieldPerMesh *fath, const MEDFileStructureElement *se, const MEDFileFieldNameScope &nasc
)
    : MEDFileFieldPerMeshPerTypeCommon(fath)
{
    _se.takeRef(se);
    INTERP_KERNEL::AutoPtr<char> pflName = MEDLoaderBase::buildEmptyString(MED_NAME_SIZE);
    INTERP_KERNEL::AutoPtr<char> locName = MEDLoaderBase::buildEmptyString(MED_NAME_SIZE);
    med_int nbProfiles(MEDfieldnProfile(
        fid, nasc.getName().c_str(), getIteration(), getOrder(), MED_STRUCT_ELEMENT, _se->getDynGT(), pflName, locName
    ));
    _field_pm_pt_pd.resize(nbProfiles);
    for (int i = 0; i < nbProfiles; i++)
    {
...
int
MEDFileFieldPerMeshPerTypeDyn::getDynGT() const
{
    return _se->getDynGT();
}

std::string
MEDFileFieldPerMeshPerTypeDyn::getModelName() const
{
    return _se->getName();
}

void
MEDFileFieldPerMeshPerTypeDyn::getDimension(int &dim) const
{
    throw INTERP_KERNEL::Exception("not implemented yet !");
}

void
MEDFileFieldPerMeshPerTypeDyn::entriesForMEDfile(TypeOfField mct, med_geometry_type &gt, med_entity_type &ent) const
{
    gt = getDynGT();
...
INTERP_KERNEL::NormalizedCellType
MEDFileFieldPerMeshPerTypeDyn::getGeoType() const
{
    throw INTERP_KERNEL::Exception("not implemented yet !");
}

INTERP_KERNEL::NormalizedCellType
MEDFileFieldPerMeshPerTypeDyn::getGeoTypeStatic() const
{
    return _se->getGeoType();
}

void
MEDFileFieldPerMeshPerTypeDyn::simpleRepr(int bkOffset, std::ostream &oss, int id) const
{
    const char startLine[] = "  ## ";
    std::string startLine2(bkOffset, ' ');
    std::string startLine3(startLine2);
    startLine3 += startLine;
    oss << startLine3 << "Entry geometry type #" << id << " is lying on geometry STRUCTURE_ELEMENT type " << getDynGT()
        << "." << std::endl;
    oss << startLine3 << "Entry is defined on " << _field_pm_pt_pd.size() << " localizations." << std::endl;
    int i = 0;
    for (std::vector<MCAuto<MEDFileFieldPerMeshPerTypePerDisc> >::const_iterator it = _field_pm_pt_pd.begin();
...
std::string
MEDFileFieldPerMeshPerTypeDyn::getGeoTypeRepr() const
{
    throw INTERP_KERNEL::Exception("not implemented yet !");
}
...
void
MEDFileFieldPerMeshPerTypeDyn::getFieldAtLevel(
    int meshDim,
    TypeOfField type,
    const MEDFileFieldGlobsReal *glob,
    std::vector<std::pair<mcIdType, mcIdType> > &dads,
...
MEDFileFieldPerMesh *
MEDFileFieldPerMesh::NewOnRead(
    med_idt fid,
    MEDFileAnyTypeField1TSWithoutSDA *fath,
    int meshCsit,
    int meshIteration,
    int meshOrder,
    const MEDFileFieldNameScope &nasc,
    const MEDFileMesh *mm,
...
MEDFileFieldPerMesh *
MEDFileFieldPerMesh::New(MEDFileAnyTypeField1TSWithoutSDA *fath, const MEDCouplingMesh *mesh)
{
    return new MEDFileFieldPerMesh(fath, mesh);
}

std::size_t
MEDFileFieldPerMesh::getHeapMemorySizeWithoutChildren() const
{
    return _field_pm_pt.capacity() * sizeof(MCAuto<MEDFileFieldPerMeshPerType>);
}

std::vector<const BigMemoryObject *>
MEDFileFieldPerMesh::getDirectChildrenWithNull() const
{
    std::vector<const BigMemoryObject *> ret;
    for (std::vector<MCAuto<MEDFileFieldPerMeshPerTypeCommon> >::const_iterator it = _field_pm_pt.begin();
         it != _field_pm_pt.end();
         it++)
        ret.push_back(*it);
    return ret;
...
void
MEDFileFieldPerMesh::simpleRepr(int bkOffset, std::ostream &oss, int id) const
{
    std::string startLine(bkOffset, ' ');
    oss << startLine << "## Field part (" << id << ") lying on mesh \"" << getMeshName()
        << "\", Mesh iteration=" << _mesh_iteration << ". Mesh order=" << _mesh_order << "." << std::endl;
...
void
MEDFileFieldPerMesh::copyTinyInfoFrom(const MEDCouplingMesh *mesh)
{
    mesh->getTime(_mesh_iteration, _mesh_order);
}

void
MEDFileFieldPerMesh::assignFieldNoProfileNoRenum(
    mcIdType &start,
    const std::vector<mcIdType> &code,
    const MEDCouplingFieldTemplate *field,
    const DataArray *arr,
    MEDFileFieldGlobsReal &glob,
    const MEDFileFieldNameScope &nasc
 ...
 * This method is the most general one. No optimization is done here.
 * \param [in] multiTypePfl is the end user profile specified in high level API
 * \param [in] code is the code of \a mesh[multiTypePfl] mesh. It is of size of number of different geometric types into
 * \a mesh[multiTypePfl].
 * \param [in] code2 is the code of the \b WHOLE mesh on the same level. So all types in \a code are in \a code2.
 * \param [in] idsInPflPerType is the selection into the \a multiTypePfl whole profile that corresponds to the given
 * geometric type. This vector is always 3 times smaller than \a code.
 * \param [in] idsPerType is a vector containing the profiles needed to be created for MED file format. \b WARNING these
 * processed MED file profiles can be subdivided again in case of Gauss points.
 * \param [in] mesh is the mesh coming from the MEDFileMesh instance in correspondence with the MEDFileField. The mesh
 * inside the \a field is simply ignored.
 */
void
MEDFileFieldPerMesh::assignFieldProfile(
    mcIdType &start,
    const DataArrayIdType *multiTypePfl,
    const std::vector<mcIdType> &code,
    const std::vector<mcIdType> &code2,
    const std::vector<DataArrayIdType *> &idsInPflPerType,
    const std::vector<DataArrayIdType *> &idsPerType,
    const MEDCouplingFieldTemplate *field,
    const DataArray *arr,
    const MEDCouplingMesh *mesh,
    MEDFileFieldGlobsReal &glob,
...
)
{
    ...
    for (std::size_t i = 0; i < nbOfTypes; i++)
    {
        ...
        if (found == nbOfTupes2)
            throw INTERP_KERNEL::Exception(
                "MEDFileFieldPerMesh::assignFieldProfile : internal problem ! Should never happen ! Please report bug "
                "to anthony.geay@cea.fr !"
            );
        _field_pm_pt[pos]->assignFieldProfile(
            nbOfTypes == 1,
            start,
            multiTypePfl,
            idsInPflPerType[i],
...
void
MEDFileFieldPerMesh::assignNodeFieldNoProfile(
    mcIdType &start, const MEDCouplingFieldTemplate *field, const DataArray *arr, MEDFileFieldGlobsReal &glob
)
{
...
void
MEDFileFieldPerMesh::loadOnlyStructureOfDataRecursively(med_idt fid, mcIdType &start, const MEDFileFieldNameScope &nasc)
{
    for (std::vector<MCAuto<MEDFileFieldPerMeshPerTypeCommon> >::iterator it = _field_pm_pt.begin();
         it != _field_pm_pt.end();
         it++)
        (*it)->loadOnlyStructureOfDataRecursively(fid, start, nasc);
}
...
void
MEDFileFieldPerMesh::loadBigArraysRecursively(med_idt fid, const MEDFileFieldNameScope &nasc)
{
    for (std::vector<MCAuto<MEDFileFieldPerMeshPerTypeCommon> >::iterator it = _field_pm_pt.begin();
         it != _field_pm_pt.end();
         it++)
...
void
MEDFileFieldPerMesh::writeLL(med_idt fid, const MEDFileFieldNameScope &nasc) const
{
...
void
MEDFileFieldPerMesh::getDimension(int &dim) const
{
    for (std::vector<MCAuto<MEDFileFieldPerMeshPerTypeCommon> >::const_iterator it = _field_pm_pt.begin();
         it != _field_pm_pt.end();
         it++)
...
bool
MEDFileFieldPerMesh::isUniqueLevel(int &dim) const
{
    for (std::vector<MCAuto<MEDFileFieldPerMeshPerTypeCommon> >::const_iterator it = _field_pm_pt.begin();
         it != _field_pm_pt.end();
         it++)
...
void
MEDFileFieldPerMesh::fillTypesOfFieldAvailable(std::set<TypeOfField> &types) const
{
    for (std::vector<MCAuto<MEDFileFieldPerMeshPerTypeCommon> >::const_iterator it = _field_pm_pt.begin();
         it != _field_pm_pt.end();
         it++)
...
std::vector<std::vector<std::pair<mcIdType, mcIdType> > >
MEDFileFieldPerMesh::getFieldSplitedByType(
...
double
MEDFileFieldPerMesh::getTime() const
{
    int tmp1, tmp2;
...
int
MEDFileFieldPerMesh::getIteration() const
{
    return _father->getIteration();
}

int
MEDFileFieldPerMesh::getOrder() const
{
    return _father->getOrder();
}

std::size_t
MEDFileFieldPerMesh::getNumberOfComponents() const
{
    return _father->getNumberOfComponents();
}

std::string
MEDFileFieldPerMesh::getMeshName() const
{
    return _father->getMeshName();
}

void
MEDFileFieldPerMesh::setMeshName(const std::string &meshName)
{
    _father->setMeshName(meshName);
}

bool
MEDFileFieldPerMesh::presenceOfMultiDiscPerGeoType() const
{
    for (std::vector<MCAuto<MEDFileFieldPerMeshPerTypeCommon> >::const_iterator it = _field_pm_pt.begin();
         it != _field_pm_pt.end();
         it++)
...
bool
MEDFileFieldPerMesh::presenceOfStructureElements() const
{
    for (std::vector<MCAuto<MEDFileFieldPerMeshPerTypeCommon> >::const_iterator it = _field_pm_pt.begin();
         it != _field_pm_pt.end();
         it++)
        if ((*it).isNotNull())
        {
            const MEDFileFieldPerMeshPerTypeDyn *pt(
                dynamic_cast<const MEDFileFieldPerMeshPerTypeDyn *>((const MEDFileFieldPerMeshPerTypeCommon *)*it)
            );
            if (pt)
                return true;
        }
...
bool
MEDFileFieldPerMesh::onlyStructureElements() const
{
    for (std::vector<MCAuto<MEDFileFieldPerMeshPerTypeCommon> >::const_iterator it = _field_pm_pt.begin();
         it != _field_pm_pt.end();
         it++)
...
void
MEDFileFieldPerMesh::killStructureElements()
{
...
void
MEDFileFieldPerMesh::keepOnlyStructureElements()
{
    std::vector<MCAuto<MEDFileFieldPerMeshPerTypeCommon> > res;
    for (std::vector<MCAuto<MEDFileFieldPerMeshPerTypeCommon> >::iterator it = _field_pm_pt.begin();
         it != _field_pm_pt.end();
...
void
MEDFileFieldPerMesh::keepOnlyOnSE(const std::string &seName)
{
    std::vector<MCAuto<MEDFileFieldPerMeshPerTypeCommon> > res;
    for (std::vector<MCAuto<MEDFileFieldPerMeshPerTypeCommon> >::iterator it = _field_pm_pt.begin();
         it != _field_pm_pt.end();
...
void
MEDFileFieldPerMesh::getMeshSENames(std::vector<std::pair<std::string, std::string> > &ps) const
{
    for (std::vector<MCAuto<MEDFileFieldPerMeshPerTypeCommon> >::const_iterator it = _field_pm_pt.begin();
         it != _field_pm_pt.end();
         it++)
    {
        if ((*it).isNotNull())
        {
            ...
            if (pt)
            {
                ...
                throw INTERP_KERNEL::Exception(
                    "MEDFileFieldPerMesh::getMeshSENames : presence of a non structure element part !"
                );
        }
    }
}
...
DataArray *
MEDFileFieldPerMesh::getOrCreateAndGetArray()
{
    if (!_father)
...
const DataArray *
MEDFileFieldPerMesh::getOrCreateAndGetArray() const
{
    if (!_father)
        throw INTERP_KERNEL::Exception("MEDFileFieldPerMesh::getOrCreateAndGetArray : no father ! internal error !");
    return _father->getOrCreateAndGetArray();
...
const std::vector<std::string> &
MEDFileFieldPerMesh::getInfo() const
{
    return _father->getInfo();
}
 ...
 * Before the call of this method 'geoTypes','dads','pfls','locs' must be reorganized so that types in geoTypes are
 * contiguous and ordered following typmai2 array. It returns 2 output vectors :
 ...
 */
void
MEDFileFieldPerMesh::SortArraysPerType(
    const MEDFileFieldGlobsReal *glob,
    TypeOfField type,
    const std::vector<INTERP_KERNEL::NormalizedCellType> &geoTypes,
    const std::vector<std::pair<mcIdType, mcIdType> > &dads,
    const std::vector<const DataArrayIdType *> &pfls,
    const std::vector<int> &locs,
    std::vector<mcIdType> &code,
    std::vector<DataArrayIdType *> &notNullPfls
)
{
    int notNullPflsSz = 0;
 ...
 */
mcIdType
MEDFileFieldPerMesh::ComputeNbOfElems(
    const MEDFileFieldGlobsReal *glob,
    TypeOfField type,
    const std::vector<INTERP_KERNEL::NormalizedCellType> &geoTypes,
    const std::vector<std::pair<mcIdType, mcIdType> > &dads,
...
std::vector<std::string>
MEDFileFieldPerMesh::getPflsReallyUsed() const
{
    std::vector<std::string> ret;
    std::set<std::string> ret2;
    for (std::vector<MCAuto<MEDFileFieldPerMeshPerTypeCommon> >::const_iterator it = _field_pm_pt.begin();
...
std::vector<std::string>
MEDFileFieldPerMesh::getPflsReallyUsedMulti() const
{
    std::vector<std::string> ret;
    for (std::vector<MCAuto<MEDFileFieldPerMeshPerTypeCommon> >::const_iterator it = _field_pm_pt.begin();
         it != _field_pm_pt.end();
...
std::vector<std::string>
MEDFileFieldPerMesh::getLocsReallyUsed() const
{
    std::vector<std::string> ret;
    std::set<std::string> ret2;
    for (std::vector<MCAuto<MEDFileFieldPerMeshPerTypeCommon> >::const_iterator it = _field_pm_pt.begin();
...
std::vector<std::string>
MEDFileFieldPerMesh::getLocsReallyUsedMulti() const
{
    std::vector<std::string> ret;
    for (std::vector<MCAuto<MEDFileFieldPerMeshPerTypeCommon> >::const_iterator it = _field_pm_pt.begin();
         it != _field_pm_pt.end();
...
bool
MEDFileFieldPerMesh::changeMeshNames(const std::vector<std::pair<std::string, std::string> > &modifTab)
{
...
void
MEDFileFieldPerMesh::convertMedBallIntoClassic()
{
    if (_field_pm_pt.size() != 1)
        throw INTERP_KERNEL::Exception(
            "MEDFileFieldPerMesh::convertMedBallIntoClassic : Only managed for single mesh !"
        );
    if (_field_pm_pt[0].isNull())
        throw INTERP_KERNEL::Exception("MEDFileFieldPerMesh::convertMedBallIntoClassic : null pointer !");
    MEDFileFieldPerMeshPerTypeDyn *pt(
        dynamic_cast<MEDFileFieldPerMeshPerTypeDyn *>((MEDFileFieldPerMeshPerTypeCommon *)_field_pm_pt[0])
    );
    if (!pt)
        throw INTERP_KERNEL::Exception(
            "MEDFileFieldPerMesh::convertMedBallIntoClassic : this is expected to be marked as structure element !"
        );
...
bool
MEDFileFieldPerMesh::renumberEntitiesLyingOnMesh(
...
)
{
    ...
    for (std::size_t i = 0; i < oldCode.size() / 3; i++)
        typesToKeep.insert((INTERP_KERNEL::NormalizedCellType)oldCode[3 * i]);
    std::vector<std::pair<std::pair<INTERP_KERNEL::NormalizedCellType, int>, std::pair<mcIdType, mcIdType> > > entries;
    ...
    if (!arr0)
        throw INTERP_KERNEL::Exception(
            "MEDFileFieldPerMesh::renumberEntitiesLyingOnMesh : DataArray storing values of field is null !"
        );
    DataArrayDouble *arr(dynamic_cast<DataArrayDouble *>(arr0));  // tony
    if (!arr0)
        throw INTERP_KERNEL::Exception(
            "MEDFileFieldPerMesh::renumberEntitiesLyingOnMesh : DataArray storing values is double ! Not managed for "
            "the moment !"
        );
    mcIdType sz = 0;
    if (!arr)
        throw INTERP_KERNEL::Exception(
            "MEDFileFieldPerMesh::renumberEntitiesLyingOnMesh : DataArrayDouble storing values of field is null !"
        );
    for (std::vector<std::pair<std::pair<INTERP_KERNEL::NormalizedCellType, int>, std::pair<mcIdType, mcIdType> > >::
    ...
    MCAuto<DataArrayIdType> renumDefrag = DataArrayIdType::New();
    renumDefrag->alloc(arr->getNumberOfTuples(), 1);
    renumDefrag->fillWithZero();
    ////////////////////
    MCAuto<DataArrayIdType> explicitIdsOldInMesh = DataArrayIdType::New();
    explicitIdsOldInMesh->alloc(sz, 1);  // sz is a majorant of the real size. A realloc will be done after
    mcIdType *workI2 = explicitIdsOldInMesh->getPointer();
    mcIdType sz1 = 0, sz2 = 0, sid = 1;
    std::vector<std::vector<const MEDFileFieldPerMeshPerTypePerDisc *> > entriesKeptML =
        MEDFileFieldPerMeshPerTypePerDisc::SplitPerDiscretization(entriesKept);
    // std::vector<int> tupleIdOfStartOfNewChuncksV(entriesKeptML.size());
    for (std::vector<std::vector<const MEDFileFieldPerMeshPerTypePerDisc *> >::const_iterator itL1 =
             entriesKeptML.begin();
         itL1 != entriesKeptML.end();
         itL1++, sid++)
    {
        //  tupleIdOfStartOfNewChuncksV[sid-1]=sz2;
        MCAuto<DataArrayIdType> explicitIdsOldInArr = DataArrayIdType::New();
        explicitIdsOldInArr->alloc(sz, 1);
        mcIdType *workI = explicitIdsOldInArr->getPointer();
        for (std::vector<const MEDFileFieldPerMeshPerTypePerDisc *>::const_iterator itL2 = (*itL1).begin();
             itL2 != (*itL1).end();
             itL2++)
        {
            mcIdType delta1 = (*itL2)->fillTupleIds(workI);
            workI += delta1;
            sz1 += delta1;
            (*itL2)->setLocId(sz2);
            (*itL2)->_tmp_work1 = (*itL2)->getStart();
            mcIdType delta2 = (*itL2)->fillEltIdsFromCode(sz2, oldCode, glob, workI2);
            workI2 += delta2;
            sz2 += delta2;
        }
    ...
    ////////////////////
    MCAuto<DataArrayIdType> permArrDefrag = renumDefrag->buildPermArrPerLevel();
    renumDefrag = 0;
    ...
    for (std::vector<const MEDFileFieldPerMeshPerTypePerDisc *>::const_iterator it = otherEntries.begin();
         it != otherEntries.end();
         it++)
    {
    ...
    for (std::vector<const MEDFileFieldPerMeshPerTypePerDisc *>::const_iterator it = entriesKept.begin();
         it != entriesKept.end();
         it++)
    {
    ...
    MCAuto<DataArrayIdType> explicitIdsNewInMesh =
        renumO2N->selectByTupleId(explicitIdsOldInMesh->begin(), explicitIdsOldInMesh->end());
    std::vector<std::vector<const MEDFileFieldPerMeshPerTypePerDisc *> > entriesKeptPerDisc =
        MEDFileFieldPerMeshPerTypePerDisc::SplitPerDiscretization(entriesKeptNew2);
    bool ret = false;
    for (std::vector<std::vector<const MEDFileFieldPerMeshPerTypePerDisc *> >::const_iterator it4 =
             entriesKeptPerDisc.begin();
         it4 != entriesKeptPerDisc.end();
         it4++)
    {
        sid = 0;
        /*for(std::vector< const MEDFileFieldPerMeshPerTypePerDisc *>::const_iterator
        ...
        ret = MEDFileFieldPerMeshPerTypePerDisc::RenumberChunks(
                  tupleIdOfStartOfNewChuncks, *it4, explicitIdsNewInMesh, newCode, glob, arr2, otherEntriesNew
              ) ||
              ret;
    }
    if (!ret)
 ...
 */
void
MEDFileFieldPerMesh::keepOnlySpatialDiscretization(
    TypeOfField tof, mcIdType &globalNum, std::vector<std::pair<mcIdType, mcIdType> > &its
)
{
 ...
 */
void
MEDFileFieldPerMesh::keepOnlyGaussDiscretization(
    std::size_t idOfDisc, mcIdType &globalNum, std::vector<std::pair<mcIdType, mcIdType> > &its
)
{
    std::vector<MCAuto<MEDFileFieldPerMeshPerTypeCommon> > ret;
...
void
MEDFileFieldPerMesh::assignNewLeaves(const std::vector<MCAuto<MEDFileFieldPerMeshPerTypePerDisc> > &leaves)
{
    std::map<INTERP_KERNEL::NormalizedCellType, std::vector<MCAuto<MEDFileFieldPerMeshPerTypePerDisc> > > types;
    for (std::vector<MCAuto<MEDFileFieldPerMeshPerTypePerDisc> >::const_iterator it = leaves.begin();
         it != leaves.end();
         it++)
    ...
    std::map<INTERP_KERNEL::NormalizedCellType, std::vector<MCAuto<MEDFileFieldPerMeshPerTypePerDisc> > >::
        const_iterator it1 = types.begin();
    std::vector<MCAuto<MEDFileFieldPerMeshPerTypeCommon> >::iterator it2 = fieldPmPt.begin();
    for (; it1 != types.end(); it1++, it2++)
    {
        MCAuto<MEDFileFieldPerMeshPerType> elt =
            MEDFileFieldPerMeshPerType::New(this, (INTERP_KERNEL::NormalizedCellType)((*it1).second[0]->getLocId()));
        elt->setLeaves((*it1).second);
        MCAuto<MEDFileFieldPerMeshPerTypeCommon> elt2(
            DynamicCast<MEDFileFieldPerMeshPerType, MEDFileFieldPerMeshPerTypeCommon>(elt)
        );
        *it2 = elt2;
    }
...
void
MEDFileFieldPerMesh::changePflsRefsNamesGen(
    const std::vector<std::pair<std::vector<std::string>, std::string> > &mapOfModif
)
{
    for (std::vector<MCAuto<MEDFileFieldPerMeshPerTypeCommon> >::iterator it = _field_pm_pt.begin();
...
void
MEDFileFieldPerMesh::changeLocsRefsNamesGen(
    const std::vector<std::pair<std::vector<std::string>, std::string> > &mapOfModif
)
{
    for (std::vector<MCAuto<MEDFileFieldPerMeshPerTypeCommon> >::iterator it = _field_pm_pt.begin();
...
) const
{
    ...
    {
        std::ostringstream oss;
        oss << "MEDFileFieldPerMesh::getFieldOnMeshAtLevel : " << "The field \"" << nasc.getName()
            << "\" exists but not with such spatial discretization or such dimension specified !";
        throw INTERP_KERNEL::Exception(oss.str());
    }
    //
    std::vector<MCAuto<DataArrayIdType> > notNullPflsPerGeoType2(
        notNullPflsPerGeoType.begin(), notNullPflsPerGeoType.end()
    );
    std::vector<const DataArrayIdType *> notNullPflsPerGeoType3(
        notNullPflsPerGeoType.begin(), notNullPflsPerGeoType.end()
    );
    if (type != ON_NODES)
    {
    ...
    else
    {
        ...
        if (code[2] == -1)
        {
            ...
            {
                std::ostringstream oss;
                oss << "MEDFileFieldPerMesh::getFieldOnMeshAtLevel : There is a problem there is " << nb
                    << " nodes in field whereas there is " << mesh->getNumberOfNodes();
                oss << " nodes in mesh !";
                throw INTERP_KERNEL::Exception(oss.str());
            }
            return finishField(type, glob, dads, locs, mesh, isPfl, arrOut, nasc);
...
) const
{
    ...
    {
        std::ostringstream oss;
        oss << "MEDFileFieldPerMesh::getFieldOnMeshAtLevelWithPfl : " << "The field \"" << nasc.getName()
            << "\" exists but not with such spatial discretization or such dimension specified !";
        throw INTERP_KERNEL::Exception(oss.str());
    }
    std::vector<MCAuto<DataArrayIdType> > notNullPflsPerGeoType2(
...
void
MEDFileFieldPerMesh::accept(MEDFileFieldVisitor &visitor) const
{
    for (std::vector<MCAuto<MEDFileFieldPerMeshPerTypeCommon> >::const_iterator it = _field_pm_pt.begin();
         it != _field_pm_pt.end();
         it++)
...
void
MEDFileFieldPerMesh::getUndergroundDataArrayExt(
    std::vector<std::pair<std::pair<INTERP_KERNEL::NormalizedCellType, int>, std::pair<mcIdType, mcIdType> > > &entries
) const
{
    mcIdType globalSz = 0;
...
MEDFileFieldPerMeshPerTypePerDisc *
MEDFileFieldPerMesh::getLeafGivenTypeAndLocId(INTERP_KERNEL::NormalizedCellType typ, mcIdType locId)
{
    for (std::vector<MCAuto<MEDFileFieldPerMeshPerTypeCommon> >::iterator it = _field_pm_pt.begin();
         it != _field_pm_pt.end();
         it++)
    ...
    std::ostringstream oss;
    oss << "MEDFileFieldPerMesh::getLeafGivenTypeAndLocId : no such geometric type \"" << cm.getRepr() << "\" in this !"
        << std::endl;
...
const MEDFileFieldPerMeshPerTypePerDisc *
MEDFileFieldPerMesh::getLeafGivenTypeAndLocId(INTERP_KERNEL::NormalizedCellType typ, mcIdType locId) const
{
    for (std::vector<MCAuto<MEDFileFieldPerMeshPerTypeCommon> >::const_iterator it = _field_pm_pt.begin();
         it != _field_pm_pt.end();
         it++)
 ...
 * \param [in] dts - (Distribution of types) = level 1 : meshes to aggregate. Level 2 : all geo type. Level 3 pair
 * specifying geo type and number of elem in geotype.
 * \param [out] extractInfo - Gives information about the where the data comes from. It is a vector of triplet. First
 * element in the triplet the mesh pos. The 2nd one the start pos. The 3rd the end pos.
 */
MCAuto<MEDFileFieldPerMeshPerTypePerDisc>
MEDFileFieldPerMeshPerTypePerDisc::Aggregate(
    mcIdType &start,
    const std::vector<std::pair<int, const MEDFileFieldPerMeshPerTypePerDisc *> > &pms,
    const std::vector<std::vector<std::pair<int, mcIdType> > > &dts,
    TypeOfField tof,
    MEDFileFieldPerMeshPerType *father,
    std::vector<std::pair<int, std::pair<mcIdType, mcIdType> > > &extractInfo
)
{
    ...
    for (std::vector<std::pair<int, const MEDFileFieldPerMeshPerTypePerDisc *> >::const_iterator it = pms.begin();
         it != pms.end();
         it++)
    {
        if (!(*it).second)
        ...
        if (!(*it).second->getProfile().empty())
            throw INTERP_KERNEL::Exception(
                "MEDFileFieldPerMeshPerTypePerDisc::Aggregate : not implemented yet for profiles !"
            );
        if (!(*it).second->getLocalization().empty())
            throw INTERP_KERNEL::Exception(
                "MEDFileFieldPerMeshPerTypePerDisc::Aggregate : not implemented yet for gauss pts !"
            );
    }
    INTERP_KERNEL::NormalizedCellType gt(pms[0].second->getGeoType());
    std::size_t i(0);
    std::vector<std::pair<int, int> > filteredDTS;
    for (std::vector<std::vector<std::pair<int, mcIdType> > >::const_iterator it = dts.begin(); it != dts.end();
         it++, i++)
    ...
    if (pms.size() != filteredDTS.size())
        throw INTERP_KERNEL::Exception(
            "MEDFileFieldPerMeshPerTypePerDisc::Aggregate : not implemented yet for generated profiles !"
        );
    ...
    for (; it1 != pms.end(); it1++, it2++)
    {
        if ((*it1).first != (*it2).first)
            throw INTERP_KERNEL::Exception(
                "MEDFileFieldPerMeshPerTypePerDisc::Aggregate : not implemented yet for generated profiles 2 !"
            );
...
MCAuto<MEDFileFieldPerMesh>
MEDFileFieldPerMesh::Aggregate(
    mcIdType &start,
    const std::vector<const MEDFileFieldPerMesh *> &pms,
    const std::vector<std::vector<std::pair<int, mcIdType> > > &dts,
    MEDFileAnyTypeField1TSWithoutSDA *father,
    std::vector<std::pair<int, std::pair<mcIdType, mcIdType> > > &extractInfo
)
{
    MCAuto<MEDFileFieldPerMesh> ret(
        new MEDFileFieldPerMesh(father, pms[0]->getMeshIteration(), pms[0]->getMeshOrder())
    );
    ...
    for (std::vector<const MEDFileFieldPerMesh *>::const_iterator it = pms.begin(); it != pms.end(); it++, i++)
    {
        ...
        for (std::vector<MCAuto<MEDFileFieldPerMeshPerTypeCommon> >::const_iterator it2 = v.begin(); it2 != v.end();
             it2++)
        {
            INTERP_KERNEL::NormalizedCellType gt((*it2)->getGeoType());
            const MEDFileFieldPerMeshPerType *elt(
                dynamic_cast<const MEDFileFieldPerMeshPerType *>((const MEDFileFieldPerMeshPerTypeCommon *)(*it2))
            );
            if (!elt)
    ...
    for (std::map<
             INTERP_KERNEL::NormalizedCellType,
             std::vector<std::pair<int, const MEDFileFieldPerMeshPerType *> > >::const_iterator it = m.begin();
         it != m.end();
         it++)
    {
        MCAuto<MEDFileFieldPerMeshPerType> agg(
            MEDFileFieldPerMeshPerType::Aggregate(start, (*it).second, dts, (*it).first, ret, extractInfo)
        );
        MCAuto<MEDFileFieldPerMeshPerTypeCommon> agg2(
            DynamicCast<MEDFileFieldPerMeshPerType, MEDFileFieldPerMeshPerTypeCommon>(agg)
        );
...
int
MEDFileFieldPerMesh::addNewEntryIfNecessary(INTERP_KERNEL::NormalizedCellType type)
{
    int i = 0;
 ...
 * \param [in] mesh is \b NOT the global mesh, but the possibly reduced mesh. \a mesh parameter will be directly
 * aggregated in the returned field
 */
MEDCouplingFieldDouble *
MEDFileFieldPerMesh::finishField(
    TypeOfField type,
    const MEDFileFieldGlobsReal *glob,
    const std::vector<std::pair<mcIdType, mcIdType> > &dads,
    const std::vector<int> &locs,
...
) const
{
    ...
    if (type == ON_GAUSS_PT)
    {
        ...
        for (std::size_t i = 0; i < nbOfArrs; i++)
        {
            std::vector<std::pair<mcIdType, mcIdType> > dads2(1, dads[i]);
            const std::vector<int> locs2(1, locs[i]);
            ...
            ret->setGaussLocalizationOnCells(
                di->getConstPointer(),
                di->getConstPointer() + nbOfElems,
                fl.getRefCoords(),
                fl.getGaussCoords(),
                fl.getGaussWeights()
            );
...
/*!
 * This method is an extension of MEDFileFieldPerMesh::finishField method. It deals with profiles. This method should be
 * called when type is different from ON_NODES. 'dads', 'locs' and 'geoTypes' input parameters have the same number of
 * elements. No check of this is performed. 'da' array contains an array in old2New style to be applied to mesh to
 * obtain the right support. The order of cells in the returned field is those imposed by the profile.
 * \param [in] mesh is the global mesh.
 */
MEDCouplingFieldDouble *
MEDFileFieldPerMesh::finishField2(
    TypeOfField type,
    const MEDFileFieldGlobsReal *glob,
    const std::vector<std::pair<mcIdType, mcIdType> > &dads,
    const std::vector<int> &locs,
...
/*!
 * This method is the complement of MEDFileFieldPerMesh::finishField2 method except that this method works for node
 * profiles.
 */
MEDCouplingFieldDouble *
MEDFileFieldPerMesh::finishFieldNode2(
    const MEDFileFieldGlobsReal *glob,
    const std::vector<std::pair<mcIdType, mcIdType> > &dads,
    const std::vector<int> &locs,
    const MEDCouplingMesh *mesh,
...
) const
{
    ...
    if (meshu)
    {
        if (meshu->getNodalConnectivity() == 0)
        {
            ...
            for (mcIdType i = 0; i < nb; i++) meshuc->insertNextCell(INTERP_KERNEL::NORM_POINT1, 1, ptr + i);
            meshuc->finishInsertingCells();
            ret->setMesh(meshuc);
            const MEDCouplingFieldDiscretization *disc = ret->getDiscretization();
            if (!disc)
                throw INTERP_KERNEL::Exception(
                    "MEDFileFieldPerMesh::finishFieldNode2 : internal error, no discretization on field !"
                );
    ...
    MCAuto<DataArrayIdType> cellIds =
        mesh->getCellIdsFullyIncludedInNodeIds(da->getConstPointer(), da->getConstPointer() + da->getNbOfElems());
    MCAuto<MEDCouplingMesh> mesh2 = mesh->buildPartAndReduceNodes(
        cellIds->getConstPointer(), cellIds->getConstPointer() + cellIds->getNbOfElems(), arr2
    );
    ...
    {
        std::ostringstream oss;
        oss << "MEDFileFieldPerMesh::finishFieldNode2 : The field on nodes lies on a node profile so that it is "
               "impossible to find a submesh having exactly the same nodes of that profile !!!";
        oss << "So it is impossible to return a well defined MEDCouplingFieldDouble instance on specified mesh on a "
               "specified meshDim !"
            << std::endl;
        oss << "To retrieve correctly such a field you have 3 possibilities :" << std::endl;
        oss << " - use an another meshDim compatible with the field on nodes (MED file does not have such information)"
            << std::endl;
        oss << " - use an another a meshDimRelToMax equal to 1 -> it will return a mesh with artificial cell POINT1 "
               "containing the profile !"
            << std::endl;
        oss << " - if definitely the node profile has no link with mesh connectivity use "
               "MEDFileField1TS::getFieldWithProfile or MEDFileFieldMultiTS::getFieldWithProfile methods instead !";
        throw INTERP_KERNEL::Exception(oss.str());
    }
    return 0;
}
 ...
 */
DataArray *
MEDFileFieldPerMesh::finishField4(
    const std::vector<std::pair<mcIdType, mcIdType> > &dads,
    const DataArrayIdType *pflIn,
    mcIdType nbOfElems,
    DataArrayIdType *&pflOut
) const
{
    if (!pflIn)
    {
    ...
    for (std::size_t i = 0; i < nbOfComp; i++) da->setInfoOnComponent(i, infos[i].c_str());
    safePfl->incrRef();
    return da.retn();
}
...
MFFPMIter *
MFFPMIter::NewCell(const MEDFileEntities *entities)
{
    if (!entities)
    ...
    else
    {
        ...
        if (entities2)
        {
            ...
            const std::vector<std::pair<TypeOfField, INTERP_KERNEL::NormalizedCellType> > &myEnt(
                entities2->getEntries()
            );
            for (std::vector<std::pair<TypeOfField, INTERP_KERNEL::NormalizedCellType> >::const_iterator it =
                     myEnt.begin();
                 it != myEnt.end();
                 it++)
            {
...
bool
MFFPMIter::IsPresenceOfNode(const MEDFileEntities *entities)
{
    if (!entities)
        return true;
    else
...
MEDFileFieldPerMesh::MEDFileFieldPerMesh(
    med_idt fid,
    MEDFileAnyTypeField1TSWithoutSDA *fath,
    int meshCsit,
    int meshIteration,
    ...
    : _mesh_iteration(meshIteration), _mesh_order(meshOrder), _father(fath)
{
    ...
    // for each geometric type inside my mesh, check if there is a field profile ie if the field is defined on this type
    // of cells (whether the discretization is on cells or on gauss_ne) and if this is the case, retrieve the part to be
    // read and build a new MedFileField from it
    for (iter0->begin(); !iter0->finished(); iter0->next())
    {
        med_int nbProfile(MEDfield23nProfile(
            fid,
            nasc.getName().c_str(),
            getIteration(),
            getOrder(),
            MED_CELL,
            typmai[iter0->current()],
            meshCsit + 1,
            meshName,
            pflName,
            locName
        ...
        med_int nbProfile2(MEDfield23nProfile(
            fid,
            nasc.getName().c_str(),
            getIteration(),
            getOrder(),
        ...
        if (nbProfile > 0 || nbProfile2 > 0)
        {
            ...
            if (mmu)
                pd = mmu->getPartDefAtLevel(
                    mmu->getRelativeLevOnGeoType(typmai2[iter0->current()]), typmai2[iter0->current()]
                );
            _field_pm_pt.push_back(
                MEDFileFieldPerMeshPerType::NewOnRead(fid, this, ON_CELLS, typmai2[iter0->current()], nasc, pd)
            );
            if (nbProfile > 0)
                setMeshName(name0);
            else
    ...
    {
        // if there is a profile on nodes for the current field, retrieve the part to be read and build a new
        // MedFileField from it
        med_int nbProfile(MEDfield23nProfile(
            fid,
            nasc.getName().c_str(),
            getIteration(),
        ...
        if (nbProfile > 0)
        {
            ...
            _field_pm_pt.push_back(
                MEDFileFieldPerMeshPerType::NewOnRead(fid, this, ON_NODES, INTERP_KERNEL::NORM_ERROR, nasc, pd)
            );
    ...
    {
        med_int nbPfl(MEDfieldnProfile(
            fid, nasc.getName().c_str(), getIteration(), getOrder(), MED_STRUCT_ELEMENT, *it, pflName, locName
        ));
        if (nbPfl > 0)
        {
    ...
    for (iter0->begin(); !iter0->finished(); iter0->next())
    {
        ...
        {
            _field_pm_pt.push_back(
                MEDFileFieldPerMeshPerType::NewOnRead(fid, this, ON_CELLS, typmai2[iter0->current()], nasc, NULL)
            );
            if (nbProfile > 0)
                setMeshName(name0);
            else
...
MEDFileFieldPerMesh::MEDFileFieldPerMesh(MEDFileAnyTypeField1TSWithoutSDA *fath, const MEDCouplingMesh *mesh)
    : _father(fath)
{
