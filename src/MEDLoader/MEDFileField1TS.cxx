...
void
MEDFileAnyTypeField1TSWithoutSDA::deepCpyLeavesFrom(const MEDFileAnyTypeField1TSWithoutSDA &other)
{
    _field_per_mesh.resize(other._field_per_mesh.size());
    std::size_t i = 0;
    for (std::vector<MCAuto<MEDFileFieldPerMesh> >::const_iterator it = other._field_per_mesh.begin();
         it != other._field_per_mesh.end();
         it++, i++)
    {
...
void
MEDFileAnyTypeField1TSWithoutSDA::accept(MEDFileFieldVisitor &visitor) const
{
    for (std::vector<MCAuto<MEDFileFieldPerMesh> >::const_iterator it = _field_per_mesh.begin();
         it != _field_per_mesh.end();
         it++)
        if ((*it).isNotNull())
        {
 ...
 */
void
MEDFileAnyTypeField1TSWithoutSDA::simpleRepr(int bkOffset, std::ostream &oss, int f1tsId) const
{
    ...
    {
        int i = 0;
        for (std::vector<MCAuto<MEDFileFieldPerMesh> >::const_iterator it2 = _field_per_mesh.begin();
             it2 != _field_per_mesh.end();
             it2++, i++)
        {
...
std::vector<MCAuto<MEDFileAnyTypeField1TSWithoutSDA> >
...
MEDFileAnyTypeField1TSWithoutSDA::MEDFileAnyTypeField1TSWithoutSDA()
    : _iteration(-1), _order(-1), _dt(0.), _csit(-1), _nb_of_tuples_to_be_allocated(-1)
{
}
 ...
 */
int
MEDFileAnyTypeField1TSWithoutSDA::getDimension() const
{
    int ret = -2;
    for (std::vector<MCAuto<MEDFileFieldPerMesh> >::const_iterator it = _field_per_mesh.begin();
         it != _field_per_mesh.end();
...
bool
MEDFileAnyTypeField1TSWithoutSDA::changeMeshNames(const std::vector<std::pair<std::string, std::string> > &modifTab)
{
    bool ret = false;
    for (std::vector<MCAuto<MEDFileFieldPerMesh> >::iterator it = _field_per_mesh.begin(); it != _field_per_mesh.end();
         it++)
    {
 ...
 */
int
MEDFileAnyTypeField1TSWithoutSDA::getMeshIteration() const
{
    if (_field_per_mesh.empty())
 ...
 */
int
MEDFileAnyTypeField1TSWithoutSDA::getMeshOrder() const
{
    if (_field_per_mesh.empty())
 ...
 */
bool
MEDFileAnyTypeField1TSWithoutSDA::isDealingTS(int iteration, int order) const
{
 ...
 */
std::pair<int, int>
MEDFileAnyTypeField1TSWithoutSDA::getDtIt() const
{
    std::pair<int, int> p;
    fillIteration(p);
    return p;
 ...
 */
void
MEDFileAnyTypeField1TSWithoutSDA::fillIteration(std::pair<int, int> &p) const
{
    p.first = _iteration;
    p.second = _order;
}
 ...
 */
void
MEDFileAnyTypeField1TSWithoutSDA::fillTypesOfFieldAvailable(std::vector<TypeOfField> &types) const
{
 ...
 *          of \a this field.
 */
std::vector<TypeOfField>
MEDFileAnyTypeField1TSWithoutSDA::getTypesOfFieldAvailable() const
{
...
MEDFileAnyTypeField1TSWithoutSDA::getPflsReallyUsed2() const
{
...
std::vector<std::string>
MEDFileAnyTypeField1TSWithoutSDA::getLocsReallyUsed2() const
{
    std::vector<std::string> ret;
    std::set<std::string> ret2;
    for (std::vector<MCAuto<MEDFileFieldPerMesh> >::const_iterator it = _field_per_mesh.begin();
...
std::vector<std::string>
MEDFileAnyTypeField1TSWithoutSDA::getPflsReallyUsedMulti2() const
{
    std::vector<std::string> ret;
    for (std::vector<MCAuto<MEDFileFieldPerMesh> >::const_iterator it = _field_per_mesh.begin();
         it != _field_per_mesh.end();
...
std::vector<std::string>
MEDFileAnyTypeField1TSWithoutSDA::getLocsReallyUsedMulti2() const
{
    std::vector<std::string> ret;
    std::set<std::string> ret2;
    for (std::vector<MCAuto<MEDFileFieldPerMesh> >::const_iterator it = _field_per_mesh.begin();
...
void
MEDFileAnyTypeField1TSWithoutSDA::changePflsRefsNamesGen2(
...
void
MEDFileAnyTypeField1TSWithoutSDA::changeLocsRefsNamesGen2(
    const std::vector<std::pair<std::vector<std::string>, std::string> > &mapOfModif
)
{
    for (std::vector<MCAuto<MEDFileFieldPerMesh> >::iterator it = _field_per_mesh.begin(); it != _field_per_mesh.end();
...
MEDFileAnyTypeField1TSWithoutSDA::getFieldSplitedByType(
 ...
 */
int
MEDFileAnyTypeField1TSWithoutSDA::getNonEmptyLevels(const std::string &mname, std::vector<int> &levs) const
{
    levs.clear();
    std::vector<INTERP_KERNEL::NormalizedCellType> types;
    std::vector<std::vector<TypeOfField> > typesF;
...
void
MEDFileAnyTypeField1TSWithoutSDA::convertMedBallIntoClassic()
{
    for (std::vector<MCAuto<MEDFileFieldPerMesh> >::iterator it = _field_per_mesh.begin(); it < _field_per_mesh.end();
         it++)
        if ((*it).isNotNull())
...
void
MEDFileAnyTypeField1TSWithoutSDA::makeReduction(
    INTERP_KERNEL::NormalizedCellType ct, TypeOfField tof, const DataArrayIdType *pfl
)
{
    if (!pfl)
    ...
    if (_field_per_mesh.size() != 1)
        throw INTERP_KERNEL::Exception(
            "MEDFileAnyTypeField1TSWithoutSDA::makeReduction : only single mesh supported !"
        );
    MCAuto<MEDFileFieldPerMesh> fpm(_field_per_mesh[0]);
    if (fpm.isNull())
        throw INTERP_KERNEL::Exception(
            "MEDFileAnyTypeField1TSWithoutSDA::makeReduction : only single not null mesh supported !"
        );
    ...
    MCAuto<DataArray> arr0(arr->selectByTupleIdSafeSlice(0, s, 1)), arr1(arr->selectByTupleIdSafeSlice(s, e, 1)),
        arr2(arr->selectByTupleIdSafeSlice(e, nt2, 1));
...
/*!
 * \param [in] mName specifies the underlying mesh name. This value can be pointer 0 for users that do not deal with
 * fields on multi mesh.
 * \param [in] typ is for the geometric cell type (or INTERP_KERNEL::NORM_ERROR for node field) entry to find the right
 * MEDFileFieldPerMeshPerTypePerDisc instance to set.
 * \param [in] locId is the localization id to find the right MEDFileFieldPerMeshPerTypePerDisc instance to set. It
 * corresponds to the position of
 *             \c pfls[std::distance(types.begin(),std::find(types.begin(),typ)] vector in
 * MEDFileField1TSWithoutSDA::getFieldSplitedByType. For non gausspoints field users, the value is 0.
 */
MEDFileFieldPerMeshPerTypePerDisc *
MEDFileAnyTypeField1TSWithoutSDA::getLeafGivenMeshAndTypeAndLocId(
    const std::string &mName, INTERP_KERNEL::NormalizedCellType typ, int locId
)
{
    if (_field_per_mesh.empty())
        throw INTERP_KERNEL::Exception(
            "MEDFileAnyTypeField1TSWithoutSDA::getLeafGivenMeshAndTypeAndLocId : This is empty !"
        );
 ...
 */
int
MEDFileAnyTypeField1TSWithoutSDA::getMeshIdFromMeshName(const std::string &mName) const
{
    if (_field_per_mesh.empty())
    ...
    oss << "MEDFileField1TSWithoutSDA::getMeshIdFromMeshName : No such mesh \"" << mName2
        << "\" as underlying mesh of field \"" << getName() << "\" !\n";
...
int
MEDFileAnyTypeField1TSWithoutSDA::addNewEntryIfNecessary(const MEDCouplingMesh *mesh)
{
    if (!mesh)
        throw INTERP_KERNEL::Exception(
            "MEDFileAnyTypeField1TSWithoutSDA::addNewEntryIfNecessary : input mesh is NULL !"
        );
    std::string tmp(mesh->getName());
    if (tmp.empty())
        throw INTERP_KERNEL::Exception(
            "MEDFileField1TSWithoutSDA::addNewEntryIfNecessary : empty mesh name ! unsupported by MED file !"
        );
    setMeshName(tmp);
...
bool
MEDFileAnyTypeField1TSWithoutSDA::renumberEntitiesLyingOnMesh(
...
/*!
 * This method splits \a this into several sub-parts so that each sub parts have exactly one spatial discretization.
 * This method implements the minimal splitting that leads to single spatial discretization of this.
 *
...
MEDFileAnyTypeField1TSWithoutSDA::splitDiscretizations() const
{
    ...
    std::vector<std::vector<TypeOfField> > typesF;
    std::vector<std::vector<std::string> > pfls, locs;
    std::vector<std::vector<std::pair<mcIdType, mcIdType> > > bgEnd(
        getFieldSplitedByType(getMeshName().c_str(), types, typesF, pfls, locs)
    );
...
/*!
 * This method performs a sub splitting as splitDiscretizations does but finer. This is the finest splitting level that
 * can be done. This method implements the minimal splitting so that each returned elements are mono Gauss
 * discretization per geometric type.
 *
...
MEDFileAnyTypeField1TSWithoutSDA::splitMultiDiscrPerGeoTypes() const
{
    ...
    if (allEnt.size() != 1)
        throw INTERP_KERNEL::Exception(
            "MEDFileAnyTypeField1TSWithoutSDA::splitMultiDiscrPerGeoTypes : this field is expected to be defined only "
            "on one spatial discretization !"
        );
    if (nbOfMDPGT == 0)
    ...
    if (nbOfMDPGT == 1)
    {
        ...
        ret0[0] = const_cast<MEDFileAnyTypeField1TSWithoutSDA *>(this);
        this->incrRef();
        return ret0;
    }
...
mcIdType
MEDFileAnyTypeField1TSWithoutSDA::keepOnlySpatialDiscretization(
    TypeOfField tof, std::vector<std::pair<mcIdType, mcIdType> > &its
)
{
...
mcIdType
MEDFileAnyTypeField1TSWithoutSDA::keepOnlyGaussDiscretization(
    std::size_t idOfDisc, std::vector<std::pair<mcIdType, mcIdType> > &its
)
{
    mcIdType globalCounter(0);
    for (std::vector<MCAuto<MEDFileFieldPerMesh> >::iterator it = _field_per_mesh.begin(); it != _field_per_mesh.end();
...
void
MEDFileAnyTypeField1TSWithoutSDA::updateData(
    mcIdType newLgth, const std::vector<std::pair<mcIdType, mcIdType> > &oldStartStops
)
{
    if (_nb_of_tuples_to_be_allocated >= 0)
    {
        ...
        if (oldArr)
        {
            ...
            setArray(newArr);
            _nb_of_tuples_to_be_allocated =
                newLgth;  // force the _nb_of_tuples_to_be_allocated because setArray has been used specialy
        }
        return;
    }
    ...
    if (_nb_of_tuples_to_be_allocated == -2 || _nb_of_tuples_to_be_allocated == -3)
    {
        ...
        mcIdType pos = 0;
        for (std::vector<std::pair<mcIdType, mcIdType> >::const_iterator it = oldStartStops.begin();
             it != oldStartStops.end();
             it++)
        {
            if ((*it).second < (*it).first)
                throw INTERP_KERNEL::Exception(
                    "MEDFileAnyTypeField1TSWithoutSDA::updateData : the range in the leaves was invalid !"
                );
...
void
MEDFileAnyTypeField1TSWithoutSDA::writeLL(
    med_idt fid, const MEDFileWritable &opts, const MEDFileFieldNameScope &nasc
) const
{
    if (_field_per_mesh.empty())
    ...
    if (_field_per_mesh.size() > 1)
        throw INTERP_KERNEL::Exception(
            "MEDFileField1TSWithoutSDA::writeLL : In MED3.0 mode in writing mode only ONE underlying mesh supported !"
        );
...
/*!
 * MED file does not support ' ' at the end of the field name. This method corrects the possibly invalid input \a
 * nonCorrectFieldName to a correct one by right stripping input.
 */
std::string
MEDFileAnyTypeField1TSWithoutSDA::FieldNameToMEDFileConvention(const std::string &nonCorrectFieldName)
{
...
/*!
 * This methods returns true is the allocation has been needed leading to a modification of state in \a
 * this->_nb_of_tuples_to_be_allocated. If false is returned the memory allocation is not required.
 */
bool
MEDFileAnyTypeField1TSWithoutSDA::allocIfNecessaryTheArrayToReceiveDataFromFile()
{
    if (_nb_of_tuples_to_be_allocated >= 0)
    {
    ...
    if (_nb_of_tuples_to_be_allocated == -1)
        throw INTERP_KERNEL::Exception(
            "MEDFileAnyTypeField1TSWithoutSDA::allocIfNecessaryTheArrayToReceiveDataFromFile : trying to read from a "
            "file an empty instance ! Need to prepare the structure before !"
        );
    if (_nb_of_tuples_to_be_allocated < -3)
        throw INTERP_KERNEL::Exception(
            "MEDFileAnyTypeField1TSWithoutSDA::allocIfNecessaryTheArrayToReceiveDataFromFile : internal error !"
        );
    throw INTERP_KERNEL::Exception(
        "MEDFileAnyTypeField1TSWithoutSDA::allocIfNecessaryTheArrayToReceiveDataFromFile : internal error !"
    );
...
void
MEDFileAnyTypeField1TSWithoutSDA::loadOnlyStructureOfDataRecursively(
    med_idt fid,
    const MEDFileFieldNameScope &nasc,
    const MEDFileMeshes *ms,
    const MEDFileEntities *entities,
    const MEDFileCapability *capability
)
{
    med_int numdt, numit;
    med_float dt;
    ...
    if (!capability || !capability->isFastReader())
    {
        ...
        MEDFILESAFECALLERRD0(
            ...
            (fid,
             ...
             &meshnumdt,
             &meshnumit)
        );  // to check with Adrien for legacy MED files
    }
    if (_iteration != numdt || _order != numit)
        throw INTERP_KERNEL::Exception(
            "MEDFileAnyTypeField1TSWithoutSDA::loadBigArraysRecursively : unexpected exception internal error !"
        );
    ...
    //
    _field_per_mesh[0] = MEDFileFieldPerMesh::NewOnRead(
        fid, this, 0, FromMedInt<int>(meshnumdt), FromMedInt<int>(meshnumit), nasc, mm, entities
    );
...
)
{
    ...
    _field_per_mesh.resize(1);
    _field_per_mesh[0] = MEDFileFieldPerMesh::NewOnRead(
        fid, this, 0, FromMedInt<int>(meshnumdt), FromMedInt<int>(meshnumit), nasc, pd, entities
    );
    _nb_of_tuples_to_be_allocated = 0;
    _field_per_mesh[0]->loadOnlyStructureOfDataRecursively(fid, _nb_of_tuples_to_be_allocated, nasc);
}
...
void
MEDFileAnyTypeField1TSWithoutSDA::loadBigArraysRecursively(med_idt fid, const MEDFileFieldNameScope &nasc)
{
...
void
MEDFileAnyTypeField1TSWithoutSDA::loadBigArraysRecursivelyIfNecessary(med_idt fid, const MEDFileFieldNameScope &nasc)
{
    if (allocIfNecessaryTheArrayToReceiveDataFromFile())
        for (std::vector<MCAuto<MEDFileFieldPerMesh> >::iterator it = _field_per_mesh.begin();
             it != _field_per_mesh.end();
             it++)
            (*it)->loadBigArraysRecursively(fid, nasc);
}
...
void
MEDFileAnyTypeField1TSWithoutSDA::loadStructureAndBigArraysRecursively(
    med_idt fid, const MEDFileFieldNameScope &nasc, const MEDFileMeshes *ms, const MEDFileEntities *entities
)
{
...
void
MEDFileAnyTypeField1TSWithoutSDA::loadStructureAndBigArraysRecursively(
    med_idt fid, const MEDFileFieldNameScope &nasc, const PartDefinition *pd, const MEDFileEntities *entities
)
{
    MEDFileCapability cap(fid);
...
void
MEDFileAnyTypeField1TSWithoutSDA::unloadArrays()
{
...
std::size_t
MEDFileAnyTypeField1TSWithoutSDA::getHeapMemorySizeWithoutChildren() const
{
    return _mesh_name.capacity() + _dt_unit.capacity() +
           _field_per_mesh.capacity() * sizeof(MCAuto<MEDFileFieldPerMesh>);
}

std::vector<const BigMemoryObject *>
MEDFileAnyTypeField1TSWithoutSDA::getDirectChildrenWithNull() const
{
 ...
 */
void
MEDFileAnyTypeField1TSWithoutSDA::setFieldNoProfileSBT(
    const TimeHolder *th,
 ...
 */
void
MEDFileAnyTypeField1TSWithoutSDA::setFieldProfile(
    const TimeHolder *th,
    const MEDCouplingFieldTemplate *field,
...
)
{
    ...
    if (!arrOfVals || !arrOfVals->isAllocated())
        throw INTERP_KERNEL::Exception(
            "MEDFileAnyTypeField1TSWithoutSDA::setFieldProfile : input array is null or not allocated !"
        );
    TypeOfField type = field->getTypeOfField();
    ...
    if (type != ON_NODES)
    {
        ...
        std::vector<MCAuto<DataArrayIdType> > idsInPflPerType2(idsInPflPerType.size());
        std::copy(idsInPflPerType.begin(), idsInPflPerType.end(), idsInPflPerType2.begin());
        std::vector<MCAuto<DataArrayIdType> > idsPerType2(idsPerType.size());
        std::copy(idsPerType.begin(), idsPerType.end(), idsPerType2.begin());
        std::vector<const DataArrayIdType *> idsPerType3(idsPerType.size());
        std::copy(idsPerType.begin(), idsPerType.end(), idsPerType3.begin());
        // start of check
        ...
        {
            std::ostringstream oss;
            oss << "MEDFileAnyTypeField1TSWithoutSDA::setFieldProfile : The array is expected to have " << nbOfTuplesExp
                << " tuples ! It has " << arrOfVals->getNumberOfTuples() << " !";
            throw INTERP_KERNEL::Exception(oss.str());
        }
        // end of check
        ...
        _field_per_mesh[pos]->assignFieldProfile(
            start, profile, code, code2, idsInPflPerType, idsPerType, field, arrOfVals, m, glob, nasc
        );
    }
    else
    {
        if (!profile || !profile->isAllocated() || profile->getNumberOfComponents() != 1)
            throw INTERP_KERNEL::Exception(
                "MEDFileAnyTypeField1TSWithoutSDA::setFieldProfile : input profile is null, not allocated or with "
                "number of components != 1 !"
            );
        std::vector<mcIdType> v(3);
        v[0] = -1;
        v[1] = profile->getNumberOfTuples();
        v[2] = 0;
        std::vector<const DataArrayIdType *> idsPerType3(1);
        idsPerType3[0] = profile;
        mcIdType nbOfTuplesExp = field->getNumberOfTuplesExpectedRegardingCode(v, idsPerType3);
        if (nbOfTuplesExp != arrOfVals->getNumberOfTuples())
        {
            std::ostringstream oss;
            oss << "MEDFileAnyTypeField1TSWithoutSDA::setFieldProfile : For node field, the array is expected to have "
                << nbOfTuplesExp << " tuples ! It has " << arrOfVals->getNumberOfTuples() << " !";
            throw INTERP_KERNEL::Exception(oss.str());
        }
        mcIdType start(copyTinyInfoFrom(th, field, arrOfVals));
        mcIdType pos(addNewEntryIfNecessary(m));
 ...
 */
void
MEDFileAnyTypeField1TSWithoutSDA::allocNotFromFile(mcIdType newNbOfTuples)
{
    if (_nb_of_tuples_to_be_allocated >= 0)
        throw INTERP_KERNEL::Exception(
            "MEDFileAnyTypeField1TSWithoutSDA::allocNotFromFile : the object is expected to be appended to a data "
            "coming from a file but not loaded ! Load before appending data !"
        );
 ...
 */
mcIdType
MEDFileAnyTypeField1TSWithoutSDA::copyTinyInfoFrom(
    const TimeHolder *th, const MEDCouplingFieldTemplate *field, const DataArray *arr
)
{
    if (!field)
    ...
    if (name.empty())
        throw INTERP_KERNEL::Exception(
            "MEDFileField1TSWithoutSDA::copyTinyInfoFrom : unsupported fields with no name in MED file !"
        );
    if (!arr)
...
void
MEDFileAnyTypeField1TSWithoutSDA::writeDescription(med_idt fid, const MEDFileWritable &opts) const
{
...
void
MEDFileAnyTypeField1TSWithoutSDA::readDescription(med_idt fid, const MEDFileWritable &opts)
{
    INTERP_KERNEL::AutoPtr<char> keyDesc = MEDLoaderBase::buildEmptyString(MED_NAME_SIZE);
    MEDLoaderBase::safeStrCpy(getName().c_str(), MED_NAME_SIZE, keyDesc, opts.getTooLongStrPolicy());
 ...
 */
std::size_t
MEDFileAnyTypeField1TSWithoutSDA::getNumberOfComponents() const
{
 ...
 */
void
MEDFileAnyTypeField1TSWithoutSDA::setInfo(const std::vector<std::string> &infos)
{
    DataArray *arr = getOrCreateAndGetArray();
...
MEDFileAnyTypeField1TSWithoutSDA::getInfo() const
{
 ...
 */
std::vector<std::string> &
MEDFileAnyTypeField1TSWithoutSDA::getInfo()
{
    DataArray *arr = getOrCreateAndGetArray();
    return arr->getInfoOnComponents();
}
...
bool
MEDFileAnyTypeField1TSWithoutSDA::presenceOfMultiDiscPerGeoType() const
{
    for (std::vector<MCAuto<MEDFileFieldPerMesh> >::const_iterator it = _field_per_mesh.begin();
         it != _field_per_mesh.end();
         it++)
...
bool
MEDFileAnyTypeField1TSWithoutSDA::presenceOfStructureElements() const
{
    for (std::vector<MCAuto<MEDFileFieldPerMesh> >::const_iterator it = _field_per_mesh.begin();
         it != _field_per_mesh.end();
         it++)
...
bool
MEDFileAnyTypeField1TSWithoutSDA::onlyStructureElements() const
{
    for (std::vector<MCAuto<MEDFileFieldPerMesh> >::const_iterator it = _field_per_mesh.begin();
         it != _field_per_mesh.end();
         it++)
...
void
MEDFileAnyTypeField1TSWithoutSDA::killStructureElements()
{
    for (std::vector<MCAuto<MEDFileFieldPerMesh> >::iterator it = _field_per_mesh.begin(); it != _field_per_mesh.end();
         it++)
        if ((*it).isNotNull())
...
void
MEDFileAnyTypeField1TSWithoutSDA::keepOnlyStructureElements()
{
    for (std::vector<MCAuto<MEDFileFieldPerMesh> >::iterator it = _field_per_mesh.begin(); it != _field_per_mesh.end();
         it++)
        if ((*it).isNotNull())
...
void
MEDFileAnyTypeField1TSWithoutSDA::keepOnlyOnSE(const std::string &seName)
{
    for (std::vector<MCAuto<MEDFileFieldPerMesh> >::iterator it = _field_per_mesh.begin(); it != _field_per_mesh.end();
         it++)
        if ((*it).isNotNull())
...
void
MEDFileAnyTypeField1TSWithoutSDA::getMeshSENames(std::vector<std::pair<std::string, std::string> > &ps) const
{
    for (std::vector<MCAuto<MEDFileFieldPerMesh> >::const_iterator it = _field_per_mesh.begin();
         it != _field_per_mesh.end();
         it++)
...
MEDFileAnyTypeField1TSWithoutSDA::fieldOnMesh(
...
) const
{
    static const char MSG0[] =
        "MEDFileAnyTypeField1TSWithoutSDA::fieldOnMesh : the field is too complex to be able to be extracted with  "
        "\"field\" method ! Call getFieldOnMeshAtLevel method instead to deal with complexity !";
    if (_field_per_mesh.empty())
        throw INTERP_KERNEL::Exception(
            "MEDFileAnyTypeField1TSWithoutSDA::fieldOnMesh : the field is empty ! Nothing to extract !"
        );
    if (_field_per_mesh.size() > 1)
    ...
    else
    {
        ...
        if (meshDimRelToMax > 0)
            throw INTERP_KERNEL::Exception(
                "MEDFileAnyTypeField1TSWithoutSDA::fieldOnMesh : the mesh attached to field is not compatible with the "
                "field !"
            );
    }
    return MEDFileAnyTypeField1TSWithoutSDA::getFieldOnMeshAtLevel(
        type, meshDimRelToMax, 0 /*renumPol*/, glob, mesh, arrOut, nasc
    );
}
 ...
 */
MEDCouplingFieldDouble *
MEDFileAnyTypeField1TSWithoutSDA::getFieldAtLevel(
    TypeOfField type,
    int meshDimRelToMax,
    const std::string &mName,
    int renumPol,
...
) const
{
    ...
    return MEDFileAnyTypeField1TSWithoutSDA::getFieldOnMeshAtLevel(
        type, meshDimRelToMax, renumPol, glob, mm, arrOut, nasc
    );
}
 ...
 */
MEDCouplingFieldDouble *
MEDFileAnyTypeField1TSWithoutSDA::getFieldOnMeshAtLevel(
    TypeOfField type,
    int meshDimRelToMax,
    int renumPol,
    const MEDFileFieldGlobsReal *glob,
 ...
 */
MEDCouplingFieldDouble *
MEDFileAnyTypeField1TSWithoutSDA::getFieldAtTopLevel(
    TypeOfField type,
    const std::string &mName,
    int renumPol,
    const MEDFileFieldGlobsReal *glob,
...
MEDFileAnyTypeField1TSWithoutSDA::getFieldOnMeshAtLevel(
    ...
    const DataArrayIdType *cellRenum,
    const DataArrayIdType *nodeRenum,
    MCAuto<DataArray> &arrOut,
    const MEDFileFieldNameScope &nasc
) const
{
    static const char msg1[] =
        "MEDFileField1TSWithoutSDA::getFieldOnMeshAtLevel : request for a renumbered field following mesh numbering "
        "whereas it is a profile field !";
    bool isPfl = false;
    ...
    switch (renumPol)
    {
        ...
        case 1:
        {
            ...
            if (cellRenum)
            {
                ...
                {
                    std::ostringstream oss;
                    oss << "MEDFileField1TSWithoutSDA::getFieldOnMeshAtLevel : Request of simple renumbering but it "
                           "seems that underlying mesh \""
                        << mesh->getName() << "\" of requested field ";
                ...
                if (!disc)
                    throw INTERP_KERNEL::Exception(
                        "MEDFileAnyTypeField1TSWithoutSDA::getFieldOnMeshAtLevel : internal error, no discretization "
                        "on field !"
                    );
        ...
        case 2:
        {
            ...
            if (nodeRenum)
            {
                ...
                if (!dynamic_cast<DataArrayDouble *>((DataArray *)arrOut))
                    throw INTERP_KERNEL::Exception(
                        "MEDFileField1TSWithoutSDA::getFieldOnMeshAtLevel : node renumbering not implemented for not "
                        "double DataArrays !"
                    );
        ...
        default:
            throw INTERP_KERNEL::Exception(
                "MEDFileField1TSWithoutSDA::getFieldOnMeshAtLevel : unsupported renum policy ! Dealing with policy 0 1 "
                "2 and 3 !"
            );
    }
    return nullptr;
 ...
 */
void
MEDFileField1TSWithoutSDA::CheckMeshDimRel(int meshDimRelToMax)
{
    if (meshDimRelToMax > 0)
        throw INTERP_KERNEL::Exception(
            "CheckMeshDimRel : This is a meshDimRel not a meshDimRelExt ! So value should be <=0 !"
        );
}
 ...
 */
std::vector<mcIdType>
MEDFileField1TSWithoutSDA::CheckSBTMesh(const MEDCouplingMesh *mesh)
{
    if (!mesh)
    ...
    if (da)
    {
        da->decrRef();
        throw INTERP_KERNEL::Exception(
            "MEDFileField1TSWithoutSDA::CheckSBTMesh : underlying mesh is not sorted by type as MED file expects !"
        );
    }
    return code;
}
 ...
 *  \throw If no field is lying on \a mname.
 */
std::vector<std::vector<DataArrayDouble *> >
MEDFileField1TSWithoutSDA::getFieldSplitedByType2(
    const std::string &mname,
    std::vector<INTERP_KERNEL::NormalizedCellType> &types,
    std::vector<std::vector<TypeOfField> > &typesF,
    std::vector<std::vector<std::string> > &pfls,
...
) const
{
    if (mname.empty())
        if (_field_per_mesh.empty())
            throw INTERP_KERNEL::Exception("MEDFileField1TSWithoutSDA::getFieldSplitedByType : This is empty !");
    std::vector<std::vector<std::pair<mcIdType, mcIdType> > > ret0 =
        _field_per_mesh[0]->getFieldSplitedByType(types, typesF, pfls, locs);
...
const char *
MEDFileField1TSWithoutSDA::getTypeStr() const
{
    return TYPE_STR;
}

MEDFileInt32Field1TSWithoutSDA *
MEDFileField1TSWithoutSDA::convertToInt() const
{
...
MEDFileField1TSWithoutSDA::convertToInt64() const
{
 ...
 */
DataArray *
MEDFileField1TSWithoutSDA::getUndergroundDataArrayExt(
...
const char *
MEDFileInt32Field1TSWithoutSDA::getTypeStr() const
{
    return TYPE_STR;
}
 ...
 */
DataArray *
MEDFileInt32Field1TSWithoutSDA::getUndergroundDataArrayExt(
    std::vector<std::pair<std::pair<INTERP_KERNEL::NormalizedCellType, int>, std::pair<mcIdType, mcIdType> > > &entries
) const
{
...
) const
{
    if (_field_per_mesh.size() != 1)
        throw INTERP_KERNEL::Exception(
            "MEDFileField1TSWithoutSDA::getUndergroundDataArrayExt : field lies on several meshes, this method has no "
            "sense !"
        );
...
const char *
MEDFileInt64Field1TSWithoutSDA::getTypeStr() const
{
    return TYPE_STR;
}
 ...
 */
DataArray *
MEDFileInt64Field1TSWithoutSDA::getUndergroundDataArrayExt(
    std::vector<std::pair<std::pair<INTERP_KERNEL::NormalizedCellType, int>, std::pair<mcIdType, mcIdType> > > &entries
) const
{
    return getUndergroundDataArrayIntExt(entries);
 ...
 */
DataArrayInt64 *
MEDFileInt64Field1TSWithoutSDA::getUndergroundDataArrayIntExt(
    std::vector<std::pair<std::pair<INTERP_KERNEL::NormalizedCellType, int>, std::pair<mcIdType, mcIdType> > > &entries
) const
{
    if (_field_per_mesh.size() != 1)
...
const char *
MEDFileFloatField1TSWithoutSDA::getTypeStr() const
{
    return TYPE_STR;
}
 ...
 */
DataArray *
MEDFileFloatField1TSWithoutSDA::getUndergroundDataArrayExt(
    std::vector<std::pair<std::pair<INTERP_KERNEL::NormalizedCellType, int>, std::pair<mcIdType, mcIdType> > > &entries
) const
{
 ...
 */
DataArrayFloat *
MEDFileFloatField1TSWithoutSDA::getUndergroundDataArrayFloatExt(
    std::vector<std::pair<std::pair<INTERP_KERNEL::NormalizedCellType, int>, std::pair<mcIdType, mcIdType> > > &entries
) const
{
    if (_field_per_mesh.size() != 1)
...
MEDFileAnyTypeField1TSWithoutSDA *
MEDFileAnyTypeField1TS::AllocateContentFrom(med_idt fid, const std::string &fieldName, int iteration, int order)
{
    ...
    {
        std::ostringstream oss;
        oss << "MEDFileField1TS(fid,fName) : file \'" << FileNameFromFID(fid) << "\' contains field with name \'"
            << fName << "\' but there is no time steps on it !";
        throw INTERP_KERNEL::Exception(oss.str());
    }
    ...
    switch (typcha)
    {
        ...
        {
            std::ostringstream oss;
            oss << "MEDFileAnyTypeField1TS::AllocateContentFrom(fid,fName,iteration,order) : file \'"
                << FileNameFromFID(fid) << "\' contains field with name \'" << fName
                << "\' but the type of field is not in [MED_FLOAT64, MED_INT32, MED_FLOAT32, MED_INT64] !";
            throw INTERP_KERNEL::Exception(oss.str());
        }
    }
    ...
    {
        std::ostringstream oss;
        oss << "No such iteration (" << iteration << "," << order << ") in existing field '" << fName << "' in file '"
            << FileNameFromFID(fid) << "' ! Available iterations are : ";
...
MEDFileAnyTypeField1TSWithoutSDA *
MEDFileAnyTypeField1TS::BuildContentFrom(
    med_idt fid, bool loadAll, const MEDFileMeshes *ms, const MEDFileEntities *entities
)
{
    std::string useless;
...
MEDFileAnyTypeField1TSWithoutSDA *
MEDFileAnyTypeField1TS::BuildContentFrom(
    med_idt fid, const std::string &fieldName, bool loadAll, const MEDFileMeshes *ms, const MEDFileEntities *entities
)
{
...
MEDFileAnyTypeField1TS *
MEDFileAnyTypeField1TS::BuildNewInstanceFromContent(MEDFileAnyTypeField1TSWithoutSDA *c)
{
    if (!c)
        throw INTERP_KERNEL::Exception(
            "MEDFileAnyTypeField1TS::BuildNewInstanceFromContent : empty content in input : unable to build a new "
            "instance !"
        );
    ...
    throw INTERP_KERNEL::Exception(
        "MEDFileAnyTypeField1TS::BuildNewInstanceFromContent : internal error ! a content of type different from "
        "FLOAT64 FLOAT32 and INT32 has been built but not intercepted !"
    );
}

MEDFileAnyTypeField1TS *
MEDFileAnyTypeField1TS::BuildNewInstanceFromContent(MEDFileAnyTypeField1TSWithoutSDA *c, med_idt fid)
{
...
MEDFileAnyTypeField1TS *
MEDFileAnyTypeField1TS::New(const std::string &fileName, bool loadAll)
{
...
MEDFileAnyTypeField1TS *
MEDFileAnyTypeField1TS::New(med_idt fid, bool loadAll)
{
...
MEDFileAnyTypeField1TS *
MEDFileAnyTypeField1TS::New(const std::string &fileName, const std::string &fieldName, bool loadAll)
{
    MEDFileUtilities::AutoFid fid(OpenMEDFileForRead(fileName));
...
MEDFileAnyTypeField1TS *
MEDFileAnyTypeField1TS::New(med_idt fid, const std::string &fieldName, bool loadAll)
{
...
MEDFileAnyTypeField1TS *
MEDFileAnyTypeField1TS::New(
    const std::string &fileName, const std::string &fieldName, int iteration, int order, bool loadAll
)
{
    MEDFileUtilities::AutoFid fid(OpenMEDFileForRead(fileName));
...
MEDFileAnyTypeField1TS *
MEDFileAnyTypeField1TS::New(med_idt fid, const std::string &fieldName, int iteration, int order, bool loadAll)
{
    MCAuto<MEDFileAnyTypeField1TSWithoutSDA> c(
        BuildContentFrom(fid, fieldName, iteration, order, loadAll, (const MEDFileMeshes *)0, 0)
    );
    MCAuto<MEDFileAnyTypeField1TS> ret(BuildNewInstanceFromContent(c, fid));
    ret->loadGlobals(fid);
    return ret.retn();
...
MEDFileAnyTypeField1TS *
MEDFileAnyTypeField1TS::NewAdv(
    med_idt fid, const std::string &fieldName, int iteration, int order, bool loadAll, const MEDFileEntities *entities
)
{
    MCAuto<MEDFileAnyTypeField1TSWithoutSDA> c(
        BuildContentFrom(fid, fieldName, iteration, order, loadAll, (const MEDFileMeshes *)0, entities)
    );
    MCAuto<MEDFileAnyTypeField1TS> ret(BuildNewInstanceFromContent(c, fid));
    ret->loadGlobals(fid);
    return ret.retn();
...
/*!
 * This constructor is a shallow copy constructor. If \a shallowCopyOfContent is true the content of \a other is shallow
 * copied. If \a shallowCopyOfContent is false, \a other is taken to be the content of \a this.
 *
...
int
MEDFileAnyTypeField1TS::LocateField2(
    med_idt fid,
    int fieldIdCFormat,
    bool checkFieldId,
...
)
{
    if (checkFieldId)
    {
        ...
        {
            std::ostringstream oss;
            oss << "MEDFileAnyTypeField1TS::LocateField2(fileName) : in file \'" << FileNameFromFID(fid)
                << "\' number of fields is " << nbFields << " ! Trying to request for id " << fieldIdCFormat << " !";
            throw INTERP_KERNEL::Exception(oss.str());
        }
    }
    ...
    for (int j = 0; j < ncomp; j++)
        infos[j] = MEDLoaderBase::buildUnionUnit(
            (char *)comp + j * MED_SNAME_SIZE, MED_SNAME_SIZE, (char *)unit + j * MED_SNAME_SIZE, MED_SNAME_SIZE
        );
    return FromMedInt<int>(nbOfStep);
}

/*!
 * This method throws an INTERP_KERNEL::Exception if \a fieldName field is not in file pointed by \a fid and with name
 * \a fileName.
 *
 * \return in case of success the number of time steps available for the field with name \a fieldName.
 */
int
MEDFileAnyTypeField1TS::LocateField(
    med_idt fid,
    const std::string &fieldName,
    int &posCFormat,
    med_field_type &typcha,
...
)
{
    ...
    {
        std::ostringstream oss;
        oss << "No such field '" << fieldName << "' in file '" << FileNameFromFID(fid) << "' ! Available fields are : ";
...
/*!
 * This method as MEDFileField1TSW::setLocNameOnLeaf, is dedicated for advanced user that a want a very fine control on
 * their data structure without overhead. This method can be called only regarding information returned by
 * MEDFileField1TSWithoutSDA::getFieldSplitedByType or MEDFileField1TSWithoutSDA::getFieldSplitedByType2. This method
 * changes the attribute (here it's profile name) of the leaf datastructure (MEDFileFieldPerMeshPerTypePerDisc
 * instance). It is the responsibility of the caller to invoke MEDFileFieldGlobs::appendProfile or
 * MEDFileFieldGlobs::getProfile to keep a valid instance. If \b this do not have any leaf that correspond to the
 * request of the input parameter (\b mName, \b typ, \b locId) an INTERP_KERNEL::Exception will be thrown. If \b
 * newPflName profile name does not already exist the profile with old name will be renamed with name \b newPflName. If
 * \b newPflName already exists and that \b forceRenameOnGlob is false (the default) an INTERP_KERNEL::Exception will be
 * thrown to avoid big confusion. In this case the called should rename before the profile name with name \b newPflName.
 *
 * \param [in] mName specifies the underlying mesh name. This value can be pointer 0 for users that do not deal with
 * fields on multi mesh.
 * \param [in] typ is for the geometric cell type (or INTERP_KERNEL::NORM_ERROR for node field) entry to find the right
 ...
 * \param [in] forceRenameOnGlob specifies the behaviour in case of profile \b newPflName already exists. If true, the
 * renaming is done without check. It can lead to major bug. If false, an exception will be thrown to force user to
 * change previously the name of the profile with name \b newPflName
 */
void
MEDFileAnyTypeField1TS::setProfileNameOnLeaf(
    const std::string &mName,
    INTERP_KERNEL::NormalizedCellType typ,
    int locId,
    const std::string &newPflName,
    bool forceRenameOnGlob
)
{
    ...
    {
        std::ostringstream oss;
        oss << "MEDFileField1TS::setProfileNameOnLeaf : Profile \"" << newPflName
            << "\" already exists or referenced more than one !";
        throw INTERP_KERNEL::Exception(oss.str());
    }
}

/*!
 * This method as MEDFileField1TSW::setProfileNameOnLeaf, is dedicated for advanced user that a want a very fine control
 * on their data structure without overhead. This method can be called only regarding information returned by
 * MEDFileField1TSWithoutSDA::getFieldSplitedByType or MEDFileField1TSWithoutSDA::getFieldSplitedByType2. This method
 * changes the attribute (here it's localization name) of the leaf datastructure (MEDFileFieldPerMeshPerTypePerDisc
 * instance). It is the responsibility of the caller to invoke MEDFileFieldGlobs::appendProfile or
 * MEDFileFieldGlobs::getProfile to keep a valid instance. If \b this do not have any leaf that correspond to the
 * request of the input parameter (\b mName, \b typ, \b locId) an INTERP_KERNEL::Exception will be thrown. This method
 * is an extension of MEDFileField1TSWithoutSDA::setProfileNameOnLeafExt method because it performs a modification of
 * global info. If \b newLocName profile name does not already exist the localization with old name will be renamed with
 * name \b newLocName. If \b newLocName already exists an INTERP_KERNEL::Exception will be thrown to avoid big
 * confusion. In this case the called should rename before the profile name with name \b newLocName.
 *
 * \param [in] mName specifies the underlying mesh name. This value can be pointer 0 for users that do not deal with
 * fields on multi mesh.
 * \param [in] typ is for the geometric cell type (or INTERP_KERNEL::NORM_ERROR for node field) entry to find the right
 ...
 * MEDFileField1TSWithoutSDA::getFieldSplitedByType. For non gausspoints field users, the value is 0.
 * \param [in] newLocName is the new localization name.
 * \param [in] forceRenameOnGlob specifies the behaviour in case of profile \b newLocName already exists. If true, the
 * renaming is done without check. It can lead to major bug. If false, an exception will be thrown to force user to
 * change previously the name of the profile with name \b newLocName
 */
void
MEDFileAnyTypeField1TS::setLocNameOnLeaf(
    const std::string &mName,
    INTERP_KERNEL::NormalizedCellType typ,
    int locId,
...
)
{
    ...
    {
        std::ostringstream oss;
        oss << "MEDFileField1TS::setLocNameOnLeaf : Localization \"" << newLocName
            << "\" already exists or referenced more than one !";
        throw INTERP_KERNEL::Exception(oss.str());
    }
}
...
MEDFileAnyTypeField1TSWithoutSDA *
MEDFileAnyTypeField1TS::contentNotNullBase()
{
...
const MEDFileAnyTypeField1TSWithoutSDA *
MEDFileAnyTypeField1TS::contentNotNullBase() const
{
 ...
 * This method should be called when a MEDFileAnyTypeField1TS::New constructor has been with false as the last
 * parameter. This method can be also called to refresh or reinit values from a file.
 *
 * \throw If the fileName is not set or points to a non readable MED file.
 * \sa MEDFileAnyTypeField1TS::loadArraysIfNecessary
 */
void
MEDFileAnyTypeField1TS::loadArrays()
{
    if (getFileName().empty())
        throw INTERP_KERNEL::Exception(
            "MEDFileAnyTypeField1TS::loadArrays : the structure does not come from a file !"
        );
...
/*!
 * This method behaves as MEDFileAnyTypeField1TS::loadArrays does, the first call, if \a this was built using a file
 * without loading big arrays. But once data loaded once, this method does nothing. Contrary to
 * MEDFileAnyTypeField1TS::loadArrays and MEDFileAnyTypeField1TS::unloadArrays this method does not throw if \a this
 * does not come from file read.
 *
...
/*!
 * This method releases potentially big data arrays and so returns to the same heap memory than status loaded with
 * 'loadAll' parameter set to false.
 * \b WARNING, this method does release arrays even if \a this does not come from a load of a MED file.
 * So this method can lead to a loss of data. If you want to unload arrays safely call
 * MEDFileAnyTypeField1TS::unloadArraysWithoutDataLoss instead.
 *
 * \sa MEDFileAnyTypeField1TS::loadArrays, MEDFileAnyTypeField1TS::loadArraysIfNecessary,
 * MEDFileAnyTypeField1TS::unloadArraysWithoutDataLoss
 */
void
MEDFileAnyTypeField1TS::unloadArrays()
{
    contentNotNullBase()->unloadArrays();
}

/*!
 * This method potentially releases big data arrays if \a this is coming from a file. If \a this has been built from
 * scratch this method will have no effect. This method is the symmetrical method of
 * MEDFileAnyTypeField1TS::loadArraysIfNecessary. This method is useful to reduce \b safely amount of heap memory
 * necessary for \a this by using MED file as database.
 *
 * \sa MEDFileAnyTypeField1TS::loadArraysIfNecessary
 */
void
...
void
MEDFileAnyTypeField1TS::writeLL(med_idt fid) const
{
...
std::size_t
MEDFileAnyTypeField1TS::getHeapMemorySizeWithoutChildren() const
{
    return MEDFileFieldGlobsReal::getHeapMemorySizeWithoutChildren();
}

std::vector<const BigMemoryObject *>
MEDFileAnyTypeField1TS::getDirectChildrenWithNull() const
{
 ...
 */
std::string
MEDFileAnyTypeField1TS::simpleRepr() const
{
    std::ostringstream oss;
 ...
 * To get non empty name profiles as time as they appear in \b this call MEDFileField1TS::getPflsReallyUsedMulti
 * instead.
 */
std::vector<std::string>
MEDFileAnyTypeField1TS::getPflsReallyUsed() const
{
 ...
 */
std::vector<std::string>
MEDFileAnyTypeField1TS::getLocsReallyUsed() const
{
...
/*!
 * This method returns all profiles whose name is non empty used.
 * \b WARNING contrary to MEDFileField1TS::getPflsReallyUsed, if profile is used several times it will be reported as
 * time as it appears.
 */
std::vector<std::string>
MEDFileAnyTypeField1TS::getPflsReallyUsedMulti() const
{
...
/*!
 * This method returns all localizations whose name is non empty used.
 * \b WARNING contrary to MEDFileField1TS::getLocsReallyUsed if localization is used several times it will be reported
 * as time as it appears.
 */
std::vector<std::string>
MEDFileAnyTypeField1TS::getLocsReallyUsedMulti() const
{
    return contentNotNullBase()->getLocsReallyUsedMulti2();
}

void
MEDFileAnyTypeField1TS::changePflsRefsNamesGen(
    const std::vector<std::pair<std::vector<std::string>, std::string> > &mapOfModif
)
{
...
void
MEDFileAnyTypeField1TS::changeLocsRefsNamesGen(
    const std::vector<std::pair<std::vector<std::string>, std::string> > &mapOfModif
)
{
...
int
MEDFileAnyTypeField1TS::getDimension() const
{
    return contentNotNullBase()->getDimension();
}

int
MEDFileAnyTypeField1TS::getIteration() const
{
    return contentNotNullBase()->getIteration();
}

int
MEDFileAnyTypeField1TS::getOrder() const
{
    return contentNotNullBase()->getOrder();
}

double
MEDFileAnyTypeField1TS::getTime(int &iteration, int &order) const
{
    return contentNotNullBase()->getTime(iteration, order);
}

void
MEDFileAnyTypeField1TS::setTime(int iteration, int order, double val)
{
    contentNotNullBase()->setTime(iteration, order, val);
}

std::string
MEDFileAnyTypeField1TS::getName() const
{
    return contentNotNullBase()->getName();
}

void
MEDFileAnyTypeField1TS::setName(const std::string &name)
{
    contentNotNullBase()->setName(name);
}

std::string
MEDFileAnyTypeField1TS::getDescription() const
{
    return contentNotNullBase()->getDescription();
}

void
MEDFileAnyTypeField1TS::setDescription(const std::string &description)
{
    contentNotNullBase()->setDescription(description);
}

void
MEDFileAnyTypeField1TS::simpleRepr(int bkOffset, std::ostream &oss, int f1tsId) const
{
    contentNotNullBase()->simpleRepr(bkOffset, oss, f1tsId);
}

std::string
MEDFileAnyTypeField1TS::getDtUnit() const
{
    return contentNotNullBase()->getDtUnit();
}

void
MEDFileAnyTypeField1TS::setDtUnit(const std::string &dtUnit)
{
...
void
MEDFileAnyTypeField1TS::setMeshName(const std::string &newMeshName)
{
    contentNotNullBase()->setMeshName(newMeshName);
}

bool
MEDFileAnyTypeField1TS::changeMeshNames(const std::vector<std::pair<std::string, std::string> > &modifTab)
{
    return contentNotNullBase()->changeMeshNames(modifTab);
}

int
MEDFileAnyTypeField1TS::getMeshIteration() const
{
    return contentNotNullBase()->getMeshIteration();
}

int
MEDFileAnyTypeField1TS::getMeshOrder() const
{
    return contentNotNullBase()->getMeshOrder();
}

std::size_t
MEDFileAnyTypeField1TS::getNumberOfComponents() const
{
    return contentNotNullBase()->getNumberOfComponents();
}

bool
MEDFileAnyTypeField1TS::isDealingTS(int iteration, int order) const
{
    return contentNotNullBase()->isDealingTS(iteration, order);
}

std::pair<int, int>
MEDFileAnyTypeField1TS::getDtIt() const
{
    return contentNotNullBase()->getDtIt();
}

void
MEDFileAnyTypeField1TS::fillIteration(std::pair<int, int> &p) const
{
    contentNotNullBase()->fillIteration(p);
}

void
MEDFileAnyTypeField1TS::fillTypesOfFieldAvailable(std::vector<TypeOfField> &types) const
{
    contentNotNullBase()->fillTypesOfFieldAvailable(types);
}

void
MEDFileAnyTypeField1TS::setInfo(const std::vector<std::string> &infos)
{
    contentNotNullBase()->setInfo(infos);
}

const std::vector<std::string> &
MEDFileAnyTypeField1TS::getInfo() const
{
    return contentNotNullBase()->getInfo();
}
std::vector<std::string> &
MEDFileAnyTypeField1TS::getInfo()
{
    return contentNotNullBase()->getInfo();
}

bool
MEDFileAnyTypeField1TS::presenceOfMultiDiscPerGeoType() const
{
    return contentNotNullBase()->presenceOfMultiDiscPerGeoType();
}

MEDFileFieldPerMeshPerTypePerDisc *
MEDFileAnyTypeField1TS::getLeafGivenMeshAndTypeAndLocId(
    const std::string &mName, INTERP_KERNEL::NormalizedCellType typ, int locId
)
{
...
int
MEDFileAnyTypeField1TS::getNonEmptyLevels(const std::string &mname, std::vector<int> &levs) const
{
    return contentNotNullBase()->getNonEmptyLevels(mname, levs);
}

void
MEDFileAnyTypeField1TS::convertMedBallIntoClassic()
{
    return contentNotNullBase()->convertMedBallIntoClassic();
}

void
MEDFileAnyTypeField1TS::makeReduction(INTERP_KERNEL::NormalizedCellType ct, TypeOfField tof, const DataArrayIdType *pfl)
{
    return contentNotNullBase()->makeReduction(ct, tof, pfl);
}

std::vector<TypeOfField>
MEDFileAnyTypeField1TS::getTypesOfFieldAvailable() const
{
    return contentNotNullBase()->getTypesOfFieldAvailable();
}

std::vector<std::vector<std::pair<mcIdType, mcIdType> > >
MEDFileAnyTypeField1TS::getFieldSplitedByType(
    const std::string &mname,
    std::vector<INTERP_KERNEL::NormalizedCellType> &types,
    std::vector<std::vector<TypeOfField> > &typesF,
    std::vector<std::vector<std::string> > &pfls,
 ...
 * The returned instances are deep copy of \a this except that for globals that are shared with those contained in \a
 * this.
 * ** WARNING ** do no forget to rename the output instances to avoid to write n-times in the same MED file field !
 */
std::vector<MCAuto<MEDFileAnyTypeField1TS> >
MEDFileAnyTypeField1TS::splitComponents() const
{
    const MEDFileAnyTypeField1TSWithoutSDA *content(_content);
    if (!content)
        throw INTERP_KERNEL::Exception(
            "MEDFileAnyTypeField1TS::splitComponents : no content in this ! Unable to split components !"
        );
 ...
 * The returned instances are shallowed copied of \a this except that for globals that are shared with those contained
 * in \a this.
 */
std::vector<MCAuto<MEDFileAnyTypeField1TS> >
MEDFileAnyTypeField1TS::splitDiscretizations() const
{
    const MEDFileAnyTypeField1TSWithoutSDA *content(_content);
    if (!content)
        throw INTERP_KERNEL::Exception(
            "MEDFileAnyTypeField1TS::splitDiscretizations : no content in this ! Unable to split discretization !"
        );
 ...
 */
std::vector<MCAuto<MEDFileAnyTypeField1TS> >
MEDFileAnyTypeField1TS::splitMultiDiscrPerGeoTypes() const
{
    const MEDFileAnyTypeField1TSWithoutSDA *content(_content);
    if (!content)
        throw INTERP_KERNEL::Exception(
            "MEDFileAnyTypeField1TS::splitMultiDiscrPerGeoTypes : no content in this ! Unable to split discretization !"
        );
...
mcIdType
MEDFileAnyTypeField1TS::copyTinyInfoFrom(const MEDCouplingFieldDouble *field, const DataArray *arr)
{
...
mcIdType
MEDFileAnyTypeField1TS::copyTinyInfoFrom(
    const TimeHolder *th, const MEDCouplingFieldTemplate *field, const DataArray *arr
)
{
...
/*!
 * This method performs a copy with datatype modification ( float64->int32 ) of \a this. The globals information are
 * copied following the given input policy.
 *
...
MEDFileField1TS::convertToInt(bool isDeepCpyGlobs) const
{
    ...
    if (content)
    {
        ...
        if (!contc)
            throw INTERP_KERNEL::Exception(
                "MEDFileField1TS::convertToInt : the content inside this is not FLOAT64 ! This is incoherent !"
            );
        MCAuto<MEDFileInt32Field1TSWithoutSDA> newc(contc->convertToInt());
        ret = static_cast<MEDFileInt32Field1TS *>(
            MEDFileAnyTypeField1TS::BuildNewInstanceFromContent((MEDFileInt32Field1TSWithoutSDA *)newc)
        );
    }
    else
...
/*!
 * This method performs a copy with datatype modification ( float64->int64 ) of \a this. The globals information are
 * copied following the given input policy.
 *
 * \param [in] isDeepCpyGlobs - a boolean that indicates the behaviour concerning globals (profiles and localizations)
 *                            By default (true) the globals are deeply copied.
...
MEDFileField1TS::convertToInt64(bool isDeepCpyGlobs) const
{
    ...
    if (content)
    {
        ...
        ret = static_cast<MEDFileInt64Field1TS *>(
            MEDFileAnyTypeField1TS::BuildNewInstanceFromContent((MEDFileInt64Field1TSWithoutSDA *)newc)
        );
    }
    else
...
try : MEDFileTemplateField1TS
    <double>(fid, loadAll, ms) {}
catch (INTERP_KERNEL::Exception &e)
{
    throw e;
}
...
try : MEDFileTemplateField1TS
    <double>(fid, fieldName, loadAll, ms) {}
catch (INTERP_KERNEL::Exception &e)
{
    throw e;
}
...
MEDFileField1TS::MEDFileField1TS(
    med_idt fid, const std::string &fieldName, int iteration, int order, bool loadAll, const MEDFileMeshes *ms
)
try : MEDFileTemplateField1TS
    <double>(fid, fieldName, iteration, order, loadAll, ms) {}
catch (INTERP_KERNEL::Exception &e)
{
    throw e;
}
...
try : MEDFileTemplateField1TS
    <double>(other, shallowCopyOfContent) {}
catch (INTERP_KERNEL::Exception &e)
{
    throw e;
}

MCAuto<MEDFileAnyTypeField1TS>
MEDFileField1TS::buildNewEmpty() const
{
...
std::vector<std::vector<DataArrayDouble *> >
MEDFileField1TS::getFieldSplitedByType2(
    const std::string &mname,
    std::vector<INTERP_KERNEL::NormalizedCellType> &types,
    std::vector<std::vector<TypeOfField> > &typesF,
    std::vector<std::vector<std::string> > &pfls,
...
MCAuto<MEDFileAnyTypeField1TS>
MEDFileInt32Field1TS::buildNewEmpty() const
{
    return MCAuto<MEDFileAnyTypeField1TS>(MEDFileInt32Field1TS::New());
}

MCAuto<MEDCouplingFieldDouble>
MEDFileInt32Field1TS::ConvertFieldIntToFieldDouble(const MEDCouplingFieldInt32 *f)
{
    if (!f)
...
MCAuto<MEDFileAnyTypeField1TS>
MEDFileInt64Field1TS::buildNewEmpty() const
{
    return MCAuto<MEDFileAnyTypeField1TS>(MEDFileInt64Field1TS::New());
}

MCAuto<MEDCouplingFieldDouble>
MEDFileInt64Field1TS::ConvertFieldIntToFieldDouble(const MEDCouplingFieldInt64 *f)
{
    if (!f)
...
MCAuto<MEDFileAnyTypeField1TS>
MEDFileFloatField1TS::buildNewEmpty() const
{
