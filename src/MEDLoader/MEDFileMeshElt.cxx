...
MEDFileUMeshPerTypeCommon *
MEDFileUMeshPerTypeCommon::New()
{
    return new MEDFileUMeshPerTypeCommon;
}

void
MEDFileUMeshPerTypeCommon::loadCommonPart(
    med_idt fid,
    const char *mName,
    int dt,
    int it,
...
)
{
    ...
    _fam = 0;
    if (MEDmeshnEntity(fid, mName, dt, it, entity, geoElt, MED_FAMILY_NUMBER, MED_NODAL, &changement, &transformation) >
        0)
    {
...
std::size_t
MEDFileUMeshPerTypeCommon::getHeapMemorySizeWithoutChildren() const
{
    return 0;
}

std::vector<const BigMemoryObject *>
MEDFileUMeshPerTypeCommon::getDirectChildrenWithNull() const
{
...
std::size_t
MEDFileUMeshPerType::getHeapMemorySizeWithoutChildren() const
{
    return MEDFileUMeshPerTypeCommon::getHeapMemorySizeWithoutChildren() + 0;
}

std::vector<const BigMemoryObject *>
MEDFileUMeshPerType::getDirectChildrenWithNull() const
{
...
void
MEDFileUMeshPerType::getMedTypes(
    med_idt fid,
    const char *mName,
    int dt,
    int it,
...
)
{
    ...
    if (geoElt2i < 0 || geoElt2i >= INTERP_KERNEL::NORM_MAXTYPE)
        throw INTERP_KERNEL::Exception(
            "MEDFileUMeshPerType::getMedTypes : Not recognized MEDCoupling/MEDLoader geometric type !"
        );
    geoElt = typmai3[MCgeoElt];
    if (!isExisting(fid, mName, dt, it, geoElt, whichEntity))
        throw INTERP_KERNEL::Exception(
            "MEDFileUMeshPerType::getMedTypes : The specified geo type is not present in the specified mesh !"
        );
}

bool
MEDFileUMeshPerType::isExisting(
...
)
{
    ...
    {
        med_bool changement, transformation;
        mcIdType tmp(MEDmeshnEntity(
            fid, mName, dt, it, entities[i], geoElt, MED_CONNECTIVITY, MED_NODAL, &changement, &transformation
        ));
        if (tmp > nbOfElt)
        {
            nbOfElt = tmp;
            ...
            if (i > 0)
                std::cerr << "WARNING : MEDFile has been detected to be noncompliant with MED 3 : Please change entity "
                             "in MEDFile for geotype "
                          << geoElt << std::endl;
        }
    }
    return nbOfElt > 0;
...
int
MEDFileUMeshPerType::getDim() const
{
...
)
{
    med_bool changement, transformation;
    mcIdType curNbOfElem(
        MEDmeshnEntity(fid, mName, dt, it, entity, geoElt, MED_CONNECTIVITY, MED_NODAL, &changement, &transformation)
    );
...
void
MEDFileUMeshPerType::loadFromStaticType(
    med_idt fid,
    const char *mName,
    int dt,
    int it,
...
)
{
    ...
    MEDFILESAFECALLERRD0(
        ...
        (fid, mName, dt, it, entity, geoElt, MED_NODAL, MED_FULL_INTERLACE, conn->getPointer())
    );
    std::transform(
        conn->begin(), conn->end(), conn->getPointer(), std::bind(std::plus<med_int>(), std::placeholders::_1, -1)
    );
...
void
MEDFileUMeshPerType::loadPartStaticType(
    med_idt fid,
    const char *mName,
    int dt,
    int it,
...
)
{
    ...
    conn->alloc(nbOfNodesPerCell * nbOfEltsToLoad, 1);
    {
        ...
        filter.fill(
            ...
            /*nentity*/ curNbOfElem,
            /*nvaluesperentity*/ 1,
            /*nconstituentpervalue*/ nbOfNodesPerCell,
            MED_ALL_CONSTITUENT,
            MED_FULL_INTERLACE,
            MED_COMPACT_STMODE,
            MED_NO_PROFILE,
        ...
        MEDFILESAFECALLERRD0(
            ...
            (fid, mName, dt, it, entity, geoElt, MED_NODAL, filter.getPtr(), conn->getPointer())
        );
    }
    std::transform(
        conn->begin(), conn->end(), conn->getPointer(), std::bind(std::plus<med_int>(), std::placeholders::_1, -1)
...
void
MEDFileUMeshPerType::loadPartOfCellCommonPart(
    med_idt fid,
    const char *mName,
    int dt,
    int it,
...
)
{
        ...
        0)
    {
        if (!mrs || mrs->isCellFamilyFieldReading())
        {
            ...
            if (MEDmeshEntityAttributeAdvancedRd(
                    fid, mName, MED_FAMILY_NUMBER, dt, it, entity, geoElt, filter.getPtr(), miFam->getPointer()
                ) != 0)
                miFam->fillWithZero();
            _fam = FromMedIntArray<mcIdType>(miFam);
        }
    ...
    if (MEDmeshnEntity(fid, mName, dt, it, entity, geoElt, MED_NUMBER, MED_NODAL, &changement, &transformation) > 0)
    {
        if (!mrs || mrs->isCellNumFieldReading())
        {
            ...
            if (MEDmeshEntityAttributeAdvancedRd(
                    fid, mName, MED_NUMBER, dt, it, entity, geoElt, filter.getPtr(), miNum->getPointer()
                ) != 0)
                miNum->fillWithZero();
            _num = FromMedIntArray<mcIdType>(miNum);
        }
    ...
    if (MEDmeshnEntity(fid, mName, dt, it, entity, geoElt, MED_NAME, MED_NODAL, &changement, &transformation) > 0)
    {
        if (!mrs || mrs->isCellNameFieldReading())
        {
            _names = DataArrayAsciiChar::New();
            _names->alloc(
                nbOfEltsToLoad + 1, MED_SNAME_SIZE
            );  // not a bug to avoid the memory corruption due to last \0 at the end
            MEDFilterEntity filter;
            filter.fill(
                fid,
            ...
            if (MEDmeshEntityAttributeAdvancedRd(
                    fid, mName, MED_NAME, dt, it, entity, geoElt, filter.getPtr(), _names->getPointer()
                ) != 0)
                _names = 0;
            else
...
)
{
    med_bool changement, transformation;
    mcIdType curNbOfElem(
        MEDmeshnEntity(fid, mName, dt, it, entity, geoElt, MED_INDEX_NODE, MED_NODAL, &changement, &transformation) - 1
    );
    _m = MEDCoupling1DGTUMesh::New(
        mName, geoElt == MED_POLYGON ? INTERP_KERNEL::NORM_POLYGON : INTERP_KERNEL::NORM_QPOLYG
    );
    ...
    std::transform(
        connI->begin(), connI->end(), connI->getPointer(), std::bind(std::plus<med_int>(), std::placeholders::_1, -1)
    );
...
{
    med_bool changement, transformation;
    med_int indexFaceLgth(MEDmeshnEntity(
        fid, mName, dt, it, MED_CELL, MED_POLYHEDRON, MED_INDEX_NODE, MED_NODAL, &changement, &transformation
    ));
    mcIdType curNbOfElem(
        MEDmeshnEntity(
            fid, mName, dt, it, MED_CELL, MED_POLYHEDRON, MED_INDEX_FACE, MED_NODAL, &changement, &transformation
        ) -
        1
    );
    ...
    for (mcIdType i = 0; i < curNbOfElem; i++) arraySize += index[i + 1] - index[i] - 1;
    ...
    {
        finalIndex[i + 1] =
            finalIndex[i] + index[i + 1] - index[i] - 1 + indexFace[index[i + 1] - 1] - indexFace[index[i] - 1];
        wFinalConn = std::transform(
            locConn + indexFace[index[i] - 1] - 1,
            locConn + indexFace[index[i]] - 1,
            wFinalConn,
            std::bind(std::plus<mcIdType>(), std::placeholders::_1, -1)
        );
        for (mcIdType j = index[i]; j < index[i + 1] - 1; j++)
        {
            *wFinalConn++ = -1;
            wFinalConn = std::transform(
                locConn + indexFace[j] - 1,
                locConn + indexFace[j + 1] - 1,
                wFinalConn,
                std::bind(std::plus<mcIdType>(), std::placeholders::_1, -1)
            );
        }
...
)
{
    ...
    if (!cm.isDynamic())
    {
        ...
        std::transform(
            arr->begin(), arr->end(), arr->getPointer(), std::bind(std::plus<med_int>(), std::placeholders::_1, 1)
        );
    ...
    else
    {
        ...
        {
            MCAuto<DataArrayMedInt> arr(DataArrayMedInt_Copy(m0->getNodalConnectivity())),
                arrI(DataArrayMedInt_Copy(m0->getNodalConnectivityIndex()));
            std::transform(
                arr->begin(), arr->end(), arr->getPointer(), std::bind(std::plus<med_int>(), std::placeholders::_1, 1)
            );
            std::transform(
                ...
                arrI->end(),
                arrI->getPointer(),
                std::bind(std::plus<med_int>(), std::placeholders::_1, 1)
            );
            MEDFILESAFECALLERWR0(
                MEDmeshPolygon2Wr,
                (fid,
                 ...
                 timm,
                 MED_CELL,
                 ikt == INTERP_KERNEL::NORM_POLYGON ? MED_POLYGON : MED_POLYGON2,
                 MED_NODAL,
                 ToMedInt(nbOfCells + 1),
                 arrI->begin(),
                 arr->begin())
            );
        }
    ...
    if (fam)
        MEDFILESAFECALLERWR0(
            ...
            (fid,
             ...
             curMedType,
             ToMedInt(nbOfCells),
             ToMedIntArray<mcIdType>(fam)->getConstPointer())
        );
    if (num)
        MEDFILESAFECALLERWR0(
            ...
            (fid,
             ...
             curMedType,
             ToMedInt(nbOfCells),
             ToMedIntArray<mcIdType>(num)->getConstPointer())
        );
    if (names)
    {
        if (names->getNumberOfComponents() != MED_SNAME_SIZE)
        {
            ...
            oss << "MEDFileUMeshPerType::write : expected a name field on cells with number of components set to "
                << MED_SNAME_SIZE;
        ...
        MEDFILESAFECALLERWR0(
            MEDmeshEntityNameWr,
            (fid, mname.c_str(), dt, it, MED_CELL, curMedType, ToMedInt(nbOfCells), names->getConstPointer())
        );
    }
}
