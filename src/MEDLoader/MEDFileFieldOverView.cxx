...
MEDFileMeshStruct *
MEDFileMeshStruct::New(const MEDFileMesh *mesh)
{
    return new MEDFileMeshStruct(mesh);
}

std::size_t
MEDFileMeshStruct::getHeapMemorySizeWithoutChildren() const
{
    std::size_t ret(0);
    for (std::vector<std::vector<mcIdType> >::const_iterator it0 = _geo_types_distrib.begin();
         it0 != _geo_types_distrib.end();
         it0++)
...
MEDFileMeshStruct::getDirectChildrenWithNull() const
{
...
int
MEDFileMeshStruct::getLevelOfGeoType(INTERP_KERNEL::NormalizedCellType t) const
{
    int j = 0;
    for (std::vector<std::vector<mcIdType> >::const_iterator it1 = _geo_types_distrib.begin();
         it1 != _geo_types_distrib.end();
         it1++, j--)
    {
    ...
    throw INTERP_KERNEL::Exception(
        "MEDFileMeshStruct::getLevelOfGeoType : The specified geometric type is not present in the mesh structure !"
    );
}
 ...
 */
mcIdType
MEDFileMeshStruct::getNumberOfElemsOfGeoType(INTERP_KERNEL::NormalizedCellType t) const
{
    for (std::vector<std::vector<mcIdType> >::const_iterator it1 = _geo_types_distrib.begin();
         it1 != _geo_types_distrib.end();
         it1++)
 ...
 */
bool
MEDFileMeshStruct::doesManageGeoType(INTERP_KERNEL::NormalizedCellType t) const
{
    for (std::vector<std::vector<mcIdType> >::const_iterator it1 = _geo_types_distrib.begin();
         it1 != _geo_types_distrib.end();
         it1++)
...
void
MEDFileMeshStruct::appendIfImplicitType(INTERP_KERNEL::NormalizedCellType t)
{
    if (!_mesh->hasImplicitPart())
        throw INTERP_KERNEL::Exception(
            "MEDFileMeshStruct::appendIfImplicitType : by default no implicit geo type can be appended !"
        );
    static const char MSG[] =
        "MEDFileMeshStruct::appendIfImplicitType : the distribution does not looks like structured standard !";
    ...
    if (nbGeo != 1)
        throw INTERP_KERNEL::Exception(MSG);
    std::vector<mcIdType> arr(3);
    arr[0] = (mcIdType)t;
    arr[1] = _mesh->buildImplicitPartIfAny(t);
    arr[2] = -1;
    _geo_types_distrib.push_back(arr);
}

int
MEDFileMeshStruct::getNumberOfLevs() const
{
    return (int)_geo_types_distrib.size();
}

int
MEDFileMeshStruct::getNumberOfGeoTypesInLev(int relativeLev) const
{
    int pos(-relativeLev);
...
std::size_t
MEDMeshMultiLev::getHeapMemorySizeWithoutChildren() const
{
    return 0;
}

std::vector<const BigMemoryObject *>
MEDMeshMultiLev::getDirectChildrenWithNull() const
{
    return std::vector<const BigMemoryObject *>();
}

MEDMeshMultiLev *
MEDMeshMultiLev::New(const MEDFileMesh *m, const std::vector<int> &levs)
{
    if (!m)
    ...
    throw INTERP_KERNEL::Exception(
        "MEDMeshMultiLev::New : unrecognized type of mesh ! Must be in "
        "[MEDFileUMesh,MEDFileCMesh,MEDFileCurveLinearMesh] !"
    );
}
...
MEDMeshMultiLev::New(
    const MEDFileMesh *m,
    const std::vector<INTERP_KERNEL::NormalizedCellType> &gts,
    const std::vector<const DataArrayIdType *> &pfls,
    const std::vector<mcIdType> &nbEntities
)
{
    if (!m)
    ...
    throw INTERP_KERNEL::Exception(
        "MEDMeshMultiLev::New 2 : unrecognized type of mesh ! Must be in "
        "[MEDFileUMesh,MEDFileCMesh,MEDFileCurveLinearMesh] !"
    );
}

MEDMeshMultiLev *
MEDMeshMultiLev::NewOnlyOnNode(const MEDFileMesh *m, const DataArrayIdType *pflOnNode)
{
...
void
MEDMeshMultiLev::setNodeReduction(const DataArrayIdType *nr)
{
    if (nr)
        nr->incrRef();
...
void
MEDMeshMultiLev::setCellReduction(const DataArrayIdType *cr)
{
    if (_pfls.size() != 1)
        throw INTERP_KERNEL::Exception(
            "MEDMeshMultiLev::setCellReduction : can be used only for single geo type mesh !"
        );
...
bool
MEDMeshMultiLev::isFastlyTheSameStruct(const MEDFileField1TSStructItem &fst, const MEDFileFieldGlobsReal *globs) const
{
...
DataArray *
MEDMeshMultiLev::buildDataArray(
    const MEDFileField1TSStructItem &fst, const MEDFileFieldGlobsReal *globs, const DataArray *vals
) const
{
    MCAuto<DataArray> ret(const_cast<DataArray *>(vals));
    ret->incrRef();
 ...
 * \param [out] isWithoutCopy - When true the returned instance \a famIds if not null is directly those in the data
 * structure.
 */
void
MEDMeshMultiLev::retrieveFamilyIdsOnCells(DataArrayIdType *&famIds, bool &isWithoutCopy) const
{
    ...
    if (!presenceOfPfls)
    {
        famIds = const_cast<DataArrayIdType *>(fids);
        famIds->incrRef();
 ...
 * \param [out] isWithoutCopy - When true the returned instance \a numIds if not null is directly those in the data
 * structure.
 */
void
MEDMeshMultiLev::retrieveNumberIdsOnCells(DataArrayIdType *&numIds, bool &isWithoutCopy) const
{
    ...
    if (!presenceOfPfls)
    {
        numIds = const_cast<DataArrayIdType *>(nids);
        numIds->incrRef();
 ...
 */
void
MEDMeshMultiLev::retrieveFamilyIdsOnNodes(DataArrayIdType *&famIds, bool &isWithoutCopy) const
{
 ...
 */
void
MEDMeshMultiLev::retrieveNumberIdsOnNodes(DataArrayIdType *&numIds, bool &isWithoutCopy) const
{
    ...
    {
        numIds = const_cast<DataArrayIdType *>(fids);
        numIds->incrRef();
        isWithoutCopy = _mesh->isObjectInTheProgeny(numIds);
    }
}
...
/*!
 * This method returns, if any, a new object containing the global node ids **BUT CONTRARY TO OTHER RETRIEVE METHODS**
 * the returned object is always a NON AGGREGATED object. So the returned object if not null can be used as this safely.
 */
DataArrayIdType *
MEDMeshMultiLev::retrieveGlobalNodeIdsIfAny() const
{
    ...
    if (nr)
        ...
        return tmp
            ->deepCopy();  // Yes a deep copy is needed because this method has to return a non aggregated object !
}

std::vector<INTERP_KERNEL::NormalizedCellType>
MEDMeshMultiLev::getGeoTypes() const
{
    return _geo_types;
}

void
MEDMeshMultiLev::setFamilyIdsOnCells(DataArrayIdType *famIds)
{
...
void
MEDMeshMultiLev::setNumberIdsOnCells(DataArrayIdType *numIds)
{
...
void
MEDMeshMultiLev::setFamilyIdsOnNodes(DataArrayIdType *famIds)
{
...
void
MEDMeshMultiLev::setNumberIdsOnNodes(DataArrayIdType *numIds)
{
...
std::string
MEDMeshMultiLev::getPflNameOfId(int id) const
{
 ...
 */
mcIdType
MEDMeshMultiLev::getNumberOfCells(INTERP_KERNEL::NormalizedCellType t) const
{
...
mcIdType
MEDMeshMultiLev::getNumberOfNodes() const
{
    return _nb_nodes;
}

DataArray *
MEDMeshMultiLev::constructDataArray(
    const MEDFileField1TSStructItem &fst, const MEDFileFieldGlobsReal *globs, const DataArray *vals
) const
{
    if (fst.getType() == ON_NODES)
    {
        ...
        if (!pflName.empty() && nr)
        {
            ...
            if (!p1->isEqualWithoutConsideringStr(*p2))
                throw INTERP_KERNEL::Exception(
                    "MEDMeshMultiLev::constructDataArray : it appears that a profile on nodes does not cover the cells "
                    "correctly !"
                );
        ...
        if (!pflName.empty() && !nr)
        {
            ...
            if (!p1->isIota(getNumberOfNodes()))
                throw INTERP_KERNEL::Exception(
                    "MEDMeshMultiLev::constructDataArray : unexpected situation for nodes 4 !"
                );
            MCAuto<DataArray> ret(vals->deepCopy());
    ...
    else
    {
        ...
        for (std::vector<INTERP_KERNEL::NormalizedCellType>::const_iterator it = _geo_types.begin();
             it != _geo_types.end();
             it++, iii++)
        {
            ...
            if (ps.empty())
                throw INTERP_KERNEL::Exception(
                    "MEDMeshMultiLev::constructDataArray : unexpected situation for cells 1 !"
                );
            if (ps.size() == 1)
            {
                ...
                if (thisP && otherP)
                {
                    ...
                    ret->rearrange(nbi * nc);
                    ret = ret->selectByTupleIdSafe(p2->begin(), p2->end());
                    ret->rearrange(nc);
                    ret->setInfoOnComponents(compInfo);
                    arrSafe[iii] = ret;
                    arr[iii] = ret;
                    continue;
                }
                ...
                throw INTERP_KERNEL::Exception(
                    "MEDMeshMultiLev::constructDataArray : unexpected situation for cells 3 !"
                );
            }
            else
            {
                ...
                int jj(0);
                for (std::vector<const MEDFileField1TSStructItem2 *>::const_iterator it2 = ps.begin(); it2 != ps.end();
                     it2++, jj++)
                {
                    ...
                    if (!otherPfl)
                        throw INTERP_KERNEL::Exception(
                            "MEDMeshMultiLev::constructDataArray : unexpected situation for cells 4 !"
                        );
                    arr2[jj] = ret2;
                    arr2Safe[jj] = ret2;
                    otherPS[jj] = otherPfl;
                    nbisSafe[jj] = DataArrayIdType::New();
                    nbisSafe[jj]->alloc(otherPfl->getNumberOfTuples(), 1);
                    nbisSafe[jj]->fillWithValue(nbi);
                    nbis[jj] = nbisSafe[jj];
                }
 ...
 */
void
MEDMeshMultiLev::appendVertices(const DataArrayIdType *verticesToAdd, DataArrayIdType *nr)
{
    ...
    if (cf)
    {
        ...
        {
            tmp = DataArrayIdType::New();
            tmp->alloc(nbOfVertices, 1);
            tmp->fillWithZero();
        }
        a[1] = tmp;
...
MEDMeshMultiLev::MEDMeshMultiLev(
    ...
    : _mesh(mesh), _geo_types(gts), _nb_entities(nbEntities), _nb_nodes(nbNodes)
{
    std::size_t sz(_geo_types.size());
...
MEDMeshMultiLev::MEDMeshMultiLev(const MEDMeshMultiLev &other)
    : RefCountObject(other),
      _mesh(other._mesh),
      _pfls(other._pfls),
      _geo_types(other._geo_types),
      _nb_entities(other._nb_entities),
      _node_reduction(other._node_reduction),
      _nb_nodes(other._nb_nodes),
      _cell_fam_ids(other._cell_fam_ids),
      _cell_num_ids(other._cell_num_ids),
      _node_fam_ids(other._node_fam_ids),
      _node_num_ids(other._node_num_ids)
{
}
...
MEDUMeshMultiLev *
MEDUMeshMultiLev::New(const MEDFileUMesh *m, const std::vector<int> &levs)
{
...
MEDUMeshMultiLev *
MEDUMeshMultiLev::New(
    const MEDFileUMesh *m,
    const std::vector<INTERP_KERNEL::NormalizedCellType> &gts,
    const std::vector<const DataArrayIdType *> &pfls,
    const std::vector<mcIdType> &nbEntities
)
...
MEDUMeshMultiLev::MEDUMeshMultiLev(
    const MEDFileUMesh *m,
    const std::vector<INTERP_KERNEL::NormalizedCellType> &gts,
    const std::vector<const DataArrayIdType *> &pfls,
    const std::vector<mcIdType> &nbEntities
)
    : MEDMeshMultiLev(m, m->getNumberOfNodes(), gts, pfls, nbEntities)
{
    ...
    if (isSameDim && isNoPfl && m->getGeoTypesAtLevel(lev) == gts)  // optimized part
    {
        ...
        if (famIds)
        {
            _cell_fam_ids = const_cast<DataArrayIdType *>(famIds);
            famIds->incrRef();
        }
        const DataArrayIdType *numIds(m->getNumberFieldAtLevel(lev));
        if (numIds)
        {
            _cell_num_ids = const_cast<DataArrayIdType *>(numIds);
            numIds->incrRef();
        }
        famIds = m->getFamilyFieldAtLevel(1);
        if (famIds)
        {
            _node_fam_ids = const_cast<DataArrayIdType *>(famIds);
            famIds->incrRef();
        }
        numIds = m->getNumberFieldAtLevel(1);
        if (numIds)
        {
            _node_num_ids = const_cast<DataArrayIdType *>(numIds);
            numIds->incrRef();
        }
        return;
    }
    ...
    if (nodeFamIds)
    {
        _node_fam_ids = const_cast<DataArrayIdType *>(nodeFamIds);
        nodeFamIds->incrRef();
    }
    const DataArrayIdType *nodeNumIds(m->getNumberFieldAtLevel(1));
    if (nodeNumIds)
    {
        _node_num_ids = const_cast<DataArrayIdType *>(nodeNumIds);
        nodeNumIds->incrRef();
    }
}

void
MEDUMeshMultiLev::selectPartOfNodes(const DataArrayIdType *pflNodes)
{
    ...
    for (std::size_t i = 0; i < sz; i++)
    {
        ...
        a[i] = o2n->invertArrayO2N2N2O(tmp);
        aa[i] = a[i];
        if (pfl)
    ...
    if (_node_reduction->getNumberOfTuples() == pflNodes->getNumberOfTuples())
        return;  // This is the classical case where the input node profile corresponds perfectly to a subset of cells
                 // in _parts
    if (_node_reduction->getNumberOfTuples() > pflNodes->getNumberOfTuples())
        throw INTERP_KERNEL::Exception(
            "MEDUMeshMultiLev::selectPartOfNodes : internal error in MEDCoupling during cell select from a list of "
            "nodes !"
        );
    // Here the cells available in _parts is not enough to cover all the nodes in pflNodes. So adding vertices cells in
    // _parts...
...
MEDMeshMultiLev *
MEDUMeshMultiLev::prepare() const
{
    return new MEDUMeshMultiLev(*this);
}

MEDUMeshMultiLev::MEDUMeshMultiLev(const MEDUMeshMultiLev &other)
    : MEDMeshMultiLev(other), _parts(other._parts), _coords(other._coords)
{
}

MEDUMeshMultiLev::MEDUMeshMultiLev(const MEDStructuredMeshMultiLev &other, const MCAuto<MEDCoupling1GTUMesh> &part)
    : MEDMeshMultiLev(other)
{
    _parts.resize(1);
    _parts[0] = part;
    _geo_types.resize(1);
    _geo_types[0] = part->getCellModelEnum();
    _nb_entities.resize(1);
    _nb_entities[0] = part->getNumberOfCells();
    _pfls.resize(1);
    _pfls[0] = 0;
}

/*!
 * To be called only once ! Because due to some optimizations (sometimes aggressive) the internal state can be
 * changed... If returned value is false output pointer \a coords is not the internal pointer. If returned value is true
 * output pointer \a coords is directly the internal pointer. If true is returned, the \a coords output parameter should
 * be used with care (non const method call) to avoid to change the internal state of MEDFileUMesh instance.
 */
bool
MEDUMeshMultiLev::buildVTUArrays(
    DataArrayDouble *&coords,
    DataArrayByte *&types,
    DataArrayIdType *&cellLocations,
    DataArrayIdType *&cells,
    DataArrayIdType *&faceLocations,
    DataArrayIdType *&faces
) const
{
    ...
    MCAuto<DataArrayDouble> a(const_cast<DataArrayDouble *>(tmp));
    tmp->incrRef();
    ...
    for (std::vector<MCAuto<MEDCoupling1GTUMesh> >::const_iterator it = _parts.begin(); it != _parts.end(); it++, iii++)
    {
        ...
        if (!pfl)
        {
            cur2 = const_cast<MEDCoupling1GTUMesh *>(cur);
            cur2->incrRef();
        }
        else
        {
            cur2 = dynamic_cast<MEDCoupling1GTUMesh *>(cur->buildPartOfMySelfKeepCoords(pfl->begin(), pfl->end()));
            cur = cur2;
        }
        //
    ...
    MCAuto<DataArrayByte> b(DataArrayByte::New());
    b->alloc(szBCE, 1);
    char *bPtr(b->getPointer());
    MCAuto<DataArrayIdType> c(DataArrayIdType::New());
    c->alloc(szBCE, 1);
    mcIdType *cPtr(c->getPointer());
    MCAuto<DataArrayIdType> d(DataArrayIdType::New());
    d->alloc(szD, 1);
    mcIdType *dPtr(d->getPointer());
    MCAuto<DataArrayIdType> e(DataArrayIdType::New()), f(DataArrayIdType::New());
    mcIdType *ePtr(nullptr), *fPtr(nullptr);
    if (isPolyh)
    {
        e->alloc(szBCE, 1);
        ePtr = e->getPointer();
    ...
    for (std::vector<MCAuto<MEDCoupling1GTUMesh> >::const_iterator it = _parts.begin(); it != _parts.end(); it++, iii++)
    {
        ...
        if (gtvtk == 255)
            throw INTERP_KERNEL::Exception(
                "MEDUMeshMultiLev::getVTUArrays : no VTK type for the requested INTERP_KERNEL geometric type !"
            );
        std::fill(bPtr, bPtr + curNbCells, gtvtk);
        bPtr += curNbCells;
    ...
    coords = a.retn();
    types = b.retn();
    cellLocations = c.retn();
    cells = d.retn();
    if (!isPolyh)
    {
        faceLocations = nullptr;
        faces = nullptr;
    }
    else
    {
        faceLocations = e.retn();
        faces = f.retn();
    }
...
bool
MEDUMeshMultiLev::buildVTUArrays94(
    MCAuto<DataArrayDouble> &coords,
    MCAuto<DataArrayByte> &types,
    MCAuto<DataArrayIdType> &cellLocations,
    MCAuto<DataArrayIdType> &cells,
    MCAuto<DataArrayIdType> &faceLocationsOffset,
    MCAuto<DataArrayIdType> &faceLocationsConn,
    MCAuto<DataArrayIdType> &facesOffset,
    MCAuto<DataArrayIdType> &facesConn
) const
{
    const DataArrayDouble *tmp(nullptr);
    if (_parts.empty())
    ...
    d->alloc(szD, 1);
    mcIdType *dPtr(d->getPointer());
    MCAuto<DataArrayIdType> e(DataArrayIdType::New()), f(DataArrayIdType::New()), g(DataArrayIdType::New()),
        h(DataArrayIdType::New());
...
void
MEDUMeshMultiLev::reorderNodesIfNecessary(
    MCAuto<DataArrayDouble> &coords, DataArrayIdType *nodalConnVTK, DataArrayIdType *polyhedNodalConnVTK
) const
{
    const DataArrayIdType *nr(_node_reduction);
    if (!nr)
    ...
    {
        work = polyhedNodalConnVTK->begin();
        endW = polyhedNodalConnVTK->end();
        while (work != endW)
        {
            mcIdType nb(*work++);
            for (mcIdType i = 0; i < nb && work != endW; i++)
            {
                ...
                for (mcIdType j = 0; j < nb2 && work != endW; j++, work++)
                {
                    if (*work >= 0 && *work < sz)
                        ...
                        throw INTERP_KERNEL::Exception(
                            "MEDUMeshMultiLev::reorderNodesIfNecessary : internal error #2 !"
                        );
                }
            }
        }
    ...
    {
        mcIdType nb(*work2++);
        for (mcIdType i = 0; i < nb && work2 != endW2; i++, work2++) *work2 = permPtr[o2nPtrc[*work2]];
    }
    if (polyhedNodalConnVTK)
    {
        work2 = polyhedNodalConnVTK->getPointer();
        endW2 = polyhedNodalConnVTK->getPointer() + polyhedNodalConnVTK->getNumberOfTuples();
        while (work2 != endW2)
        {
            mcIdType nb(*work2++);
            ...
            {
                mcIdType nb2(*work2++);
                for (mcIdType j = 0; j < nb2 && work2 != endW2; j++, work2++) *work2 = permPtr[o2nPtrc[*work2]];
            }
        }
    }
    coords = (coords->selectByTupleIdSafe(nr->begin(), nr->end()));
...
void
MEDUMeshMultiLev::appendVertices(const DataArrayIdType *verticesToAdd, DataArrayIdType *nr)
{
    ...
    MCAuto<MEDCoupling1GTUMesh> elt2((MEDCoupling1SGTUMesh *)elt);
    elt2->incrRef();
...
MEDStructuredMeshMultiLev::MEDStructuredMeshMultiLev(const MEDFileStructuredMesh *m, const std::vector<int> &lev)
    : MEDMeshMultiLev(m), _is_internal(true)
{
    initStdFieldOfIntegers(m);
}

MEDStructuredMeshMultiLev::MEDStructuredMeshMultiLev(
    const MEDFileStructuredMesh *m,
    mcIdType nbOfNodes,
    const std::vector<INTERP_KERNEL::NormalizedCellType> &gts,
    const std::vector<const DataArrayIdType *> &pfls,
    const std::vector<mcIdType> &nbEntities
)
    : MEDMeshMultiLev(m, nbOfNodes, gts, pfls, nbEntities), _is_internal(true)
{
    initStdFieldOfIntegers(m);
}

MEDStructuredMeshMultiLev::MEDStructuredMeshMultiLev(const MEDStructuredMeshMultiLev &other)
    : MEDMeshMultiLev(other), _is_internal(true), _face_fam_ids(other._face_fam_ids), _face_num_ids(other._face_num_ids)
{
}

void
MEDStructuredMeshMultiLev::initStdFieldOfIntegers(const MEDFileStructuredMesh *m)
{
    // ids fields management
...
void
MEDStructuredMeshMultiLev::moveFaceToCell() const
{
    const_cast<MEDStructuredMeshMultiLev *>(this)->_cell_fam_ids = _face_fam_ids;
    const_cast<MEDStructuredMeshMultiLev *>(this)->_face_fam_ids = 0;
    const_cast<MEDStructuredMeshMultiLev *>(this)->_cell_num_ids = _face_num_ids;
    const_cast<MEDStructuredMeshMultiLev *>(this)->_face_num_ids = 0;
}

bool
MEDStructuredMeshMultiLev::prepareForImplicitUnstructuredMeshCase(MEDMeshMultiLev *&ret) const
{
    ret = 0;
    if (_geo_types.empty())
        return false;
    if (_geo_types.size() != 1)
        throw INTERP_KERNEL::Exception(
            "MEDStructuredMeshMultiLev::prepareForImplicitUnstructuredMeshCase only one geo types supported at most "
            "supported for the moment !"
        );
    INTERP_KERNEL::NormalizedCellType gt(
        MEDCouplingStructuredMesh::GetGeoTypeGivenMeshDimension(_mesh->getMeshDimension())
    );
    if (_geo_types[0] == gt)
        return false;
    ...
    if (!_pfls.empty())
        pfl = _pfls[0];
    MCAuto<MEDCoupling1GTUMesh> facesIfPresent2(facesIfPresent);
    facesIfPresent->incrRef();
    moveFaceToCell();
    ...
    if (nr)
        throw INTERP_KERNEL::Exception(
            "MEDStructuredMeshMultiLev::prepareForImplicitUnstructuredMeshCase : case is not treated yet for node "
            "reduction on implicit unstructured mesh."
        );
    ret = ret2.retn();
    return true;
}
...
void
MEDStructuredMeshMultiLev::dealWithImplicitUnstructuredMesh(const MEDFileMesh *m)
{
    const DataArrayIdType *tmp(0);
    tmp = m->getFamilyFieldAtLevel(-1);
    if (tmp)
...
void
MEDStructuredMeshMultiLev::selectPartOfNodes(const DataArrayIdType *pflNodes)
{
    if (!pflNodes || !pflNodes->isAllocated())
        return;
    ...
    MCAuto<MEDCoupling1SGTUMesh> m(
        MEDCoupling1SGTUMesh::New("", MEDCouplingStructuredMesh::GetGeoTypeGivenMeshDimension((int)ngs.size()))
    );
...
MEDCMeshMultiLev *
MEDCMeshMultiLev::New(const MEDFileCMesh *m, const std::vector<int> &levs)
{
    return new MEDCMeshMultiLev(m, levs);
}

MEDCMeshMultiLev *
MEDCMeshMultiLev::New(
    const MEDFileCMesh *m,
    const std::vector<INTERP_KERNEL::NormalizedCellType> &gts,
    const std::vector<const DataArrayIdType *> &pfls,
    const std::vector<mcIdType> &nbEntities
)
...
MEDCMeshMultiLev::MEDCMeshMultiLev(const MEDFileCMesh *m, const std::vector<int> &levs)
    : MEDStructuredMeshMultiLev(m, levs)
{
    if (!m)
    ...
    for (int i = 0; i < sdim; i++)
    {
        ...
        if (!elt)
            throw INTERP_KERNEL::Exception(
                "MEDCMeshMultiLev constructor 2 : presence of null pointer for an vector of double along an axis !"
            );
        elt->incrRef();
        _coords[i] = elt;
    }
...
MEDCMeshMultiLev::MEDCMeshMultiLev(
    const MEDFileCMesh *m,
    const std::vector<INTERP_KERNEL::NormalizedCellType> &gts,
    const std::vector<const DataArrayIdType *> &pfls,
    const std::vector<mcIdType> &nbEntities
)
    : MEDStructuredMeshMultiLev(m, m->getNumberOfNodes(), gts, pfls, nbEntities)
{
    if (!m)
        throw INTERP_KERNEL::Exception("MEDCMeshMultiLev constructor 2 : null input pointer !");
    if (gts.size() != 1 || pfls.size() != 1)
        throw INTERP_KERNEL::Exception(
            "MEDCMeshMultiLev constructor 2 : lengths of gts and pfls must be equal to one !"
        );
...
MEDCMeshMultiLev::MEDCMeshMultiLev(const MEDCMeshMultiLev &other)
    : MEDStructuredMeshMultiLev(other), _coords(other._coords)
{
}

std::vector<mcIdType>
MEDCMeshMultiLev::getNodeGridStructure() const
{
    std::vector<mcIdType> ret(_coords.size());
    for (std::size_t i = 0; i < _coords.size(); i++) ret[i] = _coords[i]->getNumberOfTuples();
    return ret;
}

MEDMeshMultiLev *
MEDCMeshMultiLev::prepare() const
{
    ...
    if (pfl)
    {
        ...
        if (MEDCouplingStructuredMesh::IsPartStructured(pfl->begin(), pfl->end(), cgs, cellParts))
        {
            ...
            ret->_coords = coords;
            ret2 = (MEDCMeshMultiLev *)ret;
            ret2->incrRef();
        }
        else
        {
            MCAuto<MEDCouplingCMesh> m(MEDCouplingCMesh::New());
            for (unsigned int i = 0; i < ngs.size(); i++) m->setCoordsAt(i, _coords[i]);
            MCAuto<MEDCoupling1SGTUMesh> m2(m->build1SGTUnstructured());
            MCAuto<MEDCoupling1GTUMesh> m3 =
                dynamic_cast<MEDCoupling1GTUMesh *>(m2->buildPartOfMySelfKeepCoords(pfl->begin(), pfl->end()));
...
/*!
 * \a param [out] isInternal if true the returned pointers are those in main data structure. If false those pointers
 * have been built especially for that method.
 */
std::vector<DataArrayDouble *>
MEDCMeshMultiLev::buildVTUArrays(bool &isInternal) const
{
...
MEDCurveLinearMeshMultiLev::New(const MEDFileCurveLinearMesh *m, const std::vector<int> &levs)
{
    return new MEDCurveLinearMeshMultiLev(m, levs);
}

MEDCurveLinearMeshMultiLev *
MEDCurveLinearMeshMultiLev::New(
    const MEDFileCurveLinearMesh *m,
    const std::vector<INTERP_KERNEL::NormalizedCellType> &gts,
    const std::vector<const DataArrayIdType *> &pfls,
    const std::vector<mcIdType> &nbEntities
)
...
MEDCurveLinearMeshMultiLev::MEDCurveLinearMeshMultiLev(
    const MEDFileCurveLinearMesh *m,
    const std::vector<INTERP_KERNEL::NormalizedCellType> &gts,
    const std::vector<const DataArrayIdType *> &pfls,
    const std::vector<mcIdType> &nbEntities
    ...
    : MEDStructuredMeshMultiLev(m, m->getNumberOfNodes(), gts, pfls, nbEntities)
{
    ...
    if (gts.size() != 1 || pfls.size() != 1)
        throw INTERP_KERNEL::Exception(
            "MEDCurveLinearMeshMultiLev constructor 2 : lengths of gts and pfls must be equal to one !"
        );
    INTERP_KERNEL::NormalizedCellType gt(
        MEDCouplingStructuredMesh::GetGeoTypeGivenMeshDimension(m->getMeshDimension())
    );
    if (gt == gts[0])
    {
        DataArrayDouble *coords(const_cast<DataArrayDouble *>(m->getMesh()->getCoords()));
...
MEDCurveLinearMeshMultiLev::MEDCurveLinearMeshMultiLev(const MEDCurveLinearMeshMultiLev &other)
    : MEDStructuredMeshMultiLev(other), _coords(other._coords), _structure(other._structure)
{
}

std::vector<mcIdType>
MEDCurveLinearMeshMultiLev::getNodeGridStructure() const
{
    return _structure;
}

MEDMeshMultiLev *
MEDCurveLinearMeshMultiLev::prepare() const
{
    MEDMeshMultiLev *retSpecific(0);
    if (prepareForImplicitUnstructuredMeshCase(retSpecific))
        return retSpecific;
    ...
    if (pfl)
    {
        ...
        if (MEDCouplingStructuredMesh::IsPartStructured(pfl->begin(), pfl->end(), cgs, cellParts))
        {
            ...
            ret->_structure = st;
            ret2 = (MEDCurveLinearMeshMultiLev *)ret;
            ret2->incrRef();
        }
        else
        {
            ...
            m->setCoords(_coords);
            m->setNodeGridStructure(&_structure[0], &_structure[0] + _structure.size());
            MCAuto<MEDCoupling1SGTUMesh> m2(m->build1SGTUnstructured());
            MCAuto<MEDCoupling1GTUMesh> m3 =
                dynamic_cast<MEDCoupling1GTUMesh *>(m2->buildPartOfMySelfKeepCoords(pfl->begin(), pfl->end()));
            MCAuto<MEDUMeshMultiLev> ret(new MEDUMeshMultiLev(*this, m3));
...
void
MEDCurveLinearMeshMultiLev::buildVTUArrays(
    DataArrayDouble *&coords, std::vector<mcIdType> &nodeStrct, bool &isInternal
) const
{
    isInternal = _is_internal;
    ...
    coords = const_cast<DataArrayDouble *>(coo);
    coords->incrRef();
}
...
MEDFileField1TSStructItem2::MEDFileField1TSStructItem2(
    INTERP_KERNEL::NormalizedCellType a,
    const std::pair<mcIdType, mcIdType> &b,
    const std::string &c,
    const std::string &d
)
    : _geo_type(a), _start_end(b), _pfl(DataArrayIdType::New()), _loc(d), _nb_of_entity(-1)
{
    _pfl->setName(c.c_str());
}

void
MEDFileField1TSStructItem2::checkWithMeshStructForCells(
    const MEDFileMeshStruct *mst, const MEDFileFieldGlobsReal *globs
)
{
...
void
MEDFileField1TSStructItem2::checkWithMeshStructForGaussNE(
    const MEDFileMeshStruct *mst, const MEDFileFieldGlobsReal *globs
)
{
    mcIdType nbOfEnt = mst->getNumberOfElemsOfGeoType(_geo_type);
...
void
MEDFileField1TSStructItem2::checkWithMeshStructForGaussPT(
    const MEDFileMeshStruct *mst, const MEDFileFieldGlobsReal *globs
)
{
    if (!globs)
        throw INTERP_KERNEL::Exception(
            "MEDFileField1TSStructItem2::checkWithMeshStructForGaussPT : no globals specified !"
        );
    if (_loc.empty())
        throw INTERP_KERNEL::Exception(
            "MEDFileField1TSStructItem2::checkWithMeshStructForGaussPT : no localization specified !"
        );
...
int
MEDFileField1TSStructItem2::getNbOfIntegrationPts(const MEDFileFieldGlobsReal *globs) const
{
    if (_loc.empty())
    {
        if (getPflName().empty())
...
std::string
MEDFileField1TSStructItem2::getPflName() const
{
    return _pfl->getName();
}

const DataArrayIdType *
MEDFileField1TSStructItem2::getPfl(const MEDFileFieldGlobsReal *globs) const
{
    if (!_pfl->isAllocated())
    {
 ...
 */
void
MEDFileField1TSStructItem2::checkInRange(mcIdType nbOfEntity, int nip, const MEDFileFieldGlobsReal *globs)
{
    ...
    if (_pfl->getName().empty())
    {
        if (nbOfEntity != (_start_end.second - _start_end.first) / nip)
            throw INTERP_KERNEL::Exception(
                "MEDFileField1TSStructItem2::checkInRange : Mismatch between number of entities and size of field !"
            );
        return;
    }
    else
    {
        if (!globs)
            throw INTERP_KERNEL::Exception(
                "MEDFileField1TSStructItem2::checkInRange : Presence of a profile on field whereas no globals found in "
                "file !"
            );
        const DataArrayIdType *pfl = globs->getProfile(_pfl->getName().c_str());
        if (!pfl)
            throw INTERP_KERNEL::Exception(
                "MEDFileField1TSStructItem2::checkInRange : Presence of a profile on field whereas no such profile "
                "found in file !"
            );
...
bool
MEDFileField1TSStructItem2::isFastlyEqual(
    mcIdType &startExp, INTERP_KERNEL::NormalizedCellType gt, const std::string &pflName
) const
{
...
bool
MEDFileField1TSStructItem2::operator==(const MEDFileField1TSStructItem2 &other) const
{
    //_nb_of_entity is not taken into account here. It is not a bug, because no mesh consideration needed here to
    // perform fast compare. idem for _loc. It is not an effective attribute for support comparison.
    return _geo_type == other._geo_type && _start_end == other._start_end && _pfl->getName() == other._pfl->getName();
}

bool
MEDFileField1TSStructItem2::isCellSupportEqual(
    const MEDFileField1TSStructItem2 &other, const MEDFileFieldGlobsReal *globs
) const
{
    ...
    if ((_pfl->getName().empty() && !other._pfl->getName().empty()) ||
        (!_pfl->getName().empty() && other._pfl->getName().empty()))
        return false;
    if (_pfl->getName().empty() && other._pfl->getName().empty())
        return true;
...
MEDFileField1TSStructItem2::BuildAggregationOf(
    const std::vector<const MEDFileField1TSStructItem2 *> &objs, const MEDFileFieldGlobsReal *globs
)
{
    if (objs.empty())
    ...
    for (std::size_t i = 0; i < sz; i++)
    {
        ...
        if (gt != obj->_geo_type)
            throw INTERP_KERNEL::Exception(
                "MEDFileField1TSStructItem2::BuildAggregationOf : invalid situation ! All input must have the same geo "
                "type !"
            );
        if (nbEntityRef != obj->_nb_of_entity)
            throw INTERP_KERNEL::Exception(
                "MEDFileField1TSStructItem2::BuildAggregationOf : invalid situation ! All input must have the global "
                "nb of entity !"
            );
        if (obj->_pfl->getName().empty())
            throw INTERP_KERNEL::Exception(
                "MEDFileField1TSStructItem2::BuildAggregationOf : invalid situation ! Several same geo type chunk must "
                "all lie on profiles !"
            );
    ...
    if (oldNbTuples != arr->getNumberOfTuples())
        throw INTERP_KERNEL::Exception(
            "MEDFileField1TSStructItem2::BuildAggregationOf : some entities are present several times !"
        );
...
std::size_t
MEDFileField1TSStructItem2::getHeapMemorySizeWithoutChildren() const
{
...
std::vector<const BigMemoryObject *>
MEDFileField1TSStructItem2::getDirectChildrenWithNull() const
{
...
MEDFileField1TSStructItem::MEDFileField1TSStructItem(TypeOfField a, const std::vector<MEDFileField1TSStructItem2> &b)
    : _computed(false), _type(a), _items(b)
{
}

void
MEDFileField1TSStructItem::checkWithMeshStruct(const MEDFileMeshStruct *mst, const MEDFileFieldGlobsReal *globs)
{
    switch (_type)
    {
        case ON_NODES:
        {
            ...
            if (_items.size() != 1)
                throw INTERP_KERNEL::Exception(
                    "MEDFileField1TSStructItem::checkWithMeshStruct : for nodes field only one subdivision supported !"
                );
...
bool
MEDFileField1TSStructItem::operator==(const MEDFileField1TSStructItem &other) const
{
...
bool
MEDFileField1TSStructItem::isCellSupportEqual(
    const MEDFileField1TSStructItem &other, const MEDFileFieldGlobsReal *globs
) const
{
    if (_type != other._type)
        return false;
...
bool
MEDFileField1TSStructItem::isNodeSupportEqual(
    const MEDFileField1TSStructItem &other, const MEDFileFieldGlobsReal *globs
) const
{
    if (_type != other._type)
...
bool
MEDFileField1TSStructItem::isEntityCell() const
{
    if (_type == ON_NODES)
        return false;
    else
...
class CmpGeo
{
   public:
    ...
    bool operator()(const std::pair<INTERP_KERNEL::NormalizedCellType, std::vector<std::size_t> > &v) const
    {
...
MEDFileField1TSStructItem::simplifyMeOnCellEntity(const MEDFileFieldGlobsReal *globs) const
{
    if (!isEntityCell())
        throw INTERP_KERNEL::Exception(
            "MEDFileField1TSStructItem::simplifyMeOnCellEntity : must be on ON_CELLS, ON_GAUSS_NE or ON_GAUSS_PT !"
        );
    ...
    {
        std::vector<std::pair<INTERP_KERNEL::NormalizedCellType, std::vector<std::size_t> > >::iterator it0(
            std::find_if(m.begin(), m.end(), CmpGeo((*it).getGeo()))
        );
        if (it0 == m.end())
            m.push_back(
                std::pair<INTERP_KERNEL::NormalizedCellType, std::vector<std::size_t> >(
                    (*it).getGeo(), std::vector<std::size_t>(1, i)
                )
            );
        else
    ...
    for (i = 0; i < sz; i++)
    {
        ...
        for (std::size_t j = 0; j < ids.size(); j++) objs[j] = &_items[ids[j]];
 ...
 */
bool
MEDFileField1TSStructItem::isCompatibleWithNodesDiscr(
    const MEDFileField1TSStructItem &other, const MEDFileMeshStruct *meshSt, const MEDFileFieldGlobsReal *globs
) const
{
    if (other._type != ON_NODES)
        throw INTERP_KERNEL::Exception(
            "MEDFileField1TSStructItem::isCompatibleWithNodesDiscr : other must be on nodes !"
        );
    if (other._items.size() != 1)
        throw INTERP_KERNEL::Exception(
            "MEDFileField1TSStructItem::isCompatibleWithNodesDiscr : other is on nodes but number of subparts !"
        );
    int theFirstLevFull;
...
bool
MEDFileField1TSStructItem::isFullyOnOneLev(const MEDFileMeshStruct *meshSt, int &theFirstLevFull) const
{
    if (_type != ON_CELLS)
        throw INTERP_KERNEL::Exception(
            "MEDFileField1TSStructItem::isFullyOnOneLev : works only for ON_CELLS discretization !"
        );
    if (_items.empty())
    ...
    if (nbOfLevs == 0)
        throw INTERP_KERNEL::Exception(
            "MEDFileField1TSStructItem::isFullyOnOneLev : no levels in input mesh structure !"
        );
...
const MEDFileField1TSStructItem2 &
MEDFileField1TSStructItem::operator[](std::size_t i) const
{
    if (i >= _items.size())
...
std::size_t
MEDFileField1TSStructItem::getHeapMemorySizeWithoutChildren() const
{
...
std::vector<const BigMemoryObject *>
MEDFileField1TSStructItem::getDirectChildrenWithNull() const
{
    std::vector<const BigMemoryObject *> ret;
    for (std::vector<MEDFileField1TSStructItem2>::const_iterator it = _items.begin(); it != _items.end(); it++)
        ret.push_back(&(*it));
...
MEDMeshMultiLev *
MEDFileField1TSStructItem::buildFromScratchDataSetSupportOnCells(
    const MEDFileMeshStruct *mst, const MEDFileFieldGlobsReal *globs
) const
{
...
std::vector<INTERP_KERNEL::NormalizedCellType>
MEDFileField1TSStructItem::getGeoTypes(const MEDFileMesh *m) const
{
...
MEDFileField1TSStructItem
MEDFileField1TSStructItem::BuildItemFrom(const MEDFileAnyTypeField1TS *ref, const MEDFileMeshStruct *meshSt)
{
    ...
    std::vector<std::vector<std::pair<mcIdType, mcIdType> > > strtEnds =
    ...
    for (std::size_t i = 0; i < nbOfGeoTypes; i++)
    {
        ...
        for (std::size_t j = 0; j < sz; j++)
        {
            if (atype == typesF[i][j])
                ...
                throw INTERP_KERNEL::Exception(
                    "MEDFileField1TSStruct : can be applied only on single spatial discretization fields ! Call "
                    "SplitPerDiscretization method !"
                );
        }
    }
    ...
    catch (INTERP_KERNEL::Exception &e)
    {
        ...
        oss << e.what() << " (" << MEDCouplingFieldDiscretization::GetTypeOfFieldRepr(ret.getType()) << ")";
...
MEDFileField1TSStruct::New(const MEDFileAnyTypeField1TS *ref, MEDFileMeshStruct *mst)
{
...
void
MEDFileField1TSStruct::checkWithMeshStruct(MEDFileMeshStruct *mst, const MEDFileFieldGlobsReal *globs)
{
...
bool
MEDFileField1TSStruct::isEqualConsideringThePast(
    const MEDFileAnyTypeField1TS *other, const MEDFileMeshStruct *mst
) const
{
    MEDFileField1TSStructItem b(MEDFileField1TSStructItem::BuildItemFrom(other, mst));
    for (std::vector<MEDFileField1TSStructItem>::const_iterator it = _already_checked.begin();
         it != _already_checked.end();
         it++)
    {
        if ((*it) == b)
            return true;
 ...
 */
bool
MEDFileField1TSStruct::isSupportSameAs(const MEDFileAnyTypeField1TS *other, const MEDFileMeshStruct *meshSt)
{
    if (_already_checked.empty())
 ...
 */
bool
MEDFileField1TSStruct::isCompatibleWithNodesDiscr(const MEDFileAnyTypeField1TS *other, const MEDFileMeshStruct *meshSt)
{
    if (_already_checked.empty())
...
std::size_t
MEDFileField1TSStruct::getHeapMemorySizeWithoutChildren() const
{
...
std::vector<const BigMemoryObject *>
MEDFileField1TSStruct::getDirectChildrenWithNull() const
{
    std::vector<const BigMemoryObject *> ret;
    for (std::vector<MEDFileField1TSStructItem>::const_iterator it = _already_checked.begin();
         it != _already_checked.end();
...
MEDMeshMultiLev *
MEDFileField1TSStruct::buildFromScratchDataSetSupport(
    const MEDFileMeshStruct *mst, const MEDFileFieldGlobsReal *globs
) const
{
    if (_already_checked.empty())
        throw INTERP_KERNEL::Exception(
            "MEDFileField1TSStruct::buildFromScratchDataSetSupport : No outline structure in this !"
        );
    int pos0(-1), pos1(-1);
...
bool
MEDFileField1TSStruct::isDataSetSupportFastlyEqualTo(
    const MEDFileField1TSStruct &other, const MEDFileFieldGlobsReal *globs
) const
{
    int b0, b1;
...
std::vector<INTERP_KERNEL::NormalizedCellType>
MEDFileField1TSStruct::getGeoTypes(const MEDFileMesh *m) const
{
    std::vector<INTERP_KERNEL::NormalizedCellType> ret;
    for (std::vector<MEDFileField1TSStructItem>::const_iterator it = _already_checked.begin();
         it != _already_checked.end();
         it++)
    {
        ...
        for (std::vector<INTERP_KERNEL::NormalizedCellType>::const_iterator it2 = ret2.begin(); it2 != ret2.end();
             it2++)
        {
 ...
 * If true is returned the pos of the easiest is returned. The easiest is the first element in \a this having the less
 * split subparts.
 */
bool
MEDFileField1TSStruct::presenceOfCellDiscr(int &pos) const
{
    ...
    if (refSz == 0)
        throw INTERP_KERNEL::Exception(
            "MEDFileField1TSStruct::presenceOfCellDiscr : an element in this on entity CELL is empty !"
        );
    return ret;
}
         ...
         it++, i++)
    {
        if ((*it).getType() == ON_NODES)
        {
            ...
            if (sz == 1)
            {
                ...
                throw INTERP_KERNEL::Exception(
                    "MEDFileField1TSStruct::presenceOfPartialNodeDiscr : an element in this on entity NODE is split "
                    "into several parts !"
                );
        }
    }
...
MEDFileFastCellSupportComparator::New(const MEDFileMeshStruct *m, const MEDFileAnyTypeFieldMultiTS *ref)
{
    return new MEDFileFastCellSupportComparator(m, ref);
}

MEDFileFastCellSupportComparator::MEDFileFastCellSupportComparator(
    const MEDFileMeshStruct *m, const MEDFileAnyTypeFieldMultiTS *ref
)
{
    if (!m)
        throw INTERP_KERNEL::Exception("MEDFileFastCellSupportComparator constructor : null input mesh struct !");
    _mesh_comp = const_cast<MEDFileMeshStruct *>(m);
    _mesh_comp->incrRef();
    ...
    for (int i = 0; i < nbPts; i++)
    {
        ...
        {
            std::ostringstream oss;
            oss << "Problem in field with name \"" << ref->getName() << "\"" << std::endl;
...
std::size_t
MEDFileFastCellSupportComparator::getHeapMemorySizeWithoutChildren() const
{
...
std::vector<const BigMemoryObject *>
MEDFileFastCellSupportComparator::getDirectChildrenWithNull() const
{
    std::vector<const BigMemoryObject *> ret;
    const MEDFileMeshStruct *mst(_mesh_comp);
    if (mst)
        ret.push_back(mst);
    for (std::vector<MCAuto<MEDFileField1TSStruct> >::const_iterator it = _f1ts_cmps.begin(); it != _f1ts_cmps.end();
         it++)
...
bool
MEDFileFastCellSupportComparator::isEqual(const MEDFileAnyTypeFieldMultiTS *other)
{
    int nbPts = other->getNumberOfTS();
    if (nbPts != (int)_f1ts_cmps.size())
    {
        std::ostringstream oss;
        oss << "MEDFileFastCellSupportComparator::isEqual : unexpected nb of time steps in  input ! Should be "
            << _f1ts_cmps.size() << " it is in reality " << nbPts << " !";
        throw INTERP_KERNEL::Exception(oss.str().c_str());
    }
    for (int i = 0; i < nbPts; i++)
    {
...
bool
MEDFileFastCellSupportComparator::isCompatibleWithNodesDiscr(const MEDFileAnyTypeFieldMultiTS *other)
{
    int nbPts = other->getNumberOfTS();
    if (nbPts != (int)_f1ts_cmps.size())
    {
        std::ostringstream oss;
        oss << "MEDFileFastCellSupportComparator::isCompatibleWithNodesDiscr : unexpected nb of time steps in  input ! "
               "Should be "
            << _f1ts_cmps.size() << " it is in reality " << nbPts << " !";
        throw INTERP_KERNEL::Exception(oss.str().c_str());
    }
...
MEDMeshMultiLev *
MEDFileFastCellSupportComparator::buildFromScratchDataSetSupport(
    int timeStepId, const MEDFileFieldGlobsReal *globs
) const
{
    if (timeStepId < 0 || timeStepId >= (int)_f1ts_cmps.size())
    {
        std::ostringstream oss;
        oss << "MEDFileFastCellSupportComparator::buildFromScratchDataSetSupport : requested time step id #"
            << timeStepId << " is not in [0," << _f1ts_cmps.size() << ") !";
        throw INTERP_KERNEL::Exception(oss.str().c_str());
    }
    ...
    {
        std::ostringstream oss;
        oss << "MEDFileFastCellSupportComparator::buildFromScratchDataSetSupport : at time step id #" << timeStepId
            << " no field structure overview defined !";
        throw INTERP_KERNEL::Exception(oss.str().c_str());
    }
...
bool
MEDFileFastCellSupportComparator::isDataSetSupportEqualToThePreviousOne(
    int timeStepId, const MEDFileFieldGlobsReal *globs
) const
{
    if (timeStepId <= 0 || timeStepId >= (int)_f1ts_cmps.size())
    {
        std::ostringstream oss;
        oss << "MEDFileFastCellSupportComparator::isDataSetSupportEqualToThePreviousOne : requested time step id #"
            << timeStepId << " is not in [1," << _f1ts_cmps.size() << ") !";
        throw INTERP_KERNEL::Exception(oss.str().c_str());
    }
    const MEDFileField1TSStruct *obj(_f1ts_cmps[timeStepId]);
...
int
MEDFileFastCellSupportComparator::getNumberOfTS() const
{
    return (int)_f1ts_cmps.size();
}

std::vector<INTERP_KERNEL::NormalizedCellType>
MEDFileFastCellSupportComparator::getGeoTypesAt(int timeStepId, const MEDFileMesh *m) const
{
    if (timeStepId < 0 || timeStepId >= (int)_f1ts_cmps.size())
    {
        std::ostringstream oss;
        oss << "MEDFileFastCellSupportComparator::getGeoTypesAt : requested time step id #" << timeStepId
            << " is not in [0," << _f1ts_cmps.size() << ") !";
        throw INTERP_KERNEL::Exception(oss.str().c_str());
    }
