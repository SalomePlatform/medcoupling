...
MEDPARTITIONER::MeshCollection::MeshCollection(
    MeshCollection &initialCollection, Topology *topology, bool family_splitting, bool create_empty_groups
)
    : _topology(topology),
      _owns_topology(false),
      _driver(0),
      ...
      _joint_finder(0)
{
    ...
    castIntField(initialCollection.getMesh(), this->getMesh(), initialCollection.getCellFamilyIds(), "cellFamily");
    castIntField(
        initialCollection.getFaceMesh(), this->getFaceMesh(), initialCollection.getFaceFamilyIds(), "faceFamily"
    );
...
void
MEDPARTITIONER::MeshCollection::castCellMeshes(
        ...
        if (!isParallelMode() || initialCollection._domain_selector->isMyDomain(iold))
        {
            ...
            for (int inew = 0; inew < nbNewDomain; inew++)
            {
                splitMeshes[inew][iold] =
                    (MEDCoupling::MEDCouplingUMesh *)(initialCollection.getMesh())[iold]->buildPartOfMySelf(
                        &new2oldIds[iold][inew][0], &new2oldIds[iold][inew][0] + new2oldIds[iold][inew].size(), true
                    );
    ...
    for (int inew = 0; inew < nbNewDomain; inew++)
    {
        ...
        if (!isParallelMode() || _domain_selector->isMyDomain(inew))
        {
            if (meshes.size() == 0)
            {
                ...
                std::cout << "WARNING : castCellMeshes fusing : no meshes try another number of processors"
                          << std::endl;
            }
            else
            {
...
/*!
  ...
  \param nodeMapping structure containing the correspondency between nodes in the initial collection and the node(s) in
  the new collection
*/
void
MEDPARTITIONER::MeshCollection::createNodeMapping(MeshCollection &initialCollection, NodeMapping &nodeMapping)
{
    using std::make_pair;
    using std::vector;
...
void
getNodeIds(
    MEDCoupling::MEDCouplingUMesh &meshOne, MEDCoupling::MEDCouplingUMesh &meshTwo, std::vector<mcIdType> &nodeIds
)
{
...
*/
void
MEDPARTITIONER::MeshCollection::castFaceMeshes(
    MeshCollection &initialCollection,
...
)
{
    ...
    for (int iold = 0; iold < nbOldDomain; iold++)
    {
        ...
        if (meshesCastFrom[iold] != 0)
        {
            ...
            for (int inew = 0; inew < nbNewDomain; inew++)
            {
                ...
                {
                    splitMeshes[inew][iold] = (MEDCoupling::MEDCouplingUMesh *)(meshesCastFrom[iold]->buildPartOfMySelf(
                        &new2oldIds[iold][inew][0], &new2oldIds[iold][inew][0] + new2oldIds[iold][inew].size(), true
                    ));
        ...
        {
            std::cout << "proc " << MyGlobals::_Rank << " : castFaceMeshes empty mesh from iodDomain " << iold
                      << std::endl;
        }
    }
        ...
        for (int iold = 0; iold < nbOldDomain; iold++)
            for (int inew = 0; inew < nbNewDomain; inew++)
            {
                if (initialCollection._domain_selector->isMyDomain(iold) && !_domain_selector->isMyDomain(inew))
                {
                    ...
                    {
                        _domain_selector->sendMesh(*(splitMeshes[inew][iold]), _domain_selector->getProcessorID(inew));
                        // std::cout << "proc " << MyGlobals::_Rank << " : castFaceMeshes send " <<inew<<" "<<iold<<"
                        // "<<splitMeshes[inew][iold]->getNumberOfCells()<<std::endl;
                    }
                    else
                    {
                        _domain_selector->sendMesh(*(empty), _domain_selector->getProcessorID(inew));
                        // std::cout << "proc " << MyGlobals::_Rank << " : castFaceMeshes sen0 " <<inew<<"
                        // "<<iold<<std::endl;
                    }
                }
                if (!initialCollection._domain_selector->isMyDomain(iold) && _domain_selector->isMyDomain(inew))
                ...
                // std::cout << "proc " << MyGlobals::_Rank << " : castFaceMeshes recv "<<inew<<" "<<iold<<"
                // "<<nb<<std::endl;//" "<<splitMeshes[inew][iold]->getNumberOfCells()<<std::endl;
            }
        empty->decrRef();
    }
#endif
    ...
    for (int inew = 0; inew < nbNewDomain; inew++)
    {
        ...
        if (_subdomain_boundary_creates && _mesh[inew] && _mesh[inew]->getNumberOfCells() > 0)
        {
            bndMesh = ((MEDCoupling::MEDCouplingUMesh *)_mesh[inew]->buildBoundaryMesh(/*keepCoords=*/true));
...
void
MEDPARTITIONER::MeshCollection::castIntField(
...
)
{
    ...
    for (unsigned int iold = 0; iold < ioldMax; iold++)
        if (isParallelMode() && _domain_selector->isMyDomain(iold))
        {
            ...
            acceleratingStructures[iold] = new BBTreeOfDim(
                sourceCoords->getNumberOfComponents(),
                bbox[iold]->getConstPointer(),
                0,
                0,
                bbox[iold]->getNumberOfTuples()
            );
    ...
    for (unsigned int inew = 0; inew < inewMax; inew++)
    {
        for (unsigned int iold = 0; iold < ioldMax; iold++)
        {
            ...
            if (isParallelMode() && _domain_selector->isMyDomain(iold) && !_domain_selector->isMyDomain(inew))
            {
                ...
                if (MyGlobals::_Verbose > 400)
                    std::cout << "proc " << _domain_selector->rank() << " : castIntField SendIntVec size " << size
                              << std::endl;
                if (size > 0)  // no empty
                {
                    sendIds.resize(size);
            ...
            if (isParallelMode() && !_domain_selector->isMyDomain(iold) && _domain_selector->isMyDomain(inew))
            {
                ...
                // receive vector
                if (MyGlobals::_Verbose > 400)
                    std::cout << "proc " << _domain_selector->rank() << " : castIntField recIntVec " << std::endl;
        ...
        for (unsigned int iold = 0; iold < ioldMax; iold++)
            if (!isParallelMode() || (_domain_selector->isMyDomain(iold) && _domain_selector->isMyDomain(inew)))
            {
                remapIntField(
                    ...
                    *meshesCastFrom[iold],
                    *meshesCastTo[inew],
                    arrayFrom[iold]->getConstPointer(),
                    nameArrayTo,
                    acceleratingStructures[iold]
                );
            }
    }
    for (unsigned int iold = 0; iold < ioldMax; iold++)
...
void
MEDPARTITIONER::MeshCollection::remapIntField(
    int inew,
    int iold,
...
{
    if (sourceMesh.getNumberOfCells() <= 0)
        return;  // empty mesh could exist
...
void
MEDPARTITIONER::MeshCollection::castAllFields(MeshCollection &initialCollection, std::string nameArrayTo)
{
...
void
MEDPARTITIONER::MeshCollection::remapDoubleField(
    int inew, int iold, MEDCoupling::DataArrayDouble *fromArray, std::string nameArrayTo, std::string descriptionField
)
// here we use 'cellFamily_ccI inew iold' set in remapIntField
{
    ...
    if (MyGlobals::_Verbose > 300)
        std::cout << "proc " << MyGlobals::_Rank << " : remapDoubleField " << key << " size " << ccI->getNbOfElems()
                  << std::endl;
    ...
    {
        std::cout << "proc " << MyGlobals::_Rank << " nbcell " << nbcell << " nbcomp " << nbcomp << " nbPtGauss "
                  << nbPtGauss << " fromArray nbOfElems " << fromArrayNbOfElem << " nbTuples "
                  << fromArray->getNumberOfTuples() << " nbcells " << fromArrayNbOfCell << " nbComponents "
                  << fromArray->getNumberOfComponents() << std::endl;
    }
    ...
    {
        if (MyGlobals::_Verbose > 300)
            std::cout << "proc " << MyGlobals::_Rank << " : remapDoubleField key '" << key
                      << "' not found and create it" << std::endl;
...
DataArrayIdType *
sortCorrespondences(DataArrayIdType *ids1, DataArrayIdType *ids2, int delta, bool removeEqual = false)
{
    // sort
    ...
    if (delta != 0)
    {
        ...
        for (; id < ids1->end(); ++id) ++(*id);
        id = ids2->getPointer();
        for (; id < ids2->end(); ++id) ++(*id);
    }
...
void
renumber(DataArrayIdType *ids, const DataArrayIdType *o2nRenumber)
{
    if (!ids || !o2nRenumber)
        return;
...
void
MEDPARTITIONER::MeshCollection::buildConnectZones(
    const NodeMapping &nodeMapping, const std::vector<MEDCoupling::DataArrayIdType *> &o2nRenumber, int nbInitialDomains
)
{
    ...
    for (int idomain = 0; idomain < nb_domains; ++idomain)
    {
        for (int idomainNear = 0; idomainNear < nb_domains; ++idomainNear)
        {
            ...
            for (size_t i = 0; i < czVec.size() && !cz; ++i)
                if (czVec[i] && czVec[i]->getLocalDomainNumber() == idomain &&
    ...
    for (size_t i = 0; i < czVec.size(); ++i)
    {
        ...
        if (!cz || cz->getEntityCorrespNumber(0, 0) == 0 || cz->getLocalDomainNumber() > (int)_mesh.size() ||
        ...
        for (size_t j = 0; j < czVec.size() && !cz21; ++j)
            if (czVec[j] && czVec[j]->getLocalDomainNumber() == cz->getDistantDomainNumber() &&
        ...
        if (types1.size() == 1 && types2.size() == 1)  // split not needed, only sort
        {
            ...
            cz->setEntityCorresp(
                *types1.begin(), *types2.begin(), ids12Sorted->begin(), ids12Sorted->getNbOfElems() / 2
            );

            if (cz21)  // set 2->1 correspondence
            {
                ids12Sorted = sortCorrespondences(ids2, ids1, /*delta=*/0);
                cz21->setEntityCorresp(
                    *types2.begin(), *types1.begin(), ids12Sorted->begin(), ids12Sorted->getNbOfElems() / 2
                );
            }
        }
        else  // split and sort
        {
            ...
            for (t1 = 0; t1 < maxType; ++t1)
                for (t2 = 0; t2 < maxType; ++t2)
                {
                    ...
                    cz->setEntityCorresp(t1, t2, ids12Sorted->begin(), ids12Sorted->getNbOfElems() / 2);

                    if (cz21)  // set 2->1 correspondence
                    {
                        ids12Sorted = sortCorrespondences(ids2, ids1, /*delta=*/0);
                        cz21->setEntityCorresp(t2, t1, ids12Sorted->begin(), ids12Sorted->getNbOfElems() / 2);
                        break;
                    }
                }
        }  // split and sort
...
void
MEDPARTITIONER::MeshCollection::buildBoundaryFaces()
{
    ...
    for (unsigned int inew = 0; inew < nbMeshes - 1; inew++)
        if (!isParallelMode() || _domain_selector->isMyDomain(inew))
        {
            ...
            bbTrees[inew] = new BBTreeOfDim(
                bcCoords->getNumberOfComponents(), bbox[inew]->getConstPointer(), 0, 0, bbox[inew]->getNumberOfTuples()
            );
            bcCoords->decrRef();
        }
        ...
        for (unsigned int inew2 = inew1 + 1; inew2 < nbMeshes; inew2++)
        {
            ...
            if (isParallelMode())
            {
                ...
                if (!mesh1Here && mesh2Here)
                {
                    ...
                    _domain_selector->sendMesh(*faceMeshes[inew2], _domain_selector->getProcessorID(inew1));
                }
            ...
            if (mesh1Here && !mesh1)
                mesh1 = faceMeshes[inew1];
            if (mesh2Here && !mesh2)
                mesh2 = faceMeshes[inew2];
            ...
            {
                createJointGroup(is2nd ? faces2 : faces1, inew1, inew2, is2nd);
            }
...
void
MEDPARTITIONER::MeshCollection::createJointGroup(
    const std::vector<mcIdType> &faces, const int inew1, const int inew2, const bool is2nd
)
{
    ...
    std::string groupName;
    {
        ...
        oss << "JOINT_" << (is2nd ? inew2 : inew1) << "_" << (is2nd ? inew1 : inew2) << "_"
    ...
    for (size_t i = 0; i < faces.size(); ++i) famIDs[faces[i]] = familyID;
      ...
      <chunk subdomain=\"1\">\n \
        <name>$meshName</name>\n \
            ...
            {  // Handle all exceptions
                ...
                throw INTERP_KERNEL::Exception(
                    "file medpartitioner_xxx.xml does not comply with any recognized format"
                );
            }
        }
        else  // no extension
        {
            ...
            catch (...)
            {
                ...
                _driver = 0;
                throw INTERP_KERNEL::Exception(
                    "file name with no extension does not comply with any recognized format"
                );
            }
        }
    }
...
MEDPARTITIONER::MeshCollection::~MeshCollection()
{
    ...
    for (std::map<std::string, MEDCoupling::DataArrayIdType *>::iterator it = _map_dataarray_int.begin();
         it != _map_dataarray_int.end();
         it++)
        if ((*it).second != 0)
            (*it).second->decrRef();

    for (std::map<std::string, MEDCoupling::DataArrayDouble *>::iterator it = _map_dataarray_double.begin();
         it != _map_dataarray_double.end();
         it++)
        if ((*it).second != 0)
            (*it).second->decrRef();
 ...
 */
void
MEDPARTITIONER::MeshCollection::write(const std::string &filename)
{
 ...
 */
MEDPARTITIONER::MeshCollectionDriver *
MEDPARTITIONER::MeshCollection::retrieveDriver()
{
    if (_driver == 0)
    {
        switch (_driver_type)
 ...
 */
MEDPARTITIONER::MeshCollectionDriver *
MEDPARTITIONER::MeshCollection::getDriver() const
{
    return _driver;
}
 ...
 */
int
MEDPARTITIONER::MeshCollection::getMeshDimension() const
{
    return _i_non_empty_mesh < 0 ? -1 : _mesh[_i_non_empty_mesh]->getMeshDimension();
}

int
MEDPARTITIONER::MeshCollection::getNbOfLocalMeshes() const
{
    int nb = 0;
...
mcIdType
MEDPARTITIONER::MeshCollection::getNbOfLocalCells() const
{
    mcIdType nb = 0;
    for (size_t i = 0; i < _mesh.size(); i++)
    {
        if (_mesh[i])
            nb = nb + _mesh[i]->getNumberOfCells();
    }
    return nb;
}

mcIdType
MEDPARTITIONER::MeshCollection::getNbOfLocalFaces() const
{
    mcIdType nb = 0;
    for (size_t i = 0; i < _face_mesh.size(); i++)
    {
        if (_face_mesh[i])
            nb = nb + _face_mesh[i]->getNumberOfCells();
    }
    return nb;
}

std::vector<MEDCoupling::MEDCouplingUMesh *> &
MEDPARTITIONER::MeshCollection::getMesh()
{
    return _mesh;
}

std::vector<MEDCoupling::MEDCouplingUMesh *> &
MEDPARTITIONER::MeshCollection::getFaceMesh()
{
    return _face_mesh;
}

MEDCoupling::MEDCouplingUMesh *
MEDPARTITIONER::MeshCollection::getMesh(int idomain) const
{
    return _mesh[idomain];
}

MEDCoupling::MEDCouplingUMesh *
MEDPARTITIONER::MeshCollection::getFaceMesh(int idomain)
{
    return _face_mesh[idomain];
}

std::vector<MEDPARTITIONER::ConnectZone *> &
MEDPARTITIONER::MeshCollection::getCZ()
{
    if (_topology)
...
MEDPARTITIONER::MeshCollection::getTopology() const
{
    return _topology;
}

void
MEDPARTITIONER::MeshCollection::setTopology(Topology *topo, bool takeOwneship)
{
    if (_topology != 0)
    {
 ...
 */
void
MEDPARTITIONER::MeshCollection::buildCellGraph(MEDCoupling::MEDCouplingSkyLineArray *&array, int *&edgeweights)
{
    using std::make_pair;
    using std::map;
    using std::pair;
    ...
    if (_topology->nbDomain() > 1)
        throw INTERP_KERNEL::Exception("buildCellGraph should be used for one domain only");
 ...
 */
void
MEDPARTITIONER::MeshCollection::buildParallelCellGraph(MEDCoupling::MEDCouplingSkyLineArray *&array, int *&edgeweights)
{
    using std::make_pair;
    using std::multimap;
    using std::pair;
    ...
    for (int idomain = 0; idomain < nbdomain; idomain++)
    {
        ...
        if (MyGlobals::_Verbose > 100)
            std::cout << "proc " << MyGlobals::_Rank << " : getReverseNodalConnectivity done on " << nbNodes << " nodes"
                      << std::endl;
        ...
        for (int iother = 0; iother < nbdomain; iother++)
        {
            ...
            for (it = _joint_finder->getDistantNodeCell()[isource][itarget].begin();
                 it != _joint_finder->getDistantNodeCell()[isource][itarget].end();
                 it++)
            {
    ...
    if (MyGlobals::_Verbose > 50)
        std::cout << "proc " << MyGlobals::_Rank << " : creating graph arcs on nbNodes " << _topology->nbNodes()
                  << std::endl;
    ...
    for (mcIdType inode = 0; inode < _topology->nbNodes(); inode++)
    {
        ...
        for (MI cell1 = nodeRange.first; cell1 != nodeRange.second; cell1++)
            for (MI cell2 = nodeRange.first; cell2 != cell1; cell2++)
            {
                ...
                if (icell1 > icell2)
                    std::swap(icell1, icell2);
                std::map<pair<mcIdType, mcIdType>, mcIdType>::iterator it =
                    cell2cellcounter.find(make_pair(icell1, icell2));
                if (it == cell2cellcounter.end())
                    cell2cellcounter.insert(make_pair(make_pair(icell1, icell2), 1));
                else
                    (it->second)++;
            }
    }
    ...
    if (MyGlobals::_Verbose > 100)
    {
        ...
        if (index.size() > 1)
        {
            for (std::size_t i = 0; i < max; ++i) std::cout << index[i] << " ";
            std::cout << "... " << index[index.size() - 1] << std::endl;
            for (std::size_t i = 0; i < max; ++i) std::cout << value[i] << " ";
 ...
 */
MEDPARTITIONER::Topology *
MEDPARTITIONER::MeshCollection::createPartition(
    int nbdomain,
...
{
    if (MyGlobals::_Verbose > 10)
        std::cout << "proc " << MyGlobals::_Rank << " : MeshCollection::createPartition : Building cell graph"
                  << std::endl;
    ...
    switch (split)
    {
        case Graph::METIS:
            ...
            if (!cellGraph)
                throw INTERP_KERNEL::Exception(
                    "MeshCollection::createPartition : PARMETIS/METIS is not available (or you're not running in //). "
                    "Check your products, please."
                );
            break;
...
#else
            throw INTERP_KERNEL::Exception(
                "MeshCollection::createPartition : SCOTCH is not available. Check your products, please."
            );
#endif
            break;
    }
 ...
 */
MEDPARTITIONER::Topology *
MEDPARTITIONER::MeshCollection::createPartition(const int *partition)
{
    MEDCoupling::MEDCouplingSkyLineArray *array = 0;
    int *edgeweights = 0;
...
void
MEDPARTITIONER::MeshCollection::setDomainNames(const std::string &name)
{
...
MEDPARTITIONER::MeshCollection::getField(std::string descriptionField, int iold)
...
// something like MCAuto<MEDCouplingFieldDouble> f2=ReadFieldCell(name,f1->getMesh()->getName(),0,f1->getName(),0,1);
{
    ...
    if (MyGlobals::_Verbose > 10)
        std::cout << "proc " << MyGlobals::_Rank << " : in " << fileName << " " << iold << " " << descriptionIold
                  << std::endl;
    FieldShortDescriptionToData(descriptionIold, fieldName, typeField, entity, DT, IT);
    meshName = MyGlobals::_Mesh_Names[iold];

    MCAuto<MEDCoupling::MEDCouplingField> f2Tmp(
        ReadField((MEDCoupling::TypeOfField)typeField, fileName, meshName, 0, fieldName, DT, IT)
    );
    MCAuto<MEDCoupling::MEDCouplingFieldDouble> f2(
        MEDCoupling::DynamicCast<MEDCouplingField, MEDCouplingFieldDouble>(f2Tmp)
    );
...
void
MEDPARTITIONER::MeshCollection::prepareFieldDescriptions()
...
// filter _field_descriptions to be in all procs compliant and equal
{
    ...
    for (std::size_t i = 0; i < _field_descriptions.size(); i++)
    {
        ...
        for (std::size_t ii = 0; ii < r1.size(); ii++) r2.push_back(r1[ii]);
    }
    ...
    if ((nbfields % nbfiles) != 0)
    {
        if (MyGlobals::_Rank == 0)
        {
            std::cerr << "\nERROR : incoherent number of fields references in all files .med\n"
                      ...
                      << ReprVectorOfString(MyGlobals::_File_Names) << "field_descriptions :" << std::endl
...
bool
isFaceOncell(std::vector<mcIdType> &inodesFace, std::vector<mcIdType> &inodesCell)
{
    std::size_t ires = 0;
...
void
MEDPARTITIONER::MeshCollection::filterFaceOnCell()
{
    for (int inew = 0; inew < _topology->nbDomain(); inew++)
    {
        if (!isParallelMode() || _domain_selector->isMyDomain(inew))
        {
            if (MyGlobals::_Verbose > 200)
                std::cout << "proc " << MyGlobals::_Rank << " : filterFaceOnCell on inewDomain " << inew
                          << " nbOfFaces " << _face_mesh[inew]->getNumberOfCells() << std::endl;
            ...
            for (mcIdType iface = 0; iface < nbface; iface++)
            {
                ...
                int nbok = 0;
                for (int i = 0; i < nbnodFace; i++) nbok += ((inodesFace[i] = nodeIds[inodesFace[i]]) >= 0);
                if (nbok != nbnodFace)
                    continue;
            ...
            if (faceOnCell.empty())
                ...
                _face_mesh[inew] = (MEDCoupling::MEDCouplingUMesh *)mfac->buildPartOfMySelf(
                    &faceOnCell[0], &faceOnCell[0] + faceOnCell.size(), true
                );
            mfac->decrRef();
            ...
            if (getMapDataArrayInt().count(key))
            {
                ...
                for (size_t i = 0; i < faceOnCell.size(); i++) pfamFilter[i] = pfam[faceOnCell[i]];
                fam->decrRef();
                fam = famFilter;
            }
        }
