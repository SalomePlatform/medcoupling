...
MEDPARTITIONER::SendAndReceiveVectorOfString(const std::vector<std::string> &vec, const int source, const int target)
{
 ...
 */
std::vector<std::string>
MEDPARTITIONER::AllgathervVectorOfString(const std::vector<std::string> &vec)
{
    ...
    int size = (int)str.length();
    MPI_Allgather(&size, 1, MPI_INT, &indexes[0], 1, MPI_INT, MPI_COMM_WORLD);
...
*/
void
MEDPARTITIONER::SendDoubleVec(const std::vector<double> &vec, const int target)
{
    int tag = 111002;
...
*/
std::vector<double> *
MEDPARTITIONER::RecvDoubleVec(const int source)
{
    int tag = 111002;
    int size;
#ifdef HAVE_MPI
...
void
MEDPARTITIONER::RecvDoubleVec(std::vector<double> &vec, const int source)
{
    int tag = 111002;
    int size;
#ifdef HAVE_MPI
...
*/
void
MEDPARTITIONER::SendIntVec(const std::vector<mcIdType> &vec, const int target)
{
    int tag = 111003;
    int size = (int)vec.size();
    if (MyGlobals::_Verbose > 1000)
...
*/
std::vector<int> *
MEDPARTITIONER::RecvIntVec(const int source)
{
    int tag = 111003;
    int size;
#ifdef HAVE_MPI
...
void
MEDPARTITIONER::RecvIntVec(std::vector<mcIdType> &vec, const int source)
{
    int tag = 111003;
    int size;
#ifdef HAVE_MPI
...
*/
void
MEDPARTITIONER::SendDataArrayInt(const MEDCoupling::DataArrayInt *da, const int target)
{
    if (da == 0)
...
MEDPARTITIONER::RecvDataArrayInt(const int source)
{
    int tag = 111004;
    int size[3];
#ifdef HAVE_MPI
...
*/
void
MEDPARTITIONER::SendDataArrayDouble(const MEDCoupling::DataArrayDouble *da, const int target)
{
    if (da == 0)
...
MEDPARTITIONER::RecvDataArrayDouble(const int source)
{
    int tag = 111005;
    int size[3];
#ifdef HAVE_MPI
...
void
MEDPARTITIONER::TestVectorOfStringMpi()
{
    int rank = MyGlobals::_Rank;
    int world_size = MyGlobals::_World_Size;
    std::vector<std::string> myVector;
    std::ostringstream oss;
    oss << "hello from " << std::setw(5) << rank << " " << std::string(rank + 1, 'n') << " next is an empty one";
...
void
MEDPARTITIONER::TestMapOfStringIntMpi()
{
    int rank = MyGlobals::_Rank;
...
void
MEDPARTITIONER::TestMapOfStringVectorOfStringMpi()
{
    int rank = MyGlobals::_Rank;
    std::vector<std::string> myVector;
    std::ostringstream oss;
    oss << "hello from " << std::setw(5) << MyGlobals::_Rank << " " << std::string(rank + 1, 'n')
...
void
MEDPARTITIONER::TestDataArrayMpi()
{
    int rank = MyGlobals::_Rank;
    // int
    {
    ...
    // double
    {
        ...
        std::copy(vals.begin(), vals.end(), send->getPointer());
        if (rank == 0)
            SendDataArrayDouble(send, 1);
        if (rank == 1)
            recv = RecvDataArrayDouble(0);
        if (rank == 1 && MyGlobals::_Verbose > 20)
        {
            std::cout << send->repr() << std::endl;
            std::cout << recv->repr() << std::endl;
...
void
MEDPARTITIONER::TestPersistantMpi0To1(int taille, int nb)
{
...
void
MEDPARTITIONER::TestPersistantMpiRing(int taille, int nb)
{
    double temps_debut = MPI_Wtime();
...
void
MEDPARTITIONER::TestPersistantMpiRingOnCommSplit(int size, int nb)
{
    double temps_debut = MPI_Wtime();
    int rank = MyGlobals::_Rank;
    MPI_Comm newcomm;
