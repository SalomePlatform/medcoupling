 ...
 * */
int
MeshCollectionDriver::readSeq(const char *filename, const char *meshname)
{
...
void
MeshCollectionDriver::readMEDFileData(const MEDCoupling::MEDFileData *filedata)
{
    ...
    for (int i = 0; i < nbDomains; i++)
    {
        MEDCoupling::MEDFileUMesh *mfm =
            dynamic_cast<MEDCoupling::MEDFileUMesh *>(filedata->getMeshes()->getMeshAtPos(i));
        readData(mfm, i);
...
void
MeshCollectionDriver::readFileData(std::string file, std::string meshname, int idomain) const
{
...
void
MeshCollectionDriver::readData(MEDCoupling::MEDFileUMesh *mfm, int idomain) const
{
...
void
MeshCollectionDriver::readSubdomain(int idomain)
{
...
MEDCoupling::MEDFileMesh *
MeshCollectionDriver::getMesh(int idomain) const
{
    ...
    //     faceMeshFilter=(MEDCoupling::MEDCouplingUMesh *)
    //     faceMesh->buildPartOfMySelf(index,index+filter->getNbOfElems(),true); faceMesh=faceMeshFilter;
    //   }
    // if (faceMeshFilter!=0)
    ...
    if (_collection->getCZ().size() > 0)
    {
        ...
        for (size_t i = 0; i < _collection->getCZ().size(); ++i)
        {
            ...
            if (!cz || cz->getLocalDomainNumber() != idomain)
                continue;
            {
            ...
            MCAuto<MEDFileJoint> joint =
                MEDFileJoint::New(cz->getName(), finalMeshName, finalMeshName, cz->getDistantDomainNumber());
            ...
            if (nodeCorr)
            {
                MCAuto<MEDFileJointCorrespondence> corr = MEDFileJointCorrespondence::New(nodeCorr->getValuesArray());
            ...
            {
                const MEDCouplingSkyLineArray *cellCorr = cz->getEntityCorresp(types[it].first, types[it].second);
                if (cellCorr && cellCorr->getNumberOf() > 0)
                {
                    ...
                    MCAuto<MEDFileJointCorrespondence> corr =
                        MEDFileJointCorrespondence::New(cellCorr->getValuesArray(), t1, t2);
                    j1st->pushCorrespondence(corr);
                }
            }
        }
...
) const
{
    ...
    if (field && typeData == 6)
    {
        ...
        {
            std::cerr << "Problem On field " << fieldName << " : number of components unexpected "
                      << da->getNumberOfComponents() << std::endl;
        }
        field->setArray(da);
...
void
MeshCollectionDriver::writeMedFile(int idomain, const std::string &distfilename) const
{
    ...
    for (it = _collection->getMapDataArrayDouble().begin(); it != _collection->getMapDataArrayDouble().end(); it++)
    {
        ...
        catch (INTERP_KERNEL::Exception &e)
        {
            ...
            std::cout << "WARNING : writeMedFile : create a new file name with only one field because WriteField throw:"
                      << newName << std::endl;
...
MeshCollectionDriver::getMEDFileData()
{
