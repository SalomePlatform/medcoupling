...
METISGraph::METISGraph(MEDCoupling::MEDCouplingSkyLineArray *graph, int *edgeweight) : Graph(graph, edgeweight) {}

METISGraph::~METISGraph() {}

void
METISGraph::partGraph(int ndomain, const std::string &options_string, ParaDomainSelector *parallelizer)
{
    using std::vector;
    ...
    */
    ...
    if (nparts > 1)
    {
        ...
        if (options_string != "k")
            MEDPARTITIONER_METIS_PartGraphRecursive(
                &n, xadj, adjncy, vwgt, adjwgt, &wgtflag, &base, &nparts, options, &edgecut, partition
            );
        else
            MEDPARTITIONER_METIS_PartGraphKway(
                &n, xadj, adjncy, vwgt, adjwgt, &wgtflag, &base, &nparts, options, &edgecut, partition
            );
    }
