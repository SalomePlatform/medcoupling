    ...
    : _input_collection(0), _output_collection(0), _new_topology(0)
{
    ...
    MEDPARTITIONER::ParallelTopology *aPT = (MEDPARTITIONER::ParallelTopology *)_input_collection->getTopology();
    ...
    : _input_collection(0), _output_collection(0), _new_topology(0)
{
    ...
    _new_topology =
        new MEDPARTITIONER::ParallelTopology(graph, aPT, graph->nbDomains(), _input_collection->getMeshDimension());
...
MEDPARTITIONER::MEDPartitioner::~MEDPartitioner()
{
    delete _input_collection;
    _input_collection = 0;
    delete _output_collection;
    _output_collection = 0;
    delete _new_topology;
    _new_topology = 0;
}

void
MEDPARTITIONER::MEDPartitioner::createPartitionCollection(
...
void
MEDPARTITIONER::MEDPartitioner::write(const std::string &filename)
{
...
MEDPARTITIONER::MEDPartitioner::getMEDFileData()
{
    return _output_collection->retrieveDriver()->getMEDFileData();
}

MEDPARTITIONER::Graph *
MEDPARTITIONER::MEDPartitioner::Graph(
    MEDCoupling::MEDCouplingSkyLineArray *graph, Graph::splitter_type split, int *edgeweight, DataArrayIdType *vlbloctab
)
{
    MEDPARTITIONER::Graph *cellGraph = 0;
    // will be destroyed by XXXGraph class:
    MEDCoupling::MCAuto<MEDCoupling::MEDCouplingSkyLineArray> arr(
        MEDCoupling::MEDCouplingSkyLineArray::New(graph->getIndexArray(), graph->getValuesArray())
    );
    switch (split)
    {
        case Graph::METIS:
            ...
            if (!cellGraph)
                throw INTERP_KERNEL::Exception(
                    "MEDPartitioner::Graph : PARMETIS/METIS is not available. Check your products, please."
                );
            break;
        case Graph::SCOTCH:
#ifdef MED_ENABLE_SCOTCH
...
#else
            throw INTERP_KERNEL::Exception(
                "MEDPartitioner::Graph : SCOTCH is not available. Check your products, please."
            );
#endif
            break;
        case Graph::PTSCOTCH:
...
#else
            throw INTERP_KERNEL::Exception(
                "MEDPartitioner::Graph : PTSCOTCH is not available. Check your products, please."
            );
#endif
            break;
        }
...
MEDPARTITIONER::MEDPartitioner::AvailableAlgorithms()
{
...
bool
MEDPARTITIONER::MEDPartitioner::HasMetisAlg()
{
#ifdef MED_ENABLE_METIS
    return true;
#else
...
bool
MEDPARTITIONER::MEDPartitioner::HasScotchAlg()
{
#ifdef MED_ENABLE_SCOTCH
    return true;
#else
...
bool
MEDPARTITIONER::MEDPartitioner::HasPTScotchAlg()
{
#ifdef MED_ENABLE_PTSCOTCH
    return true;
#else
...
std::vector<std::string>
MEDPARTITIONER::MEDPartitioner::AllAlgorithms()
{
    std::vector<std::string> ret;
    ret.push_back(std::string(METIS_PART_ALG));
    ret.push_back(std::string(SCOTCH_PART_ALG));
