...
int
MeshCollectionMedXmlDriver::read(const char *filename, ParaDomainSelector *domainSelector)
{
    // distributed meshes
    int nbdomain;
    ...
    try
    {
        ...
        for (int i = 0; i < nbfiles; i++)
        {
            ...
            xmlXPathObjectPtr xpathMeshObj =
                xmlXPathEvalExpression(BAD_CAST mesh_search_string.str().c_str(), xpathCtx);
 ...
 */
void
MeshCollectionMedXmlDriver::write(const char *filename, ParaDomainSelector *domainSelector) const
{
    ...
    sprintf(date, "%02d%02d%02d", time_asc->tm_year, time_asc->tm_mon + 1, time_asc->tm_mday);
#else
    SYSTEMTIME st;
    GetLocalTime(&st);
