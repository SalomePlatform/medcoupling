...
void
MEDPARTITIONERTest::setSize(int ni, int nj, int nk)
{
...
void
MEDPARTITIONERTest::setSmallSize()
{
    setSize(2, 3, 5);  // nb of hexa9
}

void
MEDPARTITIONERTest::setMedianSize()
{
    setSize(20, 30, 50);  // nb of hexa9
}

void
MEDPARTITIONERTest::setbigSize()
{
    setSize(200, 300, 500);  // nb of hexa9
}

std::string
MEDPARTITIONERTest::getPartitionerExe() const
{
    std::string execName;
...
// ============================================================================
void
MEDPARTITIONERTest::tearDown()
{
}

MEDCoupling::MEDCouplingUMesh *
MEDPARTITIONERTest::buildCUBE3DMesh()
// only hexa8
{
...
MEDCoupling::MEDCouplingUMesh *
MEDPARTITIONERTest::buildCARRE3DMesh()
// only quad4 in oblique (k=j)
{
    ...
    if (false)  //(_verbose)
    {
        ...
        for (int i = 0; i < (int)coor.size(); i++) cout << coor[i] << " ";
        cout << endl;
...
MEDCoupling::MEDCouplingUMesh *
MEDPARTITIONERTest::buildFACE3DMesh()
...
MEDPARTITIONERTest::buildVecFieldOnCells(string myfileName)
{
...
MEDCouplingFieldDouble *
MEDPARTITIONERTest::buildVecFieldOnNodes()
{
    // int ni=2,nj=3,nk=5; //nb of hexa9
    vector<double> field;
    for (int k = 0; k <= _nk; k++)
...
MEDPARTITIONERTest::createTestMeshWithoutField()
{
...

    {
        ...
        MEDCoupling::MEDFileUMesh *mfm =
            MEDCoupling::MEDFileUMesh::New(_file_name_with_faces.c_str(), mesh1->getName().c_str());
        ...
        /*ce truc marche pas!
          MEDCoupling::MEDFileUMesh* mfm=MEDCoupling::MEDFileUMesh::New(_file_name_with_faces.c_str(),
          mesh1->getName()); vector<const MEDCoupling::MEDCouplingUMesh*> ms; ms.push_back(mesh2);
          mfm->setGroupsFromScratch(-1, ms);
          mfm->write(_file_name_with_faces.c_str(),0);
        */

        if (_verbose)
            cout << endl << _file_name_with_faces << " created" << endl;
        if (_ntot < 1000000)  // too long
        {
            MEDCouplingUMesh *mesh_rw = ReadUMeshFromFile(_file_name_with_faces.c_str(), mesh1->getName().c_str(), 0);
            if (_verbose)
                cout << _file_name_with_faces << " reread" << endl;
...
*/
void
MEDPARTITIONERTest::createHugeTestMesh(int ni, int nj, int nk, int nbx, int nby, int nbz, int nbTarget)
{
    setSize(ni, nj, nk);
...
<root>\n \
  ...
  <mapping>\n$tagMesh \
    ...
    for (int z = 0; z < nbz; z++)
    {
        ...
        for (int y = 0; y < nby; y++)
        {
            ...
            for (int x = 0; x < nbx; x++)
            {
                string fileName;
                sxyz = IntToStr(xyz);
                fileName = "tmp_testMeshHuge_" + IntToStr(_ni) + "x" + IntToStr(_nj) + "x" + IntToStr(_nk) + "_" +
                           sxyz + ".med";
    ...
    string nameFileXml;
    _file_name_huge_xml =
        "tmp_testMeshHuge_" + IntToStr(_ni) + "x" + IntToStr(_nj) + "x" + IntToStr(_nk) + "_" + sxyz + ".xml";
...
void
MEDPARTITIONERTest::createTestMeshWithVecFieldOnCells()
{
    {
        ...
        if (_verbose)
            cout << endl << name << " created" << endl;
        if (_ntot < 1000000)  // too long
        {
            MCAuto<MEDCoupling::MEDCouplingField> f2Tmp(
                ReadFieldCell(name.c_str(), f1->getMesh()->getName().c_str(), 0, f1->getName().c_str(), 0, 1)
            );
    ...
    }
    {
        ...
        */
        ...
        if (_ntot < 1000000)  // too long
        {
            MCAuto<MEDCoupling::MEDCouplingField> f4Tmp(
                ReadField(ON_GAUSS_NE, name.c_str(), f3->getMesh()->getName().c_str(), 0, "MyFieldOnGaussNE", 5, 6)
            );
            MCAuto<MEDCoupling::MEDCouplingFieldDouble> f4(
                MEDCoupling::DynamicCast<MEDCouplingField, MEDCouplingFieldDouble>(f4Tmp)
            );
            if (_verbose)
                cout << "MyFieldOnGaussNE reread" << endl;
        }
        f3->decrRef();
    }
    {
...
void
MEDPARTITIONERTest::createTestMeshWithVecFieldOnNodes()
{
    ...
    if (_ntot < 1000000)  // too long
    {
        MCAuto<MEDCouplingField> f2 =
            ReadFieldNode(name.c_str(), f1->getMesh()->getName().c_str(), 0, f1->getName().c_str(), 0, 1);
        if (_verbose)
            cout << name << " reread" << endl;
        // CPPUNIT_ASSERT(f1->isEqual(f2,1e-12,1e-12)); assertion failed!!
    }
...
void
MEDPARTITIONERTest::verifyTestMeshWithVecFieldOnNodes()
{
    string name = _file_name;
    name.replace(name.find(".med"), 4, "_WithVecFieldOnNodes.med");
    ...
    {
        cout << "\n types in " << name << " : ";
        // for (std::set<INTERP_KERNEL::NormalizedCellType>::iterator t=types.begin(); t!=types.end(); ++t) cout<<"
        // "<<*t;
...
void
MEDPARTITIONERTest::createTestMeshes()
{
...
void
MEDPARTITIONERTest::deleteTestMeshes()
{
...
void
MEDPARTITIONERTest::testMeshCollectionSingle()
{
    setSmallSize();
    createTestMeshes();
    MyGlobals::_World_Size = 1;
...
void
MEDPARTITIONERTest::testMeshCollectionXml()
{
    setSmallSize();
...
#if defined(MED_ENABLE_METIS)
void
MEDPARTITIONERTest::testMeshCollectionSinglePartitionMetis()
{
    setSmallSize();
    createTestMeshes();
    ...
    // \n"<<ReprVectorOfString(MyGlobals::_General_Informations); new_collection.write("ttmp")
...
void
MEDPARTITIONERTest::testMeshCollectionComplexPartitionMetis()
{
    setSmallSize();
...
void
MEDPARTITIONERTest::testMetisSmallSize()
{
    // #if !defined(HAVE_MPI)
    setSmallSize();
    createTestMeshes();
...
#if defined(MED_ENABLE_SCOTCH)
void
MEDPARTITIONERTest::testMeshCollectionSinglePartitionScotch()
{
    setSmallSize();
    createTestMeshes();
    // MyGlobals::_Verbose=500;
...
void
MEDPARTITIONERTest::testMeshCollectionComplexPartitionScotch()
{
    setSmallSize();
    createHugeTestMesh(_ni, _nj, _nk, 2, 2, 2, 32);  // xml on 2*2*2 meshes but not so huge
    string fileName = _file_name_huge_xml;
...
void
MEDPARTITIONERTest::testScotchSmallSize()
{
    // #if !defined(HAVE_MPI)
    setSmallSize();
    createTestMeshes();
...
void
MEDPARTITIONERTest::launchMetisOrScotchMedpartitionerOnTestMeshes(std::string MetisOrScotch)
{
    int res;
...
void
MEDPARTITIONERTest::verifyMetisOrScotchMedpartitionerOnSmallSizeForMesh(std::string MetisOrScotch)
{
    int res;
    ...
    */
    ...
    meshes.resize(0);
    for (std::size_t i = 0; i < corr.size(); i++) corr[i]->decrRef();
    corr.resize(0);
...
void
MEDPARTITIONERTest::verifyMetisOrScotchMedpartitionerOnSmallSizeForFieldOnCells(std::string MetisOrScotch)
{
    int res;
    string fileName, cmd, execName, sourceName, targetName, input;
    execName = getPartitionerExe();
    ...
    MCAuto<MEDCouplingField> field1Tmp(
        ReadFieldCell(fileName.c_str(), initialMesh->getName().c_str(), 0, "VectorFieldOnCells", 0, 1)
    );
    MCAuto<MEDCouplingField> field2Tmp(
        ReadFieldCell(refusedName.c_str(), refusedCellMesh->getName().c_str(), 0, "VectorFieldOnCells", 0, 1)
    );
    MCAuto<MEDCouplingFieldDouble> field1(
        MEDCoupling::DynamicCast<MEDCouplingField, MEDCouplingFieldDouble>(field1Tmp)
    ),
        field2(MEDCoupling::DynamicCast<MEDCouplingField, MEDCouplingFieldDouble>(field2Tmp));
    ...
    if (_verbose > 300)
    {
        ...
        for (std::size_t i = 0; i < corr.size(); i++) cout << "\ncorr " << i << " : " << corr[i]->reprZip();
    }
    int nbequal = 0;
...
void
MEDPARTITIONERTest::verifyMetisOrScotchMedpartitionerOnSmallSizeForFieldOnGaussNe(std::string MetisOrScotch)
{
    int res;
    string fileName, cmd, execName, sourceName, targetName, input;
    execName = getPartitionerExe();
    ...
    MCAuto<MEDCouplingField> field1Tmp =
        ReadField(ON_GAUSS_NE, fileName.c_str(), initialMesh->getName().c_str(), 0, "MyFieldOnGaussNE", 5, 6);
    MCAuto<MEDCouplingField> field2Tmp =
        ReadField(ON_GAUSS_NE, refusedName.c_str(), refusedCellMesh->getName().c_str(), 0, "MyFieldOnGaussNE", 5, 6);
    MCAuto<MEDCouplingFieldDouble> field1(
        MEDCoupling::DynamicCast<MEDCouplingField, MEDCouplingFieldDouble>(field1Tmp)
    ),
        field2(MEDCoupling::DynamicCast<MEDCouplingField, MEDCouplingFieldDouble>(field2Tmp));
...
void
MEDPARTITIONERTest::testCreateBoundaryFaces2D()
{
    // Fixed complains are:
    // - 2D is not available
    ...
    int nbFam1, nbFam2, nbc;
    {
        ...
        for (int i = 0; i < nbc; i++, pConn += 4) mesh->insertNextCell(INTERP_KERNEL::NORM_QUAD4, 4, pConn);
        mesh->finishInsertingCells();
    ...
    for (; na2id != famName2id.end(); ++na2id)
    {
        ...
        CPPUNIT_ASSERT_EQUAL((int)famId2nb[na2id2->second], (int)famId2nb[na2id->second]);
