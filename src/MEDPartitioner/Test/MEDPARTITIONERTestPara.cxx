...
#if defined(HAVE_MPI)
std::string
MEDPARTITIONERTest::getPartitionerParaExe() const
{
    std::string execName;
...
void
MEDPARTITIONERTest::verifyMedpartitionerOnSmallSizeForMesh()
{
    int res;
...
void
MEDPARTITIONERTest::verifyMedpartitionerOnSmallSizeForFieldOnCells()
{
    int res;
    string fileName, cmd, execName, sourceName, targetName, input;
    execName = getPartitionerParaExe();
    ...
    MCAuto<MEDCouplingFieldDouble> field1 = dynamic_cast<MEDCouplingFieldDouble *>(
        ReadFieldCell(fileName.c_str(), initialMesh->getName().c_str(), 0, "VectorFieldOnCells", 0, 1)
    );
    MCAuto<MEDCouplingFieldDouble> field2 = dynamic_cast<MEDCouplingFieldDouble *>(
...
void
MEDPARTITIONERTest::verifyMedpartitionerOnSmallSizeForFieldOnGaussNe()
{
    int res;
    string fileName, cmd, execName, sourceName, targetName, input;
    execName = getPartitionerParaExe();
...
void
MEDPARTITIONERTest::launchMedpartitionerOnTestMeshes()
{
    /* examples
       ...
       mpirun -np 2 valgrind  medpartitioner_para --input-file=tmp_testMesh_20x30x50.med  --output-file=ttmp1petit_
       --ndomains=4  --dump-cpu-memory --verbose=111
    */
    int res;
...
void
MEDPARTITIONERTest::launchMedpartitionerOnHugeTestMeshes()
{
    int res = 0;
    string cmd, execName, sourceName, targetName;
    execName = getPartitionerParaExe();
    ...
    cmd = "mpirun -np " + snbTarget + " " + execName + " --ndomains=" + snbTarget +
          " --split-method=metis";  // on same proc
...
void
MEDPARTITIONERTest::testMpirunSmallSize()
{
    setSmallSize();
    createTestMeshes();
...
void
MEDPARTITIONERTest::testMpirunMedianSize()
{
    setMedianSize();
    createTestMeshes();
    launchMedpartitionerOnTestMeshes();
...
void
MEDPARTITIONERTest::testMpirunHugeSize()
{
