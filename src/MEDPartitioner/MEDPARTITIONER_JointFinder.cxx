...
void
MEDPARTITIONER::JointFinder::findCommonDistantNodes()
{
    ...
    for (int isource = 0; isource < nbdomain; isource++)
    {
        for (int itarget = 0; itarget < nbdomain; itarget++)
        {
            ...
            if (_domain_selector->isMyDomain(isource))
            {
                ...
                std::copy(
                    sourceMesh->getCoords()->getConstPointer(),
                    sourceMesh->getCoords()->getConstPointer() + sourceMesh->getNumberOfNodes() * spaceDim,
                    &vec[0]
                );
                ...
                {
                    _distant_node_cell[isource][itarget].insert(
                        std::make_pair(localCorrespondency[2 * i], localCorrespondency[2 * i + 1])
                    );
                }
            }
            ...
            if (_domain_selector->isMyDomain(itarget))
            {
                ...
                for (std::map<mcIdType, mcIdType>::iterator iter = commonNodes.begin(); iter != commonNodes.end();
                     iter++)
                {
                    _node_node[itarget][isource].push_back(
                        std::make_pair(iter->first, iter->second)
                    );  // storing node pairs in a vector
                ...
                SendIntVec(
                    nodeCellCorrespondency, sourceProc
                );  // itarget proc send to other (otherLocalNode-itargetGlobalCell)
            }
        }
    }
...
std::vector<std::vector<std::multimap<mcIdType, mcIdType> > > &
MEDPARTITIONER::JointFinder::getDistantNodeCell()
{
    return _distant_node_cell;
}

std::vector<std::vector<std::vector<std::pair<mcIdType, mcIdType> > > > &
MEDPARTITIONER::JointFinder::getNodeNode()
{
    return _node_node;
}

void
MEDPARTITIONER::JointFinder::print()
// it is for debug on small arrays under mpi 2,3 cpus
{
        ...
        for (int itarget = 0; itarget < nbdomain; itarget++)
        {
            for (std::size_t i = 0; i < _node_node[itarget][isource].size(); i++)
                std::cout << " nn" << _domain_selector->rank() << itarget << "|" << isource << "|" << i << "|"
                          << _node_node[itarget][isource][i].first << "-" << _node_node[itarget][isource][i].second;
        }
    }
    std::cout << std::endl;
    // MPI_Barrier(MPI_COMM_WORLD);
    for (int isource = 0; isource < nbdomain; isource++)
    {
        for (int itarget = 0; itarget < nbdomain; itarget++)
        {
            ...
            for (it = _distant_node_cell[isource][itarget].begin(); it != _distant_node_cell[isource][itarget].end();
                 it++)
            {
                std::cout << " nc" << _domain_selector->rank() << "|" << itarget << "|" << isource << "|" << (*it).first
                          << "=" << (*it).second;
            }
        }
    }
