...
PTSCOTCHGraph::PTSCOTCHGraph(MEDCoupling::MEDCouplingSkyLineArray *graph, int *edgeweight, DataArrayIdType *vlbloctab)
    : Graph(graph, edgeweight), _vlbloctab(vlbloctab)
{
}
...
void
PTSCOTCHGraph::partGraph(int ndomain, const std::string &options_string, ParaDomainSelector *sel)
{
    ...
    throw INTERP_KERNEL::Exception(
        "PTSCOTCHGraph::partGraph : PTSCOTCH is not available. Check your products, please."
    );
#else
    // output parameters
    ...
    {
        for (int i = 0; i < n + 1; i++) partition[i] = 0;
    }
