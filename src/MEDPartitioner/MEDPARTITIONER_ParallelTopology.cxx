...
ParallelTopology::ParallelTopology(const std::vector<MEDCoupling::MEDCouplingUMesh *> &meshes)
{
    ...
    for (int idomain = 0; !parallel_mode && idomain < _nb_domain; idomain++) parallel_mode = (!meshes[idomain]);
...
void
ParallelTopology::setGlobalNumerotationDefault(ParaDomainSelector *domainSelector)
{
    if (MyGlobals::_Is0verbose > 100)
        std::cout << "setGlobalNumerotationDefault on " << _nb_domain << " domains\n";
    if (_loc_to_glob.size() != 0)
        throw INTERP_KERNEL::Exception("a global numerotation is done yet");
...
#ifdef HAVE_MPI
    if (MyGlobals::_Verbose > 500 && MyGlobals::_World_Size > 1)
        MPI_Barrier(MPI_COMM_WORLD);  // synchronize verbose trace
#endif
    if (MyGlobals::_Is0verbose > 500)
        std::cout << std::endl;

    if (MyGlobals::_Is0verbose > 500)
        std::cout << "(n)idomain|ilocalNode|iglobalNode" << std::endl;
    for (int idomain = 0; idomain < _nb_domain; idomain++)
    {
    ...
    if (MyGlobals::_Is0verbose > 200)
        std::cout << "globalNumerotation default done meshDimension " << _mesh_dimension << " nbTotalCells "
                  << _nb_total_cells << " nbTotalNodes " << _nb_total_nodes << std::endl;
}

// constructing topology according to mesh collection
ParallelTopology::ParallelTopology(
    const std::vector<MEDCoupling::MEDCouplingUMesh *> &meshes,
...
ParallelTopology::ParallelTopology(Graph *graph, Topology *oldTopology, int nb_domain, int mesh_dimension)
{
    ...
    if (MyGlobals::_Verbose > 200)
        std::cout << "proc " << MyGlobals::_Rank << " : new topology oldNbDomain " << oldTopology->nbDomain()
                  << " newNbDomain " << _nb_domain << std::endl;
    ...
    if (MyGlobals::_Verbose > 300)
        std::cout << "proc " << MyGlobals::_Rank << " : topology from partition, nbTotalCells " << _nb_total_cells
                  << std::endl;
    ...
    if (MyGlobals::_Verbose > 300)
        for (int idomain = 0; idomain < _nb_domain; idomain++)
            std::cout << "proc " << MyGlobals::_Rank << " : nbCells in new domain " << idomain << " : "
                      << _nb_cells[idomain] << std::endl;
 ...
 * */
void
ParallelTopology::convertGlobalNodeList(const mcIdType *node_list, mcIdType nbnode, mcIdType *local, int *ip)
{
 ...
 * */
void
ParallelTopology::convertGlobalNodeListWithTwins(
    const mcIdType *node_list, mcIdType nbnode, mcIdType *&local, int *&ip, mcIdType *&full_array, mcIdType &size
)
{
    if (_node_glob_to_loc.empty())
        throw INTERP_KERNEL::Exception("Node mapping has not yet been built");
 ...
 * */
void
ParallelTopology::convertGlobalFaceListWithTwins(
...
//! to a distributed array with local cell numbers
void
ParallelTopology::convertGlobalCellList(const mcIdType *cell_list, mcIdType nbcell, mcIdType *local, int *ip)
{
    for (mcIdType i = 0; i < nbcell; i++)
    {
        INTERP_KERNEL::HashMap<mcIdType, std::pair<int, mcIdType> >::const_iterator iter =
            _glob_to_loc.find(cell_list[i]);
 ...
 */
void
ParallelTopology::convertGlobalFaceList(const mcIdType *face_list, mcIdType nbface, mcIdType *local, int *ip)
{
    for (mcIdType i = 0; i < nbface; i++)
    {
        INTERP_KERNEL::HashMap<mcIdType, std::pair<int, mcIdType> >::const_iterator iter =
            _face_glob_to_loc.find(face_list[i]);
...
void
ParallelTopology::convertToLocal2ndVersion(mcIdType *nodes, mcIdType nbnodes, int idomain)
{
 ...
 */
mcIdType
ParallelTopology::getMaxGlobalFace() const
{
    mcIdType max = 0;
...
mcIdType
ParallelTopology::getNodeNumber() const
{
    if (_node_glob_to_loc.empty())
        return 0;
    std::set<mcIdType> keys;
    for (INTERP_KERNEL::HashMultiMap<mcIdType, std::pair<int, mcIdType> >::const_iterator iter =
             _node_glob_to_loc.begin();
 ...
 */
void
ParallelTopology::getNodeList(int idomain, mcIdType *list) const
{
    for (mcIdType i = 0; i < _nb_nodes[idomain]; i++) list[i] = _node_loc_to_glob[idomain][i];
}
 ...
 */
void
ParallelTopology::getCellList(int idomain, mcIdType *list) const
{
    for (mcIdType i = 0; i < _nb_cells[idomain]; i++) list[i] = _loc_to_glob[idomain][i];
}

mcIdType
ParallelTopology::getFaceNumber() const
{
    if (_face_glob_to_loc.empty())
        return 0;
    std::set<mcIdType> keys;
    for (INTERP_KERNEL::HashMultiMap<mcIdType, std::pair<int, mcIdType> >::const_iterator iter =
             _face_glob_to_loc.begin();
 ...
 */
void
ParallelTopology::getFaceList(int idomain, mcIdType *list) const
{
    for (mcIdType i = 0; i < _nb_faces[idomain]; i++) list[i] = _face_loc_to_glob[idomain][i];
}

mcIdType
ParallelTopology::convertGlobalFace(mcIdType iglobal, int idomain)
{
...
mcIdType
ParallelTopology::convertGlobalNode(mcIdType iglobal, int idomain)
{
    typedef INTERP_KERNEL::HashMultiMap<mcIdType, std::pair<int, mcIdType> >::const_iterator MMiter;
 ...
 */
void
ParallelTopology::appendFace(int idomain, mcIdType ilocal, mcIdType iglobal)
{
