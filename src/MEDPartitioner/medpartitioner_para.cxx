...
/*
  ...
  rm ttmp* tttmp*
  export verb=11
  mpirun -np 2 medpartitioner_para --input-file=blade.med --output-file=ttmp1_ --ndomains=2 --dump-cpu-memory
  --verbose=$verb mpirun -np 5 medpartitioner_para --input-file=blade.med --output-file=ttmp1_ --ndomains=2
  --dump-cpu-memory --verbose=$verb mpirun -np 4 medpartitioner_para --input-file=ttmp1_.xml --output-file=tttmp1_
  --ndomains=4 --verbose=$verb

  mpirun -np 2 medpartitioner_para --ndomains=2 --input-file=tmp_testMesh_20x30x50.med --output-file=ttmp2_
  --verbose=$verb mpirun -np 2 medpartitioner_para --ndomains=2 --input-file=ttmp2_.xml --output-file=ttmp3_
  --verbose=$verb

  mpirun -np 2 medpartitioner_para --ndomains=2 --input-file=tmp_testMeshWithFaces_20x30x50.med --output-file=ttmp2_
  --verbose=$verb mpirun -np 2 medpartitioner_para --ndomains=2 --input-file=ttmp2_.xml --output-file=ttmp3_
  --verbose=$verb

  mpirun -np 2 medpartitioner_para --ndomains=2 --input-file=tmp_testMesh_20x30x50_WithVecFieldOnCells.med
  --output-file=ttmp2_ --verbose=$verb mpirun -np 2 medpartitioner_para --ndomains=2
  --input-file=tmp_testMesh_20x30x50_WithVecFieldOnNodes.med --output-file=ttmp2_ --verbose=$verb mpirun -np 2
  medpartitioner_para --ndomains=2 --input-file=ttmp2_.xml --output-file=ttmp3_ --verbose=$verb

  mpirun -np 4 medpartitioner_para --ndomains=4 --input-file=tmp_testMeshHuge_20x30x50_6.xml --output-file=ttmp3_
  --verbose=1
*/
...
main(int argc, char **argv)
{
    ...
    string desc(
        "Available options of medpartitioner_para V1.0.3:\n"
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        if (TestArg(argv[i], "--verbose", value))
        {
            ...
            if (value != "")
                MyGlobals::_Verbose = atoi(value.c_str());
        }
        else if (TestArg(argv[i], "--help", value))
            help = 1;
        else if (TestArg(argv[i], "--test", value))
            test = 1;
        else if (TestArg(argv[i], "--input-file", value))
            input = value;
        else if (TestArg(argv[i], "--output-file", value))
            output = value;
        else if (TestArg(argv[i], "--split-method", value))
            library = value;
        else if (TestArg(argv[i], "--ndomains", value))
            ndomains = atoi(value.c_str());
        else if (TestArg(argv[i], "--randomize", value))
            MyGlobals::_Randomize = atoi(value.c_str());
        else if (TestArg(argv[i], "--atomize", value))
            MyGlobals::_Atomize = atoi(value.c_str());
        else if (TestArg(argv[i], "--create-boundary-faces", value))
            MyGlobals::_Create_Boundary_Faces = 1;
        else if (TestArg(argv[i], "--dump-cpu-memory", value))
            mesure_memory = true;
        else
        {
            if (MyGlobals::_Rank == 0)
    ...
    {
        if (MyGlobals::_Rank == 0)
            cerr << "randomize only available on 1 proc (mpirun -np 1)" << endl;
...
// user choice
    ...
    if (test == 1)  // only for debugger
    {
        if (MyGlobals::_Is0verbose > 0)
            cout << "tests on " << MyGlobals::_Atomize << " " << ndomains << endl;
    ...
    try
    {
        ...
        if (MyGlobals::_Is0verbose)
        {
            cout << "fileNames :" << endl << ReprVectorOfString(MyGlobals::_File_Names);
            cout << "fieldDescriptions :" << endl << ReprFieldDescriptions(collection.getFieldDescriptions(), " ");
        ...
        if (filter_face)
            new_collection.filterFaceOnCell();
        ...
        {
            for (std::size_t i = 0; i < r2.size(); i++) r2[i] = EraseTagSerialized(r2[i], "ioldDomain=");
        ...
        // may be yes? or not?
        for (std::size_t i = 0; i < r2.size(); i++) r2[i] = EraseTagSerialized(r2[i], "ioldFieldDouble=");
        r2 = DeleteDuplicatesInVectorOfString(r2);
        for (std::size_t i = 0; i < r2.size(); i++) finalInformations.push_back(r2[i]);
        ...
        if (MyGlobals::_Is0verbose)
            cout << "Writing " << ndomains << " output files " << output << "xx.med" << " and " << output << ".xml"
                 << endl;
        ...
        if (mesure_memory)
            if (parallelizer.isOnDifferentHosts() || MyGlobals::_Rank == 0)
            {
                cout << "Elapsed time = " << parallelizer.getPassedTime()
                     << ", max memory usage = " << parallelizer.evaluateMemory() << " KB" << endl;
            }
