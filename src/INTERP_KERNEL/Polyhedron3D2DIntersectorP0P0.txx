...
template <class MyMeshType, class MyMatrixType>
void
Polyhedron3D2DIntersectorP0P0<MyMeshType, MyMatrixType>::releaseArrays()
{
...
template <class MyMeshType, class MyMatrixType>
void
Polyhedron3D2DIntersectorP0P0<MyMeshType, MyMatrixType>::intersectCells(
    ConnType targetCell, const std::vector<ConnType> &srcCells, MyMatrixType &matrix
)
{
    ConnType nbOfNodesT = Intersector3D<MyMeshType, MyMatrixType>::_target_mesh.getNumberOfNodesOfElement(
        OTT<ConnType, numPol>::indFC(targetCell)
    );
    releaseArrays();
         ...
         iterCellS++)
    {
        ...
        NormalizedCellType normCellType =
            Intersector3D<MyMeshType, MyMatrixType>::_src_mesh.getTypeOfElement(cellSrcIdx);
        ...
        ConnType nbOfNodes4Type = cellModelCell.isDynamic() ? src_mesh.getNumberOfNodesOfElement(cellSrcIdx)
        ...
        for (int i = 0; i < (int)nbOfNodes4Type; ++i)
        {
            ...
            polyCoords[i] =
                const_cast<double *>(src_mesh.getCoordinatesPtr() + MyMeshType::MY_SPACEDIM * globalNodeNum);
        }

        for (typename std::vector<SplitterTetra<MyMeshType> *>::iterator iter = _tetra.begin(); iter != _tetra.end();
             ++iter)
            surface += (*iter)->intersectSourceFace(
                normCellType,
                nbOfNodes4Type,
                polyNodes,
                polyCoords,
