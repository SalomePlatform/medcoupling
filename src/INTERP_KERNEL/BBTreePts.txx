...
class BBTreePts
{
   ...
   public:
    /*!
      ...
      \param [in] epsilon precision to which points are decided to be coincident. Contrary to BBTree, the absolute
      epsilon is computed. So the internal epsilon is always positive.

      Parameters \a elems and \a level are used only by BBTreePts itself for creating trees recursively. A typical use
      is therefore :
      \code
      int nbelems=...
    ...
    double getElementsAroundPoint2(const double *xx, double threshold, ConnType &elem) const
    {
        ...
        if (_terminal)
        {
            ...
            for (ConnType i = 0; i < _nbelems; i++)
            {
                ...
                double tmp = 0.;
                for (int idim = 0; idim < dim; idim++) tmp += (bb_ptr[idim] - xx[idim]) * (bb_ptr[idim] - xx[idim]);
                if (tmp < threshold)
                {
                    if (tmp < ret)
                    {
