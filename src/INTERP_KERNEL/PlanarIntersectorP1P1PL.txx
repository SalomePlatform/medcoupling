...
PlanarIntersectorP1P1PL<MyMeshType, MyMatrix>::PlanarIntersectorP1P1PL(
...
template <class MyMeshType, class MyMatrix>
void
PlanarIntersectorP1P1PL<MyMeshType, MyMatrix>::intersectCells(
...
)
{
    ...
    for (typename std::vector<ConnType>::const_iterator iter = icellsS.begin(); iter != icellsS.end(); iter++)
    {
        ...
        const ConnType *startOfCellNodeConnT =
            this->_connectT + OTT<ConnType, numPol>::conn2C(this->_connIndexT[icellT]);
        for (int nodeIdT = 0; nodeIdT < nbOfNodesT; nodeIdT++)
        {
            ...
            if (PointLocatorAlgos<MyMeshType>::isElementContainsPointAlg2DSimple(
                    CoordsTTmp.data() + nodeIdT * SPACEDIM, CoordsS.data(), 3, this->_precision
                ))
            {
                double resLoc[3];
                barycentric_coords<SPACEDIM>(&CoordsS[0], &CoordsTTmp[nodeIdT * SPACEDIM], resLoc);
                const ConnType *startOfCellNodeConnS =
                    this->_connectS + OTT<ConnType, numPol>::conn2C(this->_connIndexS[*iter]);
...
PlanarIntersectorP1P1PL<MyMeshType, MyMatrix>::getNumberOfRowsOfResMatrix() const
{
...
template <class MyMeshType, class MyMatrix>
typename MyMeshType::MyConnType
PlanarIntersectorP1P1PL<MyMeshType, MyMatrix>::getNumberOfColsOfResMatrix() const
{
