...
INTERP_KERNEL::FEInterpolationOptions::FEInterpolationOptions()
    : _proj_dist_max(PROJ_DIST_MAX_DFT), _proj_on_surf(PROJ_DIST_ON_SURF_DFT), _use_dist_max(PROJ_USE_DIST_MAX_DFT)
{
}

void
INTERP_KERNEL::InterpolationOptions::init()
{
    _print_level = 0;
...
std::string
INTERP_KERNEL::InterpolationOptions::getIntersectionTypeRepr() const
{
...
bool
INTERP_KERNEL::InterpolationOptions::setOptionDouble(const std::string &key, double value)
{
...
bool
INTERP_KERNEL::InterpolationOptions::setOptionInt(const std::string &key, int value)
{
...
bool
INTERP_KERNEL::InterpolationOptions::setOptionString(const std::string &key, const std::string &value)
{
...
std::string
INTERP_KERNEL::InterpolationOptions::getSplittingPolicyRepr() const
{
...
std::string
INTERP_KERNEL::InterpolationOptions::filterInterpolationMethod(const std::string &meth) const
{
...
)
{
    ...
    return (
        setOptionString(INTERSEC_TYPE_STR, intersection_type) && setOptionString(SPLITTING_POLICY_STR, splitting_policy)
    );
}

std::string
INTERP_KERNEL::InterpolationOptions::printOptions() const
{
    std::ostringstream oss;
    oss.precision(15);
    oss << "Interpolation Options ******" << std::endl;
...
void
INTERP_KERNEL::InterpolationOptions::CheckAndSplitInterpolationMethod(
    const std::string &method, std::string &srcMeth, std::string &trgMeth
)
{
    const int NB_OF_METH_MANAGED = 4;
    const char *METH_MANAGED[NB_OF_METH_MANAGED] = {"P0P0", "P0P1", "P1P0", "P1P1"};
    bool found = false;
    for (int i = 0; i < NB_OF_METH_MANAGED && !found; i++) found = (method == METH_MANAGED[i]);
    if (!found)
    {
        std::string msg("The interpolation method : \'");
        msg += method;
        msg +=
            "\' not managed by INTERP_KERNEL interpolators ! Supported are \"P0P0\", \"P0P1\", \"P1P0\" and \"P1P1\".";
