...
LineSearches(
    ...
    std::vector<double> &x,
    T &func,
    const double ALF = 0.1,
    const mcIdType MAXIT = 10
)
{
    ...
    {
        for (mcIdType j = 0; j < n; j++) x[j] = xold[j] + rho * dx[j];
        const auto norm = func(x);

        double f = 0.0;
        for (mcIdType j = 0; j < n; j++) f += fvec[j] * dx[j];
        return f;
    };
   ...
   public:
    ...
    INTERP_KERNEL::DenseMatrix operator()(const std::vector<double> &x, const std::vector<double> &fvec)
    {
        ...
        for (mcIdType j = 0; j < n; j++)
        {
            ...
            xh[j] = temp;
            for (mcIdType i = 0; i < n; i++) df[i][j] = (f[i] - fvec[i]) / h;
        }
        return df;
    }
};
...
template <class T>
void
SolveWithNewtonWithJacobian(
    std::vector<double> &x,
    bool &check,
    T &vecfunc,
    std::function<void(const std::vector<double> &, const std::vector<double> &, INTERP_KERNEL::DenseMatrix &)>
        jacobian,
...
template <class T>
void
SolveWithNewton(std::vector<double> &x, bool &check, T &vecfunc)
{
    JacobianCalculator<T> fdjac(vecfunc);
    auto myJacobian =
        [&fdjac,
         vecfunc](const std::vector<double> &x, const std::vector<double> &fvec, INTERP_KERNEL::DenseMatrix &fjac)
    { fjac = fdjac(x, fvec); };
