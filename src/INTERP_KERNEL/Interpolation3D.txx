...
Interpolation3D::interpolateMeshes(
...
)
{
    ...
    if (methC == "P0P0")
    {
        switch (InterpolationOptions::getIntersectionType())
        {
            case Triangulation:
                intersector.reset(
                    new PolyhedronIntersectorP0P0<MyMeshType, MatrixType>(targetMesh, srcMesh, getSplittingPolicy())
                );
                break;
            case PointLocator:
                intersector.reset(
                    new PointLocator3DIntersectorP0P0<MyMeshType, MatrixType>(targetMesh, srcMesh, getPrecision())
                );
                break;
            default:
                throw INTERP_KERNEL::Exception(
                    "Invalid 3D intersection type for P0P0 interp specified : must be Triangle or PointLocator."
                );
        }
    }
    else if (methC == "P0P1")
    {
        switch (InterpolationOptions::getIntersectionType())
        {
            case Triangulation:
                intersector.reset(
                    new PolyhedronIntersectorP0P1<MyMeshType, MatrixType>(targetMesh, srcMesh, getSplittingPolicy())
                );
                break;
            case PointLocator:
                intersector.reset(
                    new PointLocator3DIntersectorP0P1<MyMeshType, MatrixType>(targetMesh, srcMesh, getPrecision())
                );
                break;
            default:
                throw INTERP_KERNEL::Exception(
                    "Invalid 3D intersection type for P0P1 interp specified : must be Triangle or PointLocator."
                );
        }
    }
    else if (methC == "P1P0")
    {
        switch (InterpolationOptions::getIntersectionType())
        {
            case Triangulation:
                intersector.reset(
                    new PolyhedronIntersectorP1P0<MyMeshType, MatrixType>(targetMesh, srcMesh, getSplittingPolicy())
                );
                break;
            case PointLocator:
                intersector.reset(
                    new PointLocator3DIntersectorP1P0<MyMeshType, MatrixType>(targetMesh, srcMesh, getPrecision())
                );
                break;
            case Barycentric:
                intersector.reset(
                    new PolyhedronIntersectorP1P0Bary<MyMeshType, MatrixType>(targetMesh, srcMesh, getSplittingPolicy())
                );
                break;
            default:
                throw INTERP_KERNEL::Exception(
                    "Invalid 3D intersection type for P1P0 interp specified : must be Triangle, PointLocator or "
                    "Barycentric."
                );
        }
    }
    else if (methC == "P1P1")
    {
        switch (InterpolationOptions::getIntersectionType())
        {
            case Triangulation:
                intersector.reset(
                    new PolyhedronIntersectorP1P1<MyMeshType, MatrixType>(targetMesh, srcMesh, getSplittingPolicy())
                );
                break;
            case PointLocator:
                intersector.reset(
                    new PointLocator3DIntersectorP1P1<MyMeshType, MatrixType>(targetMesh, srcMesh, getPrecision())
                );
                break;
            case Barycentric:
                intersector.reset(
                    new Barycentric3DIntersectorP1P1<MyMeshType, MatrixType>(targetMesh, srcMesh, getPrecision())
                );
                break;
            case MappedBarycentric:
                intersector.reset(
                    new MappedBarycentric3DIntersectorP1P1<MyMeshType, MatrixType>(targetMesh, srcMesh, getPrecision())
                );
                break;
            default:
                throw INTERP_KERNEL::Exception(
                    "Invalid 3D intersection type for P1P1 interp specified : must be Triangle, PointLocator, "
                    "Barycentric or MappedBarycentric."
                );
        }
    }
    else
    ...
    /*
     * Performs a depth-first search over srcMesh, using bounding boxes to recursively eliminate the elements of
     * targetMesh which cannot intersect smaller and smaller regions of srcMesh. At each level, each region is divided
     * in two, forming a binary search tree with leaves consisting of only one element of the source mesh together with
     * the elements of the target mesh that can intersect it. The recursion is implemented with a stack of RegionNodes,
     * each one containing a source region and a target region. Each region has an associated bounding box and a vector
     * of pointers to the elements that belong to it. Each MeshElement contains a bounding box and the global number of
     * the corresponding element in the mesh.
     */
    ...
    // Using a stack, descend recursively, creating at each step two new RegionNodes having as source region the left
    // and right part of the source region of the current node (created using MeshRegion::split()) and as target region
    // all the elements of the target mesh whose bounding box intersects the corresponding part Continue until the
    // source region contains only one element, at which point the intersection volumes are calculated with all the
    // remaining target mesh elements and stored in the matrix if they are non-zero.
    ...
    while (!nodes.empty())
    {
        ...
        if (currNode->getTargetRegion().getNumberOfElements() == 1)
        {
            ...
            for (typename std::vector<MeshElement<ConnType> *>::const_iterator iter =
                     currNode->getSrcRegion().getBeginElements();
                 iter != currNode->getSrcRegion().getEndElements();
                 ++iter)
        ...
        else  // recursion
        {
            ...
            currNode->getTargetRegion().split(
                leftNode->getTargetRegion(), rightNode->getTargetRegion(), axis, targetMesh
            );

            LOG(5,
                "After split, left target region has "
                    << leftNode->getTargetRegion().getNumberOfElements() << " elements and right target region has "
                    << rightNode->getTargetRegion().getNumberOfElements() << " elements");
            ...
            LOG(5,
                "Left src region has " << numLeftElements << " elements and right src region has " << numRightElements
                                       << " elements");
