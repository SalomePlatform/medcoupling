...
std::ostream &
operator<<(std::ostream &in, const Matrix<U, type> &m);
template <class U, NumberingPolicy type>
std::istream &
operator>>(std::istream &in, Matrix<U, type> &m);
...
class Matrix
{
    ...
    class Row : public std::vector<std::pair<int, T> >
    {
       public:
        ...
        Row(const Row &row)
        {
            ...
            for (int i = 0; i < this->size(); i++) (*this)[i] = row[i];
        }
        ...
        {
            return std::find_if(
                std::vector<std::pair<int, T> >::begin(), std::vector<std::pair<int, T> >::end(), KeyComparator(elem)
            );
        }

        void erase(int elem)
        {
            std::vector<std::pair<int, T> >::erase(
                std::find_if(
                    std::vector<std::pair<int, T> >::begin(),
                    std::vector<std::pair<int, T> >::end(),
                    KeyComparator(elem)
                )
            );
        }
   ...
   public:
    ...
    Matrix(int nbrows) : _coeffs(0), _cols(0), _is_configured(false) { _nb_rows = nbrows; }
    Matrix(std::vector<std::map<int, T> > &matrix) : _coeffs(0), _cols(0), _is_configured(false)
    {
    ...
    */
    void multiply(const T *const input, T *const output, int nb_comp)
    {
        ...
        for (int i = 0; i < _nb_rows; i++)
        {
            for (int comp = 0; comp < nb_comp; comp++) output[i * nb_comp + comp] = 0.;
            for (unsigned int j = _ncols_offset[i]; j < _ncols_offset[i + 1]; j++)
            {
                int icol = _cols[j];
    ...
    */
    void transposeMultiply(const T *const input, T *const output, int nb_cols)
    {
        ...
        for (int icol = 0; icol < nb_cols; icol++) output[icol] = 0.;
        for (int i = 0; i < _nb_rows; i++)
        {
            for (unsigned int j = _ncols_offset[i]; j < _ncols_offset[i + 1]; j++)
            {
    ...
    */
    void transposeMultiply(const T *const input, T *const output, int nb_cols, int nb_comp)
    {
        ...
        for (int icol = 0; icol < nb_cols; icol++)
            for (int comp = 0; comp < nb_comp; comp++) output[icol * nb_comp + comp] = 0.;
    ...
    */
    void rowSum(std::vector<T> &output)
    {
        ...
        {
            output[i] = 0;
            for (unsigned int j = _ncols_offset[i]; j < _ncols_offset[i + 1]; j++) output[i] += _coeffs[j];
        }
    }
     ...
     * 0 <= irow < n
     */
    Row &operator[](unsigned int irow) { return _auxiliary_matrix[irow]; }

    int getNbRows() { return _nb_rows; }
};
...
template <class T, NumberingPolicy type>
std::ostream &
operator<<(std::ostream &out, const Matrix<T, type> &m)
{
    if (m._is_configured)
    {
    ...
    else
    {
        ...
        for (unsigned int i = 0; i < m._nb_rows; i++)
        {
            ...
            for (unsigned int j = 0; j < m._auxiliary_matrix[i].size(); j++)
                out << "\t" << m._auxiliary_matrix[i][j].first << "\t" << m._auxiliary_matrix[i][j].second;
            out << "\n";
        }
    }
    return out;
...
template <class T, NumberingPolicy type>
std::istream &
operator>>(std::istream &in, Matrix<T, type> &m)
{
    int index_base_test;
    in >> index_base_test;
