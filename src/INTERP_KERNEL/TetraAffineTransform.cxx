...
TetraAffineTransform::TetraAffineTransform(const double *pts)
{
    ...
    {
        _translation[i] =
            -(_linear_transform[3 * i] * pts[0] + _linear_transform[3 * i + 1] * pts[1] +
              _linear_transform[3 * i + 2] * pts[2]);
    }
 ...
 */
void
TetraAffineTransform::apply(double *destPt, const double *srcPt) const
{
    double *dest = destPt;
    ...
    for (int i = 0; i < 3; ++i)
    {
        ...
        dest[i] = _linear_transform[3 * i] * srcPt[0] + _linear_transform[3 * i + 1] * srcPt[1] +
                  _linear_transform[3 * i + 2] * srcPt[2];
 ...
 */
void
TetraAffineTransform::reverseApply(double *destPt, const double *srcPt) const
{
    double *dest = destPt;
    ...
    {
        // matrix - vector multiplication
        dest[i] = _back_linear_transform[3 * i] * srcPt[0] + _back_linear_transform[3 * i + 1] * srcPt[1] +
                  _back_linear_transform[3 * i + 2] * srcPt[2];
 ...
 */
double
TetraAffineTransform::determinant() const
{
    return _determinant;
}
 ...
 */
void
TetraAffineTransform::dump() const
{
    std::cout << "A = " << std::endl << "[";
    for (int i = 0; i < 3; ++i)
    {
        std::cout << _linear_transform[3 * i] << ", " << _linear_transform[3 * i + 1] << ", "
                  << _linear_transform[3 * i + 2];
        if (i != 2)
            std::cout << std::endl;
    }
    std::cout << "]" << std::endl;

    std::cout << "b = " << "[" << _translation[0] << ", " << _translation[1] << ", " << _translation[2] << "]"
              << std::endl;
}
 ...
 */
void
TetraAffineTransform::invertLinearTransform()
{
 ...
 */
void
TetraAffineTransform::calculateDeterminant()
{
    ...
    _determinant =
        _linear_transform[0] * subDet[0] - _linear_transform[1] * subDet[1] + _linear_transform[2] * subDet[2];
}
 ...
 */
void
TetraAffineTransform::factorizeLU(double *lu, int *idx) const
{
 ...
 */
void
TetraAffineTransform::forwardSubstitution(double *x, const double *lu, const double *b, const int *idx) const
{
 ...
 */
void
TetraAffineTransform::backwardSubstitution(double *x, const double *lu, const double *b, const int *idx) const
{
