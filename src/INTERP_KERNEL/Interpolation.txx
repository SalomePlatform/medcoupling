...
Interpolation<TrueMainInterpolator>::fromToIntegralUniform(
...
)
{
    ...
    if (method == "P0")
    {
        IntegralUniformIntersectorP0<MyMeshType, MatrixType> intersector(
            mesh, InterpolationOptions::getMeasureAbsStatus()
        );
    ...
    {
        IntegralUniformIntersectorP1<MyMeshType, MatrixType> intersector(
            mesh, InterpolationOptions::getMeasureAbsStatus()
        );
        intersector.setFromTo(fromTo);
        std::vector<ConnType> tmp;
        ...
        throw INTERP_KERNEL::Exception(
            "Invalid method specified in fromIntegralUniform : must be in { \"P0\", \"P1\"}"
        );
    return ret;
}
...
double
Interpolation<TrueMainInterpolator>::CalculateCharacteristicSizeOfMeshes(
