...
inline bool
samePoint(const double *p1, const double *p2)
{
...
void
UnitTetraIntersectionBary::init(bool isTetraInversed)
{
    _int_volume = 0;
...
void
UnitTetraIntersectionBary::addSide(const TransformedTriangle &triangle)
{
    ...
    double *p1 = *p++;
    double *p2 = *p;
    while (samePoint(p1, p2) && ++p != pEnd) p2 = *p;
    if (p == pEnd)
    {
#ifdef DMP_UNITTETRAINTERSECTIONBARY
    ...
    double *p3 = *p;
    while ((samePoint(p2, p3) || samePoint(p1, p3)) && ++p != pEnd) p3 = *p;
    if (p == pEnd)
    {
#ifdef DMP_UNITTETRAINTERSECTIONBARY
    ...
    std::cout << "NORM: ( " << _polyNormals.back()[0] << ", " << _polyNormals.back()[1] << ", "
              << _polyNormals.back()[2] << " )" << std::endl;
#endif
    clearPolygons();
}
...
bool
UnitTetraIntersectionBary::getBary(double *baryCenter)
{
    ...
    for (++f; f != fEnd; ++f)
    {
        ...
        for (; !pBelongsToPoly && v != vEnd; ++v) pBelongsToPoly = samePoint(PP, *v);
        if (pBelongsToPoly)
            continue;
...
int
UnitTetraIntersectionBary::addSideFaces()
{
    ...
    for (; f != fEnd; ++f)
    {
        ...
        if (!sideAdded[3] && (epsilonEqual((coordSum[0] + coordSum[1] + coordSum[2]) / (int)polygon.size(), 1.)))
    ...
    for (std::size_t iF = 0; iF < nbIntersectPolygs; ++f, ++iF)  // loop on added intersection polygons
    {
        ...
        for (std::size_t i = 0; i < polygon.size(); ++i)
        {
            ...
            if (sideFaces[3] && epsilonEqual(p1[_XX] + p1[_YY] + p1[_ZZ], 1.0) &&
    ...
    if (nbCutOffCorners == 0)
        ...
        for (int ic = 0; ic < NB_TETRA_NODES; ++ic) std::cout << cutOffCorners[ic];
    std::cout << std::endl;
#endif
    // ------------------------------------------------------------------------
...
void
UnitTetraIntersectionBary::setTriangleOnSide(int iSide)
{
    if (iSide >= 3)
        iSide = 0;
    for (int i = 0; i < 3; ++i)
...
void
UnitTetraIntersectionBary::clearPolygons(bool andFaces)
{
...
UnitTetraIntersectionBary::~UnitTetraIntersectionBary() { clearPolygons(/*andFaces=*/true); }
