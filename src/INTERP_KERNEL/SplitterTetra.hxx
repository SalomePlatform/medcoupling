...
static const int SPLIT_NODES_5_WO[20] = /* WO for well oriented !!! normals of 3 first points are OUTSIDE the TETRA4 */
    {0, 5, 1, 2, 0, 4, 5, 7, 0, 3, 7, 2, 5, 7, 6, 2, 0, 5, 2, 7};
...
static const int SPLIT_NODES_6_WO[24] = /* WO for well oriented !!! normals of 3 first points are OUTSIDE the TETRA4 */
    {0, 5, 1, 6, 0, 1, 2, 6, 0, 4, 5, 6, 0, 7, 4, 6, 0, 2, 3, 6, 0, 3, 7, 6};
...
static const int GENERAL_48_SUBZONES[64] = {0,  8,  21, 12, 9,  20, 26, 22, 8,  1,  13, 21, 20, 10, 23, 26,
                                            12, 21, 16, 3,  22, 26, 25, 17, 21, 13, 2,  16, 26, 23, 18, 25,
                                            9,  20, 26, 22, 4,  11, 24, 14, 20, 10, 23, 26, 11, 5,  15, 24,
                                            22, 26, 25, 17, 14, 24, 19, 7,  26, 23, 18, 25, 24, 15, 6,  19};

static const mcIdType GENERAL_48_SUBZONES_2[64] = {0,  -1,  -14, -5,  -2,  -13, -19, -15, -1,  1,   -6,  -14, -13,
                                                   -3, -16, -19, -5,  -14, -9,  3,   -15, -19, -18, -10, -14, -6,
                                                   2,  -9,  -19, -16, -11, -18, -2,  -13, -19, -15, 4,   -4,  -17,
                                                   -7, -13, -3,  -16, -19, -4,  5,   -8,  -17, -15, -19, -18, -10,
                                                   -7, -17, -12, 7,   -19, -16, -11, -18, -17, -8,  6,   -12};
...
class TriangleFaceKey
{
   public:
     ...
     */
    mcIdType hashVal() const { return _hashVal; }
 ...
 */
inline void
TriangleFaceKey::Sort3Ints(mcIdType *sorted, mcIdType x1, mcIdType x2, mcIdType x3)
{
    if (x1 < x2)
    {
        if (x1 < x3)
...
class hash<INTERP_KERNEL::TriangleFaceKey>
{
   public:
     ...
     */
    mcIdType operator()(const INTERP_KERNEL::TriangleFaceKey &key) const { return key.hashVal(); }
};
}  // namespace INTERP_KERNEL
...
class SplitterTetra
{
   ...
   private:
    ...
    inline static void CheckIsStrictlyOutside(
        const double *pt, bool *isStrictlyOutside, const double errTol = DEFAULT_ABS_TOL
    );
    ...
    static inline bool IsFacesCoplanar(
        const double *const planeNormal,
        const double planeConstant,
        const double *const *const coordsFace,
        const double precision
    );
    static inline double CalculateIntersectionSurfaceOfCoplanarTriangles(
        const double *const planeNormal,
        const double planeConstant,
    ...
    /// Since the scale is always the same in the transformed space (the target tetrahedron is unitary), this number is
    /// independent of the scale of the meshes.
 ...
 * the corresponding array element will be true if and only if it was true when the method was called and pt lies in the
 * halfspace.
 *
...
template <class MyMeshType>
inline void
SplitterTetra<MyMeshType>::CheckIsOutside(const double *pt, bool *isOutside, const double errTol)
{
...
template <class MyMeshType>
inline void
SplitterTetra<MyMeshType>::CheckIsStrictlyOutside(const double *pt, bool *isStrictlyOutside, const double errTol)
{
...
template <class MyMeshType>
inline void
SplitterTetra<MyMeshType>::calculateNode(typename MyMeshType::MyConnType globalNodeNum)
{
 ...
 * The only difference with the previous method calculateNode is that the coordinates of the node are passed in
 * arguments and are not recalculated in order to optimize the method.
 *
 * @param globalNodeNum  global node number of the node in the mesh _src_mesh
 *
 */
template <class MyMeshType>
inline void
SplitterTetra<MyMeshType>::calculateNode2(typename MyMeshType::MyConnType globalNodeNum, const double *node)
{
    double *transformedNode = new double[MyMeshType::MY_SPACEDIM];
    assert(transformedNode != 0);
    _t->apply(transformedNode, node);
...
template <class MyMeshType>
inline void
SplitterTetra<MyMeshType>::calculateVolume(TransformedTriangle &tri, const TriangleFaceKey &key)
{
...
template <class MyMeshType>
inline void
SplitterTetra<MyMeshType>::calculateSurface(TransformedTriangle &tri, const TriangleFaceKey &key)
{
...
class SplitterTetra2
{
   public:
    ...
    void splitConvex(
        typename MyMeshTypeT::MyConnType targetCell,  // to suppress
        typename std::vector<SplitterTetra<MyMeshTypeS> *> &tetra
    );                                                                                                   // to suppress
    ...
    inline const double *getCoordsOfSubNode2(
        typename MyMeshTypeT::MyConnType node, typename MyMeshTypeT::MyConnType &nodeId
    );  // to suppress
    // template<int n>
   ...
   private:
    ...
    void calculateGeneral24TetraGen(
        typename std::vector<SplitterTetra<MyMeshTypeS> *> &tetra,
...
// template<int n>
inline void
SplitterTetra2<MyMeshTypeT, MyMeshTypeS>::calcBarycenter(
    typename MyMeshTypeT::MyConnType n, double *barycenter, const int *pts
)
{
...
template <class MyMeshTypeT, class MyMeshTypeS>
inline const double *
SplitterTetra2<MyMeshTypeT, MyMeshTypeS>::getCoordsOfSubNode(typename MyMeshTypeT::MyConnType node)
{
...
template <class MyMeshTypeT, class MyMeshTypeS>
const double *
SplitterTetra2<MyMeshTypeT, MyMeshTypeS>::getCoordsOfSubNode2(
    typename MyMeshTypeT::MyConnType node, typename MyMeshTypeT::MyConnType &nodeId
)
{
