...
{
inline void
calculateBarycenterDyn(const double **pts, mcIdType nbPts, int dim, double *bary);

inline double
calculateAreaForPolyg(const double **coords, mcIdType nbOfPtsInPolygs, int spaceDim);

inline double
calculateAreaForQPolyg(const double **coords, mcIdType nbOfPtsInPolygs, int spaceDim);

inline double
calculateLgthForSeg2(const double *p1, const double *p2, int spaceDim)
{
    if (spaceDim == 1)
        return *p2 - *p1;
    else
    {
        double ret = 0;
        for (int i = 0; i < spaceDim; i++) ret += (p2[i] - p1[i]) * (p2[i] - p1[i]);
        return sqrt(ret);
    }
}

inline double
calculateLgthForSeg3(const double *begin, const double *end, const double *middle, int spaceDim)
{
    if (spaceDim == 2)
    {
        Edge *ed = Edge::BuildEdgeFrom3Points(begin, middle, end);
        double ret = ed->getCurveLength();
        ed->decrRef();
        return ret;
    }
    else
...
// ===========================
inline double
calculateAreaForTria(const double *p1, const double *p2, const double *p3, int spaceDim)
{
    double area;
...
// =============================
inline double
calculateAreaForQuad(const double *p1, const double *p2, const double *p3, const double *p4, int spaceDim)
{
    double area;
    ...
    if (spaceDim == 2)
    {
        ...
        area = -4.0 * (b1 * c2 - c1 * b2 + a1 * c2 - c1 * a2 + b1 * d2 - d1 * b2 + a1 * d2 - d1 * a2);
    }
    else
    {
        area = (sqrt(
...
// ====================================
inline void
calculateNormalForTria(const double *p1, const double *p2, const double *p3, double *normal)
{
...
// ======================================
inline void
calculateNormalForQuad(const double *p1, const double *p2, const double *p3, const double *p4, double *normal)
{
...
// ===================================
inline void
calculateNormalForPolyg(const double **coords, mcIdType nbOfPtsInPolygs, double *normal)
{
...
calculateAreaForQPolyg(const double **coords, mcIdType nbOfPtsInPolygs, int spaceDim)
{
    if (nbOfPtsInPolygs % 2 == 0)
    {
        if (spaceDim == 2)
        {
            ...
            for (mcIdType i = 0; i < nbOfPtsInPolygs; i++) nodes[i] = new Node(coords[i][0], coords[i][1]);
    ...
    else
    {
        ...
        oss << "INTERP_KERNEL::calculateAreaForQPolyg : nb of points in quadratic polygon is " << nbOfPtsInPolygs
            << " should be even !";
...
// ==========================
inline double
calculateVolumeForTetra(const double *p1, const double *p2, const double *p3, const double *p4)
{
    return ((p3[0] - p1[0]) * ((p2[1] - p1[1]) * (p4[2] - p1[2]) - (p2[2] - p1[2]) * (p4[1] - p1[1])) -
            (p2[0] - p1[0]) * ((p3[1] - p1[1]) * (p4[2] - p1[2]) - (p3[2] - p1[2]) * (p4[1] - p1[1])) +
            (p4[0] - p1[0]) * ((p3[1] - p1[1]) * (p2[2] - p1[2]) - (p3[2] - p1[2]) * (p2[1] - p1[1]))) /
           6.0;
}
...
// =========================
inline double
calculateVolumeForPyra(const double *p1, const double *p2, const double *p3, const double *p4, const double *p5)
{
...
)
{
    ...
    double C = (a1 * c2 * h3 + b1 * c2 * f3) - (a1 * c3 * h2 + b1 * c3 * f2) - (a2 * c1 * h3 + b2 * c1 * f3) +
               (a2 * c3 * h1 + b2 * c3 * f1) + (a3 * c1 * h2 + b3 * c1 * f2) - (a3 * c2 * h1 + b3 * c2 * f1);
    ...
    double F = (a1 * d2 * h3 + b1 * d2 * f3) - (a1 * d3 * h2 + b1 * d3 * f2) - (a2 * d1 * h3 + b2 * d1 * f3) +
               (a2 * d3 * h1 + b2 * d3 * f1) + (a3 * d1 * h2 + b3 * d1 * f2) - (a3 * d2 * h1 + b3 * d2 * f1);
    ...
    double P = (a1 * e2 * h3 + b1 * e2 * f3) - (a1 * e3 * h2 + b1 * e3 * f2) - (a2 * e1 * h3 + b2 * e1 * f3) +
               (a2 * e3 * h1 + b2 * e3 * f1) + (a3 * e1 * h2 + b3 * e1 * f2) - (a3 * e2 * h1 + b3 * e2 * f1);
...
)
{
    ...
    double C = (a1 * h2 + c1 * e2) * q3 - (a1 * h3 + c1 * e3) * q2 - (a2 * h1 + c2 * e1) * q3 +
               (a2 * h3 + c2 * e3) * q1 + (a3 * h1 + c3 * e1) * q2 - (a3 * h2 + c3 * e2) * q1;
    ...
    double F = (b1 * h2 + d1 * e2) * s3 - (b1 * h3 + d1 * e3) * s2 - (b2 * h1 + d2 * e1) * s3 +
               (b2 * h3 + d2 * e3) * s1 + (b3 * h1 + d3 * e1) * s2 - (b3 * h2 + d3 * e2) * s1;
    double G = (a1 * e2 * s3 + b1 * e2 * q3) - (a1 * e3 * s2 + b1 * e3 * q2) - (a2 * e1 * s3 + b2 * e1 * q3) +
               (a2 * e3 * s1 + b2 * e3 * q1) + (a3 * e1 * s2 + b3 * e1 * q2) - (a3 * e2 * s1 + b3 * e2 * q1);
    double H = (c1 * h2 * s3 + d1 * h2 * q3) - (c1 * h3 * s2 + d1 * h3 * q2) - (c2 * h1 * s3 + d2 * h1 * q3) +
               (c2 * h3 * s1 + d2 * h3 * q1) + (c3 * h1 * s2 + d3 * h1 * q2) - (c3 * h2 * s1 + d3 * h2 * q1);
    double I =
        ((a1 * h2 + c1 * e2) * s3 + (b1 * h2 + d1 * e2) * q3) - ((a1 * h3 + c1 * e3) * s2 + (b1 * h3 + d1 * e3) * q2) -
        ((a2 * h1 + c2 * e1) * s3 + (b2 * h1 + d2 * e1) * q3) + ((a2 * h3 + c2 * e3) * s1 + (b2 * h3 + d2 * e3) * q1) +
        ((a3 * h1 + c3 * e1) * s2 + (b3 * h1 + d3 * e1) * q2) - ((a3 * h2 + c3 * e2) * s1 + (b3 * h2 + d3 * e2) * q1);
    ...
    double L = (a1 * p2 + c1 * f2) * r3 - (a1 * p3 + c1 * f3) * r2 - (a2 * p1 + c2 * f1) * r3 +
               (a2 * p3 + c2 * f3) * r1 + (a3 * p1 + c3 * f1) * r2 - (a3 * p2 + c3 * f2) * r1;
    ...
    double O = (b1 * p2 + d1 * f2) * t3 - (b1 * p3 + d1 * f3) * t2 - (b2 * p1 + d2 * f1) * t3 +
               (b2 * p3 + d2 * f3) * t1 + (b3 * p1 + d3 * f1) * t2 - (b3 * p2 + d3 * f2) * t1;
    double P = (a1 * f2 * t3 + b1 * f2 * r3) - (a1 * f3 * t2 + b1 * f3 * r2) - (a2 * f1 * t3 + b2 * f1 * r3) +
               (a2 * f3 * t1 + b2 * f3 * r1) + (a3 * f1 * t2 + b3 * f1 * r2) - (a3 * f2 * t1 + b3 * f2 * r1);
    double Q = (c1 * p2 * t3 + d1 * p2 * r3) - (c1 * p3 * t2 + d1 * p3 * r2) - (c2 * p1 * t3 + d2 * p1 * r3) +
               (c2 * p3 * t1 + d2 * p3 * r1) + (c3 * p1 * t2 + d3 * p1 * r2) - (c3 * p2 * t1 + d3 * p2 * r1);
    double R =
        ((a1 * p2 + c1 * f2) * t3 + (b1 * p2 + d1 * f2) * r3) - ((a1 * p3 + c1 * f3) * t2 + (b1 * p3 + d1 * f3) * r2) -
        ((a2 * p1 + c2 * f1) * t3 + (b2 * p1 + d2 * f1) * r3) + ((a2 * p3 + c2 * f3) * t1 + (b2 * p3 + d2 * f3) * r1) +
        ((a3 * p1 + c3 * f1) * t2 + (b3 * p1 + d3 * f1) * r2) - ((a3 * p2 + c3 * f2) * t1 + (b3 * p2 + d3 * f2) * r1);
    double S = (a1 * e2 * r3 + a1 * f2 * q3) - (a1 * e3 * r2 + a1 * f3 * q2) - (a2 * e1 * r3 + a2 * f1 * q3) +
               (a2 * e3 * r1 + a2 * f3 * q1) + (a3 * e1 * r2 + a3 * f1 * q2) - (a3 * e2 * r1 + a3 * f2 * q1);
    double T = (c1 * h2 * r3 + c1 * p2 * q3) - (c1 * h3 * r2 + c1 * p3 * q2) - (c2 * h1 * r3 + c2 * p1 * q3) +
               (c2 * h3 * r1 + c2 * p3 * q1) + (c3 * h1 * r2 + c3 * p1 * q2) - (c3 * h2 * r1 + c3 * p2 * q1);
    double U =
        ((a1 * h2 + c1 * e2) * r3 + (a1 * p2 + c1 * f2) * q3) - ((a1 * h3 + c1 * e3) * r2 + (a1 * p3 + c1 * f3) * q2) -
        ((a2 * h1 + c2 * e1) * r3 + (a2 * p1 + c2 * f1) * q3) + ((a2 * h3 + c2 * e3) * r1 + (a2 * p3 + c2 * f3) * q1) +
        ((a3 * h1 + c3 * e1) * r2 + (a3 * p1 + c3 * f1) * q2) - ((a3 * h2 + c3 * e2) * r1 + (a3 * p2 + c3 * f2) * q1);
    double V = (b1 * e2 * t3 + b1 * f2 * s3) - (b1 * e3 * t2 + b1 * f3 * s2) - (b2 * e1 * t3 + b2 * f1 * s3) +
               (b2 * e3 * t1 + b2 * f3 * s1) + (b3 * e1 * t2 + b3 * f1 * s2) - (b3 * e2 * t1 + b3 * f2 * s1);
    double W = (d1 * h2 * t3 + d1 * p2 * s3) - (d1 * h3 * t2 + d1 * p3 * s2) - (d2 * h1 * t3 + d2 * p1 * s3) +
               (d2 * h3 * t1 + d2 * p3 * s1) + (d3 * h1 * t2 + d3 * p1 * s2) - (d3 * h2 * t1 + d3 * p2 * s1);
    double X =
        ((b1 * h2 + d1 * e2) * t3 + (b1 * p2 + d1 * f2) * s3) - ((b1 * h3 + d1 * e3) * t2 + (b1 * p3 + d1 * f3) * s2) -
        ((b2 * h1 + d2 * e1) * t3 + (b2 * p1 + d2 * f1) * s3) + ((b2 * h3 + d2 * e3) * t1 + (b2 * p3 + d2 * f3) * s1) +
        ((b3 * h1 + d3 * e1) * t2 + (b3 * p1 + d3 * f1) * s2) - ((b3 * h2 + d3 * e2) * t1 + (b3 * p2 + d3 * f2) * s1);
    ...
    double AA =
        ((a1 * h2 + c1 * e2) * t3 + (a1 * p2 + c1 * f2) * s3 + (b1 * h2 + d1 * e2) * r3 + (b1 * p2 + d1 * f2) * q3) -
        ((a1 * h3 + c1 * e3) * t2 + (a1 * p3 + c1 * f3) * s2 + (b1 * h3 + d1 * e3) * r2 + (b1 * p3 + d1 * f3) * q2) -
        ((a2 * h1 + c2 * e1) * t3 + (a2 * p1 + c2 * f1) * s3 + (b2 * h1 + d2 * e1) * r3 + (b2 * p1 + d2 * f1) * q3) +
        ((a2 * h3 + c2 * e3) * t1 + (a2 * p3 + c2 * f3) * s1 + (b2 * h3 + d2 * e3) * r1 + (b2 * p3 + d2 * f3) * q1) +
        ((a3 * h1 + c3 * e1) * t2 + (a3 * p1 + c3 * f1) * s2 + (b3 * h1 + d3 * e1) * r2 + (b3 * p1 + d3 * f1) * q2) -
        ((a3 * h2 + c3 * e2) * t1 + (a3 * p2 + c3 * f2) * s1 + (b3 * h2 + d3 * e2) * r1 + (b3 * p2 + d3 * f2) * q1);

    return 64.0 *
           (8.0 * (A + B + D + E + J + K + M + N) + 4.0 * (C + F + G + H + L + O + P + Q + S + T + V + W) +
            2.0 * (I + R + U + X + Y + Z) + AA) /
           27.0;
}

// =========================================================================================================================
// Calculate Volume for Generic Polyedron, even not convex one, WARNING !!! The polyedron's faces must be correctly
// ordered
// =========================================================================================================================
inline double
calculateVolumeForPolyh(const double ***pts, const int *nbOfNodesPerFaces, int nbOfFaces, const double *bary)
{
    double volume = 0.;
...
/*!
 * Calculate Volume for Generic Polyedron, even not convex one, WARNING !!! The polyedron's faces must be correctly
 * ordered. 2nd API avoiding to create double** arrays. The returned value could be negative if polyhedrons faces are
 * not oriented with normal outside of the polyhedron
 */
template <class ConnType, NumberingPolicy numPol>
inline double
calculateVolumeForPolyh2(const ConnType *connec, mcIdType lgth, const double *coords)
{
...
template <class ConnType, NumberingPolicy numPol>
inline void
areaVectorOfPolygon(const ConnType *connec, mcIdType lgth, const double *coords, double *res)
{
    res[0] = 0.;
    res[1] = 0.;
    res[2] = 0.;
...
template <class ConnType, NumberingPolicy numPol>
inline void
computePolygonBarycenter3D(const ConnType *connec, mcIdType lgth, const double *coords, double *res)
{
    double area[3];
    ...
    if (norm > std::numeric_limits<double>::min())
    {
        ...
        for (mcIdType i = 1; i < lgth - 1; i++)
        {
            ...
            v[1] = (coords[3 * OTT<ConnType, numPol>::coo2C(connec[0]) + 1] +
                    coords[3 * OTT<ConnType, numPol>::coo2C(connec[i]) + 1] +
                    coords[3 * OTT<ConnType, numPol>::coo2C(connec[i + 1]) + 1]) /
                   3.;
            v[2] = (coords[3 * OTT<ConnType, numPol>::coo2C(connec[0]) + 2] +
                    coords[3 * OTT<ConnType, numPol>::coo2C(connec[i]) + 2] +
                    coords[3 * OTT<ConnType, numPol>::coo2C(connec[i + 1]) + 2]) /
                   3.;
            ...
            if (norm2 > 1e-12)
            {
                ...
                res[0] += signOfArea * norm2 * v[0] / norm;
                res[1] += signOfArea * norm2 * v[1] / norm;
                res[2] += signOfArea * norm2 * v[2] / norm;
            }
        }
    }
    else
    {
        ...
        {
            v[0] = coords[3 * OTT<ConnType, numPol>::coo2C(connec[(i + 1) % lgth])] -
                   coords[3 * OTT<ConnType, numPol>::coo2C(connec[i])];
            v[1] = coords[3 * OTT<ConnType, numPol>::coo2C(connec[(i + 1) % lgth]) + 1] -
                   coords[3 * OTT<ConnType, numPol>::coo2C(connec[i]) + 1];
            v[2] = coords[3 * OTT<ConnType, numPol>::coo2C(connec[(i + 1) % lgth]) + 2] -
                   coords[3 * OTT<ConnType, numPol>::coo2C(connec[i]) + 2];
            double norm2 = sqrt(v[0] * v[0] + v[1] * v[1] + v[2] * v[2]);
            res[0] += (coords[3 * OTT<ConnType, numPol>::coo2C(connec[(i + 1) % lgth])] +
                       coords[3 * OTT<ConnType, numPol>::coo2C(connec[i])]) /
                      2. * norm2;
            res[1] += (coords[3 * OTT<ConnType, numPol>::coo2C(connec[(i + 1) % lgth]) + 1] +
                       coords[3 * OTT<ConnType, numPol>::coo2C(connec[i]) + 1]) /
                      2. * norm2;
            res[2] += (coords[3 * OTT<ConnType, numPol>::coo2C(connec[(i + 1) % lgth]) + 2] +
                       coords[3 * OTT<ConnType, numPol>::coo2C(connec[i]) + 2]) /
                      2. * norm2;
            norm += norm2;
        }
        ...
        else
        {
            ...
            res[0] /= FromIdType<double>(lgth);
            res[1] /= FromIdType<double>(lgth);
            res[2] /= FromIdType<double>(lgth);
            return;
        }
    }
}
...
inline double
integrationOverA3DLine(double u1, double v1, double u2, double v2, double A, double B, double C)
{
    return (u1 - u2) *
           (6. * C * C * (v1 + v2) + B * B * (v1 * v1 * v1 + v1 * v1 * v2 + v1 * v2 * v2 + v2 * v2 * v2) +
            A * A * (2. * u1 * u2 * (v1 + v2) + u1 * u1 * (3. * v1 + v2) + u2 * u2 * (v1 + 3. * v2)) +
            4. * C * (A * (2 * u1 * v1 + u2 * v1 + u1 * v2 + 2. * u2 * v2) + B * (v1 * v1 + v1 * v2 + v2 * v2)) +
            A * B * (u1 * (3. * v1 * v1 + 2. * v1 * v2 + v2 * v2) + u2 * (v1 * v1 + 2. * v1 * v2 + 3. * v2 * v2))) /
           24.;
}

template <class ConnType, NumberingPolicy numPol>
inline void
barycenterOfPolyhedron(const ConnType *connec, mcIdType lgth, const double *coords, double *res)
{
    std::size_t nbOfFaces = std::count(connec, connec + lgth, -1) + 1;
    res[0] = 0.;
    res[1] = 0.;
    ...
    for (std::size_t i = 0; i < nbOfFaces; i++)
    {
        ...
        if (normOfNormal < std::numeric_limits<double>::min())
            continue;
        normal[0] /= normOfNormal;
        normal[1] /= normOfNormal;
        normal[2] /= normOfNormal;
    ...
    else
    {
        ...
        for (std::size_t i = 0; i < nbOfFaces; i++)
        {
            ...
            res[0] += normOfNormal * tmpBary[0];
            res[1] += normOfNormal * tmpBary[1];
            res[2] += normOfNormal * tmpBary[2];
            work = work2 + 1;
        }
        res[0] /= sum;
        res[1] /= sum;
...
// ============================================================================================================================================
// Calculate Volume for NON Generic Polyedron. Only polydrons with bary included in pts is supported by this method.
// Result is always positive.
// ============================================================================================================================================
inline double
calculateVolumeForPolyhAbs(const double ***pts, const int *nbOfNodesPerFaces, int nbOfFaces, const double *bary)
{
    double volume = 0.;
...
template <int N>
inline double
addComponentsOfVec(const double **pts, int rk)
{
...
template <>
inline double
addComponentsOfVec<1>(const double **pts, int rk)
{
    return pts[0][rk];
}
...
inline void
calculateBarycenter(const double **pts, double *bary)
{
...
template <>
inline void
calculateBarycenter<2, 0>(const double ** /*pts*/, double * /*bary*/)
{
}

template <>
inline void
calculateBarycenter<3, 0>(const double ** /*pts*/, double * /*bary*/)
{
}

template <>
inline void
calculateBarycenter<4, 0>(const double ** /*pts*/, double * /*bary*/)
{
}

template <>
inline void
calculateBarycenter<5, 0>(const double ** /*pts*/, double * /*bary*/)
{
}

template <>
inline void
calculateBarycenter<6, 0>(const double ** /*pts*/, double * /*bary*/)
{
}

template <>
inline void
calculateBarycenter<7, 0>(const double ** /*pts*/, double * /*bary*/)
{
}

template <>
inline void
calculateBarycenter<8, 0>(const double ** /*pts*/, double * /*bary*/)
{
}
...
inline void
calculateBarycenterDyn2(const double *pts, mcIdType nbPts, double *bary)
{
...
inline void
computePolygonBarycenter2DEngine(double **coords, mcIdType lgth, double *res)
{
    double area = 0.;
    res[0] = 0.;
    res[1] = 0.;
...
template <class ConnType, NumberingPolicy numPol>
inline void
computePolygonBarycenter2D(const ConnType *connec, mcIdType lgth, const double *coords, double *res)
{
...
inline void
computeQPolygonBarycenter2D(double **coords, mcIdType nbOfPtsInPolygs, int spaceDim, double *res)
{
    if (nbOfPtsInPolygs % 2 == 0)
    {
        if (spaceDim == 2)
    ...
    {
        std::ostringstream oss;
        oss << "INTERP_KERNEL::computeQPolygonBarycenter2D : nb of points in quadratic polygon is " << nbOfPtsInPolygs
            << " should be even !";
        throw INTERP_KERNEL::Exception(oss.str().c_str());
    }
}
