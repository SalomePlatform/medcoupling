...
#define _CU_TEMPLATE template <class MyCMeshType, class MyUMeshType, class MyMatrix, class ConcreteIntersector>
#define _INTERSECTOR_CU_ IntersectorCU<MyCMeshType, MyUMeshType, MyMatrix, ConcreteIntersector>
...
_CU_TEMPLATE
_INTERSECTOR_CU_::IntersectorCU(const MyCMeshType &meshS, const MyUMeshType &meshT) : _meshU(meshT), _meshC(meshS)
{
...
_CU_TEMPLATE
void
_INTERSECTOR_CU_::getUElemBB(double *bb, UConnType icell)
{
    ...
    {
        // const double*
        // coord_node=_coordsU+SPACEDIM*(OTT<ConnType,numPol>::coo2C(conn[OTT<ConnType,numPol>::conn2C(conn_index[OTT<ConnType,numPol>::ind2C(iP)]+i)]));
...
_CU_TEMPLATE
void
_INTERSECTOR_CU_::getUCoordinates(UConnType icell, std::vector<double> &coords)
{
    UConnType nb_nodes = _connIndexU[icell + 1] - _connIndexU[icell];
    ...
    for (UConnType i = 0; i < nb_nodes; i++)
        for (int j = 0; j < SPACEDIM; j++)
            coords[SPACEDIM * i + j] =
                _coordsU[SPACEDIM * _FMCOO(_connectU[_FMCON(_connIndexU[_FMIU(icell)] + i)]) + j];
}

_CU_TEMPLATE
typename MyCMeshType::MyConnType
_INTERSECTOR_CU_::getNumberOfRowsOfResMatrix() const
{
    return _meshU.getNumberOfElements();
}
_CU_TEMPLATE
typename MyCMeshType::MyConnType
_INTERSECTOR_CU_::getNumberOfColsOfResMatrix() const
{
...
_CU_TEMPLATE
void
_INTERSECTOR_CU_::intersectCells(CConnType icellU, const std::vector<CConnType> &icellC, MyMatrix &res)
{
