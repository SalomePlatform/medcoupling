...
void
QRDecomp::solve(const std::vector<double> &b, std::vector<double> &x)
{
    qtmult(b, x);
    rsolve(x, x);
}
...
void
QRDecomp::qtmult(const std::vector<double> &b, std::vector<double> &x)
{
    mcIdType i, j;
    double sum;
    for (i = 0; i < n; i++)
...
void
QRDecomp::rsolve(const std::vector<double> &b, std::vector<double> &x)
{
    mcIdType i, j;
    double sum;
    if (sing)
        THROW_IK_EXCEPTION("attempting solve in a singular QR");
    for (i = n - 1; i >= 0; i--)
    {
        sum = b[i];
...
void
QRDecomp::update(const std::vector<double> &u, const std::vector<double> &v)
{
    mcIdType i, k;
...
void
QRDecomp::rotate(const mcIdType i, const double a, const double b)
{
    mcIdType j;
    double c, fact, s, w, y;
    if (a == 0.0)
