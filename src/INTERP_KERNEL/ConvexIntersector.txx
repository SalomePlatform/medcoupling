...
#define CONVINTERSECTOR_TEMPLATE \
    template <                   \
        class MyMeshType,        \
        class MyMatrix,          \
...
CONVEX_INTERSECTOR_::ConvexIntersector(
    ...
    : InterpType<MyMeshType, MyMatrix, CONVEX_INTERSECTOR_>(
          meshT,
          meshS,
          dimCaracteristic,
          precision,
      ...
      _epsilon(precision * dimCaracteristic)
{
    if (PlanarIntersector<MyMeshType, MyMatrix>::_print_level >= 1)
    {
        ...
        if (SPACEDIM == 3)
        {
            if (PlanarIntersector<MyMeshType, MyMatrix>::_do_rotate)
                std::cout << "  _do_rotate = true" << std::endl;
            else
                std::cout << "  _do_rotate = false" << std::endl;
        }
    }
}
...
CONVINTERSECTOR_TEMPLATE
double
CONVEX_INTERSECTOR_::intersectGeometry(ConnType icellT, ConnType icellS, ConnType nbNodesT, ConnType nbNodesS)
{
    double result = 0;
    int orientation = 1;
    ...
    std::deque<double> inter = P.intersectConvexPolygons(
        &CoordsT[0], &CoordsS[0], int(CoordsT.size()) / SPACEDIM, int(CoordsS.size()) / SPACEDIM
    );
    ...
    if (PlanarIntersector<MyMeshType, MyMatrix>::_print_level >= 3)
    {
        ...
        {
            for (int idim = 0; idim < SPACEDIM; idim++) std::cout << inter[SPACEDIM * i + idim] << " ";
            std::cout << std::endl;
        }
...
CONVINTERSECTOR_TEMPLATE
double
CONVEX_INTERSECTOR_::intersectGeometryWithQuadrangle(
...
)
{
    ...
    /*** Compute the intersection area ***/
    INTERP_KERNEL::PolygonAlgorithms<SPACEDIM> P(_epsilon, PlanarIntersector<MyMeshType, MyMatrix>::_precision);
    std::deque<double> inter = P.intersectConvexPolygons(quadrangle, &sourceCoords[0], 4, nbOfNodesS);
    double area[SPACEDIM];
    int nb_inter = ((int)inter.size()) / SPACEDIM;
    for (int i = 1; i < nb_inter - 1; i++)
    {
...
CONVINTERSECTOR_TEMPLATE
double
CONVEX_INTERSECTOR_::intersectGeometryGeneral(
...
)
{
    ...
    /*** Compute the intersection area ***/
    INTERP_KERNEL::PolygonAlgorithms<SPACEDIM> P(_epsilon, PlanarIntersector<MyMeshType, MyMatrix>::_precision);
    std::deque<double> inter = P.intersectConvexPolygons(&targetCoords[0], &sourceCoords[0], nbOfNodesT, nbOfNodesS);
    double area[SPACEDIM];
    int nb_inter = ((int)inter.size()) / SPACEDIM;
    for (int i = 1; i < nb_inter - 1; i++)
    {
...
CONVINTERSECTOR_TEMPLATE
double
CONVEX_INTERSECTOR_::intersectGeoBary(
