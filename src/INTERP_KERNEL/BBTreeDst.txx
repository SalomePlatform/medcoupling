...
class BBTreeDst
{
   ...
   public:
        ...
        : _left(0), _right(0), _level(level), _bb(bbs), _terminal(0), _nbelems(nbelems)
    {
        ...
        for (mcIdType i = 0; i < nbelems; i++) _elems[i] = elems ? elems[i] : i;
        if (_terminal)
        {
        ...
        for (mcIdType i = 0; i < nbelems; i++) nodes[i] = bbs[_elems[i] * dim * 2 + (level % dim) * 2];
    ...
    void getElemsWhoseMinDistanceToPtSmallerThan(
        const double *pt, double minOfMaxDstsSq, std::vector<mcIdType> &elems
    ) const
    {
        if (_terminal)
        {
    ...
    */
    ...
    static double GetMinDistanceFromBBoxToPt(const double *bbox, const double *pt)
    {
        if (bbox[0] <= bbox[1])
        {
            ...
            {
                double val1 = pt[idim] - bbox[idim * 2], val2 = pt[idim] - bbox[idim * 2 + 1];
                char pos = static_cast<char>(
                    (((0. < val1) - (val1 < 0.)) + ((0. < val2) - (val2 < 0.))) / 2
                );  // sign(val) = (0.<val)-(val<0.)
                if (pos != 0)
                {
