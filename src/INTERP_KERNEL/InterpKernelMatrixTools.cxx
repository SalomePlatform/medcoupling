 ...
 */
double
ddot(mcIdType n, const double *dx, mcIdType incx, const double *dy, mcIdType incy)
{
    double dtemp = 0.0;
    mcIdType i, ix, iy, m;
    if (n <= 0)
    ...
    if (incx != 1 || incy != 1)
    {
        ...
        if (incy >= 0)
            ...
            iy = (-n + 1) * incy;
        for (i = 0; i < n; i++, ix += incx, iy += incy) dtemp += dx[ix] * dy[iy];
    }
    ...
    else
    {
        ...
        for (i = m; i < n; i += 5)
            dtemp += dx[i] * dy[i] + dx[i + 1] * dy[i + 1] + dx[i + 2] * dy[i + 2] + dx[i + 3] * dy[i + 3] +
                     dx[i + 4] * dy[i + 4];
    }
    return dtemp;
}
...
void
dscal(mcIdType n, double sa, double *x, mcIdType incx)
{
    mcIdType i, ix, m;
    ...
    else
    {
        ...
        for (i = 0; i < n; i++, ix += incx) x[ix] = sa * x[ix];
    }
}
...
daxpy(mcIdType n, double da, const double *dx, mcIdType incx, double *dy, mcIdType incy)
{
    ...
    if (incx != 1 || incy != 1)
    {
        ...
        for (i = 0; i < n; i++, ix += incx, iy += incy) dy[iy] = dy[iy] + da * dx[ix];
    }
    // Code for both increments equal to 1.
    else
    {
...
void
dswap(mcIdType n, double *x, mcIdType incx, double *y, mcIdType incy)
{
    mcIdType i, ix, iy, m;
    double temp;
 ...
 */
mcIdType
idamax(mcIdType n, const double *dx, mcIdType incx)
{
    double dmax;
    mcIdType i, ix, value;
    value = -1;
 ...
 */
mcIdType
dgefa(double *a, mcIdType lda, mcIdType n, mcIdType *ipvt)
{
    mcIdType info = 0;
    mcIdType l;
    double t;
 ...
 *  Parameters:
 *
 *    \param [in,out] a matrix of size lda*n, on input, the LU factor information, as output of dgfa. On output, the
 * inverse matrix.
 ...
 */
void
dgedi(double *a, mcIdType lda, mcIdType n, const mcIdType *ipvt, double *work)
{
    double t;
...
matrixProduct(const double *A, mcIdType n1, mcIdType p1, const double *B, mcIdType n2, mcIdType p2, double *C)
{
    if (p1 != n2)
    {
        ...
        oss << "matrixProduct : the size of input matrix are not coherent the nb of cols of input matrix #0 is " << p1
            << " whereas the number of rows of input matrix #1 is " << n2 << " !";
    ...
    for (mcIdType i = 0; i < n1; i++)
    {
        ...
        {
            C[i * p2 + j] = 0.;
            for (mcIdType k = 0; k < p1; k++) C[i * p2 + j] += A[i * p1 + k] * B[k * p2 + j];
        }
    }
}
