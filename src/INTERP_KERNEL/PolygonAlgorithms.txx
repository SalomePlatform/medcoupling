...
template <int DIM>
bool
PolygonAlgorithms<DIM>::intersectSegmentSegment(
    const double *A, const double *B, const double *C, const double *D, const double *E, double *V
)
{
        ...
        switch (DIM)
        {
            case 2:
            {
                if (distance2<DIM>(A, D) < _epsilon)
                    crossprod<DIM>(
                        A, C, E, _vdouble
                    );         // store the crossprod between vectors AC and AE (E=vertex preceding A)
    ...
    else if (fabs(t1) <= _precision)
    {
        if (t2 > _precision && t2 < 1 - _precision)  // vertex on an edge
        {
            ...
            else if (same_side > _epsilon)
                _terminus = !_is_in_intersection;  // reflexion
...
template <int DIM>
inline void
PolygonAlgorithms<DIM>::addNewVertex(int i, int i_glob, int i_next_glob, int i_prev_glob, const double *P)
{
...
template <int DIM>
inline void
PolygonAlgorithms<DIM>::addCrossing(double *ABCD, std::pair<int, int> i_i_next, std::pair<int, int> j_j_next)
{
    if (!_Inter.empty())
    {
...
template <int DIM>
void
PolygonAlgorithms<DIM>::addCrossing0(
    const double *A, const double *B, int i, int i_next, const double *C, const double *D, int j, int j_next
)
{
    double ABCD[DIM];
    ...
    // fifth and sixth arguments are useless here
    {
        ...
        _Status.insert(std::make_pair(i_next, std::make_pair(i, true)));
}
...
template <int DIM>
inline void
PolygonAlgorithms<DIM>::addCrossings(
    const double *A,
    const double *B,
    int i,
    int i_next,
...
template <int DIM>
inline void
PolygonAlgorithms<DIM>::defineIndices(
    int &i_loc,
    int &i_next,
    int &i_prev,
    const double *&Poly1,
...
template <int DIM>
std::deque<double>
PolygonAlgorithms<DIM>::intersectConvexPolygons(const double *P_1, const double *P_2, int N1, int N2)
{
    int i_loc, i_glob, j1, j1_glob, j2, j2_glob, j3, j3_glob, j4, j4_glob, i_prev, i_prev_glob, i_next, i_next_glob,
        nb_prev = 0, sign, idim;
    ...
    for (i_loc = 0; i_loc < N1; i_loc++) mmap_events.insert(std::make_pair(&P_1[DIM * i_loc], i_loc));
    for (i_loc = 0; i_loc < N2; i_loc++) mmap_events.insert(std::make_pair(&P_2[DIM * i_loc], i_loc + N1));
    ...
    if (!_terminus)
    {
        ...
        if (i_glob < N1)
        {
            i_next_glob = (i_glob + 1) % N1;
            i_prev_glob = (i_glob - 1 + N1) % N1;
        }
        else
        {
            i_next_glob = (i_glob - N1 + 1) % N2 + N1;
            i_prev_glob = (i_glob - N1 - 1 + N2) % N2 + N1;
        }
        ...
        while (!four_neighbours)
        {
            ...
            switch (nb_prev)
            {
                case 1:
                    ...
                    if (i_glob < N1)
                        i_next_glob = (i_next + N1) % N1;
                    else
                        i_next_glob = (i_next - N1 + N2) % N2 + N1;
                    _Status.erase(mi);
                    _Status.insert(std::make_pair(i_next_glob, std::make_pair(i_glob, false)));
                    mi1++;
                    break;
            ...
            switch (nb_prev)
            {
                case 1:
                    ...
                    if (_is_in_intersection)
                        addNewVertex(i_loc, i_glob, i_next_glob, i_prev_glob, Poly1);
                    addCrossings(
                        &Poly1[DIM * i_loc],
                        &Poly1[DIM * i_next],
                        i_glob,
                ...
                case 2:
                    ...
                    {
                        if (i_glob < N1)
                            for (idim = 0; idim < DIM; idim++) _Inter.push_back(P_1[DIM * i_glob + idim]);
                        else
                            for (idim = 0; idim < DIM; idim++) _Inter.push_back(P_2[DIM * (i_glob - N1) + idim]);
                    }
                    return _Inter;
                case 0:  // To do if possible : remove this case from here
                    if (_Inter.empty() && (i_glob < N1) != which_start)
                    {
                        ...
                        double inside = check_inside<DIM>(
                            &Poly1[DIM * i_loc],
                            &Poly2[DIM * j1],
                            &Poly2[DIM * j2],
                            &Poly2[DIM * j3],
                        ...
                        if (fabs(inside) > _epsilon)  // vertex clearly inside or outside
                        {
                            ...
                            if (_is_in_intersection)
                            {
                                for (int iidim = 0; iidim < DIM; iidim++) _Inter.push_back(Poly1[DIM * i_loc + iidim]);
                        ...
                        else  // vertex on an edge
                        {
                            ...
                            if (dotprod<DIM>(V34, V34) > _epsilon)  // vertex i on edge (j1,j2), not on (j3,j4)
                            {
                                ...
                                if (!(is_inside_next || is_inside_prev))
                            ...
                            else  // vertices i, j1 and j3 share the same coordinates
                            {
                                ...
                                std::multimap<int, std::pair<int, bool> >::iterator j2_in_status =
                                    _Status.find(((*min).second).second);
                                std::multimap<int, std::pair<int, bool> >::iterator j4_in_status =
                                    _Status.find(((*minext).second).second);
                                ...
                                if ((*min).first < -_epsilon)  // there is someone clearly inside
                                {
                                    ...
                                    is_inside_next = ((*min).second).second == i_next_glob ||
                                                     ((*minext).second).second == i_next_glob;
                                    is_inside_prev = ((*min).second).second == i_prev_glob ||
                                                     ((*minext).second).second == i_prev_glob;
                                }
                                else if (fabs((*min).first) <=
                                         _epsilon)  // nobody is clearly inside but two segments are superposed
                                {
...
template <int DIM>
inline void
PolygonAlgorithms<DIM>::convHull(
    const double *P,
    int N,
    double *normal,
...
)
{
    if (NsubP > 3)
    {
        ...
        {
            directframe = direct_frame<DIM>(
                &P[DIM * (*mi1).second], &P[DIM * (*subP.begin()).second], &P[DIM * (*mi2).second], normal
            );
            mi2++;
        }
        if (directframe < 0)
            for (int idim = 0; idim < DIM; idim++) normal[idim] *= -1;

        /* Core of the algorithm */
        while (mi_next != subP.end())
        {
            directframe = direct_frame<DIM>(
                &P[DIM * (*mi).second], &P[DIM * (*mi_prev).second], &P[DIM * (*mi_next).second], normal
            );
        ...
        directframe = direct_frame<DIM>(
            &P[DIM * (*mi).second], &P[DIM * (*mi_prev).second], &P[DIM * (*subP.begin()).second], normal
        );
        if (directframe < -epsilon)
        {
            not_in_hull.insert(*mi);
...
template <int DIM>
void
PolygonAlgorithms<DIM>::convexDecomposition(
    const double *P,
    int N,
    double *normal,
    std::vector<int> subP,
