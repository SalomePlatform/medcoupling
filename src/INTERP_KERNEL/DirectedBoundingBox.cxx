// Copyright (C) 2009-2025  OPEN CASCADE
//
...
#define __DMP(msg)  //  cout << msg << endl
...
inline void
addPointToInertiaTensor3D(const double *coord, const double *gc, vector<double> &tensor)
{
...
inline void
addPointToInertiaTensor2D(const double *coord, const double *gc, vector<double> &tensor)
{
    // we fill the upper triangle of tensor only
    const int _dim = 2;
...
bool
JacobiEigenvectorsSearch(const int _dim, vector<double> &tensor, vector<double> &_axes)
{
    if (_dim == 3)
    {
        __DMP(
            "Tensor : {" << "{ " << __TENSOR(0, 0) << ", " << __TENSOR(0, 1) << ", " << __TENSOR(0, 2) << "} "
    ...
    {
        __DMP(
            "Tensor : {" << "{ " << __TENSOR(0, 0) << ", " << __TENSOR(0, 1) << "} "
    ...
    int i, j;
    for (i = 0; i < _dim; ++i)
        for (j = 0; j < _dim; ++j) __AXIS(i)[j] = (i == j ? 1. : 0);
...
#define __ROTATE(T, r1, c1, r2, c2)                   \
    ...
    else
    {
        ...
        for (int ii = 0; ii < 2; ++ii) __DMP(ii << ": " << __AXIS(ii)[0] << ", " << __AXIS(ii)[1]);
    }
...
inline bool
isMinMaxOut(const double *minmax1, const double *minmax2, int dim)
{
    for (int i = 0; i < dim; ++i)
    {
        if (minmax1[i * 2] > minmax2[i * 2 + 1] || minmax1[i * 2 + 1] < minmax2[i * 2])
            return true;
    }
    return false;
}
...
DirectedBoundingBox::DirectedBoundingBox(const double *pts, const unsigned numPts, const unsigned dim)
    : _dim(dim), _axes(dim * dim), _minmax(2 * dim)
{
    // init box extremities
    for (unsigned i = 0; i < _dim; ++i)
        _minmax[1 + i * 2] = -numeric_limits<double>::max(), _minmax[i * 2] = numeric_limits<double>::max();
    ...
    if (dim > 1)
    {
        for (coord = pts; coord < coordEnd;)
            for (int i = 0; i < (int)dim; ++i) gc[i] += *coord++;
        for (int j = 0; j < (int)dim; ++j) gc[j] /= numPts;
    }
    ...
    {
        case 3:
            for (coord = pts; coord < coordEnd; coord += dim) addPointToInertiaTensor3D(coord, gc, tensor);

            // computeAxes3D(tensor);
            JacobiEigenvectorsSearch(_dim, tensor, _axes);

            for (coord = pts; coord < coordEnd; coord += dim) addPointToBox(coord);

            break;

        case 2:
            for (coord = pts; coord < coordEnd; coord += dim) addPointToInertiaTensor2D(coord, gc, tensor);
    ...
    : _dim(dim), _axes(dim * dim), _minmax(2 * dim)
{
    ...
    if (dim > 1)
    {
        for (unsigned i = 0; i < numPts; ++i)
            for (int j = 0; j < (int)dim; ++j) gc[j] += pts[i][j];
        for (int j = 0; j < (int)dim; ++j) gc[j] /= numPts;
    }
    ...
    {
        case 3:
            for (unsigned i = 0; i < numPts; ++i) addPointToInertiaTensor3D(pts[i], gc, tensor);

            // computeAxes3D(tensor);
            JacobiEigenvectorsSearch(_dim, tensor, _axes);

            for (unsigned i = 0; i < numPts; ++i) addPointToBox(pts[i]);

            break;
        case 2:
            for (unsigned i = 0; i < numPts; ++i) addPointToInertiaTensor2D(pts[i], gc, tensor);
        ...
        default:
            for (unsigned i = 0; i < numPts; ++i)
            {
                if (pts[i][0] < _minmax[0])
                    _minmax[0] = pts[i][0];
                if (pts[i][0] > _minmax[1])
                    _minmax[1] = pts[i][0];
            }
            _axes[0] = 1.0;
    }
...
void
DirectedBoundingBox::toLocalCS(const double *p, double *pLoc) const
{
    switch (_dim)
    {
        case 3:
...
void
DirectedBoundingBox::fromLocalCS(const double *p, double *pGlob) const
{
    switch (_dim)
    {
        case 3:
...
void
DirectedBoundingBox::enlarge(const double tol)
{
    for (unsigned i = 0; i < _dim; ++i) __MIN(i) -= tol, __MAX(i) += tol;
}
...
void
DirectedBoundingBox::getCorners(std::vector<double> &corners, const double *minmax) const
{
    int iC, nbCorners = 1;
...
bool
DirectedBoundingBox::isDisjointWith(const DirectedBoundingBox &box) const
{
    if (_dim < 1 || box._dim < 1)
        return false;  // empty box includes all
    if (_dim == 1)
...
bool
DirectedBoundingBox::isDisjointWith(const double *box) const
{
    if (_dim < 1)
        return false;  // empty box includes all
    if (_dim == 1)
    ...
    // compare minmaxes in global CS
    {
        ...
        for (std::size_t iC = 0, nC = cornersThis.size() / _dim; iC < nC; ++iC)
        {
            ...
            {
                if (globCorner[i] < mmBox._minmax[i * 2])
                    mmBox._minmax[i * 2] = globCorner[i];
                if (globCorner[i] > mmBox._minmax[i * 2 + 1])
                    mmBox._minmax[i * 2 + 1] = globCorner[i];
            }
        }
...
bool
DirectedBoundingBox::isOut(const double *point) const
{
    if (_dim < 1)
        return false;  // empty box includes all
    ...
    {
        case 3:
            __DMP(__MYID << ": " << point[0] << ", " << point[1] << ", " << point[2] << " " << (out ? "OUT" : "IN"));
            break;
        case 2:
            __DMP(__MYID << ": " << point[0] << ", " << point[1] << " " << (out ? "OUT" : "IN"));
            break;
        case 1:
            __DMP(__MYID << ": " << point[0] << " " << (out ? "OUT" : "IN"));
            break;
    }
#endif
    return out;
...
vector<double>
DirectedBoundingBox::getData() const
{
...
void
DirectedBoundingBox::setData(const double *data)
{
    _dim = unsigned(*data++);
    if (_dim > 0)
    {
        _axes.assign(data, data + _dim * _dim);
        data += _dim * _dim;
...
int
DirectedBoundingBox::dataSize(int dim)
{
