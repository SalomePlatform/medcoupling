...
MappedBarycentric2DIntersectorP1P1<MyMeshType, MyMatrix>::MappedBarycentric2DIntersectorP1P1(
...
template <class MyMeshType, class MyMatrix>
void
MappedBarycentric2DIntersectorP1P1<MyMeshType, MyMatrix>::intersectCells(
...
)
{
    ...
    for (typename std::vector<ConnType>::const_iterator iter = icellsS.begin(); iter != icellsS.end(); iter++)
    {
        NormalizedCellType tS =
            PlanarIntersector<MyMeshType, MyMatrix>::_meshS.getTypeOfElement(OTT<ConnType, numPol>::indFC(*iter));
        if (tS != NORM_QUAD4)
        ...
        if (SPACEDIM == 3)
            PlanarIntersector<MyMeshType, MyMatrix>::projectionThis(
                &CoordsS[0], &CoordsTTmp[0], ToConnType(CoordsS.size()) / SPACEDIM, nbOfNodesT
            );
        ...
        for (ConnType nodeIdT = 0; nodeIdT < nbOfNodesT; nodeIdT++)
        {
            ...
            if (PointLocatorAlgos<MyMeshType>::isElementContainsPointAlg2DSimple(
                    &CoordsTTmp[nodeIdT * SPACEDIM], &CoordsS[0], 4, PlanarIntersector<MyMeshType, MyMatrix>::_precision
                ))
            {
                double mco[2];  // mapped coordinates in the quad4
                std::vector<const double *> coo(4);
                coo[0] = &CoordsS[0];
                coo[1] = &CoordsS[SPACEDIM];
                coo[2] = &CoordsS[2 * SPACEDIM];
                coo[3] = &CoordsS[3 * SPACEDIM];
                ...
                const ConnType *startOfCellNodeConnS =
                    PlanarIntersector<MyMeshType, MyMatrix>::_connectS +
                    OTT<ConnType, numPol>::conn2C(PlanarIntersector<MyMeshType, MyMatrix>::_connIndexS[*iter]);
                for (ConnType nodeIdS = 0; nodeIdS < 4; nodeIdS++)
                {
                    if (fabs(resLoc[nodeIdS]) > PlanarIntersector<MyMeshType, MyMatrix>::_precision)
                    {
                        ...
                        if (iterRes == resRow.end())
                            resRow.insert(
                                std::make_pair(OTT<ConnType, numPol>::indFC(curNodeSInCmode), resLoc[nodeIdS])
                            );
                        else
                        {
...
MappedBarycentric2DIntersectorP1P1<MyMeshType, MyMatrix>::getNumberOfRowsOfResMatrix() const
{
...
template <class MyMeshType, class MyMatrix>
typename MyMeshType::MyConnType
MappedBarycentric2DIntersectorP1P1<MyMeshType, MyMatrix>::getNumberOfColsOfResMatrix() const
{
