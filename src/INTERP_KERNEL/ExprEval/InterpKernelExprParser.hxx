...
{
   public:
    LeafExprVar(const LeafExprVar &other)
        : _fast_pos(other._fast_pos), _ref_pos(other._ref_pos), _var_name(other._var_name), _val(other._val)
    {
    }
    ...
    INTERPKERNEL_EXPORT void prepareExprEvaluation(
        const std::vector<std::string> &vars, int nbOfCompo, int targetNbOfCompo
    ) const;
    INTERPKERNEL_EXPORT void prepareExprEvaluationDouble(
...
class ExprParserOfEval
{
   public:
    ...
    ExprParserOfEval(
        LeafExpr *leaf, const std::vector<ExprParserOfEval> &subParts, const std::vector<Function *> &funcs
    )
        : _leaf(leaf), _sub_parts(subParts), _funcs(funcs)
    {
    }
    void evaluateDoubleInternal(std::vector<double> &stck) const
    {
        if (_leaf)
            ...
            for (std::vector<ExprParserOfEval>::const_iterator iter = _sub_parts.begin(); iter != _sub_parts.end();
                 iter++)
