...
LeafExpr *
LeafExpr::buildInstanceFrom(const std::string &expr)
{
...
void
LeafExprVal::fillValue(Value *val) const
{
    val->setDouble(_value);
}

void
LeafExprVal::replaceValues(const std::vector<double> &valuesInExpr)
{
    int pos = (int)_value;
    int lgth = (int)valuesInExpr.size();
    if (pos >= lgth || pos < 0)
        throw INTERP_KERNEL::Exception(
            "LeafExprVal::replaceValues : Big Problem detected! Send a mail to Salome support with expression."
        );
...
void
LeafExprVar::fillValue(Value *val) const
{
    if (_val)
...
void
LeafExprVar::prepareExprEvaluation(const std::vector<std::string> &vars, int nbOfCompo, int targetNbOfCompo) const
{
    ...
    if (iter == vars.end())
    {
        ...
        else
        {
            ...
            {
                std::ostringstream oss;
                oss << "LeafExprVar::prepareExprEvaluation : Found recognized unitary vector \"" << _var_name
                    << "\" which implies that component #" << relPos;
    ...
    {
        std::ostringstream oss;
        oss << "LeafExprVar::prepareExprEvaluation : Found var \"" << _var_name << "\" on place " << _fast_pos
            << " whereas only must be in [0," << nbOfCompo << ") !";
        throw INTERP_KERNEL::Exception(oss.str().c_str());
    }
}
 ...
 */
void
LeafExprVar::prepareExprEvaluationDouble(
    const std::vector<std::string> &vars,
    int nbOfCompo,
    int targetNbOfCompo,
    int refPos,
    const double *ptOfInputStart,
    const double *ptOfInputEnd
) const
{
    if ((int)vars.size() != std::distance(ptOfInputStart, ptOfInputEnd))
        throw INTERP_KERNEL::Exception(
            "LeafExprVar::prepareExprEvaluationDouble : size of input vector must be equal to the input vector !"
        );
...
void
LeafExprVar::prepareExprEvaluationVec() const
{
    if (!isRecognizedKeyVar(_var_name, _fast_pos))
        _fast_pos = -2;
}
...
bool
LeafExprVar::isRecognizedKeyVar(const std::string &var, int &pos)
{
 ...
 */
void
LeafExprVar::replaceValues(const std::vector<double> &valuesInExpr)
{
}
...
void
ExprParserOfEval::clearSortedMemory()
{
    delete _leaf;
    for (std::vector<ExprParserOfEval>::iterator it = _sub_parts.begin(); it != _sub_parts.end(); it++)
        (*it).clearSortedMemory();
    for (std::vector<Function *>::iterator it = _funcs.begin(); it != _funcs.end(); it++) delete *it;
}

void
ExprParserOfEval::sortMemory()
{
    for (std::vector<ExprParserOfEval>::iterator it = _sub_parts.begin(); it != _sub_parts.end(); it++)
        (*it).sortMemory();
    if (_leaf)
...
ExprParser::ExprParser(const std::string &expr, ExprParser *father)
    : _father(father), _is_parsed(false), _leaf(0), _is_parsing_ok(false), _expr(expr)
{
    _expr = deleteWhiteSpaces(_expr);
}

//! For \b NOT null terminated strings coming from FORTRAN.
ExprParser::ExprParser(const char *expr, int lgth, ExprParser *father)
    : _father(father), _is_parsed(false), _leaf(0), _is_parsing_ok(false)
{
...
std::size_t
ExprParser::FindCorrespondingOpenBracket(const std::string &expr, std::size_t posOfCloseBracket)
{
    int level = 0;
...
std::string
ExprParser::buildStringFromFortran(const char *expr, int lgth)
{
...
std::string
ExprParser::deleteWhiteSpaces(const std::string &expr)
{
    std::string ret(expr);
    std::string whiteSpaces(WHITE_SPACES);
...
void
ExprParser::evaluateExpr(int szOfOutParam, const double *inParam, double *outParam) const
{
...
void
ExprParser::prepareExprEvaluation(const std::vector<std::string> &vars, int nbOfCompo, int targetNbOfCompo) const
{
    if (_leaf)
    {
 ...
 */
void
ExprParser::prepareExprEvaluationDouble(
    const std::vector<std::string> &vars,
    int nbOfCompo,
    int targetNbOfCompo,
    int refPos,
...
) const
{
    if ((int)vars.size() != std::distance(ptOfInputStart, ptOfInputEnd))
        throw INTERP_KERNEL::Exception(
            "ExprParser::prepareExprEvaluationDouble : size of input vector must be equal to the input vector !"
        );
    if (_leaf)
    {
        LeafExprVar *leafC = dynamic_cast<LeafExprVar *>(_leaf);
...
void
ExprParser::prepareFastEvaluator() const
{
    _for_eval.clearSortedMemory();
 ...
 */
double
ExprParser::evaluateDouble() const
{
    checkForEvaluation();
...
void
ExprParser::checkForEvaluation() const
{
    if (!_is_parsing_ok)
...
void
ExprParser::prepareExprEvaluationVec() const
{
    ...
    {
        std::ostringstream oss;
        oss << "For this type of evaluation only one not keyword variable authorized : ";
        oss << "having " << trueVars.size() << " : ";
        std::copy(trueVars.begin(), trueVars.end(), std::ostream_iterator<std::string>(oss, " "));
        oss << " !";
        throw INTERP_KERNEL::Exception(oss.str().c_str());
    }
...
void
ExprParser::prepareExprEvaluationVecLowLev() const
{
    if (_leaf)
    {
        LeafExprVar *leafC = dynamic_cast<LeafExprVar *>(_leaf);
...
Value *
ExprParser::evaluateLowLev(Value *valGen) const
{
    checkForEvaluation();
    ...
    try
    {
        ...
        else
        {
            ...
            for (std::vector<ExprParser>::const_iterator iter = _sub_expr.begin(); iter != _sub_expr.end();
                 iter++, iter2++)
                *iter2 = (*iter).evaluateLowLev(valGen);
        }
        for (std::vector<Function *>::const_iterator iter3 = _func_btw_sub_expr.begin();
             iter3 != _func_btw_sub_expr.end();
             iter3++)
...
ExprParser::ExprParser(ExprParser &&other)
...
ExprParser::reverseThis()
{
    ...
    for (std::size_t i = 0; i < nbOfTurn; i++) std::swap(_sub_expr[i], _sub_expr[sz - i - 1]);
}

ExprParserOfEval
ExprParser::convertMeTo() const
{
    std::size_t sz(_sub_expr.size());
    std::vector<ExprParserOfEval> subExpr(sz);
    for (std::size_t i = 0; i < sz; i++) subExpr[i] = _sub_expr[i].convertMeTo();
    return ExprParserOfEval(_leaf, subExpr, _func_btw_sub_expr);
}

void
ExprParser::getSetOfVars(std::set<std::string> &vars) const
{
    if (_leaf)
    {
        LeafExprVar *leafC = dynamic_cast<LeafExprVar *>(_leaf);
...
void
ExprParser::getTrueSetOfVars(std::set<std::string> &trueVars) const
{
 ...
 * This method first try to see if this->_expr is a leaf, if not it try a unary function of something (see
 * INTERP_KERNEL::ExprParser::parseUnaryFunc method) If true is returned, no deeper parsing needed, if false is returned
 * for a full parsing of this->_expr INTERP_KERNEL::ExprParser::parseDeeper call needed.
 */
bool
ExprParser::simplify()
{
...
void
ExprParser::checkBracketsParity() const
{
    std::string::const_iterator iter;
    int curLevel = 0;
    for (iter = _expr.begin(); iter != _expr.end(); iter++)
...
/*!
 * This method substitutes part in [bg,end) in expr by the content of (str(id)) and returns the double value
 * representation in expr[bg,end). If double representation is invalid an exception is thrown. This method returns a
 * delta that is the delta to operate to pos in expr after substitution.
 */
double
ExprParser::ReplaceAndTraduce(std::string &expr, int id, std::size_t bg, std::size_t end, int &delta)
{
 ...
 */
void
ExprParser::fillValuesInExpr(std::vector<double> &valuesInExpr)
{
    ...
    for (std::size_t pos = 0; pos != std::string::npos; id++)
    {
        ...
        {  // treat case of "x*log10(x)" -> "10" should NOT be intercepted by this
            ...
            if (_expr[pos2 - 1] == ')')
            {
                pos = _expr.find_first_not_of(FIGURES, pos2, 10);
                std::ostringstream oss;
                oss << "Problem on parsing : Number \""
                    << _expr.substr(pos2, pos != std::string::npos ? pos2 - pos : std::string::npos);
                ...
                else
                {
                    if (++pos4 < lgth)
                    {
                        ...
                        {  //"x+1223334.223e+" or "1223334.223E-"
                            std::ostringstream oss;
                            oss << "Invalid expr : float number at the end of expr is invalid lacking number after "
                        ...
                        {  //"x+1223334.223e+x" or "1223334.223E-y"
                            std::ostringstream oss;
                            oss << "Invalid expr : float number in expr is invalid lacking number after exponential ! "
                                   "-> \""
                                << _expr.substr(pos2, pos4 - pos2) << "\"";
                            throw INTERP_KERNEL::Exception(oss.str().c_str());
                        }
                        // OK, normal case
...
void
ExprParser::replaceValues(const std::vector<double> &valuesInExpr)
{
    if (_leaf)
...
void
ExprParser::LocateError(std::ostream &stringToDisp, const std::string &srcOfErr, int posOfErr)
{
    stringToDisp << "Position is " << posOfErr << " of string : \"" << srcOfErr << "\"" << std::endl;
}

char *
ExprParser::compileX86() const
{
...
char *
ExprParser::compileX86_64() const
{
    std::vector<std::string> ass;
    // need in stack
...
void
ExprParser::compileX86LowLev(std::vector<std::string> &ass) const
{
    if (_leaf)
    ...
    for (std::vector<Function *>::const_iterator iter2 = _func_btw_sub_expr.begin(); iter2 != _func_btw_sub_expr.end();
         iter2++)
        (*iter2)->operateX86(ass);
}

void
ExprParser::compileX86_64LowLev(std::vector<std::string> &ass) const
{
    if (_leaf)
...
double
LeafExprVal::getDoubleValue() const
{
    return _value;
}

void
LeafExprVal::compileX86(std::vector<std::string> &ass) const
{
...
void
LeafExprVal::compileX86_64(std::vector<std::string> &ass) const
{
    ass.push_back("sub rsp,8");
    const int *b = reinterpret_cast<const int *>(&_value), *c = reinterpret_cast<const int *>(&_value);
    c++;
...
double
LeafExprVar::getDoubleValue() const
{
    if (_fast_pos >= 0)
...
void
LeafExprVar::compileX86(std::vector<std::string> &ass) const
{
    ass.push_back("fld qword [ebp+8]");
}

void
LeafExprVar::compileX86_64(std::vector<std::string> &ass) const
{
    ass.push_back("sub rsp,8");
...
int
ExprParser::getStackSizeToPlayX86(const ExprParser *asker) const
{
    if (asker)
    {
        int sz = _father->getStackSizeToPlayX86(this);
        int i = 0;
        for (std::vector<ExprParser>::const_reverse_iterator iter = _sub_expr.rbegin(); iter != _sub_expr.rend();
             iter++, i++)
        {
        ...
        throw INTERP_KERNEL::Exception(
            "error getStackSizeToPlayX86 an object ExprParser called as father, whereas it is not one !"
        );
    }
    else
    {
