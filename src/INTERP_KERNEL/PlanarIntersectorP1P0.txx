...
PlanarIntersectorP1P0<MyMeshType, MyMatrix, ConcreteP1P0Intersector>::PlanarIntersectorP1P0(
...
PlanarIntersectorP1P0<MyMeshType, MyMatrix, ConcreteP1P0Intersector>::getNumberOfRowsOfResMatrix() const
{
...
template <class MyMeshType, class MyMatrix, class ConcreteP1P0Intersector>
typename MyMeshType::MyConnType
PlanarIntersectorP1P0<MyMeshType, MyMatrix, ConcreteP1P0Intersector>::getNumberOfColsOfResMatrix() const
{
...
template <class MyMeshType, class MyMatrix, class ConcreteP1P0Intersector>
void
PlanarIntersectorP1P0<MyMeshType, MyMatrix, ConcreteP1P0Intersector>::intersectCells(
...
)
{
    ...
    PlanarIntersector<MyMeshType, MyMatrix>::getRealTargetCoordinates(
        OTT<ConnType, numPol>::indFC(icellT), targetCellCoords
    );
    ...
    for (typename std::vector<ConnType>::const_iterator iter = icellsS.begin(); iter != icellsS.end(); iter++)
    {
        ...
        for (ConnType nodeIdS = 0; nodeIdS < nbNodesS; nodeIdS++)
        {
            ...
            std::copy(
                PlanarIntersector<MyMeshType, MyMatrix>::_coordsS + curNodeSInCmode * SPACEDIM,
                PlanarIntersector<MyMeshType, MyMatrix>::_coordsS + curNodeSInCmode * SPACEDIM + SPACEDIM,
                triangle
            );
            for (ConnType subTriS = 1; subTriS <= nbNodesS - 2; subTriS++)
            {
                std::copy(
                    PlanarIntersector<MyMeshType, MyMatrix>::_coordsS +
                        OTT<ConnType, numPol>::coo2C(startOfCellNodeConn[(nodeIdS + subTriS) % nbNodesS]) * SPACEDIM,
                    PlanarIntersector<MyMeshType, MyMatrix>::_coordsS +
                        OTT<ConnType, numPol>::coo2C(startOfCellNodeConn[(nodeIdS + subTriS) % nbNodesS]) * SPACEDIM +
                        SPACEDIM,
                    triangle + SPACEDIM
                ...
                std::copy(
                    PlanarIntersector<MyMeshType, MyMatrix>::_coordsS +
                        OTT<ConnType, numPol>::coo2C(startOfCellNodeConn[(nodeIdS + subTriS + 1) % nbNodesS]) *
                            SPACEDIM,
                    PlanarIntersector<MyMeshType, MyMatrix>::_coordsS +
                        OTT<ConnType, numPol>::coo2C(startOfCellNodeConn[(nodeIdS + subTriS + 1) % nbNodesS]) *
                            SPACEDIM +
                ...
                if (SPACEDIM == 3)
                    orientation = PlanarIntersector<MyMeshType, MyMatrix>::projectionThis(
                        &targetCellCoordsTmp[0], quadrangle, ToConnType(targetCellCoords.size()) / SPACEDIM, 4
                    );
                double surf =
                    orientation * intersectGeometryWithQuadrangle(quadrangle, targetCellCoordsTmp, isTargetQuad);
                ...
                if (surf != 0.)
                {
                    typename MyMatrix::value_type::const_iterator iterRes =
                        resRow.find(OTT<ConnType, numPol>::indFC(curNodeSInCmode));
