...
template <class MyMeshType, class MyMatrix>
void
PointLocator3DIntersectorP1P1<MyMeshType, MyMatrix>::intersectCells(
    ConnType targetCell, const std::vector<ConnType> &srcCells, MyMatrix &res
)
{
    std::vector<double> CoordsT;
    Intersector3DP1P1<MyMeshType, MyMatrix>::getRealTargetCoordinates(
        OTT<ConnType, numPol>::indFC(targetCell), CoordsT
    );
    ...
    for (typename std::vector<ConnType>::const_iterator iterCellS = srcCells.begin(); iterCellS != srcCells.end();
         iterCellS++)
    {
        NormalizedCellType tS = Intersector3DP1P1<MyMeshType, MyMatrix>::_src_mesh.getTypeOfElement(
        ...
        const ConnType *startOfCellNodeConnT =
            Intersector3DP1P1<MyMeshType, MyMatrix>::getStartConnOfTargetCell(targetCell);
        for (std::size_t nodeIdT = 0; nodeIdT < nbOfNodesT; nodeIdT++)
        {
                ...
                ))
            {
                ...
                Intersector3DP1P1<MyMeshType, MyMatrix>::getRealSourceCoordinates(
                    OTT<ConnType, numPol>::indFC(*iterCellS), localCoordsS
                );
                std::vector<const double *> eap(4);
                eap[0] = &localCoordsS[0];
                eap[1] = &localCoordsS[3];
                eap[2] = &localCoordsS[6];
                eap[3] = &localCoordsS[9];
                barycentric_coords(eap, &CoordsT[nodeIdT * SPACEDIM], resLoc);
                const ConnType *startOfCellNodeConnS =
                    Intersector3DP1P1<MyMeshType, MyMatrix>::getStartConnOfSourceCell(*iterCellS);
