...
PolyhedronIntersectorP1P0<MyMeshType, MyMatrix>::PolyhedronIntersectorP1P0(
    ...
    : Intersector3DP1P0<MyMeshType, MyMatrix>(targetMesh, srcMesh), _split(srcMesh, targetMesh, policy)
{
}
...
template <class MyMeshType, class MyMatrix>
void
PolyhedronIntersectorP1P0<MyMeshType, MyMatrix>::releaseArrays()
{
...
void
AddContributionInRow(RowType &row, ConnType colId, double value)
{
    if (value != 0.)
    {
...
template <class MyMeshType, class MyMatrix>
void
PolyhedronIntersectorP1P0<MyMeshType, MyMatrix>::intersectCells(
...
)
{
         ...
         iterCellS++)
    {
        ...
        INTERP_KERNEL::NormalizedCellType srcType =
            this->_src_mesh.getTypeOfElement(OTT<ConnType, numPol>::indFC(*iterCellS));
        if (srcType == NORM_TETRA4 || (srcType == NORM_HEXA8 && sp != GENERAL_24))
        {
            for (typename std::vector<SplitterTetra<MyMeshType> *>::const_iterator iter = _tetra.cbegin();
                 iter != _tetra.cend();
                 ++iter)
            {
        ...
        {  // for HEXA and GENERAL_24 no need to use subsplitting into dual mesh
            for (typename std::vector<ConnType>::const_iterator iterCellS = srcCells.begin();
                 iterCellS != srcCells.end();
                 iterCellS++)
            {
                releaseArrays();
                ConnType nbOfNodesS = Intersector3D<MyMeshType, MyMatrix>::_src_mesh.getNumberOfNodesOfElement(
