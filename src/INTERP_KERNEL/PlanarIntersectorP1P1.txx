...
PlanarIntersectorP1P1<MyMeshType, MyMatrix, ConcreteP1P1Intersector>::PlanarIntersectorP1P1(
...
PlanarIntersectorP1P1<MyMeshType, MyMatrix, ConcreteP1P1Intersector>::getNumberOfRowsOfResMatrix() const
{
...
template <class MyMeshType, class MyMatrix, class ConcreteP1P1Intersector>
typename MyMeshType::MyConnType
PlanarIntersectorP1P1<MyMeshType, MyMatrix, ConcreteP1P1Intersector>::getNumberOfColsOfResMatrix() const
{
...
template <class MyMeshType, class MyMatrix, class ConcreteP1P1Intersector>
void
PlanarIntersectorP1P1<MyMeshType, MyMatrix, ConcreteP1P1Intersector>::intersectCells(
...
)
{
    ...
    for (ConnType nodeIdT = 0; nodeIdT < nbNodesT; nodeIdT++)
    {
        ...
        PlanarIntersector<MyMeshType, MyMatrix>::getRealTargetCoordinatesPermute(
            OTT<ConnType, numPol>::indFC(icellT), nodeIdT, polygT
        );
        ...
        for (typename std::vector<ConnType>::const_iterator iter = icellsS.begin(); iter != icellsS.end(); iter++)
        {
            ...
            for (ConnType nodeIdS = 0; nodeIdS < nbNodesS; nodeIdS++)
            {
                ...
                PlanarIntersector<MyMeshType, MyMatrix>::getRealSourceCoordinatesPermute(
                    OTT<ConnType, numPol>::indFC(iS), nodeIdS, polygS
                );
                ...
                if (SPACEDIM == 3)
                    orientation = PlanarIntersector<MyMeshType, MyMatrix>::projectionThis(
                        &polygDualS[0],
                        &polygDualTTmp[0],
                        ToConnType(polygDualS.size()) / SPACEDIM,
                        ToConnType(polygDualT.size()) / SPACEDIM
                    );
