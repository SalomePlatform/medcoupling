...
class ProjectedCentralCircularSortOrder
{
   public:
     ...
     * @param type        plane to project on when comparing. The comparison will not work if all the points are in a
     * plane perpendicular to the plane being projected on
     */
    ProjectedCentralCircularSortOrder(const double *barycenter, const CoordType type)
        : _aIdx((type == YZ) ? 1 : 0), _bIdx((type == XY) ? 1 : 2), _a(barycenter[_aIdx]), _b(barycenter[_bIdx])
    {
    }
    ...
    bool operator()(const double *pt1, const double *pt2)
    {
        ...
        const double dy1 = pt1[_aIdx] - _a, dx1 = pt1[_bIdx] - _b, dy2 = pt2[_aIdx] - _a, dx2 = pt2[_bIdx] - _b;
        const double deno1 = std::fabs(dx1) + fabs(dy1), deno2 = std::fabs(dx2) + fabs(dy2);
...
TransformedTriangle::~TransformedTriangle()
{
    ...
    for (auto &it : _polygonA) delete[] it;
    for (auto &it : _polygonB) delete[] it;
}
 ...
 */
double
TransformedTriangle::calculateIntersectionVolume()
{
    ...
    if (_polygonA.size() > 2)
    {
        ...
        for (const auto &pt : _polygonA) LOG(3, vToStr(pt));
#endif
    ...
    if (_polygonB.size() > 2 &&
        !isTriangleInPlaneOfFacet(
            XYZ
        ))  // second condition avoids double counting in case triangle fully included in h=0 facet
    {
        LOG(2, "---- Treating polygon B ... ");
#if LOG_LEVEL > 0
        LOG(3, "  --- Final points in polygon B");
        for (const auto &pt : _polygonB) LOG(3, vToStr(pt));
#endif
 ...
 */
double
TransformedTriangle::calculateIntersectionSurface(TetraAffineTransform *tat)
{
    // check first that we are not below z - plane
    if (isTriangleBelowTetraeder())
    {
    ...
    if (_polygonA.size() > 2)
    {
        ...
        for (std::size_t i = 0; i < nbPoints; ++i) tat->reverseApply(_polygonA[i], _polygonA[i]);
 ...
 */
void
TransformedTriangle::calculateIntersectionAndProjectionPolygons()
{
#if LOG_LEVEL >= 3
    ...
    for (TriSegment seg = PQ; seg < NO_TRI_SEGMENT; seg = TriSegment(seg + 1))
    {
        ...
        for (TetraFacet facet = OYZ; facet < NO_TET_FACET; facet = TetraFacet(facet + 1))
        {
            ...
            const bool doTest = !isZero[DP_FOR_SEG_FACET_INTERSECTION[3 * facet]] &&
            ...
            if (doTest && testSegmentFacetIntersection(seg, facet))
            {
                ...
                _polygonA.push_back(ptA);
                LOG(3,
                    "Segment-facet (facet " << strTF(facet) << ", seg " << strTriS(seg) << "): " << vToStr(ptA)
                                            << " added to A");
                if (facet == XYZ)
                {
                    double *ptB = new double[3];
                    ...
                    _polygonB.push_back(ptB);
                    LOG(3,
                        "Segment-facet (facet " << strTF(facet) << ", seg " << strTriS(seg) << "): " << vToStr(ptB)
                                                << " added to B");
                }
            }
        }
        ...
        for (TetraEdge edge = OX; edge <= ZX; edge = TetraEdge(edge + 1))
        {
            ...
            if (isZero[edge_dp] && testSegmentEdgeIntersection(seg, edge))
            {
                ...
                _polygonA.push_back(ptA);
                LOG(3,
                    "Segment-edge (edge " << strTE(edge) << ", seg " << strTriS(seg) << "): " << vToStr(ptA)
                                          << " added to A");
                if (edge >= XY)
                {
                    double *ptB = new double[3];
        ...
        {
            const bool doTest = isZero[DoubleProduct(EDGES_FOR_CORNER[3 * corner])] &&
            ...
            if (doTest && testSegmentCornerIntersection(seg, corner))
            {
                ...
                _polygonA.push_back(ptA);
                LOG(3,
                    "Segment-corner (corner " << strTC(corner) << ", seg " << strTriS(seg) << "): " << vToStr(ptA)
                                              << " added to A");
                if (corner != O)
                {
                    double *ptB = new double[3];
                    ...
                    copyVector3(&COORDS_TET_CORNER[3 * corner], ptB);
                    LOG(3,
                        "Segment-corner (corner " << strTC(corner) << ", seg " << strTriS(seg) << "): " << vToStr(ptB)
                                                  << " added to B");
                }
            }
        }
        ...
        for (TetraCorner corner = X; corner < NO_TET_CORNER; corner = TetraCorner(corner + 1))
        {
            if (isZero[DP_SEGMENT_RAY_INTERSECTION[7 * (corner - 1)]] && testSegmentRayIntersection(seg, corner))
            {
                ...
                _polygonB.push_back(ptB);
                LOG(3,
                    "Segment-ray (corner " << strTC(corner) << ", seg " << strTriS(seg) << "): " << vToStr(ptB)
                                           << " added to B");
            }
        }
 ...
 */
void
TransformedTriangle::calculateIntersectionPolygon()
{
    assert(_polygonA.size() == 0);
    // avoid reallocations in push_back() by pre-allocating enough memory
    // we should never have more than 20 points
 ...
 */
double
TransformedTriangle::calculateSurfacePolygon()
{
    const std::size_t nbPoints = _polygonA.size();
    double pdt[3];
    double sum[3] = {0., 0., 0.};
 ...
 */
void
TransformedTriangle::calculatePolygonBarycenter(const IntersectionPolygon poly, double *barycenter)
{
    ...
    for (int j = 0; j < 3; ++j) barycenter[j] = 0.0;

    for (std::size_t i = 0; i < m; ++i)
    {
        const double *pt = polygon[i];
        for (int j = 0; j < 3; ++j) barycenter[j] += pt[j];
    }

    if (m != 0)
    {
        for (int j = 0; j < 3; ++j) barycenter[j] = barycenter[j] / double(m);
    }
 ...
 */
void
TransformedTriangle::sortIntersectionPolygon(const IntersectionPolygon poly, const double *barycenter)
{
 ...
 */
double
TransformedTriangle::calculateVolumeUnderPolygon(IntersectionPolygon poly, const double *barycenter)
{
    ...
    for (std::size_t i = 0; i < m; ++i)
    {
        ...
        const double factor2 = ptCurr[0] * (ptNext[1] - barycenter[1]) + ptNext[0] * (barycenter[1] - ptCurr[1]) +
 ...
 */
bool
TransformedTriangle::isTriangleInPlaneOfFacet(const TetraFacet facet) const
{
    // coordinate to check
 ...
 */
bool
TransformedTriangle::isTriangleParallelToFacet(const TetraFacet facet) const
{
    // coordinate to check
    const int coord = static_cast<int>(facet);
    return (epsilonEqual(_coords[5 * P + coord], _coords[5 * Q + coord])) &&
           (epsilonEqual(_coords[5 * P + coord], _coords[5 * R + coord]));
}
 ...
 */
int
TransformedTriangle::isTriangleInclinedToFacet(const TetraFacet facet) const
{
    // coordinate to check
    const int coord = static_cast<int>(facet);
    ...
    const double uv_xy[4] = {
        // u_x, u_y
        _coords[5 * Q + ind1] - _coords[5 * P + ind1],
        _coords[5 * Q + ind2] - _coords[5 * P + ind2],
        // v_x, v_y
        _coords[5 * R + ind1] - _coords[5 * P + ind1],
        _coords[5 * R + ind2] - _coords[5 * P + ind2]
    };
 ...
 */
bool
TransformedTriangle::isTriangleBelowTetraeder() const
{
    for (TriCorner c = P; c < NO_TRI_CORNER; c = TriCorner(c + 1))
 ...
 */
void
TransformedTriangle::dumpCoords() const
{
    std::cout << "Coords : ";
    for (int i = 0; i < 3; ++i) std::cout << vToStr(&_coords[5 * i]) << ",";
