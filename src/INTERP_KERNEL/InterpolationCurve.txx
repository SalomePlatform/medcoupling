...
InterpolationCurve<RealCurve>::interpolateMeshesInternal(
    const MyMeshType &myMeshS,
...
)
{
    ...
    if (method == "P0P0")
    {
        switch (InterpolationOptions::getIntersectionType())
        {
            case Triangulation:
            {
                intersector.reset(new CurveIntersectorP0P0<MyMeshType, MatrixType>(
                    myMeshT,
                    myMeshS,
            ...
            default:
                throw INTERP_KERNEL::Exception(
                    "For P0P0 in 1D or 2D curve only Triangulation supported for the moment !"
                );
        }
    }
    else if (method == "P0P1")
    {
        switch (InterpolationOptions::getIntersectionType())
        {
            case Triangulation:
            {
                intersector.reset(new CurveIntersectorP0P1<MyMeshType, MatrixType>(
                    myMeshT,
                    myMeshS,
                    InterpolationOptions::getPrecision(),
                    InterpolationOptions::getBoundingBoxAdjustmentAbs(),
            ...
            default:
                throw INTERP_KERNEL::Exception(
                    "For P0P1 in 1D or 2D curve only Triangulation supported for the moment !"
                );
        }
    }
    else if (method == "P1P0")
    {
        switch (InterpolationOptions::getIntersectionType())
        {
            case Triangulation:
            {
                intersector.reset(new CurveIntersectorP1P0<MyMeshType, MatrixType>(
                    myMeshT,
                    myMeshS,
                    InterpolationOptions::getPrecision(),
                    InterpolationOptions::getBoundingBoxAdjustmentAbs(),
            ...
            default:
                throw INTERP_KERNEL::Exception(
                    "For P1P0 in 1D or 2D curve only Triangulation supported for the moment !"
                );
        }
    }
    else if (method == "P1P1")
    {
        switch (InterpolationOptions::getIntersectionType())
        {
            ...
            default:
                throw INTERP_KERNEL::Exception(
                    "For P1P1 in 1D or 2D curve only Triangulation and PointLocator supported !"
                );
        }
    }
    else
        throw INTERP_KERNEL::Exception(
            "Invalid method specified ! Must be in : \"P0P0\" \"P0P1\" \"P1P0\" or \"P1P1\""
        );
    /****************************************************************/
    ...
    intersector->adjustBoundingBoxes(
        bbox, InterpolationOptions::getBoundingBoxAdjustment(), InterpolationOptions::getBoundingBoxAdjustmentAbs()
    );
