...
PolyhedronIntersectorP1P0Bary<MyMeshType, MyMatrix>::PolyhedronIntersectorP1P0Bary(
    const MyMeshType &targetMesh, const MyMeshType &srcMesh, SplittingPolicy policy
)
...
template <class MyMeshType, class MyMatrix>
void
PolyhedronIntersectorP1P0Bary<MyMeshType, MyMatrix>::releaseArrays()
{
    for (typename std::vector<SplitterTetra<MyMeshType> *>::iterator iter = _tetra.begin(); iter != _tetra.end();
         ++iter)
        delete *iter;
...
template <class MyMeshType, class MyMatrix>
void
PolyhedronIntersectorP1P0Bary<MyMeshType, MyMatrix>::intersectCells(
    ConnType tgtCell, const std::vector<ConnType> &srcCells, MyMatrix &res
)
{
    typename MyMatrix::value_type &resRow = res[tgtCell];

    ConnType nbOfNodesT = Intersector3D<MyMeshType, MyMatrix>::_target_mesh.getNumberOfNodesOfElement(
        OTT<ConnType, numPol>::indFC(tgtCell)
    );
    releaseArrays();
         ...
         iterCellS++)
    {
        ...
        for (typename std::vector<SplitterTetra<MyMeshType> *>::iterator iterTetraT = _tetra.begin();
             iterTetraT != _tetra.end();
             ++iterTetraT)
        {
            ...
            {
                interVolume += volume;
                for (int i = 0; i < SPACEDIM; ++i) total_baryCentre[i] += baryCentre[i] * volume;
            }
        }
        if (interVolume != 0)
        {
            for (int i = 0; i < SPACEDIM; ++i) total_baryCentre[i] /= interVolume;
            ...
            const ConnType *srcCellNodes =
                Intersector3D<MyMeshType, MyMatrix>::_src_mesh.getConnectivityPtr() +
                OTT<ConnType, numPol>::conn2C(
                    Intersector3D<MyMeshType, MyMatrix>::_src_mesh.getConnectivityIndexPtr()[*iterCellS]
                );
            for (int n = 0; n < 4; ++n)
            {
