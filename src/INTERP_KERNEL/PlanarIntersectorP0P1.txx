...
PlanarIntersectorP0P1<MyMeshType, MyMatrix, ConcreteP0P1Intersector>::PlanarIntersectorP0P1(
...
PlanarIntersectorP0P1<MyMeshType, MyMatrix, ConcreteP0P1Intersector>::getNumberOfRowsOfResMatrix() const
{
...
template <class MyMeshType, class MyMatrix, class ConcreteP0P1Intersector>
typename MyMeshType::MyConnType
PlanarIntersectorP0P1<MyMeshType, MyMatrix, ConcreteP0P1Intersector>::getNumberOfColsOfResMatrix() const
{
    return PlanarIntersector<MyMeshType, MyMatrix>::_meshS.getNumberOfElements();
}

/*!
 * This methods split on the fly, into triangles in order to compute dual mesh of target cell (with icellT id in target
 * mesh in C mode).
 */
template <class MyMeshType, class MyMatrix, class ConcreteP0P1Intersector>
void
PlanarIntersectorP0P1<MyMeshType, MyMatrix, ConcreteP0P1Intersector>::intersectCells(
...
)
{
    ...
    for (ConnType nodeIdT = 0; nodeIdT < nbNodesT; nodeIdT++)
    {
        ...
        std::copy(
            PlanarIntersector<MyMeshType, MyMatrix>::_coordsT + curNodeTInCmode * SPACEDIM,
            PlanarIntersector<MyMeshType, MyMatrix>::_coordsT + curNodeTInCmode * SPACEDIM + SPACEDIM,
            triangle
        );
        ...
        for (typename std::vector<ConnType>::const_iterator iter = icellsS.begin(); iter != icellsS.end(); iter++)
        {
            ...
            PlanarIntersector<MyMeshType, MyMatrix>::getRealSourceCoordinates(
                OTT<ConnType, numPol>::indFC(iS), sourceCellCoords
            );
            for (ConnType subTriT = 1; subTriT <= nbNodesT - 2; subTriT++)
            {
                std::copy(
                    PlanarIntersector<MyMeshType, MyMatrix>::_coordsT +
                        OTT<ConnType, numPol>::coo2C(startOfCellNodeConn[(nodeIdT + subTriT) % nbNodesT]) * SPACEDIM,
                    PlanarIntersector<MyMeshType, MyMatrix>::_coordsT +
                        OTT<ConnType, numPol>::coo2C(startOfCellNodeConn[(nodeIdT + subTriT) % nbNodesT]) * SPACEDIM +
                        SPACEDIM,
                    triangle + SPACEDIM
                ...
                std::copy(
                    PlanarIntersector<MyMeshType, MyMatrix>::_coordsT +
                        OTT<ConnType, numPol>::coo2C(startOfCellNodeConn[(nodeIdT + subTriT + 1) % nbNodesT]) *
                            SPACEDIM,
                    PlanarIntersector<MyMeshType, MyMatrix>::_coordsT +
                        OTT<ConnType, numPol>::coo2C(startOfCellNodeConn[(nodeIdT + subTriT + 1) % nbNodesT]) *
                            SPACEDIM +
                ...
                if (SPACEDIM == 3)
                    orientation = PlanarIntersector<MyMeshType, MyMatrix>::projectionThis(
                        &sourceCellCoordsTmp[0], quadrangle, ToConnType(sourceCellCoords.size()) / SPACEDIM, 4
                    );
                NormalizedCellType tS =
                    PlanarIntersector<MyMeshType, MyMatrix>::_meshS.getTypeOfElement(OTT<ConnType, numPol>::indFC(iS));
                double surf =
                    orientation * intersectGeometryWithQuadrangle(
                                      quadrangle, sourceCellCoordsTmp, CellModel::GetCellModel(tS).isQuadratic()
                                  );
                ...
                if (surf != 0.)
                {
                    typename MyMatrix::value_type::const_iterator iterRes =
                        resRow.find(OTT<ConnType, numPol>::indFC(iS));
