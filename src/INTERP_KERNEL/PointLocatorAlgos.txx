...
class PointLocatorAlgos : public GenericPointLocatorAlgos
{
   ...
   public:
    PointLocatorAlgos(const MyMeshType &mesh) : _mesh(mesh)
    {
        ...
        for (ConnType i = 0; i < nelem; i++)
        {
            ...
            for (ConnType index = conn_index[i]; index < conn_index[i + 1]; index++)
            {
                ...
                const double *coordelem =
                    coords + OTT<ConnType, numPol>::ind2C(conn[OTT<ConnType, numPol>::ind2C(index)]) * SPACEDIM;

                // the bounding box is updated by checking wheher the node is at the min/max in exach dimension
                for (int idim = 0; idim < SPACEDIM; idim++)
                {
                    _bb[2 * (i * SPACEDIM + idim)] = (coordelem[idim] < _bb[2 * (i * SPACEDIM + idim)])
                                                         ? coordelem[idim]
                                                         : _bb[2 * (i * SPACEDIM + idim)];
                    _bb[2 * (i * SPACEDIM + idim) + 1] = (coordelem[idim] > _bb[2 * (i * SPACEDIM + idim) + 1])
                                                             ? coordelem[idim]
                                                             : _bb[2 * (i * SPACEDIM + idim) + 1];
                }
            }
        }
    ...
    static bool isElementContainsPointAlg2DSimple(
        const double *ptToTest, const double *cellPts, mcIdType nbEdges, double eps
    )
    {
    ...
    static bool isElementContainsPointAlgo2DSimple2(
        const double *ptToTest,
        NormalizedCellType type,
        const double *coords,
        const typename MyMeshType::MyConnType *conn_elem,
        typename MyMeshType::MyConnType conn_elem_sz,
        double eps
    )
    {
        const int SPACEDIM = MyMeshType::MY_SPACEDIM;
    ...
    static bool isElementContainsPointAlgo2DPolygon(
        const double *ptToTest,
        NormalizedCellType type,
        const double *coords,
        const typename MyMeshType::MyConnType *conn_elem,
    ...
    )
    {
        ...
        double a(0.), b(0.), c(0.);
        a = pol->normalizeMe(b, c);
        n->applySimilarity(b, c, a);
    ...
    static bool isElementContainsPointAlg3D(
        const double *ptToTest,
        const typename MyMeshType::MyConnType *conn_elem,
        typename MyMeshType::MyConnType conn_elem_sz,
        const double *coords,
        const CellModel &cmType,
        double eps
    )
    {
        const int SPACEDIM = MyMeshType::MY_SPACEDIM;
        ...
        for (int iface = 0; iface < nbfaces; iface++)
        {
            ...
            {
                const double *AA =
                    coords + SPACEDIM * (OTT<ConnType, numPol>::coo2C(connOfSon[(0 + i_permutation) % nb_face_nodes]));
                const double *BB =
        ...
        double ptToTestNorm[3] = {
            (ptToTest[0] - centerPt[0]) / normalizeFact,
     ...
     */
    static bool isElementContainsPoint(
        const double *ptToTest,
        NormalizedCellType type,
        const double *coords,
        const typename MyMeshType::MyConnType *conn_elem,
...
class PointLocatorInSimplex : public PointLocatorAlgos<MyMeshType>
{
   ...
   public:
    PointLocatorInSimplex(const MyMeshType &mesh) : PointLocatorAlgos<MyMeshType>(mesh), _mesh(mesh) {}
    ...
    virtual std::list<typename MyMeshType::MyConnType> locates(const double *x, double eps)
    {
        ...
        for (; eIt != candidates.end(); ++eIt)
        {
            ...
            if (cell.isSimplex())
            {
                for (int n = 0; n < conn_elem_sz; ++n) simplexNodes.push_back(conn_elem[n]);
            }
            else
            {
                            ...
                            if (checkedSonSimplex.insert(sonSimplex).second)
                            {
                                for (unsigned cn = 0; cn < conn_elem_sz; ++cn)
                                    if (!sonNodesSet.count(conn_elem[cn]))
                                    {
                                        ...
                                        if (this->isElementContainsPoint(
                                                x, simlexType, coords, &simplexConn[0], simplexConn.size(), eps
                                            ))
                                        {
                                            simplexNodes.insert(
                                                simplexNodes.end(), simplexConn.begin(), simplexConn.end()
                                            );
                                            return simplexNodes;
                                        }
                                    }
