...
template <class MyMeshType>
void
SplitterTetra<MyMeshType>::splitIntoDualCells(SplitterTetra<MyMeshType> **output)
{
    double tmp[12];
 ...
 * single TETRA4 cell given by \a tetraCorners (of length 4) and \a nodesId (of length 4 too). \a nodedIds is given only
 * to establish if a partial computation of a triangle has already been performed (to increase performance).
 *
...
SplitterTetra<MyMeshType>::SplitterTetra(
    const MyMeshType &srcMesh, const double **tetraCorners, const typename MyMeshType::MyConnType *nodesId
)
    : _t(0), _src_mesh(srcMesh)
{
    std::copy(nodesId, nodesId + 4, _conn);
    _coords[0] = tetraCorners[0][0];
    _coords[1] = tetraCorners[0][1];
    _coords[2] = tetraCorners[0][2];
    _coords[3] = tetraCorners[1][0];
    _coords[4] = tetraCorners[1][1];
    _coords[5] = tetraCorners[1][2];
    _coords[6] = tetraCorners[2][0];
    _coords[7] = tetraCorners[2][1];
    _coords[8] = tetraCorners[2][2];
    _coords[9] = tetraCorners[3][0];
    _coords[10] = tetraCorners[3][1];
    _coords[11] = tetraCorners[3][2];
 ...
 * \param [in] tetraCorners  array 4*3 doubles containing corners of input tetrahedron
 * (P0X,P0Y,P0Y,P1X,P1Y,P1Z,P2X,P2Y,P2Z,P3X,P3Y,P3Z).
 */
template <class MyMeshType>
SplitterTetra<MyMeshType>::SplitterTetra(const MyMeshType &srcMesh, const double tetraCorners[12], const ConnType *conn)
    : _t(0), _src_mesh(srcMesh)
{
    if (!conn)
    {
    ...
    _coords[0] = tetraCorners[0];
    _coords[1] = tetraCorners[1];
    _coords[2] = tetraCorners[2];
    _coords[3] = tetraCorners[3];
    _coords[4] = tetraCorners[4];
    _coords[5] = tetraCorners[5];
    _coords[6] = tetraCorners[6];
    _coords[7] = tetraCorners[7];
    _coords[8] = tetraCorners[8];
    _coords[9] = tetraCorners[9];
    _coords[10] = tetraCorners[10];
    _coords[11] = tetraCorners[11];
    // create the affine transform
    _t = new TetraAffineTransform(_coords);
}
...
template <class MyMeshType>
void
SplitterTetra<MyMeshType>::clearVolumesCache()
{
    _volumes.clear();
}
...
template <class MyMeshType>
void
SplitterTetra<MyMeshType>::splitMySelfForDual(double *output, int i, typename MyMeshType::MyConnType &nodeId)
{
    double *tmp[4];
    int offset = i / 6;
    nodeId = _conn[offset];
    tmp[0] = _coords + 3 * offset;
    tmp[1] = _coords + ((offset + 1) % 4) * 3;
    tmp[2] = _coords + ((offset + 2) % 4) * 3;
    tmp[3] = _coords + ((offset + 3) % 4) * 3;
    int caseToTreat = i % 6;
    ...
    double pt0[3];
    pt0[0] = (tmp[curTab[case2]][0] + tmp[0][0]) / 2.;
    pt0[1] = (tmp[curTab[case2]][1] + tmp[0][1]) / 2.;
    pt0[2] = (tmp[curTab[case2]][2] + tmp[0][2]) / 2.;
    double pt1[3];
    pt1[0] = (tmp[0][0] + tmp[curTab[0]][0] + tmp[curTab[1]][0]) / 3.;
    pt1[1] = (tmp[0][1] + tmp[curTab[0]][1] + tmp[curTab[1]][1]) / 3.;
    pt1[2] = (tmp[0][2] + tmp[curTab[0]][2] + tmp[curTab[1]][2]) / 3.;
    double pt2[3];
    pt2[0] = (tmp[0][0] + tmp[1][0] + tmp[2][0] + tmp[3][0]) / 4.;
    pt2[1] = (tmp[0][1] + tmp[1][1] + tmp[2][1] + tmp[3][1]) / 4.;
    pt2[2] = (tmp[0][2] + tmp[1][2] + tmp[2][2] + tmp[3][2]) / 4.;
...
template <class MyMeshType>
double
SplitterTetra<MyMeshType>::intersectSourceCell(typename MyMeshType::MyConnType element, double *baryCentre)
{
    ...
    ConnType nbOfNodes4Type = cellModelCell.isDynamic()
                                  ? _src_mesh.getNumberOfNodesOfElement(OTT<ConnType, numPol>::indFC(element))
                                  : cellModelCell.getNumberOfNodes();
    // halfspace filtering
    ...
    if (!isTargetOutside)
    {
        ...
        const ConnType *rawCellConn = _src_mesh.getConnectivityPtr() +
                                      OTT<ConnType, numPol>::conn2C(_src_mesh.getConnectivityIndexPtr()[element]);
        const ConnType rawNbCellNodes =
            _src_mesh.getConnectivityIndexPtr()[element + 1] - _src_mesh.getConnectivityIndexPtr()[element];
        ...
        for (unsigned ii = 0; ii < nbOfSons; ++ii)
        {
            ...
            if (cellModelCell.isDynamic())
            {
                ...
                nbFaceNodes =
                    cellModelCell.fillSonCellNodalConnectivity2(ii, rawCellConn, rawNbCellNodes, faceNodes, faceType);
                for (ConnType i = 0; i < nbFaceNodes; ++i) faceNodes[i] = OTT<ConnType, numPol>::coo2C(faceNodes[i]);
            }
            else
            {
            ...
            switch (faceType)
            {
                    ...
                    for (ConnType iTri = 0; iTri < nbTria; ++iTri)
                    {
                        ...
                        if (_volumes.find(key) == _volumes.end())
                        {
                            TransformedTriangle tri(
                                _nodes[faceNodes[0]], _nodes[faceNodes[1 + iTri]], _nodes[faceNodes[2 + iTri]]
                            );
                            calculateVolume(tri, key);
                            totalVolume += _volumes[key];
                        }
                ...
                default:
                    std::cout << "+++ Error : Only elements with triangular and quadratilateral faces are supported at "
                                 "the moment."
                              << std::endl;
                    assert(false);
            }
...
template <class MyMeshType>
double
SplitterTetra<MyMeshType>::CalculateIntersectionSurfaceOfCoplanarTriangles(
    const double *const planeNormal,
...
)
{
    ...
    double surface = 0.;
    if (nb_inter > 3)
        inter2 = reconstruct_polygon(inter2);
    if (nb_inter > 0)
    {
        ...
        if (maxNormal == 0)
        {
            ...
            for (i = 0; i < nb_inter; i++)
            {
                ...
                inter3[3 * i] =
                    invNX * (planeConstant - planeNormal[1] * inter3[3 * i + 1] - planeNormal[2] * inter3[3 * i + 2]);
            }
        }
        else if (maxNormal == 1)
        {
            ...
            for (i = 0; i < nb_inter; i++)
            {
                ...
                inter3[3 * i + 2] = inter2[2 * i + 1];
                inter3[3 * i + 1] =
                    invNY * (planeConstant - planeNormal[0] * inter3[3 * i] - planeNormal[2] * inter3[3 * i + 2]);
            }
        }
        else
        {
            ...
            for (i = 0; i < nb_inter; i++)
            {
                ...
                inter3[3 * i + 2] =
                    invNZ * (planeConstant - planeNormal[0] * inter3[3 * i] - planeNormal[1] * inter3[3 * i + 1]);
            }
        }
...
template <class MyMeshType>
bool
SplitterTetra<MyMeshType>::IsFacesCoplanar(
    const double *const planeNormal,
    const double planeConstant,
...
template <class MyMeshType>
double
SplitterTetra<MyMeshType>::intersectSourceFace(
    ...
    if (!isTargetStrictlyOutside)
    {
        if (isTargetOutside)
        {
            ...
            for (int iTetraFace = 0; iTetraFace < 4; ++iTetraFace)
            {
                ...
                TriangleFaceKey key = TriangleFaceKey(
                    _conn[tetraFaceNodesConn[0]], _conn[tetraFaceNodesConn[1]], _conn[tetraFaceNodesConn[2]]
                );
                if (listOfTetraFacesTreated.find(key) == listOfTetraFacesTreated.end())
                {
                    ...
                    if (IsFacesCoplanar(planeNormal, planeConstant, polyCoords, precision))
                    {
                        ...
                        {
                            double volume = CalculateIntersectionSurfaceOfCoplanarTriangles(
                                planeNormal,
                                planeConstant,
                                polyCoords[0],
                                polyCoords[1 + iTri],
            ...
            switch (polyType)
            {
                ...
                case NORM_POLYGON:
                {
                    ...
                    for (ConnType iTri = 0; iTri < nbrPolyTri; ++iTri)
                    {
                        ...
                        {
                            TransformedTriangle tri(
                                _nodes[polyNodes[0]], _nodes[polyNodes[1 + iTri]], _nodes[polyNodes[2 + iTri]]
                            );
                            calculateSurface(tri, key);
                            totalSurface += _volumes[key];
                        }
...
template <class MyMeshType>
double
SplitterTetra<MyMeshType>::intersectTetra(const double **tetraCorners)
{
    //{ could be done on outside?
    // check if we have planar tetra element
    if (_t->determinant() == 0.0)
...
SplitterTetra2<MyMeshTypeT, MyMeshTypeS>::SplitterTetra2(
    const MyMeshTypeT &targetMesh, const MyMeshTypeS &srcMesh, SplittingPolicy policy
)
...
template <class MyMeshTypeT, class MyMeshTypeS>
void
SplitterTetra2<MyMeshTypeT, MyMeshTypeS>::releaseArrays()
{
...
template <class MyMeshTypeT, class MyMeshTypeS>
void
SplitterTetra2<MyMeshTypeT, MyMeshTypeS>::splitTargetCell2(
    typename MyMeshTypeT::MyConnType targetCell, typename std::vector<SplitterTetra<MyMeshTypeS> *> &tetra
)
{
    ...
    SplitIntoTetras(
        ...
        gt,
        cellConn,
        refConn + _target_mesh.getConnectivityIndexPtr()[targetCell + 1],
        coords,
        tetrasNodalConn,
        addCoords
    );
    std::size_t nbTetras(tetrasNodalConn.size() / 4);
    tetra.resize(nbTetras);
    double tmp[12];
...
template <class MyMeshTypeT, class MyMeshTypeS>
void
SplitterTetra2<MyMeshTypeT, MyMeshTypeS>::splitTargetCell(
 ...
 *
 * @param subZone  the local node numbers corresponding to the hexahedron corners - these are mapped onto {0,..,7}.
 * Providing this allows the splitting to be reused on the subzones of the GENERAL_* types of splitting
 */
template <class MyMeshTypeT, class MyMeshTypeS>
void
SplitterTetra2<MyMeshTypeT, MyMeshTypeS>::fiveSplit(
    const int *const subZone, typename std::vector<SplitterTetra<MyMeshTypeS> *> &tetra
)
{
    // create tetrahedra
    for (int i = 0; i < 5; ++i)
...
template <class MyMeshTypeT, class MyMeshTypeS>
void
SplitterTetra2<MyMeshTypeT, MyMeshTypeS>::sixSplit(
    const int *const subZone, typename std::vector<SplitterTetra<MyMeshTypeS> *> &tetra
)
{
    sixSplitGen(
        subZone,
        tetra,
        [](SplitterTetra2 &obj, typename MyMeshTypeS::MyConnType &conn, const double *&coords)
        {
            typename MyMeshTypeS::MyConnType realConn;
...
template <class MyMeshTypeT, class MyMeshTypeS>
void
SplitterTetra2<MyMeshTypeT, MyMeshTypeS>::sixSplitGen(
    const int *const subZone,
    typename std::vector<SplitterTetra<MyMeshTypeS> *> &tetra,
    std::function<void(SplitterTetra2 &, typename MyMeshTypeS::MyConnType &, const double *&)> func
)
{
    for (int i = 0; i < 6; ++i)
    {
...
template <class MyMeshTypeT, class MyMeshTypeS>
void
SplitterTetra2<MyMeshTypeT, MyMeshTypeS>::calculateGeneral24Tetra(
    typename std::vector<SplitterTetra<MyMeshTypeS> *> &tetra
)
{
    calculateGeneral24TetraGen(
        tetra,
        [](SplitterTetra2 &obj, typename MyMeshTypeS::MyConnType conn[4], const double *nodes[4])
        {
            typename MyMeshTypeS::MyConnType realConn;
            nodes[2] = obj.getCoordsOfSubNode2(conn[2], realConn);
            conn[2] = realConn;
            nodes[3] = obj.getCoordsOfSubNode2(conn[3], realConn);
            conn[3] = realConn;
        }
    );
}
...
template <class MyMeshTypeT, class MyMeshTypeS>
void
SplitterTetra2<MyMeshTypeT, MyMeshTypeS>::calculateGeneral24TetraOld(
    typename std::vector<SplitterTetra<MyMeshTypeS> *> &tetra
)
{
    calculateGeneral24TetraGen(
...
template <class MyMeshTypeT, class MyMeshTypeS>
void
SplitterTetra2<MyMeshTypeT, MyMeshTypeS>::calculateGeneral24TetraGen(
    typename std::vector<SplitterTetra<MyMeshTypeS> *> &tetra,
    std::function<void(SplitterTetra2 &, typename MyMeshTypeS::MyConnType[4], const double *[4])> func
)
{
...
template <class MyMeshTypeT, class MyMeshTypeS>
void
SplitterTetra2<MyMeshTypeT, MyMeshTypeS>::calculateGeneral48Tetra(
    typename std::vector<SplitterTetra<MyMeshTypeS> *> &tetra
)
{
    for (int i = 0; i < 8; ++i)
...
template <class MyMeshTypeT, class MyMeshTypeS>
void
SplitterTetra2<MyMeshTypeT, MyMeshTypeS>::splitPyram5(typename std::vector<SplitterTetra<MyMeshTypeS> *> &tetra)
{
    static const int SPLIT_PYPA5[2][4] = {{0, 1, 2, 4}, {0, 2, 3, 4}};
    ...
    for (int i = 0; i < 2; ++i)
    {
        for (int j = 0; j < 4; ++j) nodes[j] = getCoordsOfSubNode2(SPLIT_PYPA5[i][j], conn[j]);
        SplitterTetra<MyMeshTypeS> *t = new SplitterTetra<MyMeshTypeS>(_src_mesh, nodes, conn);
        tetra.push_back(t);
    }
}
...
template <class MyMeshTypeT, class MyMeshTypeS>
void
SplitterTetra2<MyMeshTypeT, MyMeshTypeS>::splitConvex(
    typename MyMeshTypeT::MyConnType targetCell, typename std::vector<SplitterTetra<MyMeshTypeS> *> &tetra
)
{
    ...
    const CellModel &cellModelCell = CellModel::GetCellModel(normCellType);
    ConnType nbOfCellNodes = cellModelCell.isDynamic()
                                 ? _target_mesh.getNumberOfNodesOfElement(OTT<ConnType, numPol>::indFC(targetCell))
                                 : cellModelCell.getNumberOfNodes();

    // get nb of cell sons (faces)
    const ConnType *rawCellConn = _target_mesh.getConnectivityPtr() +
                                  OTT<ConnType, numPol>::conn2C(_target_mesh.getConnectivityIndexPtr()[targetCell]);
    const ConnType rawNbCellNodes =
        _target_mesh.getConnectivityIndexPtr()[targetCell + 1] - _target_mesh.getConnectivityIndexPtr()[targetCell];
    unsigned nbOfSons = cellModelCell.getNumberOfSons2(rawCellConn, rawNbCellNodes);
...
template <class MyMeshTypeT, class MyMeshTypeS>
void
SplitterTetra2<MyMeshTypeT, MyMeshTypeS>::calculateSubNodes(
    const MyMeshTypeT &targetMesh, typename MyMeshTypeT::MyConnType targetCell
)
{
    // retrieve real mesh nodes

    typename MyMeshTypeT::MyConnType nbOfNodesT =
        static_cast<typename MyMeshTypeT::MyConnType>(_node_ids.size());  // Issue 0020634. _node_ids.resize(8);
