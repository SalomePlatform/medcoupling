...
class INTERPKERNEL_EXPORT TranslationRotationMatrix
{
   public:
    ...
    {
        unsigned i;
        for (i = 0; i < TRANSL_SIZE; i++) _translation_coeffs[i] = 0.;
        for (i = 0; i < ROT_SIZE; i++) _rotation_coeffs[i] = i % 4 ? 0. : 1.;
    }
    ...
    void multiply(const TranslationRotationMatrix &A)
    {
        ...
        for (int i = 0; i < 3; i++) result._rotation_coeffs[i * 4] = 0.0;
        // multiplying
        for (int i = 0; i < 3; i++)
            for (int j = 0; j < 3; j++)
                for (int k = 0; k < 3; k++)
        ...
        for (int i = 0; i < 9; i++) _rotation_coeffs[i] = result._rotation_coeffs[i];
    }
    ...
    void rotate_vector(double *P)
    {
        ...
        for (int i = 0; i < 3; i++)
            for (int j = 0; j < 3; j++) temp[i] += _rotation_coeffs[3 * i + j] * P[j];
    ...
    /**
     * Fills in rotation_matrix so that the triangle PP1, PP2, PP3 is in the plane Oxy, with PP1 at the origin and PP2
     * on Ox.
     */
    static void Rotate3DTriangle(
        const double *PP1, const double *PP2, const double *PP3, TranslationRotationMatrix &rotation_matrix
    )
    {
        double P1w[3];
        double P2w[3];
