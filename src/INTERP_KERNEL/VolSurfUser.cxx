...
{
    double prec = 1.0e-14;
    double T[2][3] = {{p1[0] - p2[0], p1[1] - p2[1], p1[2] - p2[2]}, {p3[0] - p2[0], p3[1] - p2[1], p3[2] - p2[2]}};
    double N[3] = {
...
)
{
    ...
    if (norm == 0.)
        return (pt[0] - pt0Seg2[0]) * (pt[0] - pt0Seg2[0]) +
               (pt[1] - pt0Seg2[1]) * (pt[1] - pt0Seg2[1]);  // return std::numeric_limits<double>::max();
    dx /= norm;
    dy /= norm;
    double dx2 = pt[0] - pt0Seg2[0], dy2 = pt[1] - pt0Seg2[1];
    double dotP = (dx2 * dx + dy2 * dy);
    if (dotP < 0. || dotP > norm)
        return dotP < 0. ? (pt[0] - pt0Seg2[0]) * (pt[0] - pt0Seg2[0]) + (pt[1] - pt0Seg2[1]) * (pt[1] - pt0Seg2[1])
                         : (pt[0] - pt1Seg2[0]) * (pt[0] - pt1Seg2[0]) + (pt[1] - pt1Seg2[1]) * (pt[1] - pt1Seg2[1]);
    nbOfHint++;
 ...
 */
double
DistanceFromPtToSegInSpaceDim3(const double *pt, const double *pt0Seg2, const double *pt1Seg2)
{
    double v[3], w[3];
    for (int i = 0; i < 3; i++)
    {
    ...
    if (c2 <= c1)
    {
        for (int i = 0; i < 3; i++) w[i] = pt[i] - pt1Seg2[i];
        return norm<3>(w);
    }
    double b = c1 / c2;
    for (int i = 0; i < 3; i++) w[i] = pt0Seg2[i] + b * v[i] - pt[i];
    return norm<3>(w);
}
 ...
 */
inline double
_HelperDistancePtToTri3D_1(const double aXX, const double bX, const double c)
{
    if (bX >= 0)
        return c;
    if (-bX >= aXX)
 ...
 */
inline double
_HelperDistancePtToTri3D_2(
    const double a01, const double aXX, const double aYY, const double bX, const double bY, const double c
)
{
            ...
            else  // region 0
            {
                ...
                if (fabs(det) < 1.0e-12)
                {
                    ...
                    double distance = std::min(
                        DistanceFromPtToSegInSpaceDim3(pt, pt0Tri3, pt1Tri3),
...
)
{
    ...
    if (nbOfEdges < 3)
        throw INTERP_KERNEL::Exception(
            "DistanceFromPtToPolygonInSpaceDim3 : trying to compute a distance to a polygon containing less than 3 "
            "edges !"
        );
    double baryOfNodes[3] = {0., 0., 0.};
    for (std::size_t i = 0; i < nbOfEdges; i++)
    {
        baryOfNodes[0] += coords[3 * connOfPolygonBg[i]];
        baryOfNodes[1] += coords[3 * connOfPolygonBg[i] + 1];
        baryOfNodes[2] += coords[3 * connOfPolygonBg[i] + 2];
    }
    std::transform(
        baryOfNodes,
        baryOfNodes + 3,
        baryOfNodes,
        std::bind(std::multiplies<double>(), std::placeholders::_1, 1. / ((double)nbOfEdges))
    );
    double matrix[12];
    if (!ComputeRotTranslationMatrixToPut3PointsOnOXY(
            coords + 3 * connOfPolygonBg[0], coords + 3 * connOfPolygonBg[1], baryOfNodes, matrix
        ))
        return std::numeric_limits<double>::max();
    INTERP_KERNEL::AutoPtr<double> ptXY = new double[2 * nbOfEdges];
    ptXY[0] = 0.;
    ptXY[1] = 0.;
    ptXY[2] = matrix[0] * coords[3 * connOfPolygonBg[1]] + matrix[1] * coords[3 * connOfPolygonBg[1] + 1] +
              matrix[2] * coords[3 * connOfPolygonBg[1] + 2] + matrix[3];
    ptXY[3] = 0.;
    for (std::size_t i = 2; i < nbOfEdges; i++)
    {
        ptXY[2 * i] = matrix[0] * coords[3 * connOfPolygonBg[i]] + matrix[1] * coords[3 * connOfPolygonBg[i] + 1] +
                      matrix[2] * coords[3 * connOfPolygonBg[i] + 2] + matrix[3];
        ptXY[2 * i + 1] = matrix[4] * coords[3 * connOfPolygonBg[i]] + matrix[5] * coords[3 * connOfPolygonBg[i] + 1] +
                          matrix[6] * coords[3 * connOfPolygonBg[i] + 2] + matrix[7];
    }
    double xy[2] = {
        matrix[0] * pt[0] + matrix[1] * pt[1] + matrix[2] * pt[2] + matrix[3],
 ...
 */
bool
ComputeRotTranslationMatrixToPut3PointsOnOXY(const double *p0, const double *p1, const double *p2, double *matrix)
{
    double norm =
        sqrt((p1[0] - p0[0]) * (p1[0] - p0[0]) + (p1[1] - p0[1]) * (p1[1] - p0[1]) + (p1[2] - p0[2]) * (p1[2] - p0[2]));
    ...
    double r0[9] = {
        c, -z * s, y * s, z * s, y * y * (1 - c) + c, y * z * (1 - c), -y * s, z * y * (1 - c), z * z * (1 - c) + c
    };
    // 2nd rotation matrix
    double x = p2[0] - p0[0];
    y = p2[1] - p0[1];
    ...
    matrix[5] = c * r0[4] - s * r0[7];
    matrix[6] = c * r0[5] - s * r0[8];
    matrix[8] = s * r0[3] + c * r0[6];
    matrix[9] = s * r0[4] + c * r0[7];
    matrix[10] = s * r0[5] + c * r0[8];
