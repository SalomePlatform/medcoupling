...
PlanarIntersector<MyMeshType, MyMatrix>::PlanarIntersector(
    ...
    : _meshT(meshT),
      ...
      _max_distance_3Dsurf_intersect(md3DSurf),
      _min_dot_btw_3Dsurf_intersect(minDot3DSurf),
...
/*!
  ...
  The vector returned is of dimension 6*nb_elems with bounding boxes stored as xmin1, xmax1, ymin1, ymax1, zmin1, zmax1,
  xmin2, xmax2, ymin2,... The returned pointer must be deleted by the calling code.
...
template <class MyMeshType, class MyMatrix>
void
PlanarIntersector<MyMeshType, MyMatrix>::createBoundingBoxes(const MyMeshType &mesh, std::vector<double> &bbox)
{
...
template <class MyMeshType, class MyMatrix>
void
PlanarIntersector<MyMeshType, MyMatrix>::getElemBB(double *bb, const MyMeshType &mesh, ConnType iP, ConnType nb_nodes)
{
    const double *coords = mesh.getCoordinatesPtr();
    const ConnType *conn_index = mesh.getConnectivityIndexPtr();
    const ConnType *conn = mesh.getConnectivityPtr();
...
template <class MyMeshType, class MyMatrix>
void
PlanarIntersector<MyMeshType, MyMatrix>::getRealTargetCoordinates(ConnType icellT, std::vector<double> &coordsT)
{
    ConnType nbNodesT =
...
template <class MyMeshType, class MyMatrix>
void
PlanarIntersector<MyMeshType, MyMatrix>::getRealSourceCoordinates(ConnType icellS, std::vector<double> &coordsS)
{
    ConnType nbNodesS =
...
template <class MyMeshType, class MyMatrix>
void
PlanarIntersector<MyMeshType, MyMatrix>::getRealTargetCoordinatesPermute(
    ConnType icellT, ConnType offset, std::vector<double> &coordsT
)
{
    ConnType nbNodesT =
        _connIndexT[OTT<ConnType, numPol>::ind2C(icellT) + 1] - _connIndexT[OTT<ConnType, numPol>::ind2C(icellT)];
    ...
    for (ConnType iTTmp = 0; iTTmp < nbNodesT; iTTmp++)
    {
        ...
        for (int idim = 0; idim < SPACEDIM; idim++)
            coordsT[SPACEDIM * iTTmp + idim] = _coordsT
                [SPACEDIM * OTT<ConnType, numPol>::coo2C(
                                _connectT[OTT<ConnType, numPol>::
                                              conn2C(_connIndexT[OTT<ConnType, numPol>::ind2C(icellT)] + iT)]
                            ) +
...
template <class MyMeshType, class MyMatrix>
void
PlanarIntersector<MyMeshType, MyMatrix>::getRealSourceCoordinatesPermute(
    ConnType icellS, ConnType offset, std::vector<double> &coordsS
)
{
    ConnType nbNodesS =
        _connIndexS[OTT<ConnType, numPol>::ind2C(icellS) + 1] - _connIndexS[OTT<ConnType, numPol>::ind2C(icellS)];
    ...
    for (ConnType iSTmp = 0; iSTmp < nbNodesS; iSTmp++)
    {
        ...
        for (int idim = 0; idim < SPACEDIM; idim++)
            coordsS[SPACEDIM * iSTmp + idim] = _coordsS
                [SPACEDIM * OTT<ConnType, numPol>::coo2C(
                                _connectS[OTT<ConnType, numPol>::
                                              conn2C(_connIndexS[OTT<ConnType, numPol>::ind2C(icellS)] + iS)]
                            ) +
...
)
{
    ...
    if (_print_level >= 3)
    {
        ...
        {
            for (int idim = 0; idim < SPACEDIM; idim++) std::cout << coordsT[SPACEDIM * iT + idim] << " ";
            std::cout << std::endl;
        }
        std::cout << std::endl << "icellS= " << icellS << ", nb nodes S= " << nbNodesS << std::endl;
        for (int iS = 0; iS < nbNodesS; iS++)
        {
            for (int idim = 0; idim < SPACEDIM; idim++) std::cout << coordsS[SPACEDIM * iS + idim] << " ";
            std::cout << std::endl;
        }
    }
}
...
template <class MyMeshType, class MyMatrix>
double
PlanarIntersector<MyMeshType, MyMatrix>::getValueRegardingOption(double val) const
{
    if (_orientation == 0)
        return val;
    if (_orientation == 2)
...
PlanarIntersector<MyMeshType, MyMatrix>::projectionThis(
    double *Coords_A, double *Coords_B, ConnType nb_NodesA, ConnType nb_NodesB
)
{
    return Projection(
        Coords_A,
...
template <class MyMeshType, class MyMatrix>
int
PlanarIntersector<MyMeshType, MyMatrix>::Projection(
    double *Coords_A,
    double *Coords_B,
    ConnType nb_NodesA,
    ConnType nb_NodesB,
...
)
{
    ...
    if (md3DSurf > 0.)
    {
        ...
        {
            coords_GA[i] = 0.;
            for (int j = 0; j < nb_NodesA; j++) coords_GA[i] += Coords_A[3 * j + i];
    ...
    if (i_A2 < nb_NodesA && i_B2 < nb_NodesB)
    {
        ...
        for (ConnType i_A = 0; i_A < nb_NodesA; i_A++)
        {
            ...
            for (int idim = 0; idim < SPACEDIM; idim++) Coords_A[SPACEDIM * i_A + idim] -= proj * linear_comb[idim];
        }
        for (ConnType i_B = 0; i_B < nb_NodesB; i_B++)
        {
            proj = dotprod<SPACEDIM>(Coords_B + SPACEDIM * i_B, linear_comb);
            for (int idim = 0; idim < SPACEDIM; idim++) Coords_B[SPACEDIM * i_B + idim] -= proj * linear_comb[idim];
        }
    ...
    else
    {
        ...
        std::cout << " distance2<SPACEDIM>(Coords_A,&Coords_A[i_A1])= "
                  << distance2<SPACEDIM>(Coords_A, &Coords_A[i_A1]) << std::endl;
        std::cout << "abs(normal_A) = " << fabs(normal_A[0]) << " ; " << fabs(normal_A[1]) << " ; " << fabs(normal_A[2])
                  << std::endl;
        std::cout << " i_B1= " << i_B1 << " i_B2= " << i_B2 << std::endl;
        std::cout << " distance2<SPACEDIM>(&Coords_B[0],&Coords_B[i_B1])= "
                  << distance2<SPACEDIM>(Coords_B, Coords_B + i_B1) << std::endl;
...
template <class MyMeshType, class MyMatrix>
void
PlanarIntersector<MyMeshType, MyMatrix>::Rotate3DTriangle(
    double *PP1, double *PP2, double *PP3, TranslationRotationMatrix &rotation_matrix
)
{
    // initializes
