...
{
TRI_INTER_TEMPLATE
TRI_INTERSECTOR::TriangulationIntersector(
    ...
    : InterpType<MyMeshType, MyMatrix, TRI_INTERSECTOR>(
          meshT, meshS, DimCaracteristic, Precision, md3DSurf, minDot3DSurf, MedianPlane, true, orientation, PrintLevel
      )
{
    if (PlanarIntersector<MyMeshType, MyMatrix>::_print_level >= 1)
    {
        ...
        if (SPACEDIM == 3)
            std::cout << "_do_rotate = true" << std::endl;
    }
}

TRI_INTER_TEMPLATE
double
TRI_INTERSECTOR::intersectGeometry(ConnType icellT, ConnType icellS, ConnType nbNodesT, ConnType nbNodesS)
{
    double result = 0.;
    int orientation = 1;
    ...
    for (ConnType iT = 1; iT < nbNodesT - 1; iT++)
    {
        for (ConnType iS = 1; iS < nbNodesS - 1; iS++)
        {
            ...
            INTERP_KERNEL::intersec_de_triangle(
                &CoordsT[0],
                &CoordsT[SPACEDIM * iT],
                &CoordsT[SPACEDIM * (iT + 1)],
                &CoordsS[0],
                &CoordsS[SPACEDIM * iS],
                &CoordsS[SPACEDIM * (iS + 1)],
                inter,
                PlanarIntersector<MyMeshType, MyMatrix>::_dim_caracteristic,
                PlanarIntersector<MyMeshType, MyMatrix>::_precision
            );
            ConnType nb_inter = ToConnType(inter.size()) / 2;
            if (nb_inter > 3)
                inter = reconstruct_polygon(inter);
            ...
            if (PlanarIntersector<MyMeshType, MyMatrix>::_print_level >= 3)
            {
                ...
                {
                    for (int idim = 0; idim < 2; idim++) std::cout << inter[2 * i + idim] << " ";
                    std::cout << std::endl;
                }
            }
        }
...
TRI_INTER_TEMPLATE
double
TRI_INTERSECTOR::intersectGeometryWithQuadrangle(
...
)
{
    ...
    for (ConnType iT = 1; iT < 3; iT++)
    {
        for (ConnType iS = 1; iS < nbNodesS - 1; iS++)
        {
            ...
            INTERP_KERNEL::intersec_de_triangle(
                quadrangle,
                &quadrangle[SPACEDIM * iT],
                &quadrangle[SPACEDIM * (iT + 1)],
                &sourceCoords[0],
                &sourceCoords[SPACEDIM * iS],
                &sourceCoords[SPACEDIM * (iS + 1)],
                inter,
                PlanarIntersector<MyMeshType, MyMatrix>::_dim_caracteristic,
                PlanarIntersector<MyMeshType, MyMatrix>::_precision
            );
...
TRI_INTER_TEMPLATE
double
TRI_INTERSECTOR::intersectGeometryGeneral(
...
)
{
    ...
    for (ConnType iT = 1; iT < nbNodesT - 1; iT++)
    {
        for (ConnType iS = 1; iS < nbNodesS - 1; iS++)
        {
            ...
            INTERP_KERNEL::intersec_de_triangle(
                &targetCoords[0],
                &targetCoords[SPACEDIM * iT],
                &targetCoords[SPACEDIM * (iT + 1)],
                &sourceCoords[0],
                &sourceCoords[SPACEDIM * iS],
                &sourceCoords[SPACEDIM * (iS + 1)],
                inter,
...
TRI_INTER_TEMPLATE
double
TRI_INTERSECTOR::intersectGeoBary(
...
)
{
    ...
    for (ConnType iT = 1; iT < nbNodesT - 1; iT++)
    {
        ...
        INTERP_KERNEL::intersec_de_triangle(
            &targetCell[0],
            &targetCell[SPACEDIM * iT],
            &targetCell[SPACEDIM * (iT + 1)],
            sourceCell[0],
            sourceCell[1],
            sourceCell[2],
            inter,
