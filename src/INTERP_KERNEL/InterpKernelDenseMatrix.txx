...
template <class T>
DenseMatrixT<T> &
DenseMatrixT<T>::operator=(const DenseMatrixT<T> &rhs)
{
    if (this != &rhs)
    {
        mcIdType i, j, nel;
...
template <class T>
void
DenseMatrixT<T>::resize(mcIdType newn, mcIdType newm)
{
    mcIdType i, nel;
    if (newn != nn || newm != mm)
    {
...
template <class T>
void
DenseMatrixT<T>::assign(mcIdType newn, mcIdType newm, const T &a)
{
    mcIdType i, j, nel;
    if (newn != nn || newm != mm)
    {
...
template <class T>
T
DenseMatrixT<T>::determinant() const
{
    if (nn == 1 && mm == 1)
        return v[0][0];
    if (nn == 2 && mm == 2)
...
template <class T>
T
DenseMatrixT<T>::toJacobian() const
{
    if (nn == mm)
        return determinant();
