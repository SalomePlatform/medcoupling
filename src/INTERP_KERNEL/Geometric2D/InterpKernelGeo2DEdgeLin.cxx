 ...
 */
bool
SegSegIntersector::haveTheySameDirection() const
{
 ...
 */
void
SegSegIntersector::getPlacements(
...
void
SegSegIntersector::getCurveAbscisse(Node *node, TypeOfLocInEdge &where, MergePoints &commonNode) const
{
    bool obvious;
    ...
    if (obvious)
        return;
    double ret =
        ((*node)[!_ind] - (*_e1.getStartNode())[!_ind]) / ((*_e1.getEndNode())[!_ind] - (*_e1.getStartNode())[!_ind]);
    if (ret > 0. && ret < 1.)
        where = INSIDE;
    else if (ret < 0.)
        where = OUT_BEFORE;
...
SegSegIntersector::getIntersectionsCharacteristicVal() const
{
    std::list<IntersectElement> ret;
    if (_earlyInter)
    {
        // Intersection was already found: it is a common node shared by _e1 and _e2 - see
 ...
 * If one puts the two vectors starting at the origin, determinant/dimChar is a close representative of the absolute
 * distance between the tip of one vector to the other vector.
 */
bool
SegSegIntersector::areColinears() const
{
    Bounds b1, b2;
 ...
 * \param areOverlapped if the two segs are colinears, this parameter looks if e1 and e2 are overlapped, i.e. is they
 * lie on the same line (= this is different from a true intersection, two segments can be in "overlap" mode, without
 * intersecting)
 */
void
SegSegIntersector::areOverlappedOrOnlyColinears(bool &obviousNoIntersection, bool &areOverlapped)
{
    Bounds b1, b2;
    b1.prepareForAggregation();
    b2.prepareForAggregation();
    ...
    else  // Colinear vectors
    {
        ...
        areOverlapped = fabs(x * _matrix[1 + shift] - y * _matrix[0 + shift]) <
                        dimCharE1 * dimCharE2 * QuadraticPlanarPrecision::getPrecision();
        // explanation: if areOverlapped is true, we don't know yet if there will be an intersection (see meaning of
        // areOverlapped in method doxy above) if areOverlapped is false, we have two colinear vectors, not lying on the
        // same line, so we're sure there is no intersec
...
EdgeLin::EdgeLin(Node *start, Node *end, bool direction) : Edge(start, end, direction) { updateBounds(); }

EdgeLin::EdgeLin(double sX, double sY, double eX, double eY) : Edge(sX, sY, eX, eY) { updateBounds(); }
 ...
 */
bool
EdgeLin::isIn(double characterVal) const
{
    return characterVal > 0. && characterVal < 1.;
}

Node *
EdgeLin::buildRepresentantOfMySelf() const
{
    return new Node(((*(_start))[0] + (*(_end))[0]) / 2., ((*(_start))[1] + (*(_end))[1]) / 2.);
}

double
EdgeLin::getCharactValue(const Node &node) const
{
    return getCharactValueEng(node);
}

double
EdgeLin::getCharactValueBtw0And1(const Node &node) const
{
    return getCharactValueEng(node);
}

double
EdgeLin::getDistanceToPoint(const double *pt) const
{
...
bool
EdgeLin::isNodeLyingOn(const double *coordOfNode) const
{
...
void
EdgeLin::dumpInXfigFile(std::ostream &stream, bool direction, int resolution, const Bounds &box) const
{
    stream << "2 1 0 1 ";
...
void
EdgeLin::update(Node *m)
{
    updateBounds();
}

double
EdgeLin::getNormSq() const
{
 ...
 */
double
EdgeLin::getAreaOfZone() const
{
    return ((*_start)[0] - (*_end)[0]) * ((*_start)[1] + (*_end)[1]) / 2.;
}

void
EdgeLin::getBarycenter(double *bary) const
{
 ...
 */
void
EdgeLin::getBarycenterOfZone(double *bary) const
{
    double x1 = (*_start)[0];
    double y1 = (*_start)[1];
    double x2 = (*_end)[0];
 ...
 */
void
EdgeLin::getMiddleOfPoints(const double *p1, const double *p2, double *mid) const
{
    mid[0] = (p1[0] + p2[0]) / 2.;
    mid[1] = (p1[1] + p2[1]) / 2.;
}
...
Edge *
EdgeLin::buildEdgeLyingOnMe(Node *start, Node *end, bool direction) const
{
...
double
EdgeLin::getCharactValueEng(const double *node) const
{
    double car1_1x = node[0] - (*(_start))[0];
    double car1_2x = (*(_end))[0] - (*(_start))[0];
    double car1_1y = node[1] - (*(_start))[1];
    double car1_2y = (*(_end))[1] - (*(_start))[1];
