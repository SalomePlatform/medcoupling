...
class QuadraticPolygon : public ComposedEdge
{
   public:
    ...
    INTERPKERNEL_EXPORT void splitAbs(
    ...
    INTERPKERNEL_EXPORT void buildFromCrudeDataArray(
    ...
    INTERPKERNEL_EXPORT void buildFromCrudeDataArray2(
        const std::map<mcIdType, INTERP_KERNEL::Node *> &mapp,
        bool isQuad,
        const mcIdType *nodalBg,
        const double *coords,
    ...
    INTERPKERNEL_EXPORT void updateLocOfEdgeFromCrudeDataArray2(
        const mcIdType *descBg,
        const mcIdType *descEnd,
        const std::vector<std::vector<mcIdType> > &intersectEdges,
        const INTERP_KERNEL::QuadraticPolygon &pol1,
    ...
    INTERPKERNEL_EXPORT void appendEdgeFromCrudeDataArray(
        std::size_t edgeId,
        const std::map<mcIdType, INTERP_KERNEL::Node *> &mapp,
        bool isQuad,
        const mcIdType *nodalBg,
    ...
    INTERPKERNEL_EXPORT void appendSubEdgeFromCrudeDataArray(
        Edge *baseEdge,
        std::size_t j,
        bool direct,
        mcIdType edgeId,
    ...
    INTERPKERNEL_EXPORT void appendCrudeData(
    ...
    INTERPKERNEL_EXPORT void buildPartitionsAbs(
        QuadraticPolygon &other,
    ...
    INTERPKERNEL_EXPORT void intersectForPerimeter(
        const QuadraticPolygon &other,
        double &perimeterThisPart,
        double &perimeterOtherPart,
        double &perimeterCommonPart
    ) const;
    INTERPKERNEL_EXPORT void intersectForPerimeterAdvanced(
        const QuadraticPolygon &other, std::vector<double> &polThis, std::vector<double> &polOther
    ) const;
    INTERPKERNEL_EXPORT void intersectForPoint(
        const QuadraticPolygon &other, std::vector<int> &numberOfCreatedPointsPerEdge
    ) const;
   ...
   public:  // Only public for tests reasons
    ...
    INTERPKERNEL_EXPORT static void SplitPolygonsEachOther(
        QuadraticPolygon &pol1, QuadraticPolygon &pol2, int &nbOfSplits
    );
    INTERPKERNEL_EXPORT std::vector<QuadraticPolygon *> buildIntersectionPolygons(
        const QuadraticPolygon &pol1, const QuadraticPolygon &pol2
    ) const;
    INTERPKERNEL_EXPORT bool haveIAChanceToBeCompletedBy(
        const QuadraticPolygon &pol1NotSplitted,
        const QuadraticPolygon &pol2Splitted,
        bool &direction,
        bool &needCleaning
    ) const;
    INTERPKERNEL_EXPORT static void ComputeResidual(
   ...
   protected:
    ...
    static void UpdateNeighbours(
        const MergePoints &merger,
        IteratorOnComposedEdge it1,
        IteratorOnComposedEdge it2,
        const EDGES *e1,
        const EDGES *e2
    );
    std::list<QuadraticPolygon *>::iterator fillAsMuchAsPossibleWith(
        const QuadraticPolygon &pol2Splitted,
        std::list<QuadraticPolygon *>::iterator iStart,
        std::list<QuadraticPolygon *>::iterator iEnd,
        bool direction
    );
    static std::list<QuadraticPolygon *>::iterator CheckInList(
...
template <class EDGES>
void
QuadraticPolygon::UpdateNeighbours(
    const MergePoints &merger, IteratorOnComposedEdge it1, IteratorOnComposedEdge it2, const EDGES *e1, const EDGES *e2
)
{
    it1.previousLoop();
    it2.previousLoop();
    ...
    curE1->changeEndNodeWith(e1->getStartNode());
    curE2->changeEndNodeWith(e2->getStartNode());
    it1.nextLoop();
    it1.nextLoop();
    it2.nextLoop();
    it2.nextLoop();
    curE1->changeStartNodeWith(e1->getEndNode());
    curE2->changeStartNodeWith(e2->getEndNode());
}
}  // namespace INTERP_KERNEL
