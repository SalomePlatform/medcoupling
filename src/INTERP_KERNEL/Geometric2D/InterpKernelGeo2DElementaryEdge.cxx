...
bool
ElementaryEdge::isNodeIn(Node *n) const
{
 ...
 */
void
ElementaryEdge::getBarycenterOfZone(double *bary) const
{
    double tmp[2];
...
void
ElementaryEdge::fillBounds(Bounds &output) const
{
    output.aggregate(_ptr->getBounds());
}

void
ElementaryEdge::getAllNodes(std::set<Node *> &output) const
{
...
bool
ElementaryEdge::hasSameExtremities(const ElementaryEdge &other) const
{
    std::set<Node *> s1, s2;
    getAllNodes(s1);
...
void
ElementaryEdge::getBarycenter(double *bary, double &weigh) const
{
...
void
ElementaryEdge::initLocations() const
{
    _ptr->initLocs();
}
...
ElementaryEdge::locateFullyMySelf(const ComposedEdge &pol, TypeOfEdgeLocInPolygon precEdgeLoc) const
{
...
TypeOfEdgeLocInPolygon
ElementaryEdge::locateFullyMySelfAbsolute(const ComposedEdge &pol) const
{
...
Node *
ElementaryEdge::getEndNode() const
{
    if (_direction)
...
Node *
ElementaryEdge::getStartNode() const
{
    if (_direction)
        return _ptr->getStartNode();
    else
...
bool
ElementaryEdge::changeEndNodeWith(Node *node) const
{
    if (_direction)
...
bool
ElementaryEdge::changeStartNodeWith(Node *node) const
{
    if (_direction)
        return _ptr->changeStartNodeWith(node);
    else
...
void
ElementaryEdge::dumpInXfigFile(std::ostream &stream, int resolution, const Bounds &box) const
{
    _ptr->dumpInXfigFile(stream, _direction, resolution, box);
}

void
ElementaryEdge::dumpToCout(const std::map<INTERP_KERNEL::Node *, int> &mapp, int index) const
{
    _ptr->dumpToCout(mapp, index);
}

bool
ElementaryEdge::intresicEqual(const ElementaryEdge *other) const
{
    return _ptr == other->_ptr;
}

bool
ElementaryEdge::intresicEqualDirSensitive(const ElementaryEdge *other) const
{
    return (_direction == other->_direction) && (_ptr == other->_ptr);
}

bool
ElementaryEdge::intresincEqCoarse(const Edge *other) const
{
    return _ptr == other;
}

bool
ElementaryEdge::isEqual(const ElementaryEdge &other) const
{
 ...
 */
void
ElementaryEdge::fillGlobalInfoAbs(
...
{
    _ptr->fillGlobalInfoAbs(
        _direction, mapThis, mapOther, offset1, offset2, fact, baryX, baryY, edgesThis, addCoo, mapAddCoo
    );
}

/*!
 * Called by QuadraticPolygon::splitAbs method. Close to ElementaryEdge::fillGlobalInfoAbs method expect that here
 * edgesOther (that replace edgesThis) is here an in/out parameter that only contains nodes unsorted because the "other"
 * mesh is not subdivided yet.
 */
void
ElementaryEdge::fillGlobalInfoAbs2(
    const std::map<INTERP_KERNEL::Node *, mcIdType> &mapThis,
    const std::map<INTERP_KERNEL::Node *, mcIdType> &mapOther,
    mcIdType offset1,
    mcIdType offset2,
...
) const
{
    ...
    _ptr->fillGlobalInfoAbs2(
        mapThis, mapOther, offset1, offset2, fact, baryX, baryY, skipStartOrEnd, edgesOther, addCoo, mapAddCoo
    );
}
 ...
 */
ElementaryEdge *
ElementaryEdge::BuildEdgeFromStartEndDir(bool direction, INTERP_KERNEL::Node *start, INTERP_KERNEL::Node *end)
{
