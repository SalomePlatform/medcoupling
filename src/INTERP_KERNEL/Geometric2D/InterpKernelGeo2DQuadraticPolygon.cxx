...
QuadraticPolygon::QuadraticPolygon(const char *file)
{
    ...
    try
    {
        do stream.getline(currentLine, MAX_SIZE_OF_LINE_XFIG_FILE);
...
QuadraticPolygon *
QuadraticPolygon::BuildLinearPolygon(std::vector<Node *> &nodes)
{
...
QuadraticPolygon *
QuadraticPolygon::BuildArcCirclePolygon(std::vector<Node *> &nodes)
{
    QuadraticPolygon *ret(new QuadraticPolygon);
    std::size_t size = nodes.size();
...

    {
        ...
        nodes[i]->decrRef();
        nodes[i + size / 2]->decrRef();
    }
    return ret;
}

Edge *
QuadraticPolygon::BuildLinearEdge(std::vector<Node *> &nodes)
{
    if (nodes.size() != 2)
        throw INTERP_KERNEL::Exception(
            "QuadraticPolygon::BuildLinearEdge : input vector is expected to be of size 2 !"
        );
...
Edge *
QuadraticPolygon::BuildArcCircleEdge(std::vector<Node *> &nodes)
{
    if (nodes.size() != 3)
        throw INTERP_KERNEL::Exception(
            "QuadraticPolygon::BuildArcCircleEdge : input vector is expected to be of size 3 !"
        );
...
void
QuadraticPolygon::BuildDbgFile(const std::vector<Node *> &nodes, const char *fileName)
{
...
void
QuadraticPolygon::closeMe() const
{
...
void
QuadraticPolygon::circularPermute()
{
...
bool
QuadraticPolygon::isButterflyAbs()
{
...
bool
QuadraticPolygon::isButterfly() const
{
...
void
QuadraticPolygon::dumpInXfigFileWithOther(const ComposedEdge &other, const char *fileName) const
{
    std::ofstream file(fileName);
...
void
QuadraticPolygon::dumpInXfigFile(const char *fileName) const
{
    std::ofstream file(fileName);
    const int resolution = 1200;
    Bounds box;
...
void
QuadraticPolygon::dumpInXfigFile(std::ostream &stream, int resolution, const Bounds &box) const
{
...
/*!
 * Warning contrary to intersectWith method this method is \b NOT const. 'this' and 'other' are modified after call of
 * this method.
 */
double
QuadraticPolygon::intersectWithAbs(QuadraticPolygon &other)
{
...
/*!
 * This method splits 'this' with 'other' into smaller pieces localizable. 'mapThis' is a map that gives the
 * correspondence between nodes contained in 'this' and node ids in a global mesh. In the same way, 'mapOther' gives the
 * correspondence between nodes contained in 'other' and node ids in a global mesh from which 'other' is extracted. This
 * method has 1 out parameter : 'edgesThis', After the call of this method, it contains the nodal connectivity
 * (including type) of 'this' into globlal "this mesh". This method has 2 in/out parameters : 'subDivOther' and
 * 'addCoo'.'otherEdgeIds' is useful to put values in 'edgesThis', 'subDivOther' and 'addCoo'. Size of 'otherEdgeIds'
 * has to be equal to number of ElementaryEdges in 'other'. No check of that will be done. The term 'abs' in the name
 * recalls that we normalize the mesh (spatially) so that node coordinates fit into [0;1].
 ...
 * @param edgesInOtherColinearWithThis will be appended at the end of the vector with colinear edge ids of other (if
 * any)
 ...
 */
void
QuadraticPolygon::splitAbs(
    QuadraticPolygon &other,
...
)
{
    ...
    {
        // For each edge of 'other', proceed with intersections: the edge might split into sub-edges, 'otherTmp' will
        // hold the final split result. In the process of going through all edges of 'other', 'this' (which contains
        // initially only one edge) is sub-divised into several edges : each of them has to be tested when intersecting
        // the next candidate stored in 'other'.
        QuadraticPolygon otherTmp;
        ElementaryEdge *curOther = itOther.current();
        otherTmp.pushBack(new ElementaryEdge(curOther->getPtr(), curOther->getDirection()));
        curOther->getPtr()->incrRef();
        ...
        for (itOtherTmp.first(); !itOtherTmp.finished(); itOtherTmp.next())
        {
            ...
            for (; !itThis.finished();)
            {
                ...
                merge.clear();
                //
                std::map<INTERP_KERNEL::Node *, mcIdType>::const_iterator thisStart(
                    mapThis.find(curThis->getStartNode())
                ),
                    thisEnd(mapThis.find(curThis->getEndNode())),
                    otherStart(mapOther.find(curOtherTmp->getStartNode())),
                    otherEnd(mapOther.find(curOtherTmp->getEndNode()));
                mcIdType thisStart2(thisStart == mapThis.end() ? -1 : (*thisStart).second),
                    thisEnd2(thisEnd == mapThis.end() ? -1 : (*thisEnd).second),
                    otherStart2(otherStart == mapOther.end() ? -1 : (*otherStart).second + offset1),
                    otherEnd2(otherEnd == mapOther.end() ? -1 : (*otherEnd).second + offset1);
                //
                if (curThis->getPtr()->intersectWith(curOtherTmp->getPtr(), merge, *cThis, *cOther))
                {
                    if (!curThis->getDirection())
                        cThis->reverse();
                    if (!curOtherTmp->getDirection())
                        cOther->reverse();
                    // Substitution of a single simple edge by two sub-edges resulting from the intersection
                    //    First modify the edges currently pointed by itThis and itOtherTmp so that the newly created
                    //    node becomes the end of the previous sub-edge and the beginning of the next one.
        ...
        if (otherTmp._sub_edges.size() >
            1)  // only if a new point has been added (i.e. an actual intersection was done)
        {
            std::size_t jj = 0, sz(otherTmp._sub_edges.size());
            for (std::list<ElementaryEdge *>::const_iterator it = otherTmp._sub_edges.begin();
                 it != otherTmp._sub_edges.end();
                 it++, jj++)
            {
                short skipStartOrEnd = jj == 0 ? -1 : (jj == sz - 1 ? 1 : 0);  // -1 means START, 1 means END, 0 other
                (*it)->fillGlobalInfoAbs2(
                    mapThis,
                    mapOther,
                    offset1,
                    offset2,
                    ...
                    yBaryBB,
                    skipStartOrEnd,
                    /*out*/ subDivOther[otherEdgeIds[i]],
                    addCoo,
                    mapAddCoo
                );
            }
    ...
    //
    for (std::list<ElementaryEdge *>::const_iterator it = _sub_edges.begin(); it != _sub_edges.end(); it++)
        (*it)->fillGlobalInfoAbs(
            mapThis, mapOther, offset1, offset2, /**/ fact, xBaryBB, yBaryBB, /**/ edgesThis, addCoo, mapAddCoo
        );
    //
}
 ...
 */
void
QuadraticPolygon::buildFromCrudeDataArray(
...
void
QuadraticPolygon::appendEdgeFromCrudeDataArray(
    std::size_t edgePos,
    const std::map<mcIdType, INTERP_KERNEL::Node *> &mapp,
    bool isQuad,
    const mcIdType *nodalBg,
...
void
QuadraticPolygon::appendSubEdgeFromCrudeDataArray(
    Edge *baseEdge,
    std::size_t j,
    bool direct,
    mcIdType edgeId,
...
/*!
 * This method builds from descending conn of a quadratic polygon stored in crude mode (MEDCoupling). Descending conn is
 * in FORTRAN relative mode in order to give the orientation of edge.
 */
void
QuadraticPolygon::buildFromCrudeDataArray2(
    const std::map<mcIdType, INTERP_KERNEL::Node *> &mapp,
    bool isQuad,
    const mcIdType *nodalBg,
    const double *coords,
...
)
{
    ...
    for (std::size_t i = 0; i < nbOfSeg; i++)  // loop over all edges of pol2
    {
        ...
        std::map<mcIdType, std::vector<INTERP_KERNEL::ElementaryEdge *> >::const_iterator it1 = alreadyExistingIn2.find(
                                                                                              descBg[i]
                                                                                          ),
                                                                                          it2 = alreadyExistingIn2.find(
                                                                                              -descBg[i]
                                                                                          );
        if (it1 != alreadyExistingIn2.end() || it2 != alreadyExistingIn2.end())
        {
            bool sameDir = (it1 != alreadyExistingIn2.end());
            const std::vector<INTERP_KERNEL::ElementaryEdge *> &edgesAlreadyBuilt =
                sameDir ? (*it1).second : (*it2).second;
            if (sameDir)
            {
                for (std::vector<INTERP_KERNEL::ElementaryEdge *>::const_iterator it3 = edgesAlreadyBuilt.begin();
                     it3 != edgesAlreadyBuilt.end();
                     it3++)
                {
                    Edge *ee = (*it3)->getPtr();
                    ee->incrRef();
            ...
            {
                for (std::vector<INTERP_KERNEL::ElementaryEdge *>::const_reverse_iterator it4 =
                         edgesAlreadyBuilt.rbegin();
                     it4 != edgesAlreadyBuilt.rend();
                     it4++)
                {
                    Edge *ee = (*it4)->getPtr();
                    ee->incrRef();
        ...
        {  // if the current edge of pol2 has one or more colinear edges part into pol1
            ...
            for (std::size_t j = 0; j < nbOfEdgesIn1; j++)
            {
                ...
                {
                    idIns1.push_back(
                        std::pair<mcIdType, std::pair<bool, mcIdType> >(
                            edgeId1, std::pair<bool, mcIdType>(descBg1[j] > 0, offset1)
                        )
                    );  // it exists an edge into pol1 given by tuple (idIn1,direct1) that is colinear at edge 'edgeId'
                        // in pol2
                        // std::pair<edgeId1); direct1=descBg1[j]>0;
                }
                offset1 += ToIdType(
                    intersectEdges1[edgeId1].size() / 2
                );  // offset1 is used to find the INTERP_KERNEL::Edge * instance into pol1 that will be part of edge
                    // into pol2
            }
            directos = idIns1.empty();
        }
        if (directos)
        {  // no subpart of edge 'edgeId' of pol2 is in pol1 so let's operate the same thing that
           // QuadraticPolygon::buildFromCrudeDataArray method
            ...
            for (std::size_t p = 0; p < zeSz; p++, it5++) alreadyExistingIn2[descBg[i]][zeSz - p - 1] = *it5;
        }
        else
        {  // there is subpart of edge 'edgeId' of pol2 inside pol1
            ...
            for (std::size_t j = 0; j < nbOfSubEdges; j++)
            {
                ...
                for (std::vector<std::pair<mcIdType, std::pair<bool, mcIdType> > >::const_iterator it = idIns1.begin();
                     it != idIns1.end() && !found;
                     it++)
                {
                    ...
                    {  // perform a loop on all subedges of pol1 that includes edge 'edgeId' of pol2. For the moment we
                       // iterate only on subedges of ['idIn1']... To improve
                        if (subEdge1PossiblyAlreadyIn1[2 * k] == idBg && subEdge1PossiblyAlreadyIn1[2 * k + 1] == idEnd)
                        {
                            direction11 = true;
                            found = true;
                        }
                        else if (subEdge1PossiblyAlreadyIn1[2 * k] == idEnd &&
                                 subEdge1PossiblyAlreadyIn1[2 * k + 1] == idBg)
                        {
                            direction11 = false;
                            found = true;
                        }
                ...
                if (!found)
                {  // the current subedge of edge 'edgeId' of pol2 is not a part of the colinear edge 'idIn1' of pol1 ->
                   // build new Edge instance
                ...
                {  // the current subedge of edge 'edgeId' of pol2 is part of the colinear edge 'idIn1' of pol1 -> reuse
                   // Edge instance of pol1
                    ElementaryEdge *e =
                        pol1[FromIdType<int>(offset1 + (direct1 ? offset2 : nbOfSubEdges1 - offset2 - 1))];
                    Edge *ee = e->getPtr();
                    ee->incrRef();
 ...
 */
void
QuadraticPolygon::updateLocOfEdgeFromCrudeDataArray2(
    const mcIdType *descBg,
    const mcIdType *descEnd,
    const std::vector<std::vector<mcIdType> > &intersectEdges,
    const INTERP_KERNEL::QuadraticPolygon &pol1,
        ...
        for (std::size_t j = 0; j < nbOfEdgesIn1; j++)
        {
            ...
            if (std::find(c.begin(), c.end(), edgeId1) != c.end())
            {
                for (std::size_t k = 0; k < nbOfSubEdges; k++)
                {
                    ...
                    {
                        found = (subEdge1PossiblyAlreadyIn1[2 * kk] == idBg &&
                                 subEdge1PossiblyAlreadyIn1[2 * kk + 1] == idEnd) ||
                                (subEdge1PossiblyAlreadyIn1[2 * kk] == idEnd &&
                                 subEdge1PossiblyAlreadyIn1[2 * kk + 1] == idBg);
                        if (!found)
                            offset2++;
                    }
                    if (found)
            ...
            offset1 += ToIdType(
                intersectEdges1[edgeId1].size() / 2
            );  // offset1 is used to find the INTERP_KERNEL::Edge * instance into pol1 that will be part of edge into
                // pol2
        }
    }
}
...
void
QuadraticPolygon::appendCrudeData(
...
) const
{
    ...
    if (presenceOfQuadratic)
    {
        ...
        for (std::list<ElementaryEdge *>::const_iterator it = _sub_edges.begin(); it != _sub_edges.end();
             it++, j++, nbOfNodesInPg++)
        {
...
/*!
 * This method make the hypothesis that \a this and \a other are split at the minimum into edges that are fully IN, OUT
 * or ON. This method returns newly created polygons in \a conn and \a connI and the corresponding ids ( \a idThis, \a
 * idOther) are stored respectively into \a nbThis and \a nbOther.
 * @param [in,out] edgesThis parameter that keep informed the caller about the edges in this not shared by the result of
 * intersection of \a this with \a other
 * @param [in,out] edgesBoundaryOther parameter that stores all edges in result of intersection that are not
 */
void
QuadraticPolygon::buildPartitionsAbs(
    QuadraticPolygon &other,
 ...
 */
void
QuadraticPolygon::cleanDegeneratedConsecutiveEdges()
{
 ...
 */
double
QuadraticPolygon::intersectWithAbs1D(QuadraticPolygon &other, bool &isColinear)
{
    double ret = 0., xBaryBB, yBaryBB;
    double fact = normalize(&other, xBaryBB, yBaryBB);
    ...
    isColinear = false;
    for (std::list<ElementaryEdge *>::const_iterator it = cpyOfOther._sub_edges.begin();
         it != cpyOfOther._sub_edges.end();
         it++)
    {
        switch ((*it)->getLoc())
        {
 ...
 */
double
QuadraticPolygon::intersectWithAbs(QuadraticPolygon &other, double *barycenter)
{
...
/*!
 * \b WARNING this method is const and other is const too. \b BUT location of Edges in 'this' and 'other' are
 * nevertheless modified. This is possible because loc attribute in Edge class is mutable. This implies that if 'this'
 * or/and 'other' are reused for intersect* method initLocations has to be called on each of this/them.
 */
double
QuadraticPolygon::intersectWith(const QuadraticPolygon &other) const
{
    double ret = 0.;
    std::vector<QuadraticPolygon *> polygs = intersectMySelfWith(other);
    for (std::vector<QuadraticPolygon *>::iterator iter = polygs.begin(); iter != polygs.end(); iter++)
 ...
 */
double
QuadraticPolygon::intersectWith(const QuadraticPolygon &other, double *barycenter) const
{
    double ret = 0., bary[2];
    barycenter[0] = barycenter[1] = 0.;
    std::vector<QuadraticPolygon *> polygs = intersectMySelfWith(other);
 ...
 */
void
QuadraticPolygon::intersectForPerimeter(
    const QuadraticPolygon &other, double &perimeterThisPart, double &perimeterOtherPart, double &perimeterCommonPart
) const
{
    perimeterThisPart = 0.;
    perimeterOtherPart = 0.;
    perimeterCommonPart = 0.;
    QuadraticPolygon cpyOfThis(*this);
    QuadraticPolygon cpyOfOther(other);
    int nbOfSplits = 0;
    SplitPolygonsEachOther(cpyOfThis, cpyOfOther, nbOfSplits);
 ...
 */
void
QuadraticPolygon::intersectForPerimeterAdvanced(
    const QuadraticPolygon &other, std::vector<double> &polThis, std::vector<double> &polOther
) const
{
    polThis.resize(size());
 ...
 */
void
QuadraticPolygon::intersectForPoint(const QuadraticPolygon &other, std::vector<int> &numberOfCreatedPointsPerEdge) const
{
 ...
 * \b WARNING this method is const and other is const too. \b BUT location of Edges in 'this' and 'other' are
 * nevertheless modified. This is possible because loc attribute in Edge class is mutable. This implies that if 'this'
 * or/and 'other' are reused for intersect* method initLocations has to be called on each of this/them. This method is
 * currently not used by any high level functionality.
 */
std::vector<QuadraticPolygon *>
QuadraticPolygon::intersectMySelfWith(const QuadraticPolygon &other) const
{
    QuadraticPolygon cpyOfThis(*this);
    QuadraticPolygon cpyOfOther(other);
    int nbOfSplits = 0;
 ...
 * This method perform the minimal splitting so that at the end each edges constituting pol1 are fully either IN or OUT
 * or ON.
 * @param pol1 IN/OUT param that is equal to 'this' when called.
 */
void
QuadraticPolygon::SplitPolygonsEachOther(QuadraticPolygon &pol1, QuadraticPolygon &pol2, int &nbOfSplits)
{
    ...
    for (it2.first(); !it2.finished(); it2.next())
    {
        ...
        for (; !it1.finished();)
        {
            ...
            {
                if (!curE1->getDirection())
                    c1->reverse();
                if (!curE2->getDirection())
                    c2->reverse();
...
void
QuadraticPolygon::performLocatingOperation(QuadraticPolygon &pol1) const
{
...
void
QuadraticPolygon::performLocatingOperationSlow(QuadraticPolygon &pol2) const
{
 ...
 */
std::vector<QuadraticPolygon *>
QuadraticPolygon::buildIntersectionPolygons(const QuadraticPolygon &pol1, const QuadraticPolygon &pol2) const
{
...
QuadraticPolygon::zipConsecutiveInSegments() const
{
 ...
 */
void
QuadraticPolygon::ClosePolygons(
...
)
{
    ...
    for (std::list<QuadraticPolygon *>::iterator iter = pol1Zip.begin(); iter != pol1Zip.end();)
    {
        ...
        std::list<QuadraticPolygon *>::iterator iter2 = iter;
        iter2++;
        // Fill as much as possible the current iterate (=a part of pol1) with consecutive pieces from pol2:
        std::list<QuadraticPolygon *>::iterator iter3 =
            (*iter)->fillAsMuchAsPossibleWith(pol2, iter2, pol1Zip.end(), directionInPol2);
 ...
 */
bool
QuadraticPolygon::haveIAChanceToBeCompletedBy(
    const QuadraticPolygon &pol1NotSplitted, const QuadraticPolygon &pol2Splitted, bool &direction, bool &needCleaning
) const
{
    needCleaning = false;
    ...
    if (e->getLoc() == FULL_ON_1)
    {
        if (e->getPtr() == cur->getPtr())
        {
            ...
            //    - either this means that pol1 and pol2 have opposite orientation (since we matched end node with start
            //    node before)
            //    - or (more tricky, see testIntersect2DMeshes11()) that pol1 and pol2 have same orientation but 'this'
            //    turns in such a way that it attaches to pol2 on an edge in opposite orientation.
            ...
            repr->decrRef();
            direction = ret;
            needCleaning = ret;  // if true we are in tricky case 2 above, we know that we will produce two consecutive
                                 // overlapping edges in result
            return ret;
        }
 ...
 */
std::list<QuadraticPolygon *>::iterator
QuadraticPolygon::fillAsMuchAsPossibleWith(
    const QuadraticPolygon &pol2Splitted,
...
)
{
    ...
    if (!direction)
        it1.previousLoop();
    Node *nodeToTest;
    int szMax(pol2Splitted.size() + 1),
        ii(0);  // protection against aggressive users of IntersectMeshes using invalid input meshes ...
    std::list<QuadraticPolygon *>::iterator ret;
    do
    {  // Stack (consecutive) edges of pol1 into the result (no need to care about ordering, edges from pol1 are already
       // consecutive)
        cur1 = it1.current();
    ...
    if (ii == szMax)  // here a protection against aggressive users of IntersectMeshes of invalid input meshes
        throw INTERP_KERNEL::Exception(
            "QuadraticPolygon::fillAsMuchAsPossibleWith : Something is invalid with input polygons !"
        );
    return ret;
}

std::list<QuadraticPolygon *>::iterator
QuadraticPolygon::CheckInList(
    Node *n, std::list<QuadraticPolygon *>::iterator iStart, std::list<QuadraticPolygon *>::iterator iEnd
)
{
 ...
 *      - complete those pieces by edges found in edgesInPol2OnBoundary, which are edges from pol2 located on the
 * boundary of the previously built intersecting cells
 */
void
QuadraticPolygon::ComputeResidual(
    const QuadraticPolygon &pol1,
...
{
    // Initialise locations on pol1. Remember that edges found in 'notUsedInPol1' are also part of the edges forming
    // pol1.
    pol1.initLocations();
    ...
    for (std::set<Edge *>::const_iterator it2 = edgesInPol2OnBoundary.begin(); it2 != edgesInPol2OnBoundary.end();
         it2++)
    {
        (*it2)->initLocs();
        (*it2)->declareIn();
    ...
    int sz = pol1.size();
    std::list<QuadraticPolygon *> pol1Zip;
    // If none of the edges of pol1 was consumed by the rebuilding process, we can directly take pol1 as it is to form a
    // cell:
    if (pol1.size() == (int)notUsedInPol1.size() && edgesInPol2OnBoundary.empty())
    {
        pol1.appendCrudeData(mapp, 0., 0., 1., offset, addCoordsQuadratic, conn, connI);
        nb1.push_back(idThis);
        nb2.push_back(-1);
        return;
    }
    // Zip consecutive edges found in notUsedInPol1L and which are not overlapping with boundary edge from
    // edgesInPol2OnBoundary:
    while (!notUsedInPol1L.empty())
    {
        // If all nodes are IN or edge is ON (i.e. as initialised at the begining of the method) then error
        for (int i = 0;
             i < sz && (itPol1.current()->getStartNode()->getLoc() != IN_1 || itPol1.current()->getLoc() != FULL_ON_1);
             i++)
            itPol1.nextLoop();
        if (itPol1.current()->getStartNode()->getLoc() != IN_1 || itPol1.current()->getLoc() != FULL_ON_1)
            throw INTERP_KERNEL::Exception(
                "Presence of a target polygon fully included in source polygon ! The partition of this leads to a non "
                "simply connex cell (with hole) ! Impossible ! Such resulting cell cannot be stored in MED cell format "
                "!"
            );
        QuadraticPolygon *tmp1 = new QuadraticPolygon;
        do
    ...
    for (std::list<QuadraticPolygon *>::const_iterator itMNT = pol1Zip.begin(); itMNT != pol1Zip.end();
         itMNT++, iiMNT++)
        nbOfTurn += (*itMNT)->size();
    maxNbOfTurn = maxNbOfTurn * nbOfTurn;
    maxNbOfTurn *= maxNbOfTurn;
    // [ABN] at least 3 turns for very small cases (typically one (quad) edge against one (quad or lin) edge forming a
    // new cell)!
    maxNbOfTurn = maxNbOfTurn < 3 ? 3 : maxNbOfTurn;
    nbOfTurn = 0;
    while (nbOfTurn <
           maxNbOfTurn)  // the 'normal' way out of this loop is the break towards the end when pol1Zip is empty.
    {
        // retPolsUnderConstruction initially empty -> see if(!pol1Zip.empty()) below ...
        for (std::list<QuadraticPolygon *>::iterator itConstr = retPolsUnderContruction.begin();
             itConstr != retPolsUnderContruction.end();)
        {
            ...
            {
                if (curN == (*it2)->getEndNode())  // only end node should be considered if orientation is correct for
                                                   // input meshes in the funny case of cells exactly included (see test
                                                   // case testIntersect2DMeshesTmp13) this is mandatory to take edges
                                                   // from pol2 in the right order.
                {
                    (*it2)->incrRef();
            ...
            if (!smthHappened)
            {
                for (std::list<Edge *>::iterator it2 = edgesInPol2OnBoundaryL.begin();
                     it2 != edgesInPol2OnBoundaryL.end();)
                {
            ...
            if (smthHappened)  // Now continue the construction by finding the next bit in pol1Zip. Not too sure what
                               // are the cases where the boolean if False ...
            {
                for (std::list<QuadraticPolygon *>::iterator itZip = pol1Zip.begin(); itZip != pol1Zip.end();)
                {
                    if (curN == (*itZip)->getStartNode())  // we found a matching piece to append in pol1Zip. Append all
                                                           // of it to the current polygon being reconstr
                    {
                        for (std::list<ElementaryEdge *>::const_iterator it4 = (*itZip)->_sub_edges.begin();
                             it4 != (*itZip)->_sub_edges.end();
                             it4++)
                        {
                            (*it4)->getPtr()->incrRef();
                            bool dir = (*it4)->getDirection();
                            (*itConstr)->pushBack(new ElementaryEdge((*it4)->getPtr(), dir));
                        }
            ...
            {
                for (std::list<ElementaryEdge *>::const_iterator it5 = (*itConstr)->_sub_edges.begin();
                     it5 != (*itConstr)->_sub_edges.end();
                     it5++)
                {
                    Edge *ee = (*it5)->getPtr();
                ...
                for (std::list<QuadraticPolygon *>::iterator it6 = pol1ZipConsumed[*itConstr].begin();
                     it6 != pol1ZipConsumed[*itConstr].end();
                     it6++)
        ...
        if (!pol1Zip.empty())  // the filling process of retPolsUnderConstruction starts here
        {
            ...
            for (std::list<ElementaryEdge *>::const_iterator it4 = first->_sub_edges.begin();
                 it4 != first->_sub_edges.end();
                 it4++)
            {
                (*it4)->getPtr()->incrRef();
                bool dir = (*it4)->getDirection();
                tmp->pushBack(new ElementaryEdge((*it4)->getPtr(), dir));
            }
    ...
    if (nbOfTurn == maxNbOfTurn)
    {
        ...
        oss << "Error during reconstruction of residual of cell ! It appears that either source or/and target mesh "
               "is/are not conform !";
         ...
         itConstr++)
    {
        if ((*itConstr)->getStartNode() == (*itConstr)->getEndNode())  // take only fully closed reconstructed polygon
        {
            ...
            (*itConstr)->appendCrudeData(mapp, 0., 0., 1., offset, addCoordsQuadratic, conn, connI);
            nb1.push_back(idThis);
            nb2.push_back(-1);
            for (std::list<QuadraticPolygon *>::iterator it6 = pol1ZipConsumed[*itConstr].begin();
                 it6 != pol1ZipConsumed[*itConstr].end();
        ...
        {
            std::ostringstream oss;
            oss << "Internal error during reconstruction of residual of cell! Non fully closed polygon built!";
            throw INTERP_KERNEL::Exception(oss.str().c_str());
        }
    }
}
