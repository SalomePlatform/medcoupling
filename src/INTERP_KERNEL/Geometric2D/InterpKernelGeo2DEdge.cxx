...
bool
MergePoints::isStart1(unsigned rk) const
{
    if (rk == 0)
        return _ass1Start1;
    else
...
bool
MergePoints::isEnd1(unsigned rk) const
{
    if (rk == 0)
        return _ass1End1;
    else
...
bool
MergePoints::isStart2(unsigned rk) const
{
    if (rk == 0)
        return _ass1Start2;
    else
...
bool
MergePoints::isEnd2(unsigned rk) const
{
    if (rk == 0)
        return _ass1End2;
    else
...
unsigned
MergePoints::getNumberOfAssociations() const
{
    unsigned ret = 0;
...
void
MergePoints::PushInMap(mcIdType key, mcIdType value, std::map<mcIdType, mcIdType> &mergedNodes)
{
    if (key != -1 && value != -1)
...
IntersectElement::IntersectElement(const IntersectElement &other)
    : _1S(other._1S),
      ...
      _node(other._node),
      _loc_of_node(other._loc_of_node),
      _e1(other._e1),
      _e2(other._e2)
{
    if (_node)
...
bool
IntersectElement::operator<(const IntersectElement &other) const
{
 ...
 */
bool
IntersectElement::isOnMergedExtremity() const
{
 ...
 */
void
IntersectElement::performMerging(MergePoints &commonNode) const
{
    if (_1S && _2S)
    {
 ...
 */
void
IntersectElement::setNode(Node *node) const
{
    if (node != _node)
    {
        if (_node)
...
bool
IntersectElement::isLowerOnOther(const IntersectElement &other) const
{
    return _e2.isLower(_chararct_val_for_e2, other._chararct_val_for_e2);
}

unsigned
IntersectElement::isOnExtrForAnEdgeAndInForOtherEdge() const
{
...
bool
IntersectElement::isIncludedByBoth() const
{
    return _e1.isIn(_chararct_val_for_e1) && _e2.isIn(_chararct_val_for_e2);
}

bool
EdgeIntersector::intersect(std::vector<Node *> &newNodes, bool &order, MergePoints &commonNode)
{
 ...
 */
void
EdgeIntersector::identifyEarlyIntersection(bool &i1S2S, bool &i1E2S, bool &i1S2E, bool &i1E2E)
{
    ...
    if (i1S2S || i1E2S || i1S2E || i1E2E)
    {
        ...
        node->incrRef();
        _earlyInter = new IntersectElement(
            _e1.getCharactValue(*node), _e2.getCharactValue(*node), i_1S, i_1E, i_2S, i_2E, node, _e1, _e2, keepOrder()
        );
    }
}

/*!
 * Locates 'node' regarding edge this->_e1. If node is located close to (with distant lt epsilon) start or end point of
 * _e1, 'node' takes its place. In this case 'obvious' is set to true and 'commonNode' stores information of merge point
 * and finally 'where' is set. Furthermore 'node' is declared as ON LIMIT to indicate in locating process that an
 * absolute location computation will have to be done. If 'node' is not close to start or end point of _e1, 'obvious' is
 * set to false and 'commonNode' and 'where' are let unchanged.
 */
void
EdgeIntersector::obviousCaseForCurvAbscisse(
    Node *node, TypeOfLocInEdge &where, MergePoints &commonNode, bool &obvious
) const
{
    obvious = true;
...
Edge::Edge(double sX, double sY, double eX, double eY)
    : _cnt(1), _loc(FULL_UNKNOWN), _start(new Node(sX, sY)), _end(new Node(eX, eY))
{
}
...
void
Edge::fillXfigStreamForLoc(std::ostream &stream) const
{
    switch (_loc)
    {
        case FULL_IN_1:
...
bool
Edge::changeStartNodeWith(Node *otherStartNode) const
{
...
bool
Edge::changeStartNodeWithAndKeepTrack(Node *otherStartNode, std::vector<Node *> &track) const
{
    if (_start == otherStartNode)
        return true;
...
bool
Edge::changeEndNodeWith(Node *otherEndNode) const
{
...
bool
Edge::changeEndNodeWithAndKeepTrack(Node *otherEndNode, std::vector<Node *> &track) const
{
    if (_end == otherEndNode)
        return true;
 ...
 */
void
Edge::addSubEdgeInVector(Node *start, Node *end, ComposedEdge &vec) const
{
 ...
 */
void
Edge::getNormalVector(double *vectOutput) const
{
    ...
    std::transform(
        vectOutput, vectOutput + 2, vectOutput, bind(std::multiplies<double>(), std::placeholders::_1, norm)
    );
...
Edge *
Edge::BuildEdgeFrom3Points(const double *start, const double *middle, const double *end)
{
...
Edge *
Edge::BuildEdgeFrom(Node *start, Node *end)
{
    return new EdgeLin(start, end);
}

Edge *
Edge::BuildFromXfigLine(std::istream &str)
{
    unsigned char type;
    str >> type;
    if (type == '2')
 ...
 * \param outVal1 Output filled in case true is returned. It specifies the new or not new edges by which 'this' is
 * replaced after intersecting op.
 * \param outVal2 Output filled in case true is returned. It specifies the new or not new edges by which 'other' is
 * replaced after intersecting op. return true if the intersection between this.
 */
bool
Edge::intersectWith(const Edge *other, MergePoints &commonNode, ComposedEdge &outVal1, ComposedEdge &outVal2) const
{
    bool ret = true;
...
)
{
    ...
    {
        std::vector<double>::const_iterator iter = find_if(
            distrib2.begin() + 1, distrib2.end(), bind(std::greater_equal<double>(), std::placeholders::_1, distrib1[i])
        );
        if (iter != distrib2.end())
        {
            for (unsigned int j = (unsigned)((iter - 1) - distrib2.begin()); j < nbOfV2; j++)
            {
                if (distrib2[j] <= distrib1[i + 1])
                {
                    ...
                    EdgeLin *e2 = new EdgeLin(n3, n4);
                    n1->setNewCoords(distrib1[i], 0.);
                    n2->setNewCoords(distrib1[i + 1], 0.);
                    n3->setNewCoords(distrib2[j], 0.);
                    n4->setNewCoords(distrib2[j + 1], 0.);
                    ...
                    ComposedEdge::Delete(f1);
                    ComposedEdge::Delete(f2);
                    e1->decrRef();
                    e2->decrRef();
                }
            }
...
EdgeIntersector *
Edge::BuildIntersectorWith(const Edge *e1, const Edge *e2)
{
    ...
    switch (type1)
    {
        ...
        default:
            // Should never happen
            throw Exception(
                "A non managed association of edge has been detected. Go work for intersection computation "
                "implementation."
            );
    }
    return ret;
 ...
 */
void
Edge::applySimilarity(double xBary, double yBary, double dimChar)
{
    _bounds.applySimilarity(xBary, yBary, dimChar);
}

void
Edge::unApplySimilarity(double xBary, double yBary, double dimChar)
{
    _bounds.unApplySimilarity(xBary, yBary, dimChar);
}

void
Edge::getMiddleOfPointsOriented(const double *p1, const double *p2, double *mid) const
{
...
)
{
    ...
    if (intersector->intersect(newNodes, order, commonNode))
    {
        if (newNodes.empty())
            throw Exception(
                "Internal error occurred - error in intersector implementation!"
...
int
Edge::CombineCodes(TypeOfLocInEdge code1, TypeOfLocInEdge code2)
{
    int ret = (int)code1;
 ...
 *
 * @param nS start node of e2 with the SAME DIRECTION as e1. The pointer nS should be equal to start node of e2 or to
 * its end node.
 * @param nE end node of e2 with the SAME DIRECTION as e1. The pointer nE should be equal to start node of e2 or to its
 * end node.
...
)
{
    ...
    switch (code)
    {
        ...
        case INSIDE *OFFSET_FOR_TYPEOFLOCINEDGE + OUT_AFTER:  // INSIDE - OUT_AFTER
            ...
            tmp = e1->buildEdgeLyingOnMe(nS, e1->getEndNode());
            tmp->incrRef();
        ...
        case INSIDE *OFFSET_FOR_TYPEOFLOCINEDGE + INSIDE:  // INSIDE - INSIDE
        {
            if (!e2->isIn(e2->getCharactValue(*(e1->getStartNode()))))
            {
                ...
                outVal2.pushBack(const_cast<Edge *>(e2));
                e2->declareOn();
                return true;
            }
            else
            {
                outVal1.resize(3);
                outVal2.resize(3);
                tmp = e1->buildEdgeLyingOnMe(e1->getStartNode(), nE);
                tmp->incrRef();
                tmp->declareOn();
                outVal1.setValueAt(0, tmp, true);
                outVal2.setValueAt(direction ? 2 : 0, tmp, direction);
                outVal1.setValueAt(1, e1->buildEdgeLyingOnMe(nE, nS));
                tmp = e1->buildEdgeLyingOnMe(nS, e1->getEndNode());
                tmp->incrRef();
                tmp->declareOn();
                outVal1.setValueAt(2, tmp, true);
                outVal2.setValueAt(direction ? 0 : 2, tmp, direction);
        ...
        case OUT_BEFORE *OFFSET_FOR_TYPEOFLOCINEDGE + OUT_AFTER:  // OUT_BEFORE - OUT_AFTER
        {
            ...
            outVal2.setValueAt(1, const_cast<Edge *>(e1), direction);
            e1->declareOn();
        ...
        case START *OFFSET_FOR_TYPEOFLOCINEDGE + END:  // START - END
        {
            ...
            e1->incrRef();
            outVal1.pushBack(const_cast<Edge *>(e1));
            outVal2.pushBack(const_cast<Edge *>(e1), direction);
            e1->declareOn();
            return true;
        }
        case START *OFFSET_FOR_TYPEOFLOCINEDGE + OUT_AFTER:  // START - OUT_AFTER
        {
            ...
            outVal1.pushBack(const_cast<Edge *>(e1));
            outVal2.resize(2);
            outVal2.setValueAt(direction ? 0 : 1, const_cast<Edge *>(e1), direction);
            e1->declareOn();
            outVal2.setValueAt(direction ? 1 : 0, e1->buildEdgeLyingOnMe(e1->getEndNode(), nE, direction));
            return true;
        }
        ...
        case OUT_BEFORE *OFFSET_FOR_TYPEOFLOCINEDGE + END:  // OUT_BEFORE - END
        {
            ...
            outVal2.resize(2);
            outVal2.setValueAt(direction ? 0 : 1, e1->buildEdgeLyingOnMe(nS, e1->getStartNode(), direction));
            outVal2.setValueAt(direction ? 1 : 0, const_cast<Edge *>(e1), direction);
            e1->declareOn();
            return true;
        }
        ...
        case END *OFFSET_FOR_TYPEOFLOCINEDGE + INSIDE:  // END - INSIDE
        {
            ...
            outVal2.setValueAt(
                direction ? 0 : 1, e1->buildEdgeLyingOnMe(e1->getEndNode(), e1->getStartNode(), direction)
            );
            outVal2.setValueAt(direction ? 1 : 0, tmp, direction);
            return true;
        }
...
void
Edge::dumpToCout(const std::map<INTERP_KERNEL::Node *, int> &mapp, int index) const
{
    ...
    switch (getLoc())
    {
        ...
        case FULL_UNKNOWN:
            ...
            break;
        default:
            locs = "oh my God! This is so wrong.";
    }
    std::cout << "Edge [" << index << "] : (" << std::hex << this << std::dec << ") -> (" << start << ", " << end
              << ")\t" << locs << std::endl;
}

bool
Edge::isEqual(const Edge &other) const
{
 ...
 */
bool
Edge::sortSubNodesAbs(const double *coo, std::vector<mcIdType> &subNodes)
{
    Bounds b;
