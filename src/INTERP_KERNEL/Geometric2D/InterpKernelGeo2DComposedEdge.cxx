...
{
    for (std::list<ElementaryEdge *>::const_iterator iter = other._sub_edges.begin(); iter != other._sub_edges.end();
         iter++)
        _sub_edges.push_back((*iter)->clone());
}

ComposedEdge::~ComposedEdge() { clearAll(_sub_edges.begin()); }

void
ComposedEdge::setValueAt(int i, Edge *e, bool direction)
{
...
double
ComposedEdge::getCommonLengthWith(const ComposedEdge &other) const
{
    double ret = 0.;
...
void
ComposedEdge::pushBack(Edge *edge, bool direction)
{
    _sub_edges.push_back(new ElementaryEdge(edge, direction));
}

void
ComposedEdge::pushBack(ElementaryEdge *elem)
{
    _sub_edges.push_back(elem);
}

void
ComposedEdge::pushBack(ComposedEdge *elem)
{
...
ElementaryEdge *
ComposedEdge::operator[](int i) const
{
...
bool
ComposedEdge::presenceOfOn() const
{
    bool ret = false;
    for (std::list<ElementaryEdge *>::const_iterator iter = _sub_edges.begin(); iter != _sub_edges.end() && !ret;
         iter++)
...
bool
ComposedEdge::presenceOfQuadraticEdge() const
{
    bool ret = false;
    for (std::list<ElementaryEdge *>::const_iterator iter = _sub_edges.begin(); iter != _sub_edges.end() && !ret;
         iter++)
...
void
ComposedEdge::initLocations() const
{
    for (std::list<ElementaryEdge *>::const_iterator iter = _sub_edges.begin(); iter != _sub_edges.end(); iter++)
        (*iter)->initLocations();
}
 ...
 */
void
ComposedEdge::InitLocationsWithOther(const ComposedEdge &first, const ComposedEdge &other)
{
    std::set<Edge *> s1, s2;
    for (std::list<ElementaryEdge *>::const_iterator it1 = first._sub_edges.begin(); it1 != first._sub_edges.end();
         it1++)
        s1.insert((*it1)->getPtr());
    for (std::list<ElementaryEdge *>::const_iterator it2 = other._sub_edges.begin(); it2 != other._sub_edges.end();
         it2++)
    ...
    std::set_intersection(
        s1.begin(), s1.end(), s2.begin(), s2.end(), std::back_insert_iterator<std::vector<Edge *> >(s3)
    );
    for (std::vector<Edge *>::const_iterator it3 = s3.begin(); it3 != s3.end(); it3++) (*it3)->declareOn();
}
...
bool
ComposedEdge::isNodeIn(Node *n) const
{
    bool ret = false;
    for (std::list<ElementaryEdge *>::const_iterator iter = _sub_edges.begin(); iter != _sub_edges.end() && !ret;
         iter++)
 ...
 * \f[
 * \int_{Polygon} x \cdot dS=\sum_{0 \leq i < nb of edges} -\int_{Edge_{i}}ydx=\sum_{0 \leq i < nb of edges}
 * AreaOfZone_{Edge_{i}}
 * \f]
...
double
ComposedEdge::getPerimeter() const
{
    double ret = 0.;
    for (std::list<ElementaryEdge *>::const_iterator iter = _sub_edges.begin(); iter != _sub_edges.end(); iter++)
...
double
ComposedEdge::getHydraulicDiameter() const
{
 ...
 * \f$ -\int_{Edge_{i}}yxdx \f$ and \f$ -\int_{Edge_{i}}\frac{y^{2}}{2}dx \f$ are computed virtually with
 * INTERP_KERNEL::Edge::getBarycenterOfZone.
 */
void
ComposedEdge::getBarycenter(double *bary) const
{
    bary[0] = 0.;
    bary[1] = 0.;
    double area = 0.;
 ...
 */
void
ComposedEdge::getBarycenterGeneral(double *bary) const
{
    if (_sub_edges.empty())
...
double
ComposedEdge::normalizeMe(double &xBary, double &yBary)
{
    Bounds b;
...
double
ComposedEdge::normalize(ComposedEdge *other, double &xBary, double &yBary)
{
    Bounds b;
    b.prepareForAggregation();
    fillBounds(b);
 ...
 */
void
ComposedEdge::unApplyGlobalSimilarityExt(ComposedEdge &other, double xBary, double yBary, double fact)
{
    initNodeHitStatus();
...
double
ComposedEdge::normalizeExt(ComposedEdge *other, double &xBary, double &yBary)
{
    Bounds b;
    b.prepareForAggregation();
    fillBounds(b);
...
void
ComposedEdge::dumpInXfigFile(std::ostream &stream, int resolution, const Bounds &box) const
{
    stream.precision(10);
    for (std::list<ElementaryEdge *>::const_iterator iter = _sub_edges.begin(); iter != _sub_edges.end(); iter++)
...
void
ComposedEdge::dumpToCout(const std::map<INTERP_KERNEL::Node *, int> &mapp) const
{
    int i = 0;
...
Node *
ComposedEdge::getEndNode() const
{
    return _sub_edges.back()->getEndNode();
}

Node *
ComposedEdge::getStartNode() const
{
    return _sub_edges.front()->getStartNode();
}

bool
ComposedEdge::changeEndNodeWith(Node *node) const
{
    return _sub_edges.back()->changeEndNodeWith(node);
}

bool
ComposedEdge::changeStartNodeWith(Node *node) const
{
    return _sub_edges.front()->changeStartNodeWith(node);
}

void
ComposedEdge::fillBounds(Bounds &output) const
{
    for (std::list<ElementaryEdge *>::const_iterator iter = _sub_edges.begin(); iter != _sub_edges.end(); iter++)
...
/*!
 * \b WARNING : applies similarity \b ONLY on edges without any change on Nodes. To perform a global similarity call
 * applyGlobalSimilarity.
 */
void
ComposedEdge::applySimilarity(double xBary, double yBary, double dimChar)
{
    for (std::list<ElementaryEdge *>::iterator iter = _sub_edges.begin(); iter != _sub_edges.end(); iter++)
 ...
 */
void
ComposedEdge::applyGlobalSimilarity(double xBary, double yBary, double dimChar)
{
 ...
 */
void
ComposedEdge::applyGlobalSimilarity2(ComposedEdge *other, double xBary, double yBary, double dimChar)
{
    initNodeHitStatus();
    other->initNodeHitStatus();
 ...
 */
void
ComposedEdge::dispatchPerimeter(double &partConsidered) const
{
    for (std::list<ElementaryEdge *>::const_iterator iter = _sub_edges.begin(); iter != _sub_edges.end(); iter++)
    {
 ...
 */
void
ComposedEdge::dispatchPerimeterExcl(double &partConsidered, double &commonPart) const
{
    for (std::list<ElementaryEdge *>::const_iterator iter = _sub_edges.begin(); iter != _sub_edges.end(); iter++)
    {
        TypeOfEdgeLocInPolygon loc = (*iter)->getLoc();
...
void
ComposedEdge::getAllNodes(std::set<Node *> &output) const
{
    std::list<ElementaryEdge *>::const_iterator iter = _sub_edges.begin();
    for (; iter != _sub_edges.end(); iter++) (*iter)->getAllNodes(output);
}

void
ComposedEdge::initNodeHitStatus() const
{
    for (std::list<ElementaryEdge *>::const_iterator iter = _sub_edges.begin(); iter != _sub_edges.end(); iter++)
    {
...
void
ComposedEdge::applySimilarityOnMyNodes(double xBary, double yBary, double dimChar) const
{
    for (std::list<ElementaryEdge *>::const_iterator iter = _sub_edges.begin(); iter != _sub_edges.end(); iter++)
    {
...
void
ComposedEdge::unApplySimilarityOnMyNodes(double xBary, double yBary, double dimChar) const
{
    for (std::list<ElementaryEdge *>::const_iterator iter = _sub_edges.begin(); iter != _sub_edges.end(); iter++)
    {
...
void
ComposedEdge::applySimilarityOnMyNodesIfNotAlreadyHit(double xBary, double yBary, double dimChar) const
{
    for (std::list<ElementaryEdge *>::const_iterator iter = _sub_edges.begin(); iter != _sub_edges.end(); iter++)
    {
...
void
ComposedEdge::unApplySimilarityOnMyNodesIfNotAlreadyHit(double xBary, double yBary, double dimChar) const
{
    for (std::list<ElementaryEdge *>::const_iterator iter = _sub_edges.begin(); iter != _sub_edges.end(); iter++)
    {
...
void
ComposedEdge::initEdgeHitStatus() const
{
    for (std::list<ElementaryEdge *>::const_iterator iter = _sub_edges.begin(); iter != _sub_edges.end(); iter++)
...
void
ComposedEdge::applySimilarityOnMyEdges(double xBary, double yBary, double dimChar) const
{
    for (std::list<ElementaryEdge *>::const_iterator iter = _sub_edges.begin(); iter != _sub_edges.end(); iter++)
...
void
ComposedEdge::unApplySimilarityOnMyEdges(double xBary, double yBary, double dimChar) const
{
    for (std::list<ElementaryEdge *>::const_iterator iter = _sub_edges.begin(); iter != _sub_edges.end(); iter++)
...
void
ComposedEdge::applySimilarityOnMyEdgesIfNotAlreadyHit(double xBary, double yBary, double dimChar) const
{
    for (std::list<ElementaryEdge *>::const_iterator iter = _sub_edges.begin(); iter != _sub_edges.end(); iter++)
...
void
ComposedEdge::unApplySimilarityOnMyEdgesIfNotAlreadyHit(double xBary, double yBary, double dimChar) const
{
    for (std::list<ElementaryEdge *>::const_iterator iter = _sub_edges.begin(); iter != _sub_edges.end(); iter++)
...
void
ComposedEdge::getBarycenter(double *bary, double &weigh) const
{
    weigh = 0.;
    bary[0] = 0.;
    bary[1] = 0.;
 ...
 */
bool
ComposedEdge::isInOrOut(Node *nodeToTest) const
{
    Bounds b;
    b.prepareForAggregation();
    fillBounds(b);
...
/*!
 * This method is close to ComposedEdge::isInOrOut behaviour except that here EPSILON is taken into account to detect if
 * it is IN or OUT. If \a nodeToTest is close to an edge in \a this, true will be returned even if it is outside
 * informatically from \a this.
 *
 * \sa ComposedEdge::isInOrOut
 */
bool
ComposedEdge::isInOrOut2(Node *nodeToTest) const
{
    std::set<IntersectElement> inOutSwitch;
    std::set<Node *> nodes;
    getAllNodes(nodes);
...
double
ComposedEdge::isInOrOutAlg(
    Node *nodeToTest, const std::set<Node *> &nodes, std::set<IntersectElement> &inOutSwitch
) const
{
    // searching for e1
    ...
    std::transform(
        radialDistrib2.begin(),
        radialDistrib2.end(),
        radialDistrib.begin(),
        radialDistrib3.begin(),
        std::minus<double>()
    );
    ...
    for (std::list<ElementaryEdge *>::const_iterator iter4 = _sub_edges.begin(); iter4 != _sub_edges.end(); iter4++)
    {
        ...
        if (val)
        {
            ...
            if (!areOverlapped)
            {
                ...
                for (std::list<IntersectElement>::iterator iter2 = listOfIntesc.begin(); iter2 != listOfIntesc.end();
                     iter2++)
...
bool
ComposedEdge::getDirection() const
{
    throw Exception("ComposedEdge::getDirection : no sense");
}

bool
ComposedEdge::intresincEqCoarse(const Edge *other) const
{
...
void
ComposedEdge::clearAll(std::list<ElementaryEdge *>::iterator startToDel)
{
    for (std::list<ElementaryEdge *>::iterator iter = startToDel; iter != _sub_edges.end(); iter++) delete (*iter);
}
