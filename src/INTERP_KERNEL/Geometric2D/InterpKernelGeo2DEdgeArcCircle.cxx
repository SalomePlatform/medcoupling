...
ArcCArcCIntersector::ArcCArcCIntersector(const EdgeArcCircle &e1, const EdgeArcCircle &e2)
    : SameTypeEdgeIntersector(e1, e2), _dist(0.)
{
}

bool
ArcCArcCIntersector::haveTheySameDirection() const
{
    return (getE1().getAngle() > 0. && getE2().getAngle() > 0.) || (getE1().getAngle() < 0. && getE2().getAngle() < 0.);
}

bool
ArcCArcCIntersector::areColinears() const
{
 ...
 */
void
ArcCArcCIntersector::getPlacements(
 ...
 */
double
ArcCArcCIntersector::getAngle(Node *node) const
{
    return EdgeArcCircle::GetAbsoluteAngleOfNormalizedVect(
        ((*node)[0] - getE1().getCenter()[0]) / getE1().getRadius(),
        ((*node)[1] - getE1().getCenter()[1]) / getE1().getRadius()
    );
}

bool
ArcCArcCIntersector::internalAreColinears(
    const EdgeArcCircle &a1,
    const EdgeArcCircle &a2,
    double &distBetweenCenters,
    double &cst,
    double &radiusL,
    double centerL[2],
    double &radiusB,
...
)
{
    ...
    {  // a1 is the little one ('L') and a2 the big one ('B')
        a1.getCenter(centerL);
        radiusL = a1.getRadius();
        a2.getCenter(centerB);
        radiusB = a2.getRadius();
    }
    else
    {
        a2.getCenter(centerL);
        radiusL = a2.getRadius();
        a1.getCenter(centerB);
        radiusB = a1.getRadius();
    }
...
bool
ArcCArcCIntersector::areArcsOverlapped(const EdgeArcCircle &a1, const EdgeArcCircle &a2)
{
    double radiusL, radiusB;
    double centerL[2], centerB[2];
    double tmp(0.), cst(0.);
    ...
    double phi = EdgeArcCircle::GetAbsoluteAngleOfNormalizedVect(
        (centerL[0] - centerB[0]) / tmp, (centerL[1] - centerB[1]) / tmp
    );
...
void
ArcCArcCIntersector::areOverlappedOrOnlyColinears(bool &obviousNoIntersection, bool &areOverlapped)
{
    _dist = Node::distanceBtw2Pt(getE1().getCenter(), getE2().getCenter());
    double radius1 = getE1().getRadius();
    double radius2 = getE2().getRadius();
    if (_dist > radius1 + radius2 + QuadraticPlanarPrecision::getPrecision() ||
        _dist + std::min(radius1, radius2) + QuadraticPlanarPrecision::getPrecision() < std::max(radius1, radius2))
    {
        obviousNoIntersection = true;
        areOverlapped = false;
        return;
    }
    if (areArcsOverlapped(
            getE1(), getE2()
        ))  //(Node::areDoubleEquals(_dist,0.) && Node::areDoubleEquals(radius1,radius2))
    {
...
*/
std::list<IntersectElement>
ArcCArcCIntersector::getIntersectionsCharacteristicVal() const
{
    ...
    double radius2 = getE2().getRadius();
    double d1_1 =
        (_dist * _dist - radius2 * radius2 + radius1 * radius1) / (2. * _dist);  // computation of 'x' on wolfram
    double u[2];  // u is normalized vector from center1 to center2.
    u[0] = (center2[0] - center1[0]) / _dist;
    u[1] = (center2[1] - center1[1]) / _dist;
    ...
    if (!Node::areDoubleEquals(d1_1y, 0))
    {
        ...
        v1[0] = u[0] * d1_1 - u[1] * d1_1y;
        v1[1] = u[1] * d1_1 + u[0] * d1_1y;
        v2[0] = u[0] * d1_1 + u[1] * d1_1y;
        v2[1] = u[1] * d1_1 - u[0] * d1_1y;
        Node *node1 = new Node(center1[0] + v1[0], center1[1] + v1[1]);
        node1->declareOn();
        Node *node2 = new Node(center1[0] + v2[0], center1[1] + v2[1]);
        node2->declareOn();
        ...
        double v3[2], v4[2];
        v3[0] = center1[0] - center2[0] + v1[0];
        v3[1] = center1[1] - center2[1] + v1[1];
        v4[0] = center1[0] - center2[0] + v2[0];
        v4[1] = center1[1] - center2[1] + v2[1];
    ...
    else
    {
        ...
        v1[0] = d1_1 * u[0];
        v1[1] = d1_1 * u[1];
        v2[0] = center1[0] - center2[0] + v1[0];
        v2[1] = center1[1] - center2[1] + v1[1];
        ...
        Node *node = new Node(center1[0] + d1_1 * u[0], center1[1] + d1_1 * u[1]);
        node->declareOnTangent();
...
ArcCSegIntersector::ArcCSegIntersector(const EdgeArcCircle &e1, const EdgeLin &e2, bool reverse)
    : CrossTypeEdgeIntersector(e1, e2, reverse), _deltaRoot_div_dr(0.), _i1S2E(false), _i1E2E(false)
{
    ...
    _cross = ((*(_e2.getStartNode()))[0] - center[0]) * ((*(_e2.getEndNode()))[1] - center[1]) -
...
*/
void
ArcCSegIntersector::areOverlappedOrOnlyColinears(bool &obviousNoIntersection, bool &areOverlapped)
{
    ...
    // Ah ah: we will be taking a square root later. If we want the user to be able to use an epsilon finer than 1.0e-8,
    // then we need to prevent ourselves going below machine precision (typ. 1.0e-16 for double).
    ...
    // Then 2*deltaRoot_div_dr is the distance between the two intersection points of the line with the circle. This is
    // what we compare to eps. We compute it in such a way that it can be used in boolean tests too (a very negative
    // value means we're far apart from intersection)
 ...
 */
bool
ArcCSegIntersector::areColinears() const
{
    return false;
}

void
ArcCSegIntersector::getPlacements(
    Node *start, Node *end, TypeOfLocInEdge &whereStart, TypeOfLocInEdge &whereEnd, MergePoints &commonNode
) const
{
    throw Exception(
        "Internal error. Should never been called : no overlapping possible between arc of circle and a segment."
    );
}

std::list<IntersectElement>
ArcCSegIntersector::getIntersectionsCharacteristicVal() const
{
    std::list<IntersectElement> ret;
    const double *center = getE1().getCenter();
    if (!(2 * fabs(_deltaRoot_div_dr) <
          QuadraticPlanarPrecision::getPrecision()))  // see comments in areOverlappedOrOnlyColinears()
    {                                                 // Two intersection nodes
        // -> if a common node found, there is a chance that this is the only one (i.e. second intersection point is
        // outside e1 and e2)
        if (_earlyInter)
        {
            ...
            double tang[2] = {
                -(n[1] - center[1]), n[0] - center[0]
            };  // (-y, x) is the tangent vector in the trigo direction with (x,y) = (center->node)
        ...
        Node *intersect1 = new Node(x1, y1);
        intersect1->declareOn();
        ...
        Node *intersect2 = new Node(x2, y2);
        intersect2->declareOn();
        ...
        if (!isN1)
        {
            ...
            ret.push_back(IntersectElement(
                getE1().getCharactValue(*intersect1),
                getE2().getCharactValue(*intersect1),
                i1_1S,
                i1_1E,
                i1_2S,
                i1_2E,
        ...
        if (!isN2)
        {
            ...
            ret.push_back(IntersectElement(
                getE1().getCharactValue(*intersect2),
                getE2().getCharactValue(*intersect2),
                i2_1S,
                i2_1E,
                i2_2S,
                i2_2E,
    ...
    else  // tangent intersection
    {
        ...
        Node *intersect3 = new Node(x, y);
        intersect3->declareOnTangent();
        ...
        ret.push_back(IntersectElement(
            _e1.getCharactValue(*intersect3),
            _e2.getCharactValue(*intersect3),
            i_1S,
            i_1E,
            i_2S,
            i_2E,
...
EdgeArcCircle::EdgeArcCircle(std::istream &lineInXfig)
{
    ...
    std::string tmpS;
    for (unsigned i = 0; i < NB_OF_SKIP_FIELDS; i++) lineInXfig >> tmpS;
 ...
 */
EdgeArcCircle::EdgeArcCircle(
    Node *start, Node *end, const double *center, double radius, double angle0, double deltaAngle, bool direction
)
    : Edge(start, end, direction), _angle0(angle0), _angle(deltaAngle), _radius(radius)
{
    _center[0] = center[0];
    _center[1] = center[1];
    updateBounds();
...
void
EdgeArcCircle::changeMiddle(Node *newMiddle)
{
...
void
EdgeArcCircle::applySimilarity(double xBary, double yBary, double dimChar)
{
...
void
EdgeArcCircle::unApplySimilarity(double xBary, double yBary, double dimChar)
{
 ...
 * 'offset' is typically the number of nodes already existing in global 2D curve mesh. Additional coords 'addCoo' ids
 * will be put after the already existing.
 */
void
EdgeArcCircle::tesselate(
    const mcIdType *conn, mcIdType offset, double eps, std::vector<mcIdType> &newConn, std::vector<double> &addCoo
) const
{
    ...
    for (int i = 1; i < nbOfSubDiv; i++, offset2++)
    {
        ...
        addCoo.push_back(_center[0] + _radius * cos(angle));
        addCoo.push_back(_center[1] + _radius * sin(angle));
    }
    newConn.push_back(conn[1]);
}

EdgeArcCircle *
EdgeArcCircle::BuildFromNodes(Node *start, Node *middle, Node *end)
{
    EdgeLin *e1, *e2;
 ...
 */
double
EdgeArcCircle::GetAbsoluteAngle(const double *vect, double &normVect)
{
 ...
 */
double
EdgeArcCircle::GetAbsoluteAngleOfNormalizedVect(double ux, double uy)
{
    return atan2(uy, ux);
}

void
EdgeArcCircle::GetArcOfCirclePassingThru(
    const double *start,
    const double *middle,
    const double *end,
    double *center,
...
)
{
    ...
    radius =
        SafeSqrt((start[0] - center[0]) * (start[0] - center[0]) + (start[1] - center[1]) * (start[1] - center[1]));
    angleInRad0 = GetAbsoluteAngleOfNormalizedVect((start[0] - center[0]) / radius, (start[1] - center[1]) / radius);
    double angleInRadM =
        GetAbsoluteAngleOfNormalizedVect((middle[0] - center[0]) / radius, (middle[1] - center[1]) / radius);
    angleInRad = GetAbsoluteAngleOfNormalizedVect(
        ((start[0] - center[0]) * (end[0] - center[0]) + (start[1] - center[1]) * (end[1] - center[1])) /
            (radius * radius),
...
void
EdgeArcCircle::dumpInXfigFile(std::ostream &stream, bool direction, int resolution, const Bounds &box) const
{
    stream << "5 1 0 1 ";
 ...
 */
double
EdgeArcCircle::getAreaOfZone() const
{
    return -_radius * _radius * (sin(_angle) - _angle) / 2. +
           ((*_start)[0] - (*_end)[0]) * ((*_start)[1] + (*_end)[1]) / 2.;
}

double
EdgeArcCircle::getCurveLength() const
{
    return fabs(_angle * _radius);
}

void
EdgeArcCircle::getBarycenter(double *bary) const
{
 ...
 */
void
EdgeArcCircle::getBarycenterOfZone(double *bary) const
{
    double x0 = _center[0];
    double y0 = _center[1];
    ...
    bary[0] = _radius * x0 * y0 * (tmp4 - tmp3) +
              _radius * _radius *
                  (y0 * (cos(2 * _angle0) - cos(2 * angle1)) / 4. +
                   x0 * (_angle / 2. + (sin(2. * _angle0) - sin(2. * angle1)) / 4.)) +
              tmp2 * (tmp1 * tmp1 * tmp1 - tmp0 * tmp0 * tmp0) / 3.;
    bary[1] = y0 * y0 * _radius * (tmp4 - tmp3) / 2. +
              _radius * _radius * y0 * (_angle / 2. + (sin(2. * _angle0) - sin(2. * angle1)) / 4.) +
 ...
 */
void
EdgeArcCircle::getMiddleOfPoints(const double *p1, const double *p2, double *mid) const
{
    double dx1((p1[0] - _center[0]) / _radius), dy1((p1[1] - _center[1]) / _radius),
        dx2((p2[0] - _center[0]) / _radius), dy2((p2[1] - _center[1]) / _radius);
    ...
    if (_angle > 0.)
    {
        myDelta1 = myDelta1 > -QuadraticPlanarPrecision::getPrecision() ? myDelta1 : myDelta1 + 2. * M_PI;
        myDelta2 = myDelta2 > -QuadraticPlanarPrecision::getPrecision() ? myDelta2 : myDelta2 + 2. * M_PI;
    }
    else
    {
        myDelta1 = myDelta1 < QuadraticPlanarPrecision::getPrecision() ? myDelta1 : myDelta1 - 2. * M_PI;
...
/**
 * Compute the "middle" of two points on the arc of circle.
 * Walk on the circle from p1 to p2 using the rotation direction indicated by this->_angle (i.e. by the orientation of
 * the arc). This function is sensitive to the ordering of p1 and p2.
 ...
 */
void
EdgeArcCircle::getMiddleOfPointsOriented(const double *p1, const double *p2, double *mid) const
{
    double dx1((p1[0] - _center[0]) / _radius), dy1((p1[1] - _center[1]) / _radius),
        dx2((p2[0] - _center[0]) / _radius), dy2((p2[1] - _center[1]) / _radius);
    double angle1(GetAbsoluteAngleOfNormalizedVect(dx1, dy1)), angle2(GetAbsoluteAngleOfNormalizedVect(dx2, dy2));
 ...
 */
bool
EdgeArcCircle::isIn(double characterVal) const
{
    return IsIn2Pi(_angle0, _angle, characterVal);
}

Node *
EdgeArcCircle::buildRepresentantOfMySelf() const
{
    return new Node(
        _center[0] + _radius * cos(_angle0 + _angle / 2.), _center[1] + _radius * sin(_angle0 + _angle / 2.)
    );
}
 ...
 */
bool
EdgeArcCircle::isLower(double val1, double val2) const
{
    double myDelta1 = val1 - _angle0;
    double myDelta2 = val2 - _angle0;
    if (_angle > 0.)
    {
        myDelta1 = myDelta1 > -(_radius * QuadraticPlanarPrecision::getPrecision())
                       ? myDelta1
                       : myDelta1 + 2. * M_PI;  // in some cases val1 or val2 are so close to angle0 that myDelta is
 ...
 */
double
EdgeArcCircle::getCharactValue(const Node &node) const
{
...
double
EdgeArcCircle::getCharactValueBtw0And1(const Node &node) const
{
    double dx = (node[0] - _center[0]) / _radius;
    double dy = (node[1] - _center[1]) / _radius;
...
double
EdgeArcCircle::getDistanceToPoint(const double *pt) const
{
...
bool
EdgeArcCircle::isNodeLyingOn(const double *coordOfNode) const
{
 ...
 */
bool
EdgeArcCircle::IsIn2Pi(double start, double delta, double angleIn)
{
...
/*!
 * Given the arc 'a' defined by 'start' angle and a 'delta' [-Pi;Pi] states for the angle 'angleIn' [-Pi;Pi] if it owns
 * or not 'a'.
 */
bool
EdgeArcCircle::IsAngleNotIn(double start, double delta, double angleIn)
{
    double tmp = start;
    if (tmp < 0.)
        tmp += 2 * M_PI;
...
void
EdgeArcCircle::updateBounds()
{
    _bounds.setValues(
        std::min((*_start)[0], (*_end)[0]),
        std::max((*_start)[0], (*_end)[0]),
        std::min((*_start)[1], (*_end)[1]),
        std::max((*_start)[1], (*_end)[1])
    );
