...
class MEDCouplingPickleTest(unittest.TestCase):
    ...
    def test1(self):
        ...
        x = DataArrayDouble(10, 1)
        x.iota()
        x.rearrange(2)
        x.setInfoOnComponents(["aa", "bbb"])
        x.setName("toto")
        ...
        x = DataArrayDouble(1200)
        x.iota()
        x.setInfoOnComponents(["aa"])
        x.setName("titi")
        pickled = pickle.dumps(x, pickle.HIGHEST_PROTOCOL)
        xx = pickle.loads(pickled)
        self.assertTrue(xx.isEqual(x, 1e-16))
    ...
    def test5(self):
        ...
        arrX = DataArrayDouble(10)
        arrX.iota()
        arrX.setInfoOnComponents(["aa"])
        arrY = DataArrayDouble(5)
        arrY.iota()
        arrY.setInfoOnComponents(["bbb"])
        arrZ = DataArrayDouble(7)
        arrZ.iota()
        arrZ.setInfoOnComponents(["cccc"])
        #
        m = MEDCouplingCMesh()
    ...
    def test8(self):
        ...
        m = MEDCouplingCMesh()
        m.setCoords(arrX, arrY, arrZ)
        mesh3D = m.buildUnstructured()
        del m
        #
        m = MEDCouplingCMesh()
        m.setCoords(arrX, arrY)
        mesh2D = m.buildUnstructured()
        del m
        #
        mesh2D.setCoords(mesh3D.getCoords())
        mesh = MEDCouplingMappedExtrudedMesh(mesh3D, mesh2D, 0)
        del mesh3D, mesh2D
    ...
    def test17(self):
        ...
        f.setMesh(m)
        f.setArray(f2.getArray().convertToFloatArr())
