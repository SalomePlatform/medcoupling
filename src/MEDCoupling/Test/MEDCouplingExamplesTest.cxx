...
CppExample_MEDCouplingFieldDouble_WriteVTK()
{
    ...
    MCAuto<MEDCouplingFieldDouble> field1 = mesh1->getMeasureField(true);
    MCAuto<MEDCouplingFieldDouble> field2 = mesh1->buildOrthogonalField();
    MCAuto<MEDCouplingFieldDouble> field3 = mesh1->fillFromAnalytic(ON_CELLS, 1, "x");
...
CppExample_MEDCouplingFieldDouble_MergeFields()
{
    ...
    // field1
    MCAuto<MEDCouplingFieldDouble> field1 = mesh1->fillFromAnalytic(ON_CELLS, 1, "x");

    // mesh2 and field2
    MCAuto<MEDCouplingFieldDouble> field2 = field1->cloneWithMesh(true);
    double vec[1] = {5.};
    ...
    MCAuto<MEDCouplingFieldDouble> field3 = MEDCouplingFieldDouble::MergeFields(field1, field2);
    std::vector<const MEDCouplingFieldDouble *> fields(2);
    fields[0] = field1;
    fields[1] = field2;
    MCAuto<MEDCouplingFieldDouble> field4 = MEDCouplingFieldDouble::MergeFields(fields);
    //! [CppSnippet_MEDCouplingFieldDouble_MergeFields_1]
}
...
CppExample_MEDCouplingFieldDouble_substractInPlaceDM()
{
    ...
    // mesh 2
    MCAuto<MEDCouplingUMesh> mesh2 = (MEDCouplingUMesh *)mesh1->deepCopy();
...
CppExample_MEDCouplingFieldDouble_applyFunc_same_nb_comp()
{
    ...
    MCAuto<MEDCouplingFieldDouble> field = MEDCouplingFieldDouble::New(MEDCoupling::ON_CELLS);
...
CppExample_MEDCouplingFieldDouble_fillFromAnalytic3()
{
    ...
    //
    MCAuto<DataArrayDouble> bc = mesh->computeCellCenterOfMass();  // func is applied to barycenters of cells
...
bool
getNewValue(const double *pos, double *res)
{
    res[0] = pos[0];
    res[1] = pos[1];
...
CppExample_MEDCouplingFieldDouble_getValueOn_time()
{
    ...
    MCAuto<MEDCouplingFieldDouble> field = MEDCouplingFieldDouble::New(MEDCoupling::ON_CELLS, MEDCoupling::LINEAR_TIME);
    field->setMesh(mesh);
...
CppExample_MEDCouplingFieldDouble_getValueOnMulti()
{
    ...
    MCAuto<MEDCouplingCMesh> mesh = MEDCouplingCMesh::New();
    mesh->setCoords(coordsArr, coordsArr);
    MCAuto<MEDCouplingFieldDouble> field = mesh->fillFromAnalytic(MEDCoupling::ON_CELLS, 1, "x+y");
    //! [CppSnippet_MEDCouplingFieldDouble_getValueOnMulti_1]
    //! [CppSnippet_MEDCouplingFieldDouble_getValueOnMulti_2]
    // field values are located at cell barycenters
    MCAuto<DataArrayDouble> bc = mesh->computeCellCenterOfMass();
    MCAuto<DataArrayDouble> valArray = field->getValueOnMulti(bc->getConstPointer(), bc->getNumberOfTuples());
...
CppExample_MEDCouplingFieldDouble_renumberNodes()
{
    ...
    MCAuto<MEDCouplingFieldDouble> field = mesh->fillFromAnalytic(MEDCoupling::ON_NODES, 2, "IVec*x+JVec*y");
...
CppExample_MEDCouplingFieldDouble_renumberCells()
{
    ...
    MCAuto<MEDCouplingFieldDouble> field = mesh->fillFromAnalytic(MEDCoupling::ON_CELLS, 2, "IVec*x+JVec*y");
    const DataArrayDouble *values = field->getArray();
    MCAuto<DataArrayDouble> bc = mesh->computeCellCenterOfMass();
...
CppExample_MEDCouplingFieldDouble_buildNewTimeReprFromThis()
{
    ...
    MCAuto<MEDCouplingCMesh> mesh = MEDCouplingCMesh::New();
    mesh->setCoords(coordsArr, coordsArr);
    MCAuto<MEDCouplingFieldDouble> field1 = mesh->fillFromAnalytic(MEDCoupling::ON_NODES, 1, "x+y");
    ...
    MCAuto<MEDCouplingFieldDouble> field2 = field1->buildNewTimeReprFromThis(MEDCoupling::NO_TIME, false);
...
CppExample_MEDCouplingMesh_fillFromAnalytic2()
{
    ...
    const char func[] = "IVec * b + JVec * a + KVec * sqrt( a*a + b*b ) + 10";
    MCAuto<MEDCouplingFieldDouble> field = mesh->fillFromAnalyticCompo(MEDCoupling::ON_CELLS, 3, func);
    //! [CppSnippet_MEDCouplingMesh_fillFromAnalytic2_2]
...
CppExample_MEDCouplingMesh_fillFromAnalytic()
{
    ...
    const char func[] = "IVec * b + JVec * a + KVec * sqrt( a*a + b*b ) + 10";
    MCAuto<MEDCouplingFieldDouble> field = mesh->fillFromAnalytic(MEDCoupling::ON_CELLS, 3, func);
    //! [CppSnippet_MEDCouplingMesh_fillFromAnalytic_2]
...
CppExample_MEDCouplingUMesh_areCellsIncludedIn()
{
    ...
    MCAuto<MEDCouplingUMesh> mesh2 = (MEDCouplingUMesh *)mesh1->buildPartOfMySelf(cells2, cells2 + 3, true);
    //! [CppSnippet_MEDCouplingUMesh_areCellsIncludedIn_2]
...
CppExample_MEDCouplingUMesh_findAndCorrectBadOriented3DExtrudedCells()
{
    ...
    MCAuto<DataArrayIdType> fixedCells = mesh->findAndCorrectBadOriented3DExtrudedCells();
...
CppExample_MEDCouplingUMesh_buildPartOrthogonalField()
{
    ...
    MCAuto<MEDCouplingFieldDouble> vecField = mesh->buildPartOrthogonalField(part, part + 4);
...
CppExample_MEDCouplingUMesh_getPartMeasureField()
{
    ...
    const mcIdType part[4] = {1, 2, 3, 4};  // cell #0 is omitted
    MCAuto<DataArrayDouble> areaArr = mesh->getPartMeasureField(isAbs, part, part + 4);
    ...
    MCAuto<DataArrayDouble> baryCenters = mesh->getPartBarycenterAndOwner(cellIds, cellIds + 4);
...
CppExample_MEDCouplingUMesh_getCellsInBoundingBox()
{
    ...
    MCAuto<DataArrayIdType> cellIdsArr = mesh->getCellsInBoundingBox(bbox, 0.0);
...
CppExample_MEDCouplingUMesh_mergeNodes()
{
    ...
    MCAuto<DataArrayIdType> arr = mesh->mergeNodes(0.004, areNodesMerged, newNbOfNodes);
    ...
    CPPUNIT_ASSERT(
        fabs(baryCoords2[1] - coordsArr->getIJ(0, 1)) > 1e-4
...
CppExample_MEDCouplingUMesh_checkDeepEquivalWith()
{
    ...
    CPPUNIT_ASSERT_THROW(
        mesh1->checkDeepEquivalOnSameNodesWith(mesh2, cellCompPol, 0.002, cOld2New), INTERP_KERNEL::Exception
    );
...
CppExample_DataArrayInt_invertArrayO2N2N2O()
{
    ...
    for (mcIdType i = 0; i < 6; i++) CPPUNIT_ASSERT_EQUAL(expected1[i], da2->getIJ(i, 0));
    //! [CppSnippet_DataArrayInt_invertArrayO2N2N2O_1]
    da->decrRef();
    da2->decrRef();
}
...
CppExampleFieldDoubleBuildSubPart1()
{
    ...
    MEDCoupling::MEDCouplingUMesh *m2C =
        dynamic_cast<MEDCoupling::MEDCouplingUMesh *>(const_cast<MEDCoupling::MEDCouplingMesh *>(f2->getMesh()));
    ...
    for (mcIdType i = 0; i < 12; i++) CPPUNIT_ASSERT_DOUBLES_EQUAL(expected2[i], m2C->getCoords()->getIJ(0, i), 1.e-12);
...
CppSnippetUMeshStdBuild1()
{
    ...
    double coords[27] = {-0.3, -0.3, 0.,  0.2, -0.3, 0.,  0.7, -0.3, 0.,  -0.3, 0.2, 0.,  0.2, 0.2,
...
CppSnippetCMeshStdBuild1()
{
    ...
    arrX = MEDCoupling::DataArrayDouble::New();
    arrX->alloc(9, 1);
    std::copy(XCoords, XCoords + 9, arrX->getPointer());
    arrX->setInfoOnComponent(0, "X [m]");
    arrY = MEDCoupling::DataArrayDouble::New();
    arrY->alloc(7, 1);
    std::copy(YCoords, YCoords + 7, arrY->getPointer());
    arrY->setInfoOnComponent(0, "Y [m]");
...
CppSnippetUMeshAdvBuild1()
{
    ...
    nodalConn->decrRef();   // nodalConn DataArrayIdType instance is owned by mesh after call to setConnectivity method.
                            // No more need here -> decrRef()
    nodalConnI->decrRef();  // nodalConnI DataArrayIdType instance is owned by mesh after call to setConnectivity
...
CppSnippetDataArrayBuild1()
{
    ...
    double coords[3 * nbOfNodes] = {2.,  3.,  4.,  3.,  4.,  5.,  4.,  5.,  6.,  5.,  6.,  7.,
    ...
    coordsArrCpy->setIJ(0, 0, 1000.);
    CPPUNIT_ASSERT(
        coordsArrCpy->isEqual(*coordsArr, 1e-12)
    );  // coordsArr and coordsArrCpy have been modified simultaneously
    //! [CppSnippetDataArrayBuild1_9]
...
CppSnippetFieldDoubleBuild1()
{
    ...
    MEDCoupling::MEDCouplingFieldDouble *f1 =
        mesh->fillFromAnalytic(MEDCoupling::ON_CELLS, 1, "x*x+y*y*3+2.*x");  // f1 is scalar
    MEDCoupling::MEDCouplingFieldDouble *f2 =
        mesh->fillFromAnalytic(MEDCoupling::ON_CELLS, 1, "cos(x+y/x)");  // f2 is scalar too
    MEDCoupling::MEDCouplingFieldDouble *f2bis =
        mesh->fillFromAnalytic(MEDCoupling::ON_CELLS, 2, "x*x*IVec+3*y*JVec");  // f2bis is a vectors field
...
CppSnippetFieldDoubleBuild2()
{
    ...
    MEDCoupling::MEDCouplingFieldDouble *fieldOnNodes =
        MEDCoupling::MEDCouplingFieldDouble::New(MEDCoupling::ON_NODES, MEDCoupling::NO_TIME);
...
CppSnippetFieldDoubleBuild4()
{
    ...
    MEDCoupling::MEDCouplingFieldDouble *fieldOnNodes =
        MEDCoupling::MEDCouplingFieldDouble::New(MEDCoupling::ON_NODES, MEDCoupling::CONST_ON_TIME_INTERVAL);
