...
int
Voronizer1D::getDimension() const
{
    return 1;
}

int
Voronizer2D::getDimension() const
{
    return 2;
}

int
Voronizer3D::getDimension() const
{
    return 3;
}

MCAuto<MEDCouplingUMesh>
ComputeBigCellFrom(const double pt1[2], const double pt2[2], const std::vector<double> &bbox, double eps)
{
    static const double FACT = 1.2;
    MCAuto<MEDCouplingCMesh> m(MEDCouplingCMesh::New());
    MCAuto<DataArrayDouble> arr1(DataArrayDouble::New());
    arr1->alloc(2, 1);
    arr1->setIJ(0, 0, bbox[0]);
    arr1->setIJ(1, 0, bbox[1]);
    MCAuto<DataArrayDouble> arr2(DataArrayDouble::New());
    arr2->alloc(2, 1);
    arr2->setIJ(0, 0, bbox[2]);
    arr2->setIJ(1, 0, bbox[3]);
    ...
    MCAuto<MEDCouplingUMesh> line(MEDCouplingUMesh::New("line", 1));
    {
        ...
        coo->alloc(2, 2);
        coo->setIJ(0, 0, middle[0] - 2. * l * v[0]);
        coo->setIJ(0, 1, middle[1] - 2. * l * v[1]);
        coo->setIJ(1, 0, middle[0] + 2. * l * v[0]);
        coo->setIJ(1, 1, middle[1] + 2. * l * v[1]);
        line->setCoords(coo);
    }
    line->allocateCells();
...
MCAuto<MEDCouplingUMesh>
MergeVorCells2D(MEDCouplingUMesh *p, double eps, bool isZip)
{
    MCAuto<DataArrayIdType> edgeToKeep;
    MCAuto<MEDCouplingUMesh> p0;
    {
        MCAuto<DataArrayIdType> d(DataArrayIdType::New()), di(DataArrayIdType::New()), rd(DataArrayIdType::New()),
            rdi(DataArrayIdType::New());
...
MCAuto<MEDCouplingUMesh>
MergeVorCells(const std::vector<MCAuto<MEDCouplingUMesh> > &vcs, double eps)
{
 ...
 */
MCAuto<MEDCouplingUMesh>
SimplifyPolygon(const MEDCouplingUMesh *m, double eps)
{
    ...
    {
        mcIdType prev(conn[(i + nbPtsInPolygon - 1) % nbPtsInPolygon + 1]), current(conn[i % nbPtsInPolygon + 1]),
            zeNext(conn[(i + 1) % nbPtsInPolygon + 1]);
        double a[3] =
            {
...
MCAuto<MEDCouplingUMesh>
MergeVorCells3D(const std::vector<MCAuto<MEDCouplingUMesh> > &vcs, double eps)
{
    std::size_t sz(vcs.size());
    if (sz < 1)
        throw INTERP_KERNEL::Exception("MergeVorCells : len of input vec expected to be >= 1 !");
    ...
    MCAuto<DataArrayIdType> comm, commI;
    {
        ...
        comm = a;
        commI = b;
        // comm=DataArrayIdType::New(); comm->alloc(0,1); commI=DataArrayIdType::New(); commI->alloc(1,1);
        // commI->setIJ(0,0,0);
    }
    ...
    for (mcIdType i = 0; i < commI->getNumberOfTuples() - 1; i++, jj++)
    {
        if (jj != 0)
            conn.push_back(-1);
        MCAuto<MEDCouplingUMesh> tmp(skinOfRes->buildPartOfMySelf(
            comm->begin() + commI->getIJ(i, 0), comm->begin() + commI->getIJ(i + 1, 0), true
        ));
...
MCAuto<MEDCouplingUMesh>
MergeVorCells1D(const std::vector<MCAuto<MEDCouplingUMesh> > &vcs, double eps)
{
    ...
    double a0, b0, a1, b1;
    {
        ...
        a0 = coordPtr[connPtr[1]];
        b0 = coordPtr[connPtr[2]];
    }
    {
        const mcIdType *connPtr(vcs[1]->getNodalConnectivity()->begin());
        const double *coordPtr(vcs[1]->getCoords()->begin());
        a1 = coordPtr[connPtr[1]];
        b1 = coordPtr[connPtr[2]];
    }
    MCAuto<MEDCouplingUMesh> ret(MEDCouplingUMesh::New("", 1));
    ret->allocateCells();
    ret->insertNextCell(INTERP_KERNEL::NORM_SEG2, 2, CONN_SEG2_DFT);
    MCAuto<DataArrayDouble> coo(DataArrayDouble::New());
    coo->alloc(2, 1);
    ret->setCoords(coo);
    if (fabs(b0 - a1) < eps)
...
MCAuto<MEDCouplingUMesh>
MEDCoupling::Voronizer1D::doIt(const MEDCouplingUMesh *m, const DataArrayDouble *points, double eps) const
{
    static const mcIdType CONN_SEG2_DFT[2] = {0, 1};
    if (!m || !points)
    ...
    for (mcIdType i = 1; i < nbPts; i++)
    {
        ...
        for (std::vector<mcIdType>::const_iterator it = polygsToIterOn.begin(); it != polygsToIterOn.end(); it++)
        {
            ...
            double a, b;
            {
                ...
                a = coordPtr[connPtr[1]];
                b = coordPtr[connPtr[2]];
            }
            double pol0[2], pol1[2];
            MCAuto<DataArrayDouble> t0(DataArrayDouble::New());
            t0->alloc(3, 1);
            t0->setIJ(0, 0, zept);
            t0->setIJ(1, 0, mid);
            t0->setIJ(2, 0, seed);
            ...
            MCAuto<MEDCouplingUMesh> modifiedCell(MEDCouplingUMesh::New("", 1));
            modifiedCell->allocateCells();
            MCAuto<DataArrayDouble> coo1(DataArrayDouble::New());
            coo1->alloc(2, 1);
            coo1->setIJ(0, 0, pol1[0]);
            coo1->setIJ(1, 0, pol1[1]);
            modifiedCell->setCoords(coo1);
            modifiedCell->insertNextCell(INTERP_KERNEL::NORM_SEG2, 2, CONN_SEG2_DFT);
            //
            MCAuto<MEDCouplingUMesh> newVorCell(MEDCouplingUMesh::New("", 1));
            newVorCell->allocateCells();
            MCAuto<DataArrayDouble> coo2(DataArrayDouble::New());
            coo2->alloc(2, 1);
            coo2->setIJ(0, 0, pol0[0]);
            coo2->setIJ(1, 0, pol0[1]);
            newVorCell->setCoords(coo2);
            newVorCell->insertNextCell(INTERP_KERNEL::NORM_SEG2, 2, CONN_SEG2_DFT);
            //
...
MCAuto<MEDCouplingUMesh>
MEDCoupling::Voronizer2D::doIt(const MEDCouplingUMesh *m, const DataArrayDouble *points, double eps) const
{
    if (!m || !points)
    ...
    for (mcIdType i = 1; i < nbPts; i++)
    {
        ...
        std::set<mcIdType> elemsToDo, elemsDone;
        elemsToDo.insert(polygsToIterOn[0]);
        std::vector<MCAuto<MEDCouplingUMesh> > newVorCells;
        while (!elemsToDo.empty())
        {
            mcIdType poly(*elemsToDo.begin());
            elemsToDo.erase(elemsToDo.begin());
            elemsDone.insert(poly);
            ...
            for (mcIdType j = 0; j < newCoords->getNumberOfTuples(); j++, cPtr += 2)
            {
                ...
                std::set_union(
                    ...
                    tmp2.begin(),
                    tmp2.end(),
                    std::inserter(newElementsToDo, newElementsToDo.begin())
                );
...
MCAuto<MEDCouplingUMesh>
Split3DCellInParts(const MEDCouplingUMesh *m, const double pt[3], const double seed[3], double eps, mcIdType tmp[2])
{
...
MCAuto<MEDCouplingUMesh>
MEDCoupling::Voronizer3D::doIt(const MEDCouplingUMesh *m, const DataArrayDouble *points, double eps) const
{
