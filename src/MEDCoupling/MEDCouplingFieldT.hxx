...
class MEDCouplingFieldT : public MEDCouplingField
{
   protected:
    ...
    MEDCouplingFieldT(
        const MEDCouplingField &other, MEDCouplingTimeDiscretizationTemplate<T> *timeDiscr, bool deepCopy = true
    );
   ...
   public:
    ...
    MEDCOUPLING_EXPORT typename Traits<T>::FieldType *buildSubPart(
        const mcIdType *partBg, const mcIdType *partEnd
    ) const;
    MEDCOUPLING_EXPORT typename Traits<T>::FieldType *buildSubPartRange(
        mcIdType begin, mcIdType end, mcIdType step
    ) const;
    ...
    {
        std::vector<typename Traits<T>::ArrayType *> ret;
        _time_discr->getArrays(ret);
        return ret;
    }
    ...
    MEDCOUPLING_EXPORT virtual bool isEqualWithoutConsideringStr(
        const MEDCouplingFieldT<T> *other, double meshPrec, T valsPrec
    ) const;
    ...
    MEDCOUPLING_EXPORT void serialize(
        DataArrayIdType *&dataInt, std::vector<typename Traits<T>::ArrayType *> &arrays
    ) const;
