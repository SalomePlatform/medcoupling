...
MEDCouplingSkyLineArray::New()
{
    return new MEDCouplingSkyLineArray();
}

MEDCouplingSkyLineArray *
MEDCouplingSkyLineArray::New(const std::vector<mcIdType> &index, const std::vector<mcIdType> &value)
{
...
MEDCouplingSkyLineArray *
MEDCouplingSkyLineArray::New(DataArrayIdType *index, DataArrayIdType *value)
{
    MEDCouplingSkyLineArray *ret = new MEDCouplingSkyLineArray();
    ret->set(index, value);
    return ret;
...
MEDCouplingSkyLineArray *
MEDCouplingSkyLineArray::New(const MEDCouplingSkyLineArray &other)
{
    MEDCouplingSkyLineArray *ret = new MEDCouplingSkyLineArray();
 ...
 */
MEDCouplingSkyLineArray *
MEDCouplingSkyLineArray::BuildFromPolyhedronConn(const DataArrayIdType *c, const DataArrayIdType *cI)
{
    using namespace std;
    ...
    if (c->getNbOfElems() != *(cI->end() - 1))
        throw INTERP_KERNEL::Exception(
            "MEDCouplingSkyLineArray::BuildFromDynamicConn: misformatted connectivity (wrong nb of tuples)!"
        );
    for (mcIdType i = 0; i < cI->getNbOfElems(); i++)
    {
        ...
        if (cIP[i] < prev)
            throw INTERP_KERNEL::Exception(
                "MEDCouplingSkyLineArray::BuildFromDynamicConn: misformatted connectivity (indices not monotonic "
                "ascending)!"
            );
        prev = cIP[i];
        if (i != cI->getNbOfElems() - 1)
            if (cP[j] != INTERP_KERNEL::NORM_POLYHED)
                throw INTERP_KERNEL::Exception(
                    "MEDCouplingSkyLineArray::BuildFromDynamicConn: connectivity containing other types than POLYHED!"
                );
    }
 ...
 */
void
MEDCouplingSkyLineArray::convertToPolyhedronConn(MCAuto<DataArrayIdType> &c, MCAuto<DataArrayIdType> &cI) const
{
    ...
    mcIdType sz =
        dsi->accumulate((std::size_t)0) +
        ToIdType(dsi->getNbOfElems());  // think about it: one slot for the type, -1 at the end of each face of the cell
    c->alloc(sz, 1);
...
std::size_t
MEDCouplingSkyLineArray::getHeapMemorySizeWithoutChildren() const
{
    return _index->getHeapMemorySizeWithoutChildren() + _values->getHeapMemorySizeWithoutChildren() +
           _super_index->getHeapMemorySizeWithoutChildren();
}

std::vector<const BigMemoryObject *>
MEDCouplingSkyLineArray::getDirectChildrenWithNull() const
{
...
void
MEDCouplingSkyLineArray::set(DataArrayIdType *index, DataArrayIdType *value)
{
    _index = index;
    _values = value;
    if ((DataArrayIdType *)_index)
        _index->incrRef();
    else
        _index = DataArrayIdType::New();
    if ((DataArrayIdType *)_values)
        _values->incrRef();
    else
        _values = DataArrayIdType::New();
}

void
MEDCouplingSkyLineArray::set3(DataArrayIdType *superIndex, DataArrayIdType *index, DataArrayIdType *value)
{
    _super_index = superIndex;
    if ((DataArrayIdType *)_super_index)
        _super_index->incrRef();
    else
        _super_index = DataArrayIdType::New();
    set(index, value);
}

DataArrayIdType *
MEDCouplingSkyLineArray::getSuperIndexArray() const
{
    return const_cast<MEDCouplingSkyLineArray *>(this)->_super_index;
}

DataArrayIdType *
MEDCouplingSkyLineArray::getIndexArray() const
{
    return const_cast<MEDCouplingSkyLineArray *>(this)->_index;
}

DataArrayIdType *
MEDCouplingSkyLineArray::getValuesArray() const
{
...
void
MEDCouplingSkyLineArray::checkSuperIndex(const std::string &func) const
{
    if (!_super_index->getNbOfElems())
    {
        std::ostringstream oss;
        oss << "MEDCouplingSkyLineArray::" << func
            << ": not a three level SkyLineArray! Method is not available for two-level SkyLineArray.";
        throw INTERP_KERNEL::Exception(oss.str());
    }
}

void
MEDCouplingSkyLineArray::validSuperIndex(const std::string &func, mcIdType superIndex) const
{
...
void
MEDCouplingSkyLineArray::validIndex(const std::string &func, mcIdType idx) const
{
...
void
MEDCouplingSkyLineArray::validSuperIndexAndIndex(const std::string &func, mcIdType superIndex, mcIdType index) const
{
...
std::string
MEDCouplingSkyLineArray::simpleRepr() const
{
    std::ostringstream oss;
    ...
    if (_super_index->getNbOfElems())
    {
        ...
        for (; i != _super_index->end(); ++i) oss << *i << " ";
        oss << std::endl;
    }
 ...
 */
void
MEDCouplingSkyLineArray::thresholdPerPack(
    mcIdType threshold, MCAuto<MEDCouplingSkyLineArray> &left, MCAuto<MEDCouplingSkyLineArray> &right
) const
{
    mcIdType nbPacks(this->getNumberOf());
    MCAuto<DataArrayIdType> lCount(DataArrayIdType::New());
    lCount->alloc(nbPacks, 1);
    lCount->fillWithZero();
    ...
    for (mcIdType i = 0; i < nbPacks; ++i, ++lCountPtr)
    {
        *lCountPtr = ToIdType(
            std::count_if(
                valuesPtr + indexPtr[i],
                valuesPtr + indexPtr[i + 1],
                [threshold](mcIdType elt) { return elt < threshold; }
            )
        );
    }
    ...
    lCount->computeOffsetsFull();
    sizeOfPacks->computeOffsetsFull();
    MCAuto<DataArrayIdType> leftValues(DataArrayIdType::New());
    leftValues->alloc(leftNbOfVal, 1);
    MCAuto<DataArrayIdType> rightValues(DataArrayIdType::New());
    rightValues->alloc(rightNbOfVal, 1);
    ...
    for (mcIdType i = 0; i < nbPacks; ++i)
    {
        std::for_each(
            valuesPtr + indexPtr[i],
            valuesPtr + indexPtr[i + 1],
            [threshold, &rvPtr, &lvPtr](mcIdType elt)
            {
                if (elt < threshold)
                {
                    *lvPtr++ = elt;
    ...
    left = MEDCouplingSkyLineArray::New(lCount, leftValues);
    right = MEDCouplingSkyLineArray::New(sizeOfPacks, rightValues);
}
...
{
    mcIdType nbPacks(this->getNumberOf());
    MCAuto<DataArrayIdType> retIndex(DataArrayIdType::New());
    retIndex->alloc(nbPacks + 1, 1);
    const mcIdType *valuesPtr(this->_values->begin()), *indexPtr(this->_index->begin());
    mcIdType *retIndexPtr(retIndex->getPointer());
    *retIndexPtr = 0;
    ...
    MCAuto<DataArrayIdType> retValues(DataArrayIdType::New());
    retValues->alloc(retIndex->back(), 1);
 ...
 *
 * \throw if either a lenght of not nullptr instances in \a sks is zero or if number of packs of not nullptr instances
 * in \a sks is not the same.
 * \return a newly allocated skyline array that is the aggregated packs of inputs
 */
MEDCouplingSkyLineArray *
MEDCouplingSkyLineArray::AggregatePacks(const std::vector<const MEDCouplingSkyLineArray *> &sks)
{
    ...
    for (auto sk : sks)
    {
        if (sk)
        {
            ...
            {
                std::ostringstream oss;
                oss << MSG << "first not null input ska has " << nbOfPacks << " whereas there is presence of ska with "
                    << curNbPacks << " !";
                throw INTERP_KERNEL::Exception(oss.str());
            }
            sksEff.push_back(sk);
    ...
    {
        std::ostringstream oss;
        oss << MSG << "input vector contains no not nullptr elements !";
        throw INTERP_KERNEL::Exception(oss.str());
    }
    //
    MCAuto<DataArrayIdType> index(DataArrayIdType::New());
    index->alloc(nbOfPacks + 1, 1);
    mcIdType *indexPtr(index->getPointer());
    *indexPtr = 0;
    std::vector<const mcIdType *> indicesIn(
        SkyLineArrayIndexIterator(0, &sksEff), SkyLineArrayIndexIterator(sksEff.size(), &sksEff)
    );
    for (mcIdType packId = 0; packId < nbOfPacks; ++packId, ++indexPtr)
    {
        ...
        std::for_each(
            indicesIn.begin(),
            indicesIn.end(),
            [packId, &nbOfAggPacks](const mcIdType *elt) { nbOfAggPacks += elt[packId + 1] - elt[packId]; }
        );
        indexPtr[1] = indexPtr[0] + nbOfAggPacks;
    }
    mcIdType nbOfTuplesOut(index->back());
    MCAuto<DataArrayIdType> values(DataArrayIdType::New());
    values->alloc(nbOfTuplesOut, 1);
    mcIdType *valuesPtr(values->getPointer());
    // let's go to populate values array
    std::vector<const mcIdType *> valuesIn(
        SkyLineArrayValuesIterator(0, &sksEff), SkyLineArrayValuesIterator(sksEff.size(), &sksEff)
    );
    for (mcIdType packId = 0; packId < nbOfPacks; ++packId)
    {
        ...
        std::for_each(
            valuesIn.begin(),
            valuesIn.end(),
            [packId, &indicesIn, &valuesPtr, &pos](const mcIdType *elt)
            {
                valuesPtr = std::copy(elt + indicesIn[pos][packId], elt + indicesIn[pos][packId + 1], valuesPtr);
                ++pos;
            }
        );
    }
 ...
 */
void
MEDCouplingSkyLineArray::getSimplePackSafe(const mcIdType absolutePackId, std::vector<mcIdType> &pack) const
{
 ...
 */
const mcIdType *
MEDCouplingSkyLineArray::getSimplePackSafePtr(const mcIdType absolutePackId, mcIdType &packSize) const
{
    if (absolutePackId < 0 || absolutePackId >= _index->getNbOfElems())
        throw INTERP_KERNEL::Exception("MEDCouplingSkyLineArray::getPackSafe: invalid index!");
    const mcIdType *iP(_index->begin()), *vP(_values->begin());
 ...
 * \param[out] out a vector of mcIdType, having the same size as superPackIndices and containing for each inspected
 * super-pack the index of the first matching pack, or -1 if none found.
 */
void
MEDCouplingSkyLineArray::findPackIds(
    const std::vector<mcIdType> &superPackIndices,
    const mcIdType *packBg,
    const mcIdType *packEnd,
    std::vector<mcIdType> &out
) const
{
    using namespace std;
 ...
 */
void
MEDCouplingSkyLineArray::deletePack(const mcIdType superIdx, const mcIdType idx)
{
    ...
    _index->reAlloc(nt - 1);
    iP = _index->getPointer();  // better not forget this ...
    for (mcIdType ii = siP[superIdx] + idx; ii < nt - 1; ii++) iP[ii] -= (end - start);

    // _super_index
    for (mcIdType ii = superIdx + 1; ii < _super_index->getNbOfElems(); ii++) (siP[ii])--;
}

void
MEDCouplingSkyLineArray::deleteSimplePack(const mcIdType idx)
{
    ...
    for (mcIdType ii = idx; ii < nt - 1; ii++) iP[ii] += deltaSz;
    _index->reAlloc(nt - 1);
}

void
MEDCouplingSkyLineArray::replaceSimplePacks(
    const DataArrayIdType *idx, const std::vector<const DataArrayIdType *> &packs
)
{
    if (idx->empty())
        return;

    for (const mcIdType *id = idx->begin(); id != idx->end(); id++) validIndex("deleteSimplePacks", *id);
    ...
    mcIdType *iP(_index->getPointer());
    mcIdType nt = _index->getNbOfElems();
    for (mcIdType ii = 0; ii < nt - 1; ii++) valuesByIdx.push_back(std::set<mcIdType>(vP + iP[ii], vP + iP[ii + 1]));
    ...
    mcIdType valSz(0);
    *iP = 0;
    for (std::deque<std::set<mcIdType> >::const_iterator values = valuesByIdx.begin(); values != valuesByIdx.end();
         values++)
    {
...
void
MEDCouplingSkyLineArray::deleteSimplePacks(const DataArrayIdType *idx)
{
    for (auto id = idx->begin(); id != idx->end(); id++) validIndex("deleteSimplePacks", *id);
    ...
    for (std::set<mcIdType>::const_iterator ii = packsToDelete.begin(); ii != packsToDelete.end(); ii++)
    {
        if (end_prec != 0)
        {
            ...
            for (mcIdType i = start_prec; i < *ii; i++) iP[i] -= offset;
        }
    ...
    if (end_prec != 0)
    {
        ...
        for (mcIdType i = start_prec; i < nt; i++) iP[i] -= offset;
    }
    _index->reAlloc(nt - deleted);
}

/**!
 * Insert a new pack in super-pack at index 'superIdx'. The pack is inserted at the end of the pack list of the chosen
 * super-pack.
 */
void
MEDCouplingSkyLineArray::pushBackPack(const mcIdType superIdx, const mcIdType *packBg, const mcIdType *packEnd)
{
    using namespace std;
    ...
    for (mcIdType ii = siP[superIdx + 1] + 2; ii < nt + 1; ii++) iP[ii] += sz;
 ...
 */
void
MEDCouplingSkyLineArray::replaceSimplePack(const mcIdType idx, const mcIdType *packBg, const mcIdType *packEnd)
{
    ...
    // _index
    for (mcIdType ii = idx + 1; ii < _index->getNbOfElems(); ii++) iP[ii] += deltaSz;
}
 ...
 */
void
MEDCouplingSkyLineArray::replacePack(
    const mcIdType superIdx, const mcIdType idx, const mcIdType *packBg, const mcIdType *packEnd
)
{
    ...
    // _index
    for (mcIdType ii = siP[superIdx] + idx + 1; ii < _index->getNbOfElems(); ii++) iP[ii] += deltaSz;
}
