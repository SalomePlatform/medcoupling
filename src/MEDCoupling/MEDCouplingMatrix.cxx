...
DenseMatrix *
DenseMatrix::New(mcIdType nbRows, mcIdType nbCols)
{
    return new DenseMatrix(nbRows, nbCols);
}

DenseMatrix *
DenseMatrix::New(DataArrayDouble *array, mcIdType nbRows, mcIdType nbCols)
{
...
{
    MCAuto<DenseMatrix> ret(
        DenseMatrix::New(const_cast<DataArrayDouble *>(getData()), getNumberOfRows(), getNumberOfCols())
    );
    return ret.retn();
}

std::size_t
DenseMatrix::getHeapMemorySizeWithoutChildren() const
{
    return sizeof(DenseMatrix);
}

std::vector<const BigMemoryObject *>
DenseMatrix::getDirectChildrenWithNull() const
{
 ...
 */
void
DenseMatrix::reBuild(DataArrayDouble *array, mcIdType nbRows, mcIdType nbCols)
{
...
/*!
 * This method does \b not change the content of the data in \a this. It only changes the shape (with a same number of
 * elements in the matrix). If the number of elements needs to be changed call reBuild method instead.
 *
 ...
 */
void
DenseMatrix::reShape(mcIdType nbRows, mcIdType nbCols)
{
    ...
    if (nbRows * nbCols != getNbOfElems())
        throw INTERP_KERNEL::Exception(
            "DenseMatrix::reShape : This method is designed to change only the shape ! Number of elements must remain "
            "the same !"
        );
    if (_nb_rows != nbRows)
    {
...
bool
DenseMatrix::isEqual(const DenseMatrix &other, double eps) const
{
    std::string tmp;
...
bool
DenseMatrix::isEqualIfNotWhy(const DenseMatrix &other, double eps, std::string &reason) const
{
    if (_nb_rows != other._nb_rows)
    {
        std::ostringstream oss;
        oss << "Number of rows differs (" << _nb_rows << "!=" << other._nb_rows << ") !";
        reason += oss.str();
        return false;
    }
    ...
    {
        std::ostringstream oss;
        oss << "Number of cols differs (" << _nb_cols << "!=" << other._nb_cols << ") !";
        reason += oss.str();
        return false;
    }
    std::string tmp1;
...
DataArrayDouble *
DenseMatrix::matVecMult(const DataArrayDouble *vec) const
{
    return MatVecMult(this, vec);
}

DataArrayDouble *
DenseMatrix::MatVecMult(const DenseMatrix *mat, const DataArrayDouble *vec)
{
    if (!mat || !vec)
    ...
    if (vec->getNumberOfTuples() != mat->getNumberOfCols())
        throw INTERP_KERNEL::Exception(
            "DenseMatrix::MatVecMult : Number of columns of this must be equal to number of tuples of vec !"
        );
    MCAuto<DataArrayDouble> ret(DataArrayDouble::New());
    ret->alloc(mat->getNumberOfRows(), 1);
    INTERP_KERNEL::matrixProduct(
        mat->getData()->begin(),
        mat->getNumberOfRows(),
        mat->getNumberOfCols(),
        vec->begin(),
        vec->getNumberOfTuples(),
        1,
        ret->getPointer()
    );
    return ret.retn();
...
DenseMatrix *
DenseMatrix::Add(const DenseMatrix *a1, const DenseMatrix *a2)
{
    if (!a1 || !a2)
...
void
DenseMatrix::addEqual(const DenseMatrix *other)
{
    if (!other)
...
DenseMatrix *
DenseMatrix::Substract(const DenseMatrix *a1, const DenseMatrix *a2)
{
    if (!a1 || !a2)
...
void
DenseMatrix::substractEqual(const DenseMatrix *other)
{
    if (!other)
...
DenseMatrix *
DenseMatrix::Multiply(const DenseMatrix *a1, const DenseMatrix *a2)
{
    if (!a1 || !a2)
...
DenseMatrix *
DenseMatrix::Multiply(const DenseMatrix *a1, const DataArrayDouble *a2)
{
...
DenseMatrix::DenseMatrix(mcIdType nbRows, mcIdType nbCols)
    : _nb_rows(nbRows), _nb_cols(nbCols), _data(DataArrayDouble::New())
{
    if (_nb_rows < 0 || _nb_cols < 0)
        throw INTERP_KERNEL::Exception(
            "constructor of DenseMatrix : number of rows and number of cols must be > 0 both !"
        );
...
mcIdType
DenseMatrix::getNumberOfRowsExt(mcIdType nbRows) const
{
    if (nbRows < -1)
...
mcIdType
DenseMatrix::getNumberOfColsExt(mcIdType nbCols) const
{
    if (nbCols < -1)
...
void
DenseMatrix::checkValidData() const
{
    if (!getData())
...
void
DenseMatrix::CheckArraySizes(DataArrayDouble *array, mcIdType nbRows, mcIdType nbCols)
{
    if (nbRows < 0 || nbCols < 0)
        throw INTERP_KERNEL::Exception(
            "constructor #2 of DenseMatrix : number of rows and number of cols must be > 0 both !"
        );
    ...
    if (nbRows * nbCols != array->getNbOfElems())
        throw INTERP_KERNEL::Exception(
            "constructor #2 of DenseMatrix : the number of elems in input array is not equal to the product of nbRows "
            "and nbCols !"
        );
}

void
DenseMatrix::CheckSameSize(const DenseMatrix *a1, const DenseMatrix *a2)
{
    if (!a1 || !a2)
...
void
DenseMatrix::CheckCompatibleSizeForMul(const DenseMatrix *a1, const DenseMatrix *a2)
{
    if (!a1 || !a2)
    ...
    if (a1->getNumberOfCols() != a2->getNumberOfRows())
        throw INTERP_KERNEL::Exception(
            "DenseMatrix::CheckCompatibleSizeForMul : number of cols of a1 must be equal to number of rows of a2 !"
        );
}
