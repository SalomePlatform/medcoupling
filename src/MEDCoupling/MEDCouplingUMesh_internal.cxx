...
MEDCouplingUMesh::checkFullyDefined() const
{
    if (!_nodal_connec_index || !_nodal_connec || !_coords)
        throw INTERP_KERNEL::Exception(
            "Reverse nodal connectivity computation requires full connectivity and coordinates set in unstructured "
            "mesh."
        );
}
...
MEDCouplingUMesh::checkConnectivityFullyDefined() const
{
    if (!_nodal_connec_index || !_nodal_connec)
        throw INTERP_KERNEL::Exception(
            "Reverse nodal connectivity computation requires full connectivity set in unstructured mesh."
        );
}
    ...
    if (_nodal_connec != 0 && _nodal_connec_index != 0)
    {
        ...
        for (mcIdType i = 0; i < nbOfCells; i++)
        {
            const INTERP_KERNEL::CellModel &cm =
                INTERP_KERNEL::CellModel::GetCellModel((INTERP_KERNEL::NormalizedCellType)c[ci[i]]);
...
MEDCouplingUMesh::simplexizePol0()
{
    ...
    if (getMeshDimension() != 2)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::simplexizePol0 : this policy is only available for mesh with meshdim == 2 !"
        );
    mcIdType nbOfCells = getNumberOfCells();
    ...
    for (mcIdType i = 0; i < nbOfCells; i++, ci++)
    {
        ...
        {
            const mcIdType tmp[8] = {
                ToIdType(INTERP_KERNEL::NORM_TRI3),
                oldc[ci[0] + 1],
                oldc[ci[0] + 2],
                oldc[ci[0] + 3],
                ToIdType(INTERP_KERNEL::NORM_TRI3),
...
/*!
 * This method implements policy INTERP_KERNEL::PLANAR_FACE_5 of virtual method
 * MEDCoupling::MEDCouplingUMesh::simplexize.
 */
DataArrayIdType *
MEDCouplingUMesh::simplexizePlanarFace5()
{
    ...
    if (getMeshDimension() != 3)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::simplexizePlanarFace5 : this policy is only available for mesh with meshdim == 3 !"
        );
    mcIdType nbOfCells = getNumberOfCells();
    MCAuto<DataArrayIdType> ret = DataArrayIdType::New();
    ...
    for (mcIdType i = 0; i < nbOfCells; i++, ci++)
    {
        if ((INTERP_KERNEL::NormalizedCellType)oldc[ci[0]] == INTERP_KERNEL::NORM_HEXA8)
        {
            for (int j = 0; j < 5; j++, pt += 5, ptI++)
            {
                ...
                pt[1] = oldc[ci[0] + INTERP_KERNEL::SPLIT_NODES_5_WO[4 * j + 0] + 1];
                pt[2] = oldc[ci[0] + INTERP_KERNEL::SPLIT_NODES_5_WO[4 * j + 1] + 1];
                pt[3] = oldc[ci[0] + INTERP_KERNEL::SPLIT_NODES_5_WO[4 * j + 2] + 1];
                pt[4] = oldc[ci[0] + INTERP_KERNEL::SPLIT_NODES_5_WO[4 * j + 3] + 1];
                *retPt++ = i;
                ptI[1] = ptI[0] + 5;
            }
        }
...
/*!
 * This method implements policy INTERP_KERNEL::PLANAR_FACE_6 of virtual method
 * MEDCoupling::MEDCouplingUMesh::simplexize.
 */
DataArrayIdType *
MEDCouplingUMesh::simplexizePlanarFace6()
{
    ...
    if (getMeshDimension() != 3)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::simplexizePlanarFace6 : this policy is only available for mesh with meshdim == 3 !"
        );
    mcIdType nbOfCells = getNumberOfCells();
    MCAuto<DataArrayIdType> ret = DataArrayIdType::New();
    mcIdType nbOfCutCells = getNumberOfCellsWithType(INTERP_KERNEL::NORM_HEXA8);
    ...
    for (mcIdType i = 0; i < nbOfCells; i++, ci++)
    {
        if ((INTERP_KERNEL::NormalizedCellType)oldc[ci[0]] == INTERP_KERNEL::NORM_HEXA8)
        {
            ...
            {
                pt[0] = ToIdType(INTERP_KERNEL::NORM_TETRA4);
                pt[1] = oldc[ci[0] + INTERP_KERNEL::SPLIT_NODES_6_WO[4 * j + 0] + 1];
                pt[2] = oldc[ci[0] + INTERP_KERNEL::SPLIT_NODES_6_WO[4 * j + 1] + 1];
                pt[3] = oldc[ci[0] + INTERP_KERNEL::SPLIT_NODES_6_WO[4 * j + 2] + 1];
                pt[4] = oldc[ci[0] + INTERP_KERNEL::SPLIT_NODES_6_WO[4 * j + 3] + 1];
                *retPt++ = i;
                ptI[1] = ptI[0] + 5;
            }
        }
 ...
 */
void
MEDCouplingUMesh::tessellate2DInternal(double eps)
{
    checkFullyDefined();
    if (getMeshDimension() != 2 || getSpaceDimension() != 2)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::tessellate2DInternal works on umeshes with meshdim equal to 2 and spaceDim equal to 2 "
            "too!"
        );
    double epsa = fabs(eps);
    if (epsa < std::numeric_limits<double>::min())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::tessellate2DInternal : epsilon is null ! Please specify a higher epsilon. If too tiny "
            "it can lead to a huge amount of nodes and memory !"
        );
    ...
    subDivide2DMesh(
        mDesc->_nodal_connec->begin(), mDesc->_nodal_connec_index->begin(), desc1->begin(), descIndx1->begin()
    );
 ...
 */
void
MEDCouplingUMesh::tessellate2DCurveInternal(double eps)
{
    checkFullyDefined();
    if (getMeshDimension() != 1 || getSpaceDimension() != 2)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::tessellate2DCurveInternal works on umeshes with meshdim equal to 1 and spaceDim equal "
            "to 2 too!"
        );
    double epsa = fabs(eps);
    if (epsa < std::numeric_limits<double>::min())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::tessellate2DCurveInternal : epsilon is null ! Please specify a higher epsilon. If too "
            "tiny it can lead to a huge amount of nodes and memory !"
        );
    ...
    *newConnIPtr = 0;
    ...
    for (mcIdType i = 0; i < nbCells; i++, newConnIPtr++)
    {
        ...
        {  // assert(connI[i+1]-connI[i]-1==3)
            tmp1[0] = conn[connI[i] + 1 + 0];
            tmp1[1] = conn[connI[i] + 1 + 1];
            tmp1[2] = conn[connI[i] + 1 + 2];
    ...
    if (addCoo.empty() &&
        ToIdType(newConn.size()) ==
            _nodal_connec->getNumberOfTuples())  // nothing happens during tessellation : no update needed
        return;
    _types = types;
...
/*!
 * This private method is used to subdivide edges of a mesh with meshdim==2. If \a this has no a meshdim equal to 2 an
 * exception will be thrown. This method completely ignores coordinates.
 ...
 * \param descIndex is descending connectivity index in format specified in
 * MEDCouplingUMesh::buildDescendingConnectivity2
 */
void
MEDCouplingUMesh::subDivide2DMesh(
    const mcIdType *nodeSubdived, const mcIdType *nodeIndxSubdived, const mcIdType *desc, const mcIdType *descIndex
)
{
    checkFullyDefined();
    if (getMeshDimension() != 2)
    ...
    for (mcIdType i = 0; i < nbOfCells; i++, connI++)
    {
        ...
        mcIdType ref =
            ddirect ? nodeSubdived[nodeIndxSubdived[eedgeId + 1] - 1] : nodeSubdived[nodeIndxSubdived[eedgeId] + 1];
        for (mcIdType j = 0; j < nbOfEdges; j++)
        {
            bool direct = desc[offset + j] > 0;
            mcIdType edgeId = std::abs(desc[offset + j]) - 1;
            if (!INTERP_KERNEL::CellModel::GetCellModel(
                     (INTERP_KERNEL::NormalizedCellType)nodeSubdived[nodeIndxSubdived[edgeId]]
                )
                     .isQuadratic())
            {
                ...
                else
                {
                    ...
                    oss << "MEDCouplingUMesh::subDivide2DMesh : On polygon #" << i << " edgeid #" << j
                        << " subedges mismatch : end subedge k!=start subedge k+1 !";
            ...
            {
                throw INTERP_KERNEL::Exception(
                    "MEDCouplingUMesh::subDivide2DMesh : this method only subdivides into linear edges !"
                );
            }
        }
    ...
    for (mcIdType i = 0; i < nbOfCells; i++)
    {
        *work++ = INTERP_KERNEL::NORM_POLYGON;
        ...
        for (mcIdType j = 0; j < nbOfEdges; j++)
        {
            ...
            if (direct)
                work = std::copy(
                    nodeSubdived + nodeIndxSubdived[edgeId] + 1, nodeSubdived + nodeIndxSubdived[edgeId + 1] - 1, work
                );
            else
            {
                mcIdType nbOfSubNodes = nodeIndxSubdived[edgeId + 1] - nodeIndxSubdived[edgeId] - 1;
 ...
 */
void
MEDCouplingUMesh::fillCellIdsToKeepFromNodeIds(
...
) const
{
    ...
    mcIdType sz = getNodalConnectivity()->getMaxValue(tmp);
 ...
 * if cut3DCurve[i]==-1, it means that cell#i has been already detected to be fully (or partially) part of plane defined
 * by ('origin','vec'). This method will throw an exception if \a this contains a non linear segment.
 */
void
MEDCouplingUMesh::split3DCurveWithPlane(
    const double *origin, const double *vec, double eps, std::vector<mcIdType> &cut3DCurve
)
{
    checkFullyDefined();
    if (getMeshDimension() != 1 || getSpaceDimension() != 3)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::split3DCurveWithPlane works on umeshes with meshdim equal to 1 and spaceDim equal to 3 !"
        );
    ...
    if (normm < 1e-6)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::split3DCurveWithPlane : parameter 'vec' should have a norm2 greater than 1e-6 !"
        );
    vec2[0] = vec[0] / normm;
    vec2[1] = vec[1] / normm;
    vec2[2] = vec[2] / normm;
    ...
    for (mcIdType i = 0; i < ncells; i++)
    {
        if (conn[connI[i]] == ToIdType(INTERP_KERNEL::NORM_SEG2))
        {
            if (cut3DCurve[i] == -2)
            {
                ...
                vec3[0] = coo[3 * endd] - coo[3 * st];
                vec3[1] = coo[3 * endd + 1] - coo[3 * st + 1];
                vec3[2] = coo[3 * endd + 2] - coo[3 * st + 2];
                ...
                if (colin > eps)  // if colin<=eps -> current SEG2 is colinear to the input plane
                {
                    ...
                    vec4[0] = st2[0] - origin[0];
                    vec4[1] = st2[1] - origin[1];
                    vec4[2] = st2[2] - origin[2];
                    double pos = -(vec4[0] * vec2[0] + vec4[1] * vec2[1] + vec4[2] * vec2[2]) /
                                 ((vec3[0] * vec2[0] + vec3[1] * vec2[1] + vec3[2] * vec2[2]));
                    if (pos > eps && pos < 1 - eps)
                    {
                        mcIdType nNode = ToIdType(addCoo.size()) / 3;
                        vec4[0] = st2[0] + pos * vec3[0];
                        vec4[1] = st2[1] + pos * vec3[1];
                        vec4[2] = st2[2] + pos * vec3[2];
            ...
            throw INTERP_KERNEL::Exception(
                "MEDCouplingUMesh::split3DCurveWithPlane : this method is only available for linear cell (NORM_SEG2) !"
            );
    }
    if (!addCoo.empty())
    {
...
MEDCouplingUMesh::fillExtCoordsUsingTranslation(const MEDCouplingUMesh *mesh1D, bool isQuad) const
{
    ...
    for (mcIdType i = 0; i < nbOf1DCells; i++)
    {
        ...
        for (mcIdType j = 0; j < oldNbOfNodes; j++)
            work = std::transform(
                vec, vec + spaceDim, retPtr + spaceDim * (i * oldNbOfNodes + j), work, std::plus<double>()
            );
        if (isQuad)
        {
            c.resize(0);
...
MEDCouplingUMesh::fillExtCoordsUsingTranslAndAutoRotation2D(const MEDCouplingUMesh *mesh1D, bool isQuad) const
{
    if (isQuad)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::fillExtCoordsUsingTranslAndAutoRotation2D : not implemented for quadratic cells !"
        );
    mcIdType oldNbOfNodes = getNumberOfNodes();
    mcIdType nbOf1DCells = ToIdType(mesh1D->getNumberOfCells());
    if (nbOf1DCells < 2)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::fillExtCoordsUsingTranslAndAutoRotation2D : impossible to detect any angle of rotation "
            "! Change extrusion policy 1->0 !"
        );
    ...
    double *retPtr = ret->getPointer();
    retPtr =
        std::copy(getCoords()->getConstPointer(), getCoords()->getConstPointer() + getCoords()->getNbOfElems(), retPtr);
    ...
    for (mcIdType i = 1; i < nbOfLevsInVec; i++)
    {
        ...
        const double *third =
            i + 1 < nbOfLevsInVec ? coo1D + 2 * conn1D[connI1D[i] + 2] : coo1D + 2 * conn1D[connI1D[i - 2] + 1];
        ...
        double cosangle = i + 1 < nbOfLevsInVec
                              ? (p0[0] - tmp3[0]) * (p1[0] - tmp3[0]) + (p0[1] - tmp3[1]) * (p1[1] - tmp3[1])
                              : (p2[0] - tmp3[0]) * (p1[0] - tmp3[0]) + (p2[1] - tmp3[1]) * (p1[1] - tmp3[1]);
...
MEDCouplingUMesh::fillExtCoordsUsingTranslAndAutoRotation3D(const MEDCouplingUMesh *mesh1D, bool isQuad) const
{
    if (isQuad)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::fillExtCoordsUsingTranslAndAutoRotation3D : not implemented for quadratic cells !"
        );
    mcIdType oldNbOfNodes = getNumberOfNodes();
    mcIdType nbOf1DCells = ToIdType(mesh1D->getNumberOfCells());
    if (nbOf1DCells < 2)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::fillExtCoordsUsingTranslAndAutoRotation3D : impossible to detect any angle of rotation "
            "! Change extrusion policy 1->0 !"
        );
    MCAuto<DataArrayDouble> ret = DataArrayDouble::New();
    mcIdType nbOfLevsInVec = nbOf1DCells + 1;
    ...
    for (mcIdType i = 1; i < nbOfLevsInVec; i++)
    {
        ...
        const double *third =
            i + 1 < nbOfLevsInVec ? coo1D + 3 * conn1D[connI1D[i] + 2] : coo1D + 3 * conn1D[connI1D[i - 2] + 1];
        ...
        if (norm > 1.e-7)
        {
            ...
            double p0r[3] = {
                ...
                m[1][0] * p0[0] + m[1][1] * p0[1] + m[1][2] * p0[2],
                m[2][0] * p0[0] + m[2][1] * p0[1] + m[2][2] * p0[2]
            };
            double p1r[3] = {
                m[0][0] * p1[0] + m[0][1] * p1[1] + m[0][2] * p1[2],
                m[1][0] * p1[0] + m[1][1] * p1[1] + m[1][2] * p1[2],
                m[2][0] * p1[0] + m[2][1] * p1[1] + m[2][2] * p1[2]
            };
            double p2r[3] = {
                m[0][0] * p2[0] + m[0][1] * p2[1] + m[0][2] * p2[2],
            ...
            double cosangle = i + 1 < nbOfLevsInVec
                                  ? (p0r[0] - tmp3[0]) * (p1r[0] - tmp3[0]) + (p0r[1] - tmp3[1]) * (p1r[1] - tmp3[1])
                                  : (p2r[0] - tmp3[0]) * (p1r[0] - tmp3[0]) + (p2r[1] - tmp3[1]) * (p1r[1] - tmp3[1]);
            double angle = acos(cosangle / (radius * radius));
    ...
    *newConnIPtr++ = 0;
    ...
    for (mcIdType iz = 0; iz < nbOf1DCells; iz++)
    {
        if (iz != 0)
            std::transform(
                newConnIPtr + 1,
                newConnIPtr + 1 + nbOf2DCells,
                newConnIPtr + 1 + iz * nbOf2DCells,
                std::bind(std::plus<mcIdType>(), std::placeholders::_1, newConnIPtr[iz * nbOf2DCells])
            );
        ...
        {
            mcIdType icell(
                ToIdType(iter - newc.begin())
            );  // std::distance unfortunately cannot been called here in C++98
...
MEDCouplingUMesh::AreCellsEqualInPool(
    ...
    int compType,
    const mcIdType *conn,
    const mcIdType *connI,
    DataArrayIdType *result
)
{
...
MEDCouplingUMesh::buildPartOfMySelfKeepCoords(const mcIdType *begin, const mcIdType *end) const
{
    ...
    for (const mcIdType *work = begin; work != end; work++, newNbring++)
    {
        ...
        else
        {
            free(connIndexRet);
            std::ostringstream oss;
            oss << "MEDCouplingUMesh::buildPartOfMySelfKeepCoords : On pos #" << std::distance(begin, work)
...
MEDCouplingUMesh::buildPartOfMySelfKeepCoordsSlice(mcIdType start, mcIdType end, mcIdType step) const
{
    ...
    mcIdType newNbOfCells = DataArray::GetNumberOfItemGivenBESRelative(
        start, end, step, "MEDCouplingUMesh::buildPartOfMySelfKeepCoordsSlice : "
    );
    MCAuto<DataArrayIdType> newConnI = DataArrayIdType::New();
    newConnI->alloc(newNbOfCells + 1, 1);
    mcIdType *newConnIPtr = newConnI->getPointer();
    *newConnIPtr = 0;
    mcIdType work = start;
    const mcIdType *conn = _nodal_connec->getConstPointer();
    ...
    for (mcIdType i = 0; i < newNbOfCells; i++, newConnIPtr++, work += step)
    {
        ...
        {
            std::ostringstream oss;
            oss << "MEDCouplingUMesh::buildPartOfMySelfKeepCoordsSlice : On pos #" << i << " input cell id =" << work
                << " should be in [0," << ncell << ") !";
            throw INTERP_KERNEL::Exception(oss.str());
        }
    }
    MCAuto<DataArrayIdType> newConn = DataArrayIdType::New();
    newConn->alloc(newConnIPtr[0], 1);
    mcIdType *newConnPtr = newConn->getPointer();
    std::set<INTERP_KERNEL::NormalizedCellType> types;
    work = start;
    for (mcIdType i = 0; i < newNbOfCells; i++, newConnIPtr++, work += step)
...
/*!
 * Implements \a conversionType 0 for meshes with meshDim = 1, of MEDCouplingUMesh::convertLinearCellsToQuadratic
 * method.
 ...
 */
DataArrayIdType *
MEDCouplingUMesh::convertLinearCellsToQuadratic1D0(
...
) const
{
    ...
    coords = DataArrayDouble::Aggregate(getCoords(), tmp);
    conn = newConn.retn();
    connI = newConnI.retn();
    return ret.retn();
}
...
DataArrayIdType *
MEDCouplingUMesh::convertLinearCellsToQuadratic2DAnd3D0(
    const MEDCouplingUMesh *m1D,
...
) const
{
    ...
    MCAuto<DataArrayIdType> ret1D = m1D->convertLinearCellsToQuadratic1D0(conn1D, conn1DI, coordsTmp, types1D);
    ret1D = 0;
    ...
    conn = newConn.retn();
    connI = newConnI.retn();
    coords = coordsTmpSafe.retn();
    return ret.retn();
}

/*!
 * Implements \a conversionType 0 for meshes with meshDim = 2, of MEDCouplingUMesh::convertLinearCellsToQuadratic
 * method.
 * \return a newly created DataArrayIdType instance that the caller should deal with containing cell ids of converted
 * cells.
 * \sa MEDCouplingUMesh::convertLinearCellsToQuadratic.
 */
DataArrayIdType *
MEDCouplingUMesh::convertLinearCellsToQuadratic2D0(
    DataArrayIdType *&conn,
    DataArrayIdType *&connI,
    DataArrayDouble *&coords,
    std::set<INTERP_KERNEL::NormalizedCellType> &types
...
{
    MCAuto<DataArrayIdType> desc(DataArrayIdType::New()), descI(DataArrayIdType::New()), tmp2(DataArrayIdType::New()),
        tmp3(DataArrayIdType::New());
    MCAuto<MEDCouplingUMesh> m1D = buildDescendingConnectivity(desc, descI, tmp2, tmp3);
    tmp2 = 0;
    tmp3 = 0;
...
DataArrayIdType *
MEDCouplingUMesh::convertLinearCellsToQuadratic2D1(
    DataArrayIdType *&conn,
    DataArrayIdType *&connI,
    DataArrayDouble *&coords,
    std::set<INTERP_KERNEL::NormalizedCellType> &types
) const
{
    ...
    MCAuto<DataArrayDouble> tmp = bary->selectByTupleIdSafe(ret->begin(), ret->end());
    coords = DataArrayDouble::Aggregate(coordsTmpSafe, tmp);
    conn = newConn.retn();
    connI = newConnI.retn();
    return ret.retn();
}
...
/*!
 * Implements \a conversionType 0 for meshes with meshDim = 3, of MEDCouplingUMesh::convertLinearCellsToQuadratic
 * method.
 * \return a newly created DataArrayIdType instance that the caller should deal with containing cell ids of converted
 * cells.
 * \sa MEDCouplingUMesh::convertLinearCellsToQuadratic.
 */
DataArrayIdType *
MEDCouplingUMesh::convertLinearCellsToQuadratic3D0(
    DataArrayIdType *&conn,
    DataArrayIdType *&connI,
    DataArrayDouble *&coords,
    std::set<INTERP_KERNEL::NormalizedCellType> &types
) const
{
    MCAuto<DataArrayIdType> desc(DataArrayIdType::New()), descI(DataArrayIdType::New()), tmp2(DataArrayIdType::New()),
        tmp3(DataArrayIdType::New());
    MCAuto<MEDCouplingUMesh> m1D = explode3DMeshTo1D(desc, descI, tmp2, tmp3);
    tmp2 = 0;
    tmp3 = 0;
    return convertLinearCellsToQuadratic2DAnd3D0(m1D, desc, descI, conn, connI, coords, types);
}
...
DataArrayIdType *
MEDCouplingUMesh::convertLinearCellsToQuadratic3D1(
    DataArrayIdType *&conn,
    DataArrayIdType *&connI,
    DataArrayDouble *&coords,
    std::set<INTERP_KERNEL::NormalizedCellType> &types
...
{
    MCAuto<DataArrayIdType> desc2(DataArrayIdType::New()), desc2I(DataArrayIdType::New()), tmp2(DataArrayIdType::New()),
        tmp3(DataArrayIdType::New());
    MCAuto<MEDCouplingUMesh> m2D = buildDescendingConnectivityGen<MinusOneSonsGeneratorBiQuadratic>(
        desc2, desc2I, tmp2, tmp3, MEDCouplingFastNbrer
    );
    tmp2 = 0;
    tmp3 = 0;
    MCAuto<DataArrayIdType> desc1(DataArrayIdType::New()), desc1I(DataArrayIdType::New()), tmp4(DataArrayIdType::New()),
        tmp5(DataArrayIdType::New());
    MCAuto<MEDCouplingUMesh> m1D = explode3DMeshTo1D(desc1, desc1I, tmp4, tmp5);
    tmp4 = 0;
    tmp5 = 0;
    //
    MCAuto<DataArrayIdType> newConn = DataArrayIdType::New();
    ...
    newConnI->alloc(1, 1);
    newConnI->setIJ(0, 0, 0);
    MCAuto<DataArrayIdType> ret = DataArrayIdType::New(), ret2 = DataArrayIdType::New();
    ret->alloc(0, 1);
    ret2->alloc(0, 1);
    //
    MCAuto<DataArrayDouble> bary = computeCellCenterOfMass();
    const mcIdType *descPtr(desc1->begin()), *descIPtr(desc1I->begin()), *desc2Ptr(desc2->begin()),
        *desc2IPtr(desc2I->begin());
    ...
    MCAuto<DataArrayIdType> ret1D = m1D->convertLinearCellsToQuadratic1D0(conn1D, conn1DI, coordsTmp, types1D);
    ret1D = DataArrayIdType::New();
    ret1D->alloc(0, 1);
    MCAuto<DataArrayIdType> conn1DSafe(conn1D), conn1DISafe(conn1DI);
    MCAuto<DataArrayDouble> coordsTmpSafe(coordsTmp);
    MCAuto<DataArrayIdType> ret2D = m2D->convertLinearCellsToQuadratic2D1(conn2D, conn2DI, coordsTmp2, types2D);
    ret2D = DataArrayIdType::New();
    ret2D->alloc(0, 1);
    MCAuto<DataArrayDouble> coordsTmp2Safe(coordsTmp2);
    MCAuto<DataArrayIdType> conn2DSafe(conn2D), conn2DISafe(conn2DI);
    const mcIdType *c1DPtr = conn1D->begin(), *c1DIPtr = conn1DI->begin(), *c2DPtr = conn2D->begin(),
                   *c2DIPtr = conn2DI->begin();
    mcIdType nbOfCells = getNumberOfCells();
    const mcIdType *cPtr = _nodal_connec->begin();
    const mcIdType *icPtr = _nodal_connec_index->begin();
    mcIdType lastVal = 0;
    ...
    for (mcIdType i = 0; i < nbOfCells; i++, icPtr++, descIPtr++, desc2IPtr++)
    {
        ...
        if (!cm.isQuadratic())
        {
            ...
            {
                std::ostringstream oss;
                oss << "MEDCouplingUMesh::convertLinearCellsToQuadratic3D1 : On cell #" << i
                    << " the linear cell type does not support advanced quadratization !";
                throw INTERP_KERNEL::Exception(oss.str());
            }
            types.insert(typ2);
            newConn->pushBackSilent(typ2);
            ...
            for (const mcIdType *d = desc2Ptr + desc2IPtr[0]; d != desc2Ptr + desc2IPtr[1]; d++)
            {
                ...
                ret2D->pushBackSilent(nodeId2);
                ret1D->pushBackSilent(tmpPos);
            }
            newConn->pushBackSilent(offset + ret->getNumberOfTuples());
    ...
    for (const mcIdType *elt = ret->begin(); elt != ret->end(); elt++) c[cI[(*elt) + 1] - 1] += offset;
    coords = DataArrayDouble::Aggregate(v);
    conn = newConn.retn();
    connI = newConnI.retn();
    return ret.retn();
}
...
DataArrayIdType *
MEDCouplingUMesh::buildUnionOf2DMeshLinear(const MEDCouplingUMesh *skin, const DataArrayIdType *n2o) const
{
    ...
    MCAuto<DataArrayIdType> ret(DataArrayIdType::New());
    ret->alloc(nbOfNodesExpected + 1, 1);
            ...
            if (conn.size() == 1)
            {
                ...
                *work++ = n2oPtr[curNode];
                ...
                if (shar.size() == 1)
                {
                    ...
                    throw INTERP_KERNEL::Exception(
                        "MEDCouplingUMesh::buildUnionOf2DMeshLinear : presence of unexpected 2 !"
                    );
            }
            else
                throw INTERP_KERNEL::Exception(
                    "MEDCouplingUMesh::buildUnionOf2DMeshLinear : presence of unexpected 1 !"
                );
        }
        else
            throw INTERP_KERNEL::Exception(
                "MEDCouplingUMesh::buildUnionOf2DMeshLinear : presence of unexpected cell !"
            );
    }
    return ret.retn();
}
...
DataArrayIdType *
MEDCouplingUMesh::buildUnionOf2DMeshQuadratic(const MEDCouplingUMesh *skin, const DataArrayIdType *n2o) const
{
    mcIdType nbOfNodesExpected(skin->getNumberOfNodes());
    ...
    ret->alloc(nbOfNodesExpected + 1, 1);
    mcIdType *work(ret->getPointer());
    *work++ = INTERP_KERNEL::NORM_QPOLYG;
    if (nbOfNodesExpected < 1)
        return ret.retn();
    mcIdType prevCell(0), prevNode(nodalPtr[nodalIPtr[0] + 1]);
    *work = n2oPtr[prevNode];
    work[nbOfTurn] = n2oPtr[nodalPtr[nodalIPtr[0] + 3]];
    work++;
            ...
            if (conn.size() == 1)
            {
                ...
                *work = n2oPtr[curNode];
                ...
                if (shar.size() == 1)
                {
                    ...
                    throw INTERP_KERNEL::Exception(
                        "MEDCouplingUMesh::buildUnionOf2DMeshQuadratic : presence of unexpected 2 !"
                    );
            }
            else
                throw INTERP_KERNEL::Exception(
                    "MEDCouplingUMesh::buildUnionOf2DMeshQuadratic : presence of unexpected 1 !"
                );
        }
        else
            throw INTERP_KERNEL::Exception(
                "MEDCouplingUMesh::buildUnionOf2DMeshQuadratic : presence of unexpected cell !"
            );
    }
    return ret.retn();
}
...
MEDCouplingUMesh::DeleteCellTypeInIndexedArray(
    ...
    MCAuto<DataArrayIdType> &arrOut,
    MCAuto<DataArrayIdType> &arrIndxOut
)
{
    ...
    arrIn->checkNbOfComps(1, "input array");
    arrIndxIn->checkNbOfComps(1, "input indexed array");
    ...
    arrOut = DataArrayIdType::New();
    arrOut->alloc(arrNbTuples - arrIndxNbTuples + 1, 1);
    arrIndxOut = DataArrayIdType::New();
    arrIndxOut->alloc(arrIndxNbTuples, 1);
...
MEDCouplingUMesh *
MEDCouplingUMesh::MergeUMeshesLL(const std::vector<const MEDCouplingUMesh *> &a)
{
    if (a.empty())
    ...
    *cIPtr++ = 0;
    ...
    for (it = a.begin(); it != a.end(); it++)
    {
        ...
        cIPtr = std::transform(
            curCI + 1, curCI + curNbOfCell + 1, cIPtr, std::bind(std::plus<mcIdType>(), std::placeholders::_1, offset)
        );
 ...
 */
void
MEDCouplingUMesh::DistanceToPoint3DSurfAlg(
    const double *pt,
    const mcIdType *cellIdsBg,
...
)
{
    ...
    for (const mcIdType *zeCell = cellIdsBg; zeCell != cellIdsEnd; zeCell++)
    {
        switch ((INTERP_KERNEL::NormalizedCellType)nc[ncI[*zeCell]])
        {
            ...
            {
                double tmp = INTERP_KERNEL::DistanceFromPtToTriInSpaceDim3(
                    pt,
                    coords + 3 * nc[ncI[*zeCell] + 1],
                    coords + 3 * nc[ncI[*zeCell] + 2],
                    coords + 3 * nc[ncI[*zeCell] + 3]
                );
                if (tmp < ret0)
                {
                    ret0 = tmp;
            ...
            {
                double tmp = INTERP_KERNEL::DistanceFromPtToPolygonInSpaceDim3(
                    pt, nc + ncI[*zeCell] + 1, nc + ncI[*zeCell + 1], coords
                );
                if (tmp < ret0)
                {
                    ret0 = tmp;
            ...
            default:
                throw INTERP_KERNEL::Exception(
                    "MEDCouplingUMesh::distanceToPoint3DSurfAlg : not managed cell type ! Supporting TRI3, QUAD4 and "
                    "POLYGON !"
                );
        }
    }
 ...
 */
void
MEDCouplingUMesh::DistanceToPoint2DCurveAlg(
    const double *pt,
    const mcIdType *cellIdsBg,
    const mcIdType *cellIdsEnd,
    const double *coords,
    ...
    for (const mcIdType *zeCell = cellIdsBg; zeCell != cellIdsEnd; zeCell++)
    {
        switch ((INTERP_KERNEL::NormalizedCellType)nc[ncI[*zeCell]])
        {
            case INTERP_KERNEL::NORM_SEG2:
            {
                ...
                double tmp = INTERP_KERNEL::SquareDistanceFromPtToSegInSpaceDim2(
                    pt, coords + 2 * nc[ncI[*zeCell] + 1], coords + 2 * nc[ncI[*zeCell] + 2], uselessEntry
                );
                tmp = sqrt(tmp);
                if (tmp < ret0)
                {
            ...
            default:
                throw INTERP_KERNEL::Exception(
                    "MEDCouplingUMesh::distanceToPoint2DCurveAlg : not managed cell type ! Supporting SEG2 !"
                );
        }
    }
}
DataArrayIdType *
MEDCouplingUMesh::ComputeSpreadZoneGraduallyFromSeedAlg(
...
)
{
    ...
    if (!seedBg || !seedEnd || !arrIn || !arrIndxIn)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::ComputeSpreadZoneGraduallyFromSeedAlg : some input pointer is NULL !"
        );
    ...
    for (const mcIdType *seedElt = seedBg; seedElt != seedEnd; seedElt++, i++)
    {
        ...
        {
            fetched[*seedElt] = true;
            fetched2[*seedElt] = true;
        }
        else
        {
            std::ostringstream oss;
            oss << "MEDCouplingUMesh::ComputeSpreadZoneGraduallyFromSeedAlg : At pos #" << i << " of seeds value is "
                << *seedElt << "! Should be in [0," << nbOfTuples << ") !";
            throw INTERP_KERNEL::Exception(oss.str());
        }
    }
 ...
 */
void
MEDCouplingUMesh::FillInCompact3DMode(
    int spaceDim, mcIdType nbOfNodesInCell, const mcIdType *conn, const double *coo, double *zipFrmt
)
{
    double *w = zipFrmt;
    if (spaceDim == 3)
        for (int i = 0; i < nbOfNodesInCell; i++) w = std::copy(coo + 3 * conn[i], coo + 3 * conn[i] + 3, w);
    else if (spaceDim == 2)
    {
        ...
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::FillInCompact3DMode : Invalid spaceDim specified : must be 2 or 3 !"
        );
}

/*!
 * This method takes in input a cell defined by its MEDcouplingUMesh connectivity [ \a connBg , \a connEnd ) and returns
 * its extruded cell by inserting the result at the end of ret.
 ...
 */
void
MEDCouplingUMesh::AppendExtrudedCell(
    const mcIdType *connBg, const mcIdType *connEnd, mcIdType nbOfNodesPerLev, bool isQuad, std::vector<mcIdType> &ret
)
{
    ...
    switch (flatType)
    {
        ...
        case INTERP_KERNEL::NORM_SEG3:
        {
            mcIdType conn[8] = {
                ...
                connBg[1] + deltaz,
                connBg[2],
                connBg[3] + nbOfNodesPerLev,
                connBg[2] + deltaz,
                connBg[1] + nbOfNodesPerLev
            };
        ...
        {
            mcIdType conn[6] = {
                connBg[1], connBg[2], connBg[3], connBg[1] + deltaz, connBg[2] + deltaz, connBg[3] + deltaz
            };
        ...
        case INTERP_KERNEL::NORM_POLYGON:
        {
            ...
            *ii++ = -1;
            ...
            for (std::size_t i = 0; i < nbOfRadFaces; i++)
            {
                *ii++ = -1;
                mcIdType conn[4] = {
                    connBg[(i + 1) % nbOfRadFaces + 1],
                    connBg[i + 1],
                    connBg[i + 1] + deltaz,
                    connBg[(i + 1) % nbOfRadFaces + 1] + deltaz
                };
...
/*!
 * This method is part of the Slice3D algorithm. It is the first step of assembly process, ones coordinates have been
 * computed (by MEDCouplingUMesh::split3DCurveWithPlane method). This method allows to compute given the status of 3D
 * curve cells and the descending connectivity 3DSurf->3DCurve to deduce the intersection of each 3D surf cells with a
 * plane. The result will be put in 'cut3DSuf' out parameter.
 ...
 */
void
MEDCouplingUMesh::AssemblyForSplitFrom3DCurve(
    const std::vector<mcIdType> &cut3DCurve,
    std::vector<mcIdType> &nodesOnPlane,
    const mcIdType *nodal3DSurf,
    const mcIdType *nodalIndx3DSurf,
    const mcIdType *nodal3DCurve,
    const mcIdType *nodalIndx3DCurve,
    const mcIdType *desc,
...
)
{
    ...
    for (mcIdType i = 0; i < nbOf3DSurfCell; i++)
    {
        ...
        switch (res.size())
        {
            case 2:
            {
                cut3DSurf[i].first = res[0];
                cut3DSurf[i].second = res[1];
                break;
            }
            case 1:
            case 0:
            ...
            {  // case when plane is on a multi colinear edge of a polyhedron
                ...
                {
                    cut3DSurf[i].first = -2;
                    cut3DSurf[i].second = i;
                }
                else
                    throw INTERP_KERNEL::Exception(
                        "MEDCouplingUMesh::AssemblyPointsFrom3DCurve : unexpected situation !"
                    );
            }
        }
    }
 ...
 * This method is part of the Slice3D algorithm. It is the second step of assembly process, ones coordinates have been
 * computed (by MEDCouplingUMesh::split3DCurveWithPlane method). This method allows to compute given the result of 3D
 * surf cells with plane and the descending connectivity 3D->3DSurf to deduce the intersection of each 3D cells with a
 * plane. The result will be put in 'nodalRes' 'nodalResIndx' and 'cellIds' out parameters.
 * \param cut3DSurf  input parameter that gives for each 3DSurf its intersection with plane (result of
 * MEDCouplingUMesh::AssemblyForSplitFrom3DCurve).
 * \param desc is the descending connectivity 3D->3DSurf
 * \param descIndx is the descending connectivity index 3D->3DSurf
 */
void
MEDCouplingUMesh::assemblyForSplitFrom3DSurf(
    const std::vector<std::pair<mcIdType, mcIdType> > &cut3DSurf,
    const mcIdType *desc,
    const mcIdType *descIndx,
    DataArrayIdType *nodalRes,
    DataArrayIdType *nodalResIndx,
...
) const
{
    ...
    if (getMeshDimension() != 3 || getSpaceDimension() != 3)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::assemblyForSplitFrom3DSurf works on umeshes with meshdim equal to 3 and spaceDim equal "
            "to 3 too!"
        );
    ...
    for (mcIdType i = 0; i < nbOfCells; i++)
    {
        ...
        for (int j = 0; j < nbOfFaces; j++)
        {
            ...
            if (p.first != -1 && p.second != -1)
            {
                ...
                {
                    const INTERP_KERNEL::CellModel &cm = INTERP_KERNEL::CellModel::GetCellModel(
                        (INTERP_KERNEL::NormalizedCellType)nodal3D[nodalIndx3D[i]]
                    );
                    ...
                    unsigned nbOfNodesSon =
                        cm.fillSonCellNodalConnectivity2(j, nodal3D + nodalIndx3D[i] + 1, sz, tmp, cmsId);
                    start = tmp[0];
                    end = tmp[nbOfNodesSon - 1];
...
void
MEDCouplingUMesh::ComputeAllTypesInternal(
    std::set<INTERP_KERNEL::NormalizedCellType> &types,
    const DataArrayIdType *nodalConnec,
    const DataArrayIdType *nodalConnecIndex
)
{
 ...
 * This method will 'attract' middle points of seg3 (deduced from this by explosion if needed) of cells connected to
 * nodes specified in [\a nodeIdsBg, \a nodeIdsEnd ) For those selected mid points, their coordinates will be modified
 * by applying a dilation between node in input [\a nodeIdsBg, \a nodeIdsEnd ) and the corresponding mid points using \a
 * ratio input value. So this method is non const because coordinates are modified. If there is a couple of 2 points in
 * [\a nodeIdsBg, \a nodeIdsEnd ) that are boundaries of a seg3, the corresponding mid point will remain untouched.
 *
 ...
 */
void
MEDCouplingUMesh::attractSeg3MidPtsAroundNodes(double ratio, const mcIdType *nodeIdsBg, const mcIdType *nodeIdsEnd)
{
    checkFullyDefined();
    ...
    throw INTERP_KERNEL::Exception(
        "MEDCouplingUMesh::attractSeg3MidPtsAroundNodes : not managed dimension ! Should be in [1,2,3] !"
    );
}
 ...
 */
void
MEDCouplingUMesh::attractSeg3MidPtsAroundNodesUnderground(
    double ratio, const mcIdType *nodeIdsBg, const mcIdType *nodeIdsEnd
)
{
    ...
    {
        const mcIdType *isSelected(
            std::find_if(nc + nci[0] + 1, nc + nci[1], [&fastFinder](mcIdType v) { return fastFinder[v]; })
        );
        if (isSelected != nc + nci[1])
        {
            if ((INTERP_KERNEL::NormalizedCellType)nc[nci[0]] == INTERP_KERNEL::NORM_SEG3 && nci[1] - nci[0] == 4)
            {
                ...
                if (!cc)
                {
                    if (aa ^ bb)
                    {
                        ...
                        std::transform(
                            coords + spaceDim * attractor,
                            coords + spaceDim * (attractor + 1),
                            coords + spaceDim * endPt,
                            coords + spaceDim * ptToMove,
                            [ratio](const double &stPt2, const double &endPt2)
                            { return stPt2 + ratio * (endPt2 - stPt2); }
                        );
                    }
                    else
                ...
                {
                    std::ostringstream oss;
                    oss << "MEDCouplingUMesh::attractSeg3MidPtsAroundNodes : cell #" << cellId << " has a mid point "
                        << nc[*nci + 3] << " ! This node is in input list !";
                    throw INTERP_KERNEL::Exception(oss.str());
                }
            }
            else
            {
                std::ostringstream oss;
                oss << "MEDCouplingUMesh::attractSeg3MidPtsAroundNodes : cell #" << cellId
                    << " sharing one of the input nodes list its geo type is NOT SEG3 !";
                throw INTERP_KERNEL::Exception(oss.str());
            }
        }
    }
