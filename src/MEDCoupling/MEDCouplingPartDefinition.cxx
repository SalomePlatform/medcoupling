...
PartDefinition *
PartDefinition::New(mcIdType start, mcIdType stop, mcIdType step)
{
    return SlicePartDefinition::New(start, stop, step);
}

PartDefinition *
PartDefinition::New(DataArrayIdType *listOfIds)
{
    return DataArrayPartDefinition::New(listOfIds);
}

PartDefinition *
PartDefinition::Unserialize(std::vector<mcIdType> &tinyInt, std::vector<MCAuto<DataArrayIdType> > &bigArraysI)
{
    if (tinyInt.empty())
    {
...
DataArrayPartDefinition::New(DataArrayIdType *listOfIds)
{
    return new DataArrayPartDefinition(listOfIds);
}

bool
DataArrayPartDefinition::isEqual(const PartDefinition *other, std::string &what) const
{
    if (!other)
    {
...
PartDefinition *
DataArrayPartDefinition::operator+(const PartDefinition &other) const
{
...
std::string
DataArrayPartDefinition::getRepr() const
{
 ...
 * Example : if \a other is SlicePart(4,14,1) and if \a this is DataArrayPartDefinition([0,1,2,3,6,7,8,9]) ->
 * DataArrayPartDefinition([4,5,6,7,11,12,13]) will be returned
 */
PartDefinition *
DataArrayPartDefinition::composeWith(const PartDefinition *other) const
{
    if (!other)
    ...
    {  // special case for optim
        ...
        if (c == 1)
        {
            ...
            std::transform(
                ...
                _arr->end(),
                arr->getPointer(),
                std::bind(std::plus<mcIdType>(), std::placeholders::_1, a)
            );
...
void
DataArrayPartDefinition::checkConsistencyLight() const
{
 ...
 */
PartDefinition *
DataArrayPartDefinition::tryToSimplify() const
{
    checkConsistencyLight();
    mcIdType a(0), b(0), c(0);
...
void
DataArrayPartDefinition::serialize(
...
void
DataArrayPartDefinition::checkInternalArrayOK() const
{
    CheckInternalArrayOK(_arr);
}

void
DataArrayPartDefinition::CheckInternalArrayOK(const DataArrayIdType *listOfIds)
{
    if (!listOfIds || !listOfIds->isAllocated() || listOfIds->getNumberOfComponents() != 1)
        throw INTERP_KERNEL::Exception(
            "DataArrayPartDefinition::CheckInternalArrayOK : Input list must be not null allocated and with one "
            "components !"
        );
}

void
DataArrayPartDefinition::updateTime() const
{
...
std::size_t
DataArrayPartDefinition::getHeapMemorySizeWithoutChildren() const
{
    return sizeof(DataArrayPartDefinition);
}

std::vector<const BigMemoryObject *>
DataArrayPartDefinition::getDirectChildrenWithNull() const
{
...
SlicePartDefinition *
SlicePartDefinition::New(mcIdType start, mcIdType stop, mcIdType step)
{
    return new SlicePartDefinition(start, stop, step);
}

bool
SlicePartDefinition::isEqual(const PartDefinition *other, std::string &what) const
{
    if (!other)
    {
...
PartDefinition *
SlicePartDefinition::operator+(const PartDefinition &other) const
{
    const PartDefinition *otherPt(&other);
    if (!otherPt)
        throw INTERP_KERNEL::Exception("DataArrayPartDefinition::operator+ : NULL input !");
...
/*!
 * This method operates FoG where F is \a this and G is \a other.
 * Example : if \a this is SlicePart(4,6,1) and if \a other is DataArrayPartDefinition([12,13,17,18,22,28,34,44]) ->
 * DataArrayPartDefinition([22,28]) will be returned
 */
PartDefinition *
SlicePartDefinition::composeWith(const PartDefinition *other) const
{
    if (!other)
 ...
 */
void
SlicePartDefinition::checkConsistencyLight() const
{
}
 ...
 */
PartDefinition *
SlicePartDefinition::tryToSimplify() const
{
...
void
SlicePartDefinition::serialize(std::vector<mcIdType> &tinyInt, std::vector<MCAuto<DataArrayIdType> > &bigArraysI) const
{
...
std::string
SlicePartDefinition::getRepr() const
{
    std::ostringstream oss;
...
void
SlicePartDefinition::getSlice(mcIdType &start, mcIdType &stop, mcIdType &step) const
{
    start = _start;
    stop = _stop;
    step = _step;
...
SlicePartDefinition::SlicePartDefinition(mcIdType start, mcIdType stop, mcIdType step)
 ...
 */
void
SlicePartDefinition::updateTime() const
{
}

std::size_t
SlicePartDefinition::getHeapMemorySizeWithoutChildren() const
{
    return sizeof(SlicePartDefinition);
}

std::vector<const BigMemoryObject *>
SlicePartDefinition::getDirectChildrenWithNull() const
{
    return std::vector<const BigMemoryObject *>();
}

DataArrayPartDefinition *
SlicePartDefinition::add1(const DataArrayPartDefinition *other) const
{
    MCAuto<DataArrayIdType> a1(toDAI()), a2(other->toDAI());
    MCAuto<DataArrayIdType> a3(DataArrayIdType::Aggregate(a1, a2, 0));
    a3->sort();
...
PartDefinition *
SlicePartDefinition::add2(const SlicePartDefinition *other) const
{
