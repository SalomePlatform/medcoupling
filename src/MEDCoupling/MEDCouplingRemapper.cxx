...
MEDCouplingRemapper::MEDCouplingRemapper()
    : _src_ft(0), _target_ft(0), _interp_matrix_pol(IK_ONLY_PREFERED), _nature_of_deno(NoNature), _time_deno_update(0)
{
}

MEDCouplingRemapper::~MEDCouplingRemapper() { releaseData(false); }
 ...
 *
 * This method performs the second step (computation of remapping matrix) which may be CPU-time consuming. This phase is
 * also the most critical (where the most tricky algorithm) in the remapping process. Strictly speaking to perform the
 * computation of the remapping matrix the field templates source-side and target-side is required (which is the case of
 * MEDCouplingRemapper::prepareEx). So this method is less precise but a more user friendly way to compute a remapping
 * matrix.
 *
 * \param [in] srcMesh the source mesh
 * \param [in] targetMesh the target mesh
 * \param [in] method A string obtained by aggregation of the spatial discretisation string representation of source
 * field and target field. The string representation is those returned by MEDCouplingFieldDiscretization::getStringRepr.
 *             Example : "P0" is for cell discretization. "P1" is for node discretization. So "P0P1" for \a method
 * parameter means from a source cell field (lying on \a srcMesh) to a target node field (lying on \a targetMesh).
 *
 * \sa MEDCouplingRemapper::prepareEx
 */
int
MEDCouplingRemapper::prepare(
    const MEDCouplingMesh *srcMesh, const MEDCouplingMesh *targetMesh, const std::string &method
)
{
...
/*!
 * This method is the generalization of MEDCouplingRemapper::prepare. Indeed, MEDCouplingFieldTemplate instances gives
 * all required information to compute the remapping matrix. This method must be used instead of
 * MEDCouplingRemapper::prepare if Gauss point to Gauss point must be applied.
 *
 ...
 */
int
MEDCouplingRemapper::prepareEx(const MEDCouplingFieldTemplate *src, const MEDCouplingFieldTemplate *target)
{
...
void
MEDCouplingRemapper::setCrudeMatrix(
...
void
MEDCouplingRemapper::setCrudeMatrixEx(
...
)
{
    ...
    if (ToIdType(m.size()) != target->getNumberOfTuplesExpected())
    {
        ...
        oss << "MEDCouplingRemapper::setMatrixEx : input matrix has " << m.size() << " rows whereas there are "
            << target->getNumberOfTuplesExpected() << " expected !";
    ...
    for (auto it : m)
    {
        for (auto it2 : it)
        {
            ...
            {
                std::ostringstream oss;
                oss << "MEDCouplingRemapper::setMatrixEx : presence of elt #" << idToTest << " ! not in [0,"
                    << srcNbElem << ") !";
                throw INTERP_KERNEL::Exception(oss.str());
            }
        }
...
int
MEDCouplingRemapper::prepareInterpKernelOnly()
{
    ...
    switch (meshInterpType)
    {
        ...
        default:
            throw INTERP_KERNEL::Exception(
                "MEDCouplingRemapper::prepareInterpKernelOnly : Not managed type of meshes ! Dealt meshes type are : "
                "Unstructured<->Unstructured, Unstructured<->Cartesian, Cartesian<->Cartesian, Extruded<->Extruded !"
            );
    }
}

int
MEDCouplingRemapper::prepareNotInterpKernelOnly()
{
    ...
    switch (CheckInterpolationMethodManageableByNotOnlyInterpKernel(method))
    {
        ...
        {
            std::ostringstream oss;
            oss << "MEDCouplingRemapper::prepareNotInterpKernelOnly : INTERNAL ERROR ! the method \"" << method
                << "\" declared as managed bu not implemented !";
...
/*!
 * This method performs the operation source to target using matrix computed in
 * MEDCoupling::MEDCouplingRemapper::prepare method. If meshes of \b srcField and \b targetField do not match exactly
 * those given into \ref MEDCoupling::MEDCouplingRemapper::prepare "prepare method" an exception will be thrown.
 *
 * \param [in] srcField is the source field from which the interpolation will be done. The mesh into \b srcField should
 * be the same than those specified on MEDCoupling::MEDCouplingRemapper::prepare.
 * \param [in,out] targetField the destination field with the allocated array in which all tuples will be overwritten.
 * \param [in] dftValue is the value that will be assigned in the targetField to each entity of target mesh (entity
 * depending on the method selected on prepare invocation) that is not intercepted by any entity of source mesh. For
 * example in "P0P0" case (cell-cell) if a cell in target mesh is not overlapped by any source cell the \a dftValue
 * value will be attached on that cell in the returned \a targetField. In some cases a target cell not intercepted by
 * any source cell is a bug so in this case it is advised to set a huge value (1e300 for example) to \a dftValue to
 * quickly point to the problem. But for users doing parallelism a target cell can be intercepted by a source cell on a
 * different process. In this case 0. assigned to \a dftValue is more appropriate.
 *
 * \sa transferField
 */
void
MEDCouplingRemapper::transfer(
    const MEDCouplingFieldDouble *srcField, MEDCouplingFieldDouble *targetField, double dftValue
)
{
...
/*!
 * This method is equivalent to MEDCoupling::MEDCouplingRemapper::transfer except that here \b targetField is a in/out
 * parameter. If an entity (cell for example) in targetField is not fetched by any entity (cell for example) of \b
 * srcField, the value in targetField is let unchanged. This method requires that \b targetField was fully defined and
 * allocated. If the array is not allocated an exception will be thrown.
 *
 * \param [in] srcField is the source field from which the interpolation will be done. The mesh into \b srcField should
 * be the same than those specified on MEDCoupling::MEDCouplingRemapper::prepare.
 * \param [in,out] targetField the destination field with the allocated array in which only tuples whose entities are
 * fetched by interpolation will be overwritten only.
 */
void
MEDCouplingRemapper::partialTransfer(const MEDCouplingFieldDouble *srcField, MEDCouplingFieldDouble *targetField)
{
    if (!srcField || !targetField)
...
void
MEDCouplingRemapper::reverseTransfer(
    MEDCouplingFieldDouble *srcField, const MEDCouplingFieldDouble *targetField, double dftValue
)
{
    if (!srcField || !targetField)
    ...
    {
        std::ostringstream oss;
        oss << "MEDCouplingRemapper::reverseTransfer : in given source field the number of tuples required is "
            << _target_ft->getNumberOfTuplesExpected() << " (on prepare) and number of tuples in given target field is "
            << targetField->getNumberOfTuplesExpected();
...
/*!
 * This method performs the operation source to target using matrix computed in
 * MEDCoupling::MEDCouplingRemapper::prepare method. If mesh of \b srcField does not match exactly those given into \ref
 * MEDCoupling::MEDCouplingRemapper::prepare "prepare method" an exception will be thrown.
 *
 * \param [in] srcField is the source field from which the interpolation will be done. The mesh into \b srcField should
 * be the same than those specified on MEDCoupling::MEDCouplingRemapper::prepare.
 * \param [in] dftValue is the value that will be assigned in the targetField to each entity of target mesh (entity
...
MEDCouplingRemapper::transferField(const MEDCouplingFieldDouble *srcField, double dftValue)
{
    checkPrepare();
    if (!srcField)
    ...
    ret->copyAllTinyAttrFrom(
        srcField
    );  // perform copy of tiny strings after and not before transfer because the array will be created on transfer
    return ret.retn();
}

MEDCouplingFieldDouble *
MEDCouplingRemapper::reverseTransferField(const MEDCouplingFieldDouble *targetField, double dftValue)
{
    if (!targetField)
    ...
    ret->copyAllTinyAttrFrom(targetField);  // perform copy of tiny strings after and not before reverseTransfer because
                                            // the array will be created on reverseTransfer
    return ret;
}
 ...
 */
bool
MEDCouplingRemapper::setOptionInt(const std::string &key, int value)
{
 ...
 */
bool
MEDCouplingRemapper::setOptionDouble(const std::string &key, double value)
{
 ...
 */
bool
MEDCouplingRemapper::setOptionString(const std::string &key, const std::string &value)
{
    return INTERP_KERNEL::InterpolationOptions::setOptionString(key, value);
}

/*!
 * This method returns the interpolation matrix policy. This policy specifies which interpolation matrix method to keep
 * or preferred. If interpolation matrix policy is :
 *
 * - set to IK_ONLY_PREFERED (0) (the default) : the INTERP_KERNEL only method is preferred. That is to say, if it is
 * possible to treat the case regarding spatial discretization of source and target with INTERP_KERNEL only method,
 * INTERP_KERNEL only method will be performed. If not, the \b not only INTERP_KERNEL method will be attempt.
 *
 * - set to NOT_IK_ONLY_PREFERED (1) : the \b NOT only INTERP_KERNEL method is preferred. That is to say, if it is
 * possible to treat the case regarding spatial discretization of source and target with \b NOT only INTERP_KERNEL
 * method, \b NOT only INTERP_KERNEL method, will be performed. If not, the INTERP_KERNEL only method will be attempt.
 *
 ...
 */
int
MEDCouplingRemapper::getInterpolationMatrixPolicy() const
{
    return _interp_matrix_pol;
}

/*!
 * This method sets a new interpolation matrix policy. The default one is IK_PREFERED (0). The input is of type \c int
 * to be dealt by standard Salome CORBA component generators. This method throws an INTERP_KERNEL::Exception if a the
 * input integer is not in the available possibilities, that is to say not in [0 (IK_PREFERED) , 1 (NOT_IK_PREFERED), 2
 * (IK_ONLY_FORCED), 3 (NOT_IK_ONLY_FORCED)].
 *
 ...
 * - NOT_IK_ONLY_FORCED (3) : Only \b NOT INTERP_KERNEL only method will be launched.
 *
 * \input newInterpMatPol the new interpolation matrix method policy. This parameter is of type \c int and not of type
 * \c MEDCoupling::InterpolationMatrixPolicy for automatic generation of CORBA component.
 *
 * \sa MEDCouplingRemapper::getInterpolationMatrixPolicy
 */
void
MEDCouplingRemapper::setInterpolationMatrixPolicy(int newInterpMatPol)
{
    switch (newInterpMatPol)
    {
        ...
        default:
            throw INTERP_KERNEL::Exception(
                "MEDCouplingRemapper::setInterpolationMatrixPolicy : invalid input integer value ! Should be in [0 "
                "(IK_PREFERED) , 1 (NOT_IK_PREFERED), 2 (IK_ONLY_FORCED), 3 (NOT_IK_ONLY_FORCED)] ! For information, "
                "the default is IK_PREFERED=0 !"
            );
    }
}

int
MEDCouplingRemapper::prepareInterpKernelOnlyUU()
{
    ...
    else if (srcMeshDim == 1 && trgMeshDim == 0 && srcSpaceDim == 1)
    {
        if (getIntersectionType() != INTERP_KERNEL::PointLocator)
            throw INTERP_KERNEL::Exception(
                "Invalid interpolation requested between 1D and 0D into 1D space ! Select PointLocator as intersection "
                "type !"
            );
        MEDCouplingNormalizedUnstructuredMesh<1, 1> source_mesh_wrapper(src_mesh);
        MEDCouplingNormalizedUnstructuredMesh<1, 1> target_mesh_wrapper(target_mesh);
    ...
    else if (srcMeshDim == 1 && trgMeshDim == 0 && srcSpaceDim == 2)
    {
        if (getIntersectionType() != INTERP_KERNEL::PointLocator)
            throw INTERP_KERNEL::Exception(
                "Invalid interpolation requested between 1D and 0D into 2D space ! Select PointLocator as intersection "
                "type !"
            );
    ...
    else if (srcMeshDim == 3 && trgMeshDim == 1 && srcSpaceDim == 3)
    {
        if (getIntersectionType() != INTERP_KERNEL::PointLocator)
            throw INTERP_KERNEL::Exception(
                "Invalid interpolation requested between 3D and 1D ! Select PointLocator as intersection type !"
            );
        MEDCouplingNormalizedUnstructuredMesh<3, 3> source_mesh_wrapper(src_mesh);
        MEDCouplingNormalizedUnstructuredMesh<3, 3> target_mesh_wrapper(target_mesh);
    ...
    else if (srcMeshDim == 3 && trgMeshDim == 0 && srcSpaceDim == 3)
    {
        if (getIntersectionType() != INTERP_KERNEL::PointLocator)
            throw INTERP_KERNEL::Exception(
                "Invalid interpolation requested between 3D and 0D ! Select PointLocator as intersection type !"
            );
        MEDCouplingNormalizedUnstructuredMesh<3, 3> source_mesh_wrapper(src_mesh);
        MEDCouplingNormalizedUnstructuredMesh<3, 3> target_mesh_wrapper(target_mesh);
    ...
    else if (srcMeshDim == 1 && trgMeshDim == 0 && srcSpaceDim == 3)
    {
        if (getIntersectionType() != INTERP_KERNEL::PointLocator)
            throw INTERP_KERNEL::Exception(
                "Invalid interpolation requested between 1D and 0D into 3D space ! Select PointLocator as intersection "
                "type !"
            );
        MEDCouplingNormalizedUnstructuredMesh<3, 3> source_mesh_wrapper(src_mesh);
        MEDCouplingNormalizedUnstructuredMesh<3, 3> target_mesh_wrapper(target_mesh);
    ...
    else if (srcMeshDim == 2 && trgMeshDim == 1 && srcSpaceDim == 2)
    {
        ...
        else
        {
            ...
            ReverseMatrix(matrixTmp, nbCols, _matrix);
            nbCols = ToIdType(matrixTmp.size());
            INTERP_KERNEL::Interpolation2D1D::DuplicateFacesType duplicateFaces =
                interpolation.retrieveDuplicateFaces();
            if (!duplicateFaces.empty())
            {
                std::ostringstream oss;
                oss << "An unexpected situation happened ! For the following 1D Cells are part of edges shared by 2D "
                       "cells :\n";
                for (std::map<mcIdType, std::set<mcIdType> >::const_iterator it = duplicateFaces.begin();
                     it != duplicateFaces.end();
                     it++)
                {
    ...
    else if (srcMeshDim == 2 && trgMeshDim == 0 && srcSpaceDim == 2)
    {
        if (getIntersectionType() != INTERP_KERNEL::PointLocator)
            throw INTERP_KERNEL::Exception(
                "Invalid interpolation requested between 2D and 0D ! Select PointLocator as intersection type !"
            );
        MEDCouplingNormalizedUnstructuredMesh<2, 2> source_mesh_wrapper(src_mesh);
        MEDCouplingNormalizedUnstructuredMesh<2, 2> target_mesh_wrapper(target_mesh);
        INTERP_KERNEL::Interpolation2D interpolation(*this);
    ...
    else if (srcMeshDim == 2 && trgMeshDim == 3 && srcSpaceDim == 3)
    {
        if (getIntersectionType() == INTERP_KERNEL::PointLocator)
            throw INTERP_KERNEL::Exception(
                "Using point locator to transfer a mesh of dim 2 to a mesh of dim 3 does not make sense: 3D centers of "
                "mass can not be localized in a mesh having mesh-dim=2, space-dim=3!!"
            );
        else
        {
            MEDCouplingNormalizedUnstructuredMesh<3, 3> source_mesh_wrapper(src_mesh);
            ...
            nbCols = interpolation.interpolateMeshes(source_mesh_wrapper, target_mesh_wrapper, _matrix, method);
            INTERP_KERNEL::Interpolation2D3D::DuplicateFacesType duplicateFaces =
                interpolation.retrieveDuplicateFaces();
            if (!duplicateFaces.empty())
            {
                std::ostringstream oss;
                oss << "An unexpected situation happened ! For the following 2D Cells are part of edges shared by 3D "
                       "cells :\n";
                for (std::map<mcIdType, std::set<mcIdType> >::const_iterator it = duplicateFaces.begin();
                     it != duplicateFaces.end();
                     it++)
    ...
    else if (trgMeshDim == -1)
    {
        ...
        else if (srcMeshDim == 2 && srcSpaceDim == 3)
        {
            ...
            throw INTERP_KERNEL::Exception(
                "No interpolation available for the given mesh and space dimension of source mesh to -1D targetMesh"
            );
    }
    else if (srcMeshDim == -1)
    {
        ...
        else if (trgMeshDim == 2 && trgSpaceDim == 3)
        {
            ...
            throw INTERP_KERNEL::Exception(
                "No interpolation available for the given mesh and space dimension of source mesh from -1D sourceMesh"
            );
    }
    else
...
int
MEDCouplingRemapper::prepareInterpKernelOnlyEE()
{
    std::string srcMeth, trgMeth;
    std::string methC = checkAndGiveInterpolationMethodStr(srcMeth, trgMeth);
    const MEDCouplingMappedExtrudedMesh *src_mesh =
        static_cast<const MEDCouplingMappedExtrudedMesh *>(_src_ft->getMesh());
    const MEDCouplingMappedExtrudedMesh *target_mesh =
        static_cast<const MEDCouplingMappedExtrudedMesh *>(_target_ft->getMesh());
    if (methC != "P0P0")
        throw INTERP_KERNEL::Exception(
            "MEDCouplingRemapper::prepareInterpKernelOnlyEE : Only P0P0 method implemented for Extruded/Extruded "
            "meshes !"
        );
    MCAuto<MEDCouplingUMesh> src2D(src_mesh->getMesh2D()->clone(false));
    src2D->changeSpaceDimension(2, 0.);
    MCAuto<MEDCouplingUMesh> trg2D(target_mesh->getMesh2D()->clone(false));
    trg2D->changeSpaceDimension(2, 0.);
    ...
    MEDCouplingMappedExtrudedMesh::Project1DMeshes(
        src_mesh->getMesh1D(), target_mesh->getMesh1D(), getPrecision(), s1D, t1D, v
    );
    ...
    if (interpolation1D.getIntersectionType() ==
        INTERP_KERNEL::Geometric2D)  // For intersection type of 1D, Geometric2D do not deal with it ! -> make
                                     // interpolation1D not inherite from this
    ...
    s1D->decrRef();
    t1D->decrRef();
    buildFinalInterpolationMatrixByConvolution(
        matrix1D,
        matrix2D,
        src_mesh->getMesh3DIds()->getConstPointer(),
        nbCols2D,
        nbCols1D,
...
int
MEDCouplingRemapper::prepareInterpKernelOnlyUC()
{
    std::string srcMeth, trgMeth;
    std::string methodCpp = checkAndGiveInterpolationMethodStr(srcMeth, trgMeth);
    if (methodCpp != "P0P0")
        throw INTERP_KERNEL::Exception(
            "MEDCouplingRemapper::prepareInterpKernelOnlyUC: only P0P0 interpolation supported for the moment !"
        );
    if (InterpolationOptions::getIntersectionType() != INTERP_KERNEL::Triangulation)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingRemapper::prepareInterpKernelOnlyUC: only 'Triangulation' intersection type supported!"
        );
    ...
    if (srcMeshDim != srcSpceDim || srcMeshDim != trgMeshDim)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingRemapper::prepareInterpKernelOnlyUC: space dimension of unstructured source mesh should be "
            "equal to mesh dimension of unstructured source mesh, and should also be equal to target cartesian "
            "dimension!"
        );
    ...
    switch (srcMeshDim)
    {
        ...
        default:
            throw INTERP_KERNEL::Exception(
                "MEDCouplingRemapper::prepareInterpKernelOnlyUC : only dimension 1 2 or 3 supported !"
            );
    }
...
int
MEDCouplingRemapper::prepareInterpKernelOnlyCU()
{
    std::string srcMeth, trgMeth;
    std::string methodCpp = checkAndGiveInterpolationMethodStr(srcMeth, trgMeth);
    if (methodCpp != "P0P0")
        throw INTERP_KERNEL::Exception(
            "MEDCouplingRemapper::prepareInterpKernelOnlyCU : only P0P0 interpolation supported for the moment !"
        );
    if (InterpolationOptions::getIntersectionType() != INTERP_KERNEL::Triangulation)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingRemapper::prepareInterpKernelOnlyCU: only 'Triangulation' intersection type supported!"
        );
    ...
    if (trgMeshDim != trgSpceDim || trgMeshDim != srcMeshDim)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingRemapper::prepareInterpKernelOnlyUC: space dimension of unstructured target mesh should be "
            "equal to mesh dimension of unstructured target mesh, and should also be equal to source cartesian "
            "dimension!"
        );
    switch (srcMeshDim)
    {
        ...
        default:
            throw INTERP_KERNEL::Exception(
                "MEDCouplingRemapper::prepareInterpKernelOnlyCU : only dimension 1 2 or 3 supported !"
            );
    }
    nullifiedTinyCoeffInCrudeMatrixAbs(0.);
    //
...
int
MEDCouplingRemapper::prepareInterpKernelOnlyCC()
{
    std::string srcMeth, trgMeth;
    std::string methodCpp = checkAndGiveInterpolationMethodStr(srcMeth, trgMeth);
    if (methodCpp != "P0P0")
        throw INTERP_KERNEL::Exception(
            "MEDCouplingRemapper::prepareInterpKernelOnlyCC : only P0P0 interpolation supported for the moment !"
        );
    if (InterpolationOptions::getIntersectionType() != INTERP_KERNEL::Triangulation)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingRemapper::prepareInterpKernelOnlyCC: only 'Triangulation' intersection type supported!"
        );
    const MEDCouplingCMesh *src_mesh = static_cast<const MEDCouplingCMesh *>(_src_ft->getMesh());
    const MEDCouplingCMesh *target_mesh = static_cast<const MEDCouplingCMesh *>(_target_ft->getMesh());
    const int srcMeshDim = src_mesh->getMeshDimension();
    ...
    if (trgMeshDim != srcMeshDim)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingRemapper::prepareInterpKernelOnlyCC : dimension of target cartesian mesh should be equal to "
            "dimension of source cartesian mesh !"
        );
    switch (srcMeshDim)
    {
        case 1:
        ...
        default:
            throw INTERP_KERNEL::Exception(
                "MEDCouplingRemapper::prepareInterpKernelOnlyCC : only dimension 1 2 or 3 supported !"
            );
    }
    nullifiedTinyCoeffInCrudeMatrixAbs(0.);
    //
...
int
MEDCouplingRemapper::prepareNotInterpKernelOnlyGaussGauss()
{
    if (getIntersectionType() != INTERP_KERNEL::PointLocator)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingRemapper::prepareNotInterpKernelOnlyGaussGauss : The intersection type is not supported ! Only "
            "PointLocator is supported for Gauss->Gauss interpolation ! Please invoke "
            "setIntersectionType(PointLocator) on the MEDCouplingRemapper instance !"
        );
    ...
    {
        std::ostringstream oss;
        oss << "MEDCouplingRemapper::prepareNotInterpKernelOnlyGaussGauss : space dimensions mismatch between source "
               "and target !";
        oss << " Target discretization localization has dimension " << trgSpaceDim
            << ", whereas the space dimension of source is equal to ";
    ...
    _src_ft->getMesh()->getCellsContainingPointsLinearPartOnlyOnNonDynType(
        trgLoc->begin(), trgNbOfGaussPts, getPrecision(), eltsArr, eltsIndexArr
    );
    ...
    for (const mcIdType *trgId = ids0->begin(); trgId != ids0->end(); trgId++)
    {
        ...
        for (mcIdType srcId = srcOffsetArrPtr[srcCellId]; srcId < srcOffsetArrPtr[srcCellId + 1]; srcId++)
        {
            ...
            double tmp = 0.;
            for (mcIdType i = 0; i < trgSpaceDim; i++) tmp += (ptTrg[i] - ptSrc[i]) * (ptTrg[i] - ptSrc[i]);
            if (tmp < dist)
            {
                dist = tmp;
                srcEntry = srcId;
    ...
    if (ids0->getNumberOfTuples() != trgNbOfGaussPts)
    {
        ...
        for (const mcIdType *orphanTrgId = orphanTrgIds->begin(); orphanTrgId != orphanTrgIds->end();
             orphanTrgId++, srcIdPerTrgPtr++)
...
int
MEDCouplingRemapper::prepareNotInterpKernelOnlyFEFE()
{
    if (getIntersectionType() != INTERP_KERNEL::PointLocator)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingRemapper::prepareNotInterpKernelOnlyFEFE : The intersection type is not supported ! Only "
            "PointLocator is supported for FE->FE interpolation ! Please invoke setIntersectionType(PointLocator) on "
            "the MEDCouplingRemapper instance !"
        );
    MCAuto<DataArrayDouble> trgLoc = _target_ft->getLocalizationOfDiscr();
    mcIdType trgSpaceDim = ToIdType(trgLoc->getNumberOfComponents());
    ...
    MEDCouplingFieldDiscretizationOnNodesFE::computeCrudeMatrix(
        srcUMesh, coordsOfTrgMesh, nbOfTrgPts, this->_matrix, getFEOptions()
    );
 ...
 * If no an INTERP_KERNEL::Exception will be thrown. If yes, a magic number will be returned to switch in the
 * MEDCouplingRemapper::prepareNotInterpKernelOnly method.
 */
int
MEDCouplingRemapper::CheckInterpolationMethodManageableByNotOnlyInterpKernel(const std::string &method)
{
    ...
    if (method == "FEFE")
        return 1;
    std::ostringstream oss;
    oss << "MEDCouplingRemapper::CheckInterpolationMethodManageableByNotOnlyInterpKernel : ";
 ...
 * MEDCouplingRemapper::setInterpolationMatrixPolicy and by default equal to IK_ONLY_PREFERED, which method will be
 * applied. If \c true is returned the INTERP_KERNEL only method should be applied to \c false the \b not only
 * INTERP_KERNEL method should be applied.
 */
bool
MEDCouplingRemapper::isInterpKernelOnlyOrNotOnly() const
{
    std::string srcm, trgm, method;
    method = checkAndGiveInterpolationMethodStr(srcm, trgm);
    switch (_interp_matrix_pol)
    {
        case IK_ONLY_PREFERED:
        {
            try
            {
                ...
                INTERP_KERNEL::Interpolation<INTERP_KERNEL::Interpolation3D>::CheckAndSplitInterpolationMethod(
                    method, tmp1, tmp2
                );
                return true;
            }
        ...
        default:
            throw INTERP_KERNEL::Exception(
                "MEDCouplingRemapper::isInterpKernelOnlyOrNotOnly : internal error ! The interpolation matrix policy "
                "is not managed ! Try to change it using MEDCouplingRemapper::setInterpolationMatrixPolicy !"
            );
    }
}

void
MEDCouplingRemapper::updateTime() const
{
}

void
MEDCouplingRemapper::checkPrepare() const
{
    const MEDCouplingFieldTemplate *s(_src_ft), *t(_target_ft);
    if (!s || !t)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingRemapper::checkPrepare : it appears that MEDCouplingRemapper::prepare(Ex) has not been called !"
        );
    if (!s->getMesh() || !t->getMesh())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingRemapper::checkPrepare : it appears that no all field templates have their mesh set !"
        );
}

void
MEDCouplingRemapper::synchronizeSizeOfSideMatricesAfterMatrixComputation(mcIdType nbOfColsInMatrix)
{
 ...
 */
std::string
MEDCouplingRemapper::checkAndGiveInterpolationMethodStr(std::string &srcMeth, std::string &trgMeth) const
{
    const MEDCouplingFieldTemplate *s(_src_ft), *t(_target_ft);
    if (!s || !t)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingRemapper::checkAndGiveInterpolationMethodStr : it appears that no all field templates have "
            "been set !"
        );
    if (!s->getMesh() || !t->getMesh())
        throw INTERP_KERNEL::Exception(
...
std::string
MEDCouplingRemapper::BuildMethodFrom(const std::string &meth1, const std::string &meth2)
{
    std::string method(meth1);
    method += meth2;
    return method;
}

void
MEDCouplingRemapper::BuildFieldTemplatesFrom(
    const MEDCouplingMesh *srcMesh,
    const MEDCouplingMesh *targetMesh,
    const std::string &method,
...
)
{
    if (!srcMesh || !targetMesh)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingRemapper::BuildFieldTemplatesFrom : presence of NULL input pointer !"
        );
    std::string srcMethod, targetMethod;
    INTERP_KERNEL::Interpolation<INTERP_KERNEL::Interpolation3D>::CheckAndSplitInterpolationMethod(
        method, srcMethod, targetMethod
    );
...
void
MEDCouplingRemapper::releaseData(bool matrixSuppression)
{
    _src_ft = 0;
    _target_ft = 0;
...
void
MEDCouplingRemapper::restartUsing(const MEDCouplingFieldTemplate *src, const MEDCouplingFieldTemplate *target)
{
    if (!src || !target)
    ...
    if (!src->getMesh() || !target->getMesh())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingRemapper::prepareEx : presence of NULL mesh pointer in given field template !"
        );
    releaseData(true);
...
void
MEDCouplingRemapper::transferUnderground(
...
)
{
    ...
    {
        std::ostringstream oss;
        oss << "MEDCouplingRemapper::transferUnderground : in given source field the number of tuples required is "
            << _src_ft->getNumberOfTuplesExpected() << " (on prepare) and number of tuples in given source field is "
            << srcField->getNumberOfTuplesExpected();
    ...
    else
    {
        if (!isDftVal)
            throw INTERP_KERNEL::Exception(
                "MEDCouplingRemapper::partialTransfer : This method requires that the array of target field exists ! "
                "Allocate it or call MEDCouplingRemapper::transfer instead !"
            );
        MCAuto<DataArrayDouble> tmp(DataArrayDouble::New());
...
void
MEDCouplingRemapper::computeDeno(
...
void
MEDCouplingRemapper::computeDenoFromScratch(
    NatureOfField nat, const MEDCouplingFieldDouble *srcField, const MEDCouplingFieldDouble *trgField
)
{
    _nature_of_deno = nat;
    switch (_nature_of_deno)
    {
        ...
        {
            MEDCouplingFieldDouble *deno =
                srcField->getDiscretization()->getMeasureField(srcField->getMesh(), getMeasureAbsStatus());
            MEDCouplingFieldDouble *denoR =
                trgField->getDiscretization()->getMeasureField(trgField->getMesh(), getMeasureAbsStatus());
            ...
            int idx = 0;
            for (std::vector<std::map<mcIdType, double> >::const_iterator iter1 = _matrix.begin();
                 iter1 != _matrix.end();
                 iter1++, idx++)
                for (std::map<mcIdType, double>::const_iterator iter2 = (*iter1).begin(); iter2 != (*iter1).end();
                     iter2++)
                {
...
void
MEDCouplingRemapper::computeProduct(
...
)
{
    ...
    for (std::vector<std::map<mcIdType, double> >::const_iterator iter1 = _matrix.begin(); iter1 != _matrix.end();
         iter1++, idx++)
    {
        if ((*iter1).empty())
        {
        ...
        {
            std::transform(
                inputPointer + (*iter2).first * inputNbOfCompo,
                inputPointer + ((*iter2).first + 1) * inputNbOfCompo,
                tmp,
                std::bind(std::multiplies<double>(), std::placeholders::_1, (*iter2).second / (*iter3).second)
            );
            std::transform(
                tmp,
                tmp + inputNbOfCompo,
...
void
MEDCouplingRemapper::computeReverseProduct(
    const double *inputPointer, int inputNbOfCompo, double dftValue, double *resPointer
)
{
         ...
         iter1++, idx++)
    {
        for (std::map<mcIdType, double>::const_iterator iter2 = (*iter1).begin(); iter2 != (*iter1).end(); iter2++)
        {
            ...
            std::transform(
                inputPointer + idx * inputNbOfCompo,
                inputPointer + (idx + 1) * inputNbOfCompo,
                tmp,
                std::bind(
                    std::multiplies<double>(),
                    std::placeholders::_1,
                    (*iter2).second / _deno_reverse_multiply[(*iter2).first][idx]
                )
            );
            std::transform(
                tmp,
                tmp + inputNbOfCompo,
                resPointer + ((*iter2).first) * inputNbOfCompo,
                resPointer + ((*iter2).first) * inputNbOfCompo,
                std::plus<double>()
            );
        }
...
void
MEDCouplingRemapper::ReverseMatrix(
...
)
{
    ...
    mcIdType id = 0;
    for (std::vector<std::map<mcIdType, double> >::const_iterator iter1 = matIn.begin(); iter1 != matIn.end();
         iter1++, id++)
        for (std::map<mcIdType, double>::const_iterator iter2 = (*iter1).begin(); iter2 != (*iter1).end(); iter2++)
...
void
MEDCouplingRemapper::ComputeRowSumAndColSum(
...
)
{
    ...
    mcIdType idx = 0;
    for (std::vector<std::map<mcIdType, double> >::const_iterator iter1 = matrixDeno.begin(); iter1 != matrixDeno.end();
         iter1++, idx++)
    {
        double sum = 0.;
        for (std::map<mcIdType, double>::const_iterator iter2 = (*iter1).begin(); iter2 != (*iter1).end(); iter2++)
...
void
MEDCouplingRemapper::ComputeColSumAndRowSum(
    const std::vector<std::map<mcIdType, double> > &matrixDeno,
    std::vector<std::map<mcIdType, double> > &deno,
    std::vector<std::map<mcIdType, double> > &denoReverse
)
...
void
MEDCouplingRemapper::buildFinalInterpolationMatrixByConvolution(
...
)
{
    ...
    mcIdType id2R = 0;
    for (std::vector<std::map<mcIdType, double> >::const_iterator iter2R = m2D.begin(); iter2R != m2D.end();
         iter2R++, id2R++)
    {
        for (std::map<mcIdType, double>::const_iterator iter2C = (*iter2R).begin(); iter2C != (*iter2R).end(); iter2C++)
        {
            mcIdType id1R = 0;
            for (std::vector<std::map<mcIdType, double> >::const_iterator iter1R = m1D.begin(); iter1R != m1D.end();
                 iter1R++, id1R++)
            {
                for (std::map<mcIdType, double>::const_iterator iter1C = (*iter1R).begin(); iter1C != (*iter1R).end();
                     iter1C++)
                {
                    _matrix[corrCellIdTrg[id1R * nbOf2DCellsTrg + id2R]]
                           [corrCellIdSrc[(*iter1C).first * nbOf2DCellsSrc + (*iter2C).first]] =
                               (*iter1C).second * ((*iter2C).second);
                }
            }
        }
    }
...
void
MEDCouplingRemapper::PrintMatrix(const std::vector<std::map<mcIdType, double> > &m)
{
    mcIdType id = 0;
...
const std::vector<std::map<mcIdType, double> > &
MEDCouplingRemapper::getCrudeMatrix() const
{
    return _matrix;
}
 ...
 */
mcIdType
MEDCouplingRemapper::getNumberOfColsOfMatrix() const
{
    return ToIdType(_deno_reverse_multiply.size());
}

/*!
 * This method is supposed to be called , if needed, right after MEDCouplingRemapper::prepare or
 * MEDCouplingRemapper::prepareEx. If not the behaviour is unpredictable. This method works on precomputed \a
 * this->_matrix. All coefficients in the matrix is lower than \a maxValAbs this coefficient is set to 0. That is to say
 * that its entry disappear from the map storing the corresponding row in the data storage of sparse crude matrix. This
 * method is useful to correct at a high level some problems linked to precision. Indeed, with some \ref NatureOfField
 * "natures of field" some threshold effect can occur.
 *
 * \param [in] maxValAbs is a limit behind which a coefficient is set to 0. \a maxValAbs is expected to be positive, if
 * not this method do nothing.
 * \return a positive value that tells the number of coefficients put to 0. The 0 returned value means that the matrix
 * has remained unchanged.
 * \sa MEDCouplingRemapper::nullifiedTinyCoeffInCrudeMatrix
 */
int
MEDCouplingRemapper::nullifiedTinyCoeffInCrudeMatrixAbs(double maxValAbs)
{
    int ret = 0;
    std::vector<std::map<mcIdType, double> > matrixNew(_matrix.size());
    mcIdType i = 0;
    for (std::vector<std::map<mcIdType, double> >::const_iterator it1 = _matrix.begin(); it1 != _matrix.end();
         it1++, i++)
    {
 ...
 * This method is supposed to be called , if needed, right after MEDCouplingRemapper::prepare or
 * MEDCouplingRemapper::prepareEx. If not the behaviour is unpredictable. This method works on precomputed \a
 * this->_matrix. All coefficients in the matrix is lower than delta multiplied by \a scaleFactor this coefficient is
 * set to 0. That is to say that its entry disappear from the map storing the corresponding row in the data storage of
 * sparse crude matrix. delta is the value returned by MEDCouplingRemapper::getMaxValueInCrudeMatrix method. This method
 * is useful to correct at a high level some problems linked to precision. Indeed, with some \ref NatureOfField "natures
 * of field" some threshold effect can occur.
 *
 * \param [in] scaleFactor is the scale factor from which coefficients lower than \a scaleFactor times range width of
 * coefficients are set to zero.
 * \return a positive value that tells the number of coefficients put to 0. The 0 returned value means that the matrix
 * has remained unchanged. If -1 is returned it means that all coefficients are null.
 * \sa MEDCouplingRemapper::nullifiedTinyCoeffInCrudeMatrixAbs
 */
int
MEDCouplingRemapper::nullifiedTinyCoeffInCrudeMatrix(double scaleFactor)
{
...
/*!
 * This method is supposed to be called , if needed, right after MEDCouplingRemapper::prepare or
 * MEDCouplingRemapper::prepareEx. If not the behaviour is unpredictable. This method returns the maximum of the
 * absolute values of coefficients into the sparse crude matrix. The returned value is positive.
 */
double
MEDCouplingRemapper::getMaxValueInCrudeMatrix() const
{
    double ret = 0.;
