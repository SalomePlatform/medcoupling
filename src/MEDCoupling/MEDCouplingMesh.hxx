...
class MEDCouplingMesh : public RefCountObject, public TimeLabel
{
   public:
    ...
    MEDCOUPLING_EXPORT virtual void checkDeepEquivalWith(
        const MEDCouplingMesh *other, int cellCompPol, double prec, DataArrayIdType *&cellCor, DataArrayIdType *&nodeCor
    ) const = 0;
    MEDCOUPLING_EXPORT virtual void checkDeepEquivalOnSameNodesWith(
    ...
    MEDCOUPLING_EXPORT void checkGeoEquivalWith(
    ...
    MEDCOUPLING_EXPORT virtual DataArrayIdType *getCellIdsFullyIncludedInNodeIds(
    ...
    MEDCOUPLING_EXPORT virtual DataArrayIdType *checkTypeConsistencyAndContig(
        const std::vector<mcIdType> &code, const std::vector<const DataArrayIdType *> &idsPerType
    ) const = 0;
    MEDCOUPLING_EXPORT virtual void splitProfilePerType(
    ...
    MEDCOUPLING_EXPORT virtual void getCellsContainingPoint(
        const double *pos, double eps, std::vector<mcIdType> &elts
    ) const = 0;
    MEDCOUPLING_EXPORT virtual void getCellsContainingPoints(
        const double *pos,
        mcIdType nbOfPoints,
        double eps,
    ...
    MEDCOUPLING_EXPORT virtual void getCellsContainingPointsLinearPartOnlyOnNonDynType(
        const double *pos,
        mcIdType nbOfPoints,
        double eps,
        MCAuto<DataArrayIdType> &elts,
    ...
    MEDCOUPLING_EXPORT virtual MEDCouplingFieldDouble *fillFromAnalytic(
        TypeOfField t, int nbOfComp, FunctionToEvaluate func
    ) const;
    MEDCOUPLING_EXPORT virtual MEDCouplingFieldDouble *fillFromAnalytic(
        TypeOfField t, int nbOfComp, const std::string &func
    ) const;
    MEDCOUPLING_EXPORT virtual MEDCouplingFieldDouble *fillFromAnalyticCompo(
        TypeOfField t, int nbOfComp, const std::string &func
    ) const;
    MEDCOUPLING_EXPORT virtual MEDCouplingFieldDouble *fillFromAnalyticNamedCompo(
    ...
    MEDCOUPLING_EXPORT virtual MEDCouplingMesh *buildPartAndReduceNodes(
        const mcIdType *start, const mcIdType *end, DataArrayIdType *&arr
    ) const = 0;
    MEDCOUPLING_EXPORT virtual MEDCouplingMesh *buildPartRange(
        mcIdType beginCellIds, mcIdType endCellIds, mcIdType stepCellIds
    ) const;
    MEDCOUPLING_EXPORT virtual MEDCouplingMesh *buildPartRangeAndReduceNodes(
    ...
    MEDCOUPLING_EXPORT virtual void getReverseNodalConnectivity(
    ...
    MEDCOUPLING_EXPORT static INTERP_KERNEL::NormalizedCellType GetCorrespondingPolyType(
    ...
    MEDCOUPLING_EXPORT virtual void getTinySerializationInformation(
    ...
    MEDCOUPLING_EXPORT virtual void unserialization(
    ...
    /// @cond INTERNAL
    MEDCOUPLING_EXPORT void writeVTKAdvanced(
        const std::string &fileName, const std::string &cda, const std::string &pda, DataArrayByte *byteData
    ) const;
    MEDCOUPLING_EXPORT static void SplitExtension(
        const std::string &fileName, std::string &baseName, std::string &extension
    );
    /// @endcond
    MEDCOUPLING_EXPORT virtual void writeVTKLL(
