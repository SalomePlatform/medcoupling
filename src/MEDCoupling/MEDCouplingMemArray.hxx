...
class MEDCouplingPointer
{
   public:
    ...
    {
        if (_internal)
            return _internal + offset;
        else
            return _external + offset;
    }
    T *getPointer()
    {
        if (_internal)
            return _internal;
        if (_external)
            throw INTERP_KERNEL::Exception("Trying to write on an external pointer.");
        else
            return 0;
    }
...
DACheckNbOfTuplesAndComp(const DataArray *da, mcIdType nbOfTuples, std::size_t nbOfCompo, const std::string &msg);
...
class MEDCOUPLING_EXPORT DataArray : public RefCountObject, public TimeLabel
{
   public:
    ...
    virtual void setContigPartOfSelectedValues(
        mcIdType tupleIdStart, const DataArray *aBase, const DataArrayIdType *tuplesSelec
    ) = 0;
    virtual void setContigPartOfSelectedValuesSlice(
    ...
    static mcIdType GetNumberOfItemGivenBESRelative(
        mcIdType begin, mcIdType end, mcIdType step, const std::string &msg
    );
...
class MEDCOUPLING_EXPORT_TEMPLATE DataArrayTemplate : public DataArray
{
   ...
   public:
    ...
    {
        return ToIdType(_info_on_compo.empty() ? 0 : _mem.getNbOfElem() / getNumberOfComponents());
    }
    ...
    {
        _mem[tupleId * _info_on_compo.size() + compoId] = newVal;
        declareAsNew();
    }
    ...
    void setContigPartOfSelectedValuesSlice(
        mcIdType tupleIdStart, const DataArray *aBase, mcIdType bg, mcIdType end2, mcIdType step
    );
    ...
    {
        std::copy(
            _mem.getConstPointerLoc(tupleId * _info_on_compo.size()),
            _mem.getConstPointerLoc((tupleId + 1) * _info_on_compo.size()),
            res
        );
    }
    template <class InputIterator>
    ...
    {
        _mem.writeOnPlace(id, element0, others, sizeOfOthers);
    }
    void fillWithZero();
   ...
   protected:
    ...
    typename Traits<T>::ArrayType *mySelectByTupleRanges(
        const std::vector<std::pair<mcIdType, mcIdType> > &ranges
    ) const;
...
class MEDCOUPLING_EXPORT_TEMPLATE DataArrayTemplateClassic : public DataArrayTemplate<T>
{
   public:
    ...
    static typename Traits<T>::ArrayType *Substract(
        const typename Traits<T>::ArrayType *a1, const typename Traits<T>::ArrayType *a2
    );
    static typename Traits<T>::ArrayType *Divide(
        const typename Traits<T>::ArrayType *a1, const typename Traits<T>::ArrayType *a2
    );
    static typename Traits<T>::ArrayType *Add(
        const typename Traits<T>::ArrayType *a1, const typename Traits<T>::ArrayType *a2
    );
    static typename Traits<T>::ArrayType *Multiply(
        const typename Traits<T>::ArrayType *a1, const typename Traits<T>::ArrayType *a2
    );
    static typename Traits<T>::ArrayType *Meld(
        const typename Traits<T>::ArrayType *a1, const typename Traits<T>::ArrayType *a2
    );
...
class MEDCOUPLING_EXPORT DataArrayFloat : public DataArrayTemplateFP<float>
{
   ...
   public:  // abstract method overload
    ...
    DataArrayFloat *selectByTupleRanges(const std::vector<std::pair<mcIdType, mcIdType> > &ranges) const
    {
        return DataArrayTemplateFP<float>::mySelectByTupleRanges(ranges);
    }
    DataArrayFloat *keepSelectedComponents(const std::vector<std::size_t> &compoIds) const
    {
        return DataArrayTemplateFP<float>::myKeepSelectedComponents(compoIds);
    }
    DataArrayFloat *selectByTupleId(const mcIdType *new2OldBg, const mcIdType *new2OldEnd) const
    {
        return this->mySelectByTupleId(new2OldBg, new2OldEnd);
    }
    DataArrayFloat *selectByTupleIdSafe(const mcIdType *new2OldBg, const mcIdType *new2OldEnd) const
    {
        return DataArrayTemplateFP<float>::mySelectByTupleIdSafe(new2OldBg, new2OldEnd);
    }
    DataArrayFloat *selectByTupleIdSafeSlice(mcIdType bg, mcIdType end2, mcIdType step) const
    {
        return DataArrayTemplateFP<float>::mySelectByTupleIdSafeSlice(bg, end2, step);
    }
...
class MEDCOUPLING_EXPORT DataArrayDouble : public DataArrayTemplateFP<double>
{
   public:
    ...
    void computeTupleIdsNearTuples(
    ...
    DataArrayDouble *applyFuncNamedCompo(
        std::size_t nbOfComp, const std::vector<std::string> &varsOrder, const std::string &func, bool isSafe = true
    ) const;
    ...
    static void ComputeIntegralOfSeg2IntoTri3(
   ...
   public:
    ...
    template <int SPACEDIM>
    static void FindTupleIdsNearTuplesAlg(
        const BBTreePts<SPACEDIM, mcIdType> &myTree,
        const double *pos,
        mcIdType nbOfTuples,
        double eps,
...
class MEDCOUPLING_EXPORT_TEMPLATE DataArrayDiscrete : public DataArrayTemplateClassic<T>
{
   ...
   public:
    ...
    DataArrayIdType *locateComponentId(
    ...
    static void PutIntoToSkylineFrmt(
   ...
   public:
    ...
    static DataArrayIdType *FindPermutationFromFirstToSecondDuplicate(
        const DataArrayType *ids1, const DataArrayType *ids2
    );
    ...
    static DataArrayIdType *ConvertIndexArrayToO2N(
    ...
    static DataArrayIdType *MakePartition(
        const std::vector<const DataArrayType *> &groups,
        mcIdType newNb,
        std::vector<std::vector<mcIdType> > &fidsOfGroups
    );

   public:
    static void ExtractFromIndexedArrays(
    ...
    static void SetPartOfIndexedArraysSameIdx(
        const mcIdType *idsOfSelectBg,
        const mcIdType *idsOfSelectEnd,
    ...
    static bool RemoveIdsFromIndexedArrays(
    ...
    static void FromVTKInternalReprOfPolyedra(
        const DataArrayType *arrIn,
        const DataArrayIdType *arrIndxIn,
        MCAuto<DataArrayType> &arrOut,
        MCAuto<DataArrayIdType> &arrIndexOut
    );
    static void FromVTK94InternalReprOfPolyedra(
        const DataArrayType *arrIn,
        const DataArrayIdType *arrIndxIn,
    ...
    static void FromVTK93To94FacesInternaReprOfPolyedra(
        const DataArrayType *arrIn,
        const DataArrayIdType *arrIndxIn,
        MCAuto<DataArrayType> &arrOut,
        MCAuto<DataArrayIdType> &arrIndexOut
...
class MEDCOUPLING_EXPORT DataArrayInt32 : public DataArrayDiscreteSigned<Int32>
{
   ...
   public:
    DataArrayInt32 *selectByTupleId(const mcIdType *new2OldBg, const mcIdType *new2OldEnd) const
    {
        return this->mySelectByTupleId(new2OldBg, new2OldEnd);
    }
    DataArrayInt32 *selectByTupleId(const DataArrayIdType &di) const { return this->mySelectByTupleId(di); }
    DataArrayInt32 *selectByTupleIdSafe(const mcIdType *new2OldBg, const mcIdType *new2OldEnd) const
    {
        return this->mySelectByTupleIdSafe(new2OldBg, new2OldEnd);
    }
    DataArrayInt32 *keepSelectedComponents(const std::vector<std::size_t> &compoIds) const
    {
        return this->myKeepSelectedComponents(compoIds);
    }
    DataArrayInt32 *selectByTupleIdSafeSlice(mcIdType bg, mcIdType end2, mcIdType step) const
    {
        return this->mySelectByTupleIdSafeSlice(bg, end2, step);
    }
    DataArrayInt32 *selectByTupleRanges(const std::vector<std::pair<mcIdType, mcIdType> > &ranges) const
    {
        return this->mySelectByTupleRanges(ranges);
    }
...
class MEDCOUPLING_EXPORT DataArrayInt64 : public DataArrayDiscreteSigned<Int64>
{
   ...
   public:
    DataArrayInt64 *selectByTupleId(const mcIdType *new2OldBg, const mcIdType *new2OldEnd) const
    {
        return this->mySelectByTupleId(new2OldBg, new2OldEnd);
    }
    DataArrayInt64 *selectByTupleId(const DataArrayIdType &di) const { return this->mySelectByTupleId(di); }
    DataArrayInt64 *selectByTupleIdSafe(const mcIdType *new2OldBg, const mcIdType *new2OldEnd) const
    {
        return DataArrayTemplate<Int64>::mySelectByTupleIdSafe(new2OldBg, new2OldEnd);
    }
    DataArrayInt64 *keepSelectedComponents(const std::vector<std::size_t> &compoIds) const
    {
        return DataArrayTemplate<Int64>::myKeepSelectedComponents(compoIds);
    }
    DataArrayInt64 *selectByTupleIdSafeSlice(mcIdType bg, mcIdType end2, mcIdType step) const
    {
        return DataArrayTemplate<Int64>::mySelectByTupleIdSafeSlice(bg, end2, step);
    }
    DataArrayInt64 *selectByTupleRanges(const std::vector<std::pair<mcIdType, mcIdType> > &ranges) const
    {
        return DataArrayTemplate<Int64>::mySelectByTupleRanges(ranges);
    }
...
DataArrayTemplateClassic<T>::findIdsAdv(const OP &op) const
{
    this->checkAllocated();
...
class MEDCOUPLING_EXPORT DataArrayChar : public DataArrayTemplate<char>
{
   public:
    ...
    DataArrayChar *selectByTupleId(const mcIdType *new2OldBg, const mcIdType *new2OldEnd) const
    {
        return this->mySelectByTupleId(new2OldBg, new2OldEnd);
    }
    DataArrayChar *selectByTupleId(const DataArrayIdType &di) const { return this->mySelectByTupleId(di); }
    DataArrayChar *selectByTupleIdSafe(const mcIdType *new2OldBg, const mcIdType *new2OldEnd) const
    {
        return DataArrayTemplate<char>::mySelectByTupleIdSafe(new2OldBg, new2OldEnd);
    }
    DataArrayChar *keepSelectedComponents(const std::vector<std::size_t> &compoIds) const
    {
        return DataArrayTemplate<char>::myKeepSelectedComponents(compoIds);
    }
    DataArrayChar *selectByTupleIdSafeSlice(mcIdType bg, mcIdType end2, mcIdType step) const
    {
        return DataArrayTemplate<char>::mySelectByTupleIdSafeSlice(bg, end2, step);
    }
    bool isUniform(char val) const;
    void meldWith(const DataArrayChar *other);
    DataArray *selectByTupleRanges(const std::vector<std::pair<mcIdType, mcIdType> > &ranges) const
    {
        return DataArrayTemplate<char>::mySelectByTupleRanges(ranges);
    }
...
class MEDCOUPLING_EXPORT DataArrayAsciiChar : public DataArrayChar
{
   public:
    ...
    DataArrayAsciiChar *copySorted(bool asc = true) const override
    {
        (void)asc;
        throw INTERP_KERNEL::Exception("DataArrayAsciiChar::copySorted : not implemented for DataArrayByte");
    }
...
template <class InputIterator>
void
MemArray<T>::insertAtTheEnd(InputIterator first, InputIterator last)
{
...
template <class InputIterator>
void
DataArrayTemplate<T>::insertAtTheEnd(InputIterator first, InputIterator last)
{
    ...
    else if (nbCompo == 0)
    {
        ...
        throw INTERP_KERNEL::Exception(
            "DataArrayDouble::insertAtTheEnd : not available for DataArrayDouble with number of components different "
            "than 1 !"
        );
}

/*!
 * This method adds at the end of \a this a series of values [\c valsBg,\c valsEnd). This method do \b not update its
 * time label to avoid useless incrementation of counter. So the caller is expected to call TimeLabel::declareAsNew on
 * \a this at the end of the push session.
 *
...
template <class InputIterator>
void
DataArrayTemplate<T>::pushBackValsSilent(InputIterator valsBg, InputIterator valsEnd)
{
    ...
    else
    {
        ...
        oss << Traits<T>::ArrayTypeName
            << "::pushBackValsSilent : not available for DataArrayDouble with number of components different than 1 !";
