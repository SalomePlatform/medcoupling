...
class MEDCoupling1GTUMesh : public MEDCouplingPointSet
{
   public:
    ...
    MEDCOUPLING_EXPORT void getCellsContainingPoint(
        const double *pos, double eps, std::vector<mcIdType> &elts
    ) const override;
    ...
    MEDCOUPLING_EXPORT static MEDCouplingUMesh *AggregateOnSameCoordsToUMesh(
        const std::vector<const MEDCoupling1GTUMesh *> &parts
    );

   public:
    MEDCOUPLING_EXPORT virtual void allocateCells(mcIdType nbOfCells = 0) = 0;
    MEDCOUPLING_EXPORT virtual void insertNextCell(
        const mcIdType *nodalConnOfCellBg, const mcIdType *nodalConnOfCellEnd
    ) = 0;
    ...
    MEDCOUPLING_EXPORT virtual void accumulateVTK93Arrays(
        mcIdType &k, mcIdType *&cPtr, mcIdType *&dPtr, mcIdType *&ePtr, mcIdType *&fPtr
    ) const = 0;
    MEDCOUPLING_EXPORT virtual void accumulateVTK94Arrays(
        mcIdType &k,
        mcIdType *&cPtr,
        mcIdType *&dPtr,
        mcIdType *&ePtr,
...
class MEDCoupling1SGTUMesh : public MEDCoupling1GTUMesh
{
   public:
    MEDCOUPLING_EXPORT static MEDCoupling1SGTUMesh *New(
        const std::string &name, INTERP_KERNEL::NormalizedCellType type
    );
    ...
    MEDCOUPLING_EXPORT MEDCouplingPointSet *buildPartOfMySelfKeepCoords(
        const mcIdType *begin, const mcIdType *end
    ) const;
    MEDCOUPLING_EXPORT MEDCouplingPointSet *buildPartOfMySelfKeepCoordsSlice(
    ...
    MEDCOUPLING_EXPORT void fillCellIdsToKeepFromNodeIds(
    ...
    MEDCOUPLING_EXPORT void accumulateVTK93Arrays(
        mcIdType &k, mcIdType *&cPtr, mcIdType *&dPtr, mcIdType *&ePtr, mcIdType *&fPtr
    ) const override;
    MEDCOUPLING_EXPORT void accumulateVTK94Arrays(
        mcIdType &k,
        mcIdType *&cPtr,
        mcIdType *&dPtr,
        mcIdType *&ePtr,
   ...
   public:  // specific
    ...
    MEDCOUPLING_EXPORT static MEDCoupling1SGTUMesh *Merge1SGTUMeshes(
        const MEDCoupling1SGTUMesh *mesh1, const MEDCoupling1SGTUMesh *mesh2
    );
    MEDCOUPLING_EXPORT static MEDCoupling1SGTUMesh *Merge1SGTUMeshes(std::vector<const MEDCoupling1SGTUMesh *> &a);
    MEDCOUPLING_EXPORT static MEDCoupling1SGTUMesh *Merge1SGTUMeshesOnSameCoords(
        std::vector<const MEDCoupling1SGTUMesh *> &a
    );
    ...
    MEDCOUPLING_EXPORT MEDCouplingCMesh *structurizeMe(
        DataArrayIdType *&cellPerm, DataArrayIdType *&nodePerm, double eps = 1e-12
    ) const;
...
class MEDCoupling1DGTUMesh : public MEDCoupling1GTUMesh
{
   ...
   public:  // specific
    ...
    MEDCOUPLING_EXPORT bool retrievePackedNodalConnectivity(
        DataArrayIdType *&nodalConn, DataArrayIdType *&nodalConnIndx
    ) const;
    MEDCOUPLING_EXPORT bool isPacked() const;
    MEDCOUPLING_EXPORT static MEDCoupling1DGTUMesh *Merge1DGTUMeshes(
        const MEDCoupling1DGTUMesh *mesh1, const MEDCoupling1DGTUMesh *mesh2
    );
    MEDCOUPLING_EXPORT static MEDCoupling1DGTUMesh *Merge1DGTUMeshes(std::vector<const MEDCoupling1DGTUMesh *> &a);
    MEDCOUPLING_EXPORT static MEDCoupling1DGTUMesh *Merge1DGTUMeshesOnSameCoords(
        std::vector<const MEDCoupling1DGTUMesh *> &a
    );
    MEDCOUPLING_EXPORT static DataArrayIdType *AggregateNodalConnAndShiftNodeIds(
        const std::vector<const DataArrayIdType *> &nodalConns, const std::vector<mcIdType> &offsetInNodeIdsPerElt
    );
    MEDCOUPLING_EXPORT static std::vector<mcIdType> BuildAPolygonFromParts(
