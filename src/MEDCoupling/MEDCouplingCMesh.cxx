...
MEDCouplingCMesh::MEDCouplingCMesh(const MEDCouplingCMesh &other, bool deepCpy)
    : MEDCouplingStructuredMesh(other, deepCpy)
{
    if (deepCpy)
    {
        if (other._x_array)
...
MEDCouplingCMesh *
MEDCouplingCMesh::New()
{
    return new MEDCouplingCMesh;
}

MEDCouplingCMesh *
MEDCouplingCMesh::New(const std::string &meshName)
{
...
MEDCouplingCMesh::getDirectAccessOfCoordsArrIfInStructure() const
{
    throw INTERP_KERNEL::Exception(
        "MEDCouplingCMesh::getDirectAccessOfCoordsArrIfInStructure : MEDCouplingCMesh does not aggregate array of "
        "coordinates !"
    );
}

MEDCouplingCurveLinearMesh *
MEDCouplingCMesh::buildCurveLinear() const
{
...
void
MEDCouplingCMesh::updateTime() const
{
    if (_x_array)
...
std::size_t
MEDCouplingCMesh::getHeapMemorySizeWithoutChildren() const
{
    return MEDCouplingStructuredMesh::getHeapMemorySizeWithoutChildren();
}

std::vector<const BigMemoryObject *>
MEDCouplingCMesh::getDirectChildrenWithNull() const
{
 ...
 */
void
MEDCouplingCMesh::copyTinyStringsFrom(const MEDCouplingMesh *other)
{
...
bool
MEDCouplingCMesh::isEqualIfNotWhy(const MEDCouplingMesh *other, double prec, std::string &reason) const
{
    if (!other)
...
bool
MEDCouplingCMesh::isEqualWithoutConsideringStr(const MEDCouplingMesh *other, double prec) const
{
    const MEDCouplingCMesh *otherC = dynamic_cast<const MEDCouplingCMesh *>(other);
    if (!otherC)
        return false;
...
void
MEDCouplingCMesh::checkDeepEquivalWith(
 ...
 * The user intend that the nodes are the same, so by construction of MEDCoupling::MEDCouplingCMesh, \a this and \a
 * other are the same !
 */
void
MEDCouplingCMesh::checkDeepEquivalOnSameNodesWith(
...
void
MEDCouplingCMesh::checkConsistencyLight() const
{
...
void
MEDCouplingCMesh::checkConsistency(double eps) const
{
    checkConsistencyLight();
    if (_x_array)
...
void
MEDCouplingCMesh::getNodeGridStructure(mcIdType *res) const
{
...
std::vector<mcIdType>
MEDCouplingCMesh::getNodeGridStructure() const
{
    static const char MSG[] =
        "MEDCouplingCMesh::getNodeGridStructure : mesh is invalid ! null vectors (X, Y or Z) must be put contiguously "
        "at the end !";
    ...
    if (_x_array)
    {
        if (!_x_array->isAllocated() || _x_array->getNumberOfComponents() != 1)
            throw INTERP_KERNEL::Exception(
                "MEDCouplingCMesh::getNodeGridStructure : X array exits but it is not allocated or with nb of "
    ...
    if (_y_array)
    {
        if (!_y_array->isAllocated() || _y_array->getNumberOfComponents() != 1)
            throw INTERP_KERNEL::Exception(
                "MEDCouplingCMesh::getNodeGridStructure : Y array exits but it is not allocated or with nb of "
                "components equal to one !"
            );
        if (!isOK)
    ...
    if (_z_array)
    {
        if (!_z_array->isAllocated() || _z_array->getNumberOfComponents() != 1)
            throw INTERP_KERNEL::Exception(
                "MEDCouplingCMesh::getNodeGridStructure : Z array exits but it is not allocated or with nb of "
                "components equal to one !"
            );
        if (!isOK)
            throw INTERP_KERNEL::Exception(MSG);
...
MEDCouplingCMesh::buildStructuredSubPart(const std::vector<std::pair<mcIdType, mcIdType> > &cellPart) const
{
    checkConsistencyLight();
    ...
    {
        std::ostringstream oss;
        oss << "MEDCouplingCMesh::buildStructuredSubPart : the space dimension is " << dim << " and cell part size is "
            << cellPart.size() << " !";
    ...
    for (int i = 0; i < dim; i++)
    {
        MCAuto<DataArrayDouble> tmp(
            ret->getCoordsAt(i)->selectByTupleIdSafeSlice(cellPart[i].first, cellPart[i].second + 1, 1)
        );
 ...
 * This method throws exceptions if the not null arrays defining this are not contiguously at the end. For example
 * X!=0,Y==0,Z!=0 will throw.
 */
int
MEDCouplingCMesh::getSpaceDimension() const
{
    return (int)getNodeGridStructure().size();
}

void
MEDCouplingCMesh::getCoordinatesOfNode(mcIdType nodeId, std::vector<double> &coo) const
{
    mcIdType tmp[3];
...
std::string
MEDCouplingCMesh::simpleRepr() const
{
...
std::string
MEDCouplingCMesh::advancedRepr() const
{
    return simpleRepr();
}
 ...
 */
const DataArrayDouble *
MEDCouplingCMesh::getCoordsAt(int i) const
{
    switch (i)
    {
        case 0:
 ...
 */
DataArrayDouble *
MEDCouplingCMesh::getCoordsAt(int i)
{
    switch (i)
    {
        case 0:
 ...
 */
void
MEDCouplingCMesh::setCoordsAt(int i, const DataArrayDouble *arr)
{
    if (arr)
 ...
 */
void
MEDCouplingCMesh::setCoords(
    const DataArrayDouble *coordsX, const DataArrayDouble *coordsY, const DataArrayDouble *coordsZ
)
{
    if (coordsX)
...
void
MEDCouplingCMesh::getBoundingBox(double *bbox) const
{
    int dim = getSpaceDimension();
    int j = 0;
...
MEDCouplingCMesh::getMeasureField(bool isAbs) const
{
    ...
    for (int i = 0; i < dim; i++) thisArr[i] = thisArr2[i]->getConstPointer();
    for (mcIdType icell = 0; icell < nbelem; icell++)
    {
        mcIdType tmp2[3];
        GetPosFromId(icell, dim, tmp, tmp2);
        area_vol[icell] = 1.;
        for (int i = 0; i < dim; i++) area_vol[icell] *= thisArr[i][tmp2[i] + 1] - thisArr[i][tmp2[i]];
    }
    delete[] thisArr;
    return field;
}
 ...
 */
MEDCouplingFieldDouble *
MEDCouplingCMesh::getMeasureFieldOnNode(bool isAbs) const
{
...
mcIdType
MEDCouplingCMesh::getCellContainingPoint(const double *pos, double eps) const
{
    int dim = getSpaceDimension();
    mcIdType ret = 0;
    mcIdType coeff = 1;
    for (int i = 0; i < dim; i++)
    {
        ...
        double ref = pos[i];
        const double *w =
            std::find_if(d, d + nbOfNodes, std::bind(std::greater_equal<double>(), std::placeholders::_1, ref));
...
void
MEDCouplingCMesh::getCellsContainingPoint(const double *pos, double eps, std::vector<mcIdType> &elts) const
{
...
void
MEDCouplingCMesh::rotate(const double *center, const double *vector, double angle)
{
 ...
 */
void
MEDCouplingCMesh::translate(const double *vector)
{
    if (_x_array)
        std::transform(
            _x_array->getConstPointer(),
            _x_array->getConstPointer() + _x_array->getNbOfElems(),
            _x_array->getPointer(),
            std::bind(std::plus<double>(), std::placeholders::_1, vector[0])
        );
    if (_y_array)
        std::transform(
            _y_array->getConstPointer(),
            _y_array->getConstPointer() + _y_array->getNbOfElems(),
            _y_array->getPointer(),
            std::bind(std::plus<double>(), std::placeholders::_1, vector[1])
        );
    if (_z_array)
        std::transform(
            _z_array->getConstPointer(),
            _z_array->getConstPointer() + _z_array->getNbOfElems(),
            _z_array->getPointer(),
            std::bind(std::plus<double>(), std::placeholders::_1, vector[2])
        );
}
 ...
 */
void
MEDCouplingCMesh::scale(const double *point, double factor)
{
    for (int i = 0; i < 3; i++)
    {
        ...
        if (c)
        {
            ...
            std::transform(
                coords, coords + lgth, coords, std::bind(std::minus<double>(), std::placeholders::_1, point[i])
            );
            std::transform(
                coords, coords + lgth, coords, std::bind(std::multiplies<double>(), std::placeholders::_1, factor)
            );
            std::transform(
                coords, coords + lgth, coords, std::bind(std::plus<double>(), std::placeholders::_1, point[i])
            );
            c->declareAsNew();
        }
    }
...
MEDCouplingMesh *
MEDCouplingCMesh::mergeMyselfWith(const MEDCouplingMesh *other) const
{
    // not implemented yet !
    return 0;
}
 ...
 */
DataArrayDouble *
MEDCouplingCMesh::getCoordinatesAndOwner() const
{
    ...
    for (mcIdType i = 0; i < nbNodes; i++)
    {
        ...
        for (int j = 0; j < spaceDim; j++) pt[i * spaceDim + j] = tabsPtr[j][tmp2[j]];
    }
    return ret.retn();
}
 ...
 */
DataArrayDouble *
MEDCouplingCMesh::computeCellCenterOfMass() const
{
...
DataArrayDouble *
MEDCouplingCMesh::computeIsoBarycenterOfNodesPerCell() const
{
...
void
MEDCouplingCMesh::getTinySerializationInformation(
...
void
MEDCouplingCMesh::resizeForUnserialization(
...
void
MEDCouplingCMesh::serialize(DataArrayIdType *&a1, DataArrayDouble *&a2) const
{
    ...
    for (int i = 0; i < 3; i++)
        if (thisArr[i])
            a2Ptr = std::copy(
                thisArr[i]->getConstPointer(), thisArr[i]->getConstPointer() + thisArr[i]->getNumberOfTuples(), a2Ptr
            );
}

void
MEDCouplingCMesh::unserialization(
...
void
MEDCouplingCMesh::reprQuickOverview(std::ostream &stream) const
{
        ...
        if (isDef[i])
        {
            ...
            else
            {
                ...
                if (nbCompo == 1)
                {
                    ...
                    if (nbTuples < 1)
                    {
                        stream2[i] << "set and allocated - WARNING number of elements < 1 !";
                        nbOfCells = -1;
                        nbOfNodes = -1;
                    }
                    else
                    {
                        ...
                        {
                            nbOfNodes *= nbTuples;
                            nbOfCells *= nbTuples - 1;
                        }
                    }
                }
                else
                {
                    stream2[i] << "set and allocated - WARNING number of components != 1 !";
                    nbOfCells = -1;
                    nbOfNodes = -1;
                }
            }
...
std::string
MEDCouplingCMesh::getVTKFileExtension() const
{
    return std::string("vtr");
}

std::string
MEDCouplingCMesh::getVTKDataSetType() const
{
