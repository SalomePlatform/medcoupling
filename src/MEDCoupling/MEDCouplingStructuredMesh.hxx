...
class MEDCouplingStructuredMesh : public MEDCouplingMesh
{
   public:
    ...
    MEDCOUPLING_EXPORT virtual MEDCouplingStructuredMesh *buildStructuredSubPart(
        const std::vector<std::pair<mcIdType, mcIdType> > &cellPart
    ) const = 0;
    MEDCOUPLING_EXPORT static std::vector<mcIdType> GetSplitVectFromStruct(const std::vector<mcIdType> &strct);
    MEDCOUPLING_EXPORT static bool IsPartStructured(
    ...
    MEDCOUPLING_EXPORT static std::vector<mcIdType> GetDimensionsFromCompactFrmt(
        const std::vector<std::pair<mcIdType, mcIdType> > &partCompactFormat
    );
    MEDCOUPLING_EXPORT static std::vector<std::pair<mcIdType, mcIdType> > GetCompactFrmtFromDimensions(
        const std::vector<mcIdType> &dims
    );
    MEDCOUPLING_EXPORT static std::vector<std::pair<mcIdType, mcIdType> > IntersectRanges(
        const std::vector<std::pair<mcIdType, mcIdType> > &r1, const std::vector<std::pair<mcIdType, mcIdType> > &r2
    );
    MEDCOUPLING_EXPORT static bool AreRangesIntersect(
        const std::vector<std::pair<mcIdType, mcIdType> > &r1, const std::vector<std::pair<mcIdType, mcIdType> > &r2
    );
    MEDCOUPLING_EXPORT static void SwitchOnIdsFrom(
        const std::vector<mcIdType> &st,
        const std::vector<std::pair<mcIdType, mcIdType> > &partCompactFormat,
        std::vector<bool> &vectToSwitchOn
    );
    MEDCOUPLING_EXPORT static void ExtractFieldOfBoolFrom(
        const std::vector<mcIdType> &st,
        const std::vector<bool> &fieldOfBool,
        const std::vector<std::pair<mcIdType, mcIdType> > &partCompactFormat,
        std::vector<bool> &fieldOut
    );
    MEDCOUPLING_EXPORT static DataArrayDouble *ExtractFieldOfDoubleFrom(
        const std::vector<mcIdType> &st,
        const DataArrayDouble *fieldOfDbl,
        const std::vector<std::pair<mcIdType, mcIdType> > &partCompactFormat
    );
    MEDCOUPLING_EXPORT static void AssignPartOfFieldOfDoubleUsing(
        const std::vector<mcIdType> &st,
        DataArrayDouble *fieldOfDbl,
        const std::vector<std::pair<mcIdType, mcIdType> > &partCompactFormat,
        const DataArrayDouble *other
    );
    MEDCOUPLING_EXPORT static void ChangeReferenceFromGlobalOfCompactFrmt(
    ...
    MEDCOUPLING_EXPORT static void PutInGhostFormat(
        mcIdType ghostSize,
        const std::vector<mcIdType> &st,
        const std::vector<std::pair<mcIdType, mcIdType> > &part,
    ...
    MEDCOUPLING_EXPORT static void ApplyGhostOnCompactFrmt(
        std::vector<std::pair<mcIdType, mcIdType> > &partBeforeFact, mcIdType ghostSize
    );
    MEDCOUPLING_EXPORT static DataArrayIdType *Build1GTNodalConnectivity(
        const mcIdType *nodeStBg, const mcIdType *nodeStEnd
    );
    ...
    MEDCOUPLING_EXPORT static mcIdType DeduceNumberOfGivenRangeInCompactFrmt(
        const std::vector<std::pair<mcIdType, mcIdType> > &partCompactFormat
    );
    MEDCOUPLING_EXPORT static mcIdType DeduceNumberOfGivenStructure(const std::vector<mcIdType> &st);
    MEDCOUPLING_EXPORT static void FindTheWidestAxisOfGivenRangeInCompactFrmt(
        const std::vector<std::pair<mcIdType, mcIdType> > &partCompactFormat, int &axisId, mcIdType &sizeOfRange
    );
    MEDCOUPLING_EXPORT static mcIdType FindMinimalPartOf(
    ...
    MEDCOUPLING_EXPORT static std::vector<std::vector<mcIdType> > ComputeSignaturePerAxisOf(
        const std::vector<mcIdType> &st, const std::vector<bool> &crit
    );

   private:
    static mcIdType GetNumberOfCellsOfSubLevelMesh(const std::vector<mcIdType> &cgs, int mdim);
    static void GetReverseNodalConnectivity1(
        const std::vector<mcIdType> &ngs, DataArrayIdType *revNodal, DataArrayIdType *revNodalIndx
    );
    static void GetReverseNodalConnectivity2(
        const std::vector<mcIdType> &ngs, DataArrayIdType *revNodal, DataArrayIdType *revNodalIndx
    );
    static void GetReverseNodalConnectivity3(
        const std::vector<mcIdType> &ngs, DataArrayIdType *revNodal, DataArrayIdType *revNodalIndx
    );
    ...
    static mcIdType FindMinimalPartOf1D(
        const std::vector<mcIdType> &st,
        const std::vector<bool> &crit,
        std::vector<std::pair<mcIdType, mcIdType> > &partCompactFormat
    );
    static mcIdType FindMinimalPartOf2D(
        const std::vector<mcIdType> &st,
        const std::vector<bool> &crit,
        std::vector<std::pair<mcIdType, mcIdType> > &partCompactFormat
    );
    static mcIdType FindMinimalPartOf3D(
        const std::vector<mcIdType> &st,
        const std::vector<bool> &crit,
        std::vector<std::pair<mcIdType, mcIdType> > &partCompactFormat
    );
