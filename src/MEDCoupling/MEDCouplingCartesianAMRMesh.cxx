...
mcIdType
MEDCouplingCartesianAMRPatchGen::getNumberOfCellsRecursiveWithOverlap() const
{
    return _mesh->getNumberOfCellsRecursiveWithOverlap();
}

mcIdType
MEDCouplingCartesianAMRPatchGen::getNumberOfCellsRecursiveWithoutOverlap() const
{
    return _mesh->getNumberOfCellsRecursiveWithoutOverlap();
}

mcIdType
MEDCouplingCartesianAMRPatchGen::getMaxNumberOfLevelsRelativeToThis() const
{
...
MEDCouplingCartesianAMRPatchGen::MEDCouplingCartesianAMRPatchGen(
    const MEDCouplingCartesianAMRPatchGen &other, MEDCouplingCartesianAMRMeshGen *father
)
    : RefCountObject(other), _mesh(other._mesh)
{
...
const MEDCouplingCartesianAMRMeshGen *
MEDCouplingCartesianAMRPatchGen::getMeshSafe() const
{
...
MEDCouplingCartesianAMRPatchGen::getMeshSafe()
{
...
MEDCouplingCartesianAMRPatchGen::getDirectChildrenWithNull() const
{
...
/*!
 * \param [in] mesh not null pointer of refined mesh replacing the cell range of \a father defined by the bottom left
 * and top right just after.
 * \param [in] bottomLeftTopRight a vector equal to the space dimension of \a mesh that specifies for each dimension,
 * the included cell start of the range for the first element of the pair, a the end cell (\b excluded) of the range for
 * the second element of the pair.
 */
MEDCouplingCartesianAMRPatch::MEDCouplingCartesianAMRPatch(
    MEDCouplingCartesianAMRMeshGen *mesh, const std::vector<std::pair<mcIdType, mcIdType> > &bottomLeftTopRight
)
    : MEDCouplingCartesianAMRPatchGen(mesh), _bl_tr(bottomLeftTopRight)
{
    std::size_t dim(bottomLeftTopRight.size()), dimExp(_mesh->getSpaceDimension());
    if (dim != dimExp)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingCartesianAMRPatch constructor : space dimension of father and input bottomLeft/topRight size "
            "mismatches !"
        );
}
...
void
MEDCouplingCartesianAMRPatch::addPatch(
    const std::vector<std::pair<mcIdType, mcIdType> > &bottomLeftTopRight, const std::vector<mcIdType> &factors
)
{
...
mcIdType
MEDCouplingCartesianAMRPatch::getNumberOfOverlapedCellsForFather() const
{
    return MEDCouplingStructuredMesh::DeduceNumberOfGivenRangeInCompactFrmt(_bl_tr);
}

/*!
 * This method states if \a other patch is in the neighborhood of \a this. The neighborhood zone is defined by \a
 * ghostLev parameter the must be >= 0. \b WARNING this method only works if \a this and \a other share the same father
 * (no check of this will be done !). Call isInMyNeighborhoodExt to deal with 2 patches not sharing directly the same
 * father.
 *
 ...
 */
bool
MEDCouplingCartesianAMRPatch::isInMyNeighborhood(const MEDCouplingCartesianAMRPatch *other, mcIdType ghostLev) const
{
    if (ghostLev < 0)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingCartesianAMRPatch::isInMyNeighborhood : the size of the neighborhood must be >= 0 !"
        );
    if (!other)
    ...
    return IsInMyNeighborhood(
        ghostLev == 0 ? 0 : 1, thisp, otherp
    );  // make hypothesis that nb this->_mesh->getFather->getFactors() is >= ghostLev
}

/*!
 * This method states if \a other patch is in the neighborhood of \a this. The neighborhood zone is defined by \a
 * ghostLev parameter the must be >= 0. This method works even if \a this and \a other does not share the same father.
 * But the level between their common ancestor must be the same. If they don't have the same ancestor an exception will
 * be thrown.
 *
 * \param [in] other - The other patch
 * \param [in] ghostLev - The size of the neighborhood zone.
 ...
 */
bool
MEDCouplingCartesianAMRPatch::isInMyNeighborhoodExt(const MEDCouplingCartesianAMRPatch *other, mcIdType ghostLev) const
{
    if (ghostLev < 0)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingCartesianAMRPatch::isInMyNeighborhoodExt : the size of the neighborhood must be >= 0 !"
        );
    if (!other)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingCartesianAMRPatch::isInMyNeighborhoodExt : the input patch is NULL !"
        );
    mcIdType lev;
 ...
 * This method states if \a other patch is in the neighborhood of \a this. The neighborhood zone is defined by \a
 * ghostLev parameter the must be >= 0. This method works even if \a this and \a other does not share the same father.
 * \a this is expected to be more refined than \a other. That is to say lev of \a this is greater than level of \a
 * other.
 *
 * \param [in] other - The other patch
 * \param [in] ghostLev - The size of the neighborhood zone.
...
MEDCouplingCartesianAMRPatch::isInMyNeighborhoodDiffLev(
    const MEDCouplingCartesianAMRPatch *other, mcIdType ghostLev
) const
{
    ...
    return IsInMyNeighborhood(
        ghostLev > 0 ? 1 : 0, thispp, otherpp
    );  // 1 not ghostLev ! It is not a bug ( I hope :) ) ! Because as \a this is a refinement of \a other ghostLev is
        // supposed to be <= factors
}

std::vector<std::pair<mcIdType, mcIdType> >
MEDCouplingCartesianAMRPatch::getBLTRRangeRelativeToGF() const
{
...
std::vector<mcIdType>
MEDCouplingCartesianAMRPatch::computeCellGridSt() const
{
    ...
    if (!father)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingCartesianAMRPatch::computeCellGridSt : no father help by underlying mesh !"
        );
...
bool
MEDCouplingCartesianAMRPatch::IsInMyNeighborhood(
    mcIdType ghostLev,
    const std::vector<std::pair<mcIdType, mcIdType> > &p1,
    const std::vector<std::pair<mcIdType, mcIdType> > &p2
)
{
    std::size_t thispsize(p1.size());
    if (thispsize != p2.size())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingCartesianAMRPatch::isInMyNeighborhood : the dimensions must be the same !"
        );
    for (std::size_t i = 0; i < thispsize; i++)
    {
        ...
        if (thispp.second < thispp.first)
            throw INTERP_KERNEL::Exception(
                "MEDCouplingCartesianAMRPatch::isInMyNeighborhood : this patch is invalid !"
            );
 ...
 */
std::vector<std::vector<std::pair<const MEDCouplingCartesianAMRPatch *, const MEDCouplingCartesianAMRPatch *> > >
MEDCouplingCartesianAMRPatch::FindNeighborsOfSubPatchesOfSameLev(
    mcIdType ghostLev, const MEDCouplingCartesianAMRPatch *p1, const MEDCouplingCartesianAMRPatch *p2
)
{
    if (!p1 || !p2)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingCartesianAMRPatch::FindNeighborsOfSubPatchesOfSameLev : the input pointers must be not NULL !"
        );
    std::vector<std::vector<std::pair<const MEDCouplingCartesianAMRPatch *, const MEDCouplingCartesianAMRPatch *> > >
        ret;
    std::vector<const MEDCouplingCartesianAMRPatch *> p1Work(p1->getMesh()->getPatches()),
        p2Work(p2->getMesh()->getPatches());
    while (!p1Work.empty())
    {
        ...
        for (std::vector<const MEDCouplingCartesianAMRPatch *>::const_iterator it1 = p1Work.begin();
             it1 != p1Work.end();
             it1++)
        {
            for (std::vector<const MEDCouplingCartesianAMRPatch *>::const_iterator it2 = p2Work.begin();
                 it2 != p2Work.end();
                 it2++)
            {
                if ((*it1)->isInMyNeighborhoodExt(
                        *it2, ghostLev > 0 ? 1 : 0
                    ))  // 1 not ghostLev ! It is not a bug ( I hope :) ) ! Because as \a this is a refinement of \a
                        // other ghostLev is supposed to be <= factors
                    retTmp.push_back(
                        std::pair<const MEDCouplingCartesianAMRPatch *, const MEDCouplingCartesianAMRPatch *>(
                            *it1, *it2
                        )
                    );
            }
...
/*!
 * This method returns all pair of patches (pa,pb) so that pb is in the neighborhood of pa (size of neighborhood is \a
 * ghostLev). pa is a refinement (a child) of \b p1 and pb is equal to \a p2. So the returned pair do not have the same
 * level as it is the case for FindNeighborsOfSubPatchesOfSameLev.
 *
 * \sa FindNeighborsOfSubPatchesOfSameLev
 */
void
MEDCouplingCartesianAMRPatch::FindNeighborsOfSubPatchesOf(
    mcIdType ghostLev,
    const MEDCouplingCartesianAMRPatch *p1,
    const MEDCouplingCartesianAMRPatch *p2,
    std::vector<std::pair<const MEDCouplingCartesianAMRPatch *, const MEDCouplingCartesianAMRPatch *> > &ret
)
{
    if (!p1 || !p2)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingCartesianAMRPatch::FindNeighborsOfSubPatchesOf : the input pointers must be not NULL !"
        );
    std::vector<const MEDCouplingCartesianAMRPatch *> p1Work(p1->getMesh()->getPatches());
    while (!p1Work.empty())
    {
        std::vector<const MEDCouplingCartesianAMRPatch *> p1Work2;
        for (std::vector<const MEDCouplingCartesianAMRPatch *>::const_iterator it0 = p1Work.begin();
             it0 != p1Work.end();
             it0++)
        {
            if ((*it0)->isInMyNeighborhoodDiffLev(p2, ghostLev))
                ret.push_back(
                    std::pair<const MEDCouplingCartesianAMRPatch *, const MEDCouplingCartesianAMRPatch *>(*it0, p2)
                );
...
/*!
 * \a p1 and \a p2 are expected to be neighbors (inside the \a ghostLev zone). This method updates \a dataOnP1 only in
 * the ghost part using a part of \a dataOnP2.
 *
 * \saUpdateNeighborsOfOneWithTwoExt
 */
void
MEDCouplingCartesianAMRPatch::UpdateNeighborsOfOneWithTwo(
    mcIdType ghostLev,
    const std::vector<mcIdType> &factors,
    const MEDCouplingCartesianAMRPatch *p1,
    const MEDCouplingCartesianAMRPatch *p2,
    DataArrayDouble *dataOnP1,
    const DataArrayDouble *dataOnP2
)
{
 ...
 */
void
MEDCouplingCartesianAMRPatch::UpdateNeighborsOfOneWithTwoExt(
    mcIdType ghostLev,
    const MEDCouplingCartesianAMRPatch *p1,
    const MEDCouplingCartesianAMRPatch *p2,
    DataArrayDouble *dataOnP1,
...
)
{
    ...
    UpdateNeighborsOfOneWithTwoInternal(
        ghostLev, p1->getMesh()->getFather()->getFactors(), p1BLTR, p2BLTR, dataOnP1, dataOnP2
    );
}

/*!
 * \a p1 is expected to be more refined than \a p2. \a p1 and \a p2 have to share a common ancestor. Compared to
 * UpdateNeighborsOfOneWithTwoExt here \a p1 and \a p2 are \b not at the same level !
 */
void
MEDCouplingCartesianAMRPatch::UpdateNeighborsOfOneWithTwoMixedLev(
    mcIdType ghostLev,
    const MEDCouplingCartesianAMRPatch *p1,
    const MEDCouplingCartesianAMRPatch *p2,
    DataArrayDouble *dataOnP1,
...
)
{
    ...
    std::transform(
        dimsP2NotRefined.begin(),
        dimsP2NotRefined.end(),
        factors.begin(),
        dimsP2Refined.begin(),
        std::multiplies<mcIdType>()
    );
    std::vector<std::pair<mcIdType, mcIdType> > p2RefinedAbs(
    ...
    std::transform(
        dimsP2Refined.begin(),
        dimsP2Refined.end(),
        dimsP2RefinedGhost.begin(),
        std::bind(std::plus<mcIdType>(), std::placeholders::_1, 2 * ghostLev)
    );
    MCAuto<DataArrayDouble> fineP2(DataArrayDouble::New());
    fineP2->alloc(
        MEDCouplingStructuredMesh::DeduceNumberOfGivenStructure(dimsP2RefinedGhost), dataOnP2->getNumberOfComponents()
    );
    ...
    if (isConservative)
    {
        ...
        std::transform(
            ...
            fineP2->end(),
            fineP2->getPointer(),
            std::bind(std::multiplies<double>(), std::placeholders::_1, 1. / ((double)fact))
        );
    }
    //
    UpdateNeighborsOfOneWithTwoInternal(
        ghostLev, p1->getMesh()->getFather()->getFactors(), p1pp, p2pp, dataOnP1, fineP2
    );
}

/*!
 * \a p1 is expected to be more refined than \a p2. \a p1 and \a p2 have to share a common ancestor. Compared to
 * UpdateNeighborsOfOneWithTwoExt here \a p1 and \a p2 are \b not at the same level ! This method has 3 outputs. 2 two
 * first are the resp the position of \a p1 and \a p2 relative to \a p1. And \a factToApplyOn2 is the coeff of
 * refinement to be applied on \a p2 to be virtually on the same level as \a p1.
 */
void
MEDCouplingCartesianAMRPatch::ComputeZonesOfTwoRelativeToOneDiffLev(
    mcIdType ghostLev,
    const MEDCouplingCartesianAMRPatch *p1,
    const MEDCouplingCartesianAMRPatch *p2,
    std::vector<std::pair<mcIdType, mcIdType> > &p1Zone,
    std::vector<std::pair<mcIdType, mcIdType> > &p2Zone,
    std::vector<mcIdType> &factToApplyOn2
)
{
    ...
    while (work && !found)
    {
        ...
        {
            levOther++;
            std::vector<const MEDCouplingCartesianAMRMeshGen *>::iterator it(
                std::find(ancestorsOfThis.begin(), ancestorsOfThis.end(), work)
            );
    ...
    if (!found)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingCartesianAMRPatch::ComputeZonesOfTwoRelativeToOneDiffLev : no common ancestor found !"
        );
    if (levThis <= levOther)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingCartesianAMRPatch::ComputeZonesOfTwoRelativeToOneDiffLev : this method is not called correctly "
            "!"
        );
    //
    const MEDCouplingCartesianAMRMeshGen *comAncestor(ancestorsOfThis[levThis]);
    mcIdType idThis(comAncestor->getPatchIdFromChildMesh(ancestorsOfThis[levThis - 1])),
        idOther(comAncestor->getPatchIdFromChildMesh(work2));
    ...
    p1Zone = thisp->getBLTRRange();
    p2Zone = MEDCouplingStructuredMesh::TranslateCompactFrmt(otherp->getBLTRRange(), offset);
    factToApplyOn2.resize(p1Zone.size());
    std::fill(factToApplyOn2.begin(), factToApplyOn2.end(), 1);
    //
...
std::size_t
MEDCouplingCartesianAMRPatch::getHeapMemorySizeWithoutChildren() const
{
...
const MEDCouplingCartesianAMRMeshGen *
MEDCouplingCartesianAMRPatch::FindCommonAncestor(
...
)
{
    ...
    if (f1 != f2)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingCartesianAMRPatch::FindCommonAncestor : no common ancestor between p1 and p2 !"
        );
    return f1;
}

std::vector<mcIdType>
MEDCouplingCartesianAMRPatch::ComputeOffsetFromTwoToOne(
    const MEDCouplingCartesianAMRMeshGen *comAncestor,
    mcIdType lev,
    const MEDCouplingCartesianAMRPatch *p1,
    const MEDCouplingCartesianAMRPatch *p2
)
{
    if (lev <= 0)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingCartesianAMRPatch::ComputeOffsetFromTwoToOne : this method is useful only for lev > 0 !"
        );
    mcIdType zeLev(lev - 1);
    mcIdType dim(p1->getMesh()->getSpaceDimension());
    if (p2->getMesh()->getSpaceDimension() != dim)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingCartesianAMRPatch::ComputeOffsetFromTwoToOne : dimension must be the same !"
        );
    ...
    for (mcIdType i = 0; i < zeLev; i++)
    {
        ...
        for (mcIdType j = 0; j < lev - i; j++)
        {
            ...
            p1h = f1tmp->getPatch(pid1);
            p2h = f2tmp->getPatch(pid2);
            f1 = f1tmp;
            f2 = f2tmp;
        }
...
void
MEDCouplingCartesianAMRPatch::UpdateNeighborsOfOneWithTwoInternal(
    mcIdType ghostLev,
    const std::vector<mcIdType> &factors,
    const std::vector<std::pair<mcIdType, mcIdType> > &p1,
    const std::vector<std::pair<mcIdType, mcIdType> > &p2,
...
{  // p1=[(1,4),(2,4)] p2=[(4,5),(3,4)]
    ...
    std::transform(
        dimsCoarse.begin(), dimsCoarse.end(), factors.begin(), dimsCoarse.begin(), std::multiplies<mcIdType>()
    );  //[12,8]
    std::transform(
        dimsCoarse.begin(),
        dimsCoarse.end(),
    ...
    std::vector<std::pair<mcIdType, mcIdType> > rangeCoarse(
        MEDCouplingStructuredMesh::GetCompactFrmtFromDimensions(dimsCoarse)
    );  //[(0,14),(0,10)]
    ...
    std::vector<std::pair<mcIdType, mcIdType> > interstRange(
        MEDCouplingStructuredMesh::IntersectRanges(tmp0, rangeCoarse)
    );  // interstRange=[(13,14),(5,9)]
    ...
    //
    MCAuto<DataArrayDouble> ghostVals(
        MEDCouplingStructuredMesh::ExtractFieldOfDoubleFrom(
            MEDCouplingStructuredMesh::GetDimensionsFromCompactFrmt(dimsFine), dataOnP2, tmp2
        )
    );
...
MEDCouplingCartesianAMRPatch::MEDCouplingCartesianAMRPatch(
    const MEDCouplingCartesianAMRPatch &other, MEDCouplingCartesianAMRMeshGen *father
)
    : MEDCouplingCartesianAMRPatchGen(other, father), _bl_tr(other._bl_tr)
{
}
 ...
 */
void
MEDCouplingCartesianAMRPatch::ApplyFactorsOnCompactFrmt(
    std::vector<std::pair<mcIdType, mcIdType> > &partBeforeFact, const std::vector<mcIdType> &factors
)
{
    std::size_t sz(factors.size());
    if (sz != partBeforeFact.size())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingCartesianAMRPatch::ApplyFactorsOnCompactFrmt : size of input vectors must be the same !"
        );
...
/*!
 * This method is different than ApplyGhostOnCompactFrmt. The \a partBeforeFact parameter is enlarger contrary to
 ...
 */
void
MEDCouplingCartesianAMRPatch::ApplyAllGhostOnCompactFrmt(
    std::vector<std::pair<mcIdType, mcIdType> > &partBeforeFact, mcIdType ghostSize
)
{
    if (ghostSize < 0)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingCartesianAMRPatch::ApplyAllGhostOnCompactFrmt : ghost size must be >= 0 !"
        );
...
MEDCouplingCartesianAMRPatchGF::MEDCouplingCartesianAMRPatchGF(MEDCouplingCartesianAMRMesh *mesh)
    : MEDCouplingCartesianAMRPatchGen(mesh)
{
}
...
std::size_t
MEDCouplingCartesianAMRPatchGF::getHeapMemorySizeWithoutChildren() const
{
    return sizeof(MEDCouplingCartesianAMRPatchGF);
}

MEDCouplingCartesianAMRPatchGF::MEDCouplingCartesianAMRPatchGF(
    const MEDCouplingCartesianAMRPatchGF &other, MEDCouplingCartesianAMRMeshGen *father
)
    : MEDCouplingCartesianAMRPatchGen(other, father)
{
}
...
int
MEDCouplingCartesianAMRMeshGen::getSpaceDimension() const
{
    return _mesh->getSpaceDimension();
}

void
MEDCouplingCartesianAMRMeshGen::setFactors(const std::vector<mcIdType> &newFactors)
{
    if (getSpaceDimension() != ToIdType(newFactors.size()))
        throw INTERP_KERNEL::Exception(
            "MEDCouplingCartesianAMRMeshGen::setFactors : size of input factors is not equal to the space dimension !"
        );
    if (_factors.empty())
    {
        _factors = newFactors;
    ...
    if (!_patches.empty())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingCartesianAMRMeshGen::setFactors : modification of factors is not allowed when presence of "
            "patches !"
        );
    _factors = newFactors;
    declareAsNew();
...
mcIdType
MEDCouplingCartesianAMRMeshGen::getMaxNumberOfLevelsRelativeToThis() const
{
    mcIdType ret(1);
    for (std::vector<MCAuto<MEDCouplingCartesianAMRPatch> >::const_iterator it = _patches.begin(); it != _patches.end();
         it++)
...
/*!
 * This method returns the number of cells of \a this with the help of the MEDCouplingIMesh instance representing \a
 * this. The patches in \a this are ignored here.
 * \sa getNumberOfCellsAtCurrentLevelGhost, getNumberOfCellsRecursiveWithOverlap
 */
mcIdType
MEDCouplingCartesianAMRMeshGen::getNumberOfCellsAtCurrentLevel() const
{
...
/*!
 * This method returns the number of cells of \a this with the help of the MEDCouplingIMesh instance representing \a
 * this enlarged by \a ghostLev size to take into account of the ghost cells for future computation. The patches in \a
 * this are ignored here.
 *
...
/*!
 * This method returns the number of cells including the current level but \b also \b including recursively all cells of
 * other levels starting from this. The set of cells which size is returned here are generally overlapping each other.
 */
mcIdType
MEDCouplingCartesianAMRMeshGen::getNumberOfCellsRecursiveWithOverlap() const
{
 ...
 */
mcIdType
MEDCouplingCartesianAMRMeshGen::getNumberOfCellsRecursiveWithoutOverlap() const
{
    mcIdType ret = _mesh->getNumberOfCells();
    for (std::vector<MCAuto<MEDCouplingCartesianAMRPatch> >::const_iterator it = _patches.begin(); it != _patches.end();
         it++)
...
/*!
 * This method returns a vector of size equal to getAbsoluteLevelRelativeTo. It allows to find position an absolute
 * position of \a this relative to \a ref (that is typically the god father).
 *
 * \sa getPatchAtPosition
 */
std::vector<mcIdType>
MEDCouplingCartesianAMRMeshGen::getPositionRelativeTo(const MEDCouplingCartesianAMRMeshGen *ref) const
{
    if (!ref)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingCartesianAMRMeshGen::getPositionRelativeTo : input pointer is NULL !"
        );
 ...
 */
const MEDCouplingCartesianAMRPatch *
MEDCouplingCartesianAMRMeshGen::getPatchAtPosition(const std::vector<mcIdType> &pos) const
{
    std::size_t sz(pos.size());
    if (sz == 0)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingCartesianAMRMeshGen::getPatchAtPosition : empty input -> no patch by definition !"
        );
    ...
    if (!elt || !elt->getMesh())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingCartesianAMRMeshGen::getPatchAtPosition : NULL element found during walk !"
        );
 ...
 */
std::vector<MEDCouplingCartesianAMRPatchGen *>
MEDCouplingCartesianAMRMeshGen::retrieveGridsAt(mcIdType absoluteLev) const
{
    if (absoluteLev < 0)
 ...
 */
void
MEDCouplingCartesianAMRMeshGen::addPatch(
    const std::vector<std::pair<mcIdType, mcIdType> > &bottomLeftTopRight, const std::vector<mcIdType> &factors
)
{
...
class InternalPatch : public RefCountObjectOnly
{
   public:
    ...
    std::vector<std::vector<mcIdType> > computeSignature() const
    {
        return MEDCouplingStructuredMesh::ComputeSignaturePerAxisOf(computeCGS(), getConstCriterion());
    }
    double getEfficiencyPerAxis(mcIdType axisId) const
    {
        return (double)_nb_of_true / ((double)(_part[axisId].second - _part[axisId].first));
    }
...
void
InternalPatch::zipToFitOnCriterion(mcIdType minPatchLgth)
{
...
void
InternalPatch::updateNumberOfTrue() const
{
    _nb_of_true = ToIdType(std::count(_crit.begin(), _crit.end(), true));
}

MCAuto<InternalPatch>
InternalPatch::extractPart(const std::vector<std::pair<mcIdType, mcIdType> > &partInGlobal) const
{
...
void
DissectBigPatch(
    const INTERP_KERNEL::BoxSplittingOptions &bso,
    const InternalPatch *patchToBeSplit,
    mcIdType axisId,
    mcIdType largestLength,
    mcIdType &cutPlace
)
{
    ...
    for (mcIdType i = minimumPatchLength - 1; i < largestLength - minimumPatchLength; i++)
    {
        ...
        ratio[i] =
            std::max(efficiencyPerAxis[0], efficiencyPerAxis[1]) / std::min(efficiencyPerAxis[0], efficiencyPerAxis[1]);
...
bool
FindInflection(
    const INTERP_KERNEL::BoxSplittingOptions &bso, const InternalPatch *patchToBeSplit, mcIdType &cutPlace, int &axisId
)
{
    bool cutFound(false);
    cutPlace = -1;  // do not set axisId before to be sure that cutFound was set to true
    ...
    for (mcIdType id = 0; id < dim; id++)
    {
        ...
        if (zero_cross.size() > 0)
        {
            ...
            mcIdType pos_distance_min =
                ToIdType(find(distance.begin(), distance.end(), distance_min) - distance.begin());
    ...
    if (zeroCrossDims[0] != -1 || zeroCrossDims[1] != -1)
    {
        ...
        if (zeroCrossVals[0] == max_cross_dims && zeroCrossVals[1] == max_cross_dims)
        {
            ...
            max_cross_dims =
                ToIdType(std::find(zeroCrossVals.begin(), zeroCrossVals.end(), max_cross_dims) - zeroCrossVals.begin());
        cutFound = true;
...
)
{
    ...
    {
        if (patchToBeSplit->getNumberOfCells() > bso.getMaximumNbOfCellsInPatch() ||
            rangeOfAxisId > bso.getMaximumPatchLength())
        {
...
MCAuto<InternalPatch>
DealWithNoCut(const InternalPatch *patch)
{
...
DealWithCut(
    ...
    int axisId,
    mcIdType cutPlace,
    std::vector<MCAuto<InternalPatch> > &listOfPatches
)
{
    ...
    leftPart->zipToFitOnCriterion(ToIdType(minPatchLgth));
    rightPart->zipToFitOnCriterion(ToIdType(minPatchLgth));
...
void
MEDCouplingCartesianAMRMeshGen::removeAllPatches()
{
    _patches.clear();
    declareAsNew();
}
...
void
MEDCouplingCartesianAMRMeshGen::removePatch(mcIdType patchId)
{
...
mcIdType
MEDCouplingCartesianAMRMeshGen::getNumberOfPatches() const
{
 ...
 *
 * This generic algorithm can be degenerated into three child ones, depending on the arguments given; in particular
 * depending on whether they are equal to 0 or not. 1/ If  minimumPatchLength = maximumPatchLength = maximumPatchVolume
 * = 0, then we have the Berger-Rigoutsos algorithm. This algorithm was developed in 1991 and seems appropriate for
 * sequential programming. 2/ If maximumPatchLength = 0, then we have the Livne algorithm. This algorithm was developed
 * in 2004 and is an improvement of the Berger-Rigoutsos algorithm. 3/ If maximumPatchVolume = 0, the we have the
 * lmin-lmax algorithm. This algorithm was developed by Arthur TALPAERT in 2014 and is an improvement of the Livne
 * algorithm. It is especially appropriate for parallel computing, where one patch would be given to one CPU. See Arthur
 * TALPAERT's 2014 CANUM poster for more information.
 *
 */
void
MEDCouplingCartesianAMRMeshGen::createPatchesFromCriterion(
    const INTERP_KERNEL::BoxSplittingOptions &bso,
...
)
{
    ...
    if (nbCells != ToIdType(criterion.size()))
        throw INTERP_KERNEL::Exception(
            "MEDCouplingCartesianAMRMeshGen::createPatchesFromCriterion : the number of tuples of criterion array must "
            "be equal to the number of cells at the current level !"
        );
    _patches.clear();
    ...
    p->setNumberOfTrue(
        MEDCouplingStructuredMesh::FindMinimalPartOf(
            bso.getMinimumPatchLength(), cgs, criterion, p->getCriterion(), p->getPart()
        )
    );
    if (p->presenceOfTrue())
    ...
    while (!listOfPatches.empty())
    {
        ...
        for (std::vector<MCAuto<InternalPatch> >::iterator it = listOfPatches.begin(); it != listOfPatches.end(); it++)
        {
            ...
            MEDCouplingStructuredMesh::FindTheWidestAxisOfGivenRangeInCompactFrmt(
                (*it)->getConstPart(), axisId, largestLength
            );
            if ((*it)->getEfficiency() >= bso.getEfficiencyThreshold() &&
                ((*it)->getNumberOfCells() > bso.getMaximumNbOfCellsInPatch() ||
                 largestLength > bso.getMaximumPatchLength()))
            {
    ...
    for (std::vector<MCAuto<InternalPatch> >::const_iterator it = listOfPatchesOK.begin(); it != listOfPatchesOK.end();
         it++)
...
/*!
 * This method creates patches in \a this (by destroying the patches if any). This method uses \a criterion array as a
 * field on cells on this level. This method only create patches at level 0 relative to \a this.
 */
void
MEDCouplingCartesianAMRMeshGen::createPatchesFromCriterion(
    const INTERP_KERNEL::BoxSplittingOptions &bso, const DataArrayByte *criterion, const std::vector<mcIdType> &factors
)
{
    if (!criterion || !criterion->isAllocated())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingCartesianAMRMeshGen::createPatchesFromCriterion : the criterion DataArrayByte instance must be "
...
)
{
    if (!criterion)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingCartesianAMRMeshGen::createPatchesFromCriterion : null criterion pointer !"
        );
...
mcIdType
MEDCouplingCartesianAMRMeshGen::getPatchIdFromChildMesh(const MEDCouplingCartesianAMRMeshGen *mesh) const
{
    mcIdType ret(0);
    for (std::vector<MCAuto<MEDCouplingCartesianAMRPatch> >::const_iterator it = _patches.begin(); it != _patches.end();
         it++, ret++)
    ...
    throw INTERP_KERNEL::Exception(
        "MEDCouplingCartesianAMRMeshGen::getPatchIdFromChildMesh : no such a mesh in my direct progeny !"
    );
}

std::vector<const MEDCouplingCartesianAMRPatch *>
MEDCouplingCartesianAMRMeshGen::getPatches() const
{
    std::size_t sz(_patches.size());
    std::vector<const MEDCouplingCartesianAMRPatch *> ret(sz);
    for (std::size_t i = 0; i < sz; i++) ret[i] = _patches[i];
    return ret;
}

const MEDCouplingCartesianAMRPatch *
MEDCouplingCartesianAMRMeshGen::getPatch(mcIdType patchId) const
{
    checkPatchId(patchId);
 ...
 */
bool
MEDCouplingCartesianAMRMeshGen::isPatchInNeighborhoodOf(mcIdType patchId1, mcIdType patchId2, mcIdType ghostLev) const
{
...
/*!
 * This method creates a new cell field array on given \a patchId patch in \a this starting from a coarse cell field on
 * \a this \a cellFieldOnThis. This method can be seen as a fast projection from the cell field \a cellFieldOnThis on \c
 * this->getImageMesh() to a refined part of \a this defined by the patch with id \a patchId.
 *
 * \param [in] patchId - The id of the patch \a cellFieldOnThis has to be put on.
 * \param [in] cellFieldOnThis - The array of the cell field on \c this->getImageMesh() to be projected to patch having
 * id \a patchId.
 ...
 */
DataArrayDouble *
MEDCouplingCartesianAMRMeshGen::createCellFieldOnPatch(mcIdType patchId, const DataArrayDouble *cellFieldOnThis) const
{
    if (!cellFieldOnThis || !cellFieldOnThis->isAllocated())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingCartesianAMRMesh::createCellFieldOnPatch : the input cell field array is NULL or not allocated "
            "!"
        );
    const MEDCouplingCartesianAMRPatch *patch(getPatch(patchId));
    const MEDCouplingIMesh *fine(patch->getMesh()->getImageMesh());
    MCAuto<DataArrayDouble> ret(DataArrayDouble::New());
    ret->alloc(fine->getNumberOfCells(), cellFieldOnThis->getNumberOfComponents());
    ret->copyStringInfoFrom(*cellFieldOnThis);
    MEDCouplingIMesh::SpreadCoarseToFine(
        cellFieldOnThis, _mesh->getCellGridStructure(), ret, patch->getBLTRRange(), getFactors()
    );
    return ret.retn();
}

/*!
 * This method is equivalent to MEDCouplingCartesianAMRMesh::createCellFieldOnPatch except that here instead of creating
 * a new instance it fills the value into the \a cellFieldOnPatch data.
 *
 * \param [in] patchId - The id of the patch \a cellFieldOnThis has to be put on.
 * \param [in] cellFieldOnThis - The array of the cell field on \c this->getImageMesh() to be projected to patch having
 * id \a patchId.
 ...
 */
void
MEDCouplingCartesianAMRMeshGen::fillCellFieldOnPatch(
    mcIdType patchId, const DataArrayDouble *cellFieldOnThis, DataArrayDouble *cellFieldOnPatch, bool isConservative
) const
{
    if (!cellFieldOnThis || !cellFieldOnThis->isAllocated())
        throw INTERP_KERNEL::Exception(
    ...
    const MEDCouplingCartesianAMRPatch *patch(getPatch(patchId));
    MEDCouplingIMesh::SpreadCoarseToFine(
        cellFieldOnThis, _mesh->getCellGridStructure(), cellFieldOnPatch, patch->getBLTRRange(), getFactors()
    );
    if (isConservative)
    {
        mcIdType fact(MEDCouplingStructuredMesh::DeduceNumberOfGivenStructure(getFactors()));
        std::transform(
            cellFieldOnPatch->begin(),
            cellFieldOnPatch->end(),
            cellFieldOnPatch->getPointer(),
            std::bind(std::multiplies<double>(), std::placeholders::_1, 1. / ((double)fact))
        );
    }
}
...
/*!
 * This method is the generalization of fillCellFieldOnPatch method. This method only projects coarse to fine without
 * considering the potential neighbor patches covered by the ghost cells of patch with id \a patchId.
 *
 * \param [in] patchId - The id of the patch \a cellFieldOnThis has to be put on.
 * \param [in] cellFieldOnThis - The array of the cell field on \c this->getImageMesh() to be projected to patch having
 * id \a patchId.
 ...
 */
void
MEDCouplingCartesianAMRMeshGen::fillCellFieldOnPatchGhost(
    mcIdType patchId,
...
) const
{
    if (!cellFieldOnThis || !cellFieldOnThis->isAllocated())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingCartesianAMRMesh::createCellFieldOnPatchGhost : the input cell field array is NULL or not "
            "allocated !"
        );
    const MEDCouplingCartesianAMRPatch *patch(getPatch(patchId));
    MEDCouplingIMesh::SpreadCoarseToFineGhost(
        cellFieldOnThis, _mesh->getCellGridStructure(), cellFieldOnPatch, patch->getBLTRRange(), getFactors(), ghostLev
    );
    if (isConservative)
    {
        mcIdType fact(MEDCouplingStructuredMesh::DeduceNumberOfGivenStructure(getFactors()));
...
/*!
 * This method is equivalent to  fillCellFieldOnPatchGhost except that here \b ONLY \b the \b ghost \b zone will be
 * updated in \a cellFieldOnPatch.
 *
 * \param [in] patchId - The id of the patch \a cellFieldOnThis has to be put on.
 * \param [in] cellFieldOnThis - The array of the cell field on \c this->getImageMesh() to be projected to patch having
 * id \a patchId.
 * \param [in,out] cellFieldOnPatch - The array of the cell field on the requested patch to be filled \b only \b in \b
 * the \b ghost \b zone.
 * \param [in] ghostLev - The size of the ghost zone (must be >=0 !)
 */
void
MEDCouplingCartesianAMRMeshGen::fillCellFieldOnPatchOnlyOnGhostZone(
    mcIdType patchId, const DataArrayDouble *cellFieldOnThis, DataArrayDouble *cellFieldOnPatch, mcIdType ghostLev
) const
{
    if (!cellFieldOnThis || !cellFieldOnThis->isAllocated())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingCartesianAMRMesh::fillCellFieldOnPatchOnlyOnGhostZone : the input cell field array is NULL or "
            "not allocated !"
        );
    const MEDCouplingCartesianAMRPatch *patch(getPatch(patchId));
 ...
 * Then for all other patches than those pointed by \a patchId that overlap the ghost zone of the patch impact the ghost
 * zone adequately.
 *
 * \param [in] patchId - The id of the patch \a cellFieldOnThis has to be put on.
 * \param [in] cellFieldOnThis - The array of the cell field on \c this->getImageMesh() to be projected to patch having
 ...
 */
void
MEDCouplingCartesianAMRMeshGen::fillCellFieldOnPatchGhostAdv(
    mcIdType patchId,
    const DataArrayDouble *cellFieldOnThis,
    mcIdType ghostLev,
...
) const
{
    ...
    if (nbp != ToIdType(arrsOnPatches.size()))
    {
        ...
        oss << "MEDCouplingCartesianAMRMesh::fillCellFieldOnPatchGhostAdv : there are " << nbp
            << " patches in this and " << arrsOnPatches.size() << " arrays in the last parameter !";
...
/*!
 * This method updates the patch with id \a patchId considering the only the all the patches in \a this to fill ghost
 * zone. So \b warning, the DataArrayDouble instance \a arrsOnPatches[patchId] is non const.
 *
 * \sa getPatchIdsInTheNeighborhoodOf
 */
void
MEDCouplingCartesianAMRMeshGen::fillCellFieldOnPatchOnlyGhostAdv(
    mcIdType patchId, mcIdType ghostLev, const std::vector<const DataArrayDouble *> &arrsOnPatches
) const
{
    mcIdType nbp(getNumberOfPatches());
    if (nbp != ToIdType(arrsOnPatches.size()))
    {
        std::ostringstream oss;
        oss << "MEDCouplingCartesianAMRMesh::fillCellFieldOnPatchOnlyGhostAdv : there are " << nbp
            << " patches in this and " << arrsOnPatches.size() << " arrays in the last parameter !";
        throw INTERP_KERNEL::Exception(oss.str().c_str());
    }
    ...
    for (std::vector<mcIdType>::const_iterator it = ids.begin(); it != ids.end(); it++)
    {
        ...
        MEDCouplingCartesianAMRPatch::UpdateNeighborsOfOneWithTwo(
            ghostLev, _factors, refP, otherP, theFieldToFill, arrsOnPatches[*it]
        );
    }
}

void
MEDCouplingCartesianAMRMeshGen::fillCellFieldOnPatchOnlyOnGhostZoneWith(
    mcIdType ghostLev,
...
{
    MEDCouplingCartesianAMRPatch::UpdateNeighborsOfOneWithTwo(
        ghostLev, _factors, patchToBeModified, neighborPatch, cellFieldOnPatch, cellFieldNeighbor
    );
}

/*!
 * This method updates \a cellFieldOnThis part of values coming from the cell field \a cellFieldOnPatch lying on patch
 * having id \a patchId.
 *
 * \param [in] patchId - The id of the patch \a cellFieldOnThis has to be put on.
 * \param [in] cellFieldOnPatch - The array of the cell field on patch with id \a patchId.
 * \param [in,out] cellFieldOnThis The array of the cell field on \a this to be updated only on the part concerning the
 * patch with id \a patchId.
 ...
 */
void
MEDCouplingCartesianAMRMeshGen::fillCellFieldComingFromPatch(
    mcIdType patchId, const DataArrayDouble *cellFieldOnPatch, DataArrayDouble *cellFieldOnThis, bool isConservative
) const
{
    if (!cellFieldOnPatch || !cellFieldOnPatch->isAllocated())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingCartesianAMRMesh::fillCellFieldComingFromPatch : the input cell field array is NULL or not "
            "allocated !"
        );
    const MEDCouplingCartesianAMRPatch *patch(getPatch(patchId));
    MEDCouplingIMesh::CondenseFineToCoarse(
        _mesh->getCellGridStructure(), cellFieldOnPatch, patch->getBLTRRange(), getFactors(), cellFieldOnThis
    );
    if (!isConservative)
    {
        mcIdType fact(MEDCouplingStructuredMesh::DeduceNumberOfGivenStructure(getFactors()));
        MEDCouplingStructuredMesh::MultiplyPartOf(
            _mesh->getCellGridStructure(), patch->getBLTRRange(), 1. / ((double)fact), cellFieldOnThis
        );
    }
}

/*!
 * This method is the extension of MEDCouplingCartesianAMRMesh::fillCellFieldComingFromPatch managing the ghost cells.
 * If this method is called with \a ghostLev equal to 0 it behaves exactly as
 * MEDCouplingCartesianAMRMesh::fillCellFieldComingFromPatch.
 *
 * \param [in] patchId - The id of the patch \a cellFieldOnThis has to be put on.
 * \param [in] cellFieldOnPatch - The array of the cell field on patch with id \a patchId.
 * \param [in,out] cellFieldOnThis The array of the cell field on \a this to be updated only on the part concerning the
 ...
 */
void
MEDCouplingCartesianAMRMeshGen::fillCellFieldComingFromPatchGhost(
    mcIdType patchId,
...
) const
{
    if (!cellFieldOnPatch || !cellFieldOnPatch->isAllocated())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingCartesianAMRMesh::fillCellFieldComingFromPatchGhost : the input cell field array is NULL or "
            "not allocated !"
        );
    const MEDCouplingCartesianAMRPatch *patch(getPatch(patchId));
    MEDCouplingIMesh::CondenseFineToCoarseGhost(
        _mesh->getCellGridStructure(), cellFieldOnPatch, patch->getBLTRRange(), getFactors(), cellFieldOnThis, ghostLev
    );
    if (!isConservative)
    {
        mcIdType fact(MEDCouplingStructuredMesh::DeduceNumberOfGivenStructure(getFactors()));
        MEDCouplingStructuredMesh::MultiplyPartOfByGhost(
            _mesh->getCellGridStructure(), patch->getBLTRRange(), ghostLev, 1. / ((double)fact), cellFieldOnThis
        );
    }
}

/*!
 * This method finds all patches (located by their ids) that are in the neighborhood of patch with id \a patchId. The
 * neighborhood size is defined by ghostLev.
 *
 * \param [in] patchId - the id of the considered patch.
 * \param [in] ghostLev - the size of the neighborhood.
 * \return DataArrayIdType * - the newly allocated array containing the list of patches in the neighborhood of the
 * considered patch. This array is to be deallocated by the caller.
 */
DataArrayIdType *
MEDCouplingCartesianAMRMeshGen::findPatchesInTheNeighborhoodOf(mcIdType patchId, mcIdType ghostLev) const
{
    mcIdType nbp(getNumberOfPatches());
...
MEDCouplingUMesh *
MEDCouplingCartesianAMRMeshGen::buildUnstructured() const
{
    ...
    for (std::size_t i = 0; i < msSafe.size(); i++) ms[i] = msSafe[i];
 ...
 *
 * \return MEDCoupling1SGTUMesh * - A new object to be managed by the caller containing as cells as there are patches in
 * \a this.
 */
MEDCoupling1SGTUMesh *
MEDCouplingCartesianAMRMeshGen::buildMeshFromPatchEnvelop() const
{
         ...
         it++)
    {
        ...
        if (patch)
        {
            ...
            cellsSafe.push_back(cell1SGT);
            cells.push_back(cell1SGT);
        }
    }
...
MEDCoupling1SGTUMesh *
MEDCouplingCartesianAMRMeshGen::buildMeshOfDirectChildrenOnly() const
{
         ...
         it++)
    {
        ...
        if (patch)
        {
            ...
            patchesSafe.push_back(patchMesh);
            patches.push_back(patchMesh);
        }
    }
...
MEDCouplingCartesianAMRMeshGen::buildCellFieldOnRecurseWithoutOverlapWithoutGhost(
    mcIdType ghostSz, const std::vector<const DataArrayDouble *> &recurseArrs
) const
{
    if (recurseArrs.empty())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingCartesianAMRMeshGen::buildCellFieldOnRecurseWithoutOverlapWithoutGhost : array is empty ! "
            "Should never happen !"
        );
    //
    std::vector<bool> bs(_mesh->getNumberOfCells(), false);
 ...
 */
DataArrayDouble *
MEDCouplingCartesianAMRMeshGen::extractGhostFrom(mcIdType ghostSz, const DataArrayDouble *arr) const
{
    ...
    std::transform(
        st.begin(), st.end(), st.begin(), std::bind(std::plus<mcIdType>(), std::placeholders::_1, 2 * ghostSz)
    );
...
/*!
 * This method returns all the patches in \a this not equal to \a patchId that are in neighborhood of patch with id \a
 * patchId.
 *
 * \sa fillCellFieldOnPatchOnlyGhostAdv
 */
std::vector<mcIdType>
MEDCouplingCartesianAMRMeshGen::getPatchIdsInTheNeighborhoodOf(mcIdType patchId, mcIdType ghostLev) const
{
    std::vector<mcIdType> ret;
    mcIdType nbp(getNumberOfPatches());
    //
...
/*!
 * This method returns a dump python of \a this. It is useful for users of createPatchesFromCriterion method for
 * debugging.
 *
 * \sa dumpPatchesOf, createPatchesFromCriterion, createPatchesFromCriterionML
 */
std::string
MEDCouplingCartesianAMRMeshGen::buildPythonDumpOfThis() const
{
    std::ostringstream oss;
...
MEDCouplingCartesianAMRMeshGen::MEDCouplingCartesianAMRMeshGen(const MEDCouplingCartesianAMRMeshGen &other)
    : RefCountObject(other), _mesh(other._mesh), _patches(other._patches), _factors(other._factors)
{
...
MEDCouplingCartesianAMRMeshGen::MEDCouplingCartesianAMRMeshGen(
...
{
    _mesh = MEDCouplingIMesh::New(
        meshName, spaceDim, nodeStrctStart, nodeStrctStop, originStart, originStop, dxyzStart, dxyzStop
    );
}
...
MEDCouplingCartesianAMRMeshGen::MEDCouplingCartesianAMRMeshGen(MEDCouplingIMesh *mesh)
{
    if (!mesh)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingCartesianAMRMeshGen(MEDCouplingIMesh *mesh) constructor : The input mesh is null !"
        );
...
void
MEDCouplingCartesianAMRMeshGen::checkPatchId(mcIdType patchId) const
{
    mcIdType sz(getNumberOfPatches());
    if (patchId < 0 || patchId >= sz)
    {
        std::ostringstream oss;
        oss << "MEDCouplingCartesianAMRMeshGen::checkPatchId : invalid patchId (" << patchId << ") ! Must be in [0,"
            << sz << ") !";
        throw INTERP_KERNEL::Exception(oss.str().c_str());
    }
}
...
void
MEDCouplingCartesianAMRMeshGen::checkFactorsAndIfNotSetAssign(const std::vector<mcIdType> &factors)
{
    if (getSpaceDimension() != ToIdType(factors.size()))
        throw INTERP_KERNEL::Exception(
            "MEDCouplingCartesianAMRMeshGen::checkFactorsAndIfNotSetAssign : invalid size of factors ! size must be "
            "equal to the spaceDimension !"
        );
    if (_factors.empty())
    {
        _factors = factors;
...
void
MEDCouplingCartesianAMRMeshGen::retrieveGridsAtInternal(
    mcIdType lev, std::vector<MCAuto<MEDCouplingCartesianAMRPatchGen> > &grids
) const
{
    if (lev == 0)
    {
        const MEDCouplingCartesianAMRMesh *thisc(dynamic_cast<const MEDCouplingCartesianAMRMesh *>(this));  // tony
        MCAuto<MEDCouplingCartesianAMRPatchGF> elt(
            new MEDCouplingCartesianAMRPatchGF(const_cast<MEDCouplingCartesianAMRMesh *>(thisc))
        );
        grids.push_back(DynamicCastSafe<MEDCouplingCartesianAMRPatchGF, MEDCouplingCartesianAMRPatchGen>(elt));
    }
    else if (lev == 1)
    {
        for (std::vector<MCAuto<MEDCouplingCartesianAMRPatch> >::const_iterator it = _patches.begin();
             it != _patches.end();
             it++)
        {
...
mcIdType
MEDCouplingCartesianAMRMeshGen::GetGhostLevelInFineRef(mcIdType ghostLev, const std::vector<mcIdType> &factors)
{
    if (ghostLev < 0)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingCartesianAMRMesh::GetGhostLevelInFineRef : the ghost size must be >=0 !"
        );
    if (factors.empty())
 ...
 */
std::vector<const DataArrayDouble *>
MEDCouplingCartesianAMRMeshGen::extractSubTreeFromGlobalFlatten(
    const MEDCouplingCartesianAMRMeshGen *head, const std::vector<const DataArrayDouble *> &all
) const
{
    ...
    for (mcIdType i = 0; i < maxLev; i++)
    {
        ...
        for (std::vector<const MEDCouplingCartesianAMRMeshGen *>::const_iterator it = meshes.begin();
             it != meshes.end();
             it++)
        {
            ...
            for (std::vector<const MEDCouplingCartesianAMRPatch *>::const_iterator it0 = ps.begin(); it0 != ps.end();
                 it0++)
            {
    ...
    if (kk != all.size())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingCartesianAMRMeshGen::extractSubTreeFromGlobalFlatten : the size of input vector is not "
            "compatible with number of leaves in this !"
        );
    return ret;
}

void
MEDCouplingCartesianAMRMeshGen::dumpPatchesOf(const std::string &varName, std::ostream &oss) const
{
    std::size_t j(0);
    for (std::vector<MCAuto<MEDCouplingCartesianAMRPatch> >::const_iterator it = _patches.begin(); it != _patches.end();
         it++)
...
std::size_t
MEDCouplingCartesianAMRMeshGen::getHeapMemorySizeWithoutChildren() const
{
    return sizeof(MEDCouplingCartesianAMRMeshGen);
}

std::vector<const BigMemoryObject *>
MEDCouplingCartesianAMRMeshGen::getDirectChildrenWithNull() const
{
    std::vector<const BigMemoryObject *> ret;
...
void
MEDCouplingCartesianAMRMeshGen::updateTime() const
{
...
MEDCouplingCartesianAMRMeshSub::MEDCouplingCartesianAMRMeshSub(
    MEDCouplingCartesianAMRMeshGen *father, MEDCouplingIMesh *mesh
)
    : MEDCouplingCartesianAMRMeshGen(mesh), _father(father)
{
    if (!_father)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingCartesianAMRMeshSub(MEDCouplingCartesianAMRMeshGen *father, MEDCouplingIMesh *mesh) "
            "constructor : empty father !"
        );
}

const MEDCouplingCartesianAMRMeshGen *
MEDCouplingCartesianAMRMeshSub::getFather() const
{
    return _father;
}

const MEDCouplingCartesianAMRMeshGen *
MEDCouplingCartesianAMRMeshSub::getGodFather() const
{
    if (!_father)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingCartesianAMRMeshSub::getGodFather : Impossible to find a god father because there is a hole in "
            "chain !"
        );
 ...
 */
mcIdType
MEDCouplingCartesianAMRMeshSub::getAbsoluteLevel() const
{
    if (!_father)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingCartesianAMRMeshSub::getAbsoluteLevel : Impossible to find a god father because there is a "
            "hole in chain !"
        );
...
void
MEDCouplingCartesianAMRMeshSub::detachFromFather()
{
    _father = 0;
    declareAsNew();
}
...
std::vector<std::pair<mcIdType, mcIdType> >
MEDCouplingCartesianAMRMeshSub::positionRelativeToGodFather(std::vector<mcIdType> &st) const
{
    ...
    for (std::size_t i = 0; i < dim; i++) start[i] = ret[i].first;
    ...
    while (!dynamic_cast<const MEDCouplingCartesianAMRMesh *>(it))
    {
        ...
        for (std::size_t i = 0; i < dim; i++) tmp[i] = start2[i].first;
        //
...
mcIdType
MEDCouplingCartesianAMRMeshSub::getAbsoluteLevelRelativeTo(const MEDCouplingCartesianAMRMeshGen *ref) const
{
    if (this == ref)
        return 0;
    if (_father == 0)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingCartesianAMRMeshSub::getAbsoluteLevelRelativeTo : ref is not in the progeny of this !"
        );
    else
...
MEDCouplingCartesianAMRMeshSub::MEDCouplingCartesianAMRMeshSub(
    const MEDCouplingCartesianAMRMeshSub &other, MEDCouplingCartesianAMRMeshGen *father
)
    : MEDCouplingCartesianAMRMeshGen(other), _father(father)
{
}

MEDCouplingCartesianAMRMeshSub *
MEDCouplingCartesianAMRMeshSub::deepCopy(MEDCouplingCartesianAMRMeshGen *fath) const
{
 ...
 */
void
MEDCouplingCartesianAMRMeshSub::getPositionRelativeToInternal(
    const MEDCouplingCartesianAMRMeshGen *ref, std::vector<mcIdType> &ret
) const
{
    if (this == ref)
        return;
    if (!_father)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingCartesianAMRMeshSub::getPositionRelativeToInternal : ref is not in the progeny of this !"
        );
...
MEDCouplingCartesianAMRMesh::New(
    const std::string &meshName,
    int spaceDim,
    const mcIdType *nodeStrctStart,
    const mcIdType *nodeStrctStop,
...
{
    return new MEDCouplingCartesianAMRMesh(
        meshName, spaceDim, nodeStrctStart, nodeStrctStop, originStart, originStop, dxyzStart, dxyzStop
    );
}

MEDCouplingCartesianAMRMesh *
MEDCouplingCartesianAMRMesh::New(MEDCouplingIMesh *mesh)
{
    return new MEDCouplingCartesianAMRMesh(mesh);
}

const MEDCouplingCartesianAMRMeshGen *
MEDCouplingCartesianAMRMesh::getFather() const
{
...
const MEDCouplingCartesianAMRMeshGen *
MEDCouplingCartesianAMRMesh::getGodFather() const
{
    return this;
}

mcIdType
MEDCouplingCartesianAMRMesh::getAbsoluteLevel() const
{
    return 0;
}

void
MEDCouplingCartesianAMRMesh::detachFromFather()
{  // not a bug - do nothing
}

std::vector<std::pair<mcIdType, mcIdType> >
MEDCouplingCartesianAMRMesh::positionRelativeToGodFather(std::vector<mcIdType> &st) const
{
...
mcIdType
MEDCouplingCartesianAMRMesh::getAbsoluteLevelRelativeTo(const MEDCouplingCartesianAMRMeshGen *ref) const
{
    if (this == ref)
        return 0;
    throw INTERP_KERNEL::Exception(
        "MEDCouplingCartesianAMRMesh::getAbsoluteLevelRelativeTo : ref is not in the progeny of this !"
    );
}

std::vector<MEDCouplingCartesianAMRPatchGen *>
MEDCouplingCartesianAMRMesh::retrieveGridsAt(mcIdType absoluteLev) const
{
...
MEDCouplingCartesianAMRMesh::deepCopy(MEDCouplingCartesianAMRMeshGen *father) const
{
    if (father)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingCartesianAMRMesh::deepCopy : specifying a not null father for a God Father object !"
        );
 ...
 * This method calls as many times as size of \a bso createPatchesFromCriterion. Size of \a bso and size of \a factors
 * must be the same !
 ...
 */
void
MEDCouplingCartesianAMRMesh::createPatchesFromCriterionML(
    const std::vector<const INTERP_KERNEL::BoxSplittingOptions *> &bso,
    const DataArrayDouble *criterion,
...
)
{
    ...
    if (nbOfLevs != factors.size())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingCartesianAMRMesh::createPatchesFromCriterionML : size of vectors must be the same !"
        );
    if (nbOfLevs == 0)
        return;
    if (!bso[0])
        throw INTERP_KERNEL::Exception(
            "MEDCouplingCartesianAMRMesh::createPatchesFromCriterionML : pointers in 1st arg must be not NULL !"
        );
    ...
    for (std::size_t i = 1; i < nbOfLevs; i++)
    {
        if (!bso[i])
            throw INTERP_KERNEL::Exception(
                "MEDCouplingCartesianAMRMesh::createPatchesFromCriterionML : presence of a NULL BoxSplittingOptions in "
                "input vector !"
            );
        //
        ...
        for (std::size_t ii = 0; ii < sz; ii++) elts2[ii] = elts[ii];
        //
        static const char TMP_STR[] = "TMP";
        std::vector<std::pair<std::string, int> > fieldNames(1);
        fieldNames[0].first = TMP_STR;
        fieldNames[0].second = 1;
        ...
        for (std::size_t ii = 0; ii < sz; ii++)
        {
            const DataArrayDouble *critOnLeaf(
                att->getFieldOn(const_cast<MEDCouplingCartesianAMRMeshGen *>(elts[ii]->getMesh()), TMP_STR)
            );
            elts3[ii] = const_cast<DataArrayDouble *>(critOnLeaf);
            elts3[ii]->incrRef();
        }
        att = 0;
        for (std::size_t ii = 0; ii < sz; ii++)
            const_cast<MEDCouplingCartesianAMRMeshGen *>(elts[ii]->getMesh())
                ->createPatchesFromCriterion(*bso[i], elts3[ii], factors[i], eps);
    }
}

void
MEDCouplingCartesianAMRMesh::getPositionRelativeToInternal(
    const MEDCouplingCartesianAMRMeshGen *ref, std::vector<mcIdType> &ret
) const
{
}
...
MEDCouplingCartesianAMRMesh::MEDCouplingCartesianAMRMesh(const MEDCouplingCartesianAMRMesh &other)
    : MEDCouplingCartesianAMRMeshGen(other)
{
}
...
std::vector<const BigMemoryObject *>
MEDCouplingCartesianAMRMesh::getDirectChildrenWithNull() const
{
