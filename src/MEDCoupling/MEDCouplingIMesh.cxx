...
MEDCouplingIMesh::MEDCouplingIMesh(const MEDCouplingIMesh &other, bool deepCpy)
    : MEDCouplingStructuredMesh(other, deepCpy), _space_dim(other._space_dim), _axis_unit(other._axis_unit)
{
    _origin[0] = other._origin[0];
    _origin[1] = other._origin[1];
    _origin[2] = other._origin[2];
    _dxyz[0] = other._dxyz[0];
    _dxyz[1] = other._dxyz[1];
    _dxyz[2] = other._dxyz[2];
    _structure[0] = other._structure[0];
    _structure[1] = other._structure[1];
    _structure[2] = other._structure[2];
}

MEDCouplingIMesh::~MEDCouplingIMesh() {}

MEDCouplingIMesh *
MEDCouplingIMesh::New()
{
...
MEDCouplingIMesh::getDirectAccessOfCoordsArrIfInStructure() const
{
    throw INTERP_KERNEL::Exception(
        "MEDCouplingIMesh::getDirectAccessOfCoordsArrIfInStructure : MEDCouplingIMesh does not aggregate array of "
        "coordinates !"
    );
}
...
MEDCouplingIMesh::buildWithGhost(mcIdType ghostLev) const
{
    ...
    MCAuto<MEDCouplingIMesh> ret(
        MEDCouplingIMesh::New(
            getName(), spaceDim, structure, structure + spaceDim, origin, origin + spaceDim, dxyz, dxyz + spaceDim
        )
    );
...
void
MEDCouplingIMesh::setNodeStruct(const mcIdType *nodeStrctStart, const mcIdType *nodeStrctStop)
{
    checkSpaceDimension();
    mcIdType sz((mcIdType)std::distance(nodeStrctStart, nodeStrctStop));
    if (sz != _space_dim)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingIMesh::setNodeStruct : input vector of node structure has not the right size ! Or change space "
            "dimension before calling it !"
        );
...
std::vector<mcIdType>
MEDCouplingIMesh::getNodeStruct() const
{
    checkSpaceDimension();
...
void
MEDCouplingIMesh::setOrigin(const double *originStart, const double *originStop)
{
    checkSpaceDimension();
    mcIdType sz(ToIdType(std::distance(originStart, originStop)));
    if (sz != _space_dim)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingIMesh::setOrigin : input vector of origin vector has not the right size ! Or change space "
            "dimension before calling it !"
        );
...
std::vector<double>
MEDCouplingIMesh::getOrigin() const
{
    checkSpaceDimension();
...
void
MEDCouplingIMesh::setDXYZ(const double *dxyzStart, const double *dxyzStop)
{
    checkSpaceDimension();
    mcIdType sz(ToIdType(std::distance(dxyzStart, dxyzStop)));
    if (sz != _space_dim)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingIMesh::setDXYZ : input vector of dxyz vector has not the right size ! Or change space "
            "dimension before calling it !"
        );
...
std::vector<double>
MEDCouplingIMesh::getDXYZ() const
{
    checkSpaceDimension();
...
void
MEDCouplingIMesh::setAxisUnit(const std::string &unitName)
{
    _axis_unit = unitName;
    declareAsNew();
}
...
std::string
MEDCouplingIMesh::getAxisUnit() const
{
    return _axis_unit;
}
 ...
 */
double
MEDCouplingIMesh::getMeasureOfAnyCell() const
{
    checkConsistencyLight();
    int dim(getSpaceDimension());
    double ret(1.);
    for (int i = 0; i < dim; i++) ret *= fabs(_dxyz[i]);
    return ret;
}
 ...
 */
MEDCouplingCMesh *
MEDCouplingIMesh::convertToCartesian() const
{
    checkConsistencyLight();
    ...
    for (int i = 0; i < spaceDim; i++)
    {
        ...
        arr->alloc(_structure[i], 1);
        arr->setInfoOnComponent(0, infos[i]);
        arr->iota();
        arr->applyLin(_dxyz[i], _origin[i]);
...
/*!
 * This method refines \a this uniformaly along all of its dimensions. In case of success the space covered by \a this
 * will remain the same before the invocation except that the number of cells will be multiplied by \a factor ^
 * this->getMeshDimension(). The origin of \a this will be not touched only spacing and node structure will be changed.
 * This method can be useful for AMR users.
 */
void
MEDCouplingIMesh::refineWithFactor(const std::vector<mcIdType> &factors)
{
    if (ToIdType(factors.size()) != _space_dim)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingIMesh::refineWithFactor : refinement factors must have size equal to spaceDim !"
        );
    checkConsistencyLight();
    ...
    for (int i = 0; i < _space_dim; i++)
    {
        if (factors[i] <= 0)
        {
            ...
            oss << "MEDCouplingIMesh::refineWithFactor : factor for axis #" << i << " (" << factors[i]
...
/*!
 * This method returns a newly created mesh containing a single cell in it. This returned cell covers exactly the space
 * covered by \a this.
 *
 * \return MEDCouplingIMesh * - A newly created object (to be managed by the caller with decrRef) containing simply one
 * cell.
 *
...
/*!
 * This static method is useful to condense field on cells of a MEDCouplingIMesh instance coming from a refinement (
 * MEDCouplingIMesh::refineWithFactor for example) to a coarse MEDCouplingIMesh instance. So this method can be seen as
 * a specialization in P0P0 conservative interpolation non overlapping from fine image mesh to a coarse image mesh. Only
 * tuples ( deduced from \a fineLocInCoarse ) of \a coarseDA will be modified. Other tuples of \a coarseDA will be let
 * unchanged.
 *
...
)
{
    if (coarseSt.size() != fineLocInCoarse.size() || coarseSt.size() != facts.size())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingIMesh::CondenseFineToCoarse : All input vectors (dimension) must have the same size !"
        );
    if (!coarseDA || !coarseDA->isAllocated() || !fineDA || !fineDA->isAllocated())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingIMesh::CondenseFineToCoarse : the parameters 1 or 3 are NULL or not allocated !"
        );
    std::size_t meshDim(coarseSt.size());
    mcIdType nbOfTuplesInCoarseExp(MEDCouplingStructuredMesh::DeduceNumberOfGivenStructure(coarseSt)),
        nbOfTuplesInFineExp(MEDCouplingStructuredMesh::DeduceNumberOfGivenRangeInCompactFrmt(fineLocInCoarse));
    std::size_t nbCompo = fineDA->getNumberOfComponents();
    if (coarseDA->getNumberOfComponents() != nbCompo)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingIMesh::CondenseFineToCoarse : the number of components of fine DA and coarse one mismatches !"
        );
    if (meshDim != fineLocInCoarse.size() || meshDim != facts.size())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingIMesh::CondenseFineToCoarse : the size of fineLocInCoarse (4th param) and facts (5th param) "
            "must be equal to the sier of coarseSt (2nd param) !"
        );
    if (coarseDA->getNumberOfTuples() != nbOfTuplesInCoarseExp)
    {
        std::ostringstream oss;
        oss << "MEDCouplingIMesh::CondenseFineToCoarse : Expecting " << nbOfTuplesInCoarseExp << " tuples having "
            << coarseDA->getNumberOfTuples() << " !";
        throw INTERP_KERNEL::Exception(oss.str().c_str());
    }
    ...
    if (nbOfTuplesInFineExp == 0)
    {
        if (nbTuplesFine == 0)
            ...
            throw INTERP_KERNEL::Exception(
                "MEDCouplingIMesh::CondenseFineToCoarse : Nothing to condense considering the range specified ! But "
                "DataArray is not empty !"
            );
    }
    if (nbTuplesFine % nbOfTuplesInFineExp != 0)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingIMesh::CondenseFineToCoarse : Invalid nb of tuples in fine DataArray regarding its structure !"
        );
    mcIdType fact(std::accumulate(facts.begin(), facts.end(), 1, std::multiplies<mcIdType>()));
    if (nbTuplesFine != fact * nbOfTuplesInFineExp)
    {
        std::ostringstream oss;
        oss << "MEDCouplingIMesh::CondenseFineToCoarse : Invalid number of tuples (" << nbTuplesFine
            << ") of fine dataarray is invalid ! Must be " << fact * nbOfTuplesInFineExp << "!";
        throw INTERP_KERNEL::Exception(oss.str().c_str());
    }
    ...
    switch (meshDim)
    {
        ...
        case 2:
        {
            mcIdType kk(fineLocInCoarse[0].first + coarseSt[0] * fineLocInCoarse[1].first), fact1(facts[1]),
                fact0(facts[0]);
        ...
        {
            mcIdType kk(
                fineLocInCoarse[0].first + coarseSt[0] * fineLocInCoarse[1].first +
                coarseSt[0] * coarseSt[1] * fineLocInCoarse[2].first
            ),
                fact2(facts[2]), fact1(facts[1]), fact0(facts[0]);
        ...
        default:
            throw INTERP_KERNEL::Exception(
                "MEDCouplingIMesh::CondenseFineToCoarse : only dimensions 1, 2 and 3 supported !"
            );
    }
}
 ...
 * \param [in,out] coarseDA The DataArrayDouble corresponding to the a cell field of a coarse mesh whose cell structure
 * is defined by \a coarseSt.
 * \param [in] ghostSize - The size of the ghost zone. The ghost zone is expected to be the same for all axis and both
 * for coarse and fine meshes.
 *
...
)
{
    ...
    if (coarseSt.size() != fineLocInCoarse.size() || coarseSt.size() != facts.size())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingIMesh::CondenseFineToCoarseGhost : All input vectors (dimension) must have the same size !"
        );
    if (!coarseDA || !coarseDA->isAllocated() || !fineDA || !fineDA->isAllocated())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingIMesh::CondenseFineToCoarseGhost : the parameters 1 or 3 are NULL or not allocated !"
        );
    std::vector<mcIdType> coarseStG(coarseSt.size());
    std::transform(
        coarseSt.begin(),
        coarseSt.end(),
        coarseStG.begin(),
    ...
    if (coarseDA->getNumberOfComponents() != nbCompo)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingIMesh::CondenseFineToCoarseGhost : the number of components of fine DA and coarse one "
            "mismatches !"
        );
    if (meshDim != fineLocInCoarse.size() || meshDim != facts.size())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingIMesh::CondenseFineToCoarseGhost : the size of fineLocInCoarse (4th param) and facts (5th "
            "param) must be equal to the sier of coarseSt (2nd param) !"
        );
    if (coarseDA->getNumberOfTuples() != nbOfTuplesInCoarseExp)
    {
        std::ostringstream oss;
        oss << "MEDCouplingIMesh::CondenseFineToCoarseGhost : Expecting " << nbOfTuplesInCoarseExp
            << " tuples in coarse DataArray having " << coarseDA->getNumberOfTuples() << " !";
        throw INTERP_KERNEL::Exception(oss.str().c_str());
    }
    //
    ...
    mcIdType nbTuplesFine(fineDA->getNumberOfTuples()),
        nbTuplesFineExp(MEDCouplingStructuredMesh::DeduceNumberOfGivenStructure(fineStG));
    if (fineDA->getNumberOfTuples() != nbTuplesFineExp)
    {
        std::ostringstream oss;
        oss << "MEDCouplingIMesh::CondenseFineToCoarseGhost : Expecting " << nbTuplesFineExp
            << " tuples in fine DataArray having " << nbTuplesFine << " !";
        throw INTERP_KERNEL::Exception(oss.str().c_str());
    }
    //
    double *outPtr(coarseDA->getPointer());
    ...
    switch (meshDim)
    {
        ...
        case 2:
        {
            ...
            mcIdType kk(fineLocInCoarse[0].first + ghostSize + nxwg * (fineLocInCoarse[1].first + ghostSize)),
                fact1(facts[1]), fact0(facts[0]);
        ...
        {
            mcIdType nxwg(coarseSt[0] + 2 * ghostSize),
                nxywg((coarseSt[0] + 2 * ghostSize) * (coarseSt[1] + 2 * ghostSize));
            mcIdType kk(
                fineLocInCoarse[0].first + ghostSize + nxwg * (fineLocInCoarse[1].first + ghostSize) +
                nxywg * (fineLocInCoarse[2].first + ghostSize)
            ),
                fact2(facts[2]), fact1(facts[1]), fact0(facts[0]);
        ...
        default:
            throw INTERP_KERNEL::Exception(
                "MEDCouplingIMesh::CondenseFineToCoarseGhost : only dimensions 1, 2, 3 supported !"
            );
    }
}
 ...
 *
 * \param [in] coarseDA The DataArrayDouble corresponding to the a cell field of a coarse mesh whose cell structure is
 * defined by \a coarseSt.
 * \param [in] coarseSt The cell structure of coarse mesh.
...
MEDCouplingIMesh::SpreadCoarseToFine(
    const DataArrayDouble *coarseDA,
    const std::vector<mcIdType> &coarseSt,
...
)
{
    if (coarseSt.size() != fineLocInCoarse.size() || coarseSt.size() != facts.size())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingIMesh::SpreadCoarseToFine : All input vectors (dimension) must have the same size !"
        );
    if (!coarseDA || !coarseDA->isAllocated() || !fineDA || !fineDA->isAllocated())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingIMesh::SpreadCoarseToFine : the parameters 1 or 3 are NULL or not allocated !"
        );
    std::size_t meshDim(coarseSt.size());
    mcIdType nbOfTuplesInCoarseExp(MEDCouplingStructuredMesh::DeduceNumberOfGivenStructure(coarseSt)),
        nbOfTuplesInFineExp(MEDCouplingStructuredMesh::DeduceNumberOfGivenRangeInCompactFrmt(fineLocInCoarse));
    ...
    if (coarseDA->getNumberOfComponents() != nbCompo)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingIMesh::SpreadCoarseToFine : the number of components of fine DA and coarse one mismatches !"
        );
    if (meshDim != fineLocInCoarse.size() || meshDim != facts.size())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingIMesh::SpreadCoarseToFine : the size of fineLocInCoarse (4th param) and facts (5th param) must "
            "be equal to the sier of coarseSt (2nd param) !"
        );
    if (coarseDA->getNumberOfTuples() != nbOfTuplesInCoarseExp)
    {
        std::ostringstream oss;
        oss << "MEDCouplingIMesh::SpreadCoarseToFine : Expecting " << nbOfTuplesInCoarseExp << " tuples having "
            << coarseDA->getNumberOfTuples() << " !";
        throw INTERP_KERNEL::Exception(oss.str().c_str());
    }
    mcIdType nbTuplesFine(fineDA->getNumberOfTuples());
    if (nbTuplesFine % nbOfTuplesInFineExp != 0)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingIMesh::SpreadCoarseToFine : Invalid nb of tuples in fine DataArray regarding its structure !"
        );
    mcIdType fact(std::accumulate(facts.begin(), facts.end(), 1, std::multiplies<mcIdType>()));
    if (nbTuplesFine != fact * nbOfTuplesInFineExp)
    {
        std::ostringstream oss;
        oss << "MEDCouplingIMesh::SpreadCoarseToFine : Invalid number of tuples (" << nbTuplesFine
            << ") of fine dataarray is invalid ! Must be " << fact * nbOfTuplesInFineExp << "!";
        throw INTERP_KERNEL::Exception(oss.str().c_str());
    }
    // to improve use jump-iterator. Factorizes with SwitchOnIdsFrom BuildExplicitIdsFrom
    ...
    switch (meshDim)
    {
        case 1:
        {
            ...
            for (int i = 0; i < dims[0]; i++)
            {
                ...
                for (mcIdType ifact = 0; ifact < fact0; ifact++) outPtr = std::copy(loc, loc + nbCompo, outPtr);
            }
            break;
        }
        case 2:
        {
            mcIdType kk(fineLocInCoarse[0].first + coarseSt[0] * fineLocInCoarse[1].first), fact0(facts[0]),
                fact1(facts[1]);
            for (int j = 0; j < dims[1]; j++)
            {
                for (mcIdType jfact = 0; jfact < fact1; jfact++)
        ...
        case 3:
        {
            mcIdType kk(
                ...
                fact0(facts[0]), fact1(facts[2]), fact2(facts[2]);
            for (int k = 0; k < dims[2]; k++)
            {
                for (mcIdType kfact = 0; kfact < fact2; kfact++)
                {
        ...
        default:
            throw INTERP_KERNEL::Exception(
                "MEDCouplingIMesh::SpreadCoarseToFine : only dimensions 1, 2 and 3 supported !"
            );
    }
}
...
)
{
    ...
    if (coarseSt.size() != fineLocInCoarse.size() || coarseSt.size() != facts.size())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingIMesh::SpreadCoarseToFineGhost : All input vectors (dimension) must have the same size !"
        );
    if (!coarseDA || !coarseDA->isAllocated() || !fineDA || !fineDA->isAllocated())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingIMesh::SpreadCoarseToFineGhost : the parameters 1 or 3 are NULL or not allocated !"
        );
    std::vector<mcIdType> coarseStG(coarseSt.size());
    std::transform(
        coarseSt.begin(),
    ...
    if (coarseDA->getNumberOfComponents() != nbCompo)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingIMesh::SpreadCoarseToFineGhost : the number of components of fine DA and coarse one mismatches "
            "!"
        );
    if (meshDim != fineLocInCoarse.size() || meshDim != facts.size())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingIMesh::SpreadCoarseToFineGhost : the size of fineLocInCoarse (4th param) and facts (5th param) "
            "must be equal to the sier of coarseSt (2nd param) !"
        );
    if (coarseDA->getNumberOfTuples() != nbOfTuplesInCoarseExp)
    {
        std::ostringstream oss;
        oss << "MEDCouplingIMesh::SpreadCoarseToFineGhost : Expecting " << nbOfTuplesInCoarseExp << " tuples having "
            << coarseDA->getNumberOfTuples() << " !";
        throw INTERP_KERNEL::Exception(oss.str().c_str());
    }
    //
    ...
    {
        std::ostringstream oss;
        oss << "MEDCouplingIMesh::SpreadCoarseToFineGhost : Expecting " << nbTuplesFineExp
            << " tuples in fine DataArray having " << nbTuplesFine << " !";
        throw INTERP_KERNEL::Exception(oss.str().c_str());
    }
    //
    ...
    switch (meshDim)
    {
        ...
        {
            std::vector<mcIdType> dims(MEDCouplingStructuredMesh::GetDimensionsFromCompactFrmt(fineLocInCoarse));
            mcIdType offset(fineLocInCoarse[0].first + ghostSize - 1),
                fact0(facts[0]);  // offset is always >=0 thanks to the fact that ghostSize>=1 !
        ...
        case 3:
        {
            ...
            mcIdType nxyWgCoarse((coarseSt[0] + 2 * ghostSize) * (coarseSt[1] + 2 * ghostSize)),
                nxyWgFine((dims[0] * fact0 + 2 * ghostSize) * (dims[1] * fact1 + 2 * ghostSize));
            mcIdType offset(
                (fineLocInCoarse[2].first + ghostSize - 1) * nxyWgCoarse
            );  // offset is always >=0 thanks to the fact that ghostSize>=1 !
            for (mcIdType i = 0; i < ghostSize; i++, outPtr += nxyWgFine * nbCompo)
                SpreadCoarseToFineGhost2D(
                    inPtr + offset * nbCompo, outPtr, nbCompo, coarseSt, fineLocInCoarse, facts, ghostSize
                );
            offset += nxyWgCoarse;
        ...
        default:
            throw INTERP_KERNEL::Exception(
                "MEDCouplingIMesh::SpreadCoarseToFineGhost : only dimensions 1, 2, 3 supported !"
            );
    }
}

/*!
 * This method spreads the values of coarse data \a coarseDA into \a fineDA \b ONLY \b in \b the \b ghost \b zone
 * (contrary to SpreadCoarseToFineGhost that spread the values everywhere).
 *
 * \param [in] coarseDA The DataArrayDouble corresponding to the a cell field of a coarse mesh whose cell structure is
 * defined by \a coarseSt.
 * \param [in] coarseSt The cell structure of coarse mesh.
...
)
{
    ...
    if (coarseSt.size() != fineLocInCoarse.size() || coarseSt.size() != facts.size())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingIMesh::SpreadCoarseToFineGhostZone : All input vectors (dimension) must have the same size !"
        );
    if (!coarseDA || !coarseDA->isAllocated() || !fineDA || !fineDA->isAllocated())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingIMesh::SpreadCoarseToFineGhostZone : the parameters 1 or 3 are NULL or not allocated !"
        );
    std::vector<mcIdType> coarseStG(coarseSt.size());
    std::transform(
        coarseSt.begin(),
    ...
    if (coarseDA->getNumberOfComponents() != nbCompo)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingIMesh::SpreadCoarseToFineGhostZone : the number of components of fine DA and coarse one "
            "mismatches !"
        );
    if (meshDim != fineLocInCoarse.size() || meshDim != facts.size())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingIMesh::SpreadCoarseToFineGhostZone : the size of fineLocInCoarse (4th param) and facts (5th "
            "param) must be equal to the sier of coarseSt (2nd param) !"
        );
    if (coarseDA->getNumberOfTuples() != nbOfTuplesInCoarseExp)
    {
        std::ostringstream oss;
        oss << "MEDCouplingIMesh::SpreadCoarseToFineGhostZone : Expecting " << nbOfTuplesInCoarseExp
            << " tuples having " << coarseDA->getNumberOfTuples() << " !";
        throw INTERP_KERNEL::Exception(oss.str().c_str());
    }
    //
    std::vector<mcIdType> fineStG(MEDCouplingStructuredMesh::GetDimensionsFromCompactFrmt(fineLocInCoarse));
    ...
    {
        std::ostringstream oss;
        oss << "MEDCouplingIMesh::SpreadCoarseToFineGhostZone : Expecting " << nbTuplesFineExp
            << " tuples in fine DataArray having " << nbTuplesFine << " !";
        throw INTERP_KERNEL::Exception(oss.str().c_str());
    }
    //
    ...
    switch (meshDim)
    {
        ...
        case 2:
        {
            SpreadCoarseToFineGhostZone2D(
                inPtr, outPtr, ToIdType(nbCompo), coarseSt, fineLocInCoarse, facts, ghostSize
            );
            break;
        }
        case 3:
        {
            ...
            for (mcIdType i = 0; i < ghostSize; i++, outPtr += nxyWgFine * nbCompo)
                SpreadCoarseToFineGhost2D(
                    inPtr + offset * nbCompo, outPtr, ToIdType(nbCompo), coarseSt, fineLocInCoarse, facts, ghostSize
                );
            offset += nxyWgCoarse;
            for (int i = 0; i < dims[2]; i++, offset += nxyWgCoarse)
                for (mcIdType j = 0; j < fact2; j++, outPtr += nxyWgFine * nbCompo)
        ...
        default:
            throw INTERP_KERNEL::Exception(
                "MEDCouplingIMesh::SpreadCoarseToFineGhostZone : only dimensions 1, 2, 3 supported !"
            );
    }
}

void
MEDCouplingIMesh::setSpaceDimension(int spaceDim)
{
...
void
MEDCouplingIMesh::updateTime() const
{
}

std::size_t
MEDCouplingIMesh::getHeapMemorySizeWithoutChildren() const
{
    return MEDCouplingStructuredMesh::getHeapMemorySizeWithoutChildren();
}

std::vector<const BigMemoryObject *>
MEDCouplingIMesh::getDirectChildrenWithNull() const
{
 ...
 */
void
MEDCouplingIMesh::copyTinyStringsFrom(const MEDCouplingMesh *other)
{
...
bool
MEDCouplingIMesh::isEqualIfNotWhy(const MEDCouplingMesh *other, double prec, std::string &reason) const
{
    if (!other)
...
bool
MEDCouplingIMesh::isEqualWithoutConsideringStr(const MEDCouplingMesh *other, double prec) const
{
    const MEDCouplingIMesh *otherC = dynamic_cast<const MEDCouplingIMesh *>(other);
    if (!otherC)
        return false;
...
bool
MEDCouplingIMesh::isEqualWithoutConsideringStrInternal(
    const MEDCouplingMesh *other, double prec, std::string &reason
) const
{
    const MEDCouplingIMesh *otherC = dynamic_cast<const MEDCouplingIMesh *>(other);
    if (!otherC)
    ...
    {
        std::ostringstream oss;
        oss << "The spaceDimension of this (" << _space_dim << ") is not equal to those of other ("
            << otherC->_space_dim << ") !";
        return false;
    }
    checkSpaceDimension();
...
void
MEDCouplingIMesh::checkDeepEquivalWith(
 ...
 * The user intend that the nodes are the same, so by construction of MEDCoupling::MEDCouplingIMesh, \a this and \a
 * other are the same !
 */
void
MEDCouplingIMesh::checkDeepEquivalOnSameNodesWith(
...
void
MEDCouplingIMesh::checkConsistencyLight() const
{
    checkSpaceDimension();
    for (int i = 0; i < _space_dim; i++)
        if (_structure[i] < 1)
        {
            std::ostringstream oss;
            oss << "MEDCouplingIMesh::checkConsistencyLight : On axis " << i << "/" << _space_dim
                << ", number of nodes is equal to " << _structure[i] << " ! must be >=1 !";
            throw INTERP_KERNEL::Exception(oss.str().c_str());
        }
}

void
MEDCouplingIMesh::checkConsistency(double eps) const
{
    checkConsistencyLight();
}

void
MEDCouplingIMesh::getNodeGridStructure(mcIdType *res) const
{
    checkSpaceDimension();
...
std::vector<mcIdType>
MEDCouplingIMesh::getNodeGridStructure() const
{
    checkSpaceDimension();
...
MEDCouplingIMesh::buildStructuredSubPart(const std::vector<std::pair<mcIdType, mcIdType> > &cellPart) const
{
    checkConsistencyLight();
    int dim(getSpaceDimension());
    ...
    {
        std::ostringstream oss;
        oss << "MEDCouplingIMesh::buildStructuredSubPart : the space dimension is " << dim << " and cell part size is "
            << cellPart.size() << " !";
        throw INTERP_KERNEL::Exception(oss.str().c_str());
    }
    ...
    for (int i = 0; i < dim; i++)
    {
        ...
        {
            std::ostringstream oss;
            oss << "MEDCouplingIMesh::buildStructuredSubPart : At dimension #" << i << " the start node id is "
                << startNode << " it should be in [0," << _structure[i] << ") !";
            throw INTERP_KERNEL::Exception(oss.str().c_str());
        }
        if (myDelta < 0 || myDelta > _structure[i])
        {
            std::ostringstream oss;
            oss << "MEDCouplingIMesh::buildStructuredSubPart : Along dimension #" << i << " the number of nodes is "
                << _structure[i] << ", and you are requesting for " << myDelta << " nodes wide range !" << std::endl;
            throw INTERP_KERNEL::Exception(oss.str().c_str());
        }
 ...
 */
int
MEDCouplingIMesh::getSpaceDimension() const
{
    return _space_dim;
}

void
MEDCouplingIMesh::getCoordinatesOfNode(mcIdType nodeId, std::vector<double> &coo) const
{
    mcIdType tmp[3];
    int spaceDim(getSpaceDimension());
    ...
    for (int j = 0; j < spaceDim; j++) coo.push_back(_origin[j] + _dxyz[j] * FromIdType<double>(tmp2[j]));
}

std::string
MEDCouplingIMesh::simpleRepr() const
{
    ...
    if (spaceDim < 0 || spaceDim > 3)
        return ret.str();
    ret << "The nodal structure is : ";
    std::copy(_structure, _structure + spaceDim, std::ostream_iterator<int>(ret, " "));
    ret << "\n";
    ret << "The origin position is [" << _axis_unit << "]: ";
    std::copy(_origin, _origin + spaceDim, std::ostream_iterator<double>(ret, " "));
    ret << "\n";
    ret << "The intervals along axis are : ";
    std::copy(_dxyz, _dxyz + spaceDim, std::ostream_iterator<double>(ret, " "));
    ret << "\n";
    return ret.str();
}

std::string
MEDCouplingIMesh::advancedRepr() const
{
    return simpleRepr();
}

void
MEDCouplingIMesh::getBoundingBox(double *bbox) const
{
    checkConsistencyLight();
    int dim(getSpaceDimension());
    for (int idim = 0; idim < dim; idim++)
    {
        ...
        {
            std::ostringstream oss;
            oss << "MEDCouplingIMesh::getBoundingBox : on axis #" << idim << " number of nodes in structure is < 0 !";
            throw INTERP_KERNEL::Exception(oss.str().c_str());
        }
...
MEDCouplingIMesh::getMeasureField(bool isAbs) const
{
    checkConsistencyLight();
 ...
 */
MEDCouplingFieldDouble *
MEDCouplingIMesh::getMeasureFieldOnNode(bool isAbs) const
{
...
mcIdType
MEDCouplingIMesh::getCellContainingPoint(const double *pos, double eps) const
{
...
void
MEDCouplingIMesh::getCellsContainingPoint(const double *pos, double eps, std::vector<mcIdType> &elts) const
{
...
void
MEDCouplingIMesh::rotate(const double *center, const double *vector, double angle)
{
 ...
 */
void
MEDCouplingIMesh::translate(const double *vector)
{
    checkSpaceDimension();
    int dim(getSpaceDimension());
 ...
 */
void
MEDCouplingIMesh::scale(const double *point, double factor)
{
    checkSpaceDimension();
    int dim(getSpaceDimension());
    std::transform(_origin, _origin + dim, point, _origin, std::minus<double>());
    std::transform(
        _origin, _origin + dim, _origin, std::bind(std::multiplies<double>(), std::placeholders::_1, factor)
    );
...
MEDCouplingMesh *
MEDCouplingIMesh::mergeMyselfWith(const MEDCouplingMesh *other) const
{
    // not implemented yet !
    return 0;
}
 ...
 */
DataArrayDouble *
MEDCouplingIMesh::getCoordinatesAndOwner() const
{
    checkConsistencyLight();
    ...
    for (mcIdType i = 0; i < nbNodes; i++)
    {
        ...
        for (int j = 0; j < spaceDim; j++) pt[i * spaceDim + j] = _dxyz[j] * FromIdType<double>(tmp2[j]) + _origin[j];
    }
    return ret.retn();
}
 ...
 */
DataArrayDouble *
MEDCouplingIMesh::computeCellCenterOfMass() const
{
    checkConsistencyLight();
    MCAuto<DataArrayDouble> ret(DataArrayDouble::New());
    int spaceDim(getSpaceDimension());
    ...
    std::transform(
        _dxyz, _dxyz + spaceDim, shiftOrigin, std::bind(std::multiplies<double>(), std::placeholders::_1, 0.5)
    );
...
DataArrayDouble *
MEDCouplingIMesh::computeIsoBarycenterOfNodesPerCell() const
{
    return MEDCouplingIMesh::computeCellCenterOfMass();
}

void
MEDCouplingIMesh::renumberCells(const mcIdType *old2NewBg, bool check)
{
    throw INTERP_KERNEL::Exception("Functionality of renumbering cell not available for IMesh !");
}

void
MEDCouplingIMesh::getTinySerializationInformation(
...
void
MEDCouplingIMesh::resizeForUnserialization(
...
void
MEDCouplingIMesh::serialize(DataArrayIdType *&a1, DataArrayDouble *&a2) const
{
...
void
MEDCouplingIMesh::unserialization(
...
)
{
    ...
    _structure[0] = tinyInfo[3];
    _structure[1] = tinyInfo[4];
    _structure[2] = tinyInfo[5];
    _dxyz[0] = tinyInfoD[1];
    _dxyz[1] = tinyInfoD[2];
    _dxyz[2] = tinyInfoD[3];
    _origin[0] = tinyInfoD[4];
    _origin[1] = tinyInfoD[5];
    _origin[2] = tinyInfoD[6];
    declareAsNew();
}

void
MEDCouplingIMesh::writeVTKLL(
...
) const
{
    ...
    ofs << "  <" << getVTKDataSetType() << " WholeExtent=\"" << extent.str() << "\" Origin=\"" << origin.str()
        << "\" Spacing=\"" << spacing.str() << "\">\n";
...
void
MEDCouplingIMesh::reprQuickOverview(std::ostream &stream) const
{
    stream << "MEDCouplingIMesh C++ instance at " << this << ". Name : \"" << getName()
           << "\". Space dimension : " << _space_dim << ".";
...
std::string
MEDCouplingIMesh::getVTKFileExtension() const
{
    return std::string("vti");
}

std::string
MEDCouplingIMesh::getVTKDataSetType() const
{
    return std::string("ImageData");
}

std::vector<std::string>
MEDCouplingIMesh::buildInfoOnComponents() const
{
    checkSpaceDimension();
    int dim(getSpaceDimension());
...
void
MEDCouplingIMesh::checkSpaceDimension() const
{
    CheckSpaceDimension(_space_dim);
}

void
MEDCouplingIMesh::CheckSpaceDimension(int spaceDim)
{
    if (spaceDim < 0 || spaceDim > 3)
...
int
MEDCouplingIMesh::FindIntRoot(int val, int order)
{
    if (order == 0)
        return 1;
    if (val < 0)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingIMesh::FindIntRoot : input val is < 0 ! Not possible to compute a root !"
        );
    if (order == 1)
        return val;
    if (order != 2 && order != 3)
    ...
    if (order == 2)
    {
        ...
        if (ret * ret != val)
            throw INTERP_KERNEL::Exception(
                "MEDCouplingIMesh::FindIntRoot : the input val is not a perfect square root !"
            );
        return ret;
    }
    else  // order==3
    {
        ...
        if (ret * ret * ret != val && ret2 * ret2 * ret2 != val)
            throw INTERP_KERNEL::Exception(
                "MEDCouplingIMesh::FindIntRoot : the input val is not a perfect cublic root !"
            );
        if (ret * ret * ret == val)
            return ret;
        else
...
void
MEDCouplingIMesh::SpreadCoarseToFineGhost2D(
    const double *inPtr,
    double *outPtr,
    std::size_t nbCompo,
    const std::vector<mcIdType> &coarseSt,
...
)
{
    ...
    mcIdType kk(
        fineLocInCoarse[0].first + ghostSize - 1 + nxwg * (fineLocInCoarse[1].first + ghostSize - 1)
    );  // kk is always >=0 thanks to the fact that ghostSize>=1 !
    ...
    for (mcIdType j = 0; j < dims[1]; j++)
    {
        ...
        for (mcIdType jfact = 0; jfact < fact1; jfact++)
        {
            ...
            for (mcIdType i = 0; i < dims[0]; i++, kk0++)
            {
                ...
                for (mcIdType ifact = 0; ifact < fact0; ifact++) outPtrWork = std::copy(loc, loc + nbCompo, outPtrWork);
            }
            for (mcIdType ig = 0; ig < ghostSize; ig++)
                outPtrWork = std::copy(inPtr + kk0 * nbCompo, inPtr + (kk0 + 1) * nbCompo, outPtrWork);
        }
...
void
MEDCouplingIMesh::SpreadCoarseToFineGhostZone2D(
    const double *inPtr,
    double *outPtr,
    std::size_t nbCompo,
    const std::vector<mcIdType> &coarseSt,
