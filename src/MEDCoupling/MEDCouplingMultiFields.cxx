...
MEDCouplingMultiFields::New(const std::vector<MEDCouplingFieldDouble *> &fs)
{
    return new MEDCouplingMultiFields(fs);
}

MEDCouplingMultiFields *
MEDCouplingMultiFields::New()
{
...
bool
MEDCouplingMultiFields::isEqual(const MEDCouplingMultiFields *other, double meshPrec, double valsPrec) const
{
...
std::string
MEDCouplingMultiFields::getName() const
{
...
std::string
MEDCouplingMultiFields::getDescription() const
{
    std::vector<MCAuto<MEDCouplingFieldDouble> >::const_iterator it = _fs.begin();
    for (; it != _fs.end(); it++)
        if ((const MEDCouplingFieldDouble *)(*it))
...
std::string
MEDCouplingMultiFields::getTimeUnit() const
{
    std::vector<MCAuto<MEDCouplingFieldDouble> >::const_iterator it = _fs.begin();
    for (; it != _fs.end(); it++)
        if ((const MEDCouplingFieldDouble *)(*it))
...
double
MEDCouplingMultiFields::getTimeResolution() const
{
    std::vector<MCAuto<MEDCouplingFieldDouble> >::const_iterator it = _fs.begin();
    for (; it != _fs.end(); it++)
        if ((const MEDCouplingFieldDouble *)(*it))
...
std::string
MEDCouplingMultiFields::simpleRepr() const
{
...
std::string
MEDCouplingMultiFields::advancedRepr() const
{
    return simpleRepr();
}

bool
MEDCouplingMultiFields::isEqualWithoutConsideringStr(
...
MEDCouplingMultiFields::getFieldWithId(int id) const
{
...
MEDCouplingMultiFields::getFields() const
{
...
int
MEDCouplingMultiFields::getNumberOfFields() const
{
    return (int)_fs.size();
}

const MEDCouplingFieldDouble *
MEDCouplingMultiFields::getFieldAtPos(int id) const
{
    if (id < 0 || id >= ToIdType(_fs.size()))
    {
        std::ostringstream oss;
        oss << "MEDCouplingMultiFields::getFieldAtPos : Invalid given pos : should be >=0 and < " << _fs.size() << " !";
...
void
MEDCouplingMultiFields::updateTime() const
{
    std::vector<MCAuto<MEDCouplingFieldDouble> >::const_iterator it = _fs.begin();
    for (; it != _fs.end(); it++)
        if ((const MEDCouplingFieldDouble *)(*it))
...
std::size_t
MEDCouplingMultiFields::getHeapMemorySizeWithoutChildren() const
{
    return 0;
}

std::vector<const BigMemoryObject *>
MEDCouplingMultiFields::getDirectChildrenWithNull() const
{
...
std::vector<MEDCouplingMesh *>
MEDCouplingMultiFields::getMeshes() const
{
    std::vector<MEDCouplingMesh *> ms;
    for (std::vector<MCAuto<MEDCouplingFieldDouble> >::const_iterator it = _fs.begin(); it != _fs.end(); it++)
    {
...
std::vector<MEDCouplingMesh *>
MEDCouplingMultiFields::getDifferentMeshes(std::vector<int> &refs) const
{
...
MEDCouplingMultiFields::getArrays() const
{
...
std::vector<DataArrayDouble *>
MEDCouplingMultiFields::getDifferentArrays(std::vector<std::vector<int> > &refs) const
{
    refs.resize(_fs.size());
    int id = 0;
...
void
MEDCouplingMultiFields::checkConsistencyLight() const
{
    std::vector<MCAuto<MEDCouplingFieldDouble> >::const_iterator it = _fs.begin();
    for (; it != _fs.end(); it++)
    {
        if ((const MEDCouplingFieldDouble *)(*it) == 0)
            throw INTERP_KERNEL::Exception(
                "MEDCouplingMultiFields::checkConsistencyLight : There is an empty Field in array..."
            );
...
MEDCouplingMultiFields::MEDCouplingMultiFields(const MEDCouplingMultiFields &other) : RefCountObject(other)
{
    ...
    for (std::size_t i = 0; i < msLgh; i++) ms2[i] = ms[i]->deepCopy();
    ...
    for (std::size_t i = 0; i < dasLgth; i++) das2[i] = das[i]->deepCopy();
    for (std::size_t i = 0; i < sz; i++)
    {
...
void
MEDCouplingMultiFields::getTinySerializationInformation(
...
) const
{
    ...
    mcIdType sz2 = 0;
    for (mcIdType i = 0; i < sz; i++) sz2 += ToIdType(refs2[i].size());
    //
    tinyInfo2.clear();
    ...
    tinyInfo[2] = sz3;
    //
    for (mcIdType i = 0; i < sz; i++) tinyInfo[i + 3] = refs[i];
    for (mcIdType i = 0; i < sz; i++) tinyInfo[i + sz + 3] = ToIdType(refs2[i].size());
    for (mcIdType i = 0; i < sz; i++) tinyInfo[i + 2 * sz + 3] = ToIdType(_fs[i]->getTimeDiscretization());
    int k = 0;
    for (mcIdType i = 0; i < sz; i++)
...
void
MEDCouplingMultiFields::finishUnserialization(
...
)
{
    ...
    for (mcIdType i = 0; i < sz; i++)
    {
        ...
        //
        std::vector<mcIdType> tdInfoI(
            tinyInfoI.begin() + sz2 + 5 * sz + 3 + offI, tinyInfoI.begin() + sz2 + 5 * sz + 3 + offI + lgthI
        );
