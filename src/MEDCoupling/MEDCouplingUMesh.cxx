...
const INTERP_KERNEL::NormalizedCellType MEDCouplingUMesh::MEDMEM_ORDER[N_MEDMEM_ORDER] = {
    ...
    INTERP_KERNEL::NORM_POLYL,   INTERP_KERNEL::NORM_TRI3,    INTERP_KERNEL::NORM_QUAD4,   INTERP_KERNEL::NORM_TRI6,
    INTERP_KERNEL::NORM_TRI7,    INTERP_KERNEL::NORM_QUAD8,   INTERP_KERNEL::NORM_QUAD9,   INTERP_KERNEL::NORM_POLYGON,
    INTERP_KERNEL::NORM_QPOLYG,  INTERP_KERNEL::NORM_TETRA4,  INTERP_KERNEL::NORM_PYRA5,   INTERP_KERNEL::NORM_PENTA6,
    INTERP_KERNEL::NORM_HEXA8,   INTERP_KERNEL::NORM_HEXGP12, INTERP_KERNEL::NORM_TETRA10, INTERP_KERNEL::NORM_PYRA13,
    INTERP_KERNEL::NORM_PENTA15, INTERP_KERNEL::NORM_PENTA18, INTERP_KERNEL::NORM_HEXA20,  INTERP_KERNEL::NORM_HEXA27,
...
/*!
 * This method behaves mostly like MEDCouplingUMesh::deepCopy method, except that only nodal connectivity arrays are
 * deeply copied. The coordinates are shared between \a this and the returned instance.
 *
 * \return MEDCouplingUMesh * - A new object instance holding the copy of \a this (deep for connectivity, shallow for
 * coordiantes)
...
void
MEDCouplingUMesh::shallowCopyConnectivityFrom(const MEDCouplingPointSet *other)
{
    if (!other)
    ...
    if (!otherC)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::shallowCopyConnectivityFrom : input pointer is not an MEDCouplingUMesh instance !"
        );
...
std::size_t
MEDCouplingUMesh::getHeapMemorySizeWithoutChildren() const
{
...
MEDCouplingUMesh::getDirectChildrenWithNull() const
{
 ...
 */
void
MEDCouplingUMesh::checkConsistencyLight() const
{
    if (_mesh_dim < -1)
        throw INTERP_KERNEL::Exception("No mesh dimension specified !");
    if (_mesh_dim != -1)
        MEDCouplingPointSet::checkConsistencyLight();
    for (std::set<INTERP_KERNEL::NormalizedCellType>::const_iterator iter = _types.begin(); iter != _types.end();
         iter++)
    {
        if (ToIdType(INTERP_KERNEL::CellModel::GetCellModel(*iter).getDimension()) != _mesh_dim)
        {
            std::ostringstream message;
            message << "Mesh invalid because dimension is " << _mesh_dim
                    << " and there is presence of cell(s) with type " << (*iter);
    ...
    if (_nodal_connec)
    {
        if (_nodal_connec->getNumberOfComponents() != 1)
            throw INTERP_KERNEL::Exception(
                "Nodal connectivity array is expected to be with number of components set to one !"
            );
        if (_nodal_connec->getInfoOnComponent(0) != "")
            throw INTERP_KERNEL::Exception(
                "Nodal connectivity array is expected to have no info on its single component !"
            );
    }
    else if (_mesh_dim != -1)
    ...
    if (_nodal_connec_index)
    {
        if (_nodal_connec_index->getNumberOfComponents() != 1)
            throw INTERP_KERNEL::Exception(
                "Nodal connectivity index array is expected to be with number of components set to one !"
            );
        if (_nodal_connec_index->getInfoOnComponent(0) != "")
            throw INTERP_KERNEL::Exception(
                "Nodal connectivity index array is expected to have no info on its single component !"
            );
    }
    else if (_mesh_dim != -1)
 ...
 */
void
MEDCouplingUMesh::checkConsistency(double eps) const
{
    ...
    for (mcIdType i = 0; i < nbOfCells; i++)
    {
        const INTERP_KERNEL::CellModel &cm =
            INTERP_KERNEL::CellModel::GetCellModel((INTERP_KERNEL::NormalizedCellType)ptr[ptrI[i]]);
        if (ToIdType(cm.getDimension()) != meshDim)
        {
            std::ostringstream oss;
            oss << "MEDCouplingUMesh::checkConsistency : cell << #" << i << " with type Type " << cm.getRepr()
                << " in 'this' whereas meshdim == " << meshDim << " !";
        ...
        if (!cm.isDynamic())
            ...
            {
                std::ostringstream oss;
                oss << "MEDCouplingUMesh::checkConsistency : cell #" << i << " with static Type '" << cm.getRepr()
                    << "' has " << cm.getNumberOfNodes();
        ...
        if (cm.isQuadratic() && cm.isDynamic() && meshDim == 2)
            ...
            {
                std::ostringstream oss;
                oss << "MEDCouplingUMesh::checkConsistency : cell #" << i << " with quadratic type '" << cm.getRepr()
                    << "' has " << nbOfNodesInCell;
        ...
        for (const mcIdType *w = ptr + ptrI[i] + 1; w != ptr + ptrI[i + 1]; w++)
        {
            ...
            if (nodeId >= 0)
            {
                ...
                {
                    std::ostringstream oss;
                    oss << "Cell #" << i << " is built with node #" << nodeId << " whereas there are only " << nbOfNodes
                        << " nodes in the mesh !";
                    throw INTERP_KERNEL::Exception(oss.str());
                }
            }
            ...
            {
                std::ostringstream oss;
                oss << "Cell #" << i << " is built with node #" << nodeId << " in connectivity ! sounds bad !";
                throw INTERP_KERNEL::Exception(oss.str());
            }
            else
            {
                ...
                {
                    std::ostringstream oss;
                    oss << "Cell #" << i << " is built with node #-1 in connectivity ! sounds bad !";
                    throw INTERP_KERNEL::Exception(oss.str());
                }
            }
        }
 ...
 */
void
MEDCouplingUMesh::checkGeomConsistency(double eps) const
{
    ...
    for (auto icell = 0; icell < nbOfCells; ++icell)
    {
        ...
        if (ToIdType(s.size()) == ptrI[icell + 1] - ptrI[icell] - 1)
            continue;
        std::ostringstream oss;
        oss << "MEDCouplingUMesh::checkGeomConsistency : for cell #" << icell << " presence of multiple same nodeID !";
        throw INTERP_KERNEL::Exception(oss.str());
    }
}
...
MEDCouplingUMesh::setMeshDimension(int meshDim)
{
    if (meshDim < -1 || meshDim > 3)
        throw INTERP_KERNEL::Exception(
            "Invalid meshDim specified ! Must be greater or equal to -1 and lower or equal to 3 !"
        );
    _mesh_dim = meshDim;
    declareAsNew();
}
 ...
 */
void
MEDCouplingUMesh::allocateCells(mcIdType nbOfCells)
{
    if (nbOfCells < 0)
 ...
 */
void
MEDCouplingUMesh::insertNextCell(INTERP_KERNEL::NormalizedCellType type, mcIdType size, const mcIdType *nodalConnOfCell)
{
    const INTERP_KERNEL::CellModel &cm = INTERP_KERNEL::CellModel::GetCellModel(type);
    if (_nodal_connec_index == 0)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::insertNextCell : nodal connectivity not set ! invoke allocateCells before calling "
            "insertNextCell !"
        );
    if (ToIdType(cm.getDimension()) == _mesh_dim)
    {
        if (!cm.isDynamic())
            ...
            {
                std::ostringstream oss;
                oss << "MEDCouplingUMesh::insertNextCell : Trying to push a " << cm.getRepr() << " cell with a size of "
                    << size;
    ...
    {
        std::ostringstream oss;
        oss << "MEDCouplingUMesh::insertNextCell : cell type " << cm.getRepr() << " has a dimension "
            << cm.getDimension();
        oss << " whereas Mesh Dimension of current UMesh instance is set to " << _mesh_dim
            << " ! Please invoke \"setMeshDimension\" method before or invoke ";
...
/*!
 * Entry point for iteration over cells groups geo types per geotypes. Warning the returned cell iterator should be
 * deallocated. If \a this is not so that the cells are grouped by geo types, this method will throw an exception. In
 * this case MEDCouplingUMesh::sortCellsInMEDFileFrmt or MEDCouplingUMesh::rearrange2ConsecutiveCellTypes methods for
 * example can be called before invoking this method. Useful for python users.
 */
 ...
 * Sorted means in the same order than the cells in \a this. A single entry in return vector means the maximal chunk of
 * consecutive cells in \a this having the same geometric type. So a same geometric type can appear more than once if
 * the cells are not sorted per geometric type.
 *
...
MEDCouplingUMesh::getAllGeoTypesSorted() const
{
    ...
    if (getNodalConnectivityArrayLen() < 1)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::getAllGeoTypesSorted : the connectivity in this seems invalid !"
        );
...
MEDCouplingUMesh::isEqualIfNotWhy(const MEDCouplingMesh *other, double prec, std::string &reason) const
{
    ...
    {
        oss << "umesh dimension mismatch : this mesh dimension=" << _mesh_dim
            << " other mesh dimension=" << otherC->_mesh_dim;
        reason = oss.str();
        return false;
    }
    if (_types != otherC->_types)
    {
        ...
        for (std::set<INTERP_KERNEL::NormalizedCellType>::const_iterator iter = otherC->_types.begin();
             iter != otherC->_types.end();
             iter++)
        {
            const INTERP_KERNEL::CellModel &cm = INTERP_KERNEL::CellModel::GetCellModel(*iter);
            oss << cm.getRepr() << ", ";
    ...
    if (_nodal_connec_index != 0 || otherC->_nodal_connec_index != 0)
        ...
        {
            reason =
                "Only one UMesh between the two this and other has its nodal connectivity index DataArrayInt defined !";
            return false;
        }
...
MEDCouplingUMesh::checkFastEquivalWith(const MEDCouplingMesh *other, double prec) const
{
    ...
    if (!otherC)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::checkFastEquivalWith : Two meshes are not not unstructured !"
        );
}
 ...
 */
void
MEDCouplingUMesh::getReverseNodalConnectivity(DataArrayIdType *revNodal, DataArrayIdType *revNodalIndx) const
{
    checkFullyDefined();
    mcIdType nbOfNodes(getNumberOfNodes());
    ...
    for (mcIdType eltId = 0; eltId < nbOfCells; eltId++)
    {
        const mcIdType *strtNdlConnOfCurCell(conn + connIndex[eltId] + 1),
            *endNdlConnOfCurCell(conn + connIndex[eltId + 1]);
...
/*!
 * This method is a generalization of buildDescendingConnectivity method. This method return mesh containing subcells of
 * this at level specified by \a targetDeltaLevel and return descending and reverse descending correspondances to this.
 *
 * \param [in] targetDeltaLevel target delta level compared to \a this mesh dimension. This parameter is expected to be
 * lower than zero.
 *
 ...
 */
MCAuto<MEDCouplingUMesh>
MEDCouplingUMesh::explodeMeshTo(
    int targetDeltaLevel,
...
) const
{
    ...
    if (targetDeltaLevel == -this->getMeshDimension())
    {
        MCAuto<MEDCouplingUMesh> ret =
            MEDCouplingUMesh::Build0DMeshFromCoords(const_cast<DataArrayDouble *>(this->getCoords()));
        MEDCouplingUMesh::DeleteCellTypeInIndexedArray(
            getNodalConnectivity(), getNodalConnectivityIndex(), desc, descIndx
        );
        revDesc = DataArrayIdType::New();
        revDescIndx = DataArrayIdType::New();
 ...
 * This behaves exactly as MEDCouplingUMesh::buildDescendingConnectivity does except that this method compute directly
...
) const
{
    ...
    if (getMeshDimension() != 3)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::explode3DMeshTo1D : This has to have a mesh dimension to 3 !"
        );
...
/*!
 * This method computes the micro edges constituting each cell in \a this. Micro edge is an edge for non quadratic
 * cells. Micro edge is an half edge for quadratic cells. This method works for both meshes with mesh dimension equal to
 * 2 or 3. Dynamical cells are not supported (polygons, polyhedrons...)
 *
 * \sa explode3DMeshTo1D, buildDescendingConnectiviy
 */
MEDCouplingUMesh *
MEDCouplingUMesh::explodeMeshIntoMicroEdges(
    DataArrayIdType *desc, DataArrayIdType *descIndx, DataArrayIdType *revDesc, DataArrayIdType *revDescIndx
) const
{
    checkFullyDefined();
    ...
    {
        case 2:
            return buildDescendingConnectivityGen<MicroEdgesGenerator2D>(
                desc, descIndx, revDesc, revDescIndx, MEDCouplingFastNbrer
            );
        case 3:
            return buildDescendingConnectivityGen<MicroEdgesGenerator2D>(
...
{
    return buildDescendingConnectivityGen<MinusOneSonsGenerator>(
        desc, descIndx, revDesc, revDescIndx, MEDCouplingOrientationSensitiveNbrer
    );
}
 ...
 */
void
MEDCouplingUMesh::computeNeighborsOfCells(DataArrayIdType *&neighbors, DataArrayIdType *&neighborsIndx) const
{
 ...
 * re-arrange the data to produce a set indexed by cell IDs. The mapping between a node ID and a cell ID is done using
 * the connectivity of the mesh (e.g. a triangular element will receive the information from its three vertices).
 ...
 * \param [out] cellNeigh This array is newly allocated and should be dealt by the caller. It contains the initial
 * identifiers provided in the input parameters but stored now by cell index (See 2nd output parameter and \ref
 * numbering-indirect).
 ...
 */
void
MEDCouplingUMesh::computeCellNeighborhoodFromNodesOne(
    const DataArrayIdType *nodeNeigh,
...
) const
{
    ...
    checkConsistencyLight();
    nodeNeigh->checkAllocated();
    nodeNeighI->checkAllocated();
    ...
    nodeNeighI->checkNbOfTuples(
        1 + getNumberOfNodes(), "MEDCouplingUMesh::computeCellNeighborhoodFromNodesOne : invalid length"
    );
    mcIdType nbCells = getNumberOfCells();
    const mcIdType *c(_nodal_connec->begin()), *ci(_nodal_connec_index->begin()), *ne(nodeNeigh->begin()),
        *nei(nodeNeighI->begin());
    cellNeigh = DataArrayIdType::New();
    cellNeigh->alloc(0, 1);
    cellNeighIndex = DataArrayIdType::New();
    cellNeighIndex->alloc(1, 1);
    cellNeighIndex->setIJ(0, 0, 0);
 ...
 * \param [in] revDescIndx reverse descending connectivity index array used to walk through \b revDesc (\ref
 * numbering-indirect).
 * \param [out] neighbors is an array storing all the neighbors of all cells in \b this. This array is newly allocated
 * and should be dealt by the caller. \b neighborsIndx 2nd output parameter allows to select the right part in this
 * array. The number of tuples is equal to the last values in \b neighborsIndx.
 * \param [out] neighborsIndx is an array of size this->getNumberOfCells()+1 newly allocated and should be dealt by the
 * caller. This arrays allow to use the first output parameter \b neighbors.
 */
void
MEDCouplingUMesh::ComputeNeighborsOfCellsAdv(
    const DataArrayIdType *desc,
    const DataArrayIdType *descIndx,
    const DataArrayIdType *revDesc,
    const DataArrayIdType *revDescIndx,
    DataArrayIdType *&neighbors,
...
)
{
    ...
    MCAuto<DataArrayIdType> out1 = DataArrayIdType::New();
    out1->alloc(nbCells + 1, 1);
 ...
 */
MCAuto<MEDCouplingUMesh>
MEDCouplingUMesh::explodeIntoEdges(
    MCAuto<DataArrayIdType> &desc,
...
) const
{
    ...
    switch (mdim)
    {
        ...
        {
            throw INTERP_KERNEL::Exception(
                "MEDCouplingUMesh::computeNeighborsOfNodes : Mesh dimension supported are [3,2] !"
            );
        }
    }
    return mesh1D;
 ...
 */
void
MEDCouplingUMesh::computeNeighborsOfNodes(DataArrayIdType *&neighbors, DataArrayIdType *&neighborsIdx) const
{
    checkFullyDefined();
    ...
    switch (mdim)
    {
        ...
        {
            throw INTERP_KERNEL::Exception(
                "MEDCouplingUMesh::computeNeighborsOfNodes : Mesh dimension supported are [3,2,1] !"
            );
        }
    }
    desc = DataArrayIdType::New();
    ...
    for (mcIdType i = 0; i < nbNodes; i++, rni++)
    {
        for (const mcIdType *oneDCellIt = rn + rni[0]; oneDCellIt != rn + rni[1]; oneDCellIt++)
            *r0Pt++ =
                c1DPtr[3 * (*oneDCellIt) + 1] == i ? c1DPtr[3 * (*oneDCellIt) + 2] : c1DPtr[3 * (*oneDCellIt) + 1];
    }
    neighbors = ret0.retn();
...
/*!
 * Computes enlarged neighbors for each nodes in \a this. The behavior of this method is close to
 * MEDCouplingUMesh::computeNeighborsOfNodes except that the neighborhood of each node is wider here. A node j is
 * considered to be in the neighborhood of i if and only if there is a cell in \a this containing in its nodal
 * connectivity both i and j. This method is useful to find ghost cells of a part of a mesh with a code based on fields
 * on nodes.
 *
 * \sa MEDCouplingUMesh::computeNeighborsOfNodes
 */
void
MEDCouplingUMesh::computeEnlargedNeighborsOfNodes(
    MCAuto<DataArrayIdType> &neighbors, MCAuto<DataArrayIdType> &neighborsIdx
) const
{
    checkFullyDefined();
    mcIdType nbOfNodes(getNumberOfNodes());
    ...
    for (mcIdType eltId = 0; eltId < nbOfCells; eltId++)
    {
        const mcIdType *strtNdlConnOfCurCell(conn + connIndex[eltId] + 1),
            *endNdlConnOfCurCell(conn + connIndex[eltId + 1]);
        std::set<mcIdType> s(strtNdlConnOfCurCell, endNdlConnOfCurCell);
        s.erase(-1);  // for polyhedrons
        for (std::set<mcIdType>::const_iterator iter2 = s.begin(); iter2 != s.end(); iter2++)
            st0[*iter2].insert(s.begin(), s.end());
    }
    neighborsIdx = DataArrayIdType::New();
    neighborsIdx->alloc(nbOfNodes + 1, 1);
    neighborsIdx->setIJ(0, 0, 0);
    {
    ...
    neighbors = DataArrayIdType::New();
    neighbors->alloc(neighborsIdx->back(), 1);
    {
        const mcIdType *neighIdx(neighborsIdx->begin());
        mcIdType *neigh(neighbors->getPointer()), nodeId(0);
        for (std::vector<std::set<mcIdType> >::const_iterator it = st0.begin(); it != st0.end();
             it++, neighIdx++, nodeId++)
        {
            std::set<mcIdType> s(*it);
            s.erase(nodeId);
 ...
 */
void
MEDCouplingUMesh::convertToPolyTypes(const mcIdType *cellIdsToConvertBg, const mcIdType *cellIdsToConvertEnd)
{
    checkFullyDefined();
    ...
    if (dim == 2)
    {
        ...
        for (const mcIdType *iter = cellIdsToConvertBg; iter != cellIdsToConvertEnd; iter++)
        {
            ...
            {
                const INTERP_KERNEL::CellModel &cm =
                    INTERP_KERNEL::CellModel::GetCellModel((INTERP_KERNEL::NormalizedCellType)conn[connIndex[*iter]]);
                if (!cm.isQuadratic())
            ...
            {
                std::ostringstream oss;
                oss << "MEDCouplingUMesh::convertToPolyTypes : On rank #" << std::distance(cellIdsToConvertBg, iter)
                    << " value is " << *iter << " which is not";
    ...
    else
    {
        ...
        MCAuto<DataArrayIdType> connNew(DataArrayIdType::New()), connNewI(DataArrayIdType::New());
        connNew->alloc(0, 1);
        connNewI->alloc(1, 1);
        connNewI->setIJ(0, 0, 0);
        ...
        for (mcIdType cellId = 0; cellId < nbOfCells; cellId++)
        {
            ...
            {
                const INTERP_KERNEL::CellModel &cm =
                    INTERP_KERNEL::CellModel::GetCellModel((INTERP_KERNEL::NormalizedCellType)connOld[pos]);
                unsigned nbOfFaces(cm.getNumberOfSons2(connOld + pos + 1, lgthOld));
                mcIdType *tmp(new mcIdType[nbOfFaces * lgthOld + 1]);
                mcIdType *work = tmp;
                *work++ = INTERP_KERNEL::NORM_POLYHED;
...
MEDCouplingUMesh::convertAllToPoly()
{
    ...
    for (mcIdType i = 0; i < nbOfCells; i++) cellIds[i] = i;
...
MEDCouplingUMesh::convertExtrudedPolyhedra()
{
    ...
    if (getMeshDimension() != 3 || getSpaceDimension() != 3)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::convertExtrudedPolyhedra works on umeshes with meshdim equal to 3 and spaceDim equal to "
            "3 too!"
        );
    mcIdType nbOfCells = getNumberOfCells();
    ...
    for (mcIdType i = 0; i < nbOfCells; i++)
    {
        ...
        if (type == INTERP_KERNEL::NORM_POLYHED)
        {
            ...
            {
                std::ostringstream oss;
                oss << "MEDCouplingUMesh::convertExtrudedPolyhedra : cell # " << i
                    << " is a polhedron BUT it has NOT exactly 1 face !";
                throw INTERP_KERNEL::Exception(oss.str());
            }
            ...
            if (n2 % 2 != 0)
            {
                std::ostringstream oss;
                oss << "MEDCouplingUMesh::convertExtrudedPolyhedra : cell # " << i
                    << " is a polhedron with 1 face but there is a mismatch of number of nodes in face should be even "
                       "!";
                throw INTERP_KERNEL::Exception(oss.str());
            }
            mcIdType n1 = ToIdType(n2 / 2);
            newci[i + 1] = 7 * n1 + 2 + newci[i];  // 6*n1 (nodal length) + n1+2 (number of faces) - 1 (number of '-1'
                                                   // separator is equal to number of faces -1) + 1 (for cell type)
        }
        else
    ...
    _nodal_connec_index->decrRef();
    _nodal_connec_index = newCi.retn();
    _nodal_connec->decrRef();
    _nodal_connec = newC.retn();
}
...
MEDCouplingUMesh::unPolyze()
{
    ...
    if (mdim < 0)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::unPolyze works on umeshes with meshdim equals to 0, 1 2 or 3 !"
        );
    if (mdim <= 1)
        return false;
    mcIdType nbOfCells = getNumberOfCells();
        ...
        if (cm.isDynamic())
        {
            switch (cm.getDimension())
            {
                case 2:
                {
                    ...
                    newType = INTERP_KERNEL::CellSimplify::tryToUnPoly2D(
                        cm.isQuadratic(), tmp, lgthOfCurCell - 1, conn + newPos + 1, newLgth
                    );
                    break;
                }
                case 3:
                {
                    ...
                    INTERP_KERNEL::AutoPtr<mcIdType> zipFullReprOfPolyh =
                        INTERP_KERNEL::CellSimplify::getFullPolyh3DCell(
                            type, conn + posOfCurCell + 1, lgthOfCurCell - 1, nbOfFaces, lgthOfPolyhConn
                        );
                    newType = INTERP_KERNEL::CellSimplify::tryToUnPoly3D(
                        zipFullReprOfPolyh, nbOfFaces, lgthOfPolyhConn, conn + newPos + 1, newLgth
                    );
                    break;
                }
 ...
 * This method performs operation only on polyhedrons in \b this. If no polyhedrons exists in \b this, \b this remains
 * unchanged. This method allows to merge if any coplanar 3DSurf cells that may appear in some polyhedrons cells.
 *
 * \b WARNING: this method will not modify edges connectivity! Take a look at colinearizeEdges for that.
 *
 * \param [in] eps is a relative precision that allows to establish if some 3D plane are coplanar or not. This epsilon
 * is used to recenter around origin to have maximal precision.
 */
void
MEDCouplingUMesh::simplifyPolyhedra(double eps)
{
    ...
    if (getMeshDimension() != 3 || getSpaceDimension() != 3)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::simplifyPolyhedra : works on meshdimension 3 and spaceDimension 3 !"
        );
    ...
    mcIdType *connINewPtr = connINew->getPointer();
    *connINewPtr++ = 0;
    MCAuto<DataArrayIdType> connNew = DataArrayIdType::New();
    connNew->alloc(0, 1);
    MCAuto<DataArrayIdType> E_Fi(DataArrayIdType::New()), E_F(DataArrayIdType::New()), F_Ei(DataArrayIdType::New()),
        F_E(DataArrayIdType::New());
 ...
 * This method expects that spaceDimension is equal to 3 and meshDimension equal to 3.
 * This method performs operation only on polyhedrons in \b this. If no polyhedrons exists in \b this, \b this remains
 * unchanged. This method allows to simplify edges of polyhedron cells so that consecutive colinear segments (with
 * intermediate points not used by any other cell) are merged together.
 *
 ...
 */
void
MEDCouplingUMesh::colinearizeEdges(double eps)
{
    //
    ...
    DAI E_Fi(DataArrayIdType::New()), E_F(DataArrayIdType::New()), F_Ei(DataArrayIdType::New()),
        F_E(DataArrayIdType::New()), F_Si(DataArrayIdType::New()), F_S(DataArrayIdType::New()),
        S_Fi(DataArrayIdType::New()), S_F(DataArrayIdType::New()), S_Pi(DataArrayIdType::New()),
        S_P(DataArrayIdType::New()), P_Si(DataArrayIdType::New()), P_S(DataArrayIdType::New());
    MCAuto<MEDCouplingUMesh> m_f(buildDescendingConnectivity(E_F, E_Fi, F_E, F_Ei)),
        ...
        m_p(
            m_s->buildDescendingConnectivity(S_P, S_Pi, P_S, P_Si)
        );  // E: elem, F: faces, S: segments (edges), P: points (vertices)
    const mcIdType *S_Pp(S_P->begin()), *S_Pip(S_Pi->begin()), *P_Sp(P_S->begin()), *P_Sip(P_Si->begin());
    std::set<mcIdType> pt_rem;
    const mcIdType *m_pi = m_p->getNodalConnectivityIndex()->begin(), *m_pc = m_p->getNodalConnectivity()->begin();
    ...
    for (const mcIdType &i : *cand)  // i is a point to be potentially eliminated, shared by 2 segs only
    {
        ...
        for (mcIdType j = 0; j < 2; j++)
            for (mcIdType k = 0; k < 2; k++)
            {
                mcIdType off1 =
                    P_Sip[i] +
                    j;  // offset to get ID of the j-th seg (around the i-th point) in the point->seg correspondance
                mcIdType pt_id =
                    P_Sp[off1] + k;  // ID of the k-th point of the j-th seg in the point->seg correspondance
                mcIdType pt_id2 = S_Pp[S_Pip[pt_id]];  // ID of the point in the point mesh
                p[j][k] = m_pc[m_pi[pt_id2] + 1];      // Absolute ID, as read from the connectvity (+1 to skip type:
                                                       // NORM_POINT1) Just for fun, as initially written by Antoine :-)
        ...
        {
            for (int j = 0; j < 2; j++) n2[j] += std::pow(coord[p[j][1]][d] - coord[p[j][0]][d], 2);
    ...
    for (mcIdType i = 0; i < getNumberOfCells(); i++)
    {
        ...
        {
            // Keep point if it is not to be removed, or if is first in connectivity (TODO this last check could be
            // removed?)
...
/*!
 * This method returns all node ids used in the connectivity of \b this. The data array returned has to be dealt by the
 * caller. The returned node ids are sorted ascendingly. This method is close to MEDCouplingUMesh::getNodeIdsInUse
 * except the format of the returned DataArrayIdType instance.
 *
...
{
    mcIdType nbOfNodes = ToIdType(nodeIdsInUse.size()), nbOfCells = getNumberOfCells();
    ...
    for (mcIdType i = 0; i < nbOfCells; i++)
        for (mcIdType j = connIndex[i] + 1; j < connIndex[i + 1]; j++)
            if (conn[j] >= 0)
            {
                ...
                {
                    std::ostringstream oss;
                    oss << "MEDCouplingUMesh::computeNodeIdsAlg : In cell #" << i << " presence of node id " << conn[j]
                        << " not in [0," << nbOfNodes << ") !";
                    throw INTERP_KERNEL::Exception(oss.str());
                }
            }
...
struct MEDCouplingAccVisit
{
    ...
    mcIdType operator()(mcIdType val)
    {
        if (val != -1)
            return _new_nb_of_nodes++;
        else
            return -1;
    }
 ...
 */
DataArrayIdType *
MEDCouplingUMesh::getNodeIdsInUse(mcIdType &nbrOfNodesInUse) const
{
    nbrOfNodesInUse = -1;
    mcIdType nbOfNodes(getNumberOfNodes());
    ...
    for (mcIdType i = 0; i < nbOfCells; i++)
        for (mcIdType j = connIndex[i] + 1; j < connIndex[i + 1]; j++)
            if (conn[j] >= 0)
            {
                ...
                {
                    std::ostringstream oss;
                    oss << "MEDCouplingUMesh::getNodeIdsInUse : In cell #" << i << " presence of node id " << conn[j]
                        << " not in [0," << nbOfNodes << ") !";
                    throw INTERP_KERNEL::Exception(oss.str());
                }
            }
...
/*!
 * This method computes effective number of nodes per cell. That is to say nodes appearing several times in nodal
 * connectivity of a cell, will be counted only once here whereas it will be counted several times in
 * MEDCouplingUMesh::computeNbOfNodesPerCell method.
 *
 * \return DataArrayIdType * - new object to be deallocated by the caller.
 * \sa MEDCouplingUMesh::computeNbOfNodesPerCell
 */
DataArrayIdType *
MEDCouplingUMesh::computeEffectiveNbOfNodesPerCell() const
{
    checkConnectivityFullyDefined();
    mcIdType nbOfCells = getNumberOfCells();
    MCAuto<DataArrayIdType> ret = DataArrayIdType::New();
...
/*!
 * This method returns a newly allocated array containing this->getNumberOfCells() tuples and 1 component.
 * For each cell in \b this the number of faces constituting (entity of dimension this->getMeshDimension()-1) cell is
 * computed.
 *
 * \return a newly allocated array
 */
DataArrayIdType *
MEDCouplingUMesh::computeNbOfFacesPerCell() const
{
    checkConnectivityFullyDefined();
    mcIdType nbOfCells = getNumberOfCells();
    MCAuto<DataArrayIdType> ret = DataArrayIdType::New();
    ...
    {
        const INTERP_KERNEL::CellModel &cm =
            INTERP_KERNEL::CellModel::GetCellModel((INTERP_KERNEL::NormalizedCellType)conn[*connI]);
 ...
 */
int
MEDCouplingUMesh::AreCellsEqual(
    const mcIdType *conn, const mcIdType *connI, mcIdType cell1, mcIdType cell2, int compType
)
{
    switch (compType)
    {
 ...
 */
int
MEDCouplingUMesh::AreCellsEqualPolicy0(const mcIdType *conn, const mcIdType *connI, mcIdType cell1, mcIdType cell2)
{
 ...
 */
int
MEDCouplingUMesh::AreCellsEqualPolicy1(const mcIdType *conn, const mcIdType *connI, mcIdType cell1, mcIdType cell2)
{
    ...
    if (sz == connI[cell2 + 1] - connI[cell2])
    {
        ...
        {
            const INTERP_KERNEL::CellModel &cm =
                INTERP_KERNEL::CellModel::GetCellModel((INTERP_KERNEL::NormalizedCellType)conn[connI[cell1]]);
            ...
            if (dim != 3)
            {
                if (dim != 1)
                {
                    ...
                    work = std::search(
                        (mcIdType *)tmp, (mcIdType *)tmp + sz1, conn + connI[cell2] + 1, conn + connI[cell2 + 1]
                    );
                    ...
                    return std::equal(conn + connI[cell1] + 1, conn + connI[cell1 + 1], conn + connI[cell2] + 1)
                               ? 1
                               : 0;  // case of SEG2 and SEG3
            }
            else
                throw INTERP_KERNEL::Exception(
                    "MEDCouplingUMesh::AreCellsEqualPolicy1 : not implemented yet for meshdim == 3 !"
                );
        }
    }
    return 0;
 ...
 */
int
MEDCouplingUMesh::AreCellsEqualPolicy2(const mcIdType *conn, const mcIdType *connI, mcIdType cell1, mcIdType cell2)
{
    if (connI[cell1 + 1] - connI[cell1] == connI[cell2 + 1] - connI[cell2])
    {
        if (conn[connI[cell1]] == conn[connI[cell2]])
...
/*!
 * This method is less restrictive than AreCellsEqualPolicy2. Here the geometric type is absolutely not taken into
 * account !
 */
int
MEDCouplingUMesh::AreCellsEqualPolicy2NoType(
 ...
 */
int
MEDCouplingUMesh::AreCellsEqualPolicy7(const mcIdType *conn, const mcIdType *connI, mcIdType cell1, mcIdType cell2)
{
    mcIdType sz = connI[cell1 + 1] - connI[cell1];
    if (sz == connI[cell2 + 1] - connI[cell2])
    {
        if (conn[connI[cell1]] == conn[connI[cell2]])
        {
            ...
            if (dim != 3)
            {
                ...
                throw INTERP_KERNEL::Exception(
                    "MEDCouplingUMesh::AreCellsEqualPolicy7 : not implemented yet for meshdim == 3 !"
                );
        }
    }
    return 0;
...
)
{
    ...
    commonCellsI->reserve(1);
    commonCellsI->pushBackSilent(0);
            ...
            if (!isFetched[i])  // if fetched i is already in a common pack
            {
                ...
                for (; connOfNode != functor.endIterator(connPtr + connIPtr[i] + 1, connPtr + connIPtr[i + 1]) &&
                    ...
                    if (*connOfNode >= 0)
                    {
                        ...
                        std::vector<mcIdType>::iterator it = std::set_intersection(
                            v.begin(), v.end(), locRevNodal, revNodalPtr + revNodalIPtr[*connOfNode + 1], v2.begin()
                        );
                ...
                for (; connOfNode != functor.endIterator(connPtr + connIPtr[i] + 1, connPtr + connIPtr[i + 1]) &&
                    ...
                    if (*connOfNode >= 0)
                    {
                        ...
                        std::vector<mcIdType>::iterator it = std::set_intersection(
                            v.begin(),
                            v.end(),
                            revNodalPtr + revNodalIPtr[*connOfNode],
                            revNodalPtr + revNodalIPtr[*connOfNode + 1],
                            v2.begin()
                        );
                        v2.resize(std::distance(v2.begin(), it));
                    }
 ...
 */
bool
MEDCouplingUMesh::areCellsIncludedIn(const MEDCouplingUMesh *other, int compType, DataArrayIdType *&arr) const
{
    MCAuto<MEDCouplingUMesh> mesh = MergeUMeshesOnSameCoords(this, other);
    mcIdType nbOfCells = getNumberOfCells();
    static const int possibleCompType[] = {0, 1, 2};
    if (std::find(possibleCompType, possibleCompType + sizeof(possibleCompType) / sizeof(int), compType) ==
        possibleCompType + sizeof(possibleCompType) / sizeof(int))
    {
        std::ostringstream oss;
        oss << "MEDCouplingUMesh::areCellsIncludedIn : only following policies are possible : ";
        std::copy(
            possibleCompType,
            possibleCompType + sizeof(possibleCompType) / sizeof(int),
    ...
    {
        MCAuto<DataArrayIdType> dftRet(DataArrayIdType::New());
        dftRet->alloc(0, 1);
        arr = dftRet.retn();
    ...
    MCAuto<DataArrayIdType> o2n = DataArrayIdType::ConvertIndexArrayToO2N(
        ToIdType(mesh->getNumberOfCells()),
        commonCells->begin(),
        commonCellsI->begin(),
        commonCellsI->end(),
        newNbOfCells
 ...
 * ! This method tries to determine if \b other is fully included in \b this. The main difference is that this method is
 * not expected to throw exception. This method has two outputs :
 *
 * \param other other mesh
 * \param arr is an output parameter that returns a \b newly created instance. This array is of size
 * 'other->getNumberOfCells()'.
 * \return If \a other is fully included in 'this 'true is returned. If not false is returned.
 */
bool
MEDCouplingUMesh::areCellsIncludedInPolicy7(const MEDCouplingUMesh *other, DataArrayIdType *&arr) const
{
    MCAuto<MEDCouplingUMesh> mesh = MergeUMeshesOnSameCoords(this, other);
...
MEDCouplingUMesh::mergeMyselfWithOnSameCoords(const MEDCouplingPointSet *other) const
{
    ...
    if (!otherC)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::mergeMyselfWithOnSameCoords : the input other mesh is not of type unstructured !"
        );
 ...
 * \param keepCoords that specifies if you want or not to keep coords as this or zip it (see
 * MEDCoupling::MEDCouplingUMesh::zipCoords). If true zipCoords is \b NOT called, if false, zipCoords is called.
 * \return a newly allocated
 *
 * \warning This method modifies can generate an unstructured mesh whose cells are not sorted by geometric type order.
 * In view of the MED file writing, a renumbering of cells of returned unstructured mesh (using
 * MEDCouplingUMesh::sortCellsInMEDFileFrmt) should be necessary.
 */
MEDCouplingUMesh *
MEDCouplingUMesh::buildPartOfMySelfSlice(mcIdType start, mcIdType end, mcIdType step, bool keepCoords) const
{
    if (getMeshDimension() != -1)
        return static_cast<MEDCouplingUMesh *>(
            MEDCouplingPointSet::buildPartOfMySelfSlice(start, end, step, keepCoords)
        );
    else
    {
        mcIdType newNbOfCells = DataArray::GetNumberOfItemGivenBESRelative(
            start, end, step, "MEDCouplingUMesh::buildPartOfMySelfSlice for -1 dimension mesh "
        );
        if (newNbOfCells != 1)
 ...
 */
MEDCouplingUMesh *
MEDCouplingUMesh::buildPartOfMySelf(const mcIdType *begin, const mcIdType *end, bool keepCoords) const
{
    if (getMeshDimension() != -1)
 ...
 *
 * This method allows to partially modify some cells in \b this (whose list is specified by [ \b cellIdsBg, \b
 * cellIdsEnd ) ) with cells coming in \b otherOnSameCoordsThanThis. Size of [ \b cellIdsBg, \b cellIdsEnd ) ) must be
 * equal to the number of cells of otherOnSameCoordsThanThis. The number of cells of \b this will remain the same with
 * this method.
 *
 ...
 * \param [in] otherOnSameCoordsThanThis an another mesh with same meshdimension than \b this with exactly the same
 * number of cells than cell ids list in [\b cellIdsBg, \b cellIdsEnd ). Coordinate pointer of \b this and those of \b
 * otherOnSameCoordsThanThis must be the same
 */
void
MEDCouplingUMesh::setPartOfMySelf(
    const mcIdType *cellIdsBg, const mcIdType *cellIdsEnd, const MEDCouplingUMesh &otherOnSameCoordsThanThis
)
{
    checkConnectivityFullyDefined();
    ...
    if (getCoords() != otherOnSameCoordsThanThis.getCoords())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::setPartOfMySelf : coordinates pointer are not the same ! Invoke setCoords or call "
            "tryToShareSameCoords method !"
        );
    if (getMeshDimension() != otherOnSameCoordsThanThis.getMeshDimension())
    {
        std::ostringstream oss;
        oss << "MEDCouplingUMesh::setPartOfMySelf : Mismatch of meshdimensions ! this is equal to "
            << getMeshDimension();
    ...
    {
        std::ostringstream oss;
        oss << "MEDCouplingUMesh::setPartOfMySelf : cells ids length (" << nbOfCellsToModify
            << ") do not match the number of cells of other mesh (" << otherOnSameCoordsThanThis.getNumberOfCells()
            << ") !";
        throw INTERP_KERNEL::Exception(oss.str());
    }
    mcIdType nbOfCells(getNumberOfCells());
    ...
    for (const mcIdType *it = cellIdsBg; it != cellIdsEnd && easyAssign; it++, connIOther++)
    {
        ...
        {
            std::ostringstream oss;
            oss << "MEDCouplingUMesh::setPartOfMySelf : On pos #" << std::distance(cellIdsBg, it) << " id is equal to "
                << *it << " which is not in [0," << nbOfCells << ") !";
            throw INTERP_KERNEL::Exception(oss.str());
        }
    }
    if (easyAssign)
    {
        DataArrayIdType::SetPartOfIndexedArraysSameIdx(
            cellIdsBg,
            cellIdsEnd,
            _nodal_connec,
            _nodal_connec_index,
            otherOnSameCoordsThanThis._nodal_connec,
            otherOnSameCoordsThanThis._nodal_connec_index
        );
        computeTypes();
    }
    else
    {
        ...
        DataArrayIdType::SetPartOfIndexedArrays(
            cellIdsBg,
            cellIdsEnd,
            _nodal_connec,
            _nodal_connec_index,
...
void
MEDCouplingUMesh::setPartOfMySelfSlice(
    mcIdType start, mcIdType end, mcIdType step, const MEDCouplingUMesh &otherOnSameCoordsThanThis
)
{
    checkConnectivityFullyDefined();
    otherOnSameCoordsThanThis.checkConnectivityFullyDefined();
    if (getCoords() != otherOnSameCoordsThanThis.getCoords())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::setPartOfMySelfSlice : coordinates pointer are not the same ! Invoke setCoords or call "
            "tryToShareSameCoords method !"
        );
    if (getMeshDimension() != otherOnSameCoordsThanThis.getMeshDimension())
    {
        std::ostringstream oss;
        oss << "MEDCouplingUMesh::setPartOfMySelfSlice : Mismatch of meshdimensions ! this is equal to "
            << getMeshDimension();
        oss << ", whereas other mesh dimension is set equal to " << otherOnSameCoordsThanThis.getMeshDimension()
            << " !";
        throw INTERP_KERNEL::Exception(oss.str());
    }
    mcIdType nbOfCellsToModify =
        DataArray::GetNumberOfItemGivenBESRelative(start, end, step, "MEDCouplingUMesh::setPartOfMySelfSlice : ");
    if (nbOfCellsToModify != otherOnSameCoordsThanThis.getNumberOfCells())
    {
        std::ostringstream oss;
        oss << "MEDCouplingUMesh::setPartOfMySelfSlice : cells ids length (" << nbOfCellsToModify
            << ") do not match the number of cells of other mesh (" << otherOnSameCoordsThanThis.getNumberOfCells()
            << ") !";
        throw INTERP_KERNEL::Exception(oss.str());
    }
    ...
    for (mcIdType i = 0; i < nbOfCellsToModify && easyAssign; i++, it += step, connIOther++)
    {
        ...
        {
            std::ostringstream oss;
            oss << "MEDCouplingUMesh::setPartOfMySelfSlice : On pos #" << i << " id is equal to " << it
                << " which is not in [0," << nbOfCells << ") !";
            throw INTERP_KERNEL::Exception(oss.str());
        }
    }
    if (easyAssign)
    {
        DataArrayIdType::SetPartOfIndexedArraysSameIdxSlice(
            start,
            end,
            step,
            _nodal_connec,
    ...
    {
        DataArrayIdType *arrOut = 0, *arrIOut = 0;
        DataArrayIdType::SetPartOfIndexedArraysSlice(
            start,
            end,
            step,
            _nodal_connec,
...
MEDCouplingUMesh::buildBoundaryMesh(bool keepCoords) const
{
    ...
    revDescIndx->decrRef();
    MEDCouplingUMesh *ret =
        meshDM1->buildPartOfMySelf(&boundaryCells[0], &boundaryCells[0] + boundaryCells.size(), keepCoords);
    return ret;
}
 ...
 * This method makes the assumption that \a this is fully defined (coords,connectivity). If not an exception will be
 * thrown.
 */
DataArrayIdType *
MEDCouplingUMesh::findCellIdsOnBoundary() const
{
    ...
    desc = (DataArrayIdType *)0;
    descIndx = (DataArrayIdType *)0;
    //
    MCAuto<DataArrayIdType> tmp = revDescIndx->deltaShiftIndex();
    MCAuto<DataArrayIdType> faceIds = tmp->findIdsEqual(1);
    tmp = (DataArrayIdType *)0;
    ...
    for (const mcIdType *pt = faceIds->begin(); pt != faceIds->end(); pt++)
        ...
        {
            ret1[revDescPtr[revDescIndxPtr[*pt]]] = true;
            sz++;
        }
    //
    ret2->alloc(sz, 1);
 ...
 * \b this and \b otherDimM1OnSameCoords have to lie on the same coordinate array pointer. The coherency of that coords
 * array with connectivity of \b this and \b otherDimM1OnSameCoords is not important here because this method works only
 * on connectivity. this->getMeshDimension() - 1 must be equal to otherDimM1OnSameCoords.getMeshDimension()
 *
 * s0 is the cell ids set in \b this lying on at least one node in the fetched nodes in \b otherDimM1OnSameCoords.
 * This method also returns the cells ids set s1 which contains the cell ids in \b this for which one of the dim-1
 * constituent equals a cell in \b otherDimM1OnSameCoords.
 *
 * \throw if \b otherDimM1OnSameCoords is not part of constituent of \b this, or if coordinate pointer of \b this and \b
 * otherDimM1OnSameCoords are not same, or if this->getMeshDimension()-1!=otherDimM1OnSameCoords.getMeshDimension()
 *
 * \param [in] otherDimM1OnSameCoords other mesh
 * \param [out] cellIdsRk0 a newly allocated array containing the cell ids of s0 (which are cell ids of \b this) in the
 * above algorithm.
 * \param [out] cellIdsRk1 a newly allocated array containing the cell ids of s1 \b indexed into the \b cellIdsRk0
 * subset. To get the absolute ids of s1, simply invoke
 *              cellIdsRk1->transformWithIndArr(cellIdsRk0->begin(),cellIdsRk0->end());
 */
void
MEDCouplingUMesh::findCellIdsLyingOn(
    const MEDCouplingUMesh &otherDimM1OnSameCoords, DataArrayIdType *&cellIdsRk0, DataArrayIdType *&cellIdsRk1
) const
{
    if (getCoords() != otherDimM1OnSameCoords.getCoords())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::findCellIdsLyingOn : coordinates pointer are not the same ! Use tryToShareSameCoords "
            "method !"
        );
    checkConnectivityFullyDefined();
    ...
    if (getMeshDimension() - 1 != otherDimM1OnSameCoords.getMeshDimension())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::findCellIdsLyingOn : invalid mesh dimension of input mesh regarding meshdimesion of "
            "this !"
        );
    ...
    MCAuto<MEDCouplingUMesh> thisPart =
        static_cast<MEDCouplingUMesh *>(buildPartOfMySelf(s0arr->begin(), s0arr->end(), true));
    MCAuto<DataArrayIdType> descThisPart = DataArrayIdType::New(), descIThisPart = DataArrayIdType::New(),
                            revDescThisPart = DataArrayIdType::New(), revDescIThisPart = DataArrayIdType::New();
    MCAuto<MEDCouplingUMesh> thisPartConsti =
        thisPart->buildDescendingConnectivity(descThisPart, descIThisPart, revDescThisPart, revDescIThisPart);
    const mcIdType *revDescThisPartPtr = revDescThisPart->getConstPointer(),
                   *revDescIThisPartPtr = revDescIThisPart->getConstPointer();
    ...
    if (!b)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::findCellIdsLyingOn : the given mdim-1 mesh in other is not a constituent of this !"
        );
    std::set<mcIdType> s1;
    for (const mcIdType *idOther = idsOtherInConsti->begin(); idOther != idsOtherInConsti->end(); idOther++)
        s1.insert(
            revDescThisPartPtr + revDescIThisPartPtr[*idOther], revDescThisPartPtr + revDescIThisPartPtr[*idOther + 1]
        );
    MCAuto<DataArrayIdType> s1arr_renum1 = DataArrayIdType::New();
    s1arr_renum1->alloc(s1.size(), 1);
    std::copy(s1.begin(), s1.end(), s1arr_renum1->getPointer());
    s1arr_renum1->sort();
...
/*!
 * This method computes the skin of \b this. That is to say the consituting meshdim-1 mesh is built and only the
 * boundary subpart is returned. This subpart of meshdim-1 mesh is built using meshdim-1 cells in it shared only one
 * cell in \b this.
 *
 ...
 * This method searches for nodes needed to be duplicated. Those are the nodes fetched by \b otherDimM1OnSameCoords
 * which are not part of the boundary of \b otherDimM1OnSameCoords. If a node is in the boundary of \b this \b and in
 * the boundary of \b otherDimM1OnSameCoords, it will be duplicated. When the set of node ids \b nodeIdsToDuplicate is
 * computed, cell ids in \b this is searched so that their connectivity includes at least 1 node in \b
 * nodeIdsToDuplicate.
 *
 * \param [in] crackingMesh a mesh lying on the same coords than \b this and with a mesh dimension equal to those of \b
 * this minus 1. WARNING this input parameter is altered during the call.
 * \return node ids which need to be duplicated following the algorithm explained above.
 *
 */
DataArrayIdType *
MEDCouplingUMesh::findNodesToDuplicate(const MEDCouplingUMesh &crackingMesh) const
{
    ...
    if (getCoords() != crackingMesh.getCoords())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::findNodesToDuplicate : meshes do not share the same coords array !"
        );
    if (crackingMesh.getMeshDimension() != getMeshDimension() - 1)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::findNodesToDuplicate : the mesh given in other parameter must have "
            "this->getMeshDimension()-1 !"
        );

    // Clean the M1 group (cracking mesh): the M1 cells which are part of M0 boundary are irrelevant (we can't create a
    // crack on the boundary of M0!)
    ...
    MCUMesh otherDimM1OnSameCoords =
        static_cast<MEDCouplingUMesh *>(crackingMesh.buildPartOfMySelf(ids2->begin(), ids2->end(), true));
    ...
    DAInt dt0 = DataArrayIdType::New(), dit0 = DataArrayIdType::New(), rdt0 = DataArrayIdType::New(),
          rdit0 = DataArrayIdType::New();
    MCUMesh meshM2 = otherDimM1OnSameCoords->buildDescendingConnectivity(
        dt0, dit0, rdt0, rdit0
    );  // 2D: a mesh of points, 3D: a mesh of segs
    DAInt dsi = rdit0->deltaShiftIndex();
    DAInt idsTmp0 = dsi->findIdsNotInRange(
        -1, 3
    );  // for 2D: if a point is connected to more than 2 segs. For 3D: if a seg is connected to more than two faces.
    if (idsTmp0->getNumberOfTuples())
        throw INTERP_KERNEL::Exception(
            "MEDFileUMesh::buildInnerBoundaryAlongM1Group: group is too complex: some points (or edges) have more than "
            "two connected segments (or faces)!"
        );
    dt0 = 0;
    dit0 = 0;
    rdt0 = 0;
    ...
    DAInt xtremIdsM2 = dsi->findIdsEqual(1);
    dsi = 0;
    MCUMesh meshM2Part =
        static_cast<MEDCouplingUMesh *>(meshM2->buildPartOfMySelf(xtremIdsM2->begin(), xtremIdsM2->end(), true));
    ...
    //    In 3D, some points on the boundary of M0 will NOT be duplicated (where as in 2D, points on the boundary of M0
    //    are always duplicated) Think of a partial (plane) crack in a cube: the points at the tip of the crack and not
    //    located inside the volume of the cube are not duplicated although they are technically on the skin of the
    //    cube.
    DAInt fNodes = m0skin->computeFetchedNodeIds();
    DAInt notDup = 0;
    if (getMeshDimension() == 3)
    {
        DAInt dnu1 = DataArrayIdType::New(), dnu2 = DataArrayIdType::New(), dnu3 = DataArrayIdType::New(),
              dnu4 = DataArrayIdType::New();
        MCUMesh m0skinDesc = m0skin->buildDescendingConnectivity(
            dnu1, dnu2, dnu3, dnu4
        );  // all segments of the skin of the 3D (M0) mesh
        dnu1 = 0;
        dnu2 = 0;
        dnu3 = 0;
        dnu4 = 0;
        ...
        {
            // Build the set of segments which are: in the desc mesh of the skin of the 3D mesh (M0) **and** in the desc
            // mesh of the M1 group: (the U-shaped polyline described above)
            MCUMesh m1IntersecSkin = static_cast<MEDCouplingUMesh *>(
                m0skinDesc->buildPartOfMySelf(validIds->begin(), validIds->end(), true)
            );
            // Its boundary nodes should no be duplicated (this is for example the tip of the crack inside the cube
            // described above)
            ...
            //   - indeed, if duplicated, such a point would lead to the duplication of a cell which has no face
            //   touching M1 ! The algorithm would be duplicating too much ...
            // This is a costly algorithm so only go into it if a simple (non sufficient) criteria is met: a node
            // connected to more than 3 segs in meshM2:
            dnu1 = DataArrayIdType::New(), dnu2 = DataArrayIdType::New(), dnu3 = DataArrayIdType::New(),
            rdit0 = DataArrayIdType::New();
            MCUMesh meshM2Desc =
                meshM2->buildDescendingConnectivity(dnu1, dnu2, dnu3, rdit0);  // a mesh made of node cells
            dnu1 = 0;
            dnu2 = 0;
            dnu3 = 0;
            dsi = rdit0->deltaShiftIndex();
            rdit0 = 0;
            DAInt singPoints = dsi->findIdsNotInRange(-1, 4);
            dsi = 0;  // points connected to (strictly) more than 3 segments
            if (singPoints->getNumberOfTuples())
            {
                ...
                DAInt cellsAroundBN = getCellIdsLyingOnNodes(
                    boundNodes->begin(), boundNodes->end(), false
                );  // false= take cell in, even if not all nodes are in dupl
                MCUMesh mAroundBN = static_cast<MEDCouplingUMesh *>(
                    this->buildPartOfMySelf(cellsAroundBN->begin(), cellsAroundBN->end(), true)
                );
                DAInt descBN = DataArrayIdType::New(), descIBN = DataArrayIdType::New(),
                      revDescBN = DataArrayIdType::New(), revDescIBN = DataArrayIdType::New();
                ...
                DAInt idsTouch = DataArrayIdType::New();
                idsTouch->alloc(0, 1);
                ...
                MCUMesh mAroundBNStrict = static_cast<MEDCouplingUMesh *>(
                    mAroundBN->buildPartOfMySelf(idsTouchCompl->begin(), idsTouchCompl->end(), true)
                );
 ...
 *
 * \param [in] otherDimM1OnSameCoords a mesh lying on the same coords than \b this and with a mesh dimension equal to
 * those of \b this minus 1.
 ...
 * \param [out] cellIdsNotModified cell ids in \b this that lies on \b otherDimM1OnSameCoords mesh whose connectivity do
 * \b not need to be modified as it is the case for \b cellIdsNeededToBeRenum.
 *
 */
void
MEDCouplingUMesh::findCellsToRenumber(
    const MEDCouplingUMesh &otherDimM1OnSameCoords,
...
) const
{
    ...
    if (getCoords() != otherDimM1OnSameCoords.getCoords())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::findCellsToRenumber: meshes do not share the same coords array !"
        );
    if (otherDimM1OnSameCoords.getMeshDimension() != getMeshDimension() - 1)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::findCellsToRenumber: the mesh given in other parameter must have "
            "this->getMeshDimension()-1 !"
        );
    ...
    {
        cellIdsNeededToBeRenum = DataArrayIdType::New();
        cellIdsNeededToBeRenum->alloc(0, 1);
        cellIdsNotModified = DataArrayIdType::New();
        cellIdsNotModified->alloc(0, 1);
        return;
    }

    // Compute cell IDs of the mesh with cells that touch the M1 group with a least one node:
    DAInt cellsAroundGroupLarge = getCellIdsLyingOnNodes(
        nodeIdsToDuplicateBg, nodeIdsToDuplicateEnd, false
    );  // false= take cell in, even if not all nodes are in dupl
    MCUMesh mAroundGrpLarge = static_cast<MEDCouplingUMesh *>(
        buildPartOfMySelf(cellsAroundGroupLarge->begin(), cellsAroundGroupLarge->end(), true)
    );
    mcIdType nCellsLarge = cellsAroundGroupLarge->getNumberOfTuples();
    DAInt descL = DataArrayIdType::New(), descIL = DataArrayIdType::New(), revDescL = DataArrayIdType::New(),
          revDescIL = DataArrayIdType::New();
    ...
    //     In the neighbor information remove the connection between high dimension cells and its low level constituents
    //     which are part of the frontier given in parameter (i.e. the cells of low dimension from the group delimiting
    //     the crack):
    ...
    DAInt hitCellsLarge = DataArrayIdType::New();
    hitCellsLarge->alloc(nCellsLarge, 1);
    ...
    {
        if (v >= nL)
            continue;  // Keep valid match only - see doc of areCellsIncludedIn()
        ...
        for (const auto c : {c1, c2})
        {
            ...
            mcIdType dnu;
            DAInt spreadZone =
                MEDCouplingUMesh::ComputeSpreadZoneGraduallyFromSeed(&c, &c + 1, neighL, neighIL, -1, dnu);
            std::set<mcIdType> sv;
            for (const mcIdType &s : *spreadZone)
                if (hitCellsLargeP[s])
                    sv.insert(hitCellsLargeP[s]);
            if (sv.size() > 1)
                // Strange: we find in the same spread zone a +1 and -1 !
                throw INTERP_KERNEL::Exception(
                    "MEDCouplingUMesh::findCellsToRenumber: internal error #0 - conflicting values - should not happen!"
                );
            ...
            if (hitCellsLargeP[other])
            {
                if (val && hitCellsLargeP[other] != -val)
                    throw INTERP_KERNEL::Exception(
                        "MEDCouplingUMesh::findCellsToRenumber: internal error #1 - conflicting values - should not "
                        "happen!"
                    );
                ;
                // We do not yet have a value, but other side has one. Use it!
                if (!val)
                    val = -hitCellsLargeP[other];
            }
    ...
    if (cellsRet1->getNumberOfTuples() + cellsRet2->getNumberOfTuples() != cellsAroundGroupLarge->getNumberOfTuples())
    {
        ...
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::findCellsToRenumber: Some cells not hit - Internal error should not happen"
        );
    }
 ...
 * Every time that a node id in [ \b nodeIdsToDuplicateBg, \b nodeIdsToDuplicateEnd ) will append in nodal connectivity
 * of \b this its ids will be modified to id
 * this->getNumberOfNodes()+std::distance(nodeIdsToDuplicateBg,std::find(nodeIdsToDuplicateBg,nodeIdsToDuplicateEnd,id)).
 * More explicitly the renumber array in nodes is not explicitly given in old2new to avoid to build a big array of
 * renumbering whereas typically few node ids needs to be renumbered. The node id nodeIdsToDuplicateBg[0] will have id
 * this->getNumberOfNodes()+0, node id nodeIdsToDuplicateBg[1] will have id this->getNumberOfNodes()+1, node id
 * nodeIdsToDuplicateBg[2] will have id this->getNumberOfNodes()+2...
 *
 * As a consequence nodal connectivity array length will remain unchanged by this method, and nodal connectivity index
 * array will remain unchanged by this method.
 *
 ...
 */
void
MEDCouplingUMesh::duplicateNodes(const mcIdType *nodeIdsToDuplicateBg, const mcIdType *nodeIdsToDuplicateEnd)
{
    mcIdType nbOfNodes = getNumberOfNodes();
...
/*!
 * This method renumbers only nodal connectivity in \a this. The renumbering is only an offset applied. So this method
 * is a specialization of
 * \a renumberNodesInConn. \b WARNING, this method does not check that the resulting node ids in the nodal connectivity
 * is in a valid range !
 *
 ...
 */
void
MEDCouplingUMesh::renumberNodesWithOffsetInConn(mcIdType offset)
{
    checkConnectivityFullyDefined();
...
/*!
 *  Same than renumberNodesInConn(const mcIdType *) except that here the format of old-to-new traducer is using map
 * instead of array. This method is dedicated for renumbering from a big set of nodes the a tiny set of nodes which is
 * the case during extraction of a big mesh.
 */
void
MEDCouplingUMesh::renumberNodesInConn(const INTERP_KERNEL::HashMap<mcIdType, mcIdType> &newNodeNumbersO2N)
{
 ...
 */
void
MEDCouplingUMesh::renumberNodesInConn(const std::map<mcIdType, mcIdType> &newNodeNumbersO2N)
{
 ...
 */
void
MEDCouplingUMesh::renumberNodesInConn(const mcIdType *newNodeNumbersO2N)
{
    checkConnectivityFullyDefined();
    mcIdType *conn = getNodalConnectivity()->getPointer();
    const mcIdType *connIndex = getNodalConnectivityIndex()->getConstPointer();
 ...
 * This method is an specialization of \ref MEDCoupling::MEDCouplingUMesh::renumberNodesInConn "renumberNodesInConn
 * method".
 *
 * \param [in] delta specifies the shift size applied to nodeId in nodal connectivity in \b this.
 */
void
MEDCouplingUMesh::shiftNodeNumbersInConn(mcIdType delta)
{
    checkConnectivityFullyDefined();
    mcIdType *conn = getNodalConnectivity()->getPointer();
    const mcIdType *connIndex = getNodalConnectivityIndex()->getConstPointer();
 ...
 * Coordinates are \b NOT considered here and will remain unchanged by this method. this->_coords can ever been null for
 * the needs of this method. Every time that a node id in [ \b nodeIdsToDuplicateBg, \b nodeIdsToDuplicateEnd ) will
 * append in nodal connectivity of \b this its ids will be modified to id
 * offset+std::distance(nodeIdsToDuplicateBg,std::find(nodeIdsToDuplicateBg,nodeIdsToDuplicateEnd,id)). More explicitly
 * the renumber array in nodes is not explicitly given in old2new to avoid to build a big array of renumbering whereas
 * typically few node ids needs to be renumbered. The node id nodeIdsToDuplicateBg[0] will have id offset+0, node id
 * nodeIdsToDuplicateBg[1] will have id offset+1, node id nodeIdsToDuplicateBg[2] will have id offset+2...
 *
 * As a consequence nodal connectivity array length will remain unchanged by this method, and nodal connectivity index
 * array will remain unchanged by this method. As an another consequense after the call of this method \b this can be
 * transiently non cohrent.
 *
 * \param [in] nodeIdsToDuplicateBg begin of node ids (included) to be duplicated in connectivity only
 * \param [in] nodeIdsToDuplicateEnd end of node ids (excluded) to be duplicated in connectivity only
 * \param [in] offset the offset applied to all node ids in connectivity that are in [ \a nodeIdsToDuplicateBg, \a
 * nodeIdsToDuplicateEnd ).
 */
void
MEDCouplingUMesh::duplicateNodesInConn(
    const mcIdType *nodeIdsToDuplicateBg, const mcIdType *nodeIdsToDuplicateEnd, mcIdType offset
)
{
    checkConnectivityFullyDefined();
    std::map<mcIdType, mcIdType> m;
    mcIdType val = offset;
    for (const mcIdType *work = nodeIdsToDuplicateBg; work != nodeIdsToDuplicateEnd; work++, val++) m[*work] = val;
    mcIdType *conn = getNodalConnectivity()->getPointer();
    const mcIdType *connIndex = getNodalConnectivityIndex()->getConstPointer();
    mcIdType nbOfCells = getNumberOfCells();
    for (mcIdType i = 0; i < nbOfCells; i++)
 ...
 * an INTERP_KERNEL::Exception will be thrown. When 'check' equals true [ \a old2NewBg ; \a old2NewEnd ) is not expected
 * to be strictly in [0;this->getNumberOfCells()).
 *
 * If 'check' equals false the method will not check the content of [ \a old2NewBg ; \a old2NewEnd ).
 * To avoid any throw of SIGSEGV when 'check' equals false, the elements in [ \a old2NewBg ; \a old2NewEnd ) should be
 * unique and should be contained in[0;this->getNumberOfCells()).
 *
 * \param [in] old2NewBg is expected to be a dynamically allocated pointer of size at least equal to
 * this->getNumberOfCells()
 * \param check whether to check content of old2NewBg
 */
void
MEDCouplingUMesh::renumberCells(const mcIdType *old2NewBg, bool check)
{
    checkConnectivityFullyDefined();
    mcIdType nbCells = getNumberOfCells();
    ...
    o2n->useArray(array, false, DeallocType::C_DEALLOC, nbCells, 1);
...
{
    MCAuto<DataArrayIdType> elems = DataArrayIdType::New();
    elems->alloc(0, 1);
    if (getMeshDimension() == -1)
    {
    ...
    for (mcIdType ielem = 0; ielem < nbOfCells; ielem++)
    {
        ...
        for (mcIdType inode = conn_index[ielem] + 1; inode < conn_index[ielem + 1];
             inode++)  //+1 due to offset of cell type.
        {
...
/*!
 * Given a boundary box 'bbox' returns elements 'elems' contained in this 'bbox' or touching 'bbox' (within 'eps'
 * distance). Warning 'elems' is incremented during the call so if elems is not empty before call returned elements will
 * be added in 'elems' parameter.
 */
DataArrayIdType *
...
MEDCouplingUMesh::getTypeOfCell(mcIdType cellId) const
{
    ...
    {
        std::ostringstream oss;
        oss << "MEDCouplingUMesh::getTypeOfCell : Requesting type of cell #" << cellId << " but it should be in [0,"
            << _nodal_connec_index->getNbOfElems() - 1 << ") !";
        throw INTERP_KERNEL::Exception(oss.str());
    }
}

/*!
 * This method returns a newly allocated array containing cell ids (ascendingly sorted) whose geometric type are equal
 * to type. This method does not throw exception if geometric type \a type is not in \a this. This method throws an
 * INTERP_KERNEL::Exception if meshdimension of \b this is not equal to those of \b type. The coordinates array is not
 * considered here.
 *
...
MEDCouplingUMesh::giveCellsWithType(INTERP_KERNEL::NormalizedCellType type) const
{
    ...
    if (mdim != ToIdType(cm.getDimension()))
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::giveCellsWithType : Mismatch between mesh dimension and dimension of the cell !"
        );
...
/*!
 * Returns nb of cells having the geometric type \a type. No throw if no cells in \a this has the geometric type \a
 * type.
 */
mcIdType
 ...
 */
void
MEDCouplingUMesh::getNodeIdsOfCell(mcIdType cellId, std::vector<mcIdType> &conn) const
{
    const mcIdType *ptI(_nodal_connec_index->begin()), *pt(_nodal_connec->begin());
...
MEDCouplingUMesh::simpleRepr() const
{
    ...
    if (_coords != 0)
    {
        ...
        for (int i = 0; i < spaceDim; i++) ret << "\"" << _coords->getInfoOnComponent(i) << "\" ";
        ret << "\n";
    }
    else
        ret << msg0 << "\n";
...
MEDCouplingUMesh::cppRepr() const
{
    ...
    checkFullyDefined();
    std::ostringstream ret;
    ret << "// coordinates" << std::endl;
    _coords->reprCppStream(coordsName, ret);
    ret << std::endl << "// connectivity" << std::endl;
    _nodal_connec->reprCppStream(connName, ret);
    ret << std::endl;
    _nodal_connec_index->reprCppStream(connIName, ret);
    ret << std::endl;
    ret << "MEDCouplingUMesh *mesh=MEDCouplingUMesh::New(\"" << getName() << "\"," << getMeshDimension() << ");"
        << std::endl;
...
/*!
 * This method builds a newly allocated instance (with the same name than \a this) that the caller has the
 * responsibility to deal with. This method returns an instance with all arrays allocated (connectivity, connectivity
 * index, coordinates) but with length of these arrays set to 0. It allows to define an "empty" mesh (with nor cells nor
 * nodes but compliant with some algos).
 *
 * This method expects that \a this has a mesh dimension set and higher or equal to 0. If not an exception will be
 * thrown. This method analyzes the 3 arrays of \a this. For each the following behaviour is done : if the array is null
 * a newly one is created with number of tuples set to 0, if not the array is taken as this in the returned instance.
 */
MEDCouplingUMesh *
MEDCouplingUMesh::buildSetInstanceFromThis(std::size_t spaceDim) const
{
    ...
    if (mdim < 0)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::buildSetInstanceFromThis : invalid mesh dimension ! Should be >= 0 !"
        );
    ...
    if (!_coords)
    {
        MCAuto<DataArrayDouble> coords = DataArrayDouble::New();
        coords->alloc(0, spaceDim);
...
mcIdType
MEDCouplingUMesh::getNumberOfNodesInCell(mcIdType cellId) const
{
    const mcIdType *ptI = _nodal_connec_index->getConstPointer();
    const mcIdType *pt = _nodal_connec->getConstPointer();
 ...
 */
std::set<INTERP_KERNEL::NormalizedCellType>
MEDCouplingUMesh::getTypesOfPart(const mcIdType *begin, const mcIdType *end) const
{
    checkFullyDefined();
    std::set<INTERP_KERNEL::NormalizedCellType> ret;
    const mcIdType *conn = _nodal_connec->getConstPointer();
    const mcIdType *connIndex = _nodal_connec_index->getConstPointer();
    for (const mcIdType *w = begin; w != end; w++) ret.insert((INTERP_KERNEL::NormalizedCellType)conn[connIndex[*w]]);
    return ret;
}
 ...
 */
void
MEDCouplingUMesh::setConnectivity(DataArrayIdType *conn, DataArrayIdType *connIndex, bool isComputingTypes)
{
 ...
 */
MEDCouplingUMesh::MEDCouplingUMesh(const MEDCouplingUMesh &other, bool deepCpy)
    : MEDCouplingPointSet(other, deepCpy),
      _mesh_dim(other._mesh_dim),
      _nodal_connec(0),
      _nodal_connec_index(0),
      _types(other._types)
{
 ...
 */
mcIdType
MEDCouplingUMesh::getNodalConnectivityArrayLen() const
{
 ...
 */
void
MEDCouplingUMesh::getTinySerializationInformation(
 ...
 */
bool
MEDCouplingUMesh::isEmptyMesh(const std::vector<mcIdType> &tinyInfo) const
{
    return tinyInfo[6] <= 0;
}
 ...
 */
void
MEDCouplingUMesh::resizeForUnserialization(
 ...
 */
void
MEDCouplingUMesh::serialize(DataArrayIdType *&a1, DataArrayDouble *&a2) const
{
 ...
 */
void
MEDCouplingUMesh::unserialization(
...
MEDCouplingUMesh::getMeasureField(bool isAbs) const
{
    ...
    if (getMeshDimension() != -1)
    {
        ...
        for (mcIdType iel = 0; iel < nbelem; iel++)
        {
            ...
            area_vol[iel] = INTERP_KERNEL::computeVolSurfOfCell2<mcIdType, INTERP_KERNEL::ALL_C_MODE>(
                type, connec + ipt + 1, connec_index[iel + 1] - ipt - 1, coords, dim_space
            );
        }
        if (isAbs)
 ...
 */
DataArrayDouble *
MEDCouplingUMesh::getPartMeasureField(bool isAbs, const mcIdType *begin, const mcIdType *end) const
{
    ...
    if (getMeshDimension() != -1)
    {
        ...
        for (const mcIdType *iel = begin; iel != end; iel++)
        {
            ipt = connec_index[*iel];
            type = (INTERP_KERNEL::NormalizedCellType)connec[ipt];
            *area_vol++ = INTERP_KERNEL::computeVolSurfOfCell2<mcIdType, INTERP_KERNEL::ALL_C_MODE>(
                type, connec + ipt + 1, connec_index[*iel + 1] - ipt - 1, coords, dim_space
            );
        }
        if (isAbs)
...
MEDCouplingUMesh::buildOrthogonalField() const
{
    if ((getMeshDimension() != 2) && (getMeshDimension() != 1 || getSpaceDimension() != 2))
        throw INTERP_KERNEL::Exception(
            "Expected a umesh with ( meshDim == 2 spaceDim == 2 or 3 ) or ( meshDim == 1 spaceDim == 2 ) !"
        );
    ...
    if (getMeshDimension() == 2)
    {
        if (getSpaceDimension() == 3)
        {
            ...
            for (mcIdType i = 0; i < nbOfCells; i++, vals += 3)
            {
                ...
                INTERP_KERNEL::crossprod<3>(
                    locPtr + 3 * i, coords + 3 * conn[offset + 1], coords + 3 * conn[offset + 2], vals
                );
                double n = INTERP_KERNEL::norm<3>(vals);
                std::transform(
                    vals, vals + 3, vals, std::bind(std::multiplies<double>(), std::placeholders::_1, 1. / n)
                );
            }
        }
        else
    ...
    else  // meshdimension==1
    {
        ...
        for (mcIdType i = 0; i < nbOfCells; i++)
        {
            mcIdType offset = connI[i];
            std::transform(
                coords + 2 * conn[offset + 2],
                coords + 2 * conn[offset + 2] + 2,
                coords + 2 * conn[offset + 1],
                tmp,
                std::minus<double>()
            );
 ...
 */
MEDCouplingFieldDouble *
MEDCouplingUMesh::buildPartOrthogonalField(const mcIdType *begin, const mcIdType *end) const
{
    if ((getMeshDimension() != 2) && (getMeshDimension() != 1 || getSpaceDimension() != 2))
        throw INTERP_KERNEL::Exception(
            "Expected a umesh with ( meshDim == 2 spaceDim == 2 or 3 ) or ( meshDim == 1 spaceDim == 2 ) !"
...
MEDCouplingUMesh::buildDirectionVectorField() const
{
    ...
    if (_types.size() != 1 || *(_types.begin()) != INTERP_KERNEL::NORM_SEG2)
        throw INTERP_KERNEL::Exception(
            "Expected a umesh with only NORM_SEG2 type of elements for buildDirectionVectorField !"
        );
    MCAuto<MEDCouplingFieldDouble> ret = MEDCouplingFieldDouble::New(ON_CELLS, ONE_TIME);
    MCAuto<DataArrayDouble> array = DataArrayDouble::New();
    mcIdType nbOfCells = getNumberOfCells();
    ...
    for (mcIdType i = 0; i < nbOfCells; i++)
    {
        ...
        pt = std::transform(
            coo + conn[1] * spaceDim, coo + (conn[1] + 1) * spaceDim, coo + conn[0] * spaceDim, pt, std::minus<double>()
        );
    }
    ret->setArray(array);
    ret->setMesh(this);
...
MEDCouplingUMesh::buildSlice3D(const double *origin, const double *vec, double eps, DataArrayIdType *&cellIds) const
{
    ...
    if (getMeshDimension() != 3 || getSpaceDimension() != 3)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::buildSlice3D works on umeshes with meshdim equal to 3 and spaceDim equal to 3 too!"
        );
    MCAuto<DataArrayIdType> candidates = getCellIdsCrossingPlane(origin, vec, eps);
    if (candidates->empty())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::buildSlice3D : No 3D cells in this intercepts the specified plane considering bounding "
            "boxes !"
        );
    std::vector<mcIdType> nodes;
    DataArrayIdType *cellIds1D = 0;
    MCAuto<MEDCouplingUMesh> subMesh =
        static_cast<MEDCouplingUMesh *>(buildPartOfMySelf(candidates->begin(), candidates->end(), false));
    ...
    MCAuto<MEDCouplingUMesh> mDesc2 =
        subMesh->buildDescendingConnectivity(desc2, descIndx2, revDesc2, revDescIndx2);  // meshDim==2 spaceDim==3
    revDesc2 = 0;
    revDescIndx2 = 0;
    MCAuto<MEDCouplingUMesh> mDesc1 =
        mDesc2->buildDescendingConnectivity(desc1, descIndx1, revDesc1, revDescIndx1);  // meshDim==1 spaceDim==3
    revDesc1 = 0;
    revDescIndx1 = 0;
    // Marking all 1D cells that contained at least one node located on the plane
    // the intersection between those cells and the plane, which consist of the nodes previously tagged, thus don't need
    // to be computed afterwards (if said intersection is computed in MEDCouplingUMesh::split3DCurveWithPlane, then we
    // might create additional nodes due to accuracy errors when the needed nodes already exist)
    ...
    for (const mcIdType *it = cellIds1D->begin(); it != cellIds1D->end(); it++) cut3DCurve[*it] = -1;
    ...
    AssemblyForSplitFrom3DCurve(
        cut3DCurve,
        nodes,
        mDesc2->getNodalConnectivity()->getConstPointer(),
        mDesc2->getNodalConnectivityIndex()->getConstPointer(),
        mDesc1->getNodalConnectivity()->getConstPointer(),
        mDesc1->getNodalConnectivityIndex()->getConstPointer(),
        desc1->getConstPointer(),
        descIndx1->getConstPointer(),
        cut3DSurf
    );
    MCAuto<DataArrayIdType> conn(DataArrayIdType::New()), connI(DataArrayIdType::New()),
        cellIds2(DataArrayIdType::New());
    connI->pushBackSilent(0);
    conn->alloc(0, 1);
    cellIds2->alloc(0, 1);
    subMesh->assemblyForSplitFrom3DSurf(
        cut3DSurf, desc2->getConstPointer(), descIndx2->getConstPointer(), conn, connI, cellIds2
    );
    if (cellIds2->empty())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::buildSlice3D : No 3D cells in this intercepts the specified plane !"
        );
...
addition to the mesh, returns a new DataArrayIdType, of length equal to the number of 1D cells in the result mesh,
holding, for each cell in the result mesh, an id of a 2D cell it comes from. If a result segment is shared by two 2D
cells, then the segment in included twice in the result mesh.
 *  \param [in] origin - 3 components of a point defining location of the plane.
 *  \param [in] vec - 3 components of a vector normal to the plane. Vector magnitude
 *         must be greater than 1e-6.
 *  \param [in] eps - half-thickness of the plane.
...
MEDCouplingUMesh::buildSlice3DSurf(const double *origin, const double *vec, double eps, DataArrayIdType *&cellIds) const
{
    ...
    if (getMeshDimension() != 2 || getSpaceDimension() != 3)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::buildSlice3DSurf works on umeshes with meshdim equal to 2 and spaceDim equal to 3 !"
        );
    MCAuto<DataArrayIdType> candidates(getCellIdsCrossingPlane(origin, vec, eps));
    if (candidates->empty())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::buildSlice3DSurf : No 3D surf cells in this intercepts the specified plane considering "
            "bounding boxes !"
        );
    std::vector<mcIdType> nodes;
    DataArrayIdType *cellIds1D(0);
    ...
    subMesh->findNodesOnPlane(origin, vec, eps, nodes);
    MCAuto<DataArrayIdType> desc1(DataArrayIdType::New()), descIndx1(DataArrayIdType::New()),
        revDesc1(DataArrayIdType::New()), revDescIndx1(DataArrayIdType::New());
    MCAuto<MEDCouplingUMesh> mDesc1(
        subMesh->buildDescendingConnectivity(desc1, descIndx1, revDesc1, revDescIndx1)
    );  // meshDim==1 spaceDim==3
    ...
    AssemblyForSplitFrom3DCurve(
        cut3DCurve,
        nodes,
        subMesh->getNodalConnectivity()->getConstPointer(),
        subMesh->getNodalConnectivityIndex()->getConstPointer(),
        mDesc1->getNodalConnectivity()->getConstPointer(),
        mDesc1->getNodalConnectivityIndex()->getConstPointer(),
        desc1->getConstPointer(),
        descIndx1->getConstPointer(),
    ...
    for (mcIdType i = 0; i < ncellsSub; i++)
    {
        if (cut3DSurf[i].first != -1 && cut3DSurf[i].second != -1)
        {
            ...
            {
                conn->pushBackSilent(ToIdType(INTERP_KERNEL::NORM_SEG2));
                conn->pushBackSilent(cut3DSurf[i].first);
                conn->pushBackSilent(cut3DSurf[i].second);
            ...
            else
            {
                ...
                {
                    conn->pushBackSilent(ToIdType(INTERP_KERNEL::NORM_SEG2));
                    conn->pushBackSilent(nodal[offset + j]);
                    conn->pushBackSilent(nodal[offset + (j + 1) % nbOfEdges]);
                    connI->pushBackSilent(conn->getNumberOfTuples());
    ...
    if (cellIds2->empty())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::buildSlice3DSurf : No 3DSurf cells in this intercepts the specified plane !"
        );
...
MCAuto<MEDCouplingUMesh>
MEDCouplingUMesh::clipSingle3DCellByPlane(const double origin[3], const double vec[3], double eps) const
{
    checkFullyDefined();
    if (getMeshDimension() != 3 || getSpaceDimension() != 3)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::clipSingle3DCellByPlane works on umeshes with meshdim equal to 3 and spaceDim equal to "
            "3 too!"
        );
    if (getNumberOfCells() != 1)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::clipSingle3DCellByPlane works only on mesh containing exactly one cell !"
        );
    //
    std::vector<mcIdType> nodes;
    findNodesOnPlane(origin, vec, eps, nodes);
    MCAuto<DataArrayIdType> desc1(DataArrayIdType::New()), desc2(DataArrayIdType::New()),
        descIndx1(DataArrayIdType::New()), descIndx2(DataArrayIdType::New()), revDesc1(DataArrayIdType::New()),
        revDesc2(DataArrayIdType::New()), revDescIndx1(DataArrayIdType::New()), revDescIndx2(DataArrayIdType::New());
    MCAuto<MEDCouplingUMesh> mDesc2(
        buildDescendingConnectivity(desc2, descIndx2, revDesc2, revDescIndx2)
    );  // meshDim==2 spaceDim==3
    revDesc2 = 0;
    revDescIndx2 = 0;
    MCAuto<MEDCouplingUMesh> mDesc1(
        mDesc2->buildDescendingConnectivity(desc1, descIndx1, revDesc1, revDescIndx1)
    );  // meshDim==1 spaceDim==3
    revDesc1 = 0;
    revDescIndx1 = 0;
    DataArrayIdType *cellIds1D(0);
    ...
    AssemblyForSplitFrom3DCurve(
        cut3DCurve,
        nodes,
        mDesc2->getNodalConnectivity()->begin(),
        mDesc2->getNodalConnectivityIndex()->begin(),
        mDesc1->getNodalConnectivity()->begin(),
        mDesc1->getNodalConnectivityIndex()->begin(),
        desc1->begin(),
        descIndx1->begin(),
        cut3DSurf
    );
    ...
    conn->alloc(0, 1);
    {
        MCAuto<DataArrayIdType> cellIds2(DataArrayIdType::New());
        cellIds2->alloc(0, 1);
    ...
    //
    MCAuto<DataArrayDouble> vec2(DataArrayDouble::New());
    vec2->alloc(1, 3);
    std::copy(vec, vec + 3, vec2->getPointer());
    ...
    {
        MCAuto<DataArrayDouble> pt(DataArrayDouble::New());
        pt->alloc(1, 3);
        std::copy(origin, origin + 3, pt->getPointer());
        occm = DataArrayDouble::Substract(ccm, pt);
    }
    vec2 = DataArrayDouble::New();
    vec2->alloc(nbCellsRet, 3);
    vec2->setPartOfValuesSimple1(vec[0], 0, nbCellsRet, 1, 0, 1, 1);
    vec2->setPartOfValuesSimple1(vec[1], 0, nbCellsRet, 1, 1, 2, 1);
    vec2->setPartOfValuesSimple1(vec[2], 0, nbCellsRet, 1, 2, 3, 1);
...
MEDCouplingUMesh::getCellIdsCrossingPlane(const double *origin, const double *vec, double eps) const
{
    ...
    if (normm < 1e-6)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::getCellIdsCrossingPlane : parameter 'vec' should have a norm2 greater than 1e-6 !"
        );
    double vec2[3];
    vec2[0] = vec[1];
    vec2[1] = -vec[0];
    vec2[2] = 0.;  // vec2 is the result of cross product of vec with (0,0,1)
    ...
    if (angle > eps)
    {
        ...
        if (normm2 / normm > 1e-6)
            DataArrayDouble::Rotate3DAlg(
                origin, vec2, angle, coo->getNumberOfTuples(), coo->getPointer(), coo->getPointer()
            );
...
/*!
 * This method checks that \a this is a contiguous mesh. The user is expected to call this method on a mesh with
 * meshdim==1. If not an exception will thrown. If this is an empty mesh with no cell an exception will be thrown too.
 * No consideration of coordinate is done by this method.
 * A 1D mesh is said contiguous if : a cell i with nodal connectivity (k,p) the cell i+1 the nodal connectivity should
 * be (p,m) If not false is returned. In case that false is returned a call to MEDCoupling::MEDCouplingUMesh::mergeNodes
 * could be useful.
 */
bool
MEDCouplingUMesh::isContiguous1D() const
{
    ...
    if (nbCells < 1)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::isContiguous1D : this method has a sense for non empty mesh !"
        );
 ...
 * This method projects this on the 3D line defined by (pt,v). This methods first checks that all SEG2 are along v
 * vector.
...
/*!
 * This method computes the distance from a point \a pt to \a this and the first \a cellId in \a this corresponding to
 ...
 * Distance from \a ptBg to \a ptEnd is expected to be equal to the space dimension. \a this is also expected to be
 * fully defined (connectivity and coordinates).
 *
 * WARNING, if there is some orphan nodes in \a this (nodes not fetched by any cells in \a this ( see
 * MEDCouplingUMesh::zipCoords ) ) these nodes will ** not ** been taken into account in this method. Only cells and
 * nodes lying on them are considered in the algorithm (even if one of these orphan nodes is closer than returned
 * distance). A user that needs to consider orphan nodes should invoke DataArrayDouble::minimalDistanceTo method on the
 * coordinates array of \a this.
 *
 ...
 * \param [out] cellId that corresponds to minimal distance. If the closer node is not linked to any cell in \a this -1
 * is returned.
 * \return the positive value of the distance.
 * \throw if distance from \a ptBg to \a ptEnd is not equal to the space dimension. An exception is also thrown if mesh
 * dimension of \a this is not equal to space dimension - 1.
 * \sa DataArrayDouble::distanceToTuple, MEDCouplingUMesh::distanceToPoints
 */
double
MEDCouplingUMesh::distanceToPoint(const double *ptBg, const double *ptEnd, mcIdType &cellId) const
{
    ...
    if (meshDim != 2 && meshDim != 1)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::distanceToPoint : only mesh dimension 2 and 1 are implemented !"
        );
    checkFullyDefined();
    if (ToIdType(std::distance(ptBg, ptEnd)) != spaceDim)
    {
        std::ostringstream oss;
        oss << "MEDCouplingUMesh::distanceToPoint : input point has to have dimension equal to the space dimension of "
               "this ("
            << spaceDim << ") !";
        throw INTERP_KERNEL::Exception(oss.str());
    }
    DataArrayIdType *ret1 = 0;
    MCAuto<DataArrayDouble> pts = DataArrayDouble::New();
    pts->useArray(ptBg, false, DeallocType::C_DEALLOC, 1, spaceDim);
...
/*!
 * This method computes the distance from each point of points serie \a pts (stored in a DataArrayDouble in which each
 * tuple represents a point) to \a this  and the first \a cellId in \a this corresponding to the returned distance.
 * WARNING, if there is some orphan nodes in \a this (nodes not fetched by any cells in \a this ( see
 * MEDCouplingUMesh::zipCoords ) ) these nodes will ** not ** been taken into account in this method. Only cells and
 * nodes lying on them are considered in the algorithm (even if one of these orphan nodes is closer than returned
 * distance). A user that needs to consider orphan nodes should invoke DataArrayDouble::minimalDistanceTo method on the
 ...
 * \a this is expected to be a mesh so that its space dimension is equal to its
 * mesh dimension + 1. Furthermore only mesh dimension 1 and 2 are supported for the moment.
 * Number of components of \a pts is expected to be equal to the space dimension. \a this is also expected to be fully
 * defined (connectivity and coordinates).
 *
 * So this method is more accurate (so, more costly) than simply searching for each point in \a pts the closest point in
 * \a this. If only this information is enough for you simply call \c getCoords()->distanceToTuple on \a this.
 *
 * \param [in] pts the list of points in which each tuple represents a point
 * \param [out] cellIds a newly allocated object that tells for each point in \a pts the first cell id in \a this that
 * minimizes the distance.
 * \return a newly allocated object to be dealed by the caller that tells for each point in \a pts the distance to \a
 * this.
 ...
 */
DataArrayDouble *
MEDCouplingUMesh::distanceToPoints(const DataArrayDouble *pts, DataArrayIdType *&cellIds) const
{
    if (!pts)
    ...
    if (meshDim != 2 && meshDim != 1)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::distanceToPoints : only mesh dimension 2 and 1 are implemented !"
        );
    if (ToIdType(pts->getNumberOfComponents()) != spaceDim)
    {
        std::ostringstream oss;
        oss << "MEDCouplingUMesh::distanceToPoints : input pts DataArrayDouble has " << pts->getNumberOfComponents()
            << " components whereas it should be equal to " << spaceDim << " (mesh spaceDimension) !";
        throw INTERP_KERNEL::Exception(oss.str());
    }
    checkFullyDefined();
    mcIdType nbCells = getNumberOfCells();
    ...
    ret0->alloc(nbOfPts, 1);
    MCAuto<DataArrayIdType> ret1 = DataArrayIdType::New();
    ret1->alloc(nbOfPts, 1);
    const mcIdType *nc = _nodal_connec->begin(), *ncI = _nodal_connec_index->begin();
    const double *coords = _coords->begin();
    double *ret0Ptr = ret0->getPointer();
    mcIdType *ret1Ptr = ret1->getPointer();
    const double *ptsPtr = pts->begin();
    ...
    switch (spaceDim)
    {
        case 3:
        {
            ...
            for (mcIdType i = 0; i < nbOfPts; i++, ret0Ptr++, ret1Ptr++, ptsPtr += 3)
            {
                ...
                DistanceToPoint3DSurfAlg(
                    ptsPtr, &elems[0], &elems[0] + elems.size(), coords, nc, ncI, *ret0Ptr, *ret1Ptr
                );
            }
            break;
        }
 ...
 * For speed reasons, the INTERP_KERNEL::NORM_QUAD4, INTERP_KERNEL::NORM_TRI6 and INTERP_KERNEL::NORM_QUAD8 cells are
 * considered as convex cells to detect if a point is IN or OUT. If it is not the case, please change their types to
 * INTERP_KERNEL::NORM_POLYGON or INTERP_KERNEL::NORM_QPOLYG before invoking this method.
 *
 ...
 *          faster.
 *  \param [in] pos - array of coordinates of the ball central point.
 *  \param [in] eps - ball radius (i.e. the precision) relative to max dimension along X, Y or Z of the the considered
 * cell bounding box.
 ...
 */
void
MEDCouplingUMesh::getCellsContainingPoint(const double *pos, double eps, std::vector<mcIdType> &elts) const
{
    MCAuto<DataArrayIdType> eltsUg, eltsIndexUg;
    getCellsContainingPoints(pos, 1, eps, eltsUg, eltsIndexUg);
    elts.clear();
    elts.insert(elts.end(), eltsUg->begin(), eltsUg->end());
}

void
MEDCouplingUMesh::getCellsContainingPointsZeAlg(
    const double *pos,
    mcIdType nbOfPoints,
    double eps,
    MCAuto<DataArrayIdType> &elts,
    MCAuto<DataArrayIdType> &eltsIndex,
    std::function<bool(INTERP_KERNEL::NormalizedCellType, mcIdType)> sensibilityTo2DQuadraticLinearCellsFunc
) const
{
    ...
    if (spaceDim == 3)
    {
        ...
        {
            const double *coords = _coords->getConstPointer();
            getCellsContainingPointsAlg<3>(
                coords, pos, nbOfPoints, eps, elts, eltsIndex, sensibilityTo2DQuadraticLinearCellsFunc
            );
        }
        else
            throw INTERP_KERNEL::Exception(
                "For spaceDim==3 only meshDim==3 implemented for getelementscontainingpoints !"
            );
    }
    else if (spaceDim == 2)
    {
        ...
        {
            const double *coords = _coords->getConstPointer();
            getCellsContainingPointsAlg<2>(
                coords, pos, nbOfPoints, eps, elts, eltsIndex, sensibilityTo2DQuadraticLinearCellsFunc
            );
        }
        else
            throw INTERP_KERNEL::Exception(
                "For spaceDim==2 only meshDim==2 implemented for getelementscontainingpoints !"
            );
    }
    else if (spaceDim == 1)
    {
        ...
        {
            const double *coords = _coords->getConstPointer();
            getCellsContainingPointsAlg<1>(
                coords, pos, nbOfPoints, eps, elts, eltsIndex, sensibilityTo2DQuadraticLinearCellsFunc
            );
        }
        else
            throw INTERP_KERNEL::Exception(
                "For spaceDim==1 only meshDim==1 implemented for getelementscontainingpoints !"
            );
    }
    else
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::getCellsContainingPoints : not managed for mdim not in [1,2,3] !"
        );
}
 ...
 */
void
MEDCouplingUMesh::getCellsContainingPoints(
    const double *pos,
    mcIdType nbOfPoints,
    double eps,
    MCAuto<DataArrayIdType> &elts,
...
{
    auto yesImSensibleTo2DQuadraticLinearCellsFunc(
        [](INTERP_KERNEL::NormalizedCellType ct, int mdim)
        { return INTERP_KERNEL::CellModel::GetCellModel(ct).isQuadratic() && mdim == 2; }
    );
    this->getCellsContainingPointsZeAlg(
        pos, nbOfPoints, eps, elts, eltsIndex, yesImSensibleTo2DQuadraticLinearCellsFunc
    );
}
 ...
 */
void
MEDCouplingUMesh::getCellsContainingPointsLinearPartOnlyOnNonDynType(
    const double *pos,
    mcIdType nbOfPoints,
    double eps,
    MCAuto<DataArrayIdType> &elts,
...
) const
{
    ...
    this->getCellsContainingPointsZeAlg(
        pos, nbOfPoints, eps, elts, eltsIndex, noImNotSensibleTo2DQuadraticLinearCellsFunc
    );
}
 ...
 */
void
MEDCouplingUMesh::checkButterflyCells(std::vector<mcIdType> &cells, double eps) const
{
    ...
    for (mcIdType i = 0; i < nbOfCells; i++)
    {
        ...
        if (nbOfNodesForCell <= 3)
            continue;
        bool isQuad =
            INTERP_KERNEL::CellModel::GetCellModel((INTERP_KERNEL::NormalizedCellType)conn[offset]).isQuadratic();
 ...
 *
 * This method expects that space dimension is equal to 2 and mesh dimension is equal to 2 too. If it is not the case an
 * INTERP_KERNEL::Exception will be thrown. This method works only for linear 2D cells. If there is any of non linear
 * cells (INTERP_KERNEL::NORM_QUAD8 for example) an INTERP_KERNEL::Exception will be thrown too.
 *
 ...
 * Only connectivity of some cells could be modified if those cells were not representing a convex envelop. If a cell
 * already equals its convex envelop (regardless orientation) its connectivity will remain unchanged. If the computation
 * leads to a modification of nodal connectivity of a cell its geometric type will be modified to
 * INTERP_KERNEL::NORM_POLYGON.
 *
 * \return a newly allocated array containing cellIds that have been modified if any. If no cells have been impacted by
    ...
    *workIndexOut = 0;
    ...
    for (mcIdType i = 0; i < nbOfCells; i++, workIndexOut++)
    {
        ...
        if (BuildConvexEnvelopOf2DCellJarvis(
                coords, nodalConnecIn + nodalConnecIndexIn[i], nodalConnecIn + nodalConnecIndexIn[i + 1], nodalConnecOut
            ))
 ...
 *   - \b 1 for translation and rotation: the translation is done as above. For each level, an arc of circle is fitted
 * on the 3 preceding points of the 1D mesh. The center of the arc is the center of rotation for each level, the
 * rotation is done along an axis normal to the plane containing the arc, and finally the angle of rotation is defined
 * by the first two points on the arc.
...
MEDCouplingUMesh::buildExtrudedMesh(const MEDCouplingUMesh *mesh1D, int policy)
{
    ...
    if (getSpaceDimension() != mesh1D->getSpaceDimension())
        throw INTERP_KERNEL::Exception(
            "Invalid call to buildExtrudedMesh this and mesh1D must have same space dimension !"
        );
    if ((getMeshDimension() != 2 || getSpaceDimension() != 3) && (getMeshDimension() != 1 || getSpaceDimension() != 2))
        throw INTERP_KERNEL::Exception(
            "Invalid 'this' for buildExtrudedMesh method : must be (meshDim==2 and spaceDim==3) or (meshDim==1 and "
            "spaceDim==2) !"
        );
    ...
    if (isPresenceOfQuadratic())
    {
        if (mesh1D->isFullyQuadratic())
            ...
            throw INTERP_KERNEL::Exception(
                "Invalid 2D mesh and 1D mesh because 2D mesh has quadratic cells and 1D is not fully quadratic !"
            );
    }
 ...
 * type of cells expected. For example INTERP_KERNEL::NORM_TRI3 can be converted to INTERP_KERNEL::NORM_TRI6 if \a
 * conversionType is equal to 0 (the default) or to INTERP_KERNEL::NORM_TRI7 if \a conversionType is equal to 1. All non
 * linear cells and polyhedron in \a this are let untouched. Contrary to MEDCouplingUMesh::convertQuadraticCellsToLinear
 * method, the coordinates in \a this can be become bigger. All created nodes will be put at the end of the existing
 * coordinates.
 *
 * \param [in] conversionType specifies the type of conversion expected. Only 0 (default) and 1 are supported presently.
 * 0 those that creates the 'most' simple corresponding quadratic cells. 1 is those creating the 'most' complex.
 * \return a newly created DataArrayIdType instance that the caller should deal with containing cell ids of converted
 * cells.
 *
 ...
 */
DataArrayIdType *
MEDCouplingUMesh::convertLinearCellsToQuadratic(int conversionType)
{
    ...
    switch (conversionType)
    {
        case 0:
            switch (meshDim)
            {
                ...
                default:
                    throw INTERP_KERNEL::Exception(
                        "MEDCouplingUMesh::convertLinearCellsToQuadratic : conversion of type 0 mesh dimensions "
                        "available are [1,2,3] !"
                    );
            }
            break;
        case 1:
        {
            ...
            {
                case 1:
                    ret = convertLinearCellsToQuadratic1D0(
                        conn, connI, coords, types
                    );  // it is not a bug. In 1D policy 0 and 1 are equals
                    connSafe = conn;
                    connISafe = connI;
                    coordsSafe = coords;
                    break;
                ...
                default:
                    throw INTERP_KERNEL::Exception(
                        "MEDCouplingUMesh::convertLinearCellsToQuadratic : conversion of type 1 mesh dimensions "
                        "available are [1,2,3] !"
                    );
            }
            break;
        ...
        default:
            throw INTERP_KERNEL::Exception(
                "MEDCouplingUMesh::convertLinearCellsToQuadratic : conversion type available are 0 (default, the "
                "simplest) and 1 (the most complex) !"
            );
    }
 ...
 */
MCAuto<MEDCouplingUMesh>
MEDCouplingUMesh::convertToQuadraticBasedOnSeg3(const MEDCoupling1SGTUMesh *seg3) const
{
    ...
    for (auto type : types)
    {
        ...
        if (nbNodePerCellQ != nbNodePerCell + nbEdgesPerCell)
            THROW_IK_EXCEPTION(
                "number of nodes of quadratic cell is supposed to be equal to nb of nodes of linear cell associated "
                "and number of edges !"
            );
...
MCAuto<MEDCouplingUMesh>
MEDCouplingUMeshExtrudeConnectivity(
    const MEDCouplingUMesh *instance, mcIdType nbOfCellsToExtrude, std::function<mcIdType(mcIdType)> func
)
{
    ...
    MCAuto<DataArrayIdType> o2nTmp(DataArrayIdType::New());
    o2nTmp->alloc(nbOfCells * nbOfCellsToExtrude, 1);
 ...
 * - INTERP_KERNEL::PLANAR_FACE_5 - to split HEXA8  into 5 TETRA4 (for 3D mesh only - see INTERP_KERNEL::SplittingPolicy
 * for an image).
 * - INTERP_KERNEL::PLANAR_FACE_6 - to split HEXA8  into 6 TETRA4 (for 3D mesh only - see INTERP_KERNEL::SplittingPolicy
 * for an image).
 *
 *
...
MEDCouplingUMesh::simplexize(int policy)
{
    switch (policy)
    {
        ...
        default:
            throw INTERP_KERNEL::Exception(
                "MEDCouplingUMesh::simplexize : unrecognized policy ! Must be :\n  - 0 or 1 (only available for "
...
MEDCouplingUMesh::areOnlySimplexCells() const
{
    ...
    if (mdim < 1 || mdim > 3)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::areOnlySimplexCells : only available with meshes having a meshdim 1, 2 or 3 !"
        );
    mcIdType nbCells = getNumberOfCells();
    ...
    {
        const INTERP_KERNEL::CellModel &cm =
            INTERP_KERNEL::CellModel::GetCellModel((INTERP_KERNEL::NormalizedCellType)conn[connI[i]]);
        if (!cm.isSimplex())
            return false;
    }
    return true;
...
MEDCouplingUMesh::convertDegeneratedCells()
{
    ...
    if (getMeshDimension() <= 1)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::convertDegeneratedCells works on umeshes with meshdim equals to 2 or 3 !"
        );
    mcIdType nbOfCells = getNumberOfCells();
    if (nbOfCells < 1)
        return;
    ...
    for (mcIdType i = 0; i < nbOfCells; i++)
    {
        ...
        INTERP_KERNEL::NormalizedCellType type = (INTERP_KERNEL::NormalizedCellType)conn[posOfCurCell];
        mcIdType newLgth;
        INTERP_KERNEL::NormalizedCellType newType = INTERP_KERNEL::CellSimplify::simplifyDegeneratedCell(
            type, conn + posOfCurCell + 1, lgthOfCurCell - 1, conn + newPos + 1, newLgth
        );
        conn[newPos] = newType;
        newPos += newLgth + 1;
        posOfCurCell = index[i + 1];
 ...
 */
DataArrayIdType *
MEDCouplingUMesh::convertDegeneratedCellsAndRemoveFlatOnes()
{
    checkFullyDefined();
    if (getMeshDimension() <= 1)
        throw INTERP_KERNEL::Exception(
 ...
 */
bool
MEDCouplingUMesh::removeDegenerated1DCells()
{
    checkConnectivityFullyDefined();
    if (getMeshDimension() != 1)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::removeDegenerated1DCells works on umeshes with meshdim equals to 1 !"
        );
    ...
    const mcIdType *conn(getNodalConnectivity()->begin()), *conni(getNodalConnectivityIndex()->begin());
    {
        for (std::size_t i = 0; i < nbCells; i++)
        {
            ...
            {
                std::ostringstream oss;
                oss << "MEDCouplingUMesh::removeDegenerated1DCells : cell #" << i
                    << " in this is not of type SEG2/SEG3 !";
                throw INTERP_KERNEL::Exception(oss.str());
            }
        }
    }
    if (newSize == nbCells)  // no cells has been removed -> do nothing
        return false;
    MCAuto<DataArrayIdType> newConn(DataArrayIdType::New()), newConnI(DataArrayIdType::New());
    newConnI->alloc(newSize + 1, 1);
    newConn->alloc(newSize2, 1);
    mcIdType *newConnPtr(newConn->getPointer()), *newConnIPtr(newConnI->getPointer());
    newConnIPtr[0] = 0;
    for (std::size_t i = 0; i < nbCells; i++)
    {
        if (conn[conni[i] + 1] != conn[conni[i] + 2])
 ...
 */
void
MEDCouplingUMesh::are2DCellsNotCorrectlyOriented(const double *vec, bool polyOnly, std::vector<mcIdType> &cells) const
{
    if (getMeshDimension() != 2 || getSpaceDimension() != 3)
        throw INTERP_KERNEL::Exception(
            "Invalid mesh to apply are2DCellsNotCorrectlyOriented on it : must be meshDim==2 and spaceDim==3 !"
        );
    mcIdType nbOfCells = getNumberOfCells();
    const mcIdType *conn = _nodal_connec->begin();
    const mcIdType *connI = _nodal_connec_index->begin();
...
MEDCouplingUMesh::orientCorrectly2DCells(const double *vec, bool polyOnly)
{
    if (getMeshDimension() != 2 || getSpaceDimension() != 3)
        throw INTERP_KERNEL::Exception(
            "Invalid mesh to apply orientCorrectly2DCells on it : must be meshDim==2 and spaceDim==3 !"
        );
    mcIdType nbOfCells = getNumberOfCells();
    mcIdType *conn(_nodal_connec->getPointer());
    const mcIdType *connI(_nodal_connec_index->begin());
...
/*!
 * This method change the orientation of cells in \a this without any consideration of coordinates. Only connectivity is
 * impacted.
 *
 * \sa orientCorrectly2DCells
 */
void
MEDCouplingUMesh::changeOrientationOfCells()
{
    int mdim(getMeshDimension());
    if (mdim != 2 && mdim != 1)
        throw INTERP_KERNEL::Exception(
            "Invalid mesh to apply changeOrientationOfCells on it : must be meshDim==2 or meshDim==1 !"
        );
    mcIdType nbOfCells = getNumberOfCells();
    mcIdType *conn(_nodal_connec->getPointer());
    const mcIdType *connI(_nodal_connec_index->begin());
 ...
 */
void
MEDCouplingUMesh::arePolyhedronsNotCorrectlyOriented(std::vector<mcIdType> &cells) const
{
    if (getMeshDimension() != 3 || getSpaceDimension() != 3)
        throw INTERP_KERNEL::Exception(
            "Invalid mesh to apply arePolyhedronsNotCorrectlyOriented on it : must be meshDim==3 and spaceDim==3 !"
        );
    mcIdType nbOfCells = getNumberOfCells();
    const mcIdType *conn = _nodal_connec->begin();
    const mcIdType *connI = _nodal_connec_index->begin();
...
MEDCouplingUMesh::orientCorrectlyPolyhedrons()
{
    if (getMeshDimension() != 3 || getSpaceDimension() != 3)
        throw INTERP_KERNEL::Exception(
            "Invalid mesh to apply orientCorrectlyPolyhedrons on it : must be meshDim==3 and spaceDim==3 !"
        );
    mcIdType nbOfCells = getNumberOfCells();
    mcIdType *conn = _nodal_connec->getPointer();
    const mcIdType *connI = _nodal_connec_index->begin();
    ...
    for (mcIdType i = 0; i < nbOfCells; i++)
    {
        ...
        if (type == INTERP_KERNEL::NORM_POLYHED)
        {
            ...
            {
                std::ostringstream oss;
                oss << "Something wrong in polyhedron #" << i << " : " << e.what();
                throw INTERP_KERNEL::Exception(oss.str());
            }
        }
    }
 ...
 */
void
MEDCouplingUMesh::invertOrientationOfAllCells()
{
    checkConnectivityFullyDefined();
    ...
    {
        INTERP_KERNEL::AutoCppPtr<INTERP_KERNEL::OrientationInverter> oi(
            INTERP_KERNEL::OrientationInverter::BuildInstanceFrom(*gt)
        );
...
MEDCouplingUMesh::findAndCorrectBadOriented3DExtrudedCells()
{
    ...
    MCAuto<DataArrayIdType> cells(DataArrayIdType::New());
    cells->alloc(0, 1);
    for (mcIdType i = 0; i < nbOfCells; i++)
    {
        const INTERP_KERNEL::CellModel &cm =
 ...
 * This method makes the hypothesis that \a this a coherent that is to say MEDCouplingUMesh::checkConsistency should
 * throw no exception.
 *
 * \return a newly allocated mcIdType array with one components containing cell ids renumbered to fit the convention of
 * MED (MED file and MEDCoupling)
...
MEDCouplingUMesh::findAndCorrectBadOriented3DCells()
{
    if (getMeshDimension() != 3 || getSpaceDimension() != 3)
        throw INTERP_KERNEL::Exception(
            "Invalid mesh to apply findAndCorrectBadOriented3DCells on it : must be meshDim==3 and spaceDim==3 !"
        );
    mcIdType nbOfCells = getNumberOfCells();
    mcIdType *conn = _nodal_connec->getPointer();
    const mcIdType *connI = _nodal_connec_index->begin();
    ...
    for (mcIdType i = 0; i < nbOfCells; i++)
    {
        ...
        switch (type)
        {
            ...
            default:
                throw INTERP_KERNEL::Exception(
                    "MEDCouplingUMesh::orientCorrectly3DCells : Your mesh contains type of cell not supported yet ! "
                    "send mail to anthony.geay@cea.fr to add it !"
                );
        }
    }
    updateTime();
...
MEDCouplingUMesh::getFastAveragePlaneOfThis(double *vec, double *pos) const
{
    if (getMeshDimension() != 2 || getSpaceDimension() != 3)
        throw INTERP_KERNEL::Exception(
            "Invalid mesh to apply getFastAveragePlaneOfThis on it : must be meshDim==2 and spaceDim==3 !"
        );
    const mcIdType *conn = _nodal_connec->begin();
    const mcIdType *connI = _nodal_connec_index->begin();
    const double *coordsPtr = _coords->begin();
    INTERP_KERNEL::areaVectorOfPolygon<mcIdType, INTERP_KERNEL::ALL_C_MODE>(
        conn + 1, connI[1] - connI[0] - 1, coordsPtr, vec
    );
...
MEDCouplingUMesh::getEdgeRatioField() const
{
    ...
    if (spaceDim != 2 && spaceDim != 3)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::getEdgeRatioField : SpaceDimension must be equal to 2 or 3 !"
        );
    ...
    for (mcIdType i = 0; i < nbOfCells; i++, pt++)
    {
        ...
        switch (t)
        {
            ...
            default:
                throw INTERP_KERNEL::Exception(
                    "MEDCouplingUMesh::getEdgeRatioField : A cell with not manged type (NORM_TRI3, NORM_QUAD4 and "
                    "NORM_TETRA4) has been detected !"
                );
        }
...
MEDCouplingUMesh::getAspectRatioField() const
{
    ...
    if (spaceDim != 2 && spaceDim != 3)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::getAspectRatioField : SpaceDimension must be equal to 2 or 3 !"
        );
    if (meshDim != 2 && meshDim != 3)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::getAspectRatioField : MeshDimension must be equal to 2 or 3 !"
        );
    MCAuto<MEDCouplingFieldDouble> ret = MEDCouplingFieldDouble::New(ON_CELLS, ONE_TIME);
    ret->setMesh(this);
    mcIdType nbOfCells = getNumberOfCells();
    ...
    for (mcIdType i = 0; i < nbOfCells; i++, pt++)
    {
        ...
        switch (t)
        {
            ...
            default:
                throw INTERP_KERNEL::Exception(
                    "MEDCouplingUMesh::getAspectRatioField : A cell with not manged type (NORM_TRI3, NORM_QUAD4 and "
                    "NORM_TETRA4) has been detected !"
                );
        }
        conn += connI[i + 1] - connI[i];
...
MEDCouplingUMesh::getWarpField() const
{
    ...
    for (mcIdType i = 0; i < nbOfCells; i++, pt++)
    {
        ...
        switch (t)
        {
            ...
            default:
                throw INTERP_KERNEL::Exception(
                    "MEDCouplingUMesh::getWarpField : A cell with not manged type (NORM_QUAD4) has been detected !"
                );
        }
        conn += connI[i + 1] - connI[i];
    }
...
MEDCouplingUMesh::getSkewField() const
{
    ...
    for (mcIdType i = 0; i < nbOfCells; i++, pt++)
    {
        ...
        switch (t)
        {
            ...
            default:
                throw INTERP_KERNEL::Exception(
                    "MEDCouplingUMesh::getSkewField : A cell with not manged type (NORM_QUAD4) has been detected !"
                );
        }
        conn += connI[i + 1] - connI[i];
    }
...
/*!
 * Returns the cell field giving for each cell in \a this its diameter. Diameter means the max length of all possible
 * SEG2 in the cell.
 *
 ...
 */
MEDCouplingFieldDouble *
MEDCouplingUMesh::computeDiameterField() const
{
    checkConsistencyLight();
    MCAuto<MEDCouplingFieldDouble> ret(MEDCouplingFieldDouble::New(ON_CELLS, ONE_TIME));
    ret->setMesh(this);
    ...
    {
        INTERP_KERNEL::AutoCppPtr<INTERP_KERNEL::DiameterCalculator> dc(
            INTERP_KERNEL::CellModel::GetCellModel(*it).buildInstanceOfDiameterCalulator(spaceDim)
        );
        MCAuto<DataArrayIdType> cellIds(giveCellsWithType(*it));
        dc->computeForListOfCellIdsUMeshFrmt(
            cellIds->begin(),
            cellIds->end(),
            _nodal_connec_index->begin(),
            _nodal_connec->begin(),
            getCoords()->begin(),
            arr->getPointer()
        );
 ...
 * linear and arc circle. (By default 1e-12) For all other cases this input parameter is ignored.
 * \return DataArrayDouble * - newly created object (to be managed by the caller) \a this number of cells tuples and
 * 2*spacedim components.
 *
 ...
 */
DataArrayDouble *
MEDCouplingUMesh::getBoundingBoxForBBTree(double arcDetEps) const
{
    int mDim(getMeshDimension()), sDim(getSpaceDimension());
    if ((mDim == 3 && sDim == 3) || (mDim == 2 && sDim == 3) || (mDim == 1 && sDim == 1) ||
        (mDim == 1 && sDim == 3))  // Compute refined boundary box for quadratic elements only in 2D.
    ...
    throw INTERP_KERNEL::Exception(
        "MEDCouplingUMesh::getBoundingBoxForBBTree : Managed dimensions are (mDim=1,sDim=1), (mDim=1,sDim=2), "
        "(mDim=1,sDim=3), (mDim=2,sDim=2), (mDim=2,sDim=3) and (mDim=3,sDim=3) !"
    );
}

/*!
 * This method aggregate the bbox of each cell only considering the nodes constituting each cell and put it into bbox
 * parameter. So meshes having quadratic cells the computed bounding boxes can be invalid !
 *
 * \return DataArrayDouble * - newly created object (to be managed by the caller) \a this number of cells tuples and
 * 2*spacedim components.
 *
 ...
 */
DataArrayDouble *
MEDCouplingUMesh::getBoundingBoxForBBTreeFast() const
{
    checkFullyDefined();
    int spaceDim(getSpaceDimension());
    mcIdType nbOfCells(getNumberOfCells()), nbOfNodes(getNumberOfNodes());
    MCAuto<DataArrayDouble> ret(DataArrayDouble::New());
    ret->alloc(nbOfCells, 2 * spaceDim);
    ...
    for (mcIdType i = 0; i < nbOfCells; i++)
    {
        ...
        for (mcIdType j = 0; j < nbOfNodesForCell; j++)
        {
            ...
            if (nodeId >= 0 && nodeId < nbOfNodes)
            {
                ...
                {
                    bbox[2 * spaceDim * i + 2 * k] =
                        std::min(bbox[2 * spaceDim * i + 2 * k], coordsPtr[spaceDim * nodeId + k]);
                    bbox[2 * spaceDim * i + 2 * k + 1] =
                        std::max(bbox[2 * spaceDim * i + 2 * k + 1], coordsPtr[spaceDim * nodeId + k]);
                }
                kk++;
            }
        }
        ...
        {
            std::ostringstream oss;
            oss << "MEDCouplingUMesh::getBoundingBoxForBBTree : cell #" << i << " contains no valid nodeId !";
            throw INTERP_KERNEL::Exception(oss.str());
        }
    }
    return ret.retn();
 ...
 * because for this type of cells getBoundingBoxForBBTreeFast method may return invalid bounding boxes (since it just
 * considers the two extremities of the arc of circle).
 *
 * \param [in] arcDetEps - a parameter specifying in case of 2D quadratic polygon cell the detection limit between
 * linear and arc circle. (By default 1e-12)
 * \return DataArrayDouble * - newly created object (to be managed by the caller) \a this number of cells tuples and
 * 2*spacedim components.
 ...
 */
DataArrayDouble *
MEDCouplingUMesh::getBoundingBoxForBBTree2DQuadratic(double arcDetEps) const
{
    checkFullyDefined();
    ...
    if (spaceDim != 2 || mDim != 2)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::getBoundingBoxForBBTree2DQuadratic : This method should be applied on mesh with mesh "
            "dimension equal to 2 and space dimension also equal to 2!"
        );
    MCAuto<DataArrayDouble> ret(DataArrayDouble::New());
    ret->alloc(nbOfCells, 2 * spaceDim);
    double *bbox(ret->getPointer());
 ...
 */
DataArrayDouble *
MEDCouplingUMesh::getBoundingBoxForBBTree1DQuadratic(double arcDetEps) const
{
    checkFullyDefined();
    int spaceDim(getSpaceDimension()), mDim(getMeshDimension());
    mcIdType nbOfCells = getNumberOfCells();
    if (spaceDim != 2 || mDim != 1)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::getBoundingBoxForBBTree1DQuadratic : This method should be applied on mesh with mesh "
            "dimension equal to 1 and space dimension also equal to 2!"
        );
    INTERP_KERNEL::QuadraticPlanarPrecision arcPrec(arcDetEps);
    MCAuto<DataArrayDouble> ret(DataArrayDouble::New());
    ret->alloc(nbOfCells, 2 * spaceDim);
 ...
 * This method returns in the same format as code (see MEDCouplingUMesh::checkTypeConsistencyAndContig or
 * MEDCouplingUMesh::splitProfilePerType) how
...
MEDCouplingUMesh::getDistributionOfTypes() const
{
    ...
    for (std::size_t i = 0; work != connI + nbOfCells; i++)
    {
        ...
        {
            std::ostringstream oss;
            oss << "MEDCouplingUMesh::getDistributionOfTypes : Type "
                << INTERP_KERNEL::CellModel::GetCellModel(typ).getRepr();
...
) const
{
    ...
    for (std::size_t i = 0; i < n; i++)
        if (std::find(types.begin(), types.end(), (INTERP_KERNEL::NormalizedCellType)code[3 * i]) == types.end())
        {
            ...
            if (_types.find((INTERP_KERNEL::NormalizedCellType)code[3 * i]) == _types.end())
                throw INTERP_KERNEL::Exception(
                    "MEDCouplingUMesh::checkTypeConsistencyAndContig : expected geo types not in this !"
                );
            isNoPflUsed = isNoPflUsed && (code[3 * i + 2] == -1);
        }
    if (types.size() != n)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::checkTypeConsistencyAndContig : code contains duplication of types in unstructured mesh "
            "!"
        );
    if (isNoPflUsed)
    {
    ...
    int kk = 0;
    for (std::vector<INTERP_KERNEL::NormalizedCellType>::const_iterator it = types.begin(); it != types.end();
         it++, kk++)
    {
        ...
        if (code[3 * kk + 2] == -1)
            for (mcIdType k = 0; k < nbOfCellsOfCurType; k++) *retPtr++ = k + offset;
        else
        {
                ...
                if (zePfl)
                {
                    ...
                    if (zePfl->getNumberOfComponents() == 1)
                    {
                        for (const mcIdType *k = zePfl->begin(); k != zePfl->end(); k++, retPtr++)
                        {
                            ...
                            {
                                std::ostringstream oss;
                                oss << "MEDCouplingUMesh::checkTypeConsistencyAndContig : the section " << kk
                                    << " points to the profile #" << idInIdsPerType;
                                oss << ", and this profile contains a value " << *k << " should be in [0,"
                                    << nbOfCellsOfCurType << ") !";
                                throw INTERP_KERNEL::Exception(oss.str());
                            }
                        }
                        ...
                        throw INTERP_KERNEL::Exception(
                            "MEDCouplingUMesh::checkTypeConsistencyAndContig : presence of a profile with nb of compo "
                            "!= 1 !"
                        );
                }
                else
                    throw INTERP_KERNEL::Exception(
                        "MEDCouplingUMesh::checkTypeConsistencyAndContig : presence of null profile !"
                    );
            }
            else
            {
                std::ostringstream oss;
                oss << "MEDCouplingUMesh::checkTypeConsistencyAndContig : at section " << kk
                    << " of code it points to the array #" << idInIdsPerType;
 ...
 * This method is the opposite of MEDCouplingUMesh::checkTypeConsistencyAndContig method. Given a list of cells in \a
 * profile it returns a list of sub-profiles sorted by geo type. The result is put in the array \a idsPerType. In the
 * returned parameter \a code, foreach i \a code[3*i+2] refers (if different from -1) to a location into the \a
 * idsPerType. This method has 1 input \a profile and 3 outputs \a code \a idsInPflPerType and \a idsPerType.
 *
 * \param [in] profile list of IDs constituing the profile
 * \param [out] code is a vector of size 3*n where n is the number of different geometric type in \a this \b reduced to
 * the profile \a profile. \a code has exactly the same semantic than in MEDCouplingUMesh::checkTypeConsistencyAndContig
 * method.
 * \param [out] idsInPflPerType is a vector of size of different geometric type in the subpart defined by \a profile of
 * \a this ( equal to \a code.size()/3). For each i,
 *              \a idsInPflPerType[i] stores the tuple ids in \a profile that correspond to the geometric type
 * code[3*i+0]
 * \param [out] idsPerType is a vector of size of different sub profiles needed to be defined to represent the profile
 * \a profile for a given geometric type. This vector can be empty in case of all geometric type cells are fully covered
 * in ascending in the given input \a profile.
 * \throw if \a profile has not exactly one component. It throws too, if \a profile contains some values not in
 * [0,getNumberOfCells()) or if \a this is not fully defined
 */
void
MEDCouplingUMesh::splitProfilePerType(
...
) const
{
    ...
    if (profile->getNumberOfComponents() != 1)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::splitProfilePerType : input profile should have exactly one component !"
        );
    checkConnectivityFullyDefined();
    const mcIdType *conn = _nodal_connec->begin();
    const mcIdType *connI = _nodal_connec_index->begin();
    ...
    for (const mcIdType *i = connI; i != connI + nbOfCells;)
    {
        ...
        {
            throw INTERP_KERNEL::Exception(
                "MEDCouplingUMesh::splitProfilePerType : current mesh is not sorted by type !"
            );
        }
    ...
    profile->splitByValueRange(
        &typeRangeVals[0], &typeRangeVals[0] + typeRangeVals.size(), castArr, rankInsideCast, castsPresent
    );
    ...
    for (mcIdType i = 0; i < nbOfCastsFinal; i++)
    {
        ...
        MCAuto<DataArrayIdType> tmp4 =
            rankInsideCast->selectByTupleId(tmp3->begin(), tmp3->begin() + tmp3->getNumberOfTuples());
...
/*!
 * This method is here too emulate the MEDMEM behaviour on BDC (buildDescendingConnectivity). Hoping this method becomes
 * deprecated very soon. This method make the assumption that \a this and 'nM1LevMesh' mesh lyies on same coords (same
 * pointer) as MED and MEDMEM does. The following equality should be verified
 * 'nM1LevMesh->getMeshDimension()==this->getMeshDimension()-1' This method returns 5+2 elements. 'desc', 'descIndx',
 * 'revDesc', 'revDescIndx' and 'meshnM1' behaves exactly as MEDCoupling::MEDCouplingUMesh::buildDescendingConnectivity
 * except the content as described after. The returned array specifies the n-1 mesh reordered by type as MEDMEM does.
 * 'nM1LevMeshIds' contains the ids in returned 'meshnM1'. Finally 'meshnM1Old2New' contains numbering old2new that is
 * to say the cell #k in coarse 'nM1LevMesh' will have the number ret[k] in returned mesh 'nM1LevMesh' MEDMEM reordered.
 */
MEDCouplingUMesh *
MEDCouplingUMesh::emulateMEDMEMBDC(
    const MEDCouplingUMesh *nM1LevMesh,
...
) const
{
    ...
    if (getMeshDimension() - 1 != nM1LevMesh->getMeshDimension())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::emulateMEDMEMBDC : The mesh passed as first argument should have a meshDim equal to "
            "this->getMeshDimension()-1 !"
        );
    if (_coords != nM1LevMesh->getCoords())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::emulateMEDMEMBDC : 'this' and mesh in first argument should share the same coords : Use "
            "tryToShareSameCoords method !"
        );
    ...
    if (!ret1->areCellsIncludedIn(nM1LevMesh, 2, ret))
    {
        ...
        ret->decrRef();
        std::ostringstream oss;
        oss << "MEDCouplingUMesh::emulateMEDMEMBDC : input N-1 mesh present a cell not in descending mesh ... Id of "
               "cell is "
            << tmp2 << " !";
        throw INTERP_KERNEL::Exception(oss.str());
    }
...
/*!
 * This method performs the same job as checkConsecutiveCellTypes except that the order of types sequence is analyzed to
 * check that the order is specified in array defined by [ \a orderBg , \a orderEnd ). If there is some geo types in \a
 * this \b NOT in [ \a orderBg, \a orderEnd ) it is OK (return true) if contiguous. If there is some geo types in [ \a
 * orderBg, \a orderEnd ) \b NOT in \a this it is OK too (return true) if contiguous.
 */
bool
MEDCouplingUMesh::checkConsecutiveCellTypesAndOrder(
    const INTERP_KERNEL::NormalizedCellType *orderBg, const INTERP_KERNEL::NormalizedCellType *orderEnd
) const
{
    checkFullyDefined();
    const mcIdType *conn = _nodal_connec->begin();
...
/*!
 * This method returns 2 newly allocated DataArrayIdType instances. The first is an array of size
 * 'this->getNumberOfCells()' with one component, that tells for each cell the pos of its type in the array on type
 * given in input parameter. The 2nd output parameter is an array with the same number of tuples than input type array
 * and with one component. This 2nd output array gives type by type the number of occurrence of type in 'this'.
 */
DataArrayIdType *
MEDCouplingUMesh::getLevArrPerCellTypes(
    const INTERP_KERNEL::NormalizedCellType *orderBg,
...
) const
{
    ...
    {
        const INTERP_KERNEL::NormalizedCellType *where =
            std::find(orderBg, orderEnd, (INTERP_KERNEL::NormalizedCellType)conn[*i]);
        if (where != orderEnd)
        {
        ...
        {
            const INTERP_KERNEL::CellModel &cm =
                INTERP_KERNEL::CellModel::GetCellModel((INTERP_KERNEL::NormalizedCellType)conn[*i]);
            std::ostringstream oss;
            oss << "MEDCouplingUMesh::getLevArrPerCellTypes : Cell #" << std::distance(connI, i);
...
/*!
 * This method behaves exactly as MEDCouplingUMesh::getRenumArrForConsecutiveCellTypesSpec but the order is those
 * defined in MED file spec.
 *
 ...
 */
DataArrayIdType *
MEDCouplingUMesh::getRenumArrForMEDFileFrmt() const
{
    return getRenumArrForConsecutiveCellTypesSpec(MEDMEM_ORDER, MEDMEM_ORDER + N_MEDMEM_ORDER);
}

/*!
 * This method is similar to method MEDCouplingUMesh::rearrange2ConsecutiveCellTypes except that the type order is
 * specified by [ \a orderBg , \a orderEnd ) (as MEDCouplingUMesh::checkConsecutiveCellTypesAndOrder method) and that
 * this method is \b const and performs \b NO permutation in \a this. This method returns an array of size
 * getNumberOfCells() that gives a renumber array old2New that can be used as input of MEDCouplingMesh::renumberCells.
 * The mesh after this call to MEDCouplingMesh::renumberCells will pass the test of
 * MEDCouplingUMesh::checkConsecutiveCellTypesAndOrder with the same inputs. The returned array minimizes the
 * permutations that is to say the order of cells inside same geometric type remains the same.
 */
DataArrayIdType *
MEDCouplingUMesh::getRenumArrForConsecutiveCellTypesSpec(
    const INTERP_KERNEL::NormalizedCellType *orderBg, const INTERP_KERNEL::NormalizedCellType *orderEnd
) const
{
...
MEDCouplingUMesh::rearrange2ConsecutiveCellTypes()
{
    ...
    for (std::vector<INTERP_KERNEL::NormalizedCellType>::const_iterator iter = types.begin(); iter != types.end();
         iter++)
    {
        for (const mcIdType *i = connI; i != connI + nbOfCells; i++)
...
MEDCouplingUMesh::splitByType() const
{
    ...
    for (const mcIdType *i = connI; i != connI + nbOfCells;)
    {
        ...
        for (mcIdType j = 0; j < sz; j++) cells[j] = beginCellId + j;
 ...
 * MEDCoupling1GTUMesh instance. The returned instance will aggregate the same DataArrayDouble instance of coordinates
 * than \a this.
 *
 ...
 */
MEDCoupling1GTUMesh *
MEDCouplingUMesh::convertIntoSingleGeoTypeMesh() const
{
    checkConnectivityFullyDefined();
    if (_types.size() != 1)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::convertIntoSingleGeoTypeMesh : current mesh does not contain exactly one geometric type "
            "!"
        );
...
DataArrayIdType *
MEDCouplingUMesh::convertNodalConnectivityToStaticGeoTypeMesh() const
{
    checkConnectivityFullyDefined();
    if (_types.size() != 1)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::convertNodalConnectivityToStaticGeoTypeMesh : current mesh does not contain exactly one "
            "geometric type !"
        );
    INTERP_KERNEL::NormalizedCellType typ = *_types.begin();
    ...
    {
        std::ostringstream oss;
        oss << "MEDCouplingUMesh::convertNodalConnectivityToStaticGeoTypeMesh : this contains a single geo type ("
            << cm.getRepr() << ") but ";
        oss << "this type is dynamic ! Only static geometric type is possible for that type ! call "
               "convertNodalConnectivityToDynamicGeoTypeMesh instead !";
        throw INTERP_KERNEL::Exception(oss.str());
    }
    mcIdType nbCells = getNumberOfCells();
    mcIdType typi = ToIdType(typ);
    mcIdType nbNodesPerCell = ToIdType(cm.getNumberOfNodes());
    MCAuto<DataArrayIdType> connOut = DataArrayIdType::New();
    connOut->alloc(nbCells * nbNodesPerCell, 1);
    ...
    for (mcIdType i = 0; i < nbCells; i++, connI++)
    {
        ...
        {
            std::ostringstream oss;
            oss << "MEDCouplingUMesh::convertNodalConnectivityToStaticGeoTypeMesh : there something wrong in cell #"
                << i
                << " ! The type of cell is not those expected, or the length of nodal connectivity is not those "
                   "expected ("
                << nbNodesPerCell - 1 << ") !";
            throw INTERP_KERNEL::Exception(oss.str());
        }
 ...
 */
void
MEDCouplingUMesh::convertNodalConnectivityToDynamicGeoTypeMesh(
    DataArrayIdType *&nodalConn, DataArrayIdType *&nodalConnIndex
) const
{
    static const char msg0[] =
        "MEDCouplingUMesh::convertNodalConnectivityToDynamicGeoTypeMesh : nodal connectivity in this are invalid ! "
        "Call checkConsistency !";
    checkConnectivityFullyDefined();
    if (_types.size() != 1)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::convertNodalConnectivityToDynamicGeoTypeMesh : current mesh does not contain exactly "
            "one geometric type !"
        );
    mcIdType nbCells = getNumberOfCells(), lgth = _nodal_connec->getNumberOfTuples();
    if (lgth < nbCells)
        throw INTERP_KERNEL::Exception(msg0);
    MCAuto<DataArrayIdType> c(DataArrayIdType::New()), ci(DataArrayIdType::New());
    c->alloc(lgth - nbCells, 1);
    ci->alloc(nbCells + 1, 1);
    ...
    nodalConn = c.retn();
    nodalConnIndex = ci.retn();
}

/*!
 * This method takes in input a vector of MEDCouplingUMesh instances lying on the same coordinates with same mesh
 * dimensions. Each mesh in \b ms must be sorted by type with the same order (typically using
 * MEDCouplingUMesh::sortCellsInMEDFileFrmt). This method is particularly useful for MED file interaction. It allows to
 * aggregate several meshes and keeping the type sorting and the track of the permutation by chunk of same geotype cells
 * to retrieve it. The traditional formats old2new and new2old are not used here to avoid the build of big permutation
 * array.
 *
 * \param [in] ms meshes with same mesh dimension lying on the same coords and sorted by type following de the same
 * geometric type order than those specified in MEDCouplingUMesh::sortCellsInMEDFileFrmt method.
 * \param [out] szOfCellGrpOfSameType is a newly allocated DataArrayIdType instance whose number of tuples is equal to
 * the number of chunks of same geotype in all meshes in \b ms. The accumulation of all values of this array is equal to
 * the number of cells of returned mesh.
 * \param [out] idInMsOfCellGrpOfSameType is a newly allocated DataArrayIdType instance having the same size than \b
 * szOfCellGrpOfSameType. This output array gives for each chunck of same type the corresponding mesh id in \b ms.
 * \return A newly allocated unstructured mesh that is the result of the aggregation on same coords of all meshes in \b
 * ms. This returned mesh is sorted by type following the geo cell types order of
 * MEDCouplingUMesh::sortCellsInMEDFileFrmt method.
 */
MEDCouplingUMesh *
MEDCouplingUMesh::AggregateSortedByTypeMeshesOnSameCoords(
...
)
{
    ...
    if (ms2.empty())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::AggregateSortedByTypeMeshesOnSameCoords : input vector is empty !"
        );
    ...
    for (std::vector<const MEDCouplingUMesh *>::const_iterator it = ms2.begin(); it != ms2.end(); it++, rk++)
    {
        if (meshDim != (*it)->getMeshDimension())
            throw INTERP_KERNEL::Exception(
                "MEDCouplingUMesh::AggregateSortedByTypeMeshesOnSameCoords : meshdims mismatch !"
            );
        if (refCoo != (*it)->getCoords())
            throw INTERP_KERNEL::Exception(
                "MEDCouplingUMesh::AggregateSortedByTypeMeshesOnSameCoords : meshes are not shared by a single "
                "coordinates coords !"
            );
        ...
        {
            MEDCouplingUMesh *singleCell =
                static_cast<MEDCouplingUMesh *>((*it2)->buildPartOfMySelf(&fake, &fake + 1, true));
            m1ssmSingleAuto.push_back(singleCell);
            m1ssmSingle.push_back(singleCell);
            ret1->pushBackSilent((*it2)->getNumberOfCells());
            ret2->pushBackSilent(rk);
        }
    }
    ...
    for (mcIdType i = 0; i < ToIdType(m1ssm.size()); i++) m1ssmfinal[renum->getIJ(i, 0)] = m1ssm[i];
 ...
 */
DataArrayIdType *
MEDCouplingUMesh::keepCellIdsByType(
    INTERP_KERNEL::NormalizedCellType type, const mcIdType *begin, const mcIdType *end
) const
{
    checkFullyDefined();
    const mcIdType *conn = _nodal_connec->begin();
...
/*!
 * This method makes the assumption that da->getNumberOfTuples()<this->getNumberOfCells(). This method makes the
 * assumption that ids contained in 'da' are in [0:getNumberOfCells())
 */
DataArrayIdType *
MEDCouplingUMesh::convertCellArrayPerGeoType(const DataArrayIdType *da) const
{
    checkFullyDefined();
    const mcIdType *conn = _nodal_connec->begin();
    const mcIdType *connI = _nodal_connec_index->begin();
    ...
    for (mcIdType k = 0; k < nbOfElems; k++) retPtr[k] = tmp[daPtr[k]];
    delete[] tmp;
    return ret;
}
 ...
 */
MEDCouplingUMesh *
MEDCouplingUMesh::keepSpecifiedCells(
    INTERP_KERNEL::NormalizedCellType type, const mcIdType *idsPerGeoTypeBg, const mcIdType *idsPerGeoTypeEnd
) const
{
    ...
    for (const mcIdType *work = idsPerGeoTypeBg; work != idsPerGeoTypeEnd; work++)
        ...
        {
            std::ostringstream oss;
            oss << "MEDCouplingUMesh::keepSpecifiedCells : Request on type " << type << " at place #"
                << std::distance(idsPerGeoTypeBg, work) << " value " << *work;
            oss << ". It should be in [0," << szOfType << ") !";
            throw INTERP_KERNEL::Exception(oss.str());
        }
    MCAuto<DataArrayIdType> idsTokeep = DataArrayIdType::New();
    idsTokeep->alloc(sz + std::distance(idsPerGeoTypeBg, idsPerGeoTypeEnd), 1);
    mcIdType *idsPtr = idsTokeep->getPointer();
    mcIdType offset = 0;
    for (std::size_t i = 0; i < nOfTypesInThis; i++)
    {
        if (code[3 * i] != type)
            for (mcIdType j = 0; j < code[3 * i + 1]; j++) *idsPtr++ = offset + j;
        else
            idsPtr = std::transform(
                idsPerGeoTypeBg,
                idsPerGeoTypeEnd,
                idsPtr,
                std::bind(std::plus<mcIdType>(), std::placeholders::_1, offset)
            );
        offset += code[3 * i + 1];
    }
    MCAuto<MEDCouplingUMesh> ret =
        static_cast<MEDCouplingUMesh *>(buildPartOfMySelf(idsTokeep->begin(), idsTokeep->end(), true));
    ret->copyTinyInfoFrom(this);
    return ret.retn();
}
 ...
 * This happens with a default detection precision of eps=1.0e-14. If you need control over this use
 ...
 */
DataArrayDouble *
MEDCouplingUMesh::computeCellCenterOfMass() const
{
    MCAuto<DataArrayDouble> ret = DataArrayDouble::New();
    int spaceDim = getSpaceDimension();
    mcIdType nbOfCells = getNumberOfCells();
    ...
    for (mcIdType i = 0; i < nbOfCells; i++)
    {
        ...
        INTERP_KERNEL::computeBarycenter2<mcIdType, INTERP_KERNEL::ALL_C_MODE>(
            type, nodal + nodalI[i] + 1, nodalI[i + 1] - nodalI[i] - 1, coor, spaceDim, ptToFill
        );
        ptToFill += spaceDim;
    }
    return ret.retn();
 ...
 */
DataArrayDouble *
MEDCouplingUMesh::computeCellCenterOfMassWithPrecision(double eps) const
{
 ...
 * the cell. Contrary to badly named MEDCouplingUMesh::computeCellCenterOfMass method that returns the center of inertia
 * of the
 *
 * \return a newly allocated DataArrayDouble instance that the caller has to deal with. The returned
 *          DataArrayDouble instance will have \c this->getNumberOfCells() tuples and \c this->getSpaceDimension()
 * components.
 *
        ...
        if (type != INTERP_KERNEL::NORM_POLYHED)
        {
            for (const mcIdType *conn = nodal + nodalI[i] + 1; conn != nodal + nodalI[i + 1]; conn++)
            {
                if (*conn >= 0 && *conn < nbOfNodes)
                    std::transform(
                        coor + spaceDim * conn[0],
                        coor + spaceDim * (conn[0] + 1),
                        ptToFill,
                        ptToFill,
                        std::plus<double>()
                    );
                ...
                {
                    std::ostringstream oss;
                    oss << "MEDCouplingUMesh::computeIsoBarycenterOfNodesPerCell : on cell #" << i
                        << " presence of nodeId #" << *conn << " should be in [0," << nbOfNodes << ") !";
                    throw INTERP_KERNEL::Exception(oss.str());
                }
            }
            ...
            if (nbOfNodesInCell > 0)
                std::transform(
                    ...
                    ptToFill + spaceDim,
                    ptToFill,
                    std::bind(std::multiplies<double>(), std::placeholders::_1, 1. / (double)nbOfNodesInCell)
                );
            else
            {
                std::ostringstream oss;
                oss << "MEDCouplingUMesh::computeIsoBarycenterOfNodesPerCell : on cell #" << i
                    << " presence of cell with no nodes !";
                throw INTERP_KERNEL::Exception(oss.str());
            }
        }
        else
        {
            ...
            for (std::set<mcIdType>::const_iterator it = s.begin(); it != s.end(); it++)
            {
                if (*it >= 0 && *it < nbOfNodes)
                    std::transform(
                        coor + spaceDim * (*it), coor + spaceDim * ((*it) + 1), ptToFill, ptToFill, std::plus<double>()
                    );
                else
                {
                    std::ostringstream oss;
                    oss << "MEDCouplingUMesh::computeIsoBarycenterOfNodesPerCell : on cell polyhedron cell #" << i
                        << " presence of nodeId #" << *it << " should be in [0," << nbOfNodes << ") !";
                    throw INTERP_KERNEL::Exception(oss.str());
                }
            }
            if (!s.empty())
                std::transform(
                    ...
                    ptToFill + spaceDim,
                    ptToFill,
                    std::bind(std::multiplies<double>(), std::placeholders::_1, 1. / (double)s.size())
                );
            else
            {
                std::ostringstream oss;
                oss << "MEDCouplingUMesh::computeIsoBarycenterOfNodesPerCell : on polyhedron cell #" << i
                    << " there are no nodes !";
                throw INTERP_KERNEL::Exception(oss.str());
            }
        }
 ...
 */
DataArrayDouble *
MEDCouplingUMesh::getPartBarycenterAndOwner(const mcIdType *begin, const mcIdType *end) const
{
    ...
    for (const mcIdType *w = begin; w != end; w++)
    {
        ...
        INTERP_KERNEL::computeBarycenter2<mcIdType, INTERP_KERNEL::ALL_C_MODE>(
            type, nodal + nodalI[*w] + 1, nodalI[*w + 1] - nodalI[*w] - 1, coor, spaceDim, ptToFill
        );
        ptToFill += spaceDim;
    }
    delete[] tmp;
 ...
 *
 * \return DataArrayDouble * - a new instance of DataArrayDouble having 4 components and a number of tuples equal to
 * number of cells in \a this.
 ...
 */
DataArrayDouble *
MEDCouplingUMesh::computePlaneEquationOf3DFaces() const
{
    MCAuto<DataArrayDouble> ret(DataArrayDouble::New());
    mcIdType nbOfCells = getNumberOfCells();
    mcIdType nbOfNodes(getNumberOfNodes());
    if (getSpaceDimension() != 3 || getMeshDimension() != 2)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::computePlaneEquationOf3DFaces : This method must be applied on a mesh having "
            "meshDimension equal 2 and a spaceDimension equal to 3 !"
        );
    ...
    for (mcIdType i = 0; i < nbOfCells; i++, nodalI++, retPtr += 4)
    {
        ...
        {
            double aa[3] =
                {coor[nodal[nodalI[0] + 1 + 1] * 3 + 0] - coor[nodal[nodalI[0] + 1 + 0] * 3 + 0],
                 coor[nodal[nodalI[0] + 1 + 1] * 3 + 1] - coor[nodal[nodalI[0] + 1 + 0] * 3 + 1],
                 coor[nodal[nodalI[0] + 1 + 1] * 3 + 2] - coor[nodal[nodalI[0] + 1 + 0] * 3 + 2]},
                   bb[3] = {
                       coor[nodal[nodalI[0] + 1 + 2] * 3 + 0] - coor[nodal[nodalI[0] + 1 + 0] * 3 + 0],
                       coor[nodal[nodalI[0] + 1 + 2] * 3 + 1] - coor[nodal[nodalI[0] + 1 + 0] * 3 + 1],
                       coor[nodal[nodalI[0] + 1 + 2] * 3 + 2] - coor[nodal[nodalI[0] + 1 + 0] * 3 + 2]
                   };
            double cc[3] = {
                aa[1] * bb[2] - aa[2] * bb[1], aa[2] * bb[0] - aa[0] * bb[2], aa[0] * bb[1] - aa[1] * bb[0]
            };
            double aa_norm(sqrt(aa[0] * aa[0] + aa[1] * aa[1] + aa[2] * aa[2])),
                bb_norm(sqrt(bb[0] * bb[0] + bb[1] * bb[1] + bb[2] * bb[2]));
            for (int j = 0; j < 3; j++)
            {
                ...
                {
                    std::ostringstream oss;
                    oss << "MEDCouplingUMesh::computePlaneEquationOf3DFaces : invalid 2D cell #" << i
                        << " ! This cell points to an invalid nodeId : " << nodeId << " !";
                    throw INTERP_KERNEL::Exception(oss.str());
                }
            }
            ...
            else
            {
                ...
                {
                    std::ostringstream oss;
                    oss << "MEDCouplingUMesh::computePlaneEquationOf3DFaces : cell" << i
                        << " : Presence of The 3 colinear points !";
                    throw INTERP_KERNEL::Exception(oss.str());
                }
                //
                double dd[3] = {0., 0., 0.};
                for (mcIdType offset = nodalI[0] + 1; offset < nodalI[1]; offset++)
                    std::transform(
                        coor + 3 * nodal[offset], coor + 3 * (nodal[offset] + 1), dd, dd, std::plus<double>()
                    );
        ...
        else
        {
            std::ostringstream oss;
            oss << "MEDCouplingUMesh::computePlaneEquationOf3DFaces : invalid 2D cell #" << i
                << " ! Must be constitued by more than 3 nodes !";
            throw INTERP_KERNEL::Exception(oss.str());
        }
    }
    return ret.retn();
...
/*!
 * This method expects as input a DataArrayDouble non nul instance 'da' that should be allocated. If not an exception is
 * thrown.
 *
 */
MEDCouplingUMesh *
MEDCouplingUMesh::Build0DMeshFromCoords(DataArrayDouble *da)
{
    if (!da)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::Build0DMeshFromCoords : instance of DataArrayDouble must be not null !"
        );
    da->checkAllocated();
...
MCAuto<MEDCouplingUMesh>
MEDCouplingUMesh::Build1DMeshFromCoords(DataArrayDouble *da)
{
    if (!da)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::Build01MeshFromCoords : instance of DataArrayDouble must be not null !"
        );
    da->checkAllocated();
    std::string name(da->getName());
    MCAuto<MEDCouplingUMesh> ret;
...
MEDCouplingUMesh::MergeUMeshes(const MEDCouplingUMesh *mesh1, const MEDCouplingUMesh *mesh2)
{
    ...
    tmp[0] = const_cast<MEDCouplingUMesh *>(mesh1);
    tmp[1] = const_cast<MEDCouplingUMesh *>(mesh2);
 ...
 */
MEDCouplingUMesh *
MEDCouplingUMesh::MergeUMeshes(const std::vector<const MEDCouplingUMesh *> &a)
{
    ...
    for (std::size_t ii = 0; ii < sz; ii++)
        ...
        {
            std::ostringstream oss;
            oss << "MEDCouplingUMesh::MergeUMeshes : item #" << ii << " in input array of size " << sz << " is empty !";
            throw INTERP_KERNEL::Exception(oss.str());
        }
    ...
    if (spaceDim == -3)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::MergeUMeshes : no spaceDim specified ! unable to perform merge !"
        );
    for (std::size_t i = 0; i < sz; i++)
    {
...
MEDCouplingUMesh::MergeUMeshesOnSameCoords(const std::vector<const MEDCouplingUMesh *> &meshes)
{
    ...
    for (std::size_t ii = 0; ii < meshes.size(); ii++)
        ...
        {
            std::ostringstream oss;
            oss << "MEDCouplingUMesh::MergeUMeshesOnSameCoords : item #" << ii << " in input array of size "
                << meshes.size() << " is empty !";
            throw INTERP_KERNEL::Exception(oss.str());
        }
    ...
    for (; iter != meshes.end(); iter++)
    {
        if (coords != (*iter)->getCoords())
            throw INTERP_KERNEL::Exception(
                "meshes does not share the same coords ! Try using tryToShareSameCoords method !"
            );
 ...
 */
MEDCouplingUMesh *
MEDCouplingUMesh::FuseUMeshesOnSameCoords(
    const std::vector<const MEDCouplingUMesh *> &meshes, int compType, std::vector<DataArrayIdType *> &corr
)
{
...
MEDCouplingUMesh::PutUMeshesOnSameAggregatedCoords(const std::vector<MEDCouplingUMesh *> &meshes)
{
    ...
    for (std::vector<MEDCouplingUMesh *>::const_iterator it = meshes.begin(); it != meshes.end(); it++, it2++)
    {
        if ((*it))
        {
            ...
            {
                std::ostringstream oss;
                oss << " MEDCouplingUMesh::PutUMeshesOnSameAggregatedCoords : Item #"
                    << std::distance(meshes.begin(), it) << " inside the vector of length " << meshes.size();
        ...
        {
            std::ostringstream oss;
            oss << " MEDCouplingUMesh::PutUMeshesOnSameAggregatedCoords : Item #" << std::distance(meshes.begin(), it)
                << " inside the vector of length " << meshes.size();
            oss << " is null !";
            throw INTERP_KERNEL::Exception(oss.str());
        }
    }
...
MEDCouplingUMesh::MergeNodesOnUMeshesSharingSameCoords(const std::vector<MEDCouplingUMesh *> &meshes, double eps)
{
    ...
    for (std::vector<MEDCouplingUMesh *>::const_iterator it = meshes.begin(); it != meshes.end(); it++)
    {
        ...
        {
            std::ostringstream oss;
            oss << "MEDCouplingUMesh::MergeNodesOnUMeshesSharingSameCoords : In input vector of unstructured meshes of "
                   "size "
                << meshes.size() << " the element #" << std::distance(meshes.begin(), it) << " is null !";
            throw INTERP_KERNEL::Exception(oss.str());
        }
    }
    if (s.size() != 1)
    {
        ...
        oss << "MEDCouplingUMesh::MergeNodesOnUMeshesSharingSameCoords : In input vector of unstructured meshes of "
               "size "
            << meshes.size()
            << ", it appears that they do not share the same instance of DataArrayDouble for coordiantes ! "
               "tryToShareSameCoordsPermute method can help to reach that !";
        throw INTERP_KERNEL::Exception(oss.str());
    }
    ...
    MCAuto<DataArrayIdType> o2n = DataArrayIdType::ConvertIndexArrayToO2N(
        oldNbOfNodes, comm->begin(), commI->begin(), commI->end(), newNbOfNodes
    );
 ...
 */
bool
MEDCouplingUMesh::IsPolygonWellOriented(
    bool isQuadratic, const double *vec, const mcIdType *begin, const mcIdType *end, const double *coords
)
{
    std::size_t i, ip1;
    double v[3] = {0., 0., 0.};
    ...
    if (!isQuadratic)
        for (i = 0; i < sz; i++)
        {
            ...
            v[0] += coords[3 * begin[i] + 1] * coords[3 * begin[(i + 1) % sz] + 2] -
                    coords[3 * begin[i] + 2] * coords[3 * begin[(i + 1) % sz] + 1];
            v[1] += coords[3 * begin[i] + 2] * coords[3 * begin[(i + 1) % sz]] -
                    coords[3 * begin[i]] * coords[3 * begin[(i + 1) % sz] + 2];
            v[2] += coords[3 * begin[i]] * coords[3 * begin[(i + 1) % sz] + 1] -
                    coords[3 * begin[i] + 1] * coords[3 * begin[(i + 1) % sz]];
        }
    else
    {
        ...
        for (std::size_t j = 0; j < sz; j++)
        {
            ...
            v[0] += coords[3 * begin[i] + 1] * coords[3 * begin[ip1] + 2] -
                    coords[3 * begin[i] + 2] * coords[3 * begin[ip1] + 1];
            v[1] +=
                coords[3 * begin[i] + 2] * coords[3 * begin[ip1]] - coords[3 * begin[i]] * coords[3 * begin[ip1] + 2];
            v[2] +=
                coords[3 * begin[i]] * coords[3 * begin[ip1] + 1] - coords[3 * begin[i] + 1] * coords[3 * begin[ip1]];
        }
    }
 ...
 */
bool
MEDCouplingUMesh::IsPolyhedronWellOriented(const mcIdType *begin, const mcIdType *end, const double *coords)
{
    ...
    return INTERP_KERNEL::calculateVolumeForPolyh2<mcIdType, INTERP_KERNEL::ALL_C_MODE>(
               begin, ToIdType(std::distance(begin, end)), coords
 ...
 */
bool
MEDCouplingUMesh::Is3DExtrudedStaticCellWellOriented(const mcIdType *begin, const mcIdType *end, const double *coords)
{
    double vec0[3], vec1[3];
    std::size_t sz = std::distance(begin, end);
    if (sz % 2 != 0)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::Is3DExtrudedStaticCellWellOriented : the length of nodal connectivity of extruded cell "
            "is not even !"
        );
...
void
MEDCouplingUMesh::CorrectExtrudedStaticCell(mcIdType *begin, mcIdType *end)
{
    std::size_t sz = std::distance(begin, end);
...
bool
MEDCouplingUMesh::IsTetra4WellOriented(const mcIdType *begin, const mcIdType *end, const double *coords)
{
    std::size_t sz = std::distance(begin, end);
    if (sz != 4)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::IsTetra4WellOriented : Tetra4 cell with not 4 nodes ! Call checkConsistency !"
        );
    double vec0[3], vec1[3];
    const double *pt0 = coords + 3 * begin[0], *pt1 = coords + 3 * begin[1], *pt2 = coords + 3 * begin[2],
                 *pt3 = coords + 3 * begin[3];
    vec0[0] = pt1[0] - pt0[0];
    vec0[1] = pt1[1] - pt0[1];
    vec0[2] = pt1[2] - pt0[2];
    vec1[0] = pt2[0] - pt0[0];
    vec1[1] = pt2[1] - pt0[1];
    vec1[2] = pt2[2] - pt0[2];
    return ((vec0[1] * vec1[2] - vec0[2] * vec1[1]) * (pt3[0] - pt0[0]) +
            (vec0[2] * vec1[0] - vec0[0] * vec1[2]) * (pt3[1] - pt0[1]) +
            (vec0[0] * vec1[1] - vec0[1] * vec1[0]) * (pt3[2] - pt0[2])) < 0;
}

bool
MEDCouplingUMesh::IsPyra5WellOriented(const mcIdType *begin, const mcIdType *end, const double *coords)
{
    std::size_t sz = std::distance(begin, end);
    if (sz != 5)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::IsPyra5WellOriented : Pyra5 cell with not 5 nodes ! Call checkConsistency !"
        );
    double vec0[3];
...
/*!
 * This method performs a simplyfication of a single polyedron cell. To do that each face of cell whose connectivity is
 * defined by [ \b begin , \b end ) is compared with the others in order to find faces in the same plane (with approx of
 * eps). If any, the cells are grouped together and projected to a 2D space.
 *
 ...
 */
void
MEDCouplingUMesh::SimplifyPolyhedronCell(
    double eps,
    const DataArrayDouble *coords,
    mcIdType index,
    DataArrayIdType *res,
    MEDCouplingUMesh *faces,
    DataArrayIdType *E_Fi,
    DataArrayIdType *E_F,
    DataArrayIdType *F_Ei,
    DataArrayIdType *F_E
)
{
    mcIdType nbFaces = E_Fi->getIJ(index + 1, 0) - E_Fi->getIJ(index, 0);
    MCAuto<DataArrayDouble> v = DataArrayDouble::New();
    v->alloc(nbFaces, 3);
    double *vPtr = v->getPointer();
    MCAuto<DataArrayDouble> p = DataArrayDouble::New();
    p->alloc(nbFaces, 2);
    double *pPtr = p->getPointer();
    mcIdType *e_fi = E_Fi->getPointer(), *e_f = E_F->getPointer(), *f_ei = F_Ei->getPointer(), *f_e = F_E->getPointer();
    const mcIdType *f_idx = faces->getNodalConnectivityIndex()->getPointer(),
                   *f_cnn = faces->getNodalConnectivity()->getPointer();
    for (mcIdType i = 0; i < nbFaces; i++, vPtr += 3, pPtr++)
    {
        ...
        pPtr++, *pPtr = 0;
        for (mcIdType j = f_ei[face]; j < f_ei[face + 1]; j++) *pPtr += FromIdType<double>(f_e[j]);
    }
    pPtr = p->getPointer();
    vPtr = v->getPointer();
    ...
    for (mcIdType i = 0; i < nbOfGrps1; i++)
    {
        ...
        for (mcIdType j = 0; j < nbOfGrps2; j++)
        {
            ...
            else
            {
                ...
                MCAuto<MEDCouplingUMesh> mm3 =
                    static_cast<MEDCouplingUMesh *>(faces->buildPartOfMySelf(ids2->begin(), ids2->end(), true));
                ...
                double center[3];
                center[0] = pPtr[2 * pointId] * vPtr[3 * vecId];
                center[1] = pPtr[2 * pointId] * vPtr[3 * vecId + 1];
                center[2] = pPtr[2 * pointId] * vPtr[3 * vecId + 2];
                double vec[3];
                vec[0] = vPtr[3 * vecId + 1];
                vec[1] = -vPtr[3 * vecId];
                vec[2] = 0.;
...
/*!
 * This method computes the normalized vector of the plane and the pos of the point belonging to the plane and the line
 * defined by the vector going through origin. The plane is defined by its nodal connectivity [ \b begin, \b end ).
 *
 ...
 */
void
MEDCouplingUMesh::ComputeVecAndPtOfFace(
    double eps, const double *coords, const mcIdType *begin, const mcIdType *end, double *v, double *p
)
{
    std::size_t nbPoints = std::distance(begin, end);
    if (nbPoints < 3)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::ComputeVecAndPtOfFace : < of 3 points in face ! not able to find a plane on that face !"
        );
    double vec[3] = {0., 0., 0.};
    std::size_t j = 0;
    bool refFound = false;
    ...
    for (std::size_t i = j; i < nbPoints - 1; i++)
    {
        ...
        curVec[1] /= norm;
        curVec[2] /= norm;
        v[0] = vec[1] * curVec[2] - vec[2] * curVec[1];
        v[1] = vec[2] * curVec[0] - vec[0] * curVec[2];
        v[2] = vec[0] * curVec[1] - vec[1] * curVec[0];
 ...
 */
void
MEDCouplingUMesh::TryToCorrectPolyhedronOrientation(mcIdType *begin, mcIdType *end, const double *coords)
{
    ...
    {
        std::pair<mcIdType, mcIdType> p1(bgFace[l], bgFace[(l + 1) % nbOfEdgesInFace]);
            ...
            if (!isPerm[i])
            {
                ...
                if (isPerm[i])
                {
                    ...
                    for (std::size_t j = 0; j < nbOfEdgesInFace; j++)
                    {
                        ...
                        {
                            std::ostringstream oss;
                            oss << "Face #" << j << " of polyhedron looks bad !";
                            throw INTERP_KERNEL::Exception(oss.str());
                        }
                        if (std::find(edgesFinished.begin(), edgesFinished.end(), p1) != edgesFinished.end() ||
                            std::find(edgesFinished.begin(), edgesFinished.end(), p2) != edgesFinished.end())
                        {
                            std::ostringstream oss;
                            oss << "Face #" << j << " of polyhedron looks bad !";
                            throw INTERP_KERNEL::Exception(oss.str());
                        }
                        std::list<std::pair<mcIdType, mcIdType> >::iterator it =
                            std::find(edgesOK.begin(), edgesOK.end(), p2);
                        if (it != edgesOK.end())
                        {
                            edgesOK.erase(it);
    ...
    {
        throw INTERP_KERNEL::Exception(
            "The polyhedron looks too bad to be repaired : Some edges are shared only once !"
        );
    }
    if (INTERP_KERNEL::calculateVolumeForPolyh2<mcIdType, INTERP_KERNEL::ALL_C_MODE>(
            begin, ToIdType(std::distance(begin, end)), coords
        ) < -EPS_FOR_POLYH_ORIENTATION)
...
MEDCouplingUMesh::buildUnionOf2DMesh() const
{
    if (getMeshDimension() != 2 || getSpaceDimension() != 2)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::buildUnionOf2DMesh : meshdimension, spacedimension must be equal to 2 !"
        );
    MCAuto<MEDCouplingUMesh> skin(computeSkin());
    ...
    if (nbCells == nbOfNodesExpected)
        ...
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::buildUnionOf2DMesh : the mesh 2D in input appears to be not in a single part of a 2D "
            "mesh !"
        );
}
...
MEDCouplingUMesh::buildUnionOf3DMesh() const
{
    if (getMeshDimension() != 3 || getSpaceDimension() != 3)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::buildUnionOf3DMesh : meshdimension, spacedimension must be equal to 2 !"
        );
    ...
    MCAuto<DataArrayIdType> ret = DataArrayIdType::New();
    ret->alloc(m->getNodalConnectivity()->getNumberOfTuples(), 1);
    mcIdType *work = ret->getPointer();
    *work++ = INTERP_KERNEL::NORM_POLYHED;
    if (nbOfCells < 1)
        return ret.retn();
...
MEDCouplingUMesh::generateGraph() const
{
    checkConnectivityFullyDefined();
        ...
        for (mcIdType iconn = index_ptr[icell] + 1; iconn < index_ptr[icell + 1]; iconn++)
        {
            ...
            for (mcIdType iconnr = indexr_ptr[inode]; iconnr < indexr_ptr[inode + 1]; iconnr++)
            {
                ...
                if (iter != counter.end())
                    ...
                    counter.insert(std::make_pair(icell2, 1));
            }
        }
        for (std::map<mcIdType, mcIdType>::const_iterator iter = counter.begin(); iter != counter.end(); iter++)
...
) const
{
    ...
    MCAuto<DataArrayIdType> faceoffsets = DataArrayIdType::New();
    faceoffsets->alloc(nbOfCells, 1);
    MCAuto<DataArrayIdType> types = DataArrayIdType::New();
    types->alloc(nbOfCells, 1);
    MCAuto<DataArrayIdType> offsets = DataArrayIdType::New();
    offsets->alloc(nbOfCells, 1);
    MCAuto<DataArrayIdType> connectivity = DataArrayIdType::New();
    connectivity->alloc(_nodal_connec->getNumberOfTuples() - nbOfCells, 1);
    mcIdType *w1 = faceoffsets->getPointer(), *w2 = types->getPointer(), *w3 = offsets->getPointer(),
             *w4 = connectivity->getPointer();
    ...
    for (mcIdType i = 0; i < nbOfCells; i++, w1++, w2++, w3++)
    {
        *w2 = cPtr[cIPtr[i]];
        ...
        else
        {
            ...
            *w1 = szFaceOffsets + deltaFaceOffset;
            szFaceOffsets += deltaFaceOffset;
            std::set<mcIdType> c(cPtr + cIPtr[i] + 1, cPtr + cIPtr[i + 1]);
            c.erase(-1);
            *w3 = szConn + ToIdType(c.size());
            szConn += ToIdType(c.size());
    ...
    for (auto ii = 0; ii < MEDCOUPLING2VTKTYPETRADUCER_LGTH; ++ii)
        medcoupling2vtkTypeTraducer_mcIdType[ii] =
            MEDCOUPLING2VTKTYPETRADUCER[ii] != MEDCOUPLING2VTKTYPETRADUCER_NONE ? MEDCOUPLING2VTKTYPETRADUCER[ii] : -1;
    types->transformWithIndArr(
        medcoupling2vtkTypeTraducer_mcIdType.get(),
        medcoupling2vtkTypeTraducer_mcIdType.get() + MEDCOUPLING2VTKTYPETRADUCER_LGTH
    );
    ...
    {  // presence of Polyhedra
        ...
        MCAuto<DataArrayIdType> faces = DataArrayIdType::New();
        faces->alloc(szFaceOffsets, 1);
        w1 = faces->getPointer();
        for (mcIdType i = 0; i < nbOfCells; i++)
            if ((INTERP_KERNEL::NormalizedCellType)cPtr[cIPtr[i]] == INTERP_KERNEL::NORM_POLYHED)
            {
...
MEDCouplingUMesh::reprQuickOverview(std::ostream &stream) const
{
    ...
    if (!_coords)
    {
        stream << " No coordinates set !";
        return;
    }
    if (!_coords->isAllocated())
    {
        stream << " Coordinates set but not allocated !";
        return;
    }
    ...
    if (!_nodal_connec_index)
    {
        stream << std::endl << "Nodal connectivity NOT set !";
        return;
    }
    if (!_nodal_connec_index->isAllocated())
    {
        stream << std::endl << "Nodal connectivity set but not allocated !";
        return;
    }
...
std::string
MEDCouplingUMesh::getVTKFileExtension() const
{
 ...
 * This doesn't modify the mesh. This method only works using nodal connectivity consideration. Coordinates of nodes are
 * ignored here. The caller is to deal with the resulting DataArrayIdType.
 ...
 */
DataArrayIdType *
MEDCouplingUMesh::orderConsecutiveCells1D() const
{
    checkFullyDefined();
    if (getMeshDimension() != 1)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::orderConsecutiveCells1D works on unstructured mesh with meshdim = 1 !"
        );
    ...
    if (dsii->getNumberOfTuples())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::orderConsecutiveCells1D only work with a mesh being a (piecewise) connected line!"
        );
    ...
    do
    {
        ...
        for (int direction = 0; direction < 2; direction++)  // direction=0 --> forward, direction=1 --> backward
        {
            ...
            while (!edgeSet.empty())
            {
                ...
                bool segmentAlreadyTreated =
                    (std::find(linePiece.begin(), linePiece.end(), activeSeg) != linePiece.end());
...
)
{
    ...
    desc->checkAllocated();
    descI->checkAllocated();
    subNodesInSeg->checkAllocated();
    subNodesInSegI->checkAllocated();
    if (getSpaceDimension() != 2 || getMeshDimension() != 2)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::split2DCells : This method only works for meshes with spaceDim=2 and meshDim=2 !"
        );
    if (midOpt == 0 && midOptI == 0)
    {
        ...
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::split2DCells : middle parameters must be set to null for all or not null for all."
        );
}

/*!
 * This method compute the convex hull of a single 2D cell. This method tries to conserve at maximum the given input
 * connectivity. In particular, if the orientation of cell is not clockwise as in MED format norm. If definitely the
 * result of Jarvis algorithm is not matchable with the input connectivity, the result will be copied into \b
 * nodalConnecOut parameter and the geometric cell type set to INTERP_KERNEL::NORM_POLYGON. This method excepts that \b
 * coords parameter is expected to be in dimension 2. [ \b nodalConnBg , \b nodalConnEnd ) is the nodal connectivity of
 * the input cell (geometric cell type included at the position 0). If the meshdimension of the input cell is not equal
 * to 2 an INTERP_KERNEL::Exception will be thrown.
 *
 * \return false if the input connectivity represents already the convex hull, true if the input cell needs to be
 * reordered.
 */
bool
MEDCouplingUMesh::BuildConvexEnvelopOf2DCellJarvis(
    const double *coords, const mcIdType *nodalConnBg, const mcIdType *nodalConnEnd, DataArrayIdType *nodalConnecOut
)
{
    ...
    {
        const INTERP_KERNEL::CellModel &cm =
            INTERP_KERNEL::CellModel::GetCellModel((INTERP_KERNEL::NormalizedCellType)*nodalConnBg);
        if (cm.getDimension() == 2)
        {
            ...
            std::vector<mcIdType> tmpOut;
            tmpOut.reserve(sz);
            tmpOut.push_back(startNode);
            refX = 1e300;
            double tmp1;
            double tmp2[2];
            ...
            while (nextNode != startNode)
            {
                ...
                for (node = nodalConnBg + 1; node != nodalConnEnd; node++)
                {
                    ...
                    {
                        tmp2[0] = coords[2 * (*node)] - coords[2 * tmpOut.back()];
                        tmp2[1] = coords[2 * (*node) + 1] - coords[2 * tmpOut.back() + 1];
            ...
            throw INTERP_KERNEL::Exception(
                "MEDCouplingUMesh::BuildConvexEnvelopOf2DCellJarvis : invalid 2D cell connectivity !"
            );
    }
    else
        throw INTERP_KERNEL::Exception(
 ...
 * This method expects that these two input arrays come from the output of MEDCouplingUMesh::computeNeighborsOfCells
 * method. This method start from id 0 that will be contained in output DataArrayIdType. It searches then all neighbors
 * of id0 looking at arrIn[arrIndxIn[0]:arrIndxIn[0+1]]. Then it is repeated recursively until either all ids are
 * fetched or no more ids are reachable step by step. A negative value in \b arrIn means that it is ignored. This method
 * is useful to see if a mesh is contiguous regarding its connectivity. If it is not the case the size of returned array
 * is different from arrIndxIn->getNumberOfTuples()-1.
 *
 ...
 */
DataArrayIdType *
MEDCouplingUMesh::ComputeSpreadZoneGradually(const DataArrayIdType *arrIn, const DataArrayIdType *arrIndxIn)
{
 ...
 * This method expects that these two input arrays come from the output of MEDCouplingUMesh::computeNeighborsOfCells
 * method. This method start from id 0 that will be contained in output DataArrayIdType. It searches then all neighbors
 * of id0 regarding arrIn[arrIndxIn[0]:arrIndxIn[0+1]]. Then it is repeated recursively until either all ids are fetched
 * or no more ids are reachable step by step. A negative value in \b arrIn means that it is ignored. This method is
 * useful to see if a mesh is contiguous regarding its connectivity. If it is not the case the size of returned array is
 * different from arrIndxIn->getNumberOfTuples()-1.
 ...
 * \param [out] nbOfDepthPeelingPerformed the number of peels effectively performed. May be different from \a
 * nbOfDepthPeeling
 * \return a newly allocated DataArray that stores all ids fetched by the gradually spread process.
 * \sa MEDCouplingUMesh::partitionBySpreadZone
 */
DataArrayIdType *
MEDCouplingUMesh::ComputeSpreadZoneGraduallyFromSeed(
    const mcIdType *seedBg,
    const mcIdType *seedEnd,
    const DataArrayIdType *arrIn,
    const DataArrayIdType *arrIndxIn,
    mcIdType nbOfDepthPeeling,
    mcIdType &nbOfDepthPeelingPerformed
)
{
    nbOfDepthPeelingPerformed = 0;
    if (!arrIndxIn)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::ComputeSpreadZoneGraduallyFromSeed : arrIndxIn input pointer is NULL !"
        );
    ...
    return ComputeSpreadZoneGraduallyFromSeedAlg(
        fetched, seedBg, seedEnd, arrIn, arrIndxIn, nbOfDepthPeeling, nbOfDepthPeelingPerformed
    );
}
...
MEDCouplingUMesh::buildSpreadZonesWithPoly() const
{
    ...
    if (mdim != spaceDim)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::buildSpreadZonesWithPoly : meshdimension and spacedimension do not match !"
        );
    std::vector<DataArrayIdType *> partition = partitionBySpreadZone();
    std::vector<MCAuto<DataArrayIdType> > partitionAuto;
    partitionAuto.reserve(partition.size());
    std::copy(
        partition.begin(),
        partition.end(),
        std::back_insert_iterator<std::vector<MCAuto<DataArrayIdType> > >(partitionAuto)
    );
    MCAuto<MEDCouplingUMesh> ret = MEDCouplingUMesh::New(getName(), mdim);
    ret->setCoords(getCoords());
    ...
    for (std::vector<DataArrayIdType *>::const_iterator it = partition.begin(); it != partition.end(); it++)
    {
        MCAuto<MEDCouplingUMesh> tmp =
            static_cast<MEDCouplingUMesh *>(buildPartOfMySelf((*it)->begin(), (*it)->end(), true));
        ...
        switch (mdim)
        {
            ...
            default:
                throw INTERP_KERNEL::Exception(
                    "MEDCouplingUMesh::buildSpreadZonesWithPoly : meshdimension supported are [2,3] ! Not implemented "
                    "yet for others !"
                );
        }

        ret->insertNextCell(
            (INTERP_KERNEL::NormalizedCellType)cell->getIJSafe(0, 0), cell->getNumberOfTuples() - 1, cell->begin() + 1
        );
    }
    //
...
std::vector<DataArrayIdType *>
MEDCouplingUMesh::PartitionBySpreadZone(const DataArrayIdType *arrIn, const DataArrayIdType *arrIndxIn)
{
    if (!arrIn || !arrIndxIn)
        throw INTERP_KERNEL::Exception("PartitionBySpreadZone : null input pointers !");
    arrIn->checkAllocated();
    ...
    while (seed < nbOfCellsCur)
    {
        ...
        ret2.push_back(ComputeSpreadZoneGraduallyFromSeedAlg(
            fetchedCells, &seed, &seed + 1, arrIn, arrIndxIn, -1, nbOfPeelPerformed
        ));
        seed = ToIdType(
            std::distance(fetchedCells.begin(), std::find(fetchedCells.begin() + seed, fetchedCells.end(), false))
        );
    }
...
/*!
 * This method returns given a distribution of cell type (returned for example by
 * MEDCouplingUMesh::getDistributionOfTypes method and customized after) a newly allocated DataArrayIdType instance with
 * 2 components ready to be interpreted as input of DataArrayIdType::findRangeIdForEachTuple method.
 *
 * \param [in] code a code with the same format than those returned by MEDCouplingUMesh::getDistributionOfTypes except
 * for the code[3*k+2] that should contain start id of chunck.
 ...
 */
DataArrayIdType *
MEDCouplingUMesh::ComputeRangesFromTypeDistribution(const std::vector<mcIdType> &code)
{
    MCAuto<DataArrayIdType> ret = DataArrayIdType::New();
 ...
 * This method will split **all** 3D cells in \a this into INTERP_KERNEL::NORM_TETRA4 cells and put them in the returned
 * mesh. It leads to an increase to number of cells. This method contrary to MEDCouplingUMesh::simplexize can append
 * coordinates in \a this to perform its work. The \a nbOfAdditionalPoints returned value informs about it. If > 0, the
 * coordinates array in returned mesh will have \a nbOfAdditionalPoints more tuples (nodes) than in \a this. Anyway, all
 * the nodes in \a this (with the same order) will be in the returned mesh.
 *
 * \param [in] policy - the policy of splitting that must be in (PLANAR_FACE_5, PLANAR_FACE_6, GENERAL_24, GENERAL_48).
 * The policy will be used only for INTERP_KERNEL::NORM_HEXA8 cells. For all other cells, the splitting policy will be
 * ignored. See INTERP_KERNEL::SplittingPolicy for the images.
 * \param [out] nbOfAdditionalPoints - number of nodes added to \c this->_coords. If > 0 a new coordinates object will
 * be constructed result of the aggregation of the old one and the new points added.
 ...
 *
 * \warning This method operates on each cells in this independently ! So it can leads to non conform mesh in returned
 * value ! If you expect to have a conform mesh in output the policy PLANAR_FACE_6 should be used on a mesh sorted with
...
MEDCouplingUMesh::tetrahedrize(int policy, DataArrayIdType *&n2oCells, mcIdType &nbOfAdditionalPoints) const
{
    INTERP_KERNEL::SplittingPolicy pol((INTERP_KERNEL::SplittingPolicy)policy);
    checkConnectivityFullyDefined();
    if (getMeshDimension() != 3 || getSpaceDimension() != 3)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::tetrahedrize : only available for mesh with meshdim == 3 and spacedim == 3 !"
        );
    mcIdType nbOfCells = getNumberOfCells();
    mcIdType nbNodes(getNumberOfNodes());
    ...
    MCAuto<DataArrayIdType> newConn(DataArrayIdType::New());
    newConn->alloc(0, 1);
    MCAuto<DataArrayDouble> addPts(DataArrayDouble::New());
    addPts->alloc(0, 1);
    ...
    {
        std::vector<mcIdType> a;
        std::vector<double> b;
        INTERP_KERNEL::SplitIntoTetras(
            pol, (INTERP_KERNEL::NormalizedCellType)oldc[oldci[0]], oldc + oldci[0] + 1, oldc + oldci[1], coords, a, b
        );
        std::size_t nbOfTet(a.size() / 4);
        *retPt = ToIdType(nbOfTet);
        ...
        for (std::size_t j = 0; j < nbOfTet; j++, aa += 4) newConn->insertAtTheEnd(aa, aa + 4);
    }
    if (!addPts->empty())
    {
        addPts->rearrange(3);
...
MEDCouplingUMeshCellIterator::MEDCouplingUMeshCellIterator(MEDCouplingUMesh *mesh)
    : _mesh(mesh), _cell(new MEDCouplingUMeshCell(mesh)), _own_cell(true), _cell_id(-1), _nb_cell(0)
{
    if (mesh)
    {
        mesh->incrRef();
...
MEDCouplingUMeshCellIterator::MEDCouplingUMeshCellIterator(
    MEDCouplingUMesh *mesh, MEDCouplingUMeshCell *itc, mcIdType bg, mcIdType end
)
    : _mesh(mesh), _cell(itc), _own_cell(false), _cell_id(bg - 1), _nb_cell(end)
{
    if (mesh)
        mesh->incrRef();
}

MEDCouplingUMeshCell *
MEDCouplingUMeshCellIterator::nextt()
{
    _cell_id++;
...
MEDCouplingUMeshCellByTypeEntry::iterator()
{
...
MEDCouplingUMeshCellEntry::MEDCouplingUMeshCellEntry(
    MEDCouplingUMesh *mesh, INTERP_KERNEL::NormalizedCellType type, MEDCouplingUMeshCell *itc, mcIdType bg, mcIdType end
)
    : _mesh(mesh), _type(type), _itc(itc), _bg(bg), _end(end)
{
    if (_mesh)
        _mesh->incrRef();
}
...
INTERP_KERNEL::NormalizedCellType
MEDCouplingUMeshCellEntry::getType() const
{
    return _type;
}

mcIdType
MEDCouplingUMeshCellEntry::getNumberOfElems() const
{
    return _end - _bg;
}

MEDCouplingUMeshCellIterator *
MEDCouplingUMeshCellEntry::iterator()
{
    return new MEDCouplingUMeshCellIterator(_mesh, _itc, _bg, _end);
}

MEDCouplingUMeshCellByTypeIterator::MEDCouplingUMeshCellByTypeIterator(MEDCouplingUMesh *mesh)
    : _mesh(mesh), _cell(new MEDCouplingUMeshCell(mesh)), _cell_id(0), _nb_cell(0)
{
    if (mesh)
    {
        mesh->incrRef();
...
MEDCouplingUMeshCellByTypeIterator::nextt()
{
    ...
    if (_cell_id < _nb_cell)
    {
        ...
        mcIdType nbOfElems = ToIdType(
            std::distance(
                ci + _cell_id, std::find_if(ci + _cell_id, ci + _nb_cell, MEDCouplingImpl::ConnReader(c, type))
            )
        );
...
MEDCouplingUMeshCell::MEDCouplingUMeshCell(MEDCouplingUMesh *mesh)
    : _conn(0), _conn_indx(0), _conn_lgth(NOTICABLE_FIRST_VAL)
{
    if (mesh)
    {
...
std::string
MEDCouplingUMeshCell::repr() const
{
    if (_conn_lgth != NOTICABLE_FIRST_VAL)
    {
        std::ostringstream oss;
        oss << "Cell Type "
            << INTERP_KERNEL::CellModel::GetCellModel((INTERP_KERNEL::NormalizedCellType)_conn[0]).getRepr();
        oss << " : ";
...
INTERP_KERNEL::NormalizedCellType
MEDCouplingUMeshCell::getType() const
{
    if (_conn_lgth != NOTICABLE_FIRST_VAL)
...
const mcIdType *
MEDCouplingUMeshCell::getAllConn(mcIdType &lgth) const
{
    lgth = _conn_lgth;
    if (_conn_lgth != NOTICABLE_FIRST_VAL)
        return _conn;
...
mcIdType
encodeID(mcIdType id, int iMesh)
{
...
mcIdType
decodeID(mcIdType id, int &iMesh)
{
    iMesh = (id < 0);
    return std::abs(id) - 1;
}
...
)
{
    ...
    if (nbF > 1)
    {
        if (prevNbF > 0 && prevNbF < nbF)  // faces found in both meshes
        {
            ...
            const mcIdType *conn[2] = {
                mesh[0]->getNodalConnectivity()->getConstPointer(), mesh[1]->getNodalConnectivity()->getConstPointer()
            };
            const mcIdType *connI[2] = {
                mesh[0]->getNodalConnectivityIndex()->getConstPointer(),
            ...
            for (size_t i0 = 0; i0 < prevNbF; ++i0)
            {
                ...
                for (size_t i1 = prevNbF; i1 < facesByEdge.size(); ++i1)
                {
                    ...
                    if (std::equal(fConn0, fConn0 + nbNodes0, fConn1))
                    {
                        ...
                        if (std::find(equalFaces.begin(), equalFaces.end(), objFaceID) == equalFaces.end())
...
void
removeFromRevNodal(
    mcIdType node, mcIdType face, MCAuto<DataArrayIdType> &revNodal, MCAuto<DataArrayIdType> &revNodalIndx
)
{
...
bool
isReverseOrder(mcIdType n0, mcIdType n1, mcIdType iFEnc, MEDCouplingUMesh *mesh[])
{
    int iMesh;
    ...
    auto it0 = std::find(conn + connI[iF] + 1, conn + connI[iF + 1], n0);
    auto it1 = std::find(conn + connI[iF] + 1, conn + connI[iF + 1], n1);
    long i0 = it0 - conn;
    long i1 = it1 - conn;
...
void
reverseFace(mcIdType iFEnc, MEDCouplingUMesh *mesh[])
{
    int iMesh;
    ...
    const INTERP_KERNEL::CellModel &cm = INTERP_KERNEL::CellModel::GetCellModel(mesh[iMesh]->getTypeOfCell(face));

    cm.changeOrientationOf2D(conn + connI[face] + 1, (unsigned int)(connI[face + 1] - connI[face] - 1));
    return;
}
}  // namespace MEDCouplingImpl
...
void
MEDCouplingUMesh::orientCorrectly2DCells(const MEDCouplingUMesh *refFaces)
{
    checkConsistencyLight();
    if (getMeshDimension() != 2)
    ...
    mcIdType nbFacesToCheck[2] = {mesh[_OBJ]->getNumberOfCells(), mesh[_REF]->getNumberOfCells()};
    ...
    while (nbFacesToCheck[_OBJ] + nbFacesToCheck[_REF] > 0)  // until all faces checked
    {
        ...
        const INTERP_KERNEL::CellModel &cm =
            INTERP_KERNEL::CellModel::GetCellModel(mesh[iMesh]->getTypeOfCell(refFace));
        ...
        for (int edge = 0; edge < nbEdges; ++edge)
        {
            ...
            MEDCouplingImpl::getFacesOfEdge(n0, n1, fID, mesh, revNodal, revNodalIndx, facesByEdge, equalFaces);
        ...
        for (mcIdType face : equalFaces)
        {
            ...
            for (const mcIdType *n = conn + connI[f] + 1, *nEnd = n + nbNodes; n < nEnd; ++n)
                ...
                MEDCouplingImpl::removeFromRevNodal(
                    *n,
                    f,  // not to treat f again
                    revNodal[iMesh2],
                    revNodalIndx[iMesh2]
                );
        }
 ...
 */
void
MEDCouplingUMesh::orientCorrectly3DCells()
{
    ...
    MCAuto<MEDCouplingUMesh> meshToInvert =
        this->buildPartOfMySelf(idsOfCellToInvert->begin(), idsOfCellToInvert->end(), true);
