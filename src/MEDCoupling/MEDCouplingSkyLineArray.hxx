...
{
/**!
 * Class allowing the easy manipulation of the indexed array format, where the first array (the "indices") is a set of
 * offsets to be used in the second array (the "values"), to extract packs of values.
 *
 * Example:
 *    index = [0,2,7,10]
...
class MEDCOUPLING_EXPORT MEDCouplingSkyLineArray : public RefCountObject
{
   public:
    ...
    static std::vector<MCAuto<DataArrayIdType> > RetrieveVecIndex(
        const std::vector<MCAuto<MEDCouplingSkyLineArray> > &vecSka
    )
    {
    ...
    static std::vector<MCAuto<DataArrayIdType> > RetrieveVecValues(
        const std::vector<MCAuto<MEDCouplingSkyLineArray> > &vecSka
    )
    {
    ...
    static std::vector<MCAuto<DataArrayIdType> > RetrieveVecOfSkyLineArrayGen(
        const std::vector<MCAuto<MEDCouplingSkyLineArray> > &vecSka,
        std::function<DataArrayIdType *(MEDCouplingSkyLineArray *)> fct
    )
    {
        ...
        std::for_each(
            vecSka.begin(),
            vecSka.end(),
            [&it, fct](MCAuto<MEDCouplingSkyLineArray> elt) { *it++ = MCAuto<DataArrayIdType>::TakeRef(fct(elt)); }
        );
        return ret;
    }
...
class SkyLineArrayGenIterator
{
   public:
    ...
    explicit SkyLineArrayGenIterator(std::size_t num, std::vector<const MEDCouplingSkyLineArray *> *data)
        : _num(num), _data(data)
    {
    }
    SkyLineArrayGenIterator<T> &operator++()
    {
        ++_num;
        return *this;
    }
...
{
    const mcIdType *operator()(const MEDCouplingSkyLineArray *ska) { return ska->getIndex(); }
};

using SkyLineArrayIndexIterator = SkyLineArrayGenIterator<SkyLineArrayIndexPtrFunctor>;

struct SkyLineArrayValuesPtrFunctor
{
    const mcIdType *operator()(const MEDCouplingSkyLineArray *ska) { return ska->getValues(); }
};
