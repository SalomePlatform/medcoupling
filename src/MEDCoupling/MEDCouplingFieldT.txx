...
template <class T>
void
MEDCouplingFieldT<T>::checkConsistencyLight() const
{
...
MEDCouplingFieldT<T>::MEDCouplingFieldT(
    const MEDCouplingField &other, MEDCouplingTimeDiscretizationTemplate<T> *timeDiscr, bool deepCopy
)
...
/*!
 * This method synchronizes time information (time, iteration, order, time unit) regarding the information in \c
 * this->_mesh.
 * \throw If no mesh is set in this. Or if \a this is not compatible with time setting (typically NO_TIME)
 */
template <class T>
void
MEDCouplingFieldT<T>::synchronizeTimeWithMesh()
{
    if (!_mesh)
...
MEDCouplingFieldT<T>::cloneWithMesh(bool recDeepCpy) const
{
...
template <class T>
bool
MEDCouplingFieldT<T>::isEqual(const MEDCouplingFieldT<T> *other, double meshPrec, T valsPrec) const
{
    std::string tmp;
...
template <class T>
bool
MEDCouplingFieldT<T>::isEqualIfNotWhy(
...
template <class T>
bool
MEDCouplingFieldT<T>::isEqualWithoutConsideringStr(const MEDCouplingFieldT<T> *other, double meshPrec, T valsPrec) const
{
    if (!other)
        return false;
...
template <class T>
void
MEDCouplingFieldT<T>::copyTinyStringsFrom(const MEDCouplingField *other)
{
...
template <class T>
void
MEDCouplingFieldT<T>::copyTinyAttrFrom(const MEDCouplingFieldT<T> *other)
{
    if (other)
    {
...
template <class T>
void
MEDCouplingFieldT<T>::copyAllTinyAttrFrom(const MEDCouplingFieldT<T> *other)
{
...
template <class T>
void
MEDCouplingFieldT<T>::renumberCells(const mcIdType *old2NewBg, bool check)
{
...
template <class T>
void
MEDCouplingFieldT<T>::renumberCellsWithoutMesh(const mcIdType *old2NewBg, bool check)
{
    if (!_mesh)
    ...
    std::vector<DataArray *> arrays2(arrays.size());
    std::copy(arrays.begin(), arrays.end(), arrays2.begin());
...
template <class T>
bool
MEDCouplingFieldT<T>::areStrictlyCompatible(const MEDCouplingField *other) const
{
    std::string tmp;
...
template <class T>
bool
MEDCouplingFieldT<T>::areStrictlyCompatibleForMulDiv(const MEDCouplingField *other) const
{
...
template <class T>
bool
MEDCouplingFieldT<T>::areCompatibleForDiv(const MEDCouplingField *other) const
{
    if (!MEDCouplingField::areStrictlyCompatibleForMulDiv(other))
        return false;
    const MEDCouplingFieldT<T> *otherC(dynamic_cast<const MEDCouplingFieldT<T> *>(other));
...
template <class T>
bool
MEDCouplingFieldT<T>::areCompatibleForMul(const MEDCouplingField *other) const
{
    if (!MEDCouplingField::areStrictlyCompatibleForMulDiv(other))
        return false;
    const MEDCouplingFieldT<T> *otherC(dynamic_cast<const MEDCouplingFieldT<T> *>(other));
...
template <class T>
std::string
MEDCouplingFieldT<T>::simpleRepr() const
{
    ...
    if (getArray())
    {
        if (getArray()->isAllocated())
        {
            ...
            ret << Traits<T>::FieldTypeName << " default array has " << nbOfCompo << " components and "
                << getArray()->getNumberOfTuples() << " tuples.\n";
            ret << Traits<T>::FieldTypeName << " default array has following info on components : ";
            for (std::size_t i = 0; i < nbOfCompo; i++) ret << "\"" << getArray()->getInfoOnComponent(i) << "\" ";
            ret << "\n";
        }
        else
        {
...
template <class T>
void
MEDCouplingFieldT<T>::reprQuickOverview(std::ostream &stream) const
{
...
MEDCouplingFieldT<T>::getTimeDiscretization() const
{
 ...
 * \n This method makes the assumption that \a this field is correctly defined when this method is called (\a
 *this->checkConsistencyLight() returns without any exception thrown), **no check of this will be done**.
 * \n This method returns a restriction of \a this so that only tuple ids specified in [ \a partBg , \a partEnd ) will
 *be contained in the returned field.
 ...
 *
 * For example, \a this is a field on cells lying on a mesh that have 10 cells, \a partBg contains the following cell
 *ids [3,7,6]. Then the returned field will lie on mesh having 3 cells and will contain 3 tuples.
 *- Tuple #0 of the result field will refer to the cell #0 of returned mesh. The cell #0 of returned mesh will be equal
 *to the cell #3 of \a this->getMesh().
 *- Tuple #1 of the result field will refer to the cell #1 of returned mesh. The cell #1 of returned mesh will be equal
 *to the cell #7 of \a this->getMesh().
 *- Tuple #2 of the result field will refer to the cell #2 of returned mesh. The cell #2 of returned mesh will be equal
 *to the cell #6 of \a this->getMesh().
 *
 * Let, for example, \a this be a field on nodes lying on a mesh that have 10 cells and 11 nodes, and \a partBg contains
 *following cellIds [3,7,6]. Thus \a this currently contains 11 tuples. If the restriction of mesh to 3 cells leads to a
 *mesh with 6 nodes, then the returned field will contain 6 tuples and \a this field will lie on this restricted mesh.
 *
 ...
 *
 * \throw if there is presence of an invalid cell id in [ \a partBg, \a partEnd ) regarding the number of cells of \a
 *this->getMesh().
 *
 * \if ENABLE_EXAMPLES
 ...
 * \endif
 * \sa MEDCoupling::MEDCouplingFieldDouble::buildSubPart(const DataArrayInt *) const,
 *MEDCouplingFieldDouble::buildSubPartRange
 */
template <class T>
typename Traits<T>::FieldType *
MEDCouplingFieldT<T>::buildSubPart(const mcIdType *partBg, const mcIdType *partEnd) const
{
    if (_type.isNull())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingFieldT::buildSubPart : Expecting a not NULL spatial discretization !"
        );
    ...
    for (typename std::vector<typename Traits<T>::ArrayType *>::const_iterator iter = arrays.begin();
         iter != arrays.end();
         iter++)
    {
 ...
 * This method makes the assumption that the field is correctly defined when this method is called, no check of this
 * will be done. This method returns a restriction of \a this so that only tuples with ids specified in \a part will be
 * contained in the returned field. Parameter \a part specifies **cell ids whatever the spatial discretization of this**
 * (
 * \ref MEDCoupling::ON_CELLS "ON_CELLS",
 * \ref MEDCoupling::ON_NODES "ON_NODES",
 * \ref MEDCoupling::ON_GAUSS_PT "ON_GAUSS_PT",
 ...
 * \ref MEDCoupling::ON_NODES_FE "ON_NODES_FE").
 *
 * For example, \a this is a field on cells lying on a mesh that have 10 cells, \a part contains following cell ids
 * [3,7,6]. Then the returned field will lie on mesh having 3 cells and the returned field will contain 3 tuples.<br>
 * Tuple #0 of the result field will refer to the cell #0 of returned mesh. The cell #0 of returned mesh will be equal
 * to the cell #3 of \a this->getMesh().<br> Tuple #1 of the result field will refer to the cell #1 of returned mesh.
 * The cell #1 of returned mesh will be equal to the cell #7 of \a this->getMesh().<br> Tuple #2 of the result field
 * will refer to the cell #2 of returned mesh. The cell #2 of returned mesh will be equal to the cell #6 of \a
 * this->getMesh().
 *
 * Let, for example, \a this be a field on nodes lying on a mesh that have 10 cells and 11 nodes, and \a part contains
 * following cellIds [3,7,6]. Thus \a this currently contains 11 tuples. If the restriction of mesh to 3 cells leads to
 * a mesh with 6 nodes, then the returned field will contain 6 tuples and \a this field will lie on this restricted
 * mesh.
 *
 *  \param [in] part - an array of cell ids to include to the result field.
 *  \return MEDCouplingFieldDouble * - a new instance of MEDCouplingFieldDouble. The caller is to delete this field
...
template <class T>
typename Traits<T>::FieldType *
MEDCouplingFieldT<T>::buildSubPart(const DataArrayIdType *part) const
{
    if (part == 0)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingFieldT::buildSubPart : not empty array must be passed to this method !"
        );
    return buildSubPart(part->begin(), part->end());
}

/*!
 * This method is equivalent to MEDCouplingFieldDouble::buildSubPart, the only difference is that the input range of
 * cell ids is given using a range given \a begin, \a end and \a step to optimize the part computation.
 *
 * \sa MEDCouplingFieldDouble::buildSubPart
 */
template <class T>
typename Traits<T>::FieldType *
MEDCouplingFieldT<T>::buildSubPartRange(mcIdType begin, mcIdType end, mcIdType step) const
{
    if (_type.isNull())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingFieldDouble::buildSubPart : Expecting a not NULL spatial discretization !"
        );
    DataArrayIdType *arrSelect;
    mcIdType beginOut, endOut, stepOut;
    MCAuto<MEDCouplingMesh> m(
        _type->buildSubMeshDataRange(_mesh, begin, end, step, beginOut, endOut, stepOut, arrSelect)
    );
    MCAuto<DataArrayIdType> arrSelect2(arrSelect);
    MCAuto<typename Traits<T>::FieldType> ret(clone(false));  // quick shallow copy.
    const MEDCouplingFieldDiscretization *disc = getDiscretization();
...
template <class T>
const MEDCouplingTimeDiscretizationTemplate<T> *
MEDCouplingFieldT<T>::timeDiscrSafe() const
{
...
template <class T>
MEDCouplingTimeDiscretizationTemplate<T> *
MEDCouplingFieldT<T>::timeDiscrSafe()
{
...
template <class T>
void
MEDCouplingFieldT<T>::getTinySerializationStrInformation(std::vector<std::string> &tinyInfo) const
{
    tinyInfo.clear();
...
template <class T>
void
MEDCouplingFieldT<T>::getTinySerializationIntInformation(std::vector<mcIdType> &tinyInfo) const
{
    if (_type.isNull())
        throw INTERP_KERNEL::Exception(
            "No spatial discretization underlying this field to perform getTinySerializationIntInformation !"
        );
    tinyInfo.clear();
...
template <class T>
void
MEDCouplingFieldT<T>::getTinySerializationDbleInformation(std::vector<double> &tinyInfo) const
{
    if (_type.isNull())
        throw INTERP_KERNEL::Exception(
            "No spatial discretization underlying this field to perform getTinySerializationDbleInformation !"
        );
    tinyInfo.clear();
 ...
 * @param tinyInfoI is the value retrieves from distant result of getTinySerializationIntInformation on source instance
 * to be copied.
 * @param dataInt out parameter. If not null the pointer is already owned by \a this after the call of this method. In
 * this case no decrRef must be applied.
 * @param arrays out parameter is a vector resized to the right size. The pointers in the vector is already owned by \a
 * this after the call of this method. No decrRef must be applied to every instances in returned vector.
 * \sa checkForUnserialization
 */
template <class T>
void
MEDCouplingFieldT<T>::resizeForUnserialization(
...
)
{
    if (_type.isNull())
        throw INTERP_KERNEL::Exception(
            "No spatial discretization underlying this field to perform resizeForUnserialization !"
        );
    dataInt = 0;
...
/*!
 * This method is extremely close to resizeForUnserialization except that here the arrays in \a dataInt and in \a arrays
 * are attached in \a this after having checked that size is correct. This method is used in python pickeling context to
 * avoid copy of data.
 * \sa resizeForUnserialization
 */
template <class T>
void
MEDCouplingFieldT<T>::checkForUnserialization(
    const std::vector<mcIdType> &tinyInfoI,
...
template <class T>
void
MEDCouplingFieldT<T>::finishUnserialization(
    const std::vector<mcIdType> &tinyInfoI,
...
)
{
    if (_type.isNull())
        throw INTERP_KERNEL::Exception(
            "No spatial discretization underlying this field to perform finishUnserialization !"
        );
...
template <class T>
void
MEDCouplingFieldT<T>::serialize(DataArrayIdType *&dataInt, std::vector<typename Traits<T>::ArrayType *> &arrays) const
{
    if (_type.isNull())
