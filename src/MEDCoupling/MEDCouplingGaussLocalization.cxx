...
MEDCouplingGaussLocalization::MEDCouplingGaussLocalization(
...
MEDCouplingGaussLocalization::MEDCouplingGaussLocalization(INTERP_KERNEL::NormalizedCellType typ) : _type(typ)
{
...
void
MEDCouplingGaussLocalization::setType(INTERP_KERNEL::NormalizedCellType typ)
{
...
void
MEDCouplingGaussLocalization::checkConsistencyLight() const
{
    ...
    if (!cm.isDynamic())
    {
        if (ToIdType(_ref_coord.size()) != nbNodes * dim)
        {
            ...
            oss << "Invalid size of refCoo : expecting to be : " << nbNodes << " (nbNodePerCell) * " << dim
                << " (dim) !";
    ...
    {
        std::ostringstream oss;
        oss << "Invalid gsCoo size and weight size : gsCoo.size() must be equal to _weight.size() * " << dim
            << " (dim) !";
        throw INTERP_KERNEL::Exception(oss.str().c_str());
    }
}
...
int
MEDCouplingGaussLocalization::getDimension() const
{
    if (_weight.empty())
...
int
MEDCouplingGaussLocalization::getNumberOfPtsInRefCell() const
{
...
std::string
MEDCouplingGaussLocalization::getStringRepr() const
{
    std::ostringstream oss;
    oss << "CellType : " << INTERP_KERNEL::CellModel::GetCellModel(_type).getRepr() << std::endl;
    oss << "Ref coords : ";
    std::copy(_ref_coord.begin(), _ref_coord.end(), std::ostream_iterator<double>(oss, ", "));
    oss << std::endl;
    oss << "Localization coords : ";
    std::copy(_gauss_coord.begin(), _gauss_coord.end(), std::ostream_iterator<double>(oss, ", "));
    oss << std::endl;
    oss << "Weight : ";
    std::copy(_weight.begin(), _weight.end(), std::ostream_iterator<double>(oss, ", "));
    oss << std::endl;
    return oss.str();
}

std::size_t
MEDCouplingGaussLocalization::getMemorySize() const
{
    std::size_t ret = 0;
...
bool
MEDCouplingGaussLocalization::isEqual(const MEDCouplingGaussLocalization &other, double eps) const
{
...
double
MEDCouplingGaussLocalization::getRefCoord(int ptIdInCell, int comp) const
{
    const INTERP_KERNEL::CellModel &cm = INTERP_KERNEL::CellModel::GetCellModel(_type);
    int nbNodes = cm.getNumberOfNodes();
    int dim = cm.getDimension();
...
double
MEDCouplingGaussLocalization::getGaussCoord(int gaussPtIdInCell, int comp) const
{
...
double
MEDCouplingGaussLocalization::getWeight(int gaussPtIdInCell) const
{
 ...
 */
void
MEDCouplingGaussLocalization::pushTinySerializationIntInfo(std::vector<mcIdType> &tinyInfo) const
{
...
/*!
 * Completely useless method for end user. Only for CORBA MPI serialization/unserialization.
 * push at the end of tinyInfo its basic serialization info. The size of pushed data is \b NOT always the same contrary
 * to pushTinySerializationIntInfo.
 * @param tinyInfo inout parameter.
 */
void
MEDCouplingGaussLocalization::pushTinySerializationDblInfo(std::vector<double> &tinyInfo) const
{
...
/*!
 * This method operates the exact inverse operation than MEDCouplingGaussLocalization::pushTinySerializationDblInfo
 * method. This is one of the last step of unserialization process. This method should be called on an object resized by
 * buildNewInstanceFromTinyInfo static method. This method takes in argument a pointer 'vals' that point to the begin of
 * double data pushed remotely by pushTinySerializationDblInfo method. This method returns the pointer 'vals' with an
 * offset of size what it has been read in this method.
 */
const double *
MEDCouplingGaussLocalization::fillWithValues(const double *vals)
{
...
MEDCouplingGaussLocalization::localizePtsInRefCooForEachCell(
    const DataArrayDouble *ptsInRefCoo, const MEDCouplingUMesh *mesh
) const
{
    if (!ptsInRefCoo || !mesh)
    ...
    if (dim != ToIdType(ptsInRefCoo->getNumberOfComponents()))
        throw INTERP_KERNEL::Exception(
            "MEDCouplingGaussLocalization::localizePtsInRefCooForEachCell : number of components of input coo is not "
            "equal to dim of element !"
        );
...
MEDCouplingGaussLocalization::buildRefCell() const
{
...
/*!
 * This method returns an array containing for each Gauss Points in \a this, function values relative to the points of
 * the reference cell. Number of components of returned array is equal to the number of points of the reference cell.
 */
MCAuto<DataArrayDouble>
MEDCouplingGaussLocalization::getShapeFunctionValues() const
{
    MCAuto<DataArrayDouble> ret(DataArrayDouble::New());
    ...
    {
        std::vector<double> curGaussPt(
            _gauss_coord.begin() + iGaussPt * dim, _gauss_coord.begin() + (iGaussPt + 1) * dim
        );
...
MCAuto<DataArrayDouble>
MEDCouplingGaussLocalization::getDerivativeOfShapeFunctionValues() const
{
    MCAuto<DataArrayDouble> ret(DataArrayDouble::New());
    int nbGaussPt(getNumberOfGaussPt()), nbPtsRefCell(getNumberOfPtsInRefCell()), dim(getDimension());
 ...
 */
void
MEDCouplingGaussLocalization::setRefCoord(int ptIdInCell, int comp, double newVal)
{
    const INTERP_KERNEL::CellModel &cm = INTERP_KERNEL::CellModel::GetCellModel(_type);
    int nbNodes = cm.getNumberOfNodes();
    int dim = cm.getDimension();
...
void
MEDCouplingGaussLocalization::setGaussCoord(int gaussPtIdInCell, int comp, double newVal)
{
    int dim = checkCoherencyOfRequest(gaussPtIdInCell, comp);
...
void
MEDCouplingGaussLocalization::setWeight(int gaussPtIdInCell, double newVal)
{
    checkCoherencyOfRequest(gaussPtIdInCell, 0);
...
void
MEDCouplingGaussLocalization::setRefCoords(const std::vector<double> &refCoo)
{
    _ref_coord = refCoo;
}

void
MEDCouplingGaussLocalization::setGaussCoords(const std::vector<double> &gsCoo)
{
    _gauss_coord = gsCoo;
}

void
MEDCouplingGaussLocalization::setWeights(const std::vector<double> &w)
{
    _weight = w;
}

/*!
 * The format of 'tinyData' parameter is the same than pushed in method
 * MEDCouplingGaussLocalization::pushTinySerializationIntInfo.
 */
MEDCouplingGaussLocalization
MEDCouplingGaussLocalization::BuildNewInstanceFromTinyInfo(mcIdType dim, const std::vector<mcIdType> &tinyData)
{
...
int
MEDCouplingGaussLocalization::checkCoherencyOfRequest(mcIdType gaussPtIdInCell, int comp) const
{
    const INTERP_KERNEL::CellModel &cm = INTERP_KERNEL::CellModel::GetCellModel(_type);
    int dim = cm.getDimension();
...
bool
MEDCouplingGaussLocalization::AreAlmostEqual(const std::vector<double> &v1, const std::vector<double> &v2, double eps)
{
...
MCAuto<DataArrayDouble>
MEDCouplingGaussLocalization::GetDefaultReferenceCoordinatesOf(INTERP_KERNEL::NormalizedCellType type)
{
