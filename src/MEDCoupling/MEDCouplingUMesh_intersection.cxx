...
mcIdType
InternalAddPoint(
    const INTERP_KERNEL::Edge *e,
    mcIdType id,
    const double *coo,
    mcIdType startId,
    mcIdType endId,
    DataArrayDouble &addCoo,
    mcIdType &nodesCnter
)
{
    if (id != -1)
...
EnterTheResultOf2DCellFirst(
    ...
    bool linOrArc,
    const double *coords,
    const mcIdType *connBg,
    mcIdType offset,
    DataArrayIdType *newConnOfCell,
    DataArrayDouble *appendedCoords,
    std::vector<mcIdType> &middles
)
{
    mcIdType tmp[3];
    int trueStart(start >= 0 ? start : nbOfEdges + start);
    tmp[0] = ToIdType(linOrArc ? INTERP_KERNEL::NORM_QPOLYG : INTERP_KERNEL::NORM_POLYGON);
    tmp[1] = connBg[trueStart];
    tmp[2] = connBg[stp];
...
void
IKGeo2DInternalMapper2(
    INTERP_KERNEL::Node *n,
    const std::map<MCAuto<INTERP_KERNEL::Node>, mcIdType> &m,
    mcIdType forbVal0,
    mcIdType forbVal1,
    std::vector<mcIdType> &isect
)
{
    MCAuto<INTERP_KERNEL::Node> nTmp(n);
    nTmp->incrRef();
...
bool
IKGeo2DInternalMapper(
    const INTERP_KERNEL::ComposedEdge &c,
    const std::map<MCAuto<INTERP_KERNEL::Node>, mcIdType> &m,
    mcIdType forbVal0,
    mcIdType forbVal1,
    std::vector<mcIdType> &isect
...
INTERP_KERNEL::Edge *
MEDCouplingUMeshBuildQPFromEdge2(
    INTERP_KERNEL::NormalizedCellType typ,
    const mcIdType *bg,
    const double *coords2D,
    std::map<MCAuto<INTERP_KERNEL::Node>, mcIdType> &m
)
{
    INTERP_KERNEL::Edge *ret(0);
    MCAuto<INTERP_KERNEL::Node> n0(new INTERP_KERNEL::Node(coords2D[2 * bg[0]], coords2D[2 * bg[0] + 1])),
        n1(new INTERP_KERNEL::Node(coords2D[2 * bg[1]], coords2D[2 * bg[1] + 1]));
    m[n0] = bg[0];
    m[n1] = bg[1];
    switch (typ)
    {
        ...
        {
            INTERP_KERNEL::Node *n2(new INTERP_KERNEL::Node(coords2D[2 * bg[2]], coords2D[2 * bg[2] + 1]));
            m[n2] = bg[2];
        ...
        default:
            throw INTERP_KERNEL::Exception(
                "MEDCouplingUMeshBuildQPFromEdge2 : Expecting a mesh with spaceDim==2 and meshDim==1 !"
            );
    }
    return ret;
}
...
INTERP_KERNEL::Edge *
MEDCouplingUMeshBuildQPFromEdge(
    INTERP_KERNEL::NormalizedCellType typ, std::map<mcIdType, INTERP_KERNEL::NodeWithUsage> &mapp2, const mcIdType *bg
)
{
    INTERP_KERNEL::Edge *ret = 0;
    ...
    switch (typ)
    {
        ...
        case INTERP_KERNEL::NORM_SEG3:
        {
            ...
            if (mapp2[bg[2]].second != INTERP_KERNEL::USAGE_LINEAR)  // switch the node usage to quadratic only if it is
                                                                     // not used as an extreme point for another edge
                mapp2[bg[2]].second = INTERP_KERNEL::USAGE_QUADRATIC_ONLY;
            break;
        }
        default:
            throw INTERP_KERNEL::Exception(
                "MEDCouplingUMeshBuildQPFromEdge : Expecting a mesh with spaceDim==2 and meshDim==1 !"
            );
    }
    return ret;
}
...
/*!
 * This method creates a sub mesh in Geometric2D DS. The sub mesh is composed by the sub set of cells in 'candidates'
 * taken from the global mesh 'mDesc'. The input mesh 'mDesc' must be so that mDim==1 and spaceDim==2. 'mapp' returns a
 * mapping between local numbering in submesh (represented by a Node*) and the global node numbering in 'mDesc'.
 */
INTERP_KERNEL::QuadraticPolygon *
MEDCouplingUMeshBuildQPFromMesh(
    const MEDCouplingUMesh *mDesc,
    const std::vector<mcIdType> &candidates,
    std::map<INTERP_KERNEL::Node *, mcIdType> &mapp
)
{
    mapp.clear();
    std::map<mcIdType, INTERP_KERNEL::NodeWithUsage>
        mapp2;  // the last var is a flag specifying if node is an extreme node of the seg (LINEAR) or only a middle for
                // SEG3 (QUADRATIC_ONLY).
    ...
    for (std::map<mcIdType, INTERP_KERNEL::NodeWithUsage>::const_iterator it2 = mapp2.begin(); it2 != mapp2.end();
         it2++)
    {
...
INTERP_KERNEL::QuadraticPolygon *
MEDCouplingUMeshBuildQPFromMeshWithTree(
    const MEDCouplingUMesh *mDesc,
    const std::vector<mcIdType> &candidates,
    std::map<INTERP_KERNEL::Node *, mcIdType> &mapp,
...
)
{
    ...
    for (std::set<mcIdType>::const_iterator it2 = s.begin(); it2 != s.end(); it2++)
    {
        ...
        if (candNode.size() > 2)
            throw INTERP_KERNEL::Exception(
                "MEDCouplingUMesh::MEDCouplingUMeshBuildQPFromMeshWithTree(): some nodes are not properly merged "
                "(within eps) in input mesh!"
            );
        bool node_created = false;
        if (candNode.size())
        {
            auto itt = mapRev.find(candNode[0]);
            if (itt != mapRev.end())  // we might hit a node which is in the coords array but not used in the
                                      // connectivity in which case it won't be in the revMap
            {
                node_created = true;
                n = (*itt).second;
                n->incrRef();
    ...
    for (std::vector<mcIdType>::const_iterator it = candidates.begin(); it != candidates.end(); it++)
    {
        INTERP_KERNEL::NormalizedCellType typ = (INTERP_KERNEL::NormalizedCellType)c[cI[*it]];
        ret->pushBack(
            MEDCouplingUMeshBuildQPFromEdge(typ, mapp2, c + cI[*it] + 1)
        );  // this call will set quad points to false in the map
    }
    for (std::map<mcIdType, INTERP_KERNEL::NodeWithUsage>::const_iterator it2 = mapp2.begin(); it2 != mapp2.end();
         it2++)
    {
...
MEDCouplingUMeshBuildQPFromMesh3(
    ...
    mcIdType offset2,
    const std::vector<double> &addCoo,
    const mcIdType *desc1Bg,
    const mcIdType *desc1End,
    const std::vector<std::vector<mcIdType> > &intesctEdges1,
    /*output*/ std::map<INTERP_KERNEL::Node *, mcIdType> &mapp,
...
)
{
    for (const mcIdType *desc1 = desc1Bg; desc1 != desc1End; desc1++)
    {
        ...
        for (std::vector<mcIdType>::const_iterator it1 = intesctEdges1[eltId1].begin();
             it1 != intesctEdges1[eltId1].end();
             it1++)
        {
...
/*!
 * Returns true if a colinearization has been found in the given cell. If false is returned the content pushed in \a
 * newConnOfCell is equal to [ \a connBg , \a connEnd ) .
 * \a appendedCoords is a DataArrayDouble instance with number of components equal to one (even if the items are pushed
 * by pair).
 * \param forbiddenPoints the list of points that should not be removed in the process
 */
bool
MEDCouplingUMesh::Colinearize2DCell(
    const double *coords,
    const mcIdType *connBg,
    const mcIdType *connEnd,
    mcIdType offset,
...
)
{
    ...
    const INTERP_KERNEL::CellModel &cm(
        INTERP_KERNEL::CellModel::GetCellModel((INTERP_KERNEL::NormalizedCellType)connBg[0])
    );
    ...
    const std::size_t maxNbOfHit =
        cm.isQuadratic()
            ? nbs - 2
            : nbs - 3;  // a quad cell is authorized to end up with only two edges, a linear one has to keep 3 at least
    ...
    for (; (nbOfTurn + nbOfHit) < nbs; nbOfTurn++)
    {
        ...
        if (nbOfTurn == 0)
        {
            for (unsigned i = 1; i < nbs && nbOfHit < maxNbOfHit;
                 i++)  // 2nd condition is to avoid ending with a cell with one single edge
            {
                cm.fillSonCellNodalConnectivity2(nbs - i, connBg + 1, ToIdType(sz), tmpConn2, typeOfSon);
                // Identify common point:
                mcIdType commPoint =
                    std::find((mcIdType *)tmpConn, tmpConn + 2, tmpConn2[0]) != tmpConn + 2 ? tmpConn2[0] : tmpConn2[1];
                auto itE(forbiddenPoints.end());
                if (forbiddenPoints.find(commPoint) !=
                    itE)  // is the junction point in the list of points we can not remove?
                    break;
        ...
        for (unsigned j = fwdStart + 1; j < nbs && nbOfHit < maxNbOfHit;
             j++)  // 2nd condition is to avoid ending with a cell with one single edge
        {
            cm.fillSonCellNodalConnectivity2(
                j, connBg + 1, ToIdType(sz), tmpConn2, typeOfSon
            );  // get edge #j's connectivity
            // Identify common point:
            mcIdType commPoint =
                std::find((mcIdType *)tmpConn, tmpConn + 2, tmpConn2[0]) != tmpConn + 2 ? tmpConn2[0] : tmpConn2[1];
            auto itE(forbiddenPoints.end());
        ...
        //  The if clauses below are (voluntary) not mutually exclusive: on a quad cell with 2 edges, the end of the
        //  connectivity is also its beginning!
        if (nbOfTurn == 0)
 ...
 *  - subDiv of size 'm2->getNumberOfCells()' that lists for each seg cell in 'm2' the splitting node ids randomly
 * sorted. The aim of this method is to sort the splitting nodes, if any, and to put them in 'intersectEdge' output
 * parameter based on edges of mesh 'm2' Nodes end up lying consecutively on a cutted edge.
 * \param m1 is expected to be a mesh of meshDimension equal to 1 and spaceDim equal to 2. No check of that is performed
 * by this method. (Only present for its coords in case of 'subDiv' shares some nodes of 'm1')
 * \param m2 is expected to be a mesh of meshDimension equal to 1 and spaceDim equal to 2. No check of that is performed
 * by this method.
 ...
 */
void
MEDCouplingUMesh::BuildIntersectEdges(
    const MEDCouplingUMesh *m1,
    const MEDCouplingUMesh *m2,
    const std::vector<double> &addCoo,
    const std::vector<std::vector<mcIdType> > &subDiv,
    std::vector<std::vector<mcIdType> > &intersectEdge
)
{
    ...
    for (mcIdType i = 0; i < ncell2; i++, cI++)
    {
        ...
        INTERP_KERNEL::Edge *e =
            MEDCouplingUMeshBuildQPFromEdge((INTERP_KERNEL::NormalizedCellType)c[*cI], mapp2, c + (*cI) + 1);
        for (std::map<mcIdType, INTERP_KERNEL::NodeWithUsage>::const_iterator it = mapp2.begin(); it != mapp2.end();
             it++)
        ...
        for (std::size_t j = 0; j < divs.size(); j++)
        {
            ...
            if (id < offset1)
                ...
                tmp = new INTERP_KERNEL::Node(
                    coo[2 * (id - offset1)], coo[2 * (id - offset1) + 1]
                );  // if it happens, bad news mesh 'm2' is non conform.
            else
...
/*
 *  Build the final 1D mesh resulting from the newly created points after intersection with the segments of the
 * descending 2D mesh.
 *  @param[out] idsInRetColinear IDs of edges in the result (ret) that are colinears to one of the segment of the
 * descending 2D mesh. Indexing scheme is the one of the ret 1D mesh.
 *  @param[out] idsInMesh1DForIdsInRetColinear same IDs as above in the descending 2D mesh
 */
MEDCouplingUMesh *
BuildMesh1DCutFrom(
    const MEDCouplingUMesh *mesh1D,
    const std::vector<std::vector<mcIdType> > &intersectEdge2,
    const DataArrayDouble *coords1,
    const std::vector<double> &addCoo,
    const std::map<mcIdType, mcIdType> &mergedNodes,
    const std::vector<std::vector<mcIdType> > &colinear2,
    const std::vector<std::vector<mcIdType> > &intersectEdge1,
    MCAuto<DataArrayIdType> &idsInRetColinear,
    MCAuto<DataArrayIdType> &idsInMesh1DForIdsInRetColinear
)
{
    idsInRetColinear = DataArrayIdType::New();
    idsInRetColinear->alloc(0, 1);
    idsInMesh1DForIdsInRetColinear = DataArrayIdType::New();
    idsInMesh1DForIdsInRetColinear->alloc(0, 1);
    ...
    MCAuto<DataArrayIdType> cOut(DataArrayIdType::New()), ciOut(DataArrayIdType::New());
    cOut->alloc(0, 1);
    ciOut->alloc(1, 1);
    ciOut->setIJ(0, 0, 0);
    ...
    for (mcIdType i = 0; i < nCells; i++)
    {
        std::map<MCAuto<INTERP_KERNEL::Node>, mcIdType> m;
        INTERP_KERNEL::Edge *e(
            MEDCouplingUMeshBuildQPFromEdge2((INTERP_KERNEL::NormalizedCellType)c[ci[i]], c + ci[i] + 1, coo2Ptr, m)
        );
        ...
        {
            MCAuto<INTERP_KERNEL::Node> n1(
                MEDCouplingUMeshBuildQPNode(subEdges[2 * j], coords1->begin(), offset1, coo2Ptr, offset2, addCoo)
            ),
                n2(MEDCouplingUMeshBuildQPNode(
                    subEdges[2 * j + 1], coords1->begin(), offset1, coo2Ptr, offset2, addCoo
                ));
            ...
            if (dynamic_cast<INTERP_KERNEL::EdgeArcCircle *>(e2Ptr))
            {
                ...
                double tmp2[2];
                e2->getBarycenter(tmp2);
                addCooQuad.insert(addCooQuad.end(), tmp2, tmp2 + 2);
                cicnt += 4;
    ...
    MCAuto<DataArrayDouble> arr4(DataArrayDouble::New());
    arr4->useArray(&addCooQuad[0], false, DeallocType::C_DEALLOC, addCooQuad.size() / 2, 2);
    std::vector<const DataArrayDouble *> coordss(4);
    coordss[0] = coords1;
    coordss[1] = mesh1D->getCoords();
    coordss[2] = arr3;
    coordss[3] = arr4;
...
)
{
    ...
    for (std::size_t kk = 0; kk < nbOfEdgesOf2DCellSplit; kk++) connOut[kk] = allEdges[2 * kk];
...
)
{
    ...
    const INTERP_KERNEL::CellModel &cm(
        INTERP_KERNEL::CellModel::GetCellModel((INTERP_KERNEL::NormalizedCellType)c[ci[cellIdInMesh2D]])
    );
    int ii(0);
    ...
    MCAuto<DataArrayDouble> addCoo(DataArrayDouble::New());
    addCoo->alloc(0, 1);
    ...
    for (const mcIdType *it2(descBg); it2 != descEnd; it2++, ii++)
    {
        INTERP_KERNEL::NormalizedCellType typeOfSon;
        cm.fillSonCellNodalConnectivity2(
            ii, c + ci[cellIdInMesh2D] + 1, ci[cellIdInMesh2D + 1] - ci[cellIdInMesh2D] - 1, tmpPtr, typeOfSon
        );
        const std::vector<mcIdType> &edge1(intersectEdge1[std::abs(*it2) - 1]);
        if (*it2 > 0)
            allEdges.insert(allEdges.end(), edge1.begin(), edge1.end());
...
/*!
 * This method creates a refinement of a cell in \a mesh2D. Those cell is defined by descending connectivity and the
 * sorted subdivided nodal connectivity of those edges.
 *
 * \param [in] mesh2D - The origin 2D mesh. \b Warning \b coords are not those of \a mesh2D. But
 * mesh2D->getCoords()==coords[:mesh2D->getNumberOfNodes()]
 */
MEDCouplingUMesh *
BuildRefined2DCell(
    const DataArrayDouble *coords,
...
void
AddCellInMesh2D(
    MEDCouplingUMesh *mesh2D, const std::vector<mcIdType> &conn, const std::vector<MCAuto<INTERP_KERNEL::Edge> > &edges
)
{
    bool isQuad(false);
    ...
    else
    {
        ...
        {
            double tmp[2];
            edges[(i + 1) % sz]->getMiddleOfPoints(
                coo + 2 * conn[i], coo + 2 * conn[(i + 1) % sz], tmp
            );  // tony a chier i+1 -> i
 ...
 *
 * This method cuts in 2 parts the input 2D cell given using boundaries description (\a edge1Bis and \a edge1BisPtr)
 * using a set of edges defined in \a splitMesh1D.
 */
void
BuildMesh2DCutInternal2(
    const MEDCouplingUMesh *splitMesh1D,
    const std::vector<mcIdType> &edge1Bis,
    const std::vector<MCAuto<INTERP_KERNEL::Edge> > &edge1BisPtr,
...
)
{
    ...
    if (iEnd == 0)
        throw INTERP_KERNEL::Exception(
            "BuildMesh2DCutInternal2 : internal error ! input 1D mesh must have at least one cell !"
        );
    std::size_t ii, jj;
    ...
    else
    {
        ...
        std::vector<mcIdType> &connOutRight(
            out0[1]
        );  // connOutLeft should end with edge1Bis[2*ii] and connOutRight should end with edge1Bis[2*jj+1]
        ...
        for (std::size_t k = ii; k < jj + 1; k++)
        {
            ...
            eleft.push_back(edge1BisPtr[2 * k + 1]);
        }
        std::vector<MCAuto<INTERP_KERNEL::Edge> > ees(iEnd);
        for (mcIdType ik = 0; ik < iEnd; ik++)
        {
            std::map<MCAuto<INTERP_KERNEL::Node>, mcIdType> m;
            MCAuto<INTERP_KERNEL::Edge> ee(MEDCouplingUMeshBuildQPFromEdge2(
                (INTERP_KERNEL::NormalizedCellType)cSplitPtr[ciSplitPtr[ik]],
                cSplitPtr + ciSplitPtr[ik] + 1,
                splitMesh1D->getCoords()->begin(),
                m
            ));
            ees[ik] = ee;
        }
        for (mcIdType ik = iEnd - 1; ik >= 0; ik--) connOutLeft.push_back(cSplitPtr[ciSplitPtr[ik] + 1]);
        for (std::size_t k = jj + 1; k < nbOfEdgesOf2DCellSplit + ii; k++)
        {
            connOutRight.push_back(edge1Bis[2 * k + 1]);
            eright.push_back(edge1BisPtr[2 * k + 1]);
        }
        eleft.insert(eleft.end(), ees.rbegin(), ees.rend());
        for (mcIdType ik = 0; ik < iEnd; ik++) connOutRight.push_back(cSplitPtr[ciSplitPtr[ik] + 2]);
...
CellInfo::CellInfo(const std::vector<mcIdType> &edges, const std::vector<MCAuto<INTERP_KERNEL::Edge> > &edgesPtr)
{
    ...
    std::vector<mcIdType> edges2(2 * nbe);
    std::vector<MCAuto<INTERP_KERNEL::Edge> > edgesPtr2(2 * nbe);
    for (std::size_t i = 0; i < nbe; i++)
    {
        edges2[2 * i] = edges[i];
        edges2[2 * i + 1] = edges[(i + 1) % nbe];
        edgesPtr2[2 * i] = edgesPtr[(i + 1) % nbe];
        edgesPtr2[2 * i + 1] = edgesPtr[(i + 1) % nbe];  // tony a chier
    }
    _edges.resize(4 * nbe);
    _edges_ptr.resize(4 * nbe);
    std::copy(edges2.begin(), edges2.end(), _edges.begin());
    std::copy(edges2.begin(), edges2.end(), _edges.begin() + 2 * nbe);
    std::copy(edgesPtr2.begin(), edgesPtr2.end(), _edges_ptr.begin());
    std::copy(edgesPtr2.begin(), edgesPtr2.end(), _edges_ptr.begin() + 2 * nbe);
}

class EdgeInfo
{
   public:
    EdgeInfo(mcIdType istart, mcIdType iend, const MCAuto<MEDCouplingUMesh> &mesh)
        : _istart(istart), _iend(iend), _mesh(mesh), _left(-7), _right(-7)
    {
    }
    EdgeInfo(mcIdType istart, mcIdType iend, mcIdType pos, const MCAuto<INTERP_KERNEL::Edge> &edge)
        : _istart(istart), _iend(iend), _edge(edge), _left(pos), _right(pos + 1)
    {
    }
    bool isInMyRange(mcIdType pos) const { return pos >= _istart && pos < _iend; }
    void somethingHappendAt(
        mcIdType pos,
        const std::vector<MCAuto<INTERP_KERNEL::Edge> > &newLeft,
        const std::vector<MCAuto<INTERP_KERNEL::Edge> > &newRight
    );
...
)
{
    ...
    if (_right == pos)
    {
        bool isLeft(std::find(newLeft.begin(), newLeft.end(), _edge) != newLeft.end()),
            isRight(std::find(newRight.begin(), newRight.end(), _edge) != newRight.end());
...
void
EdgeInfo::feedEdgeInfoAt(double eps, const MEDCouplingUMesh *mesh2D, mcIdType offset, mcIdType neighbors[2]) const
{
    const MEDCouplingUMesh *mesh(_mesh);
    if (!mesh)
    {
        neighbors[0] = offset + _left;
        neighbors[1] = offset + _right;
    }
    else
    {  // not fully splitting cell case
        ...
        else
        {
            ...
            neighbors[0] = offset + cellId;
            neighbors[1] = offset + cellId;
        }
    }
}
...
class VectorOfCellInfo
{
   public:
    ...
    void setMeshAt(
        mcIdType pos,
        const MCAuto<MEDCouplingUMesh> &mesh,
        mcIdType istart,
        mcIdType iend,
        const MCAuto<MEDCouplingUMesh> &mesh1DInCase,
        const std::vector<std::vector<mcIdType> > &edges,
   ...
   private:
    std::vector<CellInfo>
        _pool;  // for a newly created 2D cell, the list of edges ToIdType( and edges ptr constiuing it
    MCAuto<MEDCouplingUMesh> _ze_mesh;  // the aggregated mesh
    std::vector<EdgeInfo> _edge_info;   // for each new edge added when cuting the 2D cell, the information on left and
                                        // right bordering 2D cell
};

VectorOfCellInfo::VectorOfCellInfo(
    const std::vector<mcIdType> &edges, const std::vector<MCAuto<INTERP_KERNEL::Edge> > &edgesPtr
)
    : _pool(1)
{
...
mcIdType
VectorOfCellInfo::getPositionOf(double eps, const MEDCouplingUMesh *mesh) const
{
    if (_pool.empty())
...
)
{
    ...
    for (mcIdType i = 0; i < pos; i++) pool[i] = _pool[i];
    for (std::size_t j = 0; j < sz; j++) pool[pos + j] = CellInfo(edges[j], edgePtrs[j]);
    for (std::size_t i = pos + 1; i < _pool.size(); i++) pool[i + sz - 1] = _pool[i];
    _pool = pool;
    //
    if (sz == 2)
    ...
    {
        MCAuto<MEDCouplingUMesh> elt(
            static_cast<MEDCouplingUMesh *>(
                _ze_mesh->buildPartOfMySelfSlice(pos + 1, _ze_mesh->getNumberOfCells(), true)
            )
        );
        ms.push_back(elt);
    }
    std::vector<const MEDCouplingUMesh *> ms2(ms.size());
    for (std::size_t j = 0; j < ms2.size(); j++) ms2[j] = ms[j];
    _ze_mesh = MEDCouplingUMesh::MergeUMeshesOnSameCoords(ms2);
}

void
VectorOfCellInfo::feedEdgeInfoAt(double eps, mcIdType pos, mcIdType offset, mcIdType neighbors[2]) const
{
    _edge_info[getZePosOfEdgeGivenItsGlobalId(pos)].feedEdgeInfoAt(eps, _ze_mesh, offset, neighbors);
}

mcIdType
VectorOfCellInfo::getZePosOfEdgeGivenItsGlobalId(mcIdType pos) const
{
    if (pos < 0)
...
void
VectorOfCellInfo::updateEdgeInfo(
    mcIdType pos,
    const std::vector<MCAuto<INTERP_KERNEL::Edge> > &newLeft,
    const std::vector<MCAuto<INTERP_KERNEL::Edge> > &newRight
)
{
    ...
    for (std::size_t i = 0; i < sz; i++) _edge_info[i].somethingHappendAt(pos, newLeft, newRight);
}

const CellInfo &
VectorOfCellInfo::get(mcIdType pos) const
{
...
CellInfo &
VectorOfCellInfo::get(mcIdType pos)
{
    if (pos < 0 || pos >= ToIdType(_pool.size()))
 ...
 *
 * Algorithm : \a splitMesh1D is cut into contiguous parts. Each contiguous parts will build incrementally the output 2D
 * cells.
 *
 * \param [in] allEdges a list of pairs (beginNode, endNode). Represents all edges (already cut) in the single 2D cell
 * being handled here. Linked with \a allEdgesPtr to get the equation of edge.
 */
MEDCouplingUMesh *
BuildMesh2DCutInternal(
    double eps,
    MEDCouplingUMesh *splitMesh1D,
    const std::vector<mcIdType> &allEdges,
    const std::vector<MCAuto<INTERP_KERNEL::Edge> > &allEdgesPtr,
    mcIdType offset,
...
)
{
    ...
    if (nbCellsInSplitMesh1D == 0)
        throw INTERP_KERNEL::Exception(
            "BuildMesh2DCutInternal : internal error ! input 1D mesh must have at least one cell !"
        );
    const mcIdType *cSplitPtr(splitMesh1D->getNodalConnectivity()->begin()),
        *ciSplitPtr(splitMesh1D->getNodalConnectivityIndex()->begin());
    ...
    //
    idsLeftRight = DataArrayIdType::New();
    idsLeftRight->alloc(nbCellsInSplitMesh1D * 2);
    idsLeftRight->fillWithValue(-2);
    idsLeftRight->rearrange(2);
    mcIdType *idsLeftRightPtr(idsLeftRight->getPointer());
    VectorOfCellInfo pool(edge1Bis, edge1BisPtr);

    // Compute contiguous parts of splitMesh1D. We can not make the full assumption that segments are consecutive in the
    // connectivity (even if the user correctly called orderConsecutiveCells1D()). Indeed the tool might be a closed
    // line whose junction point is in splitMesh1D. There can be only one such a point, and if this happens this is
    // necessarily at the start of the connectivity.
    ...
    for (i = nbCellsInSplitMesh1D - 1; cSplitPtr[ciSplitPtr[i] + 2] == first; i--) first = cSplitPtr[ciSplitPtr[i] + 1];
    ...
    {  // split [0:nbCellsInSplitMesh1D) in contiguous parts [iStart:iEnd)
        ...
        MCAuto<MEDCouplingUMesh> partOfSplitMesh1D(
            static_cast<MEDCouplingUMesh *>(splitMesh1D->buildPartOfMySelfSlice(iStart, iEnd, 1, true))
        );
        ...
        for (std::size_t cnt = 0; cnt < out0.size(); cnt++) AddCellInMesh2D(retTmp, out0[cnt], out1[cnt]);
...
BuildMesh2DCutFrom(
    double eps,
    mcIdType cellIdInMesh2D,
    const MEDCouplingUMesh *mesh2DDesc,
    MEDCouplingUMesh *splitMesh1D,
    const mcIdType *descBg,
    const mcIdType *descEnd,
    const std::vector<std::vector<mcIdType> > &intersectEdge1,
...
{
    const mcIdType *cdescPtr(mesh2DDesc->getNodalConnectivity()->begin()),
        *cidescPtr(mesh2DDesc->getNodalConnectivityIndex()->begin());
    //
    std::vector<mcIdType> allEdges;
    std::vector<MCAuto<INTERP_KERNEL::Edge> >
        allEdgesPtr;  // for each sub edge in splitMesh2D the uncut Edge object of the original mesh2D
    for (const mcIdType *it(descBg); it != descEnd;
         it++)  // for all edges in the descending connectivity of the 2D mesh in relative Fortran mode
    {
        mcIdType edgeId(std::abs(*it) - 1);
        std::map<MCAuto<INTERP_KERNEL::Node>, mcIdType> m;
        MCAuto<INTERP_KERNEL::Edge> ee(MEDCouplingUMeshBuildQPFromEdge2(
            (INTERP_KERNEL::NormalizedCellType)cdescPtr[cidescPtr[edgeId]],
            cdescPtr + cidescPtr[edgeId] + 1,
            mesh2DDesc->getCoords()->begin(),
            m
        ));
        ...
        for (std::size_t cnt = 0; cnt < sz; cnt++) allEdgesPtr.push_back(ee);
    }
    //
...
AreEdgeEqual(
    const double *coo2D,
    const INTERP_KERNEL::CellModel &typ1,
    const mcIdType *conn1,
    const INTERP_KERNEL::CellModel &typ2,
...
/*!
 * This method returns among the cellIds [ \a candidatesIn2DBg , \a candidatesIn2DEnd ) in \a mesh2DSplit those exactly
 * sharing \a cellIdInMesh1DSplitRelative in \a mesh1DSplit.
 ...
 */
mcIdType
FindRightCandidateAmong(
    const MEDCouplingUMesh *mesh2DSplit,
    const mcIdType *candidatesIn2DBg,
    const mcIdType *candidatesIn2DEnd,
    const MEDCouplingUMesh *mesh1DSplit,
...
)
{
    ...
    MCAuto<MEDCouplingUMesh> cur1D(
        static_cast<MEDCouplingUMesh *>(mesh1DSplit->buildPartOfMySelf(&edgeId, &edgeId + 1, true))
    );
    if (cellIdInMesh1DSplitRelative < 0)
        cur1D->changeOrientationOfCells();
    const mcIdType *c1D(cur1D->getNodalConnectivity()->begin());
    const INTERP_KERNEL::CellModel &ref1DType(
        INTERP_KERNEL::CellModel::GetCellModel((INTERP_KERNEL::NormalizedCellType)c1D[0])
    );
    for (const mcIdType *it = candidatesIn2DBg; it != candidatesIn2DEnd; it++)
    {
        MCAuto<MEDCouplingUMesh> cur2D(
            static_cast<MEDCouplingUMesh *>(mesh2DSplit->buildPartOfMySelf(it, it + 1, true))
        );
        const mcIdType *c(cur2D->getNodalConnectivity()->begin()), *ci(cur2D->getNodalConnectivityIndex()->begin());
        const INTERP_KERNEL::CellModel &cm(
            INTERP_KERNEL::CellModel::GetCellModel((INTERP_KERNEL::NormalizedCellType)c[ci[0]])
        );
    ...
    throw INTERP_KERNEL::Exception(
        "FindRightCandidateAmong : internal error 2 ! Unable to find the edge among split cell !"
    );
}

/*!
 * \param [out] intersectEdge1 - for each cell in \a m1Desc returns the result of the split. The result is given using
 * pair of mcIdType given resp start and stop. So for all edge \a i in \a m1Desc \a  intersectEdge1[i] is of length 2*n
 * where n is the number of sub edges. And for each j in [1,n) intersect[i][2*(j-1)+1]==intersect[i][2*j].
 * \param [out] subDiv2 - for each cell in \a m2Desc returns nodes that split it using convention \a m1Desc first, then
 * \a m2Desc, then addCoo
 ...
 * \param [out] mergedNodes - gives all pair of nodes of \a m2Desc that have same location than some nodes in \a m1Desc.
 * key is id in \a m2Desc offsetted and value is id in \a m1Desc.
 */
void
MEDCouplingUMesh::Intersect1DMeshes(
    const MEDCouplingUMesh *m1Desc,
    const MEDCouplingUMesh *m2Desc,
    double eps,
    std::vector<std::vector<mcIdType> > &intersectEdge1,
    std::vector<std::vector<mcIdType> > &colinear2,
    std::vector<std::vector<mcIdType> > &subDiv2,
    std::vector<double> &addCoo,
...
)
{
    ...
    MCAuto<DataArrayDouble> bbox1Arr(m1Desc->getBoundingBoxForBBTree(eps)),
        bbox2Arr(m2Desc->getBoundingBoxForBBTree(eps));
    ...
    BBTreePts<SPACEDIM, mcIdType> treeNodes2(
        m2Desc->getCoords()->begin(), 0, 0, m2Desc->getCoords()->getNumberOfTuples(), eps
    );
    ...
    for (mcIdType i = 0; i < nDescCell1; i++)  // for all edges in the first mesh
    {
        ...
        if (!candidates2.empty())  // candidates2 holds edges from the second mesh potentially intersecting current edge
                                   // i in mesh1
        {
            std::map<INTERP_KERNEL::Node *, mcIdType> map1, map2;
            std::map<mcIdType, INTERP_KERNEL::Node *> revMap2;
            // pol2 is not necessarily a closed polygon: just a set of (quadratic) edges (same as candidates2) in the
            // Geometric DS format
            INTERP_KERNEL::QuadraticPolygon *pol2 = MEDCouplingUMeshBuildQPFromMesh(m2Desc, candidates2, map2);
            // Build revMap2
            for (auto &kv : map2) revMap2[kv.second] = kv.first;
            candidates1[0] = i;
            ...
            INTERP_KERNEL::QuadraticPolygon *pol1 =
                MEDCouplingUMeshBuildQPFromMeshWithTree(m1Desc, candidates1, map1, treeNodes2, revMap2);
            // This following part is to avoid that some removed nodes (for example due to a merge between pol1 and
            // pol2) are replaced by a newly created one This trick guarantees that Node * are discriminant (i.e. form a
            // unique identifier)
            std::set<INTERP_KERNEL::Node *> nodes;
            pol1->getAllNodes(nodes);
            pol2->getAllNodes(nodes);
 ...
 */
void
MEDCouplingUMesh::IntersectDescending2DMeshes(
    const MEDCouplingUMesh *m1,
    const MEDCouplingUMesh *m2,
    double eps,
    std::vector<std::vector<mcIdType> > &intersectEdge1,
    std::vector<std::vector<mcIdType> > &colinear2,
    std::vector<std::vector<mcIdType> > &subDiv2,
    MEDCouplingUMesh *&m1Desc,
    DataArrayIdType *&desc1,
    DataArrayIdType *&descIndx1,
    DataArrayIdType *&revDesc1,
    DataArrayIdType *&revDescIndx1,
    std::vector<double> &addCoo,
    MEDCouplingUMesh *&m2Desc,
    DataArrayIdType *&desc2,
    DataArrayIdType *&descIndx2,
    DataArrayIdType *&revDesc2,
    DataArrayIdType *&revDescIndx2
)
{
    // Build desc connectivity
    desc1 = DataArrayIdType::New();
    descIndx1 = DataArrayIdType::New();
    revDesc1 = DataArrayIdType::New();
    revDescIndx1 = DataArrayIdType::New();
    ...
    descIndx1->incrRef();
    revDesc1->incrRef();
    revDescIndx1->incrRef();
    m2Desc->incrRef();
    desc2->incrRef();
    descIndx2->incrRef();
    revDesc2->incrRef();
    revDescIndx2->incrRef();
}
 ...
 */
void
MEDCouplingUMesh::BuildIntersecting2DCellsFromEdges(
    double eps,
    const MEDCouplingUMesh *m1,
    const mcIdType *desc1,
    const mcIdType *descIndx1,
    const std::vector<std::vector<mcIdType> > &intesctEdges1,
    const std::vector<std::vector<mcIdType> > &colinear2,
    const MEDCouplingUMesh *m2,
    const mcIdType *desc2,
    const mcIdType *descIndx2,
    const std::vector<std::vector<mcIdType> > &intesctEdges2,
    const std::vector<double> &addCoords,
    std::vector<double> &addCoordsQuadratic,
    std::vector<mcIdType> &cr,
    std::vector<mcIdType> &crI,
    std::vector<mcIdType> &cNb1,
    std::vector<mcIdType> &cNb2
)
{
    static const int SPACEDIM = 2;
    ...
    for (mcIdType i = 0; i < ncell1; i++)
    {
        ...
        // Populate mapp and mappRev with nodes from the current cell (i) from mesh1 - this also builds the Node*
        // objects:
        MEDCouplingUMeshBuildQPFromMesh3(
            coo1,
            offset1,
        ...
        //
        std::set<INTERP_KERNEL::Edge *>
            edges1;  // store all edges of pol1 that are NOT consumed by intersect cells. If any after iteration over
                     // candidates2 -> a part of pol1 should appear in result
        std::set<INTERP_KERNEL::Edge *>
            edgesBoundary2;  // store all edges that are on boundary of (pol2 intersect pol1) minus edges on pol1.
        INTERP_KERNEL::IteratorOnComposedEdge it1(&pol1);
        for (it1.first(); !it1.finished(); it1.next()) edges1.insert(it1.current()->getPtr());
        //
        ...
        for (std::vector<mcIdType>::const_iterator it2 = candidates2.begin(); it2 != candidates2.end(); it2++, ii++)
        {
            ...
            MEDCouplingUMeshBuildQPFromMesh3(
                ...
                addCoords,
                desc2 + descIndx2[*it2],
                desc2 + descIndx2[*it2 + 1],
                intesctEdges2,
                /* output */ mapp,
                mappRev
            );
            // pol2 is the new QP in the final merged result.
            pol2s[ii].buildFromCrudeDataArray2(
                mappRev,
                cm2.isQuadratic(),
                conn2 + connI2[*it2] + 1,
                coo2,
        ...
        for (std::vector<mcIdType>::const_iterator it2 = candidates2.begin(); it2 != candidates2.end(); it2++, ii++)
        {
            ...
            pol2s[ii].updateLocOfEdgeFromCrudeDataArray2(
                desc2 + descIndx2[*it2],
                desc2 + descIndx2[*it2 + 1],
                intesctEdges2,
                pol1,
            ...
            pol1.buildPartitionsAbs(
                pol2s[ii], edges1, edgesBoundary2, mapp, i, *it2, offset3, addCoordsQuadratic, cr, crI, cNb1, cNb2
            );
        }
        ...
        if (!edges1.empty())
        {
            try
            {
                INTERP_KERNEL::QuadraticPolygon::ComputeResidual(
                    pol1, edges1, edgesBoundary2, mapp, offset3, i, addCoordsQuadratic, cr, crI, cNb1, cNb2
                );
            }
            catch (INTERP_KERNEL::Exception &e)
            {
                std::ostringstream oss;
                oss << "Error when computing residual of cell #" << i
                    << " in source/m1 mesh ! Maybe the neighbours of this cell in mesh are not well connected !\n"
...
void
InsertNodeInConnIfNecessary(mcIdType nodeIdToInsert, std::vector<mcIdType> &conn, const double *coords, double eps)
{
    ...
    for (std::size_t i = 0; i < sz; i++)
    {
        ...
        double v1[3] =
            {coords[3 * pt1 + 0] - coords[3 * pt0 + 0],
             coords[3 * pt1 + 1] - coords[3 * pt0 + 1],
             coords[3 * pt1 + 2] - coords[3 * pt0 + 2]},
               v2[3] = {
                   coords[3 * nodeIdToInsert + 0] - coords[3 * pt0 + 0],
                   coords[3 * nodeIdToInsert + 1] - coords[3 * pt0 + 1],
                   coords[3 * nodeIdToInsert + 2] - coords[3 * pt0 + 2]
               };
        ...
        double normm2(sqrt(v3[0] * v3[0] + v3[1] * v3[1] + v3[2] * v3[2])),
            dotTest(v1[0] * v2[0] + v1[1] * v2[1] + v1[2] * v2[2]);
        if (normm2 < eps)
...
void
SplitIntoToPart(
    const std::vector<mcIdType> &conn,
    mcIdType pt0,
    mcIdType pt1,
    std::vector<mcIdType> &part0,
    std::vector<mcIdType> &part1
)
{
    std::size_t sz(conn.size());
...
/*!
 * this method method splits cur cells 3D Surf in sub cells 3DSurf using the previous subsplit. This method is the last
 * one used to clip.
 */
void
MEDCouplingUMesh::buildSubCellsFromCut(
    const std::vector<std::pair<mcIdType, mcIdType> > &cut3DSurf,
    const mcIdType *desc,
    const mcIdType *descIndx,
    const double *coords,
    double eps,
...
) const
{
    ...
    if (getMeshDimension() != 3 || getSpaceDimension() != 3)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::buildSubCellsFromCut works on umeshes with meshdim equal to 3 and spaceDim equal to 3 "
            "too!"
        );
    ...
    if (nbOfCells != 1)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::buildSubCellsFromCut works only with single cell presently !"
        );
    for (mcIdType i = 0; i < nbOfCells; i++)
    {
        ...
        for (int j = 0; j < nbOfFaces; j++)
        {
            ...
            const INTERP_KERNEL::CellModel &cm(
                INTERP_KERNEL::CellModel::GetCellModel((INTERP_KERNEL::NormalizedCellType)nodal3D[nodalIndx3D[i]])
            );
...
/*!
 * It is the linear part of MEDCouplingUMesh::split2DCells. Here no additional nodes will be added in \b this. So
 * coordinates pointer remain unchanged (is not even touch).
 *
 * \sa MEDCouplingUMesh::split2DCells
 */
void
MEDCouplingUMesh::split2DCellsLinear(
...
)
{
    ...
    c->alloc((std::size_t)lgthToReach);
    const mcIdType *subPtr(subNodesInSeg->begin()), *subIPtr(subNodesInSegI->begin()), *descPtr(desc->begin()),
        *descIPtr(descI->begin()), *oldConn(getNodalConnectivity()->begin());
    ...
    for (mcIdType i = 0; i < ncells; i++, ciPtr++, descIPtr++)
    {
        ...
        *cPtr++ = ToIdType(INTERP_KERNEL::NORM_POLYGON);
        *cPtr++ = oldConn[prevPosOfCi + 1];
        for (mcIdType j = 0; j < sz; j++)
        {
            mcIdType offset2(subIPtr[descPtr[offset + j]]),
                sz2(subIPtr[descPtr[offset + j] + 1] - subIPtr[descPtr[offset + j]]);
            for (mcIdType k = 0; k < sz2; k++) *cPtr++ = subPtr[offset2 + k];
            if (j != sz - 1)
                *cPtr++ = oldConn[prevPosOfCi + j + 2];
            deltaSz += sz2;
        }
        prevPosOfCi = ciPtr[1];
        ciPtr[1] = ciPtr[0] + 1 + sz + deltaSz;  // sz==old nb of nodes because (nb of subedges=nb of nodes for
                                                 // polygons)
    }
    if (c->end() != cPtr)
        throw INTERP_KERNEL::Exception("MEDCouplingUMesh::split2DCellsLinear : Some of edges to be split are orphan !");
    _nodal_connec->decrRef();
    _nodal_connec = c.retn();
    _types.clear();
    _types.insert(INTERP_KERNEL::NORM_POLYGON);
}

/*!
 * It is the quadratic part of MEDCouplingUMesh::split2DCells. Here some additional nodes can be added at the end of
 ...
 */
mcIdType
MEDCouplingUMesh::split2DCellsQuadratic(
    const DataArrayIdType *desc,
    const DataArrayIdType *descI,
    const DataArrayIdType *subNodesInSeg,
    const DataArrayIdType *subNodesInSegI,
    const DataArrayIdType *mid,
    const DataArrayIdType *midI
)
{
    ...
    for (mcIdType i = 0; i < ncells; i++, ciPtr++, descIPtr++)
    {
        ...
        for (mcIdType j = 0; j < sz; j++)
        {
            mcIdType sz2(subIPtr[descPtr[offset + j] + 1] - subIPtr[descPtr[offset + j]]);
            deltaSz += sz2;
        }
        *cPtr++ = ToIdType(INTERP_KERNEL::NORM_QPOLYG);
        cPtr[0] = oldConn[prevPosOfCi + 1];
        for (mcIdType j = 0; j < sz; j++)  // loop over subedges of oldConn
        {
            mcIdType offset2(subIPtr[descPtr[offset + j]]),
                sz2(subIPtr[descPtr[offset + j] + 1] - subIPtr[descPtr[offset + j]]),
                offset3(midIPtr[descPtr[offset + j]]);
            if (sz2 == 0)
            {
                if (j < sz - 1)
                    cPtr[1] = oldConn[prevPosOfCi + 2 + j];
                cPtr[deltaSz] = oldConn[prevPosOfCi + 1 + j + sz];
                cPtr++;
                continue;
            }
            std::vector<INTERP_KERNEL::Node *> ns(3);
            ns[0] = new INTERP_KERNEL::Node(
                oldCoordsPtr[2 * oldConn[prevPosOfCi + 1 + j]], oldCoordsPtr[2 * oldConn[prevPosOfCi + 1 + j] + 1]
            );
            ns[1] = new INTERP_KERNEL::Node(
                oldCoordsPtr[2 * oldConn[prevPosOfCi + 1 + (1 + j) % sz]],
                oldCoordsPtr[2 * oldConn[prevPosOfCi + 1 + (1 + j) % sz] + 1]
            );
            ns[2] = new INTERP_KERNEL::Node(
                oldCoordsPtr[2 * oldConn[prevPosOfCi + 1 + sz + j]],
                oldCoordsPtr[2 * oldConn[prevPosOfCi + 1 + sz + j] + 1]
            );
            ...
            for (mcIdType k = 0; k < sz2; k++)  // loop over subsplit of current subedge
            {
                ...
                cPtr[deltaSz] =
                    InternalAddPoint(e, midPtr[offset3 + k], oldCoordsPtr, cPtr[0], cPtr[1], *addCoo, nodesCnt);
                cPtr++;
            }
            ...
            cPtr[deltaSz] =
                InternalAddPoint(e, midPtr[offset3 + sz2], oldCoordsPtr, cPtr[0], tmpEnd, *addCoo, nodesCnt);
            cPtr++;
        }
        prevPosOfCi = ciPtr[1];
        cPtr += deltaSz;
    ...
    if (c->end() != cPtr)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::split2DCellsQuadratic : Some of edges to be split are orphan !"
        );
    _nodal_connec->decrRef();
    _nodal_connec = c.retn();
    _types.clear();
    _types.insert(INTERP_KERNEL::NORM_QPOLYG);
    addCoo->rearrange(2);
    MCAuto<DataArrayDouble> coo(
        DataArrayDouble::Aggregate(getCoords(), addCoo)
    );  // info are copied from getCoords() by using Aggregate
    setCoords(coo);
 ...
 * meshes.
 * \b WARNING: the two meshes should be correctly oriented for this method to work properly. Methods
 * changeSpaceDimension() and orientCorrectly2DCells() can be used for this.
 * \b WARNING: the two meshes should be "clean" (no un-merged nodes, no non-conformal cells)
 *  \param [in] m1 - the first input mesh which is a partitioned object. The mesh must be so that each point in the
 * space covered by \a m1 must be covered exactly by one entity, \b no \b more. If it is not the case, some tools are
 * available to heal the mesh (conformize2D, mergeNodes)
 *  \param [in] m2 - the second input mesh which is a partition tool. The mesh must be so that each point in the space
 * covered by \a m2 must be covered exactly by one entity, \b no \b more. If it is not the case, some tools are
 * available to heal the mesh (conformize2D, mergeNodes)
...
MEDCouplingUMesh::Intersect2DMeshes(
    ...
    const MEDCouplingUMesh *m2,
    double eps,
    DataArrayIdType *&cellNb1,
    DataArrayIdType *&cellNb2
)
{
    if (!m1 || !m2)
    ...
    INTERP_KERNEL::QuadraticPlanarPrecision prec(eps);
    if (m1->getMeshDimension() != 2 || m1->getSpaceDimension() != 2 || m2->getMeshDimension() != 2 ||
        m2->getSpaceDimension() != 2)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::Intersect2DMeshes works on umeshes m1 AND m2  with meshdim equal to 2 and spaceDim "
            "equal to 2 too!"
        );
    ...
    DataArrayIdType *desc1 = 0, *descIndx1 = 0, *revDesc1 = 0, *revDescIndx1 = 0, *desc2 = 0, *descIndx2 = 0,
                    *revDesc2 = 0, *revDescIndx2 = 0;
    std::vector<double> addCoo, addCoordsQuadratic;  // coordinates of newly created nodes
    IntersectDescending2DMeshes(
        m1,
        m2,
        eps,
        intersectEdge1,
    ...
    revDesc1->decrRef();
    revDescIndx1->decrRef();
    revDesc2->decrRef();
    revDescIndx2->decrRef();
    ...
    BuildIntersecting2DCellsFromEdges(
        eps,
        m1,
        desc1->begin(),
        descIndx1->begin(),
    ...
    std::vector<const DataArrayDouble *> coordss(4);
    coordss[0] = m1->getCoords();
    coordss[1] = m2->getCoords();
    coordss[2] = addCooDa;
    coordss[3] = addCoordsQuadraticDa;
    ...
    conn->alloc(cr.size(), 1);
    std::copy(cr.begin(), cr.end(), conn->getPointer());
    MCAuto<DataArrayIdType> connI(DataArrayIdType::New());
    connI->alloc(crI.size(), 1);
    std::copy(crI.begin(), crI.end(), connI->getPointer());
    MCAuto<DataArrayIdType> c1(DataArrayIdType::New());
    c1->alloc(cNb1.size(), 1);
    std::copy(cNb1.begin(), cNb1.end(), c1->getPointer());
    MCAuto<DataArrayIdType> c2(DataArrayIdType::New());
    c2->alloc(cNb2.size(), 1);
    std::copy(cNb2.begin(), cNb2.end(), c2->getPointer());
 ...
 * Thus the final result contains the aggregation of nodes of \a mesh2D, then nodes of \a mesh1D, then new nodes that
 * are the result of the intersection and finally, in case of quadratic polygon the centers of edges new nodes. The
 * meshes should be in 2D space. In addition, returns two arrays mapping cells of the resulting mesh to cells of the
 * input.
 *
 * \b WARNING: the 2D mesh should be correctly oriented for this method to work properly. Methods changeSpaceDimension()
 * and orientCorrectly2DCells() can be used for this.
 * \b WARNING: the two meshes should be "clean" (no un-merged nodes, no non-conformal cells)
 *
 * \param [in] mesh2D - the 2D mesh (spacedim=meshdim=2) to be intersected using \a mesh1D tool. The mesh must be so
 * that each point in the space covered by \a mesh2D must be covered exactly by one entity, \b no \b more. If it is not
 * the case, some tools are available to heal the mesh (conformize2D, mergeNodes)
 * \param [in] mesh1D - the 1D mesh (spacedim=2 meshdim=1) the is the tool that will be used to intersect \a mesh2D. \a
 * mesh1D must be ordered consecutively. If it is not the case you can invoke orderConsecutiveCells1D on \a mesh1D.
 ...
 * \param [out] cellIdInMesh2D - the array that gives for each cell id \a i in \a splitMesh2D the id in \a mesh2D it
 * comes from. So this array has a number of tuples equal to the number of cells of \a splitMesh2D and a number of
 * component equal to 1.
 * \param [out] cellIdInMesh1D - the array of pair that gives for each cell id \a i in \a splitMesh1D the cell in \a
 * splitMesh2D on the left for the 1st component and the cell in \a splitMesh2D on the right for the 2nt component. -1
 * means no cell. So this array has a number of tuples equal to the number of cells of \a splitMesh1D and a number of
...
MEDCouplingUMesh::Intersect2DMeshWith1DLine(
    ...
    MEDCouplingUMesh *&splitMesh1D,
    DataArrayIdType *&cellIdInMesh2D,
    DataArrayIdType *&cellIdInMesh1D
)
{
    if (!mesh2D || !mesh1D)
    ...
    {  // (In a scope to free mem early)
        ...
        DataArrayIdType *mesh1DDesc1(DataArrayIdType::New()), *mesh1DDescIndx1(DataArrayIdType::New()),
            *mesh1DRevDesc1(DataArrayIdType::New()), *mesh1DRevDescIndx1(DataArrayIdType::New());
        MCAuto<DataArrayIdType> m1dd1(mesh1DDesc1), m1dd2(mesh1DDescIndx1), m1dd3(mesh1DRevDesc1),
            m1dd4(mesh1DRevDescIndx1);
        MCAuto<MEDCouplingUMesh> mesh1Desc(
            mesh1D->buildDescendingConnectivity(mesh1DDesc1, mesh1DDescIndx1, mesh1DRevDesc1, mesh1DRevDescIndx1)
        );
        ...
        if (tpls->getNumberOfTuples() != 0)
            throw INTERP_KERNEL::Exception(
                "MEDCouplingUMesh::Intersect2DMeshWith1DLine : Certain nodes of the 1D mesh are connected to more than 2 edges !\
    ...
    if (mesh2D->getMeshDimension() != 2 || mesh2D->getSpaceDimension() != 2 || mesh1D->getMeshDimension() != 1 ||
        mesh1D->getSpaceDimension() != 2)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::Intersect2DMeshWith1DLine works with mesh2D with spacedim=meshdim=2 and mesh1D with "
            "meshdim=1 spaceDim=2 !"
        );
    // Step 1: compute all edge intersections (new nodes)
    std::vector<std::vector<mcIdType> > intersectEdge1, colinear2, subDiv2;
    std::vector<double> addCoo, addCoordsQuadratic;  // coordinates of newly created nodes
    ...
    //
    // Build desc connectivity
    DataArrayIdType *desc1(DataArrayIdType::New()), *descIndx1(DataArrayIdType::New()),
    ...
    ret2->alloc(0, 1);
    MCAuto<MEDCouplingUMesh> ret1(BuildMesh1DCutFrom(
        mesh1D,
        intersectEdge2,
        mesh2D->getCoords(),
        addCoo,
        ...
        intersectEdge1,
        idsInRet1Colinear,
        idsInDescMesh2DForIdsInRetColinear
    ));
    ...
    mesh2D->getCellsContainingPoints(
        centerOfMassRet1->begin(), centerOfMassRet1->getNumberOfTuples(), eps, cells, cellsIndex
    );
    MCAuto<DataArrayIdType> cellsIndex2(DataArrayIdType::New());
    cellsIndex2->alloc(0, 1);
    ...
    for (const mcIdType *it = idsInRet1With2Contacts->begin(); it != idsInRet1With2Contacts->end(); it++)
    {
        ...
        std::set<mcIdType> s1(
            realIdsInDesc2D->begin() + dd2->begin()[*cellId1], realIdsInDesc2D->begin() + dd2->begin()[*cellId1 + 1]
        );
        std::set<mcIdType> s2(
            realIdsInDesc2D->begin() + dd2->begin()[*cellId2], realIdsInDesc2D->begin() + dd2->begin()[*cellId2 + 1]
        );
        ...
        if (IsColinearOfACellOf(
                intersectEdge1,
                commonEdgeId,
                *firstNodeInColinearEdgeRet1,
                *secondNodeInColinearEdgeRet1,
                commonEdgeIdCorrectlyOriented
            ))
        {
    ...
    MCAuto<DataArrayIdType> ret3(DataArrayIdType::New());
    ret3->alloc(ret1->getNumberOfCells() * 2, 1);
    ret3->fillWithValue(std::numeric_limits<mcIdType>::max());
    ret3->rearrange(2);
    ...
    idsInDesc2DToBeRefined->abs();
    idsInDesc2DToBeRefined->applyLin(1, -1);
    idsInDesc2DToBeRefined = idsInDesc2DToBeRefined->buildUnique();

    MCAuto<DataArrayIdType> out0s;  // ids in mesh2D that are impacted by the fact that some edges of \a mesh1D are part
                                    // of the edges of those cells
    if (!idsInDesc2DToBeRefined->empty())
    {
        DataArrayIdType *out0(0), *outi0(0);
        DataArrayIdType::ExtractFromIndexedArrays(
            idsInDesc2DToBeRefined->begin(), idsInDesc2DToBeRefined->end(), dd3, dd4, out0, outi0
        );
    ...
    //
    MCAuto<MEDCouplingUMesh> ret1NonCol(
        static_cast<MEDCouplingUMesh *>(
            ret1->buildPartOfMySelf(idsInRet1NotColinear->begin(), idsInRet1NotColinear->end())
        )
    );
    MCAuto<DataArrayDouble> baryRet1(
        centerOfMassRet1->selectByTupleId(idsInRet1NotColinear->begin(), idsInRet1NotColinear->end())
    );
    DataArrayIdType *out(0), *outi(0);
    DataArrayIdType::ExtractFromIndexedArrays(
        idsInRet1NotColinear->begin(), idsInRet1NotColinear->end(), cells, cellsIndex, out, outi
    );
    MCAuto<DataArrayIdType> elts(out), eltsIndex(outi);

    MCAuto<DataArrayIdType> eltsIndex2(DataArrayIdType::New());
    eltsIndex2->alloc(0, 1);
    ...
    if ((DataArrayIdType *)out0s)
        untouchedCells = untouchedCells->buildSubstraction(
            out0s
        );  // if some edges in ret1 are colinear to descending mesh of mesh2D remove cells from untouched one
    ...
    {  // the most easy part, cells in mesh2D not impacted at all
        outMesh2DSplit.push_back(
            static_cast<MEDCouplingUMesh *>(mesh2D->buildPartOfMySelf(untouchedCells->begin(), untouchedCells->end()))
        );
    ...
    {  // here dealing with cells in out0s but not in cellsToBeModified
        ...
        {
            outMesh2DSplit.push_back(BuildRefined2DCell(
                ret1->getCoords(), mesh2D, *it, dptr + diptr[*it], dptr + diptr[*it + 1], intersectEdge1
            ));
        ...
        MCAuto<DataArrayIdType> partOfRet3(DataArrayIdType::New());
        partOfRet3->alloc(2 * idsInRet1Colinear->getNumberOfTuples(), 1);
        partOfRet3->fillWithValue(std::numeric_limits<mcIdType>::max());
        partOfRet3->rearrange(2);
        mcIdType kk(0), *ret3ptr(partOfRet3->getPointer());
        for (const mcIdType *it = idsInDescMesh2DForIdsInRetColinear->begin();
             it != idsInDescMesh2DForIdsInRetColinear->end();
             it++, kk++)
        {
        ...
        if (!outMesh2DSplit.empty())
        {
            ...
            for (std::vector<MCAuto<MEDCouplingUMesh> >::iterator itt = outMesh2DSplit.begin();
                 itt != outMesh2DSplit.end();
                 itt++)
                (*itt)->setCoords(da);
        }
    }
    ...
    for (const mcIdType *it = cellsToBeModified->begin(); it != cellsToBeModified->end(); it++)
    {
        ...
        MCAuto<DataArrayIdType> idsNonColPerCell2(
            idsInRet1NotColinear->selectByTupleId(idsNonColPerCell->begin(), idsNonColPerCell->end())
        );
        MCAuto<MEDCouplingUMesh> partOfMesh1CuttingCur2DCell(
            static_cast<MEDCouplingUMesh *>(
                ret1NonCol->buildPartOfMySelf(idsNonColPerCell->begin(), idsNonColPerCell->end())
            )
        );
        MCAuto<DataArrayIdType> partOfRet3;
        MCAuto<MEDCouplingUMesh> splitOfOneCell(BuildMesh2DCutFrom(
            eps,
            *it,
            m1Desc,
            partOfMesh1CuttingCur2DCell,
            dd1->begin() + dd2->getIJ(*it, 0),
            dd1->begin() + dd2->getIJ((*it) + 1, 0),
            intersectEdge1,
            ret2->getNumberOfTuples(),
            partOfRet3
        ));
        ...
        for (mcIdType i = 0; i < splitOfOneCell->getNumberOfCells(); i++) ret2->pushBackSilent(*it);
    }
    //
    std::size_t nbOfMeshes(outMesh2DSplit.size());
    std::vector<const MEDCouplingUMesh *> tmp(nbOfMeshes);
    for (std::size_t i = 0; i < nbOfMeshes; i++) tmp[i] = outMesh2DSplit[i];
    //
        ...
        ret3->setIJ(
            *it,
            ...
            FindRightCandidateAmong(
                ...
                candidates->end(),
                ret1,
                *it % 2 == 0 ? -((*it) / 2 + 1) : (*it) / 2 + 1,
                eps
            )
        );  // div by 2 because 2 components natively in ret3
    }
 ...
 * This method performs a conformization of \b this. So if a edge in \a this can be split into entire edges in \a this
 * method will suppress such edges to use sub edges in \a this. So this method does not add nodes in \a this if merged
 * edges are both linear (INTERP_KERNEL::NORM_SEG2). In the other cases new nodes can be created. If any are created,
 * they will be appended at the end of the coordinates object before the invocation of this method.
 *
 * Whatever the returned value, this method does not alter the order of cells in \a this neither the orientation of
 * cells. The modified cells, if any, are systematically declared as NORM_POLYGON or NORM_QPOLYG depending on the
 * initial quadraticness of geometric type.
 *
 ...
 * \return DataArrayIdType  * - The list of cellIds in \a this that have been subdivided. If empty, nothing changed in
 * \a this (as if it were a const method). The array is a newly allocated array that the user is expected to deal with.
 *
 ...
 */
DataArrayIdType *
MEDCouplingUMesh::conformize2D(double eps)
{
    static const int SPACEDIM = 2;
    checkConsistencyLight();
    if (getSpaceDimension() != 2 || getMeshDimension() != 2)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::conformize2D : This method only works for meshes with spaceDim=2 and meshDim=2 !"
        );
    ...
    const mcIdType *c(mDesc->getNodalConnectivity()->begin()), *ci(mDesc->getNodalConnectivityIndex()->begin()),
        *rd(revDesc1->begin()), *rdi(revDescIndx1->begin());
    ...
    for (mcIdType i = 0; i < nDescCell; i++)
    {
        ...
        for (std::vector<mcIdType>::const_iterator it = candidates.begin(); it != candidates.end(); it++)
            ...
            {
                std::map<MCAuto<INTERP_KERNEL::Node>, mcIdType> m;
                INTERP_KERNEL::Edge *e1(MEDCouplingUMeshBuildQPFromEdge2(
                    (INTERP_KERNEL::NormalizedCellType)c[ci[i]], c + ci[i] + 1, coords, m
                )),
                    *e2(MEDCouplingUMeshBuildQPFromEdge2(
                        (INTERP_KERNEL::NormalizedCellType)c[ci[*it]], c + ci[*it] + 1, coords, m
                    ));
    ...
    for (mcIdType i = 0; i < nDescCell; i++)
    {
        ...
        if (sz > 1)
        {
            std::map<MCAuto<INTERP_KERNEL::Node>, mcIdType> m;
            INTERP_KERNEL::Edge *e(
                MEDCouplingUMeshBuildQPFromEdge2((INTERP_KERNEL::NormalizedCellType)c[ci[i]], c + ci[i] + 1, coords, m)
            );
        ...
        if (sz != 0)
        {
            ...
            if ((INTERP_KERNEL::NormalizedCellType)c[ci[i]] == INTERP_KERNEL::NORM_SEG3)
            {
                ...
                {
                    for (std::vector<mcIdType>::const_iterator itc = trueCandidates.begin();
                         itc != trueCandidates.end();
                         itc++)
                    {
    ...
    MCAuto<DataArrayIdType> ret(DataArrayIdType::New()), notRet(DataArrayIdType::New());
    ret->alloc(nbOf2DCellsToBeSplit, 1);
    ...
    DataArrayIdType::ExtractFromIndexedArrays(ret->begin(), ret->end(), desc1, descIndx1, m, n);
    mSafe = m;
    nSafe = n;
    DataArrayIdType::PutIntoToSkylineFrmt(intersectEdge, o, p);
    oSafe = o;
    pSafe = p;
    if (middleNeedsToBeUsed)
    {
        DataArrayIdType::PutIntoToSkylineFrmt(middle, q, r);
        qSafe = q;
        rSafe = r;
    }
    ...
    setCoords(modif->getCoords());  // if nbOfNodesCreated==0 modif and this have the same coordinates pointer so this
                                    // line has no effect. But for quadratic cases this line is important.
...
/*!
 * This non const method works on 2D mesh. This method scans every cell in \a this and look if each edge constituting
 * this cell is not mergeable with neighbors edges of that cell. If yes, the cell is "repaired" to minimize at most its
 * number of edges. So this method do not change the overall shape of cells in \a this (with eps precision). This method
 * do not take care of shared edges between cells, so this method can lead to a non conform mesh (\a this). If a conform
 * mesh is required you're expected to invoke MEDCouplingUMesh::mergeNodes and MEDCouplingUMesh::conformize2D right
 * after this call. This method works on any 2D geometric types of cell (even static one). If a cell is touched its type
 * becomes dynamic automatically. For 2D "repaired" quadratic cells new nodes for center of merged edges is are
 * systematically created and appended at the end of the previously existing nodes.
 *
 * If the returned array is empty it means that nothing has changed in \a this (as if it were a const method). If the
 * array is not empty the connectivity of \a this is modified using new instance, idem for coordinates.
 *
 * If \a this is constituted by only linear 2D cells, this method is close to the computation of the convex hull of each
 * cells in \a this.
 *
 ...
 */
DataArrayIdType *
MEDCouplingUMesh::colinearize2D(double eps)
{
 ...
 * In a given 2D cell, if two edges are colinear and the junction point is used by a third edge, the two edges will not
 * be merged, contrary to colinearize2D().
 *
 * \sa MEDCouplingUMesh::colinearize2D
 */
DataArrayIdType *
MEDCouplingUMesh::colinearizeKeepingConform2D(double eps)
{
    return internalColinearize2D(eps, true);
}

/*!
 * \param stayConform is set to True, will not fuse two edges sharing a node that has (strictly) more than 2 egdes
 * connected to it
 */
DataArrayIdType *
MEDCouplingUMesh::internalColinearize2D(double eps, bool stayConform)
{
    ...
    if (getSpaceDimension() != 2 || getMeshDimension() != 2)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::colinearize2D : This method only works for meshes with spaceDim=2 and meshDim=2 !"
        );
    INTERP_KERNEL::QuadraticPlanarPrecision prec(eps);
    ...
    MCAuto<DataArrayIdType> newc(DataArrayIdType::New()), newci(DataArrayIdType::New());
    ...
    if (stayConform)                           //
    {
        ...
        MCAuto<DataArrayIdType> desc1(DataArrayIdType::New()), descI1(DataArrayIdType::New()),
            revDesc1(DataArrayIdType::New()), revDescI1(DataArrayIdType::New());
        MCAuto<MEDCouplingUMesh> mDesc1D(buildDescendingConnectivity(desc1, descI1, revDesc1, revDescI1));
        MCAuto<DataArrayIdType> desc2(DataArrayIdType::New()), descI2(DataArrayIdType::New()),
            revDesc2(DataArrayIdType::New()), revDescI2(DataArrayIdType::New());
    ...
    for (mcIdType i = 0; i < nbOfCells; i++, newciptr++, ciptr++)
    {
        if (Colinearize2DCell(
                coords, cptr + ciptr[0], cptr + ciptr[1], nbOfNodes, forbiddenPoints, /*out*/ newc, appendedCoords
            ))
    ...
    if (!appendedCoords->empty())
    {
        ...
        MCAuto<DataArrayDouble> newCoords(
            DataArrayDouble::Aggregate(getCoords(), appendedCoords)
        );  // treat info on components
        // non const part
        setCoords(newCoords);
    }
    // non const part
...
MEDCouplingUMesh::OrderPointsAlongLine(
    ...
    const mcIdType *c,
    const mcIdType *cI,
    const mcIdType *idsBg,
    const mcIdType *endBg,
    std::vector<mcIdType> &pointIds,
    std::vector<mcIdType> &hitSegs
)
{
    using namespace std;
    ...
    for (const mcIdType *it = idsBg; it != endBg; ++it)
    {
        ...
        if (itStart == pointIds.end())
            continue;
        vector<mcIdType>::const_iterator itEnd = find(pointIds.begin(), pointIds.end(), end);
        if (itEnd == pointIds.end())
            continue;
        if (abs(distance(itEnd, itStart)) != 1)
            continue;
...
void
MEDCouplingUMesh::ReplaceEdgeInFace(
    const mcIdType *sIdxConn,
    const mcIdType *sIdxConnE,
    mcIdType startNode,
    mcIdType endNode,
    const std::vector<mcIdType> &insidePoints,
...
)
{
    ...
    if (d == 1 || d == (1 - dst))  // don't use modulo, for neg numbers, result is implementation defined ...
        modifiedFace.insert(
            ++startPos, ++insidePoints.begin(), --insidePoints.end()
        );  // insidePoints also contains start and end node. Those don't need to be inserted.
    else
 ...
 * If it is not the case you can invoke MEDCouplingUMesh::mergeNodes before calling this method.
 *
 * \b WARNING this method only works for 'partition-like' non-conformities. When joining adjacent faces, the set of all
 * small faces must fit exactly into the neighboring bigger face. No real face intersection is actually computed.
 *
 ...
 */
DataArrayIdType *
MEDCouplingUMesh::conformize3D(double eps)
{
    using namespace std;
    ...
    if (getSpaceDimension() != 3 || getMeshDimension() != 3)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::conformize3D : This method only works for meshes with spaceDim=3 and meshDim=3!"
        );
    if (_types.size() != 1 || *(_types.begin()) != INTERP_KERNEL::NORM_POLYHED)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingUMesh::conformize3D : This method only works for polyhedrons! Call convertAllToPoly first."
        );

    MCAuto<MEDCouplingSkyLineArray> connSla(
        MEDCouplingSkyLineArray::BuildFromPolyhedronConn(getNodalConnectivity(), getNodalConnectivityIndex())
    );
...

    {
         ...
         *  STEP 1  -- faces
         *************************/
        MCAuto<DataArrayIdType> descDNU(DataArrayIdType::New()), descIDNU(DataArrayIdType::New()),
            revDesc(DataArrayIdType::New()), revDescI(DataArrayIdType::New());
        ...
        const mcIdType *cDesc(mDesc->getNodalConnectivity()->begin()),
            *cIDesc(mDesc->getNodalConnectivityIndex()->begin());
        MCAuto<MEDCouplingSkyLineArray> connSlaDesc(
            MEDCouplingSkyLineArray::New(mDesc->getNodalConnectivityIndex(), mDesc->getNodalConnectivity())
        );
        ...
        for (vector<pair<double, mcIdType> >::const_iterator it = S.begin(); it != S.end(); it++)
        {
            ...
            {
                bool col = INTERP_KERNEL::isColinear3D(
                    normalsP + faceIdx * SPACEDIM, normalsP + *(it2)*SPACEDIM, eps / carMeshSz
                );  // using rough relative epsilon
                if (*it2 != faceIdx && col)
            ...
            INTERP_KERNEL::TranslationRotationMatrix::Rotate3DTriangle(
                coo + SPACEDIM * (cDesc[cIDesc[faceIdx] + 1]),
                coo + SPACEDIM * (cDesc[cIDesc[faceIdx] + 2]),
                coo + SPACEDIM * (cDesc[cIDesc[faceIdx] + 3]),
                rotation
            );

            MCAuto<MEDCouplingUMesh> mPartRef(
                mDesc->buildPartOfMySelfSlice(faceIdx, faceIdx + 1, 1, false)
            );  // false=zipCoords is called
            MCAuto<MEDCouplingUMesh> mPartCand(
                mDesc->buildPartOfMySelf(&cands2[0], &cands2[0] + cands2.size(), false)
            );  // false=zipCoords is called
...

            {
                ...
                {
                    ostringstream oss;
                    oss << "MEDCouplingUMesh::conformize3D: Non expected non-conformity. Only simple (=partition-like) "
                           "non-conformities are handled. Face #"
                        << faceIdx << " violates this condition!";
                    throw INTERP_KERNEL::Exception(oss.str());
                }
            }
            ...
            {
                if (packsIds[jj] == -1)
                    // The below should never happen - if a face is used several times, with a different layout of the
                    // nodes it means that it is already conform, so it is *not* hit by the algorithm. The algorithm
                    // only hits faces which are actually used only once, by a single cell. This is different for edges
                    // below.
                    throw INTERP_KERNEL::Exception(
                        "MEDCouplingUMesh::conformize3D: Could not find face in connectivity! Internal error."
                    );
                else
            ...
            for (const mcIdType *ii = ids->begin(); ii != ids->end(); ii++)
            {
                ...
                const mcIdType *facePack2 =
                    connSlaDesc->getSimplePackSafePtr(faceIdx2, facePack2Sz);  // contains the type!
    ...
    MCAuto<DataArrayIdType> cAuto;
    cAuto.takeRef(_nodal_connec);
    MCAuto<DataArrayIdType> cIAuto;
    cIAuto.takeRef(_nodal_connec_index);
...

    {
        ...
        MCAuto<DataArrayIdType> desc(DataArrayIdType::New()), descI(DataArrayIdType::New()),
            revDesc(DataArrayIdType::New()), revDescI(DataArrayIdType::New());
        ...
        const mcIdType *cDesc2(mDesc2->getNodalConnectivity()->begin()),
            *cIDesc2(mDesc2->getNodalConnectivityIndex()->begin());
        ...
        for (vector<pair<double, mcIdType> >::const_iterator it = S.begin(); it != S.end(); it++)
        {
            ...
            for (vector<mcIdType>::const_iterator it2 = candidates.begin(); it2 != candidates.end(); it2++)
            {
                ...
                //    normalizing vCurr and vOther means multiplying by v^4, knowing that inside isColinear3D() the
                //    square (^2) of a dot product (^2) is computed
            ...
            INTERP_KERNEL::TranslationRotationMatrix rotation;
            INTERP_KERNEL::TranslationRotationMatrix::Rotate3DBipoint(
                coo + SPACEDIM * startNode, coo + SPACEDIM * endNode, rotation
            );
            MCAuto<MEDCouplingUMesh> mPartRef(
                mDesc2->buildPartOfMySelfSlice(eIdx, eIdx + 1, 1, false)
            );  // false=zipCoords is called
            MCAuto<MEDCouplingUMesh> mPartCand(
                mDesc2->buildPartOfMySelf(&cands2[0], &cands2[0] + cands2.size(), true)
            );  // true=zipCoords is NOT called
            ...
            // Eliminate all edges for which y or z is not null - those are colinear edges which are simply parallels,
            ...
            bool isSplit = OrderPointsAlongLine(
                mPartCand->_coords->getConstPointer(),
                nodeMap->begin()[startNode],
                nodeMap->begin()[endNode],
                mPartCand->getNodalConnectivity()->begin(),
                mPartCand->getNodalConnectivityIndex()->begin(),
                idsGoodLine->begin(),
                idsGoodLine->end(),
                /*out*/ insidePoints,
                hitSegs
        ...
        MCAuto<DataArrayIdType> superIdx(DataArrayIdType::New());
        superIdx->alloc(getNumberOfCells() + 1);
        superIdx->fillWithValue(0);
        MCAuto<DataArrayIdType> idx(DataArrayIdType::New());
        idx->alloc(1);
        idx->fillWithValue(0);
        MCAuto<DataArrayIdType> vals(DataArrayIdType::New());
        vals->alloc(0);
