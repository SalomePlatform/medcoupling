...
bool
MEDCouplingFieldDiscretizationOnNodesFE::isEqualIfNotWhy(
...
) const
{
    ...
    const MEDCouplingFieldDiscretizationOnNodesFE *otherC =
        dynamic_cast<const MEDCouplingFieldDiscretizationOnNodesFE *>(other);
    bool ret = otherC != 0;
    if (!ret)
...
MEDCouplingFieldDiscretizationOnNodesFE::getMeasureField(const MEDCouplingMesh *mesh, bool isAbs) const
{
    if (!mesh)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingFieldDiscretizationOnNodesFE::getMeasureField : mesh instance specified is NULL !"
        );
    throw INTERP_KERNEL::Exception(
        "getMeasureField on MEDCouplingFieldDiscretizationOnNodesFE : not implemented yet !"
    );
}
...
IsInside(
    ...
    const double locInReal[SPACEDIM],
    double locInRef[SPACEDIM]
)
{
    ...
    class Functor
    {
       ...
       public:
        Functor(
            const MEDCouplingGaussLocalization &gl,
            std::size_t nbPtsInCell,
            const double *ptsInCell,
            const double point[SPACEDIM]
        )
        ...
        std::vector<double> operator()(const std::vector<double> &x)
        {
            ...
            for (std::size_t ii = 0; ii < SPACEDIM; ii++) ret[ii] -= _point[ii];
            return ret;
        }
    };
    ...
    auto myJacobian =
        [&gl,
         nbPtsInCell,
         ptsInCell](const std::vector<double> &x, const std::vector<double> &, INTERP_KERNEL::DenseMatrix &jacobian)
    {
    ...
    for (std::size_t attemptId = 0; attemptId < nbPtsInCell + 1; ++attemptId)
    {
        ...
        try
        {
            ...
            ret = (check == false);  // looks strange but OK regarding newt (SolveWithNewton) at page 387 of numerical
                                     // recipes for semantic of check parameter
        }
...
)
{
    ...
    auto myJacobian =
        [&gl, &tangent](const std::vector<double> &x, const std::vector<double> &, INTERP_KERNEL::DenseMatrix &jacobian)
    {
        MEDCouplingGaussLocalization mygl(gl.getType(), gl.getRefCoords(), x, {1.0});
        MCAuto<DataArrayDouble> dshapeFunc = mygl.getDerivativeOfShapeFunctionValues();
        ...
        for (std::size_t i = 0; i < SPACEDIM - 1; ++i)
        {
            for (std::size_t j = i; j < SPACEDIM - 1; ++j)
            {
                ...
                for (std::size_t k = 0; k < SPACEDIM; ++k) res += tang[i][k] * tang[j][k];
                jacobian[i][j] = res;
                jacobian[j][i] = res;
            }
        }
    ...
    for (std::size_t attemptId = 0; attemptId < nbPtsInCell + 1; ++attemptId)
    {
        ...
        {
            vini =
                std::vector<double>(refCoo + (attemptId - 1) * (SPACEDIM - 1), refCoo + (attemptId) * (SPACEDIM - 1));
        }
...
void
MEDCouplingFieldDiscretizationOnNodesFE::getValueOn(
    const DataArrayDouble *arr, const MEDCouplingMesh *mesh, const double *ptsCoo, double *res
) const
{
...
)
{
    ...
    {
        case 1:
            MEDCouplingFieldDiscretizationOnNodesFE::GetRefCoordOfListOfNDPtsInND<1>(
                umesh, ptsCoo, nbOfPts, customFunc
            );
            break;

        case 2:
            MEDCouplingFieldDiscretizationOnNodesFE::GetRefCoordOfListOfNDPtsInND<2>(
                umesh, ptsCoo, nbOfPts, customFunc
            );
            break;

        case 3:
            MEDCouplingFieldDiscretizationOnNodesFE::GetRefCoordOfListOfNDPtsInND<3>(
                umesh, ptsCoo, nbOfPts, customFunc
            );
            break;
        default:
...
)
{
    ...
    {
        case 2:
            MEDCouplingFieldDiscretizationOnNodesFE::GetClosestRefCoordOfListOfNDPtsInND<2>(
                umesh, ptsCoo, nbOfPts, customFunc
            );
            break;

        case 3:
            MEDCouplingFieldDiscretizationOnNodesFE::GetClosestRefCoordOfListOfNDPtsInND<3>(
                umesh, ptsCoo, nbOfPts, customFunc
            );
            break;
        default:
...
)
{
    ...
    auto tree =
        INTERP_KERNEL::BuildBBTree<MEDCouplingNormalizedUnstructuredMesh<SPACEDIM, SPACEDIM>, SPACEDIM>(mesh_wrapper);
...
)
{
    ...
    THROW_IK_EXCEPTION(
        "GetRefCoordOfListOf1PtInND on MEDCouplingFieldDiscretizationOnNodesFE : fail to locate point " << mess << " !"
    );
}
...
)
{
    ...
    else
    {
        ...
        THROW_IK_EXCEPTION(
            "GetClosestRefCoordOfListOf1PtInND on MEDCouplingFieldDiscretizationOnNodesFE : fail to locate point "
            << mess << " !"
        );
    }
}
...
DataArrayDouble *
MEDCouplingFieldDiscretizationOnNodesFE::getValueOnMulti(
...
MEDCouplingFieldDiscretizationOnNodesFE::getCooInRefElement(
 ...
 */
MCAuto<DataArrayDouble>
MEDCouplingFieldDiscretizationOnNodesFE::getClosestCooInRefElement(
    const MEDCouplingMesh *mesh, const double *loc, mcIdType nbOfPoints
) const
{
...
template <int SPACEDIM>
void
MEDCouplingFieldDiscretizationOnNodesFE::computeCrudeMatrixNd(
    const MEDCouplingUMesh *srcMesh,
    const double *ptsCoo,
    const mcIdType nbOfPts,
    std::vector<std::map<mcIdType, double>> &matrix,
