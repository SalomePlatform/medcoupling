...
MEDCouplingFieldOverTime::New(const std::vector<MEDCouplingFieldDouble *> &fs)
{
    return new MEDCouplingFieldOverTime(fs);
}

double
MEDCouplingFieldOverTime::getTimeTolerance() const
{
...
void
MEDCouplingFieldOverTime::checkConsistencyLight() const
{
    ...
    for (; it != _fs.end(); it++)
        if ((*it)->getTimeDiscretization() == NO_TIME)
        {
            ...
            oss << "MEDCouplingFieldOverTime::checkConsistencyLight : At rank #" << std::distance(_fs.begin(), it)
                << " the field has no time !";
    ...
    for (; it != _fs.end(); it++, id++)
    {
        ...
        {
            std::ostringstream oss;
            oss << "Field slice at rank #" << id << " is not compatible with the first !";
            throw INTERP_KERNEL::Exception(oss.str().c_str());
        }
        double curt = (*it)->getStartTime(tt1, tt2);
        if (curt < reft - eps)
            throw INTERP_KERNEL::Exception(
                "MEDCouplingFieldOverTime::checkConsistencyLight : fields are NOT sorted properly in ascending time !"
            );
...
std::string
MEDCouplingFieldOverTime::simpleRepr() const
{
...
bool
MEDCouplingFieldOverTime::isEqual(const MEDCouplingMultiFields *other, double meshPrec, double valsPrec) const
{
...
bool
MEDCouplingFieldOverTime::isEqualWithoutConsideringStr(
...
MEDCouplingFieldOverTime::getMeshes() const
{
...
std::vector<MEDCouplingMesh *>
MEDCouplingFieldOverTime::getDifferentMeshes(std::vector<int> &refs) const
{
    checkConsistencyLight();
...
MEDCouplingFieldOverTime::getArrays() const
{
    checkConsistencyLight();
...
std::vector<DataArrayDouble *>
MEDCouplingFieldOverTime::getDifferentArrays(std::vector<std::vector<int> > &refs) const
{
    checkConsistencyLight();
...
MEDCouplingFieldOverTime::getDefinitionTimeZone() const
{
...
MEDCouplingFieldOverTime::MEDCouplingFieldOverTime(const std::vector<MEDCouplingFieldDouble *> &fs)
    : MEDCouplingMultiFields(fs)
{
    checkConsistencyLight();
}
