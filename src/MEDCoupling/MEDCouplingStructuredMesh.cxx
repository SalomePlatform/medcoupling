...
MEDCouplingStructuredMesh::MEDCouplingStructuredMesh(const MEDCouplingStructuredMesh &other, bool deepCpy)
    : MEDCouplingMesh(other)
{
}
...
std::size_t
MEDCouplingStructuredMesh::getHeapMemorySizeWithoutChildren() const
{
    return MEDCouplingMesh::getHeapMemorySizeWithoutChildren();
}

void
MEDCouplingStructuredMesh::copyTinyStringsFrom(const MEDCouplingMesh *other)
{
    MEDCouplingMesh::copyTinyStringsFrom(other);
}

bool
MEDCouplingStructuredMesh::isEqualIfNotWhy(const MEDCouplingMesh *other, double prec, std::string &reason) const
{
    return MEDCouplingMesh::isEqualIfNotWhy(other, prec, reason);
}

INTERP_KERNEL::NormalizedCellType
MEDCouplingStructuredMesh::getTypeOfCell(mcIdType cellId) const
{
    return GetGeoTypeGivenMeshDimension(getMeshDimension());
}

INTERP_KERNEL::NormalizedCellType
MEDCouplingStructuredMesh::GetGeoTypeGivenMeshDimension(int meshDim)
{
    switch (meshDim)
    {
        case 3:
        ...
        default:
            throw INTERP_KERNEL::Exception(
                "Unexpected dimension for MEDCouplingStructuredMesh::GetGeoTypeGivenMeshDimension !"
            );
    }
}

std::set<INTERP_KERNEL::NormalizedCellType>
MEDCouplingStructuredMesh::getAllGeoTypes() const
{
...
mcIdType
MEDCouplingStructuredMesh::getNumberOfCellsWithType(INTERP_KERNEL::NormalizedCellType type) const
{
    ...
    oss << "MEDCouplingStructuredMesh::getNumberOfCellsWithType : no specified type ! Type available is "
        << cm.getRepr() << " !";
    throw INTERP_KERNEL::Exception(oss.str().c_str());
}

DataArrayIdType *
MEDCouplingStructuredMesh::giveCellsWithType(INTERP_KERNEL::NormalizedCellType type) const
{
...
DataArrayIdType *
MEDCouplingStructuredMesh::computeNbOfNodesPerCell() const
{
...
DataArrayIdType *
MEDCouplingStructuredMesh::computeNbOfFacesPerCell() const
{
    std::size_t nbCells = getNumberOfCells();
    MCAuto<DataArrayIdType> ret = DataArrayIdType::New();
    ret->alloc(nbCells, 1);
 ...
 * MEDCouplingMesh::computeNbOfNodesPerCell method. Here for structured mesh it returns exactly as
 * MEDCouplingStructuredMesh::computeNbOfNodesPerCell does.
 *
 * \return DataArrayIdType * - new object to be deallocated by the caller.
 */
DataArrayIdType *
MEDCouplingStructuredMesh::computeEffectiveNbOfNodesPerCell() const
{
    return computeNbOfNodesPerCell();
}

void
MEDCouplingStructuredMesh::getNodeIdsOfCell(mcIdType cellId, std::vector<mcIdType> &conn) const
{
    ...
    {
        case 1:
            conn.push_back(tmp2[0]);
            conn.push_back(tmp2[0] + 1);
            break;
        case 2:
            conn.push_back(tmp2[1] * tmpNode[1] + tmp2[0]);
            conn.push_back(tmp2[1] * tmpNode[1] + tmp2[0] + 1);
            conn.push_back((tmp2[1] + 1) * tmpNode[1] + tmp2[0] + 1);
            conn.push_back((tmp2[1] + 1) * tmpNode[1] + tmp2[0]);
            break;
        case 3:
            conn.push_back(tmp2[1] * tmpNode[1] + tmp2[0] + tmp2[2] * tmpNode[2]);
            conn.push_back(tmp2[1] * tmpNode[1] + tmp2[0] + 1 + tmp2[2] * tmpNode[2]);
            conn.push_back((tmp2[1] + 1) * tmpNode[1] + tmp2[0] + 1 + tmp2[2] * tmpNode[2]);
            conn.push_back((tmp2[1] + 1) * tmpNode[1] + tmp2[0] + tmp2[2] * tmpNode[2]);
            conn.push_back(tmp2[1] * tmpNode[1] + tmp2[0] + (tmp2[2] + 1) * tmpNode[2]);
            conn.push_back(tmp2[1] * tmpNode[1] + tmp2[0] + 1 + (tmp2[2] + 1) * tmpNode[2]);
            conn.push_back((tmp2[1] + 1) * tmpNode[1] + tmp2[0] + 1 + (tmp2[2] + 1) * tmpNode[2]);
            conn.push_back((tmp2[1] + 1) * tmpNode[1] + tmp2[0] + (tmp2[2] + 1) * tmpNode[2]);
            break;
        default:
            throw INTERP_KERNEL::Exception(
                "MEDCouplingStructuredMesh::getNodeIdsOfCell : big problem spacedim must be in 1,2 or 3 !"
            );
    };
}

/*!
 * This method returns the mesh dimension of \a this. It can be different from space dimension in case of a not null
 * dimension contains only one node.
 */
int
MEDCouplingStructuredMesh::getMeshDimension() const
{
    ...
    for (std::vector<mcIdType>::const_iterator it = ngs.begin(); it != ngs.end(); it++, pos++)
    {
        ...
        {
            std::ostringstream oss;
            oss << "MEDCouplingStructuredMesh::getMeshDimension : At pos #" << pos << " number of nodes is " << *it
                << " ! Must be > 0 !";
            throw INTERP_KERNEL::Exception(oss.str().c_str());
        }
        if (*it > 1)
 ...
 */
int
MEDCouplingStructuredMesh::getSpaceDimensionOnNodeStruct() const
{
    ...
    for (std::vector<mcIdType>::const_iterator it = nodeStr.begin(); it != nodeStr.end(); it++, pos++)
    {
        ...
        {
            std::ostringstream oss;
            oss << "MEDCouplingStructuredMesh::getSpaceDimensionOnNodeStruct : At pos #" << pos
                << " value of node grid structure is " << *it << " ! must be >=1 !";
            throw INTERP_KERNEL::Exception(oss.str().c_str());
        }
        spd1++;
    }
...
void
MEDCouplingStructuredMesh::getSplitCellValues(mcIdType *res) const
{
...
void
MEDCouplingStructuredMesh::getSplitNodeValues(mcIdType *res) const
{
 ...
 */
mcIdType
MEDCouplingStructuredMesh::getNumberOfCellsOfSubLevelMesh() const
{
 ...
 */
std::vector<mcIdType>
MEDCouplingStructuredMesh::getDistributionOfTypes() const
{
 ...
 */
DataArrayIdType *
MEDCouplingStructuredMesh::checkTypeConsistencyAndContig(
...
) const
{
    ...
    if (code.size() != 3)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingStructuredMesh::checkTypeConsistencyAndContig : invalid input code should be exactly of size 3 "
            "!"
        );
    if (code[0] != ToIdType(getTypeOfCell(0)))
    {
        std::ostringstream oss;
        oss << "MEDCouplingStructuredMesh::checkTypeConsistencyAndContig : Mismatch of geometric type ! Asking for "
            << code[0] << " whereas the geometric type is \a this is " << getTypeOfCell(0) << " !";
        throw INTERP_KERNEL::Exception(oss.str().c_str());
    }
    if (code[2] == -1)
    {
        ...
        {
            std::ostringstream oss;
            oss << "MEDCouplingStructuredMesh::checkTypeConsistencyAndContig : mismatch between the number of cells in "
                   "this ("
    ...
    if (code[2] != 0)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingStructuredMesh::checkTypeConsistencyAndContig : single geo type mesh ! 0 or -1 is expected at "
            "pos #2 of input code !"
        );
    if (idsPerType.size() != 1)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingStructuredMesh::checkTypeConsistencyAndContig : input code points to DataArrayIdType #0 "
            "whereas the size of idsPerType is not equal to 1 !"
        );
    const DataArrayIdType *pfl = idsPerType[0];
    if (!pfl)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingStructuredMesh::checkTypeConsistencyAndContig : the input code points to a NULL "
            "DataArrayIdType at rank 0 !"
        );
    if (pfl->getNumberOfComponents() != 1)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingStructuredMesh::checkTypeConsistencyAndContig : input profile should have exactly one "
            "component !"
        );
 ...
 */
void
MEDCouplingStructuredMesh::splitProfilePerType(
...
) const
{
    if (!profile || !profile->isAllocated())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingStructuredMesh::splitProfilePerType : input profile is NULL or not allocated !"
        );
    if (profile->getNumberOfComponents() != 1)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingStructuredMesh::splitProfilePerType : input profile should have exactly one component !"
        );
    mcIdType nbTuples(profile->getNumberOfTuples());
    mcIdType nbOfCells = getNumberOfCells();
    code.resize(3);
    idsInPflPerType.resize(1);
    code[0] = ToIdType(getTypeOfCell(0));
    code[1] = nbOfCells;
    idsInPflPerType.resize(1);
 ...
 *
 * In the returned mesh, the nodes are ordered with the first axis varying first: (X0,Y0), (X1,Y0),  ... (X0,Y1),
 * (X1,Y1), ... and the cells are ordered with the same logic, i.e. in (i,j) notation: (0,0), (1,0), (2,0), ... (0,1),
 * (1,1), ...
 *
...
MEDCouplingStructuredMesh::build1SGTUnstructured() const
{
    int meshDim(getMeshDimension()), spaceDim(getSpaceDimensionOnNodeStruct());
    if ((meshDim < 0 || meshDim > 3) || (spaceDim < 0 || spaceDim > 3))
        throw INTERP_KERNEL::Exception(
            "MEDCouplingStructuredMesh::build1SGTUnstructured : meshdim and spacedim must be in [1,2,3] !"
        );
 ...
 * This method is equivalent to computing MEDCouplingUMesh::buildDescendingConnectivity on the unstructurized \a this
 * mesh.
 *
 * The caller is to delete the returned mesh using decrRef() as it is no more needed.
 */
MEDCoupling1SGTUMesh *
MEDCouplingStructuredMesh::build1SGTSubLevelMesh() const
{
    int meshDim(getMeshDimension());
 ...
 */
MEDCouplingUMesh *
MEDCouplingStructuredMesh::buildUnstructured() const
{
 ...
 */
MEDCouplingMesh *
MEDCouplingStructuredMesh::buildPart(const mcIdType *start, const mcIdType *end) const
{
...
MEDCouplingMesh *
MEDCouplingStructuredMesh::buildPartAndReduceNodes(
...
) const
{
    ...
    if (IsPartStructured(start, end, cgs, cellPartFormat))
    {
        ...
        for (std::vector<std::pair<mcIdType, mcIdType> >::iterator it = nodePartFormat.begin();
             it != nodePartFormat.end();
             it++)
            (*it).second++;
        MCAuto<DataArrayIdType> tmp1(BuildExplicitIdsFrom(getNodeGridStructure(), nodePartFormat));
        MCAuto<DataArrayIdType> tmp2(DataArrayIdType::New());
        tmp2->alloc(getNumberOfNodes(), 1);
        tmp2->fillWithValue(-1);
        MCAuto<DataArrayIdType> tmp3(DataArrayIdType::New());
        tmp3->alloc(tmp1->getNumberOfTuples(), 1);
        tmp3->iota(0);
...
DataArrayIdType *
MEDCouplingStructuredMesh::simplexize(int policy)
{
...
MEDCouplingStructuredMesh::buildOrthogonalField() const
{
...
void
MEDCouplingStructuredMesh::getReverseNodalConnectivity(DataArrayIdType *revNodal, DataArrayIdType *revNodalIndx) const
{
    std::vector<mcIdType> ngs(getNodeGridStructure());
    ...
    switch (dim)
    {
        ...
        default:
            throw INTERP_KERNEL::Exception(
                "MEDCouplingStructuredMesh::getReverseNodalConnectivity : only dimensions 1, 2 and 3 are supported !"
            );
    }
}

void
MEDCouplingStructuredMesh::GetReverseNodalConnectivity1(
    const std::vector<mcIdType> &ngs, DataArrayIdType *revNodal, DataArrayIdType *revNodalIndx
)
{
    ...
    {
        revNodal->alloc(0, 1);
        revNodalIndx->setIJ(0, 0, 0);
        return;
    }
    if (nbNodes == 1)
    {
        revNodal->alloc(1, 1);
        revNodal->setIJ(0, 0, 0);
        revNodalIndx->setIJ(0, 0, 0);
        revNodalIndx->setIJ(1, 0, 1);
        return;
    }
...
void
MEDCouplingStructuredMesh::GetReverseNodalConnectivity2(
    const std::vector<mcIdType> &ngs, DataArrayIdType *revNodal, DataArrayIdType *revNodalIndx
)
{
    ...
    {
        std::vector<mcIdType> ngs2(1);
        ngs2[0] = std::max(nbNodesX, nbNodesY);
        return GetReverseNodalConnectivity1(ngs2, revNodal, revNodalIndx);
    }
    revNodalIndx->alloc(nbNodes + 1, 1);
    ...
    *rn++ = off3;
    ...
    rni[1] = rni[0] + 1;
    rn[0] = nbCellsX * nbCellsY - 1;
}

void
MEDCouplingStructuredMesh::GetReverseNodalConnectivity3(
    const std::vector<mcIdType> &ngs, DataArrayIdType *revNodal, DataArrayIdType *revNodalIndx
)
{
    ...
    revNodal->alloc(
        8 * (nbNodesX - 2) * (nbNodesY - 2) * (nbNodesZ - 2) +
            4 * (2 * (nbNodesX - 2) * (nbNodesY - 2) + 2 * (nbNodesX - 2) * (nbNodesZ - 2) +
                 2 * (nbNodesY - 2) * (nbNodesZ - 2)) +
            2 * 4 * (nbNodesX - 2) + 2 * 4 * (nbNodesY - 2) + 2 * 4 * (nbNodesZ - 2) + 8,
        1
    );
    mcIdType *rn(revNodal->getPointer()), *rni(revNodalIndx->getPointer());
    *rni = 0;
    for (mcIdType k = 0; k < nbNodesZ; k++)
    {
        ...
        for (mcIdType j = 0; j < nbNodesY; j++)
        {
            ...
            mcIdType off00((j - 1) * nbCellsX + offZ0), off01(j * nbCellsX + offZ0), off10((j - 1) * nbCellsX + offZ1),
                off11(j * nbCellsX + offZ1);
...
/*!
 * \return DataArrayIdType * - newly allocated instance of nodal connectivity compatible for MEDCoupling1SGTMesh
 * instance
 */
DataArrayIdType *
MEDCouplingStructuredMesh::Build1GTNodalConnectivity(const mcIdType *nodeStBg, const mcIdType *nodeStEnd)
{
    ...
    switch (dim)
    {
        ...
        default:
            throw INTERP_KERNEL::Exception(
                "MEDCouplingStructuredMesh::Build1GTNodalConnectivity : only dimension in [0,1,2,3] supported !"
            );
    }
}

DataArrayIdType *
MEDCouplingStructuredMesh::Build1GTNodalConnectivityOfSubLevelMesh(const mcIdType *nodeStBg, const mcIdType *nodeStEnd)
{
    ...
    switch (dim)
    {
        ...
        default:
            throw INTERP_KERNEL::Exception(
                "MEDCouplingStructuredMesh::Build1GTNodalConnectivityOfSubLevelMesh: only dimension in [2,3] supported "
                "!"
            );
    }
}
 ...
 */
DataArrayIdType *
MEDCouplingStructuredMesh::ComputeCornersGhost(const std::vector<mcIdType> &st, mcIdType ghostLev)
{
    if (ghostLev < 0)
    ...
    switch (dim)
    {
        case 1:
        {
            ...
            for (mcIdType i = 0; i < ghostLev; i++, ptr++) *ptr = i;
            mcIdType offset(st[0]);
            if (offset < 0)
                throw INTERP_KERNEL::Exception(
                    "MEDCouplingStructuredMesh::ComputeCornersGhost : element in 1D structure must be >= 0 !"
                );
            for (mcIdType i = 0; i < ghostLev; i++, ptr++) *ptr = offset + ghostLev + i;
            break;
        }
        case 2:
        {
            ...
            if (offsetX < 0 || offsetY < 0)
                throw INTERP_KERNEL::Exception(
                    "MEDCouplingStructuredMesh::ComputeCornersGhost : elements in 2D structure must be >= 0 !"
                );
            ...
            {
                *ptr++ =
                    (2 * ghostLev + offsetX) * (offsetY + ghostLev) + ghostLev - 1 + i * (2 * ghostLev + offsetX - 1);
                *ptr++ = (2 * ghostLev + offsetX) * (offsetY + ghostLev) + offsetX + ghostLev +
                         i * (2 * ghostLev + offsetX + 1);
            }
            break;
        }
        case 3:
        {
            ...
            if (offsetX < 0 || offsetY < 0 || offsetZ < 0)
                throw INTERP_KERNEL::Exception(
                    "MEDCouplingStructuredMesh::ComputeCornersGhost : elements in 3D structure must be >= 0 !"
                );
            ...
            for (mcIdType i = 0; i < ghostLev; i++)
            {
                ...
                *ptr++ = (2 * ghostLev + offsetX) * (offsetY + ghostLev) + ghostLev - 1 +
                         (ghostLev - i - 1) * (2 * ghostLev + offsetX - 1) + i * zeOffsetZ;
                *ptr++ = (2 * ghostLev + offsetX) * (offsetY + ghostLev) + offsetX + ghostLev +
                         (ghostLev - i - 1) * (2 * ghostLev + offsetX + 1) + i * zeOffsetZ;
            }
            ...
            for (mcIdType i = ghostLev - 1; i >= 0; i--, j++)
            {
                ...
                *ptr++ = (2 * ghostLev + offsetX) * (offsetY + ghostLev) + ghostLev - 1 +
                         (ghostLev - i - 1) * (2 * ghostLev + offsetX - 1) + j * zeOffsetZ + zeOffsetZ2;
                *ptr++ = (2 * ghostLev + offsetX) * (offsetY + ghostLev) + offsetX + ghostLev +
                         (ghostLev - i - 1) * (2 * ghostLev + offsetX + 1) + j * zeOffsetZ + zeOffsetZ2;
            }
            break;
        ...
        default:
            throw INTERP_KERNEL::Exception(
                "MEDCouplingStructuredMesh::ComputeCornersGhost : Only dimensions 1, 2 and 3 are supported actually !"
            );
    }
    return ret.retn();
}
...
)
{
    mcIdType ret(1);
    std::size_t ii(0);
    for (std::vector<std::pair<mcIdType, mcIdType> >::const_iterator it = partCompactFormat.begin();
         it != partCompactFormat.end();
         it++, ii++)
    {
        ...
        {
            std::ostringstream oss;
            oss << "MEDCouplingStructuredMesh::DeduceNumberOfGivenRangeInCompactFrmt : invalid input at dimension "
                << ii << " !";
            throw INTERP_KERNEL::Exception(oss.str().c_str());
        }
        ret *= (b - a);
...
mcIdType
MEDCouplingStructuredMesh::DeduceNumberOfGivenStructure(const std::vector<mcIdType> &st)
{
    mcIdType ret(1);
    ...
    for (std::size_t i = 0; i < st.size(); i++)
    {
        if (st[i] < 0)
            throw INTERP_KERNEL::Exception(
                "MEDCouplingStructuredMesh::DeduceNumberOfGivenStructure : presence of a negative value in structure !"
            );
        ret *= st[i];
        isFetched = true;
    }
...
MEDCouplingStructuredMesh::FindTheWidestAxisOfGivenRangeInCompactFrmt(
    const std::vector<std::pair<mcIdType, mcIdType> > &partCompactFormat, int &axisId, mcIdType &sizeOfRange
)
{
    ...
    for (int i = 0; i < dim; i++)
    {
        ...
        {
            std::ostringstream oss;
            oss << "MEDCouplingStructuredMesh::FindTheWidestAxisOfGivenRangeInCompactFrmt : at axis #" << i
                << " the range is invalid (first value < second value) !";
            throw INTERP_KERNEL::Exception(oss.str().c_str());
        }
        if (curDelta > ret)
        {
            axisId = i;
            sizeOfRange = curDelta;
            ret = curDelta;
        }
    }
}
 ...
 * This method find for such minimalist information of mesh and field which is the part of the mesh, given by the range
 * per axis in output parameter
 * \a partCompactFormat that contains all the True in \a crit. The returned vector of boolean is the field reduced to
 * that part. So the number of True is equal in \a st and in returned vector of boolean.
 *
...
)
{
    if (minPatchLgth < 0)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingStructuredMesh::FindMinimalPartOf : the input minPatchLgth has to be >=0 !"
        );
    if (ToIdType(crit.size()) != DeduceNumberOfGivenStructure(st))
        throw INTERP_KERNEL::Exception(
            "MEDCouplingStructuredMesh::FindMinimalPartOf : size of vector of boolean is invalid regarding the "
            "declared structure !"
        );
    mcIdType ret(-1);
    switch (st.size())
    {
        ...
        default:
            throw INTERP_KERNEL::Exception(
                "MEDCouplingStructuredMesh::FindMinimalPartOf : only dimension 1, 2 and 3 are supported actually !"
            );
    }
    std::vector<mcIdType> dims(MEDCouplingStructuredMesh::GetDimensionsFromCompactFrmt(partCompactFormat));
    mcIdType i(0);
    for (std::vector<std::pair<mcIdType, mcIdType> >::iterator it = partCompactFormat.begin();
         it != partCompactFormat.end();
         it++, i++)
    {
        if (st[i] < minPatchLgth)
            throw INTERP_KERNEL::Exception(
                "MEDCouplingStructuredMesh::FindMinimalPartOf : the input patch is tinier than the min length "
                "constraint !"
            );
 ...
 * This method considers \a crit input parameter as a matrix having dimensions specified by \a st. This method returns
 * for each axis the signature, that is to say the number of elems equal to true in \a crit along this axis.
 */
std::vector<std::vector<mcIdType> >
MEDCouplingStructuredMesh::ComputeSignaturePerAxisOf(const std::vector<mcIdType> &st, const std::vector<bool> &crit)
{
    std::size_t dim(st.size());
    ...
    switch (dim)
    {
        case 1:
        {
            ...
            for (mcIdType i = 0; i < nx; i++) retX[i] = crit[i] ? 1 : 0;
            break;
        }
        case 2:
        {
        ...
        default:
            throw INTERP_KERNEL::Exception(
                "MEDCouplingStructuredMesh::ComputeSignatureOf : only dimensions 1, 2 and 3 are supported !"
            );
    }
    return ret;
}
...
DataArrayIdType *
MEDCouplingStructuredMesh::Build1GTNodalConnectivity1D(const mcIdType *nodeStBg)
{
...
DataArrayIdType *
MEDCouplingStructuredMesh::Build1GTNodalConnectivity2D(const mcIdType *nodeStBg)
{
...
DataArrayIdType *
MEDCouplingStructuredMesh::Build1GTNodalConnectivity3D(const mcIdType *nodeStBg)
{
...
DataArrayIdType *
MEDCouplingStructuredMesh::Build1GTNodalConnectivityOfSubLevelMesh3D(const mcIdType *nodeStBg)
{
    std::vector<mcIdType> ngs(3);
    mcIdType n0(nodeStBg[0] - 1), n1(nodeStBg[1] - 1), n2(nodeStBg[2] - 1);
    ngs[0] = n0;
    ngs[1] = n1;
    ngs[2] = n2;
    ...
    for (mcIdType i = 0; i < nodeStBg[0]; i++)
        for (mcIdType j = 0; j < n1; j++)
            ...
            {
                cp[0] = k * off1 + j * off0 + i;
                cp[1] = (k + 1) * off1 + j * off0 + i;
                cp[2] = (k + 1) * off1 + (j + 1) * off0 + i;
                cp[3] = k * off1 + (j + 1) * off0 + i;
            }
    // Y
    for (mcIdType j = 0; j < nodeStBg[1]; j++)
        for (mcIdType i = 0; i < n0; i++)
            for (mcIdType k = 0; k < n2; k++, cp += 4)
            {
                cp[0] = k * off1 + j * off0 + i;
                cp[1] = (k + 1) * off1 + j * off0 + i;
                cp[2] = (k + 1) * off1 + j * off0 + (i + 1);
                cp[3] = k * off1 + j * off0 + (i + 1);
            }
    // Z
    for (mcIdType k = 0; k < nodeStBg[2]; k++)
        for (mcIdType i = 0; i < n0; i++)
            ...
            {
                cp[0] = k * off1 + j * off0 + i;
                cp[1] = k * off1 + j * off0 + (i + 1);
                cp[2] = k * off1 + (j + 1) * off0 + (i + 1);
                cp[3] = k * off1 + (j + 1) * off0 + i;
            }
    return conn.retn();
}
 ...
 */
mcIdType
MEDCouplingStructuredMesh::FindMinimalPartOf1D(
    const std::vector<mcIdType> &st,
    const std::vector<bool> &crit,
    std::vector<std::pair<mcIdType, mcIdType> > &partCompactFormat
)
{
    if (st.size() != 1)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingStructuredMesh::FindMinimalPartOf1D : the input size of st must be equal to 1 !"
        );
    ...
    for (mcIdType i = 0; i < nx; i++)
    {
        if (crit[i])
        {
            nxMin = std::min(nxMin, i);
            nxMax = std::max(nxMax, i);
            ret++;
        }
    }
    if (ret == 0)
    ...
    partCompactFormat[0].first = nxMin;
    partCompactFormat[0].second = nxMax + 1;
    return ret;
}
 ...
 */
mcIdType
MEDCouplingStructuredMesh::FindMinimalPartOf2D(
    const std::vector<mcIdType> &st,
    const std::vector<bool> &crit,
    std::vector<std::pair<mcIdType, mcIdType> > &partCompactFormat
)
{
    if (st.size() != 2)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingStructuredMesh::FindMinimalPartOf2D : the input size of st must be equal to 2 !"
        );
    mcIdType nxMin(std::numeric_limits<mcIdType>::max()), nxMax(-std::numeric_limits<mcIdType>::max()),
        nyMin(std::numeric_limits<mcIdType>::max()), nyMax(-std::numeric_limits<mcIdType>::max());
    ...
    for (mcIdType i = 0; i < ny; i++)
        for (mcIdType j = 0; j < nx; j++, it++)
        {
            if (crit[it])
            {
                nxMin = std::min(nxMin, j);
                nxMax = std::max(nxMax, j);
                nyMin = std::min(nyMin, i);
                nyMax = std::max(nyMax, i);
                ret++;
            }
        }
    if (ret == 0)
    ...
    partCompactFormat[0].first = nxMin;
    partCompactFormat[0].second = nxMax + 1;
    partCompactFormat[1].first = nyMin;
    partCompactFormat[1].second = nyMax + 1;
    return ret;
}
 ...
 */
mcIdType
MEDCouplingStructuredMesh::FindMinimalPartOf3D(
    const std::vector<mcIdType> &st,
    const std::vector<bool> &crit,
    std::vector<std::pair<mcIdType, mcIdType> > &partCompactFormat
)
{
    if (st.size() != 3)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingStructuredMesh::FindMinimalPartOf3D : the input size of st must be equal to 3 !"
        );
    mcIdType nxMin(std::numeric_limits<mcIdType>::max()), nxMax(-std::numeric_limits<mcIdType>::max()),
        nyMin(std::numeric_limits<mcIdType>::max()), nyMax(-std::numeric_limits<mcIdType>::max()),
        nzMin(std::numeric_limits<mcIdType>::max()), nzMax(-std::numeric_limits<mcIdType>::max());
    ...
    for (mcIdType i = 0; i < nz; i++)
        for (mcIdType j = 0; j < ny; j++)
            for (mcIdType k = 0; k < nx; k++, it++)
            {
                if (crit[it])
                {
                    nxMin = std::min(nxMin, k);
                    nxMax = std::max(nxMax, k);
                    nyMin = std::min(nyMin, j);
                    nyMax = std::max(nyMax, j);
                    nzMin = std::min(nzMin, i);
                    nzMax = std::max(nzMax, i);
                    ret++;
                }
            }
    if (ret == 0)
    ...
    partCompactFormat[1].first = nyMin;
    partCompactFormat[1].second = nyMax + 1;
    partCompactFormat[2].first = nzMin;
    partCompactFormat[2].second = nzMax + 1;
    return ret;
}
 ...
 */
int
MEDCouplingStructuredMesh::ZipNodeStructure(const mcIdType *nodeStBg, const mcIdType *nodeStEnd, mcIdType zipNodeSt[3])
{
    ...
    for (std::size_t i = 0; i < spaceDim; i++)
    {
        ...
        {
            std::ostringstream oss;
            oss << "MEDCouplingStructuredMesh::ZipNodeStructure : the input nodal structure at pos#" << i << "("
                << nodeStBg[i] << ") is invalid !";
            throw INTERP_KERNEL::Exception(oss.str().c_str());
        }
        if (elt >= 2)
...
DataArrayIdType *
MEDCouplingStructuredMesh::Build1GTNodalConnectivityOfSubLevelMesh2D(const mcIdType *nodeStBg)
{
    std::vector<mcIdType> ngs(2);
    mcIdType n0(nodeStBg[0] - 1), n1(nodeStBg[1] - 1);
    ngs[0] = n0;
    ngs[1] = n1;
 ...
 */
mcIdType
MEDCouplingStructuredMesh::getCellIdFromPos(mcIdType i, mcIdType j, mcIdType k) const
{
 ...
 */
mcIdType
MEDCouplingStructuredMesh::getNodeIdFromPos(mcIdType i, mcIdType j, mcIdType k) const
{
    mcIdType tmp[3] = {i, j, k};
    mcIdType tmp2[3];
...
mcIdType
MEDCouplingStructuredMesh::getNumberOfCells() const
{
    std::vector<mcIdType> ngs(getNodeGridStructure());
    mcIdType ret(1);
    ...
    for (std::vector<mcIdType>::const_iterator it = ngs.begin(); it != ngs.end(); it++, ii++)
    {
        ...
        {
            std::ostringstream oss;
            oss << "MEDCouplingStructuredMesh::getNumberOfCells : at pos #" << ii
                << " the number of nodes in nodeStructure is " << *it << " ! Must be > 0 !";
            throw INTERP_KERNEL::Exception(oss.str().c_str());
        }
        if (elt > 1)
        {
...
mcIdType
MEDCouplingStructuredMesh::getNumberOfNodes() const
{
    std::vector<mcIdType> ngs(getNodeGridStructure());
    mcIdType ret(1);
    for (std::vector<mcIdType>::const_iterator it = ngs.begin(); it != ngs.end(); it++) ret *= *it;
    return ret;
}
 ...
 */
std::vector<mcIdType>
MEDCouplingStructuredMesh::getLocationFromCellId(mcIdType cellId) const
{
    int meshDim(getMeshDimension());
    ...
    {
        std::ostringstream oss;
        oss << "MEDCouplingStructuredMesh::getLocationFromCellId : Input cell id (" << cellId
            << ") is invalid ! Should be in [0," << nbCells << ") !";
        throw INTERP_KERNEL::Exception(oss.str().c_str());
    }
 ...
 */
std::vector<mcIdType>
MEDCouplingStructuredMesh::getLocationFromNodeId(mcIdType nodeId) const
{
    ...
    {
        std::ostringstream oss;
        oss << "MEDCouplingStructuredMesh::getLocationFromNodeId : Input node id (" << nodeId
            << ") is invalid ! Should be in [0," << nbNodes << ") !";
        throw INTERP_KERNEL::Exception(oss.str().c_str());
    }
    std::vector<mcIdType> spt(GetSplitVectFromStruct(struc));
...
void
MEDCouplingStructuredMesh::GetPosFromId(mcIdType eltId, int meshDim, const mcIdType *split, mcIdType *res)
{
    mcIdType work(eltId);
...
std::vector<mcIdType>
MEDCouplingStructuredMesh::getCellGridStructure() const
{
...
/*!
 * This method returns the squareness of \a this (quadrature). \a this is expected to be with a mesh dimension equal to
 * 2 or 3.
 */
double
MEDCouplingStructuredMesh::computeSquareness() const
{
    std::vector<mcIdType> cgs(getCellGridStructure());
    if (cgs.empty())
 ...
 */
std::vector<mcIdType>
MEDCouplingStructuredMesh::GetSplitVectFromStruct(const std::vector<mcIdType> &strct)
{
    ...
    {
        mcIdType val = 1;
        for (std::size_t p = 0; p < spaceDim - l - 1; p++) val *= strct[p];
...
/*!
 * This method states if given part ids [ \a startIds, \a stopIds) and a structure \a st returns if it can be considered
 * as a structured dataset. If true is returned \a partCompactFormat will contain the information to build the
 * corresponding part.
 *
...
)
{
    ...
    if (dim < 1 || dim > 3)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingStructuredMesh::isPartStructured : input structure must be of dimension in [1,2,3] !"
        );
    std::vector<mcIdType> tmp2(dim), tmp(dim), tmp3(dim), tmp4(dim);
    tmp2[0] = 1;
    for (int i = 1; i < dim; i++) tmp2[i] = tmp2[i - 1] * st[i - 1];
    ...
    partCompactFormat.resize(dim);
    for (int i = 0; i < dim; i++) partCompactFormat[i].first = tmp[i];
    if (tmp[dim - 1] < 0 || tmp[dim - 1] >= st[dim - 1])
        throw INTERP_KERNEL::Exception(
            "MEDCouplingStructuredMesh::IsPartStructured : first id in input is not in valid range !"
        );
    if (sz == 1)
    {
        for (int i = 0; i < dim; i++) partCompactFormat[i].second = tmp[i] + 1;
        return true;
    }
    ...
    for (int i = 0; i < dim; i++)
    {
        if (tmp3[i] < 0 || tmp3[i] >= st[i])
            throw INTERP_KERNEL::Exception(
                "MEDCouplingStructuredMesh::IsPartStructured : last id in input is not in valid range !"
            );
...
/*!
 * This method takes in input a compact format [[Xmax,Xmin),[Ymin,Ymax)] and returns the corresponding dimensions for
 * each axis that is to say [Xmax-Xmin,Ymax-Ymin].
 *
 * \throw if an axis range is so that max<min
 * \sa GetCompactFrmtFromDimensions
 */
std::vector<mcIdType>
MEDCouplingStructuredMesh::GetDimensionsFromCompactFrmt(
    const std::vector<std::pair<mcIdType, mcIdType> > &partCompactFormat
)
{
    ...
    for (std::size_t i = 0; i < partCompactFormat.size(); i++)
    {
        ...
        {
            std::ostringstream oss;
            oss << "MEDCouplingStructuredMesh::GetDimensionsFromCompactFrmt : For axis #" << i
                << " end is before start !";
            throw INTERP_KERNEL::Exception(oss.str().c_str());
        }
...
/*!
 * This method takes in input a vector giving the number of entity per axis and returns for each axis a range starting
 * from [0,0...]
 *
...
MEDCouplingStructuredMesh::GetCompactFrmtFromDimensions(const std::vector<mcIdType> &dims)
{
    ...
    for (std::size_t i = 0; i < sz; i++)
    {
        ...
        {
            std::ostringstream oss;
            oss << "MEDCouplingStructuredMesh::GetDimensionsFromCompactFrmt : For axis #" << i << " dimension < 0 !";
            throw INTERP_KERNEL::Exception(oss.str().c_str());
        }
        ret[i].first = 0;
        ret[i].second = dims[i];
 ...
 */
std::vector<std::pair<mcIdType, mcIdType> >
MEDCouplingStructuredMesh::IntersectRanges(
    const std::vector<std::pair<mcIdType, mcIdType> > &r1, const std::vector<std::pair<mcIdType, mcIdType> > &r2
)
{
    std::size_t sz(r1.size());
    if (sz != r2.size())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingStructuredMesh::IntersectRanges : the two ranges must have the same dimension !"
        );
    std::vector<std::pair<mcIdType, mcIdType> > ret(sz);
    for (std::size_t i = 0; i < sz; i++)
    {
        ...
        {
            std::ostringstream oss;
            oss << "MEDCouplingStructuredMesh::IntersectRanges : On axis " << i
                << " of range r1, end is before start !";
            throw INTERP_KERNEL::Exception(oss.str().c_str());
        }
        if (r2[i].first > r2[i].second)
        {
            std::ostringstream oss;
            oss << "MEDCouplingStructuredMesh::IntersectRanges : On axis " << i
                << " of range r2, end is before start !";
            throw INTERP_KERNEL::Exception(oss.str().c_str());
        }
        ...
        if (ret[i].first > ret[i].second)
        {
            std::ostringstream oss;
            oss << "MEDCouplingStructuredMesh::IntersectRanges : On axis " << i
                << " the intersection of r1 and r2 is empty !";
            throw INTERP_KERNEL::Exception(oss.str().c_str());
        }
    }
    return ret;
...
/*!
 * This method states if \a r1 and \a r2 do overlap of not. If yes you can call IntersectRanges to know the intersection
 * area.
 *
 * \sa IntersectRanges
 */
...
)
{
    ...
    if (sz != r2.size())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingStructuredMesh::AreRangesIntersect : the two ranges must have the same dimension !"
        );
    for (std::size_t i = 0; i < sz; i++)
    {
        if (r1[i].first > r1[i].second)
        {
            std::ostringstream oss;
            oss << "MEDCouplingStructuredMesh::AreRangesIntersect : On axis " << i
                << " of range r1, end is before start !";
            throw INTERP_KERNEL::Exception(oss.str().c_str());
        }
        if (r2[i].first > r2[i].second)
...
/*!
 * This method is close to BuildExplicitIdsFrom except that instead of returning a DataArrayIdType instance containing
 * explicit ids it enable elems in the vector of booleans (for performance reasons). As it is method for performance,
 * this method is \b not available in python.
 *
 ...
 */
void
MEDCouplingStructuredMesh::SwitchOnIdsFrom(
    const std::vector<mcIdType> &st,
    const std::vector<std::pair<mcIdType, mcIdType> > &partCompactFormat,
...
)
{
    if (st.size() != partCompactFormat.size())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingStructuredMesh::SwitchOnIdsFrom : input arrays must have the same size !"
        );
    if (ToIdType(vectToSwitchOn.size()) != DeduceNumberOfGivenStructure(st))
        throw INTERP_KERNEL::Exception(
            "MEDCouplingStructuredMesh::SwitchOnIdsFrom : invalid size of input vector of boolean regarding the "
            "structure !"
        );
    ...
    switch (st.size())
    {
        ...
        case 2:
        {
            ...
            {
                mcIdType b = (partCompactFormat[1].first + j) * st[0];
                for (mcIdType k = 0; k < dims[0]; k++) vectToSwitchOn[partCompactFormat[0].first + k + b] = true;
            }
            break;
        }
        case 1:
        {
            for (mcIdType k = 0; k < dims[0]; k++) vectToSwitchOn[partCompactFormat[0].first + k] = true;
            break;
        }
        default:
            throw INTERP_KERNEL::Exception(
                "MEDCouplingStructuredMesh::SwitchOnIdsFrom : Dimension supported are 1,2 or 3 !"
            );
    }
}
 ...
 * This method is close to SwitchOnIdsFrom except that here, a sub field \a fieldOut is built starting from the input
 * field \a fieldOfBool having the structure \a st. The extraction is defined by \a partCompactFormat.
 *
 * \param [in] st The entity structure.
 * \param [in] fieldOfBool field of booleans having the size equal to \c
 * MEDCouplingStructuredMesh::DeduceNumberOfGivenStructure(st).
 * \param [in] partCompactFormat The compact subpart to be enabled.
 ...
 */
void
MEDCouplingStructuredMesh::ExtractFieldOfBoolFrom(
    const std::vector<mcIdType> &st,
...
)
{
    if (st.size() != partCompactFormat.size())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingStructuredMesh::ExtractFieldOfBoolFrom : input arrays must have the same size !"
        );
    if (ToIdType(fieldOfBool.size()) != DeduceNumberOfGivenStructure(st))
        throw INTERP_KERNEL::Exception(
            "MEDCouplingStructuredMesh::ExtractFieldOfBoolFrom : invalid size of input field of boolean regarding the "
            "structure !"
        );
    std::vector<mcIdType> dims(GetDimensionsFromCompactFrmt(partCompactFormat));
    ...
    switch (st.size())
    {
        ...
        case 2:
        {
            ...
            {
                mcIdType b = (partCompactFormat[1].first + j) * st[0];
                for (mcIdType k = 0; k < dims[0]; k++) fieldOut[it++] = fieldOfBool[partCompactFormat[0].first + k + b];
            }
            break;
        }
        case 1:
        {
            for (mcIdType k = 0; k < dims[0]; k++) fieldOut[it++] = fieldOfBool[partCompactFormat[0].first + k];
            break;
        }
        default:
            throw INTERP_KERNEL::Exception(
                "MEDCouplingStructuredMesh::ExtractFieldOfBoolFrom : Dimension supported are 1,2 or 3 !"
            );
    }
}
...
/*!
 * This method is close to SwitchOnIdsFrom except that here, a sub field \a fieldOut is built starting from the input
 * field \a fieldOfDbl having the structure \a st. The extraction is defined by \a partCompactFormat.
 *
 * \param [in] st The entity structure.
 * \param [in] fieldOfDbl field of doubles having a number of tuples equal to \c
 * MEDCouplingStructuredMesh::DeduceNumberOfGivenStructure(st).
 * \param [in] partCompactFormat The compact subpart to be enabled.
...
)
{
    if (!fieldOfDbl || !fieldOfDbl->isAllocated())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingStructuredMesh::ExtractFieldOfDoubleFrom : input array of double is NULL or not allocated!"
        );
    if (st.size() != partCompactFormat.size())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingStructuredMesh::ExtractFieldOfDoubleFrom : input arrays must have the same size !"
        );
    if (fieldOfDbl->getNumberOfTuples() != DeduceNumberOfGivenStructure(st))
        throw INTERP_KERNEL::Exception(
            "MEDCouplingStructuredMesh::ExtractFieldOfDoubleFrom : invalid size of input array of double regarding the "
            "structure !"
        );
    std::vector<mcIdType> dims(GetDimensionsFromCompactFrmt(partCompactFormat));
    mcIdType nbOfTuplesOfOutField(DeduceNumberOfGivenStructure(dims));
    ...
    ret->alloc(nbOfTuplesOfOutField, nbComp);
    ...
    switch (st.size())
    {
        case 3:
        {
            for (mcIdType i = 0; i < dims[2]; i++)
            {
                ...
                for (mcIdType j = 0; j < dims[1]; j++)
                {
                    ...
                    for (mcIdType k = 0; k < dims[0]; k++)
                        ptRet = std::copy(
                            fieldOfDblPtr + (partCompactFormat[0].first + k + b + a) * nbComp,
                            fieldOfDblPtr + (partCompactFormat[0].first + k + b + a + 1) * nbComp,
                            ptRet
                        );
                }
            }
        ...
        case 2:
        {
            for (mcIdType j = 0; j < dims[1]; j++)
            {
                ...
                for (mcIdType k = 0; k < dims[0]; k++)
                    ptRet = std::copy(
                        fieldOfDblPtr + (partCompactFormat[0].first + k + b) * nbComp,
                        fieldOfDblPtr + (partCompactFormat[0].first + k + b + 1) * nbComp,
                        ptRet
                    );
            }
            break;
        ...
        case 1:
        {
            for (mcIdType k = 0; k < dims[0]; k++)
                ptRet = std::copy(
                    fieldOfDblPtr + (partCompactFormat[0].first + k) * nbComp,
                    fieldOfDblPtr + (partCompactFormat[0].first + k + 1) * nbComp,
                    ptRet
                );
            break;
        }
        default:
            throw INTERP_KERNEL::Exception(
                "MEDCouplingStructuredMesh::ExtractFieldOfDoubleFrom : Dimension supported are 1,2 or 3 !"
            );
    }
    return ret.retn();
}
...
/*!
 * This method changes the reference of a part of structured mesh \a partOfBigInAbs define in absolute reference to a
 * new reference \a bigInAbs. So this method only performs a translation by doing \a partOfBigRelativeToBig = \a
 * partOfBigInAbs - \a bigInAbs This method also checks (if \a check=true) that \a partOfBigInAbs is included in \a
 * bigInAbs. This method is useful to extract a part from a field lying on a big mesh.
 *
...
MEDCouplingStructuredMesh::ChangeReferenceFromGlobalOfCompactFrmt(
    const std::vector<std::pair<mcIdType, mcIdType> > &bigInAbs,
...
)
{
    ...
    if (dim != partOfBigInAbs.size())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingStructuredMesh::ChangeReferenceFromGlobalOfCompactFrmt : The size of parts (dimension) must be "
            "the same !"
        );
    ...
    for (std::size_t i = 0; i < dim; i++)
    {
        if (check)
        {
            ...
            {
                std::ostringstream oss;
                oss << "MEDCouplingStructuredMesh::ChangeReferenceFromGlobalOfCompactFrmt : Error at axis #" << i
                    << " the input big part invalid, end before start !";
                throw INTERP_KERNEL::Exception(oss.str().c_str());
            }
            if (partOfBigInAbs[i].first < bigInAbs[i].first || partOfBigInAbs[i].first >= bigInAbs[i].second)
            {
                std::ostringstream oss;
                oss << "MEDCouplingStructuredMesh::ChangeReferenceFromGlobalOfCompactFrmt : Error at axis #" << i
                    << " the part is not included in the big one (start) !";
                throw INTERP_KERNEL::Exception(oss.str().c_str());
            }
        }
        ...
        if (check)
        {
            if (partOfBigInAbs[i].second < partOfBigInAbs[i].first || partOfBigInAbs[i].second > bigInAbs[i].second)
            {
                std::ostringstream oss;
                oss << "MEDCouplingStructuredMesh::ChangeReferenceFromGlobalOfCompactFrmt : Error at axis #" << i
                    << " the part is not included in the big one (end) !";
                throw INTERP_KERNEL::Exception(oss.str().c_str());
            }
        }
...
)
{
    ...
    if (dim != partOfBigRelativeToBig.size())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingStructuredMesh::ChangeReferenceToGlobalOfCompactFrmt : The size of parts (dimension) must be "
            "the same !"
        );
    ...
    for (std::size_t i = 0; i < dim; i++)
    {
        if (check)
        {
            ...
            {
                std::ostringstream oss;
                oss << "MEDCouplingStructuredMesh::ChangeReferenceToGlobalOfCompactFrmt : Error at axis #" << i
                    << " the input big part invalid, end before start !";
                throw INTERP_KERNEL::Exception(oss.str().c_str());
            }
            if (partOfBigRelativeToBig[i].first < 0 ||
                partOfBigRelativeToBig[i].first >= bigInAbs[i].second - bigInAbs[i].first)
            {
                std::ostringstream oss;
                oss << "MEDCouplingStructuredMesh::ChangeReferenceToGlobalOfCompactFrmt : Error at axis #" << i
                    << " the start of part is not in the big one !";
                throw INTERP_KERNEL::Exception(oss.str().c_str());
            }
        }
        ...
        if (check)
        {
            if (partOfBigRelativeToBig[i].second < partOfBigRelativeToBig[i].first ||
                partOfBigRelativeToBig[i].second > bigInAbs[i].second - bigInAbs[i].first)
            {
                std::ostringstream oss;
                oss << "MEDCouplingStructuredMesh::ChangeReferenceToGlobalOfCompactFrmt : Error at axis #" << i
                    << " the end of part is not in the big one !";
                throw INTERP_KERNEL::Exception(oss.str().c_str());
            }
        }
 ...
 */
std::vector<std::pair<mcIdType, mcIdType> >
MEDCouplingStructuredMesh::TranslateCompactFrmt(
    const std::vector<std::pair<mcIdType, mcIdType> > &part, const std::vector<mcIdType> &translation
)
{
 ...
 */
std::vector<mcIdType>
MEDCouplingStructuredMesh::FindTranslationFrom(
    const std::vector<std::pair<mcIdType, mcIdType> > &startingFrom,
 ...
 * SwitchOnIdsFrom, ExtractFieldOfBoolFrom, ExtractFieldOfDoubleFrom, MultiplyPartOf
 */
DataArrayIdType *
MEDCouplingStructuredMesh::BuildExplicitIdsFrom(
    const std::vector<mcIdType> &st, const std::vector<std::pair<mcIdType, mcIdType> > &partCompactFormat
)
{
    if (st.size() != partCompactFormat.size())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingStructuredMesh::BuildExplicitIdsFrom : input arrays must have the same size !"
        );
    ...
    switch (st.size())
    {
        case 3:
        {
            for (mcIdType i = 0; i < dims[2]; i++)
            {
                ...
                {
                    mcIdType b = (partCompactFormat[1].first + j) * st[0];
                    for (mcIdType k = 0; k < dims[0]; k++, pt++) *pt = partCompactFormat[0].first + k + b + a;
                }
            }
            break;
        }
        case 2:
        {
            ...
            {
                mcIdType b = (partCompactFormat[1].first + j) * st[0];
                for (mcIdType k = 0; k < dims[0]; k++, pt++) *pt = partCompactFormat[0].first + k + b;
            }
            break;
        }
        case 1:
        {
            for (mcIdType k = 0; k < dims[0]; k++, pt++) *pt = partCompactFormat[0].first + k;
            break;
        }
        default:
            throw INTERP_KERNEL::Exception(
                "MEDCouplingStructuredMesh::BuildExplicitIdsFrom : Dimension supported are 1,2 or 3 !"
            );
    }
    return ret.retn();
}
 ...
 * \param [in] part - the part in the structure ( \b without considering ghost cells) contained in grid whose structure
 * is defined by \a st.
...
)
{
    if (!da || !da->isAllocated())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingStructuredMesh::MultiplyPartOf : DataArrayDouble instance must be not NULL and allocated !"
        );
    if (st.size() != part.size())
        throw INTERP_KERNEL::Exception(
            "MEDCouplingStructuredMesh::MultiplyPartOf : input arrays must have the same size !"
        );
    std::vector<mcIdType> dims(st.size());
    for (std::size_t i = 0; i < st.size(); i++)
    {
    ...
    {
        std::ostringstream oss;
        oss << "MEDCouplingStructuredMesh::MultiplyPartOf : invalid nb of tuples ! Expected " << nbOfTuplesExp
            << " having " << da->getNumberOfTuples() << " !";
        throw INTERP_KERNEL::Exception(oss.str().c_str());
    }
    ...
    switch (st.size())
    {
        ...
        default:
            throw INTERP_KERNEL::Exception(
                "MEDCouplingStructuredMesh::MultiplyPartOf : Dimension supported are 1,2 or 3 !"
            );
    }
}
 ...
 */
void
MEDCouplingStructuredMesh::PutInGhostFormat(
    mcIdType ghostSize,
    const std::vector<mcIdType> &st,
    const std::vector<std::pair<mcIdType, mcIdType> > &part,
...
)
{
    ...
    if (st.size() != dim)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingStructuredMesh::PutInGhostFormat : the dimension of input vectors must be the same !"
        );
    for (std::size_t i = 0; i < dim; i++)
        if (part[i].first < 0 || part[i].first > part[i].second || part[i].second > st[i])
            throw INTERP_KERNEL::Exception(
                "MEDCouplingStructuredMesh::PutInGhostFormat : the specified part is invalid ! The begin must be >= 0 "
                "and <= end ! The end must be <= to the size at considered dimension !"
            );
    ...
    std::transform(
        ...
        st.end(),
        stWithGhost.begin(),
        std::bind(std::plus<mcIdType>(), std::placeholders::_1, 2 * ghostSize)
    );
    partWithGhost = part;
 ...
 */
void
MEDCouplingStructuredMesh::ApplyGhostOnCompactFrmt(
    std::vector<std::pair<mcIdType, mcIdType> > &partBeforeFact, mcIdType ghostSize
)
{
    if (ghostSize < 0)
        throw INTERP_KERNEL::Exception(
            "MEDCouplingStructuredMesh::ApplyGhostOnCompactFrmt : ghost size must be >= 0 !"
        );
...
mcIdType
MEDCouplingStructuredMesh::GetNumberOfCellsOfSubLevelMesh(const std::vector<mcIdType> &cgs, int mdim)
{
    mcIdType ret(0);
