...
double
Interpolation3DTest::sumRow(const IntersectionMatrix &m, int i) const
{
    double vol = 0.0;
...
double
Interpolation3DTest::sumCol(const IntersectionMatrix &m, int i) const
{
    double vol = 0.0;
...
void
Interpolation3DTest::getVolumes(MEDCoupling::MEDCouplingUMesh &mesh, double *tab) const
{
...
double
Interpolation3DTest::sumVolume(const IntersectionMatrix &m) const
{
...
bool
Interpolation3DTest::testVolumes(const IntersectionMatrix &m, MEDCouplingUMesh &sMesh, MEDCouplingUMesh &tMesh) const
{
    bool ok = true;
...
bool
Interpolation3DTest::areCompatitable(const IntersectionMatrix &m1, const IntersectionMatrix &m2) const
{
...
bool
Interpolation3DTest::testSymmetric(const IntersectionMatrix &m1, const IntersectionMatrix &m2) const
{
    int i = 0;
    ...
    for (IntersectionMatrix::const_iterator iter = m1.begin(); iter != m1.end(); ++iter)
    {
        for (std::map<int, double>::const_iterator iter2 = iter->begin(); iter2 != iter->end(); ++iter2)
        {
            ...
            {
                LOG(2,
                    "V1( " << i << ", " << j << ") = " << v1 << " which is different from V2( " << j - 1 << ", "
                           << i + 1 << ") = " << v2 << " | diff = " << v1 - v2);
...
bool
Interpolation3DTest::testDiagonal(const IntersectionMatrix &m) const
{
...
void
Interpolation3DTest::dumpIntersectionMatrix(const IntersectionMatrix &m) const
{
    int i = 0;
...
void
Interpolation3DTest::setUp()
{
    interpolator = new Interpolation3D();
}

void
Interpolation3DTest::tearDown()
{
    delete interpolator;
}

void
Interpolation3DTest::calcIntersectionMatrix(
...
void
Interpolation3DTest::intersectMeshes(
