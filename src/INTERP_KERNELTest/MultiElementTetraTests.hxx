...
class MultiElementTetraTests : public InterpolationTestSuite<3, 3>
{
   ...
   public:
    ...
    /// \brief Status : pass
    void dividedUnitTetraReflexive() { _testTools->intersectMeshes("DividedUnitTetra", "DividedUnitTetra", 0.1666667); }
    ...
    /// \brief Status : pass
    void dividedGenTetra() { _testTools->intersectMeshes("DividedGenTetra1", "DividedGenTetra2", 0.546329); }

    /// Large box in general position with 12 elements intersecting itself
    /// \brief Status : pass
    void tinyBoxReflexive() { _testTools->intersectMeshes("TinyBox", "TinyBox", 979200); }

    /// Small box in general position with 33 elements intersecting itself
    /// \brief Status : pass
    void boxReflexive() { _testTools->intersectMeshes("Box3", "Box3", 13.9954); }
    ...
    /// \brief Status : pass
    void moderateBoxSmallReflexive() { _testTools->intersectMeshes("BoxModSmall1", "BoxModSmall1", 1.44018e6); }

    /// Large box in general position with 2943 elements intersecting itself
    /// \brief Status : pass
    void boxReflexiveModerate() { _testTools->intersectMeshes("Box1Moderate", "Box1Moderate", 1.0e6); }

    /// Two intersecting boxes in general position with 12 and 18 elements
    /// \brief Status : pass
    void tetraBoxes() { _testTools->intersectMeshes("Box1", "Box2", 124.197); }

    /// Two intersecting boxes in general position with 430 and 544 elements
    /// \brief Status : pass
    void moderateBoxesSmaller() { _testTools->intersectMeshes("BoxModSmall1", "BoxModSmall2", 321853); }

    /// Two intersecting boxes in general position with 2943 and 3068 elements
    /// \brief Status : pass
    void moderateBoxes() { _testTools->intersectMeshes("Box1Moderate", "Box2Moderate", 376856); }
};
}  // namespace INTERP_TEST
