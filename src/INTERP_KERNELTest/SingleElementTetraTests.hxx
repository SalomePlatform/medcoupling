...
{
/**
 * \brief Class testing algorithm by intersecting simple meshes having only one element each. This serves mainly to
 * verify that the volume calculations between elements is correct.
 *
 */
class SingleElementTetraTests : public InterpolationTestSuite<3, 3>
{
   ...
   public:
    ...
    /// \brief Status : pass
    void tetraReflexiveUnit() { _testTools->intersectMeshes("UnitTetra", "UnitTetra", 1.0 / 6.0); }

    /// Tetrahedron mesh with itself
    /// \brief Status : pass
    void tetraReflexiveGeneral() { _testTools->intersectMeshes("GeneralTetra", "GeneralTetra", 0.428559); }

    /// Unit tetrahedron mesh intersecting slightly displaced copy of itself
    /// \brief Status : pass
    void tetraNudged() { _testTools->intersectMeshes("UnitTetra", "NudgedTetra", 0.142896); }

    /// Single-element unit tetrahedron mesh intersecting even slightly displaced (along one axis only) copy of itself
    /// \brief Status : pass
    void tetraNudgedSimpler() { _testTools->intersectMeshes("UnitTetra", "NudgedSimpler", 0.152112); }

    /// Tetrahedron intersecting unit tetrahedron with in non-degenerate way around corner O
    /// \brief Status : pass
    void tetraCorner() { _testTools->intersectMeshes("UnitTetra", "CornerTetra", 0.0135435); }

    /// Tetrahedron situated totally inside another
    /// \brief Status : pass
    void tetraSimpleIncluded() { _testTools->intersectMeshes("SimpleIncludedTetra", "SimpleIncludingTetra", 17.0156); }

    /// Displaced unit tetrahedron intersecting another unit tetrahedron with which it shares an edge
    /// \brief Status : pass
    void tetraDegenEdge() { _testTools->intersectMeshes("UnitTetraDegenT", "DegenEdgeXY", 0.0); }

    /// Displaced unit tetrahedron intersecting another unit tetrahedron with which it shares a face
    /// \brief Status : pass
    void tetraDegenFace() { _testTools->intersectMeshes("UnitTetraDegenT", "DegenFaceXYZ", 0.0); }
    ...
    /// \brief Status : pass, but does not really test what it should - does not check that the intersections are
    /// detected. No longer needed.
    ...
    /// \brief Status : pass
    void generalTetra() { _testTools->intersectMeshes("GenTetra1", "GenTetra2", 4.91393); }

    /// Tetrahedron which is in a tricky position relative to unit tetrahedron.
    /// \brief Status : pass
    void trickyTetra1() { _testTools->intersectMeshes("UnitTetra", "TrickyTetra1", 0.0); }

    /// Two large tetrahedra which nearly share part of an edge and intersect at the origin. Created with goal of
    /// getting the as-of-yet uncovered "consistency" test part of the correction of double products covered. However,
    /// it does not succeed with this.
    /// \brief Status : fails, but is quite far-fetched as far as typical use cases are concerned
    void inconsistentTetra()
    {
        _testTools->intersectMeshes(
            "LargeUnitTetra.med", "LargeUnitTetra", "LargeInconsistentTetra.med", "LargeInconsistent", 7.86231e7
        );
    }
};
}  // namespace INTERP_TEST
