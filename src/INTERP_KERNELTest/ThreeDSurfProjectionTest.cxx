...
void
INTERP_TEST::ThreeDSurfProjectionTest::test1()
{
    ...
    const double coo[9] = {
        ...
        0.96809749223257568,
        0.24332379388106262,
        -0.059839592782071335,
        -0.23056279077409292,
        0.95852673990234838,
        0.16753294721527912
    };
    const double coo2[9] = {
        9.8122602102980502e-08,
        -1.4839144255482456e-7,
        9.8404874611628791e-7,
        0.96809759035517784,
        0.24332364548962007,
        -0.059838608733325221,
        -0.23056269265149082,
        0.9585265915109058,
        0.16753393126402524
    };
    ...
    ret = INTERP_KERNEL::PlanarIntersector<MyMeshType, MyMatrixType>::Projection(
        tmp0, tmp1, 3, 3, 1e-2, 1e-5 /* <- */, -1., 0.5, true
    );
    ...
    std::copy(coo2, coo2 + 9, tmp1);
    ret = INTERP_KERNEL::PlanarIntersector<MyMeshType, MyMatrixType>::Projection(
        tmp0, tmp1, 3, 3, 1e-2, 1e-8 /* <- */, -1., 0.5, true
    );
    CPPUNIT_ASSERT_EQUAL(0, ret);
    // here testing when max 3D distance is 1e-5 > 1e-6 with inverted cells
    std::copy(coo, coo + 9, tmp0);
    std::copy(coo2, coo2 + 3, tmp1 + 6);
    std::copy(coo2 + 3, coo2 + 6, tmp1 + 3);
    std::copy(coo2 + 6, coo2 + 9, tmp1);
    ret = INTERP_KERNEL::PlanarIntersector<MyMeshType, MyMatrixType>::Projection(
        tmp0, tmp1, 3, 3, 1e-2, 1e-5 /* <- */, -1., 0.5, true
    );
    CPPUNIT_ASSERT_EQUAL(-1, ret);
...
void
INTERP_TEST::ThreeDSurfProjectionTest::test2()
{  // here the two triangles have their center of inertia very close (eps) but the angle between the two planes is "big"
    ...
    const double coo2[9] = {
        7.2311562622637225e-07,
        6.8998795679738294e-07,
        3.1943866106249849e-08,
        0.72852072144314628,
        0.33125439126063028,
        0.5996079016637561,
        0.0090154262465889021,
        0.87059752249869415,
        -0.49191448334281612
    };
    double *tmp0(new double[9]), *tmp1(new double[9]);
    ...
    CPPUNIT_ASSERT_EQUAL(1, ret);
    // again max 3D distance is 1e-5 > 1e-6 but minDot set to 0.8. 0 expected. because the angle is pi/4 so cos(pi/3) >
    // 0.8
    std::copy(coo, coo + 9, tmp0);
    std::copy(coo2, coo2 + 9, tmp1);
    ret = INTERP_KERNEL::PlanarIntersector<MyMeshType, MyMatrixType>::Projection(
        tmp0, tmp1, 3, 3, 1e-2, 1e-5 /* <- */, 0.8 /* <- */, 0.5, true
    );
    CPPUNIT_ASSERT_EQUAL(0, ret);
    // again max 3D distance is 1e-5 > 1e-6 but minDot set to 0.7. 1 expected. because the angle is pi/4 so cos(pi/3) <
    // 0.49
    std::copy(coo, coo + 9, tmp0);
    std::copy(coo2, coo2 + 9, tmp1);
    ret = INTERP_KERNEL::PlanarIntersector<MyMeshType, MyMatrixType>::Projection(
        tmp0, tmp1, 3, 3, 1e-2, 1e-5 /* <- */, 0.49 /* <- */, 0.5, true
    );
    CPPUNIT_ASSERT_EQUAL(1, ret);
    // again max 3D distance is 1e-5 > 1e-6 but minDot set to 0.7. 0 expected. because the angle is pi/4 so cos(pi/3) >
    // 0.51
    std::copy(coo, coo + 9, tmp0);
    std::copy(coo2, coo2 + 9, tmp1);
    ret = INTERP_KERNEL::PlanarIntersector<MyMeshType, MyMatrixType>::Projection(
        tmp0, tmp1, 3, 3, 1e-2, 1e-5 /* <- */, 0.51 /* <- */, 0.5, true
    );
    CPPUNIT_ASSERT_EQUAL(0, ret);
    //
    delete[] tmp0;
