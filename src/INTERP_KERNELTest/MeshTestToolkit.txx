...
double
MeshTestToolkit<SPACEDIM, MESHDIM>::sumRow(const IntersectionMatrix &m, int i) const
{
    double vol = 0.0;
...
template <int SPACEDIM, int MESHDIM>
double
MeshTestToolkit<SPACEDIM, MESHDIM>::sumCol(const IntersectionMatrix &m, int i) const
{
    double vol = 0.0;
...
template <int SPACEDIM, int MESHDIM>
void
MeshTestToolkit<SPACEDIM, MESHDIM>::getVolumes(MEDCoupling::MEDCouplingUMesh &mesh, double *tab) const
{
...
template <int SPACEDIM, int MESHDIM>
double
MeshTestToolkit<SPACEDIM, MESHDIM>::sumVolume(const IntersectionMatrix &m) const
{
...
template <int SPACEDIM, int MESHDIM>
bool
MeshTestToolkit<SPACEDIM, MESHDIM>::testVolumes(
    const IntersectionMatrix &m, MEDCoupling::MEDCouplingUMesh &sMesh, MEDCoupling::MEDCouplingUMesh &tMesh
) const
{
    bool ok = true;
    ...
    for (int i = 0; i < sMesh.getNumberOfCells(); ++i)
    {
        ...
        {
            LOG(1,
                "Source volume inconsistent : vol of cell " << i << " = " << sVol[i] << " but the row sum is "
                                                            << sum_row);
            ok = false;
        }
    ...
    for (int i = 0; i < tMesh.getNumberOfCells(); ++i)
    {
        ...
        {
            LOG(1,
                "Target volume inconsistent : vol of cell " << i << " = " << tVol[i] << " but the col sum is "
                                                            << sum_col);
            ok = false;
        }
...
template <int SPACEDIM, int MESHDIM>
bool
MeshTestToolkit<SPACEDIM, MESHDIM>::areCompatitable(const IntersectionMatrix &m1, const IntersectionMatrix &m2) const
{
...
template <int SPACEDIM, int MESHDIM>
bool
MeshTestToolkit<SPACEDIM, MESHDIM>::testTranspose(const IntersectionMatrix &m1, const IntersectionMatrix &m2) const
{
    int i = 0;
    ...
    for (IntersectionMatrix::const_iterator iter = m1.begin(); iter != m1.end(); ++iter)
    {
        for (std::map<mcIdType, double>::const_iterator iter2 = iter->begin(); iter2 != iter->end(); ++iter2)
        {
            ...
            {
                LOG(2,
                    "V1( " << i << ", " << j << ") = " << v1 << " which is different from V2( " << j << ", " << i
                           << ") = " << v2 << " | diff = " << v1 - v2);
...
template <int SPACEDIM, int MESHDIM>
bool
MeshTestToolkit<SPACEDIM, MESHDIM>::testDiagonal(const IntersectionMatrix &m) const
{
...
template <int SPACEDIM, int MESHDIM>
void
MeshTestToolkit<SPACEDIM, MESHDIM>::dumpIntersectionMatrix(const IntersectionMatrix &m) const
{
    int i = 0;
...
template <int SPACEDIM, int MESHDIM>
void
MeshTestToolkit<SPACEDIM, MESHDIM>::calcIntersectionMatrix(
 ...
 * @param  doubleTest  if false, only the test with mesh 1 as the source mesh and mesh 2 as the target mesh will be
 * performed
 *
 */
template <int SPACEDIM, int MESHDIM>
void
MeshTestToolkit<SPACEDIM, MESHDIM>::intersectMeshes(
    const char *mesh1path,
    const char *mesh1,
...
void
MeshTestToolkit<SPACEDIM, MESHDIM>::intersectMeshes(
    const char *mesh1, const char *mesh2, const double correctVol, const double prec, bool doubleTest
) const
{
