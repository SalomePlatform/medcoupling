...
void
QuadraticPlanarInterpTest::IntersectArcCircleBase()
{
    ...
    for (unsigned k = 0; k < 8; k++)
    {
        ...
        center[1] = 0.;
        double center2[2];
        center2[0] = 3.8 * cos(k * M_PI / 4.);
        center2[1] = 3.8 * sin(k * M_PI / 4.);
        ...
        CPPUNIT_ASSERT(!order);
        CPPUNIT_ASSERT_EQUAL(2, (int)v4.size());
        CPPUNIT_ASSERT_EQUAL(0, (int)v3.getNumberOfAssociations());
        CPPUNIT_ASSERT_DOUBLES_EQUAL(
            e1->getRadius(), Node::distanceBtw2Pt(e1->getCenter(), (*(v4[0]))), ADMISSIBLE_ERROR
        );
        CPPUNIT_ASSERT_DOUBLES_EQUAL(
            e2->getRadius(), Node::distanceBtw2Pt(e2->getCenter(), (*(v4[0]))), ADMISSIBLE_ERROR
        );
        CPPUNIT_ASSERT_DOUBLES_EQUAL(
            e1->getRadius(), Node::distanceBtw2Pt(e1->getCenter(), (*(v4[1]))), ADMISSIBLE_ERROR
        );
        CPPUNIT_ASSERT_DOUBLES_EQUAL(
            e2->getRadius(), Node::distanceBtw2Pt(e2->getCenter(), (*(v4[1]))), ADMISSIBLE_ERROR
        );
        ...
        for (std::vector<Node *>::iterator iter = v4.begin(); iter != v4.end(); iter++) (*iter)->decrRef();
        v4.clear();
        v3.clear();
        delete intersector;
        e2->decrRef();
    ...
    for (unsigned k = 0; k < 8; k++)
    {
        ...
        center[1] = 0.;
        double center2[2];
        center2[0] = 2.8 * cos(k * M_PI / 4.);
        center2[1] = 2.8 * sin(k * M_PI / 4.);
        e1 = buildArcOfCircle(center, 3., (k - 1) * M_PI / 4., (k + 1) * M_PI / 4.);
    ...
    for (unsigned k = 0; k < 8; k++)
    {
        ...
        center[0] = 0.;
        center[1] = 0.;
        center2[0] = 4. * cos(k * M_PI / 4.);
        center2[1] = 4. * sin(k * M_PI / 4.);
        e1 = buildArcOfCircle(center, 1., (k + 1) * M_PI / 4., (k - 1) * M_PI / 4.);
        e2 = buildArcOfCircle(center2, 3., M_PI + (k - 1) * M_PI / 4., M_PI + (k + 1) * M_PI / 4.);
        intersector = new ArcCArcCIntersector(*e1, *e2);
        bool order;
        ...
        CPPUNIT_ASSERT(
            order
        );  // order has no sense here because v4.size() expected to 1 but for valgrind serenity test.
        CPPUNIT_ASSERT_EQUAL(1, (int)v4.size());
        CPPUNIT_ASSERT_EQUAL(0, (int)v3.getNumberOfAssociations());
        CPPUNIT_ASSERT_DOUBLES_EQUAL(
            e1->getRadius(), Node::distanceBtw2Pt(e1->getCenter(), (*(v4[0]))), ADMISSIBLE_ERROR
        );
    ...
    for (unsigned k = 0; k < 8; k++)
    {
        ...
        e2 = buildArcOfCircle(
            center2, 1., M_PI + k * M_PI / 4. - 0.55978664859225125, M_PI + k * M_PI / 4. + 0.55978664859225125
        );
        intersector = new ArcCArcCIntersector(*e1, *e2);
        bool order;
        bool obvious, areOverlapped;
    ...
    for (unsigned k = 0; k < 8; k++)
    {
        ...
        CPPUNIT_ASSERT(e2->getEndNode() == e1->getEndNode());
        v4.clear();
        v3.clear();
        delete intersector;
        e2->decrRef();
    ...
    for (unsigned k = 0; k < 8; k++)
    {
        ...
        CPPUNIT_ASSERT(e1->getStartNode() == e2->getStartNode());
        CPPUNIT_ASSERT(e1->getEndNode() == v4[0]);
        v4[0]->decrRef();
        v4.clear();
        v3.clear();
        delete intersector;
    ...
    for (unsigned k = 0; k < 8; k++)
    {
        ...
        CPPUNIT_ASSERT(e1->getEndNode() == v4[1]);
        v4[0]->decrRef();
        v4[1]->decrRef();
        v4.clear();
        v3.clear();
    ...
    for (unsigned k = 0; k < 8; k++)
    {
        ...
        center2[1] = 3.8 * sin(k * M_PI / 4.);
        Node *nodeS = new Node(
            center[0] + 3. * cos(k * M_PI / 4. - 0.17793931986099812),
            center[1] + 3. * sin(k * M_PI / 4. - 0.17793931986099812)
        );
        ...
        e1 = new EdgeArcCircle(
            nodeS,
            nodeE,  // Problem of precision 1e-14 to easily reached.
            center,
            3.,
            angle,
            0.17793931986099812
...
void
QuadraticPlanarInterpTest::IntersectArcCircleFull()
{
    double center1[2];
    center1[0] = 0.;
    center1[1] = 0.;
    ...
    commonNode.clear();
    QuadraticPolygon pol17;
    QuadraticPolygon pol18;
    QuadraticPolygon pol19;
    QuadraticPolygon pol20;
    pol19.pushBack(e1);
    pol20.pushBack(e2);
    ...
    QuadraticPolygon polTemp1;
    QuadraticPolygon polTemp2;
...
void
QuadraticPlanarInterpTest::IntersectArcCircleSegumentBase()
{
    double center[2] = {2., 2.};
    ...
    CPPUNIT_ASSERT_EQUAL(2, (int)v4.size());
    CPPUNIT_ASSERT_EQUAL(0, (int)v3.getNumberOfAssociations());
    CPPUNIT_ASSERT_DOUBLES_EQUAL(2., (*v4[0])[0], 1e-10);
    CPPUNIT_ASSERT_DOUBLES_EQUAL(4.3, (*v4[0])[1], 1e-10);
    CPPUNIT_ASSERT_DOUBLES_EQUAL(-0.3, (*v4[1])[0], 1e-10);
    CPPUNIT_ASSERT_DOUBLES_EQUAL(2., (*v4[1])[1], 1e-10);
    v4[0]->decrRef();
    v4[1]->decrRef();
    e2->decrRef();
    v3.clear();
...
QuadraticPolygon *
QuadraticPlanarInterpTest::buildQuadraticPolygonCoarseInfo(const double *coords, const int *conn, int lgth)
{
...
EdgeArcCircle *
QuadraticPlanarInterpTest::buildArcOfCircle(const double *center, double radius, double alphaStart, double alphaEnd)
{
    double alphaM = (alphaStart + alphaEnd) / 2;
    return new EdgeArcCircle(
        center[0] + cos(alphaStart) * radius,
        center[1] + sin(alphaStart) * radius,
        center[0] + cos(alphaM) * radius,
        center[1] + sin(alphaM) * radius,
        center[0] + cos(alphaEnd) * radius,
        center[1] + sin(alphaEnd) * radius
    );
}

double
QuadraticPlanarInterpTest::btw2NodesAndACenter(const Node &n1, const Node &n2, const double *center)
{
    const double *n1Pt = n1;
    const double *n2Pt = n2;
    double tmp1[2], tmp2[2];
    tmp1[0] = n1Pt[0] - center[0];
    tmp1[1] = n1Pt[1] - center[1];
    tmp2[0] = n2Pt[0] - center[0];
    tmp2[1] = n2Pt[1] - center[1];
